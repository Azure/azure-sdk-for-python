...
class SharedAccessSignature:
    ...
    def generate_signed_query_string(path, resource_type, shared_access_policy):
        query_string = {}
        ...
        if shared_access_policy.id:
            query_string[SIGNED_IDENTIFIER] = shared_access_policy

        query_string[SIGNED_SIGNATURE] = _generate_signature(path, resource_type, shared_access_policy)
        return query_string

    def sign_request(web_resource):
        for permission in self.permission_set:
            shared_access_signature = self.permission_set[permission]
            if self._permission_matches_request(shared_access_signature, web_resource, 
                                                web_resource.properties[REOURCE_TYPE], 
                ...
                web_resource.request_url += _convert_query_string(shared_access_signature.query_string)
                break

    def _convert_query_string(query_string):
        convert_str = ''
        ...
        convert_str += SIGNED_RESOURCE_TYPE + '=' + query_string[SIGNED_RESOURCE_TYPE] + '&'
    ...
    def _generate_signature(path, resource_type, shared_access_policy):

        def get_value_to_append(value, no_new_line):
            return_value = ''
            if value:
                return_value = value
            if not no_new_line:
        ...
        canonicalized_resource = '/' + this.account_name + path;
        string_to_sign = (get_value_to_append(shared_access_policy.access_policy.permissions) + 
    ...
    def _permission_matches_request(shared_access_signture, web_resource, resource_type, required_permission):
        required_resource_type = resource_type
        if required_resurce_type == RESOURCE_BLOB:
            required_resurce_type += RESOURCE_CONTAINER
