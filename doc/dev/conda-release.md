...
### Updating Conda Files (automated)
...
To locally generate file updates, run
...
1. **Review the PR.** Check the report in the pipeline output for any packages that couldn't be automatically updated and may need manual fixes.

2. **Handle new data plane packages** (if any are listed in the PR description):
   - Submit the [Conda placeholder form](https://forms.office.com/Pages/ResponsePage.aspx?id=v4j5cvGGr0GRqy180BHbR180k2XpSUFBtXHTh8-jMUlUNlA1MFpZOVhZME1aNU1EU1Y3SjZRU0JNRC4u) to create a private dummy library on Conda for each new package. This must be done **before** uploading.
   - Create an AKA link for the new release log at [http://aka.ms/](http://aka.ms/).
...
6. **Post-release cleanup** (for new packages only):
   - Delete the dummy placeholder libraries created in step 2.
   - Make the new packages publicly available on Conda.

## Developer Guide

For technical specifics on the Conda release infrastructure, see `conda-release-dev.md`
