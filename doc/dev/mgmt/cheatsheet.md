...
```shell
python -m packaging_tools.code_report --last-pypi $PACKAGE --output=~/old.json
python -m packaging_tools.code_report $PACKAGE --output=~/new.json
python -m packaging_tools.change_log ~/old.json ~/new.json
```
