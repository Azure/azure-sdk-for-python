...
@pytest.mark.liveTest
def test_psubscription_by_subscription_client_conn_str_receive_basic(live_servicebus_config, partitioned_topic_subscription):
    topic_name, subscription_name = partitioned_topic_subscription
    topic_client = TopicClient.from_connection_string(live_servicebus_config['conn_str'], name=topic_name, debug=True)
    with topic_client.get_sender() as sender:
        message = Message(b"Sample topic message")
        sender.send(message)
...
@pytest.mark.liveTest
def test_psubscription_by_servicebus_client_conn_str_send_basic(live_servicebus_config, partitioned_topic_subscription):
    topic_name, subscription_name = partitioned_topic_subscription
    client = ServiceBusClient(
        service_namespace=live_servicebus_config['hostname'],
        shared_access_key_name=live_servicebus_config['key_name'],
...
@pytest.mark.liveTest
def test_psubscription_by_servicebus_client_list_subscriptions(live_servicebus_config, partitioned_topic_subscription):
    topic_name, subscription_name = partitioned_topic_subscription
    client = ServiceBusClient(
        service_namespace=live_servicebus_config['hostname'],
        shared_access_key_name=live_servicebus_config['key_name'],
...
@pytest.mark.liveTest
def test_psubscription_by_subscription_client_conn_str_send_fail(live_servicebus_config, partitioned_topic_subscription):
    topic_name, subscription_name = partitioned_topic_subscription
