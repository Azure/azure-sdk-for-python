...
def stress_test_queue_receivedelete_send_receive(sb_config, queue):
    starttime = datetime.now()
    endtime = starttime + timedelta(seconds=130)
    sent_messages = 0
    received_messages = 0
...
if __name__ == '__main__':
    ...
    try:
        stress_test_queue_receivedelete_send_receive(live_config, queue_name)
    finally:
