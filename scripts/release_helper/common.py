...
class IssueProcess:
    ...
    def get_readme_from_pr_link(self, link: str) -> str:
        ...
        # Get Readme link
        pr_info = self.issue.rest_repo.get_pull(number=pr_number)
        pk_url_name = set()
    ...
    def add_label(self, label: str) -> None:
        ...
        self.issue.labels_name.add(label)

    def add_assignee(self, assignee: str) -> None:
        self.issue.issue.add_to_assignees(assignee)
        self.assignee = assignee

    def remove_assignee(self, assignee: str) -> None:
        self.issue.issue.remove_from_assignees(assignee)
        self.assignee = ''
    ...
    def auto_assign(self) -> None:
        ...
        if self.assignee != assignee:
            self.log(f'remove assignee "{self.issue.issue.assignee}" and add "{assignee}"')
            self.remove_assignee(self.issue.issue.assignee)
    ...
    def update_issue_instance(self):
        ...
        self.issue.issue = self.request_repo().get_issue(self.issue.issue.number)
...
class Common:
    ...
    def run(self):
        for item in self.issues:
            issue = IssueProcess(item, self.request_repo_dict)
            try:
                issue.run()
            except Exception as e:
                _LOG.error(f'Error happened during handling issue {item.issue.issue.number}: {e}')


def common_process(issues: List[IssuePackage]):
    instance = Common(issues)
    instance.run()
