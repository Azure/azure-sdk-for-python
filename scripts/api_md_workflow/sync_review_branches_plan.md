...
When a commit is pushed to a working branch and all required PR checks pass, discover every open API review PR associated with that working branch and trigger a sync pipeline for each matching review PR. Each sync pipeline compares the API artifacts from the fully validated working SHA with the same artifacts on the review branch, and updates the review branch only when those files differ.

The sync workflow must not blindly copy committed `api.md` or `api.metadata.yml` files from the working branch. Those files may be stale or incorrect, and the existing API.md consistency check may fail for the same commit. Review branches should only receive API artifacts after the API.md consistency workflow and the required Azure DevOps/GitHub PR checks have passed for the working branch commit being synchronized, and only when the working branch artifacts differ from the same files on the review branch.
...
Use three GitHub Actions workflows plus a small script layer under `scripts/api_md_workflow`:

1. **API.md consistency workflow.** Continues to run as an ordinary PR/push validation check and proves that the committed API artifacts on the working SHA are current for the package.
2. **Final CI gate / dispatcher workflow.** Runs when API.md consistency completes and when Azure DevOps/GitHub status or check updates arrive for a working branch commit. It treats those events as prompts to re-evaluate the PR head SHA. It only proceeds when API.md consistency and all required PR checks have passed for that SHA. For each package that passed consistency, it queries open PRs using the API review title convention, starting with `[API Review]`, that package name, and that package's working version. It then parses candidate PR metadata blocks and triggers one sync pipeline run for each package-specific review PR whose `workingOwner` and `workingBranch` match the checked working branch.
3. **Sync pipeline.** Runs once per matching review PR. It receives the working branch, review branch, package directory, and working SHA from the dispatcher. It compares `api.md` and `api.metadata.yml` at the working SHA with those same files on the review branch. If neither file differs, it exits successfully. If either file differs, it copies both API artifacts from the working SHA and commits them to the review branch.

The initial implementation should use the full-CI-gated copy strategy. The sync pipeline does not regenerate API artifacts; it relies on the successful API.md consistency result to prove the committed artifacts on the working branch are current, and on the final CI gate to preserve the existing APIView contract that new API review revisions are produced only after CI passes.
...
Review PRs are package-specific, while a single working branch consistency run may validate multiple packages. The dispatcher must therefore run discovery per package that passed consistency after the full CI gate is satisfied, not once for the whole working branch. Each package gets its own title-filtered review PR search and metadata verification pass.
...
## Final CI Gate / Dispatcher Workflow Steps

1. Ignore runs for `apireview/**` branches.
2. Run when the API.md consistency workflow completes and when Azure DevOps/GitHub status or check updates arrive for a working branch commit. These triggers should be treated as re-evaluation signals, not as sufficient proof that sync should run.
3. Resolve the working branch identity and SHA from the triggering event:
   - `workingOwner`: the owner of the head repository.
   - `workingBranch`: the head branch.
   - `workingSha`: the head SHA being evaluated.
4. Verify that `workingSha` is still the current head of `workingOwner`/`workingBranch`. If it is stale, exit successfully without triggering sync.
5. Verify that API.md consistency passed for `workingSha`. If it failed, was skipped, was cancelled, is still pending, or is missing, exit successfully without triggering sync.
6. Verify that all required PR checks have passed for `workingSha`, including the Azure DevOps checks that report back to GitHub as statuses or checks. If any required check is failing, pending, skipped unexpectedly, or missing, exit successfully without triggering sync.
7. Determine the package set that passed API.md consistency for `workingSha`. Each package record should include at least `packageName`, `packageDir`, and working version.
8. For each package, query open PRs in `Azure/azure-sdk-for-python` using the API review title convention: `[API Review]`, that package name, and that package's working version.
9. Parse each candidate PR body for one hidden `api-md-review-sync` metadata block.
10. Select every PR whose metadata satisfies all of these checks:
   - `workingOwner` and `workingBranch` match the completed working branch.
   - `packageDir` matches the package currently being processed.
   - `packageName` matches the package currently being processed.
11. For each matching PR, trigger a sync pipeline with the following data: `packageDir`, `reviewBranch`, `workingOwner`, `workingBranch`, and `workingSha`.
12. Summarize whether sync was skipped because the full CI gate was not satisfied, which review PRs triggered sync runs, or that no matching review PRs were found.
...
Use a dispatcher workflow concurrency group keyed by `workingOwner`, `workingBranch`, and `workingSha` with `cancel-in-progress: true`, so repeated status/check events for the same working branch commit collapse into one final gate evaluation while newer commits can independently supersede stale work through the head-SHA check.
...
The dispatcher workflow needs `pull-requests: read` to discover review PR metadata, `checks: read` and `statuses: read` to evaluate the full CI gate, and `actions: write` to trigger sync pipeline runs. The sync pipeline needs `contents: write` to push review branch updates.

Safety checks should include:

- Ignore consistency runs for `apireview/**` branches.
- Treat API.md consistency completion and Azure DevOps/GitHub status or check updates as dispatcher re-evaluation signals, not as sufficient sync conditions by themselves.
- Only trigger sync runs after API.md consistency and all required PR checks succeed for the same `workingSha`.
- Cancel duplicate in-flight dispatcher runs for the same `workingOwner`, `workingBranch`, and `workingSha`.
...
- Only trigger sync for review PR metadata whose `workingOwner`, `workingBranch`, `packageName`, and `packageDir` match the current package and fully validated working branch.
...
- Dispatcher exits without triggering sync when API.md consistency has not passed for the working SHA.
- Dispatcher exits without triggering sync when any required Azure DevOps or GitHub PR check has not passed for the working SHA.
- Dispatcher selects only metadata whose `workingOwner`, `workingBranch`, `packageName`, and `packageDir` match the current package and fully validated working branch.
...
5. Add the final CI gate / dispatcher workflow with `pull-requests: read`, `checks: read`, `statuses: read`, and `actions: write`.
