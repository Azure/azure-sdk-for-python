...
def find_whl(package_name, version, whl_directory):
    ...
    whl_name_format = "{0}-{1}-*.whl".format(package_name.replace("-", "_"), pkg_version)
    whls = [os.path.relpath(w, whl_dir) for w in glob.glob(os.path.join(whl_dir, "**", whl_name_format), recursive=True)]
