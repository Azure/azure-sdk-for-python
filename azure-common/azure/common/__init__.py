...
class Service(object):
    ...
    def __init__(self, credentials, **kwargs):
        ...
        self._log_name = kwargs.get('log_name', azure.common.filters.DEFAULT_LOG_NAME)
        self._log_level = kwargs.get('log_level', azure.common.filters.DEFAULT_LOG_LEVEL)
        self._session = kwargs.get('session', requests.Session())
        self._base_uri = kwargs.get('base_uri')
        self._first_filter = None

        self.add_filter(azure.common.filters.RequestFilter(self._session))
        self.add_filter(azure.common.filters.LogFilter(self._log_name, self._log_level))
        self.add_filter(azure.common.filters.SigningFilter(self._credentials))

        if self._user_agent:
            self.add_filter(azure.common.filters.UserAgentFilter(self._user_agent))
