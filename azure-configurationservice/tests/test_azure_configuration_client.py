...
class AzConfigurationClientTest(AzureMgmtTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.app_config_client = AzureConfigurationClient(connection_str)
        cls.test_data = conftest.setup_data()

    @classmethod
    def tearDownClass(cls):
        conftest.teardown_data(cls.test_data)

    def get_config_client(self):
        return self.__class__.app_config_client

    def get_test_data(self):
        return self.__class__.test_data

    def tearDown(self):
        #conftest.teardown_data(self.get_test_data())
        pass
    ...
    def test_add_configuration_setting(self):
        ...
        kv.key = "unit_test_key_add_" + self.get_test_data().key_uuid
        kv.label = self.get_test_data().label1
        kv.value = "test value"
    ...
    def test_add_existing_configuration_setting(self):
        kv = ConfigurationSetting()
        compare_kv = self.get_test_data().label1_data[0]
        ...
        with pytest.raises(ResourceExistsError):
            self.get_config_client().add_configuration_setting(kv)

    def test_add_configuration_setting_no_label(self):
        kv = ConfigurationSetting()
        kv.key = "unit_test_key_add" + self.get_test_data().key_uuid
    ...
    def test_add_existing_configuration_setting_no_label(self):
        kv = ConfigurationSetting()
        compare_kv = self.get_test_data().no_label_data[0]
        kv.key = compare_kv.key
        kv.value = compare_kv.value
        kv.content_type = compare_kv.content_type
    ...
    def test_set_existing_configuration_setting_no_label_etag(self):
        sample_kv = copy(self.get_test_data().no_label_data[-1])

        # create a new key value into AzConfig service
        sample_kv.key = "unit_test_key_set_" + self.get_test_data().key_uuid
        to_set_kv = self.get_config_client().add_configuration_setting(sample_kv)
        ...
        self.get_config_client().delete_configuration_setting(set_kv.key)
        assert (
    ...
    def test_set_existing_configuration_setting_label_etag(self):
        sample_kv = copy(self.get_test_data().label1_data[-1])

        # create a new key value into AzConfig service
        sample_kv.key = "unit_test_key_set" + self.get_test_data().key_uuid
        to_set_kv = self.get_config_client().add_configuration_setting(sample_kv)
        to_set_kv.value = to_set_kv.value + "a"
        to_set_kv.tags = {"a": "b", "c": "d"}
        set_kv = self.get_config_client().set_configuration_setting(to_set_kv)
    ...
    def test_set_configuration_setting_no_label_etag(self):
        to_set_kv = copy(self.get_test_data().no_label_data[-1])
        to_set_kv.key = "unit_test_key_set" + self.get_test_data().key_uuid
        with pytest.raises(ResourceModifiedError):
            self.get_config_client().set_configuration_setting(to_set_kv)

    def test_set_configuration_setting_label_etag(self):
        to_set_kv = copy(self.get_test_data().label1_data[-1])
        to_set_kv.key = "unit_test_key_set" + self.get_test_data().key_uuid
        with pytest.raises(ResourceModifiedError):
            self.get_config_client().set_configuration_setting(to_set_kv)
    ...
    def test_update_existing_configuration_setting_no_label_etag(self):
        ...
        # create a new key value into AzConfig service
        sample_kv.key = "unit_test_key_" + self.get_test_data().key_uuid
        to_update_kv = self.get_config_client().add_configuration_setting(sample_kv)
        tags = {"a": "b", "c": "d"}
        ...
        # remove the new key value from the service
        self.get_config_client().delete_configuration_setting(updated_kv.key)
        assert (
    ...
    def test_update_no_existing_configuration_setting_label_noetag(self):
        key = self.get_test_data().key_uuid
        label = "test_label1"
    ...
    def test_lock_unlock_no_label(self):
        ...
        to_lock_kv = self.get_test_data().no_label_data[0]
    ...
    def test_lock_unlock_with_label(self):
        # lock, assert locked, then unlock and assert locked
        to_lock_kv = self.get_test_data().label1_data[0]
    ...
    def test_delete_with_key_no_label(self):
        to_delete_kv = copy(self.get_test_data().no_label_data[-1])
        to_delete_kv.key = "unit_test_key_" + self.get_test_data().key_uuid
        to_delete_kv = self.get_config_client().add_configuration_setting(to_delete_kv)
    ...
    def test_delete_with_key_label(self):
        to_delete_kv = copy(self.get_test_data().label1_data[-1])
        to_delete_kv.key = "unit_test_key_" + self.get_test_data().key_uuid
        to_delete_kv = self.get_config_client().add_configuration_setting(to_delete_kv)
        self.get_config_client().delete_configuration_setting(
    ...
    def test_delete_non_existing(self):
        deleted_kv = self.get_config_client().delete_configuration_setting(
            "not_exist_" + self.get_test_data().key_uuid
        )
    ...
    def test_list_configuration_settings_key_label(self):
        to_list1 = self.get_test_data().label1_data[0]
        to_list2 = self.get_test_data().label2_data[0]
    ...
    def test_list_configuration_settings_only_label(self):
        items = self.get_config_client().list_configuration_settings(
            labels=[self.get_test_data().label1]
        )
        cnt = 0
        for kv in items:
            assert kv.label == self.get_test_data().label1
            cnt += 1
        assert cnt == 1

    def test_list_configuration_settings_only_key(self):
        to_list1 = self.get_test_data().no_label_data[0]
        items = self.get_config_client().list_configuration_settings(keys=[to_list1.key])
        cnt = 0
        for kv in items:
    ...
    def test_list_configuration_settings_fields(self):
        items = self.get_config_client().list_configuration_settings(
            keys=["*"], labels=[self.get_test_data().label1], fields=["key", "content_type"]
        )
        cnt = 0
        for kv in items:
            assert (
    ...
    def test_list_configuration_settings_reserved_chars(self):
        items = self.get_config_client().list_configuration_settings(
            labels=[self.get_test_data().label2]
        )
        cnt = 0
        for kv in items:
            assert kv.label == self.get_test_data().label2
            cnt += 1
        assert cnt == 1
    ...
    def test_list_configuration_settings_correct_etag(self):
        to_list_kv = self.get_test_data().label1_data[0]
        ...
        delete_me = [
            self.get_config_client().add_configuration_setting(
                ConfigurationSetting(
                    key="multi_" + str(i) + self.get_test_data().key_uuid,
    ...
    def test_list_revisions_only_label(self):
        items = self.get_config_client().list_revisions(labels=[self.get_test_data().label1])
        cnt = 0
        for kv in items:
            assert kv.label == self.get_test_data().label1
            cnt += 1
    ...
    def test_list_revisions_key_no_label(self):
        to_list1 = self.get_test_data().no_label_data[0]
        items = self.get_config_client().list_revisions(keys=[to_list1.key])
        cnt = 0
        for kv in items:
            assert kv.key == to_list1.key
            cnt += 1
