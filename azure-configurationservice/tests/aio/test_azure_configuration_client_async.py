...
class AzConfigurationClientAsyncTest(AzureMgmtTestCase):
    def setUp(self):
    ...
    def test_add_configuration_setting(self):
        ...
        created_kv = asyncio.get_event_loop().run_until_complete(
    ...
    def test_set_existing_configuration_setting_no_label_etag(self):
        ...
        to_set_kv = asyncio.get_event_loop().run_until_complete(
            self.get_config_client().add_configuration_setting(sample_kv)
        )
        to_set_kv.value = to_set_kv.value + "a"
        to_set_kv.tags = {"a": "b", "c": "d"}
        set_kv = asyncio.get_event_loop().run_until_complete(
    ...
    def test_update_existing_configuration_setting_no_label_etag(self):
        ...
        to_update_kv = asyncio.get_event_loop().run_until_complete(
            self.get_config_client().add_configuration_setting(sample_kv)
        )
        tags = {"a": "b", "c": "d"}
        updated_kv = asyncio.get_event_loop().run_until_complete(
    ...
    def test_get_configuration_setting_no_label(self):
        compare_kv = self.get_test_data().no_label_data[0]
        fetched_kv = asyncio.get_event_loop().run_until_complete(
            self.get_config_client().get_configuration_setting(compare_kv.key)
        )
        assert (
    ...
    def test_lock_unlock_no_label(self):
        ...
        locked_kv = asyncio.get_event_loop().run_until_complete(
            self.get_config_client().lock_configuration_setting(to_lock_kv.key)
        )
        assert locked_kv.locked is True
        unlocked_kv = asyncio.get_event_loop().run_until_complete(
            self.get_config_client().unlock_configuration_setting(to_lock_kv.key)
        )
    ...
    def test_delete_with_key_no_label(self):
        ...
        to_delete_kv = asyncio.get_event_loop().run_until_complete(
            self.get_config_client().add_configuration_setting(to_delete_kv)
        )
        asyncio.get_event_loop().run_until_complete(
    ...
    def test_delete_non_existing(self):
        deleted_kv = asyncio.get_event_loop().run_until_complete(
            self.get_config_client().delete_configuration_setting(
    ...
    def test_list_configuration_settings_key_label(self):
        ...
        items = AzConfigurationClientAsyncTest._to_list(items)
        cnt = 0
        for kv in items:
    ...
    def test_list_configuration_settings_multi_pages(self):
        ...
        items = AzConfigurationClientAsyncTest._to_list(items)
        assert len(items) > conftest.PAGE_SIZE
    ...
    def test_list_configuration_settings_only_accept_time(self):
        ...
        assert len(AzConfigurationClientAsyncTest._to_list(all_inclusive)) > len(
            AzConfigurationClientAsyncTest._to_list(exclude_today)
        )
