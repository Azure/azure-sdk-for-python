...
from ci_tools.scenario import install_into_venv
...
def create_venv(venv_path: str) -> None:
    venv_creation_command = get_venv_call()
    check_call(venv_creation_command + [venv_path])

    # TODO: go to the prebuilt wheel dir if that exists and retrieve azure-sdk-tools from THAT if it exists
    # this is ok for now though.
    install_into_venv(venv_path, os.path.join(root_dir, "eng/tools/azure-sdk-tools"), False, "build")
...
"""
    ...
    for package in targeted_packages:
        for check_name in args.checks_list.split(","):
            pass
