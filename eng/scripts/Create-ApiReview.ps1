...
function Submit-APIReview($packageArtifactname, $apiLabel, $releaseStatus)
{
...
if ($packages)
{
    foreach($pkgPath in $packages.Values)
    {
        ...
        if ( ($SourceBranch -eq $DefaultBranch) -or (-not $version.IsPrerelease))
        {
            ...
            $respCode = Submit-APIReview -packageArtifactname $pkg -apiLabel $($pkgInfo.Version) -releaseStatus $pkgInfo.ReleaseStatus 
