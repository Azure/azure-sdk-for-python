...
process {
    foreach ($p in $Path) {
        ...
        if ($file.Extension -ne '.md') {
            Write-Verbose "'$($file.FullName)' does not appear to be a valid markdown file"
            continue
        }

        [string[]] $content = $file | Get-Content
        if (!$content[0].StartsWith('---')) {
            Write-Verbose "No markdown frontmatter found in '$($file.FullName)'"
            continue
        }
        ...
        do {
            ...
            elseif ($found -and $line -match '^\s*-\s+([\w-]+)') {
                $products += $matches[1]
            }
        ...
        foreach ($product in $products) {
            if ($slugs -notcontains $product) {
                $invalidProducts += $product
                Write-Error "File '$($file.FullName)' contains invalid slug: $product"
            }
        }

        if ($invalidProducts) {
            $file | Add-Member -Type NoteProperty -Name 'InvalidSlugs' -Value $invalidProducts -PassThru
        }
    }
}
