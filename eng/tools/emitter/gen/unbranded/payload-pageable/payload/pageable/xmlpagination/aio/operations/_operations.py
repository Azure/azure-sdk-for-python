...
class XmlPaginationOperations:
    ...
    def list_with_continuation(self, **kwargs: Any) -> AsyncItemPaged["_models3.XmlPet"]:
        """list_with_continuation.

        :return: An iterator like instance of XmlPet
        :rtype: ~corehttp.paging.AsyncItemPaged[~payload.pageable.models.XmlPet]
        :raises ~corehttp.exceptions.HttpResponseError:
        """
        _headers = kwargs.pop("headers", {}) or {}
        _params = kwargs.pop("params", {}) or {}

        cls: ClsType[list[_models3.XmlPet]] = kwargs.pop("cls", None)
        ...
        async def extract_data(pipeline_response):
            ...
            list_of_elem = _deserialize(
                list[_models3.XmlPet],
                deserialized.find("Pets"),
            )
            if cls:
                list_of_elem = cls(list_of_elem)  # type: ignore
            _cont_token_elem = deserialized.find("NextMarker")
            return _cont_token_elem.text if _cont_token_elem is not None else None, AsyncList(list_of_elem)
    ...
    def list_with_next_link(self, **kwargs: Any) -> AsyncItemPaged["_models3.XmlPet"]:
        """list_with_next_link.
