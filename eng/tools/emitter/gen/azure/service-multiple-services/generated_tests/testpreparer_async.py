...
class ServiceAClientTestBaseAsync(AzureRecordedTestCase):
    ...
    def create_async_client(self, endpoint):
        credential = self.get_credential(ServiceAClient, is_async=True)
...
class ServiceBClientTestBaseAsync(AzureRecordedTestCase):

    def create_async_client(self, endpoint):
        credential = self.get_credential(ServiceBClient, is_async=True)
        return self.create_client_from_credential(
            ServiceBClient,
            credential=credential,
            endpoint=endpoint,
