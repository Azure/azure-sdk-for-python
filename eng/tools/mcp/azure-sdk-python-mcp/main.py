...
def verify_setup_tool(command_path: str, tox_ini_path: str) -> Dict[str, Any]:
    ...
    def verify_installation(command: List[str], name: str, install_suggestion: str = "") -> Dict[str, Any]:
        ...
        if not result["success"]:
            return {
                "success": False,
                "message": f"{name} is not installed or not available in PATH.{install_suggestion}",
    ...
    results = {
        ...
        "tsp-client": verify_installation(
            ...
            " Please install it using 'npm install -g @azure-tools/typespec-client-generator-cli'.",
        ),
    }
