...
def create_package(
    ...
    # `ext_modules` only covers setuptools `Extension` objects. Packages built by other
    # backends (maturin/PyO3) compile native code but declare none, so honor an explicit
    # [tool.cibuildwheel] table as well -- otherwise they are misrouted to `python -m build`,
    # which yields an untagged wheel built against whatever toolchain the agent happens to have.
