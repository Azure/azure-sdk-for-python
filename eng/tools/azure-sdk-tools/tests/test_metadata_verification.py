...
def test_verify_invalid_metadata_fails_with_missing_keys(package_type, scenario_name, scenario_path, caplog):
    ...
    try:
        ...
        assert result is False, f"verify_{package_type} should return False for invalid {scenario_name} metadata scenario with missing keys"
        ...
        assert has_missing_keys_error, f"Expected error log about missing keys '{expected_missing_keys[0]}' and '{expected_missing_keys[1]}' for {scenario_name} scenario, but got: {error_logs}"
