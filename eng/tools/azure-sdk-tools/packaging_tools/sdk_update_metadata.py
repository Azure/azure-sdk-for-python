...
def main(package_path: Path, *, pipeline_input: Optional[dict] = None, codegen_config: Optional[dict] = None, spec_folder: Optional[Path] = None, readme_or_tsp: Optional[str] = None):
    ...
    try:
            ...
            if Path("eng/emitter-package.json").exists():
                ...
                with open("eng/emitter-package.json", "r") as f:
                    ...
                    if emitter_data.get("dependencies", {}).get("@azure-tools/typespec-python"):
                        ...
                        _LOGGER.warning("'@azure-tools/typespec-python' not found in emitter-package.json dependencies.")
        ...
        if pipeline_input and codegen_config and readme_or_tsp:
            ...
            _LOGGER.error(f" Skip metadata update for missing information: pipeline input {pipeline_input}, codegen_config {codegen_config}, readme_or_tsp {readme_or_tsp}.")
    ...
    try:
        ...
        _LOGGER.warning(f"Fail to generate packaging files and ci.yml under {package_path} for {readme_or_tsp}: {str(e)}")
