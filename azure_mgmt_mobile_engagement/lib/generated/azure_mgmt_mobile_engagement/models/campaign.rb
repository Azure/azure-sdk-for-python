...
module Azure::ARM::MobileEngagement
  module Models
    ...
    class Campaign < CampaignLocalization
      ...
      # @return [CampaignAudience] Specify which users will be targeted by
      ...
      # the audience is the push quota filter. An audience is a boolean
      # expression made of criteria (variables) operators (`not`, `and` or
      # `or`) and parenthesis. Additionally, a set of filters can be added to
      # an audience. 65535 bytes max as per JSON encoding.
      ...
      # is pushed. Valid values are: * `real-time`: Never ending campaign,
      # the campaign will be delivered  to your existing users and also to
      # your new users. * `one-shot`: In this mode, the campaign will be
      # delivered only to your existing users (campaign will stop after
      # that). * `manual`: In this mode, the campaign will not be pushed
      # automatically to devices. You will have to use the Push campaign
      ...
      # text. * `text/html`: HTML announcement: `body` attribute can contain
      # HTML code. * `only_notif`: Notification-only announcement. With this
      # kind of announcements, the `body`, `title`, `actionButtonText` and
      ...
      # campaign should be delivered. Valid values are: * `any`: Campaign
      # will be delivered as soon as possible. * `background`: iOS only.
      # Campaign will be only delivered when the application is in background
      # (out of app). * `session`: Campaign will be delivered when the
      # application is running.
      # . Possible values include: 'any', 'background', 'session'
      attr_accessor :delivery_time

      # @return [Array<String>] Announcements/polls only. Array containing the
      # list of activities in which the campaign can be delivered.
      # deliveryTime must be set to session. If the platform is iOS, this
      # option can also be set if deliveryTime is set to any. In that case,
      # if the campaign is received when the application is launched, it will
      # be delivered only in the specified list of activities.
      ...
      # If you set pushMode property to manual, this attribute will be
      # ignored. * If you set pushMode property to one-shot, then the
      # timezone attribute must be specified. Example: `2011-11-21 15:23Z`
      attr_accessor :start_time

      # @return [String] The date at which the campaign should be finished.
      # The date shall conform to the following format:
      ...
      # want the system notification to make a vibration. The
      ...
      # property must be set to `system`. * `iOS`: A flag indicating whether
      # or not you want the native Apple Push notification to make a sound.
      # The `deliveryTime` property must be set to `any` or `background`.
      # This will play the 'default' sound. If you want to play a custom
      # sound, see the `notificationOptions` property. * `Windows`: A flag
