...
class SharedAccessSignature:
    ...
    def generate_signed_query_string(self, path, resource_type, shared_access_policy, version=X_MS_VERSION):
        ''' 
    ...
    def _convert_query_string(self, query_string):
        ''' Converts query string to str. The order of name, values is very important and can't be wrong.'''

        convert_str = ''
        if SIGNED_START in query_string:
            convert_str += SIGNED_START + '=' + url_quote(query_string[SIGNED_START]) + '&'
        convert_str += SIGNED_EXPIRY + '=' + url_quote(query_string[SIGNED_EXPIRY]) + '&'
    ...
    def _generate_signature(self, path, resource_type, shared_access_policy, version):
