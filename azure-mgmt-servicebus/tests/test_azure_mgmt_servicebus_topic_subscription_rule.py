...
from azure.mgmt.servicebus.models import SBNamespace,SBSku,SkuName,SBTopic,SBSubscription,Rule,Action,SqlFilter,FilterType,CorrelationFilter
...
class MgmtServiceBusTest(AzureMgmtTestCase):
    ...
    def test_sb_rule_curd(self, resource_group, location):
        ...
        createruleresponse = self.servicebus_client.rules.create_or_update(resource_group_name, namespace_name,topic_name, subscription_name, rule_name,Rule(Action(requires_preprocessing=True,sql_expression=strSqlExp),FilterType.sql_filter,SqlFilter(sql_expression=strSqlExp)))
        self.assertEqual(createruleresponse.name, rule_name)
