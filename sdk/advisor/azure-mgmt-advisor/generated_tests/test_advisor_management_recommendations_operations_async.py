...
class TestAdvisorManagementRecommendationsOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_recommendations_update(self, resource_group):
        response = await self.client.recommendations.update(
            recommendation_id="str",
            properties={
                "properties": {
                    "postponedUntilDateTime": "2020-02-20 00:00:00",
                    "recommendationDismissReason": "str",
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_recommendations_list_by_tenant(self, resource_group):
        response = self.client.recommendations.list_by_tenant(
            resource_uri="str",
        )
        result = [r async for r in response]
        # please add some check logic here by yourself
