...
class GeneratedClientConfiguration(Configuration):
    ...
    def __init__(
        ...
        super(GeneratedClientConfiguration, self).__init__(base_url)

        self.add_user_agent('generatedclient/{}'.format(VERSION))


class GeneratedClient(SDKClient):
    """EventGrid Python Client

    :ivar config: Configuration for client.
    :vartype config: GeneratedClientConfiguration
    ...
    def __init__(
        ...
        self.config = GeneratedClientConfiguration()
        super(GeneratedClient, self).__init__(None, self.config)
    ...
    def publish_cloud_event_events(
        ...
        # Construct URL
        url = self.publish_cloud_event_events.metadata['url']
        path_format_arguments = {
            'topicHostname': self._serialize.url("topic_hostname", topic_hostname, 'str', skip_quote=True)
        }
        url = self._client.format_url(url, **path_format_arguments)
        ...
        # Construct headers
        header_parameters = {}
        header_parameters['Content-Type'] = 'application/cloudevents-batch+json; charset=utf-8'
        if custom_headers:
            header_parameters.update(custom_headers)

        # Construct body
        body_content = self._serialize.body(events, '[CloudEventEvent]')
        ...
        if raw:
            client_raw_response = ClientRawResponse(None, response)
            return client_raw_response
    publish_cloud_event_events.metadata = {'url': '/api/events'}
