...
class TestEGClientExceptions(AzureRecordedTestCase):
    ...
    def test_publish_binary_mode_xml(
        ...
        xml_string = (
            """<?xml version="1.0" encoding="UTF-8"?><Data><test>test</test></Data>"""
        )
        ...
        client.publish_cloud_events(eventgrid_topic_name, body=event, binary_mode=True)

        time.sleep(5)

        events = client.receive_cloud_events(
            eventgrid_topic_name, eventgrid_event_subscription_name, max_events=1
        )
        ...
        rejected_result = client.reject_cloud_events(
            eventgrid_topic_name,
            eventgrid_event_subscription_name,
            reject_options=RejectOptions(lock_tokens=tokens),
        )
    ...
    def test_publish_receive_cloud_event(
        ...
        client.publish_cloud_events(eventgrid_topic_name, body=[event])
        ...
        ack = client.acknowledge_cloud_events(
            eventgrid_topic_name,
            eventgrid_event_subscription_name,
            lock_tokens=AcknowledgeOptions(lock_tokens=[lock_token]),
        )
