...
class EventGridSerializationTests:
    ...
    def test_cloud_event_serialization_extension_bytes(self, **kwargs):
        ...
        cloud_event = CloudEvent(
            source="http://samplesource.dev", data=data, type="Sample.Cloud.Event", extensions={"e1": 1, "e2": 2}
        )
    ...
    def test_event_grid_event_raises_on_no_data(self):
        with pytest.raises(TypeError):
            eg_event = EventGridEvent(subject="sample", event_type="Sample.EventGrid.Event", data_version="2.0")
    ...
    def test_eg_event_repr(self):
        event = EventGridEvent(
            subject="sample2", data="eventgridevent2", event_type="Sample.EventGrid.Event", data_version="2.0"
        )
