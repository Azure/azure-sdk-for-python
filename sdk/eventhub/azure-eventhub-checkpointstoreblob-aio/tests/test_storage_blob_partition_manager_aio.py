...
class StorageCheckpointStoreAsyncTests(object):
    ...
    async def _claim_and_list_ownership(self, storage_account, container_name):
        fully_qualified_namespace = 'test_namespace'
        eventhub_name = 'eventhub'
        consumer_group = '$default'
        ownership_cnt = 8

        credential = self.get_credential(BlobCheckpointStore, is_async=True)
    ...
    async def _update_checkpoint(self, storage_account, container_name):
        fully_qualified_namespace = 'test_namespace'
        eventhub_name = 'eventhub'
        consumer_group = '$default'
        partition_cnt = 8
    ...
    @pytest.mark.asyncio
    async def test_claim_and_list_ownership_async(self, storage_account):
        storage_account, container_name = self.get_live_storage_blob_client(storage_account)
        await self._claim_and_list_ownership(storage_account, container_name)


    @pytest.mark.parametrize("storage_account", STORAGE_ENV_KEYS)
    @pytest.mark.live_test_only
    @pytest.mark.asyncio
    async def test_update_checkpoint_async(self, storage_account):
        storage_account, container_name = self.get_live_storage_blob_client(storage_account)
        await self._update_checkpoint(storage_account, container_name)
