...
def parse_starting_position(args):
    ...
    if args.starting_datetime:
        starting_position = datetime.datetime.strptime(
            str(args.starting_datetime), "%Y-%m-%d %H:%M:%S"
        )
...
parser.add_argument(
    "--link_credit", default=int(os.environ.get("LINK_CREDIT", 3000)), type=int
)
parser.add_argument(
    "--output_interval",
    type=float,
    default=int(os.environ.get("OUTPUT_INTERVAL", 5000)),
)
parser.add_argument(
    "--duration",
...
parser.add_argument(
    ...
    default=os.environ.get("CONSUMER_GROUP", "$default"),
)
parser.add_argument(
    "--auth_timeout", help="Authorization Timeout", type=float, default=60
)
parser.add_argument(
    "--starting_offset",
    help="Starting offset",
    type=str,
    default=os.environ.get("STARTING_OFFSET", "-1"),
)
parser.add_argument(
    "--starting_sequence_number", help="Starting sequence number", type=int
)
parser.add_argument(
    "--starting_datetime",
    help="Starting datetime string, should be format of YYYY-mm-dd HH:mm:ss",
)
parser.add_argument(
    "--partitions",
...
parser.add_argument(
    "--owner_level",
    help="The owner level, or epoch, of the consumer",
    type=int,
    default=None,
)
parser.add_argument(
    "--recv_partition_id",
    help="Receive from a specific partition if this is set",
    type=int,
)
parser.add_argument(
    "--max_batch_size",
...
parser.add_argument(
    ...
    help="time duration in seconds between two load balance",
    type=float,
    default=10,
)
parser.add_argument(
...
parser.add_argument(
    "--sas_policy", help="Name of the shared access policy to authenticate with"
)
...
parser.add_argument(
    "--parallel_recv_cnt",
    help="Number of receive clients doing parallel receiving",
    type=int,
    default=1,
)
...
parser.add_argument(
    "--pyamqp_logging_enable", help="pyamqp logging enable", action="store_true"
)
...
parser.add_argument(
    "--uamqp_mode", help="Flag for uamqp or pyamqp", action="store_true"
)
parser.add_argument(
    "--debug_level",
...
async def on_event_received(process_monitor, partition_context, event):
    ...
    if recv_cnt_map[partition_context.partition_id] % LOG_PER_COUNT == 0:
        ...
        LOGGER.info(
            ...
            (
                LOG_PER_COUNT / (partition_current_time - partition_previous_time)
...
async def on_error(partition_context, exception):
    azure_metric_monitor.record_error(
        exception, extra="partition: {}".format(partition_context.partition_id)
    )
...
async def run(args):
    ...
    with ProcessMonitor(
        ...
        on_event_received_with_process_monitor = partial(
            on_event_received, process_monitor
        )
        on_event_batch_received_with_process_monitor = partial(
            on_event_batch_received, process_monitor
        )
            ...
            tasks = [
                asyncio.ensure_future(
                    clients[i].receive_batch(
                        on_event_batch_received_with_process_monitor, **kwargs_dict
                    )
                    if args.max_batch_size
                    else clients[i].receive(
                        on_event_received_with_process_monitor, **kwargs_dict
                    )
                )
        ...
        await asyncio.gather(
            *[clients[i].close() for i in range(args.parallel_recv_cnt)]
        )
