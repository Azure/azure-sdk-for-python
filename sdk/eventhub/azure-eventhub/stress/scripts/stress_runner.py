...
def create_stress_eventhub(stress_config, eventhub_name):
    ...
    if stress_config["CREDENTIALS"]["EVENT_HUB_CONN_STR"]:
        _, namespace, shared_access_key_name, shared_access_key, _ = (
            parse_eventhub_conn_str(stress_config["CREDENTIALS"]["EVENT_HUB_CONN_STR"])
        )
...
def create_resource_for_task_if_needed(stress_config, task_name):
    if "large_partitions" in task_name or (
        not stress_config["BASIC_CONFIG"].getboolean("create_new_eventhub")
    ):
...
def generate_base_send_command(task_name):
    ...
    partial_command += " -m {}".format(
        "stress_send_async" if "async" in task_name else "stress_send_sync"
    )
...
def generate_command_send_config(send_config, proxy_config):
    ...
    if send_config["parallel_send_cnt"]:
        partial_command += " --parallel_send_cnt {}".format(
            send_config["parallel_send_cnt"]
        )
        if send_config["parallel_create_new_client"] and send_config.getboolean(
    ...
    if send_config["retry_backoff_factor"]:
        partial_command += " --retry_backoff_factor {}".format(
            send_config["retry_backoff_factor"]
        )
    if send_config["retry_backoff_max"]:
        partial_command += " --retry_backoff_max {}".format(
            send_config["retry_backoff_max"]
        )
...
def generate_command_receive_config(receive_config, proxy_config):
    ...
    if receive_config["consumer_group"]:
        partial_command += " --consumer_group {}".format(
            receive_config["consumer_group"]
        )
    if receive_config["starting_offset"]:
        partial_command += " --starting_offset {}".format(
            receive_config["starting_offset"]
        )
    if receive_config["starting_sequence_number"]:
        partial_command += " --starting_sequence_number {}".format(
            receive_config["starting_sequence_number"]
        )
    if receive_config["starting_datetime"]:
        partial_command += ' --starting_datetime "{}"'.format(
            receive_config["starting_datetime"]
        )
    partial_command += " --partitions {}".format(receive_config["partitions"])
    partial_command += " --load_balancing_interval {}".format(
        receive_config["load_balancing_interval"]
    )
    ...
    if receive_config["parallel_recv_cnt"]:
        partial_command += " --parallel_recv_cnt".format(
            receive_config["parallel_recv_cnt"]
        )
    if receive_config["recv_partition_id"]:
        partial_command += " --recv_partition_id".format(
            receive_config["recv_partition_id"]
        )
...
def generate_command_resource_config(credential_config, eventhub_name, blob_name=None):
    ...
    if credential_config["EVENT_HUB_CONN_STR"]:
        partial_command += ' --conn_str "{}"'.format(
            credential_config["EVENT_HUB_CONN_STR"]
        )
    if credential_config["EVENT_HUB_HOSTNAME"]:
        partial_command += "--hostname {}".format(
            credential_config["EVENT_HUB_HOSTNAME"]
        )
    if eventhub_name or credential_config["EVENT_HUB_NAME"]:
        partial_command += " --eventhub {}".format(
            eventhub_name or credential_config["EVENT_HUB_NAME"]
        )
    if credential_config["EVENT_HUB_SAS_POLICY"]:
        partial_command += " --sas-policy {}".format(
            credential_config["EVENT_HUB_SAS_POLICY"]
        )
    if credential_config["EVENT_HUB_SAS_KEY"]:
        partial_command += " --sas-key {}".format(
            credential_config["EVENT_HUB_SAS_KEY"]
        )
    if credential_config["AZURE_CLIENT_SECRET"]:
        partial_command += " --aad_secret {}".format(
            credential_config["AZURE_CLIENT_SECRET"]
        )
    if credential_config["AZURE_CLIENT_ID"]:
        partial_command += " --aad_client_id {}".format(
            credential_config["AZURE_CLIENT_ID"]
        )
    if credential_config["AZURE_TENANT_ID"]:
        partial_command += " --aad_tenant_id {}".format(
            credential_config["AZURE_TENANT_ID"]
        )

    # Blob Configuration
    if blob_name:
        if credential_config["AZURE_STORAGE_CONN_STR"]:
            partial_command += ' --storage_conn_str "{}"'.format(
                credential_config["AZURE_STORAGE_CONN_STR"]
            )
...
def generate_running_command(task_name, stress_config, eventhub_name, blob_name):
    ...
    if "send" in task_name:
        ...
        send_command += generate_command_resource_config(
            stress_config["CREDENTIALS"], eventhub_name, None
        )
        send_command += generate_command_basic_config(stress_config["BASIC_CONFIG"])
        send_command += generate_command_send_config(
            stress_config["EVENTHUB_SEND_CONFIG"], stress_config["PROXY"]
        )
    ...
    if "receive" in task_name:
        ...
        receive_command += generate_command_resource_config(
            stress_config["CREDENTIALS"], eventhub_name, blob_name
        )
...
if __name__ == "__main__":
    ...
    target_stress_method = [
        m for m in config["RUN_METHODS"] if config["RUN_METHODS"].getboolean(m)
    ]
    ...
    resources = [
        create_resource_for_task_if_needed(config, task)
        for task in target_stress_method
    ]
    ...
    for i in range(len(target_stress_method)):
        python_commands.extend(
            generate_running_command(
                target_stress_method[i], config, resources[i][0], resources[i][1]
            )
        )
