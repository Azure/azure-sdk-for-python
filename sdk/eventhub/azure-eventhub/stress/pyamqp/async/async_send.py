...
async def send_batch_message_for_fixed_amount(conn_str, eventhub_name, num_of_events, single_event_size, run_times=1, description=None):
...
async def send_batch_message_worker_task(client, data, run_flag):
    total_cnt = 0
    while run_flag[0]:
        batch = await client.create_batch()
        try:
...
async def send_batch_message_in_parallel_fox_fixed_period(conn_str, eventhub_name, single_event_size, parallel_coroutine_count=4, run_times=1, run_duration=30, description=None):
    ...
    for _ in range(run_times):
        ...
        for i in range(parallel_coroutine_count):
            futures.append(
                asyncio.create_task(
                    send_batch_message_worker_task(clients[i], data, run_flag)
                )
            )
        ...
        perf_records.append(sum(results) / run_duration)
    ...
    logger.info(
        "Method: {}, The average performance is {} events/s, throughput: {} bytes/s, run times: {}.\n"
        "Configs are: Single message size: {} bytes, Parallel count: {} coroutines, Run duration: {} seconds.".format(
...
if __name__ == '__main__':
    ...
    for conn_str in CONN_STRS:
        for single_event_size in SINGLE_EVENT_SIZE_LIST:
            ...
            asyncio.run(
                send_batch_message_for_fixed_amount(
                    conn_str=conn_str,
        ...
        for parallel_coroutine_count in PARALLEL_COROUTINE_COUNT_LIST:
            for single_event_size in SINGLE_EVENT_SIZE_LIST:
                ...
                asyncio.run(
                    send_batch_message_in_parallel_fox_fixed_period(
                        conn_str=conn_str,
                        eventhub_name=EVENTHUB_NAME,
                        single_event_size=single_event_size,
                        parallel_coroutine_count=parallel_coroutine_count,
                        description='multiple coroutines sending messages'
                    )
                )
