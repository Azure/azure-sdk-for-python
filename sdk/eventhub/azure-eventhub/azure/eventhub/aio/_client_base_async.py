...
class ClientBaseAsync(ClientBase):
    ...
    async def _get_partition_properties_async(
        ...
        if partition_info:
            ...
            output["beginning_replication_segment"] = cast(
                int, partition_info[b"begin_sequence_number_epoch" ]
            )
            output["last_enqueued_sequence_number"] = cast(
                int, partition_info[b"last_enqueued_sequence_number"]
            )
            output["last_enqueued_replication_segment"] = cast(
                int, partition_info[b"last_enqueued_sequence_number_epoch"]
            )
