            ...
            for p in ownership_list:
                ...
                if not cursor_fetch:
                    ...
                    try:
                        ...
                        cursor.execute(
                            sql, tuple(p.get(field) for field in self.ownership_fields)
                        )
                    ...
                    if p.get("etag") == cursor_fetch[0][0]:
                        ...
                        sql = (
                            ...
                            + " set "
                            + ",".join(
                                [field + "=?" for field in self.ownership_data_fields]
                            )
                            + " where "
                            + " and ".join(
                        ...
                        cursor.execute(
                            sql,
                            tuple(p.get(field) for field in self.ownership_data_fields)
                            + tuple(p.get(field) for field in self.primary_keys),
                        )
                        result.append(p)
                    else:
