...
class EventHubProducer(
    ...
    def __init__(self, client, target, **kwargs):
        ...
        if self._client._is_uamqp:
            ...
            self._send_event_data = self._send_event_data_uamqp
            self.send = self.send_uamqp
        else:
            ...
            self._send_event_data = self._send_event_data_pyamqp
            self.send = self.send_pyamqp
    ...
    def _create_handler_uamqp(self, auth):
        # type: (JWTTokenAuth) -> None
        self._handler = uamqp.SendClient(
            self._target,
            auth=auth,
    ...
    def _send_event_data_uamqp(self, timeout_time=None, last_exception=None):
        # type: (Optional[float], Optional[Exception]) -> None
        if self._unsent_events:
            ...
            if self._outcome != uamqp.constants.MessageSendResult.Ok:
                if self._outcome == uamqp.constants.MessageSendResult.Timeout:
                    self._condition = OperationTimeoutError("Send operation timed out")
                if self._condition:
                    raise self._condition

    def _send_event_data_pyamqp(self, timeout_time=None):
    ...
    def _send_event_data_with_retry(self, timeout=None):    # why is this here?
        # type: (Optional[float]) -> None
        return self._do_retryable_operation(self._send_event_data, timeout=timeout)

    def _wrap_eventdata_uamqp(
        self,
    ...
    def _on_outcome(self, outcome, condition):
        # type: (uamqp.constants.MessageSendResult, Optional[Exception]) -> None
        """
    ...
    def send_uamqp(
        ...
        with self._lock:
            with send_context_manager() as child:
                self._check_closed()
                wrapper_event_data = self._wrap_eventdata_uamqp(event_data, child, partition_key)
    ...
    def send_pyamqp(
        ...
        with self._lock:
            with send_context_manager() as child:
                self._check_closed()
                wrapper_event_data = self._wrap_eventdata_pyamqp(event_data, child, partition_key)
                if child:
                    self._client._add_span_request_attributes(  # pylint: disable=protected-access
                        child
                    )
