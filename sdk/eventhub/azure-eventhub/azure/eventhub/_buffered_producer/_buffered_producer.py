    ...
    def enhanced_callback_decorator(self, callback):
        def wrapper_callback(*args, **kwargs):
            try:
                ...
                _LOGGER.warning("callback exception", callback.__name__, exc, self._partition_id)
        return wrapper_callback

    def sent_callback(self, events, future: Future):
        try:
            future.result()
            self._on_success(events, self._partition_id)
        except Exception as exc:
            self._on_error(events, exc, self._partition_id)
        finally:
        ...
        with self._not_empty:
            ...
            while not self._buffered_queue.empty():
                ...
                if ((remaining_time and remaining_time > 0) or remaining_time is None) and\
                    ...
                    future = self._executor.submit(self._producer.send, batch)
                    self._send_futures.add(future)
                    future.add_done_callback(partial(self.sent_callback, batch._internal_events))
                else:
                    raise OperationTimeoutError("Failed to flush {!r}".format(self._partition_id))
