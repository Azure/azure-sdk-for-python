...
raise unittest.SkipTest(
    "These were written by Consumption service team for 2.0.0 and would need adaption, and are disabled for now."
)
...
class TestMgmtConsumption(AzureMgmtRecordedTestCase):
    ...
    def test_consumption_billing_period_usage_filter(self):
        output = list(
            self.consumption_client.usage_details.list_by_billing_period(
    ...
    def test_consumption_reservations_summaries_monthly(self):
        output = list(
            self.consumption_client.reservations_summaries.list_by_reservation_order(
                reservation_order_id=MgmtConsumptionTest.reservationOrderId, grain="monthly"
            )
        )
    ...
    def test_consumption_reservations_summaries_monthly_withreservationid(self):
        output = list(
            self.consumption_client.reservations_summaries.list_by_reservation_order_and_reservation(
                reservation_order_id=MgmtConsumptionTest.reservationOrderId,
                reservation_id=MgmtConsumptionTest.reservationId,
                grain="monthly",
            )
        )
        self._validate_reservations_summaries(output[0])
    ...
    def test_consumption_reservations_summaries_daily(self):
        output = list(
            self.consumption_client.reservations_summaries.list_by_reservation_order(
                reservation_order_id=MgmtConsumptionTest.reservationOrderId,
                grain="daily",
                filter="properties/UsageDate ge 2017-10-01 AND properties/UsageDate le 2017-12-07",
            )
        )
        self._validate_reservations_summaries(output[0])
    ...
    def test_consumption_reservations_details(self):
        output = list(
            self.consumption_client.reservations_details.list_by_reservation_order(
                reservation_order_id=MgmtConsumptionTest.reservationOrderId,
                filter="properties/UsageDate ge 2017-10-01 AND properties/UsageDate le 2017-12-08",
            )
        )
    ...
    def test_consumption_reservations_details_withreservationid(self):
        output = list(
            self.consumption_client.reservations_details.list_by_reservation_order_and_reservation(
                reservation_order_id=MgmtConsumptionTest.reservationOrderId,
                reservation_id=MgmtConsumptionTest.reservationId,
                filter="properties/UsageDate ge 2017-10-01 AND properties/UsageDate le 2017-12-08",
            )
    ...
    def test_consumption_budget_get_by_resource_group(self):
        budget = self.consumption_client.budgets.get(
            resource_groups="testscaleset", budget_name="PythonSDKTestBudgetCost8"
        )
    ...
    def test_consumption_budget_update_By_resourceGroup(self):
        ...
        budget.amount = 90.0
        budget.time_period = self.models.BudgetTimePeriod(
            start_date=budget.time_period.start_date, end_date=MgmtConsumptionTest.endDate
        )
        output = self.consumption_client.budgets.create_or_update_by_resource_group_name(
            resource_group_name="testscaleset", budget_name="PythonSDKTestBudgetCost8", parameters=budget
        )
    ...
    def test_consumption_budget_delete_by_budget_resource_group_name(self):
        ...
        self._validate_budget(budget)
        self.consumption_client.budgets.delete_by_resource_group_name(
            resource_group_name="testscaleset", budget_name=budget.name
        )
    ...
    def test_consumption_budget_create(self):
        budget = self.models.Budget(
            category=self.models.CategoryType.cost,
            amount=60.0,
            time_grain=self.models.TimeGrainType.monthly,
            time_period=self.models.BudgetTimePeriod(
                start_date=MgmtConsumptionTest.startDate, end_date=MgmtConsumptionTest.endDate
            ),
        )
        output = self.consumption_client.budgets.create_or_update(
            resource_group_name="testResource1", budget_name="PythonSDKTestBudgetCost", parameters=budget
        )
    ...
    def test_consumption_budget_list(self):
        ...
        output = self.consumption_client.budgets.create_or_update(
            budget_name="PythonSDKTestBudgetCostCreateAndDelete", parameters=budget
        )
    ...
    def test_consumption_billing_period_marketplace_filter(self):
        pages = self.consumption_client.marketplaces.list_by_billing_period(
            billing_period_name="201804-1", filter="usageEnd ge 2018-01-26T23:59:59Z"
        )
        first_page = pages.advance_page()
        output = list(first_page)
        self._validate_marketplace(output[0])
    ...
    def test_consumption_billing_period_price_sheet(self):
        output = self.consumption_client.price_sheet.get_by_billing_period(
            billing_period_name=MgmtConsumptionTest.billingPeriodName, expand="properties/meterDetails"
        )
