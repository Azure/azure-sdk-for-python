...
- Added `refresh_enabled` parameter to the `load` method. Defaults to `False`, unless `refresh_on` keys are specified resulting in defaulting to `True`. When set to `False`, calling `refresh` will be a no-op.
