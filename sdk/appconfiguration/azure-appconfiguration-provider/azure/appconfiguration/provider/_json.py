...
def _strip_ignore_string(text, i, length, result):
    """
...
def remove_json_comments(text):
    ...
    Writes the cleaned JSON to output_path if provided, else returns as string.
    ...
    while i < length:
        ...
        if current_char == DOUBLE_QUOTE:
            result.append(current_char)
            i += 1
            result, i = _strip_ignore_string(text, i, length, result)
        elif text[i : i + 2] == DOUBLE_FORWARD_SLASH_COMMENT or current_char == HASH_COMMENT:
            # Skip to end of line
            ...
            i += 2
            while i < length - 1 and text[i : i + 2] != END_MULTI_LINE_COMMENT:
                i += 1
            i += 2
        else:
            result.append(current_char)
