...
class TestAppConfigurationProvider(AppConfigTestCase):
    ...
    async def test_provider_trim_prefixes(self, appconfiguration_endpoint_string):
        trimmed = {"test."}
        async with await self.create_aad_client(appconfiguration_endpoint_string, trim_prefixes=trimmed) as client:
    ...
    async def test_provider_selectors(self, appconfiguration_endpoint_string):
        ...
        async with await self.create_aad_client(appconfiguration_endpoint_string, selects=selects) as client:
