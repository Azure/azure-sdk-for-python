...
class TestAppConfigurationProvider(AzureRecordedTestCase):
    ...
    def build_provider_aad(self, endpoint):
        cred = self.get_credential(AzureAppConfigurationClient)
        return AzureAppConfigurationProvider.load(endpoint=endpoint, credential=cred)

    def build_provider(self, connection_string):
        return AzureAppConfigurationProvider.load(connection_string=connection_string)
