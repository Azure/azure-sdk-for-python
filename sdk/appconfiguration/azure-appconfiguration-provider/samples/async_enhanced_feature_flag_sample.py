...
DESCRIPTION:
    ...
    ``load()`` options are required to opt in. Use ``FeatureFlagSelector`` to filter which enhanced feature
    flags are loaded. This is the async version of enhanced_feature_flag_sample.py.
...
async def main():
    ...
    # resource type from the key-value based feature flags.
