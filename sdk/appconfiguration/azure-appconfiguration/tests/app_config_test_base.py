...
class AzureAppConfigurationClientTestBase(AzureMgmtTestCase):
    ...
    def tearDown(self):
        ...
        for item in self.to_delete:
            self.get_config_client().delete_configuration_setting(key=item.key, label=item.label)

    def get_config_client(self):
        return self.app_config_client

    def _add_for_test(self, kv):
        exist = bool(list(self.get_config_client().list_configuration_settings(keys=[kv.key], labels=[kv.label])))
        if exist:
    ...
    def _delete_from_test(self, key, label):
        try:
            ...
            logging.debug("Error occurred removing configuration setting %s %s during unit test" % (key, label))
