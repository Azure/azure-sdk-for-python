...
class AppConfigError(_model_base.Model):
...
class OperationDetails(_model_base.Model):
    ...
    :ivar error: An error, available when the status is ``Failed``\\ , describing why the operation
     failed.
    :vartype error: ~azure.appconfiguration.models.Error
    """
...
class Snapshot(_model_base.Model):  # pylint: disable=too-many-instance-attributes
