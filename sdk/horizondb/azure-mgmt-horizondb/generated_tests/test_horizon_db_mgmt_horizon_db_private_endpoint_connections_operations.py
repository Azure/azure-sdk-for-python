...
class TestHorizonDBMgmtHorizonDbPrivateEndpointConnectionsOperations(AzureMgmtRecordedTestCase):
    def setup_method(self, method):
        self.client = self.create_mgmt_client(HorizonDBMgmtClient)

    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy
    def test_horizon_db_private_endpoint_connections_get(self, resource_group):
        response = self.client.horizon_db_private_endpoint_connections.get(
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy
    def test_horizon_db_private_endpoint_connections_list(self, resource_group):
        response = self.client.horizon_db_private_endpoint_connections.list(
            resource_group_name=resource_group.name,
            cluster_name="str",
        )
