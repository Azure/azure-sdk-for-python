...
class TestHorizonDBMgmtClustersOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_clusters_begin_create_or_replace(self, resource_group):
        response = await (
            await self.client.clusters.begin_create_or_replace(
                resource_group_name=resource_group.name,
                cluster_name="str",
                resource={
                    "location": "str",
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_clusters_begin_failover(self, resource_group):
        response = await (
            await self.client.clusters.begin_failover(
                resource_group_name=resource_group.name,
                cluster_name="str",
            )
        ).result()  # call '.result()' to poll until service return final result
