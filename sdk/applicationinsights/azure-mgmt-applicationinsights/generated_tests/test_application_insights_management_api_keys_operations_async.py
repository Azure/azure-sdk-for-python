...
class TestApplicationInsightsManagementAPIKeysOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_api_keys_create(self, resource_group):
        response = await self.client.api_keys.create(
            resource_group_name=resource_group.name,
            resource_name="str",
            api_key_properties={"linkedReadProperties": ["str"], "linkedWriteProperties": ["str"], "name": "str"},
            api_version="2015-05-01",
        )
