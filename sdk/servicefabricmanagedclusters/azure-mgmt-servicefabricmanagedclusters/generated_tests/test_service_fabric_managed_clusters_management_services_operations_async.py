...
class TestServiceFabricManagedClustersManagementServicesOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_services_begin_restart_replica(self, resource_group):
        response = await (
            await self.client.services.begin_restart_replica(
                resource_group_name=resource_group.name,
                cluster_name="str",
                application_name="str",
                service_name="str",
