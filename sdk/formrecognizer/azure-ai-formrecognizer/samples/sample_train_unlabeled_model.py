...
"""
FILE: sample_train_unlabeled_model.py

DESCRIPTION:
    This sample demonstrates how to train a model with unlabeled data. See sample_recognize_custom_forms.py
    to recognize forms with your custom model.
USAGE:
    python sample_train_unlabeled_model.py
...
class TrainUnlabeledModelSample(object):

    endpoint = os.environ["AZURE_FORM_RECOGNIZER_ENDPOINT"]
    key = os.environ["AZURE_FORM_RECOGNIZER_KEY"]
    container_sas_url = os.environ["CONTAINER_SAS_URL"]

    def train_unlabeled_model(self):
        from azure.ai.formrecognizer import FormTrainingClient
        from azure.core.credentials import AzureKeyCredential

        form_training_client = FormTrainingClient(self.endpoint, AzureKeyCredential(self.key))

        poller = form_training_client.begin_training(self.container_sas_url, use_labels=False)
        model = poller.result()
...
if __name__ == '__main__':
    sample = TrainUnlabeledModelSample()
    sample.train_unlabeled_model()
