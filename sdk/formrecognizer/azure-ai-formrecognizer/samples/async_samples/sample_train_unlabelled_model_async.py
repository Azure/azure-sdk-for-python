...
"""
FILE: sample_train_unlabelled_model_async.py

DESCRIPTION:
    This sample demonstrates how to train a model with unlabelled data. See sample_recognize_custom_forms_async.py
    to recognize forms with your custom model.
USAGE:
    python sample_train_unlabelled_model_async.py
...
class TrainUnlabelledModelSampleAsync(object):

    endpoint = os.environ["AZURE_FORM_RECOGNIZER_ENDPOINT"]
    key = os.environ["AZURE_FORM_RECOGNIZER_KEY"]
    blob_sas_url = os.environ["BLOB_SAS_URL"]

    async def train_unlabelled_model(self):
...
async def main():
    sample = TrainUnlabelledModelSampleAsync()
    await sample.train_unlabelled_model()
