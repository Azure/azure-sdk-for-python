...
"""
FILE: sample_train_labelled_model_async.py

DESCRIPTION:
    This sample demonstrates how to train a model with labelled data. See sample_recognize_custom_forms_async.py
    to recognize forms with your custom model.
USAGE:
    python sample_train_labelled_model_async.py
...
class TrainLabelledModelSampleAsync(object):

    endpoint = os.environ["AZURE_FORM_RECOGNIZER_ENDPOINT"]
    key = os.environ["AZURE_FORM_RECOGNIZER_KEY"]
    blob_sas_url = os.environ["BLOB_SAS_URL"]

    async def train_labelled_model(self):
        ...
        async with FormTrainingClient(endpoint=self.endpoint, credential=AzureKeyCredential(self.key)) as form_training_client:
            ...
            model = await form_training_client.training(self.blob_sas_url, use_labels=True)
...
async def main():
    sample = TrainLabelledModelSampleAsync()
    await sample.train_labelled_model()
