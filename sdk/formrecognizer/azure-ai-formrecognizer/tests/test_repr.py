...
@pytest.fixture
def bounding_box():
    model = [_models.Point(1, 2), _models.Point(3, 4), _models.Point(5, 6), _models.Point(7, 8)]
...
def form_word(bounding_box):
    ...
    model_repr = "FormWord(text=Word, bounding_box={}, confidence=0.5, page_number=1, kind=word)".format(
        bounding_box[1]
    )[:1024]
    assert repr(model) == model_repr
    return model, model_repr
...
def form_line(bounding_box, form_word):
    ...
    model_repr = (
        "FormLine(text=Word Word, bounding_box={}, words=[{}, {}], page_number=1, kind=line, appearance={})".format(
            bounding_box[1], form_word[1], form_word[1], appearance
        )[:1024]
    )
    assert repr(model) == model_repr
    return model, model_repr
...
def form_table_cell(bounding_box, form_word):
    model = _models.FormTableCell(
        ...
        is_footer=False,
        page_number=3,
        field_elements=[form_word[0]],
    )
    model_repr = (
        "FormTableCell(text=Cell, row_index=3, column_index=4, row_span=2, column_span=3, bounding_box={}, confidence=0.7, "
        "is_header=True, is_footer=False, page_number=3, field_elements=[{}])".format(bounding_box[1], form_word[1])[
            :1024
        ]
    )
    assert repr(model) == model_repr
...
def form_table(form_table_cell, bounding_box):
    model = _models.FormTable(
        page_number=1,
        cells=[form_table_cell[0], form_table_cell[0]],
        row_count=3,
        column_count=4,
        bounding_box=bounding_box[0],
    )
    model_repr = "FormTable(page_number=1, cells=[{}, {}], row_count=3, column_count=4, bounding_box={})".format(
        form_table_cell[1], form_table_cell[1], bounding_box[1]
    )[:1024]
    assert repr(model) == model_repr
    return model, model_repr
...
def field_data(bounding_box, form_word, form_line):
    model = _models.FieldData(
        page_number=1, text="This is text.", bounding_box=bounding_box[0], field_elements=[form_word[0], form_line[0]]
    )
    model_repr = "FieldData(page_number=1, text=This is text., bounding_box={}, field_elements=[{}, {}])".format(
        bounding_box[1], form_word[1], form_line[1]
    )[:1024]
    assert repr(model) == model_repr
    return model, model_repr
...
def form_field_two(field_data):
    model = _models.FormField(
        value_type="string",
        label_data=field_data[0],
        value_data=field_data[0],
        name="form_field_two",
        value="value",
        confidence=0,
    )
    model_repr = "FormField(value_type=string, label_data={}, value_data={}, name=form_field_two, value='value', confidence=0)".format(
        field_data[1], field_data[1]
    )[
        :1024
    ]
    assert repr(model) == model_repr
...
def form_field_one(field_data, form_field_two):
    model = _models.FormField(
        ...
        value_data=field_data[0],
        name="form_field_one",
        value=form_field_two[0],
        confidence=1.0,
    )
    model_repr = "FormField(value_type=string, label_data={}, value_data={}, name=form_field_one, value={}, confidence=1.0)".format(
        field_data[1], field_data[1], form_field_two[1]
    )[
        :1024
    ]
    assert repr(model) == model_repr
...
def form_page(form_table, form_line):
    model = _models.FormPage(
        page_number=1, text_angle=180, width=5, height=5.5, unit="pixel", tables=[form_table[0]], lines=[form_line[0]]
    )
    model_repr = (
        "FormPage(page_number=1, text_angle=180, width=5, height=5.5, unit=pixel, tables=[{}], lines=[{}])".format(
...
def custom_form_sub_model(custom_form_model_field):
    model = _models.CustomFormSubmodel(
        accuracy=0.99, model_id=1, fields={"name": custom_form_model_field[0]}, form_type="Itemized"
    )
    model_repr = "CustomFormSubmodel(accuracy=0.99, model_id=1, fields={{'name': {}}}, form_type=Itemized)".format(
        custom_form_model_field[1]
    )[:1024]
    assert repr(model) == model_repr
    return model, model_repr
...
def training_document_info(form_recognizer_error):
    model = _models.TrainingDocumentInfo(
        name="name", status="partiallySucceeded", page_count=5, errors=[form_recognizer_error[0]], model_id=1
    )
    model_repr = (
        "TrainingDocumentInfo(name=name, status=partiallySucceeded, page_count=5, errors=[{}], model_id=1)".format(
...
def address_value():
    ...
    model_repr = (
        "AddressValue(house_number={}, po_box={}, road={}, city={}, state={}, postal_code={}, country_region={}, street_address={}, "
...
def address_document_field(bounding_region, document_span, address_value):
    model = _models.DocumentField(
        ...
        content="123 Contoso Ave, Redmond, WA 98052 P.O. 4567",
        bounding_regions=[bounding_region[0]],
        spans=[document_span[0]],
        confidence=0.98,
    )
    model_repr = (
        "DocumentField(value_type={}, value={}, content={}, bounding_regions=[{}], spans=[{}], confidence={})".format(
            "address",
            address_value[1],
...
def analyzed_document(bounding_region, document_span, document_field, address_document_field):
    model = _models.AnalyzedDocument(
        doc_type="prebuilt:invoice",
        bounding_regions=[bounding_region[0]],
        spans=[document_span[0]],
        fields={"key": document_field[0], "address": address_document_field[0]},
        confidence=0.98,
    )
...
def document_key_value_element(bounding_region, document_span):
    model = _models.DocumentKeyValueElement(
        content="my content", bounding_regions=[bounding_region[0]], spans=[document_span[0]]
    )
...
def document_key_value_pair(document_key_value_element):
    model = _models.DocumentKeyValuePair(
        key=document_key_value_element[0], value=document_key_value_element[0], confidence=0.98
    )
    model_repr = "DocumentKeyValuePair(key={}, value={}, confidence={})".format(
        document_key_value_element[1], document_key_value_element[1], 0.98
    )
    assert repr(model) == model_repr
    return model, model_repr
...
def document_paragraph(bounding_region, document_span):
    model = _models.DocumentParagraph(
        role="role", content="paragraph content", bounding_regions=[bounding_region[0]], spans=[document_span[0]]
    )
...
def document_selection_mark(bounding_box, document_span):
    model = _models.DocumentSelectionMark(
        state="selected", polygon=bounding_box[0], span=document_span[0], confidence=0.89
    )
...
def document_barcode(bounding_box, document_span):
    model = _models.DocumentBarcode(
        kind="QRCode", value="15", polygon=bounding_box[0], span=document_span[0], confidence=0.8
    )
...
def document_formula(bounding_box, document_span):
    model = _models.DocumentFormula(
        kind="inline", value="2+2=4", polygon=bounding_box[0], span=document_span[0], confidence=0.8
    )
...
@pytest.fixture
def document_page(
    document_span, document_word, document_selection_mark, document_line, document_formula, document_barcode
):
    ...
    model_repr = (
        "DocumentPage(page_number={}, angle={}, width={}, height={}, unit={}, lines=[{}], words=[{}], selection_marks=[{}], spans=[{}], "
...
def document_style(document_span):
    ...
    model_repr = (
        "DocumentStyle(is_handwritten={}, spans=[{}], confidence={}, similar_font_family={}, font_style={}, "
        "font_weight={}, color={}, background_color={})".format(
            True, document_span[1], 0.98, "Arial", "italic", "bold", "#FF0000", "#FFFFFF"
        )
    )
    assert repr(model) == model_repr
    return model, model_repr
...
def doc_type_info():
    ...
    model_repr = "DocumentTypeDetails(description={}, build_mode={}, field_schema={{'prebuilt-invoice': {}}}, field_confidence={{'CustomerName': {}}})".format(
        "my description", "neural", {"CustomerName": {"type": "string"}}, 95
    )
    assert repr(model) == model_repr
    return model, model_repr
...
def blob_file_list_source():
    ...
    model_repr = (
        f"BlobFileListSource(container_url=https://test.blob.core.windows.net/blob-sas-url, file_list=filelist.jsonl)"
    )
...
def document_analysis_error(document_analysis_inner_error):
    ...
    model_repr = "DocumentAnalysisError(code={}, message={}, target={}, details={}, innererror={})".format(
        ...
        "Resource was not found",
        "resource",
        [_models.DocumentAnalysisError(code="ResourceNotFound", message="Resource was not found")],
...
class TestRepr:
    ...
    def test_recognized_form(self, form_field_one, page_range, form_page):
        model = _models.RecognizedForm(
            form_type="receipt",
            form_type_confidence=1.0,
            model_id=1,
            fields={"one": form_field_one[0]},
    ...
    def test_custom_form_model(
        self, custom_form_sub_model, custom_form_model_properties, form_recognizer_error, training_document_info
    ):
        ...
        model_repr = (
            "CustomFormModel(model_id=1, status=creating, training_started_on=0001-01-01 00:00:00, "
            "training_completed_on=0001-01-01 00:00:00, submodels=[{}, {}], errors=[{}], training_documents=[{}, {}], "
            "model_name=my model, properties={})".format(
                custom_form_sub_model[1],
                custom_form_sub_model[1],
                form_recognizer_error[1],
                training_document_info[1],
                training_document_info[1],
    ...
    def test_custom_form_model_info(self, custom_form_model_properties):
        ...
        model_repr = "CustomFormModelInfo(model_id=1, status=ready, training_started_on=0001-01-01 00:00:00, training_completed_on=0001-01-01 00:00:00, properties={}, model_name=my model)".format(
            custom_form_model_properties[1]
        )[
            :1024
        ]
    ...
    def test_analyze_result(
        ...
        model = _models.AnalyzeResult(
            ...
            languages=[document_language[0]],
            pages=[document_page[0]],
            tables=[document_table[0]],
            key_value_pairs=[document_key_value_pair[0]],
            styles=[document_style[0]],
            documents=[analyzed_document[0]],
            paragraphs=[document_paragraph[0]],
        )
    ...
    def test_document_classifier_details(
        self, classifier_document_type_details, classifier_document_type_details_file_list
    ):
    ...
    def test_resource_details(self):
        model = _models.ResourceDetails(
            ...
            neural_document_model_quota=_models.QuotaDetails(
                used=0, quota=20, quota_resets_on=datetime.datetime(2024, 9, 16, 10, 10, 59, 342380)
            ),
        )
    ...
    def test_currency_value(self):
        model = _models.CurrencyValue(amount=10.5, symbol="$", code="USD")
        model_repr = "CurrencyValue(amount={}, symbol={}, code={})".format(10.5, "$", "USD")
        assert repr(model) == model_repr
