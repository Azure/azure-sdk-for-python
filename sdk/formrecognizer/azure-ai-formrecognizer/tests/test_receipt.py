    ...
    def test_receipt_jpg_include_field_elements(self, client):
        ...
        for field, value in receipt.__dict__.items():
            if field not in ["receipt_items", "page_range", "pages", "fields", "form_type"]:
                ...
                self.assertTextContentHasValues(form_field.value_data.field_element, receipt.page_range.first_page_number)

        for field, value in receipt.fields.items():
            self.assertTextContentHasValues(value.value_data.field_element, receipt.page_range.first_page_number)
