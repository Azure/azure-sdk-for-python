...
class TestDACAnalyzePrebuiltsAsync(AsyncFormRecognizerTest):
    ...
    async def test_passing_unsupported_url_content_type(self, **kwargs):
        ...
        with pytest.raises(TypeError):
            async with client:
                poller = await client.begin_analyze_document(
                    "prebuilt-receipt", "https://badurl.jpg", content_type="application/json"
                )
                result = await poller.result()
    ...
    async def test_receipt_stream_transform_png(self):
        ...
        async with client:
            poller = await client.begin_analyze_document("prebuilt-receipt", document=my_file, cls=callback)
            result = await poller.result()
    ...
    async def test_invoice_stream_transform_tiff(self):
        ...
        async with client:
            poller = await client.begin_analyze_document(model_id="prebuilt-invoice", document=my_file, cls=callback)
