...
class TestCustomForms(FormRecognizerTest):
    ...
    def test_custom_form_unlabeled(self, formrecognizer_storage_container_sas_url_v2, **kwargs):
        ...
        with open(self.form_jpg, "rb") as stream:
            poller = fr_client.begin_recognize_custom_forms(
                model.model_id, stream, content_type=FormContentType.IMAGE_JPEG
            )
        form = poller.result()
    ...
    def test_custom_form_multipage_unlabeled(self, formrecognizer_multipage_storage_container_sas_url_v2, **kwargs):
        ...
        with open(self.multipage_invoice_pdf, "rb") as stream:
            poller = fr_client.begin_recognize_custom_forms(
                model.model_id, stream, content_type=FormContentType.APPLICATION_PDF
            )
        forms = poller.result()
    ...
    def test_custom_form_labeled(self, formrecognizer_storage_container_sas_url_v2, **kwargs):
        ...
        poller = client.begin_training(
            formrecognizer_storage_container_sas_url_v2, use_training_labels=True, model_name="labeled"
        )
        model = poller.result()
    ...
    @FormRecognizerPreparer()
    @recorded_by_proxy
    def test_custom_form_unlabeled_transform(self, formrecognizer_storage_container_sas_url_v2, **kwargs):
        client = get_ft_client()
        fr_client = client.get_form_recognizer_client()
    ...
    def test_custom_form_continuation_token(self, **kwargs):
        ...
        with open(self.form_jpg, "rb") as fd:
            my_file = fd.read()
        initial_poller = fr_client.begin_recognize_custom_forms(model.model_id, my_file)
        cont_token = initial_poller.continuation_token()
        poller = fr_client.begin_recognize_custom_forms(model.model_id, None, continuation_token=cont_token)
