...
class BuildModelRequestPreparation(PerfStressTest):
    ...
    def __init__(self, arguments):
        ...
        self.admin_client = DocumentModelAdministrationClient(
            formrecognizer_test_endpoint, AzureKeyCredential(form_recognizer_account_key)
        )
    ...
    def run_sync(self):
        ...
        poller = self.admin_client.begin_build_document_model(
            "template", blob_container_url=self.formrecognizer_storage_container_sas_url
        )
        assert poller

    async def run_async(self):
        """The asynchronous perf test."""
        poller = await self.async_admin_client.begin_build_document_model(
            "template", blob_container_url=self.formrecognizer_storage_container_sas_url
        )
        assert poller
