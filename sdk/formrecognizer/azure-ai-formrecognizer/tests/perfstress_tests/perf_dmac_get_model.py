...
class GetModel(PerfStressTest):   

    def __init__(self, arguments):
        super().__init__(arguments)

        # read test related env vars
        formrecognizer_storage_container_sas_url = os.environ["FORMRECOGNIZER_TRAINING_DATA_CONTAINER_SAS_URL"]
        ...
        poller = self.admin_client.begin_build_model(formrecognizer_storage_container_sas_url)
        results = poller.result()
        self.model_id = results.model_id

    async def close(self):
        """This is run after cleanup."""
        self.admin_client.delete_model(self.model_id)
    ...
    def run_sync(self):
        ...
        result = self.admin_client.get_model(self.model_id)
        assert result

    async def run_async(self):
        """The asynchronous perf test."""
        result = self.async_admin_client.get_model(self.model_id)
        assert result
