...
class TestCustomFormsAsync(AsyncFormRecognizerTest):
        ...
        async with client:
            ...
            async with fr_client:
                poller = await fr_client.begin_recognize_custom_forms(
                    model.model_id, my_file, content_type=FormContentType.IMAGE_JPEG
                )
    ...
    async def test_custom_form_multipage_unlabeled(
        ...
        async with client:
            ...
            async with fr_client:
                poller = await fr_client.begin_recognize_custom_forms(
                    model.model_id, my_file, content_type=FormContentType.APPLICATION_PDF
                )
    ...
    @FormRecognizerPreparer()
    @recorded_by_proxy_async
    async def test_custom_forms_multipage_unlabeled_transform(
        self, formrecognizer_multipage_storage_container_sas_url_v2, **kwargs
    ):
        client = get_ft_client()
        fr_client = client.get_form_recognizer_client()
        ...
        async with client:
            ...
            async with fr_client:
                poller = await fr_client.begin_recognize_custom_forms(
                    model.model_id, my_file, include_field_elements=True, cls=callback
                )
                form = await poller.result()
        actual = responses[0]
    ...
    async def test_custom_form_continuation_token(self, **kwargs):
        ...
        async with client:
            ...
            async with fr_client:
                initial_poller = await fr_client.begin_recognize_custom_forms(model.model_id, my_file)
