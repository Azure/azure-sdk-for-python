...
class TestResourceOperations(AzureMgmtRecordedTestCase):
    ...
    def setup_method(self, method):
        ...
        self.akv_client = self.create_mgmt_client(
    ...
    def test_load_test_resource_operations(self, resource_group):
        ...
        self.msi1 = create_managed_identity(self.msi_client, resource_group.name+"mi1", resource_group.name, self.AZURE_LOCATION)
        self.msi2 = create_managed_identity(self.msi_client, resource_group.name+"mi2", resource_group.name, self.AZURE_LOCATION)
        self.akv = create_key_vault(self.akv_client, resource_group.name+"akv", resource_group.name, self.AZURE_LOCATION, self.msi1, self.tenant_id, self.object_id)
        ...
        loadtest_resource_poller = self.loadtestservice_client.load_tests.begin_create_or_update(resource_group.name, resource_group.name+"resource", loadtestresource_create_payload)
        ...
        assert loadtest_resource.identity != None
        ...
        assert loadtest_resource.encryption != None
        assert loadtest_resource.encryption.key_url == self.key.id
        assert loadtest_resource.encryption.identity != None
        ...
        loadtest_resource_get = self.loadtestservice_client.load_tests.get(resource_group.name, resource_group.name+"resource")
        
        # # Assert the result of the get operation
        assert loadtest_resource_get != None
        assert loadtest_resource_get.id != None
        assert loadtest_resource_get.name == resource_group.name+"resource"
        assert loadtest_resource_get.location == self.AZURE_LOCATION
        assert loadtest_resource_get.identity != None
        ...
        assert loadtest_resource_get.encryption != None
        assert loadtest_resource_get.encryption.key_url == self.key.id
        assert loadtest_resource_get.encryption.identity != None
        ...
        loadtest_resource_patch_poller = self.loadtestservice_client.load_tests.begin_update(resource_group.name, resource_group.name+"resource", loadtestresourcePatchdata)
        ...
        # Assert the result of the poller
        assert loadtest_resource_patch_response != None
        assert loadtest_resource_patch_response.id != None
        ...
        assert loadtest_resource_patch_response.identity != None
        ...
        assert loadtest_resource_patch_response.encryption != None
        assert loadtest_resource_patch_response.encryption.key_url == self.key.id
        assert loadtest_resource_patch_response.encryption.identity != None
        ...
        loadtest_resource_delete_poller = self.loadtestservice_client.load_tests.begin_delete(resource_group.name, resource_group.name+"resource")

        # Wait for the deletion LRO to complete
        while loadtest_resource_delete_poller.done()==False:
            loadtest_resource_delete_poller.wait(2)
