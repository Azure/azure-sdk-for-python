    ...
    async def test_create_ad_config_whole_series_detection(self):
        ...
        async with self.admin_client:
            try:
                ...
                await self.admin_client.delete_metric_anomaly_detection_configuration(config.id)

                with self.assertRaises(ResourceNotFoundError):
                    await self.admin_client.get_metric_anomaly_detection_configuration(config.id)
            finally:
