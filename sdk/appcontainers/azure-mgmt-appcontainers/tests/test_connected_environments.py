...
class TestMgmtAPPCONTAINERSCONNECTED_ENVIRONMENTS(AzureMgmtRecordedTestCase):
    def setup_method(self, method):
        self.mgmt_client = self.create_mgmt_client(ContainerAppsAPIClient)

    @recorded_by_proxy
    def test_connected_environments_list_by_subscription(self):
        # it proves that we can normally send request but maybe needs additional parameters
        with suppress(HttpResponseError):
            result = self.mgmt_client.connected_environments.list_by_subscription()
