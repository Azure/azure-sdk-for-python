...
class TestTrafficManagerManagementHeatMapOperationsAsync(AzureMgmtRecordedTestCase):
    def setup_method(self, method):
        self.client = self.create_mgmt_client(TrafficManagerManagementClient, is_async=True)

    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_heat_map_get(self, resource_group):
        response = await self.client.heat_map.get(
