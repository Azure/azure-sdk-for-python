...
class MgmtMixedRealityClientTest(AzureMgmtTestCase):
    ...
    def test_mixedreality(self, resource_group):
        ...
        # /RemoteRenderingAccounts/get/Get remote rendering account key[get]
        result = self.mgmt_client.remote_rendering_accounts.get_keys(resource_group_name=RESOURCE_GROUP, account_name=ACCOUNT_NAME)

        # /SpatialAnchorsAccounts/get/Get spatial anchor account key[get]
        result = self.mgmt_client.spatial_anchors_accounts.get_keys(resource_group_name=RESOURCE_GROUP, account_name=ACCOUNT_NAME)
