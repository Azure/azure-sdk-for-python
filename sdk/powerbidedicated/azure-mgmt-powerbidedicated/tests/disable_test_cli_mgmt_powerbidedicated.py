...
class MgmtPowerBIDedicatedTest(AzureMgmtTestCase):
    ...
    def test_powerbidedicated(self, resource_group):
        ...
        BODY = {
            ...
            "tags": {"test_key": "testValue"},
            "administration": {"members": ["user1@microsoft.com", "user2@microsoft.com"]},
            "location": "eastus",
        }
        result = self.mgmt_client.capacities.create(
            resource_group_name=RESOURCE_GROUP,
            dedicated_capacity_name=DEDICATED_CAPACITY_NAME,
            capacity_parameters=BODY,
        )
        result = result.result()

        # /Capacities/get/List eligible SKUs for an existing capacity[get]
        result = self.mgmt_client.capacities.list_skus_for_capacity(
            resource_group_name=RESOURCE_GROUP, dedicated_capacity_name=DEDICATED_CAPACITY_NAME
        )

        # /Capacities/get/Get details of a capacity[get]
        result = self.mgmt_client.capacities.get_details(
            resource_group_name=RESOURCE_GROUP, dedicated_capacity_name=DEDICATED_CAPACITY_NAME
        )
        ...
        result = self.mgmt_client.capacities.suspend(
            resource_group_name=RESOURCE_GROUP, dedicated_capacity_name=DEDICATED_CAPACITY_NAME
        )
        result = result.result()

        # /Capacities/post/Get details of a capacity[post]
        result = self.mgmt_client.capacities.resume(
            resource_group_name=RESOURCE_GROUP, dedicated_capacity_name=DEDICATED_CAPACITY_NAME
        )
        result = result.result()
        ...
        result = self.mgmt_client.capacities.update(
            resource_group_name=RESOURCE_GROUP,
            dedicated_capacity_name=DEDICATED_CAPACITY_NAME,
        ...
        result = self.mgmt_client.capacities.check_name_availability(
            location="eastus", name="azsdktest", type="Microsoft.PowerBIDedicated/capacities"
        )

        # /Capacities/delete/Get details of a capacity[delete]
        result = self.mgmt_client.capacities.delete(
            resource_group_name=RESOURCE_GROUP, dedicated_capacity_name=DEDICATED_CAPACITY_NAME
        )
        result = result.result()
