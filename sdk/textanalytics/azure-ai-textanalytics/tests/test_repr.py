...
def text_document_statistics():
    model = _models.TextDocumentStatistics(character_count=14, transaction_count=18)
...
def text_analytics_warning():
    model = _models.TextAnalyticsWarning(code="LongWordsInDocument", message="warning")
    model_repr = "TextAnalyticsWarning(code=LongWordsInDocument, message=warning)"
    assert repr(model) == model_repr
    return model, model_repr
...
def pii_entity():
    model = _models.PiiEntity(
        text="859-98-0987", category="SSN", subcategory=None, length=11, offset=0, confidence_score=0.899
    )
    model_repr = (
        "PiiEntity(text=859-98-0987, category=SSN, subcategory=None, length=11, offset=0, confidence_score=0.899)"
    )
    assert repr(model) == model_repr
    return model, model_repr
...
def linked_entity_match():
    model = _models.LinkedEntityMatch(confidence_score=0.999, text="Bill Gates", offset=0, length=10)
...
def target_assessment_confidence_score():
    model = _models.SentimentConfidenceScores(positive=0.5, negative=0.5, neutral=0.0)
...
def mined_opinion(target_sentiment, assessment_sentiment):
    model = _models.MinedOpinion(target=target_sentiment[0], assessments=[assessment_sentiment[0]])
...
def sentence_sentiment(sentiment_confidence_scores, mined_opinion):
    ...
    model_repr = (
        ...
        "length=19, offset=0, mined_opinions=[{}])".format(sentiment_confidence_scores[1], mined_opinion[1])
    )
    assert repr(model) == model_repr
    return model, model_repr
...
def recognize_pii_entities_result(pii_entity, text_analytics_warning, text_document_statistics):
    ...
    model_repr = (
        "RecognizePiiEntitiesResult(id=1, entities=[{}], redacted_text=***********, warnings=[{}], "
...
def healthcare_entity_assertion():
    ...
    model_repr = (
        "HealthcareEntityAssertion(conditionality=conditionality, certainty=certainty, association=association)"
    )
...
def healthcare_relation_role(healthcare_entity):
    model = _models.HealthcareRelationRole(name="ROLE", entity=healthcare_entity[0])
...
def healthcare_relation(healthcare_relation_role):
    model = _models.HealthcareRelation(
        relation_type="DOSAGE", roles=[healthcare_relation_role[0]], confidence_score=1.0
    )

    model_repr = (
        f"HealthcareRelation(relation_type=DOSAGE, roles=[{healthcare_relation_role[1]}], confidence_score=1.0)"
    )
...
class TestRepr:
    def test_text_document_input(self):
        model = _models.TextDocumentInput(id="1", text="hello world", language="en")
        model_repr = "TextDocumentInput(id=1, text=hello world, language=en)"

        assert repr(model) == model_repr

    def test_detect_language_input(self):
        model = _models.DetectLanguageInput(id="1", text="hello world", country_hint="US")
    ...
    def test_text_document_batch_statistics(self):
        model = _models.TextDocumentBatchStatistics(
            document_count=1, valid_document_count=2, erroneous_document_count=3, transaction_count=4
        )
        model_repr = (
    ...
    def test_recognized_linked_entities_result(
        self, linked_entity, text_analytics_warning, text_document_statistics, detected_language
    ):
        ...
        model_repr = "RecognizeLinkedEntitiesResult(id=1, entities=[{}], warnings=[{}], statistics={}, is_error=False, kind=EntityLinking)".format(
            linked_entity[1],
            text_analytics_warning[1],
            text_document_statistics[1],
        )
    ...
    def test_inner_error_takes_precedence(self):
        ...
        generated_error = _generated_models.TextAnalyticsError(
            code="InvalidArgument", message="Invalid Language Code.", innererror=generated_innererror
        )
        ...
        assert (
            error.message
            == "Supplied language not supported. Pass in one of: de,en,es,fr,it,ja,ko,nl,pt-PT,zh-Hans,zh-Hant"
        )
    ...
    def test_analyze_healthcare_entities_result_item(
        ...
        model_repr = "AnalyzeHealthcareEntitiesResult(id=1, entities=[{}], entity_relations=[{}], warnings=[{}], statistics={}, is_error=False, kind=Healthcare)".format(
