...
from azure.ai.textanalytics import ExtractiveSummaryResult, AbstractiveSummaryResult
...
class TestSummarization(TextAnalyticsTest):
    ...
    async def test_extract_summary(self, client):
        docs = [
            {
                "id": "1",
                "language": "en",
                "text": "The government of British Prime Minster Theresa May has been plunged into turmoil with the resignation"
        ...
        async with client:
            document_results = await (
                await client.begin_extract_summary(
                    docs,
                    show_stats=True,
    ...
    async def test_abstract_summary_action(self, client):
        ...
        async with client:
            response = await (
                await client.begin_abstract_summary(
                    docs,
                    show_stats=True,
                    polling_interval=self._interval(),
                )
