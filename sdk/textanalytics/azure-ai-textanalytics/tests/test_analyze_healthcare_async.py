...
class TestHealth(TextAnalyticsTest):
    ...
    async def test_no_single_input(self, client):
        with pytest.raises(TypeError):
            async with client:
                response = await (
                    await client.begin_analyze_healthcare_entities("hello world", polling_interval=self._interval())
                ).result()
    ...
    async def test_whole_batch_language_hint_and_dict_input(self, client):
        ...
        async with client:
            result = await (
                await client.begin_analyze_healthcare_entities(docs, language="en", polling_interval=self._interval())
            ).result()
            response = []
            async for r in result:
                response.append(r)
    ...
    async def test_invalid_language_hint_method(self, client):
        ...
        async with client:
            result = await (
                await client.begin_analyze_healthcare_entities(
                    docs, language="notalanguage", polling_interval=self._interval()
                )
            ).result()
            response = []
            async for r in result:
    ...
    async def test_default_string_index_type_is_UnicodeCodePoint(self, client):
        poller = await client.begin_analyze_healthcare_entities(
            documents=["Hello world"], polling_interval=self._interval()
        )
