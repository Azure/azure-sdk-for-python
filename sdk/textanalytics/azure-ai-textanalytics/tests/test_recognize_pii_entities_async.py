...
class TestRecognizePIIEntities(TextAnalyticsTest):
    ...
    async def test_empty_credential_class(self, client):
        with pytest.raises(ClientAuthenticationError):
            response = await client.recognize_pii_entities(["This is written in English."])
    ...
    async def test_pass_cls(self, client):
        ...
        res = await client.recognize_pii_entities(documents=["Test passing cls to endpoint"], cls=callback)
    ...
    async def test_default_string_index_type_is_UnicodeCodePoint(self, client):
        ...
        res = await client.recognize_pii_entities(documents=["Hello world"], raw_response_hook=callback)
    ...
    async def test_pii_entities_multiapi_validate_v3_0(self, **kwargs):
        ...
        with pytest.raises(ValueError) as e:
            await client.recognize_pii_entities(documents=["Test"])
        assert (
