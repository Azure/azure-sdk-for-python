...
class TestAnalyze(TextAnalyticsTest):
    ...
    def test_all_successful_passing_dict_sentiment_task(self, client):
        ...
        for idx, document_results in enumerate(pages):
            ...
            if idx == 0:
                ...
                assert (
                    document_result.sentences[0].text == "I did not like the hotel we stayed at. "
                )  # https://dev.azure.com/msazure/Cognitive%20Services/_workitems/edit/14208842
                assert document_result.sentences[1].text == "It was too expensive."
            else:
                assert document_result.sentiment == "positive"
                assert len(document_result.sentences) == 2
                assert (
                    document_result.sentences[0].text == "The restaurant had really good food. "
                )  # https://dev.azure.com/msazure/Cognitive%20Services/_workitems/edit/14208842
    ...
    def test_multiple_pages_of_results_returned_successfully(self, client):
        ...
        docs = [
            {"id": str(idx), "text": val} for (idx, val) in enumerate(list(itertools.repeat(single_doc, 25)))
    ...
    def test_disable_service_logs(
        ...
        client = TextAnalyticsClient(
            textanalytics_custom_text_endpoint, AzureKeyCredential(textanalytics_custom_text_key)
        )
        actions = [
    ...
    def test_pii_action_categories_filter(self, client):
        ...
        result = client.begin_analyze_actions(
            documents=docs, actions=actions, polling_interval=self._interval()
        ).result()
    ...
    def test_partial_success_for_actions(self, client):
        docs = [
            {"id": "1", "language": "tr", "text": "I did not like the hotel we stayed at."},
    ...
    @TextAnalyticsPreparer()
    def test_generic_action_error_no_target_v3_1(self, **kwargs):
        docs = [
            {
                "id": "1",
                "language": "en",
        ...
        response = mock.Mock(
            status_code=200, headers={"Content-Type": "application/json", "operation-location": "https://fakeurl.com"}
        )
        ...
        with pytest.raises(HttpResponseError) as e:
            response = list(
                client.begin_analyze_actions(
                    docs,
                    actions=[RecognizeEntitiesAction(), RecognizeLinkedEntitiesAction(), RecognizePiiEntitiesAction()],
                    show_stats=True,
                    polling_interval=self._interval(),
                ).result()
            )
        assert (
            e.value.message
            == "(InternalServerError) 1 out of 3 job tasks failed. Failed job tasks : v3.1/entities/general."
        )

    @TextAnalyticsPreparer()
    def test_generic_action_error_no_target(self, **kwargs):
        docs = [
            {
                "id": "1",
                "language": "en",
        ...
        assert (
            e.value.message
            == "(InternalServerError) 1 out of 5 job tasks failed. Failed job tasks : keyphrasescomposite."
        )

    @TextAnalyticsPreparer()
    def test_action_errors_with_targets_v3_1(self, **kwargs):
        docs = [
            {
                "id": "1",
                "language": "en",
    ...
    @TextAnalyticsPreparer()
    def test_action_errors_with_targets(self, **kwargs):
        docs = [
            {
                "id": "1",
                "language": "en",
    ...
    @TextAnalyticsPreparer()
    def test_action_job_failure_v3_1(self, **kwargs):
        docs = [
            {
                "id": "1",
                "language": "en",
        ...
        assert (
            e.value.message
            == "(InternalServerError) 1 out of 1 job tasks failed. Failed job tasks : v3.1/entities/general."
        )

    @TextAnalyticsPreparer()
    def test_action_job_failure(self, **kwargs):
        docs = [
            {
                "id": "1",
                "language": "en",
        ...
        assert (
            e.value.message
            == "(InternalServerError) 1 out of 1 job tasks failed. Failed job tasks : keyphrasescomposite."
        )
    ...
    def test_analyze_multiapi_validate_v3_0(self, **kwargs):
        ...
        assert (
            str(e.value) == "'TextAnalyticsClient.begin_analyze_actions' is not available in API version v3.0. "
    ...
    def test_analyze_multiapi_validate_v3_1(self, **kwargs):
        ...
        textanalytics_single_label_classify_project_name = kwargs.pop(
            "textanalytics_single_label_classify_project_name"
        )
        textanalytics_single_label_classify_deployment_name = kwargs.pop(
            "textanalytics_single_label_classify_deployment_name"
        )
        textanalytics_multi_label_classify_project_name = kwargs.pop("textanalytics_multi_label_classify_project_name")
        textanalytics_multi_label_classify_deployment_name = kwargs.pop(
            "textanalytics_multi_label_classify_deployment_name"
        )
        ...
        client = TextAnalyticsClient(
            textanalytics_custom_text_endpoint, AzureKeyCredential(textanalytics_custom_text_key), api_version="v3.1"
        )
        ...
        assert (
            str(e.value) == f"'RecognizeCustomEntitiesAction' is not available in API version v3.1. "
    ...
    def test_cancel_fail_terminal_state(self, client):
        single_doc = "A recent report by the Government Accountability Office (GAO) found that the dramatic increase in oil and natural gas development on federal lands over the past six years has stretched the staff of the BLM to a point that it has been unable to meet its environmental protection responsibilities."
        docs = [
            {"id": str(idx), "text": val} for (idx, val) in enumerate(list(itertools.repeat(single_doc, 20)))
        ]  # max number of documents is 25
        actions = [
            RecognizeEntitiesAction(),
            ExtractKeyPhrasesAction(),
