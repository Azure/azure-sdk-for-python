...
class TestAnalyzeAsync(TextAnalyticsTest):
        ...
        async with client:
            response = await (
                await client.begin_analyze_actions(
                    docs, actions=[ExtractKeyPhrasesAction()], show_stats=True, polling_interval=self._interval()
                )
            ).result()
    ...
    async def test_all_successful_passing_string_pii_entities_task(self, client):
        ...
        async with client:
            response = await (
                await client.begin_analyze_actions(
                    docs, actions=[RecognizePiiEntitiesAction()], show_stats=True, polling_interval=self._interval()
                )
            ).result()
    ...
    async def test_bad_request_on_empty_document(self, client):
        ...
        with pytest.raises(HttpResponseError):
            async with client:
                await (
                    await client.begin_analyze_actions(
                        docs, actions=[ExtractKeyPhrasesAction()], polling_interval=self._interval()
                    )
                ).result()
    ...
    async def test_pii_action_categories_filter(self, client):
        ...
        actions = [
            RecognizePiiEntitiesAction(
                categories_filter=[PiiEntityCategory.US_SOCIAL_SECURITY_NUMBER, PiiEntityCategory.ABA_ROUTING_NUMBER]
            ),
        ]
        async with client:
            result = await (
                await client.begin_analyze_actions(documents=docs, actions=actions, polling_interval=self._interval())
            ).result()
            action_results = []
            async for p in result:
    ...
    @TextAnalyticsPreparer()
    async def test_generic_action_error_no_target_v3_1(self, **kwargs):
        docs = [
            {
                "id": "1",
                "language": "en",
    ...
    @TextAnalyticsPreparer()
    async def test_generic_action_error_no_target(self, **kwargs):
        docs = [
            {
                "id": "1",
                "language": "en",
    ...
    @TextAnalyticsPreparer()
    async def test_action_errors_with_targets_v3_1(self, **kwargs):
        docs = [
            {
                "id": "1",
                "language": "en",
    ...
    @TextAnalyticsPreparer()
    async def test_action_errors_with_targets(self, **kwargs):
        docs = [
            {
                "id": "1",
                "language": "en",
    ...
    @TextAnalyticsPreparer()
    async def test_action_job_failure_v3_1(self, **kwargs):
        docs = [
            {
                "id": "1",
                "language": "en",
    ...
    @TextAnalyticsPreparer()
    async def test_action_job_failure(self, **kwargs):
        docs = [
            {
                "id": "1",
                "language": "en",
