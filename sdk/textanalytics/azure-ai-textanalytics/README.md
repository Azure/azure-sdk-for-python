...
The `credential` parameter may be provided as a `TextAnalyticsSubscriptionKeyCredential` or as a token from Azure Active Directory.
...
1. To use a [subscription key](https://docs.microsoft.com/azure/cognitive-services/cognitive-services-apis-create-account?tabs=multiservice%2Cwindows#get-the-keys-for-your-resource), 
   pass the key as a string into an instance of `TextAnalyticsSubscriptionKeyCredential("<subscription_key>")`. 
   The subscription key can be found in the Azure Portal or by running the following Azure CLI command:
    ...
    credential = TextAnalyticsSubscriptionKeyCredential("<subscription_key>")
...
text_analytics_client = TextAnalyticsClient(endpoint, TextAnalyticsSubscriptionKeyCredential(key))
