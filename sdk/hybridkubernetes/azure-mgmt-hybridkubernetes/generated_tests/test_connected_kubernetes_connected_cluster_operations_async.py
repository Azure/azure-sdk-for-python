...
class TestConnectedKubernetesConnectedClusterOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_connected_cluster_begin_update_async(self, resource_group):
        response = await (
            await self.client.connected_cluster.begin_update_async(
                resource_group_name=resource_group.name,
                cluster_name="str",
