...
class TestAVSLicensesOperationsAsync(AzureMgmtRecordedTestCase):
    def setup_method(self, method):
        self.client = self.create_mgmt_client(AVSClient, is_async=True)

    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_licenses_list(self, resource_group):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_licenses_get_properties(self, resource_group):
        response = await self.client.licenses.get_properties(
            resource_group_name=resource_group.name,
            private_cloud_name="str",
            license_name="str",
        )
