...
class TestReceivedShares(TestPurviewSharing):
    ...
    def test_get_all_detached_shares(self, purviewsharing_endpoint):
        ...
        request = build_sent_shares_create_or_replace_request(
            sent_share_id, content_type="application/json", content=json.dumps(sent_share)
        )
        ...
        invitation = {
            "invitationKind": "User",
            "properties": {"targetEmail": consumerEmail, "notify": "true", "expirationDate": "2024-01-01 00:00:00"},
        }
        ...
        assert len([x for x in list_detached["value"] if x["properties"]["shareStatus"] == "Detached"]) == len(
            list_detached["value"]
        )
    ...
    def test_attach_received_share(self, purviewsharing_endpoint):
        ...
        store_reference = {"referenceName": consumer_storage_account_resource_id, "type": "ArmResourceReference"}
        ...
        update_request = build_received_shares_create_or_replace_request(
            received_share["id"], content_type="application/json", content=json.dumps(received_share)
        )
    ...
    def test_get_all_attached_shares(self, purviewsharing_endpoint):
        ...
        list_attached_request = build_received_shares_list_attached_request(
            reference_name=consumer_storage_account_resource_id, order_by="properties/createdAt desc"
        )
        ...
        assert len([x for x in list_attached["value"] if x["properties"]["shareStatus"] == "Attached"]) == len(
            list_attached["value"]
        )
