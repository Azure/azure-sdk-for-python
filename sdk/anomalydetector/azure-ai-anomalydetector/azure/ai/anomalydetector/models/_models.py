...
class AlignPolicy(Model):
    ...
    :param align_mode: An optional field, indicates how we align different
     variables into the same time-range which is required by the model.{Inner,
     Outer}. Possible values include: 'Inner', 'Outer'
    :type align_mode: str or ~azure.ai.anomalydetector.models.AlignMode
    :param fill_na_method: An optional field, indicates how missed values will
     be filled with. Can not be set to NotFill, when alignMode is
     Outer.{Previous, Subsequent, Linear, Zero, Fix, NotFill}. Possible values
    ...
    :param padding_value: optional field, only be useful if FillNAMethod is
     set to Pad.
    ...
    _attribute_map = {
        ...
        'fill_na_method': {'key': 'fillNAMethod', 'type': 'FillNAMethod'},
...
class AnomalyContributor(Model):
    """AnomalyContributor.

    :param contribution_score: The higher the contribution score is, the more
     likely the variable to be the root cause of a anomaly.
...
class AnomalyValue(Model):
    ...
    :param contributors: If current timestamp is an anomaly, contributors will
     show potential root cause for thus anomaly. Contributors can help us
     understand why current timestamp has been detected as an anomaly.
    :type contributors:
     list[~azure.ai.anomalydetector.models.AnomalyContributor]
    :param is_anomaly: Required. To indicate whether current timestamp is
     anomaly or not
    :type is_anomaly: bool
    :param severity: Required. anomaly score of the current timestamp, the
     more significant an anomaly is, the higher the score will be
    :type severity: float
    :param score: anomaly score of the current timestamp, the more significant
     an anomaly is, the higher the score will be, score measures global
     significance
    :type score: float
    """
...
class DetectionRequest(Model):
    ...
    :param source: Required. source file link of the input variables, each
     variable will be a csv with two columns, the first column will be
     timestamp, the second column will be value.Besides these variable csv
     files, a extra meta.json can be included in th zip file if you would like
     to rename a variable.Be default, the file name of the variable will be
     used as the variable name. The variables used in detection should be
     consistent with variables in the model used for detection.
    :type source: str
    :param start_time: Required. A require field, start time of data be used
     for detection, should be date-time.
    :type start_time: datetime
    :param end_time: Required. A require field, end time of data be used for
...
class DetectionResultSummary(Model):
    ...
    :param status: Required. Multivariate anomaly detection status. Possible
     values include: 'CREATED', 'RUNNING', 'READY', 'FAILED'
...
class ModelInfo(Model):
    ...
    :param sliding_window: An optional field, indicates how many history
     points will be used to determine the anomaly score of one subsequent
     point.
    :type sliding_window: int
    :param align_policy: An optional field, since those multivariate need to
     be aligned in the same timestamp before starting the detection.
    :type align_policy: ~azure.ai.anomalydetector.models.AlignPolicy
    :param source: Required. source file link of the input variables, each
     variable will be a csv with two columns, the first column will be
     timestamp, the second column will be value.Besides these variable csv
     files, an extra meta.json can be included in th zip file if you would like
     to rename a variable.Be default, the file name of the variable will be
     used as the variable name.
    :type source: str
    :param start_time: Required. require field, start time of data be used for
     generating multivariate anomaly detection model, should be data-time
    :type start_time: datetime
    :param end_time: Required. require field, end time of data be used for
     generating multivariate anomaly detection model, should be data-time
    :type end_time: datetime
    ...
    :param diagnostics_info: Used for deep analysis model and variables
