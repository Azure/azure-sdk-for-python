# coding=utf-8
# --------------------------------------------------------------------------
# Copyright (c) Microsoft Corporation. All rights reserved.
# Licensed under the MIT License. See License.txt in the project root for license information.
# Code generated by Microsoft (R) Python Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is regenerated.
# --------------------------------------------------------------------------

from azure.identity import DefaultAzureCredential

from azure.mgmt.containerserviceaimanager import ContainerServiceAIManagerClient

"""
# PREREQUISITES
    pip install azure-identity
    pip install azure-mgmt-containerserviceaimanager
# USAGE
    python ai_manager_namespaces_create_or_update.py

    Before run the sample, please set the values of the client ID, tenant ID and client secret
    of the AAD application as environment variables: AZURE_CLIENT_ID, AZURE_TENANT_ID,
    AZURE_CLIENT_SECRET. For more info about how to get the value, please see:
    https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal
"""


def main():
    client = ContainerServiceAIManagerClient(
        credential=DefaultAzureCredential(),
        subscription_id="SUBSCRIPTION_ID",
    )

    response = client.ai_manager_namespaces.begin_create_or_update(
        resource_group_name="rg1",
        ai_manager_name="aimanager1",
        namespace_name="namespace1",
        resource={"properties": {"annotations": {"note": "example"}, "labels": {"app": "myapp"}}},
    ).result()
    print(response)


# x-ms-original-file: 2026-05-02-preview/AIManagerNamespaces_CreateOrUpdate.json
if __name__ == "__main__":
    main()
