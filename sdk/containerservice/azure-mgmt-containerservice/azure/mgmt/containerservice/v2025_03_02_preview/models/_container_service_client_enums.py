...
class AdvancedNetworkPolicies(str, Enum, metaclass=CaseInsensitiveEnumMeta):
    """This allows users to configure Layer 7 network policies (FQDN, HTTP, Kafka). Policies
...
class NodeProvisioningDefaultNodePools(str, Enum, metaclass=CaseInsensitiveEnumMeta):
    """This field has no effect unless mode is 'Auto'. Warning: Changing this from Auto to None on an
    existing cluster will cause the default Karpenter NodePools to be deleted, which will in turn
    drain and delete the nodes associated with those pools. It is strongly recommended to not do
    this unless there are idle nodes ready to take the pods evicted by that action. If not
    specified, the default is Auto. For more information see aka.ms/something.
    """

    NONE = "None"
    """No Karpenter NodePools are provisioned automatically. Automatic scaling will not happen unless
    the user creates one or more NodePool instances."""
    AUTO = "Auto"
