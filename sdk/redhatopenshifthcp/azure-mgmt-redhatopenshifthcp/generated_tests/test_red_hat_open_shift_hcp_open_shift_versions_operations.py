...
class TestRedHatOpenShiftHcpOpenShiftVersionsOperations(AzureMgmtRecordedTestCase):
    ...
    @recorded_by_proxy
    def test_hcp_open_shift_versions_get(self, resource_group):
        response = self.client.hcp_open_shift_versions.get(
            location="str",
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy
    def test_hcp_open_shift_versions_list(self, resource_group):
        response = self.client.hcp_open_shift_versions.list(
            location="str",
        )
