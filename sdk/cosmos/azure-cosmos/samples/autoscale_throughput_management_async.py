...
async def read_autoscale_throughput(database, container):
    ...
    try:
        ...
        if autopilot_settings:
            ...
            print(
                f"  - Increment percent: {autopilot_settings.get('autoUpgradePolicy', {}).get('throughputPolicy', {}).get('incrementPercent', 0)}"
            )
        else:
...
async def update_autoscale_max_throughput(container, new_max_throughput):
    ...
    try:
        ...
        new_throughput = ThroughputProperties(
            auto_scale_max_throughput=new_max_throughput, auto_scale_increment_percent=0
        )
