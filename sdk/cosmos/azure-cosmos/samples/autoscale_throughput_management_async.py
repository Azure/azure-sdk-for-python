...
async def migrate_to_autoscale(container):
    """
    Migrate a container from manual throughput to autoscale asynchronously.
    ...
    """
    print("\nMigrate from Manual to Autoscale Throughput (Async)")
    print("=" * 70)
    
    try:
        # Check current throughput type
        current_offer = await container.get_throughput()
    ...
    try:
        ...
        for container, offer in zip(valid_containers, offers):
            if not isinstance(offer, Exception):
                autopilot_settings = offer.properties['content']['offerAutopilotSettings']
                max_throughput = autopilot_settings.get('maxThroughput')
...
async def run_sample():
    ...
    async with CosmosClient(HOST, {'masterKey': MASTER_KEY}) as client:
        try:
            ...
            manual_container = await database.create_container(
                id=CONTAINER_ID + '_manual_async',
                partition_key=PartitionKey(path='/id'),
            ...
            await migrate_to_autoscale(manual_container)
            
            # 8. Read autoscale settings after migration
            await read_autoscale_throughput(database, manual_container)
            
            # 9. Demonstrate concurrent operations
            await demonstrate_concurrent_throughput_operations(database)
            
            # Cleanup manual container
            await database.delete_container(manual_container.id)
