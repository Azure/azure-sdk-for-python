...
def create_client_with_custom_user_agent():
    ...
    client = cosmos_client.CosmosClient(HOST, {"masterKey": MASTER_KEY}, user_agent_suffix=custom_user_agent)
...
def create_client_with_detailed_user_agent():
    ...
    client = cosmos_client.CosmosClient(HOST, {"masterKey": MASTER_KEY}, user_agent_suffix=detailed_user_agent)
...
def create_multiple_clients_with_different_user_agents():
    ...
    ingestion_client = cosmos_client.CosmosClient(
        HOST, {"masterKey": MASTER_KEY}, user_agent_suffix="DataIngestionService/1.0.0"
    )
    print("Created client for data ingestion: DataIngestionService/1.0.0")

    # Client for query service
    query_client = cosmos_client.CosmosClient(HOST, {"masterKey": MASTER_KEY}, user_agent_suffix="QueryService/1.0.0")
    ...
    analytics_client = cosmos_client.CosmosClient(
        HOST, {"masterKey": MASTER_KEY}, user_agent_suffix="AnalyticsService/1.0.0"
    )
...
def demonstrate_user_agent_in_operations(client, custom_suffix="DemoClient/1.0.0"):
    ...
    try:
        ...
        item = {"id": "sample_item_1", "name": "Sample Item", "description": "Created with custom user agent"}
...
def demonstrate_operations_with_multiple_user_agents():
    ...
    def operation_with_user_agent(user_agent_suffix, operation_name):
        ...
        client = cosmos_client.CosmosClient(HOST, {"masterKey": MASTER_KEY}, user_agent_suffix=user_agent_suffix)
        db = client.create_database_if_not_exists(id=DATABASE_ID)
