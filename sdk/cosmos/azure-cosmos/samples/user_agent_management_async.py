...
async def create_client_with_custom_user_agent():
    ...
    async with CosmosClient(HOST, {"masterKey": MASTER_KEY}, user_agent_suffix=custom_user_agent) as client:
...
async def create_client_with_detailed_user_agent():
    ...
    async with CosmosClient(HOST, {"masterKey": MASTER_KEY}, user_agent_suffix=detailed_user_agent) as client:
...
async def demonstrate_user_agent_in_operations():
    ...
    async with CosmosClient(HOST, {"masterKey": MASTER_KEY}, user_agent_suffix=custom_suffix) as client:
        try:
            # Create database
            ...
            query_iterable = container.query_items(
                query="SELECT * FROM c WHERE c.id = @id", parameters=[{"name": "@id", "value": "async_sample_item_1"}]
            )
...
async def demonstrate_concurrent_operations_with_user_agents():
    ...
    async def operation_with_user_agent(user_agent_suffix, operation_name):
        ...
        async with CosmosClient(HOST, {"masterKey": MASTER_KEY}, user_agent_suffix=user_agent_suffix) as client:
            db = await client.create_database_if_not_exists(id=DATABASE_ID)
