...
def obtain_client():
    return cosmos_client.CosmosClient(HOST, MASTER_KEY, "Session", connection_verify=CA_CERT_FILE)
...
def query_documents_with_custom_query(
...
def range_scan_on_hash_index(db):
    ...
    try:
        ...
        query_documents_with_custom_query(created_Container, query)
        results = list(
            created_Container.query_items(query, enable_scan_in_query=True, enable_cross_partition_query=True)
        )
...
def perform_multi_orderby_query(db):
    try:
        ...
        # Insert some documents
        doc1 = created_container.create_item(
...
def use_full_text_policy(db):
    try:
        ...
        full_text_policy = {"defaultLanguage": "en-US", "fullTextPaths": [{"path": "/text1", "language": "en-US"}]}
