...
async def query_items(container, doc_id):
    ...
    query_items_response = container.query_items(
        query="SELECT * FROM r WHERE r.id=@id", parameters=[{"name": "@id", "value": doc_id}]
    )
...
async def query_items_with_continuation_token(container):
    ...
    second_page_items_with_continuation = [
        i async for i in await anext(items_from_continuation)
    ]  # type: ignore[name-defined]  # cspell:disable-line
...
async def replace_item_using_etags(container, doc_id):
    ...
    read_item["subtotal"] = read_item["subtotal"] + 1
    response = await container.replace_item(read_item, read_item, if_match=item_etag)

    print("Replaced Item's Id is {0}, new subtotal={1}".format(response["id"], response["subtotal"]))

    read_item = await container.read_item(item=doc_id, partition_key=doc_id)
    read_item["subtotal"] = read_item["subtotal"] + 1
    response = await container.replace_item(read_item, read_item, if_none_match="some-etag")
...
async def patch_item(container, doc_id):
    ...
    print(
        ...
        "move from path freight={6} to path service_addition={7}".format(
            response["id"],
...
async def query_mh_items(container, pk):
    ...
    query_items_response = container.query_items(
        ...
        parameters=[{"name": "@account_number", "value": pk[0]}, {"name": "@purchase_order_number", "value": pk[1]}],
    )
...
async def query_items_with_continuation_token_size_limit(container, doc_id):
    ...
    items = container.query_items(
        query="SELECT * FROM r", partition_key=doc_id, continuation_token_limit=size_limit_in_kb
    )
...
async def run_sample():
    async with CosmosClient(HOST, {"masterKey": MASTER_KEY}) as client:
        try:
            ...
            container_multi_hash = await db.create_container_if_not_exists(
                id=CONTAINER_MH_ID,
