...
def create_nonpartitioned_container(db):
    ...
    resource_type = "docs"
    data = json.dumps(get_sales_order('SalesOrder0'))
    ...
    if not six.PY2:
        ...
        data = data.decode('utf-8')
    data = json.loads(data)
    return created_container, "SalesOrder0"
...
async def read_item(container, doc_id):
    ...
    response = await container.read_item(doc_id, partition_key=partition_key.NonePartitionKeyValue)
...
async def read_items(container):
    ...
    item_list = [item async for item in container.read_all_items(max_item_count=10)]
...
async def replace_item(container, doc_id):
    ...
    read_item = await container.read_item(doc_id, partition_key=partition_key.NonePartitionKeyValue)
...
async def delete_item(container, doc_id):
    ...
    await container.delete_item(item=doc_id, partition_key=partition_key.NonePartitionKeyValue)
