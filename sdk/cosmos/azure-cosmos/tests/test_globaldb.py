...
def _mock_pipeline_run_function_error(pipeline_client, request, **kwargs):
    response = test_config.FakePipelineResponse(
        headers={"x-ms-substatus": SubStatusCodes.WRITE_FORBIDDEN},
...
def _mock_get_database_account(url_connection=None, **kwargs):
    ...
    database_account._ReadableLocations = [{"databaseAccountEndpoint": TestGlobalDB.host, "name": "West US 2"}]
...
class TestGlobalDB(unittest.TestCase):
    ...
    def test_global_db_read_write_endpoints(self):
        ...
        client = cosmos_client.CosmosClient(
            TestGlobalDB.host, TestGlobalDB.masterKey, connection_policy=connection_policy
        )

        document_definition = {"id": "doc", "pk": "pk", "name": "sample document", "key": "value"}
    ...
    def test_global_db_endpoint_discovery(self):
        ...
        read_location_client = cosmos_client.CosmosClient(
            self.read_location_host, self.masterKey, connection_policy=connection_policy
        )

        document_definition = {"id": "doc1", "name": "sample document", "key": "value"}
        ...
        if is_not_default_host(self.read_location_host):
            self.__AssertHTTPFailureWithStatus(
                StatusCodes.FORBIDDEN, SubStatusCodes.WRITE_FORBIDDEN, container.create_item, document_definition
            )

        # Query databases will pass for the read location client as it's a GET operation
        list(
            read_location_client.query_databases(
                query="SELECT * FROM root r WHERE r.id=@id", parameters=[{"name": "@id", "value": self.test_db.id}]
            )
        )
    ...
    def test_global_db_preferred_locations(self):
        ...
        document_definition = {"id": "doc3", "pk": "pk", "name": "sample document", "key": "value"}
    ...
    def test_global_db_endpoint_assignments(self):
        ...
        self.assertEqual(client.client_connection.WriteEndpoint, client.client_connection.ReadEndpoint)
        if is_not_default_host(self.write_location_host):
            self.assertEqual(client.client_connection.WriteEndpoint, self.write_location_host)
        ...
        if is_not_default_host(self.read_location2):
            ...
            # Test that the preferred location is set as ReadEndpoint instead of default write endpoint when no preference is set
            self.assertEqual(client.client_connection._global_endpoint_manager.WriteEndpoint, self.write_location_host)
            self.assertEqual(client.client_connection._global_endpoint_manager.ReadEndpoint, self.read_location2_host)

    def test_global_db_update_locations_cache(self):
        client = cosmos_client.CosmosClient(self.host, self.masterKey)

        writable_locations = [{"name": self.write_location, "databaseAccountEndpoint": self.write_location_host}]
        readable_locations = [
            {"name": self.read_location, "databaseAccountEndpoint": self.read_location_host},
            {"name": self.read_location2, "databaseAccountEndpoint": self.read_location2_host},
        ]

        if (
            is_not_default_host(self.write_location_host)
            ...
            write_endpoint, read_endpoint = (
                client.client_connection._global_endpoint_manager.location_cache.update_location_cache(
            ...
            writable_locations = []
            readable_locations = [{"name": self.read_location, "databaseAccountEndpoint": self.read_location_host}]
            ...
            writable_locations = [
                {"name": self.write_location, "databaseAccountEndpoint": self.write_location_host},
                {"name": self.read_location2, "databaseAccountEndpoint": self.read_location2_host},
            ]
            readable_locations = [{"name": self.read_location, "databaseAccountEndpoint": self.read_location_host}]
    ...
    def test_global_db_endpoint_discovery_retry_policy_mock(self):
        ...
        with patch.object(_synchronized_request, "_PipelineRunFunction", new=_mock_pipeline_run_function_error):
            ...
            document_definition = {"id": "doc7", "pk": "pk", "name": "sample document", "key": "value"}

            max_retry_attempt_count = 10
            retry_after_in_milliseconds = 500
            _endpoint_discovery_retry_policy.EndpointDiscoveryRetryPolicy.Max_retry_attempt_count = (
