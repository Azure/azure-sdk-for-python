...
async def _read_and_assert(
    container: ContainerProxy, doc_id: str, pk_field: Optional[str] = "pk", pk_value: Any = None
) -> None:
    item = await container.read_item(item=doc_id, partition_key=pk_value)
    _assert_pk(item, pk_field)


async def _query_and_assert_single(
    container: ContainerProxy, pk_field: Optional[str] = "pk", pk_value: Any = None
) -> None:
...
async def _change_feed_and_assert(
    container: ContainerProxy, pk_field: Optional[str] = "pk", pk_value: Any = None
) -> None:
...
async def setup_async():
    ...
    yield {DATABASE: created_db, COLLECTION: created_db.get_container_client(TestPartitionKeyAsync.TEST_CONTAINER_ID)}
    await test_client.close()


async def _assert_no_conflicts(container: ContainerProxy, pk_value: PkField) -> None:
    conflict_definition: ItemDict = {
        "id": "new conflict",
        "resourceId": "doc1",
    ...
    if pk_value:
        conflicts_iter = container.query_conflicts(
...
async def _perform_operations_on_pk(created_container, pk_field, pk_value):
    ...
    items_iter = created_container.query_items(query="SELECT * FROM c", feed_range=feed_range)
...
class TestPartitionKeyAsync:
    ...
    async def test_multi_partition_collection_read_document_with_no_pk_async(self, setup_async, version) -> None:
        ...
        created_container = await setup_async[DATABASE].create_container_if_not_exists(
            id="container_with_no_pk" + str(uuid.uuid4()), partition_key=PartitionKey(path="/pk", kind="Hash")
        )
        try:
    ...
    async def test_with_null_pk_async(self, setup_async, version) -> None:
        ...
        created_container = await setup_async[DATABASE].create_container_if_not_exists(
            id="container_with_nul_pk" + str(uuid.uuid4()), partition_key=PartitionKey(path="/pk", kind="Hash")
        )
        try:
    ...
    async def test_hash_v1_partition_key_definition_async(self, setup_async) -> None:
        created_container = await setup_async[DATABASE].create_container(
