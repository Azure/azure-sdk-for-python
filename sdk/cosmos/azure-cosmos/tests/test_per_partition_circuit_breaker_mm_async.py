...
async def perform_write_operation(operation, container, fault_injection_container, doc_id, pk, expected_uri=None):
    ...
    if operation == CREATE:
        ...
        resp = await fault_injection_container.patch_item(
...
class TestPerPartitionCircuitBreakerMMAsync:
    ...
    async def setup_method_with_custom_transport(
        self, custom_transport: Union[AioHttpTransport, Any], default_endpoint=host, **kwargs
    ):
    ...
    async def setup_info(self, error, **kwargs):
        ...
        custom_setup = await self.setup_method_with_custom_transport(
            custom_transport, default_endpoint=self.host, **kwargs
        )
    ...
    async def test_read_consecutive_failure_threshold_async(self, read_operation, error):
        ...
        await perform_read_operation(read_operation, fault_injection_container, doc["id"], doc["pk"], expected_uri)
        ...
        try:
            ...
            for i in range(num_operations):
                ...
                await perform_read_operation(
                    read_operation, fault_injection_container, doc["id"], PK_VALUE, expected_uri
                )
            if read_operation in (CHANGE_FEED, QUERY, READ_ALL_ITEMS):
                # these operations are cross partition so they would mark both partitions as unavailable
                expected_unhealthy_partitions = 5
    ...
    async def test_stat_reset_async(self):
        status_code = 500
        error_lambda = lambda r: asyncio.create_task(
        ...
        try:
            for i in range(2):
                validate_unhealthy_partitions(global_endpoint_manager, 0)
                # read will fail and retry in other region
                await perform_read_operation(READ, fault_injection_container, doc["id"], PK_VALUE, expected_uri)
                try:
                    await perform_write_operation(
    ...
    async def test_service_request_error_async(self, read_operation, write_operation):
        ...
        fault_injection_container = custom_setup["col"]
        await container.upsert_item(body=doc)
        await perform_read_operation(read_operation, fault_injection_container, doc["id"], PK_VALUE, expected_uri)
        global_endpoint_manager = fault_injection_container.client_connection._global_endpoint_manager
        ...
        custom_transport.add_fault(
            predicate, lambda r: asyncio.create_task(FaultInjectionTransportAsync.error_region_down())
        )
    ...
    async def test_recovering_only_fails_one_requests_async(self):
        error_lambda = lambda r: asyncio.create_task(
            FaultInjectionTransportAsync.error_after_delay(
                0, CosmosHttpResponseError(status_code=502, message="Some envoy error.")
            )
        )
        setup, doc, expected_uri, uri_down, custom_setup, custom_transport, predicate = await self.setup_info(
            error_lambda
        ...
        async def concurrent_upsert():
            ...
            doc = {"id": str(uuid.uuid4()), "pk": PK_VALUE, "name": "sample document", "key": "value"}
            try:
