...
class TestComputedPropertiesQueryAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.computed_properties = [
            {"name": "cp_lower", "query": "SELECT VALUE LOWER(c.db_group) FROM c"},
    ...
    async def computedPropertiesTestCases(self, created_collection):
        ...
        queried_items = [
            q
            async for q in created_collection.query_items(
                query='Select * from c Where c.cp_upper = "GROUP2"', partition_key="test"
            )
        ]
        ...
        queried_items = [
            q
            async for q in created_collection.query_items(
                query='Select * from c Where c.cp_lower = "group1"', partition_key="test"
            )
        ]
        ...
        queried_items = [
            q
            async for q in created_collection.query_items(
                query='Select * from c Where c.cp_lower = "group4"', partition_key="test"
            )
        ]
        assert len(queried_items) == 0
        ...
        queried_items = [
            q
            async for q in created_collection.query_items(
                query="Select * from c Where c.cp_power = 25", partition_key="test"
            )
        ]
        ...
        queried_items = [
            q
            async for q in created_collection.query_items(
                query="Select * from c Where c.cp_power = 16", partition_key="test"
            )
        ]
        assert len(queried_items) == 0
        ...
        queried_items = [
            q
            async for q in created_collection.query_items(
                query="Select * from c Where c.cp_str_len = 9", partition_key="test"
            )
        ]
        ...
        queried_items = [
            q
            async for q in created_collection.query_items(
                query="Select * from c Where c.cp_str_len = 3", partition_key="test"
            )
        ]
        assert len(queried_items) == 0
    ...
    async def test_replace_with_new_computed_properties_async(self):
        ...
        new_computed_properties = [
            {"name": "cp_upper", "query": "SELECT VALUE UPPER(c.db_group) FROM c"},
        ...
        queried_items = [
            q
            async for q in replaced_collection.query_items(
                query='Select * from c Where c.cp_upper = "GROUP2"', partition_key="test"
            )
        ]
        ...
        queried_items = [
            q
            async for q in replaced_collection.query_items(
                query="Select * from c Where c.cp_len = 9", partition_key="test"
            )
        ]
    ...
    async def test_replace_with_remove_computed_properties_async(self):
        ...
        # Replace Container
        replaced_collection = await self.created_db.replace_container(
            container=created_collection.id, partition_key=PartitionKey(path="/pk")
        )
