...
async def setup():
    ...
    created_container = await database.create_container_if_not_exists(
        id=config.TEST_SINGLE_PARTITION_CONTAINER_ID, partition_key=PartitionKey(path="/pk")
    )
    container_hpk = await database.create_container_if_not_exists(
        id=HPK_CONTAINER, partition_key=PartitionKey(path=["/state", "/city"], kind="MultiHash")
    )
...
class TestRetryableWritesAsync:
    ...
    async def test_retryable_writes_request_level_async(self, setup):
        ...
        read_item_create = await container.read_item(item=test_item_create["id"], partition_key=test_item_create["pk"])
        ...
        test_patch_operations = [{"op": "add", "path": "/data", "value": "patched retryable write test"}]
        try:
            await container.patch_item(
                item=test_item_create["id"],
        ...
        try:
            await container.replace_item(
                item=test_item_replace["id"],
                body={"id": test_item_replace["id"], "pk": "test", "data": "original data"},
            )
            pytest.fail("Expected an exception without retry_write")
        except exceptions.CosmosHttpResponseError:
        ...
        try:
            await container.replace_item(
                item=test_item_replace["id"],
                body={"id": test_item_replace["id"], "pk": "test", "data": "replaced data"},
                retry_write=2,
            )
        except exceptions.CosmosHttpResponseError as e:
            assert me.counter > 1, "Expected multiple retries due to simulated errors"
        ...
        read_item_replace = await container.read_item(
            item=test_item_replace["id"], partition_key=test_item_replace["pk"]
        )
    ...
    async def test_retryable_writes_client_level_write(self, setup):
        ...
        try:
            ...
            try:
                await container.patch_item(
                    item=test_item["id"], partition_key=test_item["pk"], patch_operations=test_patch_operations
                )
    ...
    async def test_retryable_writes_hpk_request_level_async(self, setup):
        ...
        read_item = await container.read_item(
            item=test_item["id"], partition_key=[test_item["state"], test_item["city"]]
        )
        assert read_item["id"] == test_item["id"], "Item was not written successfully after retries"
        ...
        # Verify the item was created
        read_item_create = await container.read_item(
            item=test_item_create["id"], partition_key=[test_item_create["state"], test_item_create["city"]]
        )
        assert read_item_create["id"] == test_item_create["id"], "Item was not created successfully after retries"
        ...
        test_patch_operations = [{"op": "add", "path": "/year", "value": "2025"}]
        # Test without retry_write for patch_item
        try:
            await container.patch_item(
                item=test_item["id"],
        ...
        # Verify the item was patched
        patched_item = await container.read_item(
            item=test_item["id"], partition_key=[test_item["state"], test_item["city"]]
        )
        ...
        try:
            await container.delete_item(
                item=test_item["id"], partition_key=[test_item["state"], test_item["city"]], retry_write=2
            )
        except exceptions.CosmosHttpResponseError:
            assert me.counter > 1, "Expected multiple retries due to simulated errors"
        finally:
        ...
        try:
            await container.replace_item(
                item=test_item_replace["id"],
                body={"id": test_item_replace["id"], "state": "TX", "city": "Austin", "data": "original data"},
            )
            pytest.fail("Expected an exception without retry_write")
        except exceptions.CosmosHttpResponseError:
            assert me.counter == 1, "Expected no retries for replace_item without retry_write"
        ...
        try:
            await container.replace_item(
                item=test_item_replace["id"],
                body={"id": test_item_replace["id"], "state": "TX", "city": "Austin", "data": "replaced data"},
                retry_write=2,
            )
        except exceptions.CosmosHttpResponseError as e:
            assert me.counter > 1, "Expected multiple retries due to simulated errors"
        ...
        # Verify the item was replaced
        read_item_replace = await container.read_item(
            item=test_item_replace["id"], partition_key=[test_item_replace["state"], test_item_replace["city"]]
        )
        assert read_item_replace["data"] == "replaced data", "Item was not replaced successfully after retries"
    ...
    async def test_retryable_writes_hpk_client_level_async(self, setup):
        ...
        try:
            ...
            # Test patch_item
            me.counter = 0  # Reset counter
            test_patch_operations_hpk = [{"op": "add", "path": "/data", "value": "patched retryable write test"}]
            try:
                await container.patch_item(
                    item=test_item["id"],
                    partition_key=[test_item["state"], test_item["city"]],
    ...
    @pytest.mark.parametrize(
        "injected_error",
        [
            FaultInjectionTransportAsync.error_request_timeout(),
    ...
    async def test_retryable_writes_mrw_request_level_async(self, injected_error):
        ...
        is_request_to_first_region: Callable[[HttpRequest], bool] = (
            lambda r: FaultInjectionTransportAsync.predicate_targets_region(r, first_region_uri)
            and FaultInjectionTransportAsync.predicate_is_document_operation(r)
        )

        custom_transport.add_fault(
            is_request_to_first_region, lambda r: asyncio.create_task(FaultInjectionTransportAsync.error_region_down())
        )
        ...
        initialized_objects = await self.setup_method_with_custom_transport(
            custom_transport, preferred_locations=["First Region", "Second Region"], multiple_write_locations=True
        )
        try:
            ...
            replace_document = await container.replace_item(
                item=document_definition["id"], body=document_definition, retry_write=2
            )
            ...
            patch_document = await container.patch_item(
                item=document_definition["id"],
    ...
    async def test_retryable_writes_mrw_client_level_async(self, injected_error):
        ...
        try:
            ...
            try:
                await container.patch_item(
                    item=document_definition["id"], partition_key=document_definition["pk"], patch_operations=operations
                )
    ...
    class MockExecuteFunction(object):
        ...
        async def __call__(self, func, *args, **kwargs):
            ...
            if self.counter < 2:
                ...
                raise exceptions.CosmosHttpResponseError(status_code=408, message="Simulated timeout error.")
            else:
