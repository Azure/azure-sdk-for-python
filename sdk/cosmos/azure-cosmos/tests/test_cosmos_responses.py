...
class TestCosmosResponses(unittest.TestCase):
    ...
    def test_point_operation_headers(self):
        container = self.test_database.create_container(
            id="responses_test" + str(uuid.uuid4()), partition_key=PartitionKey(path="/company")
        )
    ...
    def test_create_database_if_not_exists_headers(self):
        first_response = self.client.create_database_if_not_exists(
            id="responses_test" + str(uuid.uuid4()), return_properties=True
        )
        assert len(first_response[1].get_response_headers()) > 0
    ...
    def test_create_container_headers(self):
        first_response = self.test_database.create_container(
            id="responses_test" + str(uuid.uuid4()), partition_key=PartitionKey(path="/company"), return_properties=True
        )
        assert len(first_response[1].get_response_headers()) > 0
    ...
    def test_create_container_if_not_exists_headers_negative(self):
        first_response = self.test_database.create_container_if_not_exists(
            id="responses_test", partition_key=PartitionKey(path="/company"), return_properties=True
        )
    ...
    def test_replace_container_headers(self):
        ...
        second_response = self.test_database.replace_container(
            first_response.id, partition_key=PartitionKey(path="/company"), return_properties=True
        )
        assert len(second_response[1].get_response_headers()) > 0
    ...
    def test_container_replace_throughput(self):
        container = self.test_database.create_container(
            id="responses_test" + str(uuid.uuid4()), partition_key=PartitionKey(path="/company"), offer_throughput=400
        )
