...
class TestCosmosResponses(unittest.TestCase):
    ...
    def test_point_operation_headers(self):
        ...
        first_response = container.upsert_item({"id": str(uuid.uuid4()), "company": "Microsoft"}, assert_kwarg_passthrough=True)
        lsn = first_response.get_response_headers()['lsn']

        create_response = container.create_item({"id": str(uuid.uuid4()), "company": "Microsoft"}, assert_kwarg_passthrough=True)
        ...
        read_response = container.read_item(create_response['id'], create_response['company'], assert_kwarg_passthrough=True)
        assert len(read_response.get_response_headers()) > 0

        upsert_response = container.upsert_item({"id": str(uuid.uuid4()), "company": "Microsoft"}, assert_kwarg_passthrough=True)
        ...
        replace_response = container.replace_item(upsert_response['id'], upsert_response, assert_kwarg_passthrough=True)
    ...
    def test_create_database_headers(self):
        first_response = self.client.create_database(id="responses_test" + str(uuid.uuid4()), return_properties=True, assert_kwarg_passthrough=True)

        assert len(first_response[1].get_response_headers()) > 0

    def test_create_database_returns_database_proxy(self):
        first_response = self.client.create_database(id="responses_test" + str(uuid.uuid4()), assert_kwarg_passthrough=True)
        assert isinstance(first_response, DatabaseProxy)

    def test_create_database_if_not_exists_headers(self):
        first_response = self.client.create_database_if_not_exists(id="responses_test" + str(uuid.uuid4()), return_properties=True, assert_kwarg_passthrough=True)
        assert len(first_response[1].get_response_headers()) > 0

    def test_create_database_if_not_exists_headers_negative(self):
        first_response = self.client.create_database_if_not_exists(id="responses_test", return_properties=True, assert_kwarg_passthrough=True)
        second_response = self.client.create_database_if_not_exists(id="responses_test", return_properties=True, assert_kwarg_passthrough=True)
    ...
    def test_create_container_headers(self):
        first_response = self.test_database.create_container(id="responses_test" + str(uuid.uuid4()),
                                                             partition_key=PartitionKey(path="/company"), return_properties=True, assert_kwarg_passthrough=True)
        assert len(first_response[1].get_response_headers()) > 0
    ...
    def test_database_read_headers(self):
        db = self.client.create_database(id="responses_test" + str(uuid.uuid4()), assert_kwarg_passthrough=True)
        first_response = db.read(assert_kwarg_passthrough=True)
        assert len(first_response.get_response_headers()) > 0

    def test_container_read_headers(self):
        container = self.test_database.create_container(id="responses_test" + str(uuid.uuid4()),
                                                             partition_key=PartitionKey(path="/company"), assert_kwarg_passthrough=True)
        first_response = container.read(assert_kwarg_passthrough=True)
        assert len(first_response.get_response_headers()) > 0

    def test_container_replace_throughput(self):
        container = self.test_database.create_container(id="responses_test" + str(uuid.uuid4()),
                                                        partition_key=PartitionKey(path="/company"), offer_throughput=400, assert_kwarg_passthrough=True)
        replace_throughput_value = 500
        first_response = container.replace_throughput(replace_throughput_value, assert_kwarg_passthrough=True)
        assert len(first_response.get_response_headers()) > 0
        assert replace_throughput_value == container.get_throughput(assert_kwarg_passthrough=True).offer_throughput

    def test_database_replace_throughput(self):
        db = self.client.create_database(id="responses_test" + str(uuid.uuid4()), offer_throughput=400, assert_kwarg_passthrough=True)
        replace_throughput_value = 500
        first_response = db.replace_throughput(replace_throughput_value, assert_kwarg_passthrough=True)
        assert len(first_response.get_response_headers()) > 0
        assert replace_throughput_value == db.get_throughput(assert_kwarg_passthrough=True).offer_throughput
