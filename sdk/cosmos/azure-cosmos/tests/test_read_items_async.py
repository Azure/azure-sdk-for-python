...
class TestReadItemsAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.container = await self.database.create_container(
            id="read_container_" + str(uuid.uuid4()), partition_key=PartitionKey(path="/id")
        )
    ...
    def _setup_fault_injection_async(self, error_to_inject, inject_once=False):
        ...
        def predicate(request):
            ...
            is_query = request.method == "POST" and FaultInjectionTransportAsync.predicate_is_operation_type(
                request, _OperationType.SqlQuery
            )
            if is_query:
                if inject_once:
    ...
    async def test_read_items_different_partition_key_async(self):
        ...
        container_pk = await self.database.create_container(
            id="read_pk_container_" + str(uuid.uuid4()), partition_key=PartitionKey(path="/pk")
        )
        try:
            items_to_read = []
            item_ids = []
    ...
    async def test_read_items_fails_with_incomplete_hierarchical_pk_async(self):
        ...
        try:
            ...
            self.assertIn(
                "Number of components in partition key value (1) does not match definition (2)", str(context.exception)
            )
        finally:
    ...
    async def test_read_items_preserves_input_order_async(self):
        ...
        container_pk = await self.database.create_container(
            id="read_order_container_" + str(uuid.uuid4()), partition_key=PartitionKey(path="/pk")
        )
    ...
    async def test_read_items_order_using_zip_comparison_async(self):
        ...
        container_pk = await self.database.create_container(
            id="read_order_zip_async_container_" + str(uuid.uuid4()), partition_key=PartitionKey(path="/pk")
        )
        ...
        try:
            ...
            self.assertTrue(
                all(matching), "Order was not preserved in async version. Input order doesn't match output order."
            )
    ...
    async def test_read_failure_preserves_headers_async(self):
        ...
        error_to_inject = CosmosHttpResponseError(
            message="Simulated query failure with headers", response=mock_response()
        )
    ...
    async def test_read_items_with_throttling_retry_async(self):
        ...
        error_to_inject = CosmosHttpResponseError(
            status_code=429, message="Throttling error injected for testing", headers={"x-ms-retry-after-ms": "10"}
        )
        container_with_faults, client_with_faults = self._setup_fault_injection_async(error_to_inject, inject_once=True)
        try:
    ...
    async def test_read_items_with_gone_retry_async(self):
        ...
        try:
            ...
            with patch(
                "azure.cosmos._gone_retry_policy.PartitionKeyRangeGoneRetryPolicy.ShouldRetry", autospec=True
            ) as mock_should_retry:
                read_items = await container_with_faults.read_items(items=items_to_read)
                mock_should_retry.assert_called_once()
                self.assertEqual(len(read_items), len(item_ids))
    ...
    async def test_read_after_container_recreation_async(self):
        ...
        initial_items_to_read, initial_item_ids = await self._create_records_for_read_items(
            self.container, 3, "initial"
        )
        ...
        self.container = await self.database.create_container(
            id=container_id, partition_key=PartitionKey(path="/pk"), offer_throughput=10100
        )
    ...
    async def test_read_items_multiple_physical_partitions_and_hook_async(self):
        ...
        try:
            ...
            pk_ranges = [
                pk_range
                async for pk_range in multi_partition_container.client_connection._ReadPartitionKeyRanges(
            ...
            for pk_range in pk_ranges:
                ...
                items_in_partition = [
                    item
                    async for item in multi_partition_container.query_items(
                        "SELECT c.id FROM c", partition_key_range_id=pk_range_id
                    )
                ]
            ...
            self.assertGreater(
                len(partition_item_map), 1, "Items were not distributed across multiple physical partitions."
            )
            ...
            read_items_result = await multi_partition_container.read_items(
                items=items_to_read, response_hook=response_hook
            )
