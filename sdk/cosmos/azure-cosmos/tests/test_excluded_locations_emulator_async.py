...
from test_excluded_locations import L1, L2, read_item_test_data, TestDataType, set_test_data_type
from test_excluded_locations_emulator import (
    L1_URL,
    L2_URL,
    get_location,
...
class TestExcludedLocationsEmulatorAsync:
    ...
    async def test_delete_all_items_by_partition_key(
        self: "TestExcludedLocationsEmulatorAsync", test_data: List[List[str]]
    ):
        # Init test variables
        ...
        is_get_account_predicate: Callable[[AioHttpTransport], bool] = (
        ...
        for multiple_write_locations in [True, False]:
            ...
            if request_excluded_locations is None:
                ...
                await container.delete_all_items_by_partition_key(
                    id_value, excluded_locations=request_excluded_locations
                )
    ...
    async def test_metadata_read_with_excluded_locations(
        self: "TestExcludedLocationsEmulatorAsync", test_data: List[List[str]]
    ):
        # Init test variables
        ...
        for target_url, expected_location, expected_count in zip(target_urls, expected_locations, expected_counts):
            ...
            is_request_to_target_region: Callable[[HttpRequest], bool] = lambda r: (
                FaultInjectionTransportAsync.predicate_targets_region(r, target_url)
                and FaultInjectionTransportAsync.predicate_is_resource_type(r, ResourceType.Collection)
                and not FaultInjectionTransportAsync.predicate_is_write_operation(r, target_url)
            )
            fault_factory = lambda r: asyncio.create_task(
                custom_transport.error_with_counter(
