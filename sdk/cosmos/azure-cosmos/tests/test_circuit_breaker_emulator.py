...
class TestCircuitBreakerEmulator:
    ...
    def create_custom_transport_sm_mrr(self):
        ...
        is_write_operation_in_read_region_predicate = lambda r: FaultInjectionTransport.predicate_is_write_operation(
            r, self.host
        )

        custom_transport.add_fault(
            is_write_operation_in_read_region_predicate, lambda r: FaultInjectionTransport.error_write_forbidden()
        )
    ...
    def setup_info(self, error=None, mm=False):
        ...
        doc = create_doc()
        predicate = lambda r: (
            FaultInjectionTransport.predicate_is_resource_type(r, ResourceType.Collection)
            and FaultInjectionTransport.predicate_is_operation_type(r, documents._OperationType.Delete)
            and FaultInjectionTransport.predicate_targets_region(r, uri_down)
        )
        if error is not None:
            custom_transport.add_fault(predicate, error)
        custom_setup = self.setup_method_with_custom_transport(
