...
class TestCRUDOperations(unittest.TestCase):
    ...
    def test_partitioned_collection_document_crud_and_query(self):
        ...
        read_document = created_collection.read_item(
            item=created_document.get("id"), partition_key=created_document.get("pk")
        )
        ...
        new_document_definition = {"id": "document2", "key": "value2", "pk": "pk"}
    ...
    def test_document_crud(self):
        ...
        no_response = created_collection.create_item(
            body=document_definition, enable_automatic_id_generation=True, no_response=True
        )
        ...
        documents = list(created_collection.read_all_items())
        self.assertEqual(
            len(documents), before_create_documents_count + 3, "create should increase the number of documents"
        )
        # query documents
        documents = list(
            created_collection.query_items(
    ...
    def test_read_collection_only_once(self):
        ...
        assert (
            sum(
                "'x-ms-thinclient-proxy-resource-type': 'colls'" in response.message
                for response in mock_handler.messages
            )
            == 1
        )
    ...
    def test_document_upsert(self):
        ...
        document_definition = {"id": "doc", "name": "sample document", "spam": "eggs", "pk": "pk", "key": "value"}
    ...
    def test_authorization(self):
        def __SetupEntities(client):
            ...
            document = collection.create_item(
                body={"id": "doc1", "spam": "eggs", "key": "value"},
            )
        ...
        try:
            cosmos_client.CosmosClient(
                TestCRUDOperations.host, {}, "Session", connection_policy=TestCRUDOperations.connectionPolicy
            )
    ...
    def test_client_request_timeout(self):
        ...
        if not ("localhost" in self.host or "127.0.0.1" in self.host):
            ...
            client = cosmos_client.CosmosClient(
                TestCRUDOperations.host, TestCRUDOperations.masterKey, "Session", connection_policy=connection_policy
            )
    ...
    def test_read_timeout(self):
        ...
        # this succeeds because the timeout is changed for database account call retries
    ...
    def test_timeout_on_read_operation(self):
        ...
        client = cosmos_client.CosmosClient(self.host, self.masterKey, "Session", transport=timeout_transport)
    ...
    def test_timeout_for_read_items(self):
        ...
        pk_ranges = list(created_container.client_connection._ReadPartitionKeyRanges(created_container.container_link))
        ...
        container_with_delay = client_with_delay.get_database_client(self.databaseForTest.id).get_container_client(
        ...
        with self.assertRaises(exceptions.CosmosClientTimeoutError):
            ...
            list(container_with_delay.read_items(items=items_to_read, timeout=5))  # 5 second total timeout
        ...
        items = list(
            container_with_delay.read_items(
                items=items_to_read,
            )
        )
    ...
    def test_timeout_for_paged_request(self):
        ...
        created_container = self.databaseForTest.create_container(
            id="paged_timeout_container_" + str(uuid.uuid4()), partition_key=PartitionKey(path="/pk")
        )
    ...
    def test_timeout_for_point_operation(self):
        ...
        created_container = self.databaseForTest.create_container(
            id="point_op_timeout_container_" + str(uuid.uuid4()), partition_key=PartitionKey(path="/pk")
        )

        # Create a test item
        test_item = {"id": "test_item_1", "pk": "partition1", "data": "test_data"}
        ...
        result = created_container.read_item(item="test_item_1", partition_key="partition1", timeout=3.0)
    ...
    def test_point_operation_read_timeout(self):
        ...
        try:
            container.read_item(item="test_item_1", partition_key="partition1", read_timeout=0.000003)
    ...
    def test_query_operation_single_partition_read_timeout(self):
        ...
        for i in range(500):  # Insert 500 documents
            container.create_item(
                {"id": f"item_{i}", "pk": single_partition_key, "data": large_string, "order_field": i}
            )
    ...
    def test_query_operation_cross_partition_read_timeout(self):
        ...
        large_string = "a" * 1000  # 1KB string
        for i in range(500):  # Insert 500 documents
            container.create_item({"id": f"item_{i}", "pk": i % 2, "data": large_string, "order_field": i})

        pk_ranges = list(container.client_connection._ReadPartitionKeyRanges(container.container_link))
    ...
    def test_patch_operations(self):
        ...
        patched_item = created_container.patch_item(
