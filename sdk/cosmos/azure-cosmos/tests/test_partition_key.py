...
def setup():
    ...
    return {DATABASE: created_db, COLLECTION: created_db.get_container_client(TestPartitionKey.TEST_CONTAINER_ID)}
...
def _assert_no_conflicts(container: ContainerProxy, pk_value: PkField) -> None:
    ...
    if pk_value:
        assert (
            len(
                list(
                    container.query_conflicts(
                        query="SELECT * FROM root r WHERE r.resourceType='"
                        + conflict_definition.get("resourceType")
                        + "'",
...
def _perform_operations_on_pk(created_container, pk_field, pk_value):
    ...
    items_iter = created_container.query_items(
        query="SELECT * FROM c", feed_range=feed_range, enable_cross_partition_query=True
    )
    items_list = list(items_iter)
