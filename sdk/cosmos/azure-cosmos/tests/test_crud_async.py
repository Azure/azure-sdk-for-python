...
class TestCRUDOperationsAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_document_crud_async(self):
        ...
        created_document = await created_collection.create_item(
            body=document_definition, enable_automatic_id_generation=True
        )
    ...
    async def test_authorization_async(self):
        ...
        async with CosmosClient(TestCRUDOperationsAsync.host, TestCRUDOperationsAsync.masterKey) as client:
            # setup entities
        ...
        async with CosmosClient(TestCRUDOperationsAsync.host, resource_tokens) as col_client:
            db = entities["db"]
        ...
        async with CosmosClient(TestCRUDOperationsAsync.host, resource_tokens) as doc_client:
    ...
    async def test_read_collection_only_once_async(self):
        ...
        async with CosmosClient(self.host, self.masterKey) as client:
            ...
            tasks = [container.upsert_item(body={"id": str(uuid.uuid4()), "name": f"sample-{i}"}) for i in range(10)]
    ...
    async def initialize_client_with_connection_urllib_retry_config(self, retries):
        async with CosmosClient(
            ...
            retry_read=retries,
            retry_backoff_max=0.3,
            connection_timeout=0.000000001,
        ) as client:
            databaseForTest = client.get_database_client(self.configs.TEST_DATABASE_ID)
            container = databaseForTest.get_container_client(self.configs.TEST_SINGLE_PARTITION_CONTAINER_ID)
    ...
    async def test_timeout_on_read_operation_async(self):
        ...
        async with CosmosClient(self.host, self.masterKey, transport=timeout_transport) as client:
    ...
    async def test_timeout_for_read_items_async(self):
        ...
        pk_ranges = [
            pk
            async for pk in created_container.client_connection._ReadPartitionKeyRanges(
        ...
        async with CosmosClient(self.host, self.masterKey, transport=delayed_transport) as client_with_delay:
            ...
            with self.assertRaises(exceptions.CosmosClientTimeoutError):
                ...
                await container_with_delay.read_items(items=items_to_read, timeout=5)  # 5 second total timeout
    ...
    async def test_timeout_for_point_operation_async(self):
        ...
        result = await created_container.read_item(
            item="test_item_1", partition_key="partition1", timeout=1.0  # 1 second timeout
        )
    ...
    async def test_query_operation_cross_partition_read_timeout_async(self):
        ...
        for i in range(1000):  # Insert 500 documents
            await container.create_item({"id": f"item_{i}", "pk": i % 2, "data": large_string, "order_field": i})

        pk_ranges = [pk async for pk in container.client_connection._ReadPartitionKeyRanges(container.container_link)]
