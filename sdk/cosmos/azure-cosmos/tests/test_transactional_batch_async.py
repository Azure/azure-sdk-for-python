...
class TestTransactionalBatchAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_invalid_batch_sizes_async(self):
        container = await self.test_database.create_container(
            id="invalid_batch_size_async" + str(uuid.uuid4()), partition_key=PartitionKey(path="/company")
        )
    ...
    async def test_batch_create_async(self):
        container = await self.test_database.create_container(
            id="batch_create_async" + str(uuid.uuid4()), partition_key=PartitionKey(path="/company")
        )
        batch = []
        for i in range(100):
            batch.append(("create", ({"id": "item" + str(i), "company": "Microsoft"},)))
    ...
    async def test_batch_read_async(self):
        container = await self.test_database.create_container(
            id="batch_read_async" + str(uuid.uuid4()), partition_key=PartitionKey(path="/company")
        )
        batch = []
        for i in range(100):
    ...
    async def test_batch_replace_async(self):
        container = await self.test_database.create_container(
            id="batch_replace_async" + str(uuid.uuid4()), partition_key=PartitionKey(path="/company")
        )
        batch = [
    ...
    async def test_batch_upsert_async(self):
        container = await self.test_database.create_container(
            id="batch_upsert_async" + str(uuid.uuid4()), partition_key=PartitionKey(path="/company")
        )
        item_id = str(uuid.uuid4())
        batch = [
            ("upsert", ({"id": item_id, "company": "Microsoft"},)),
    ...
    async def test_batch_patch_async(self):
        container = await self.test_database.create_container(
            id="batch_patch_async" + str(uuid.uuid4()), partition_key=PartitionKey(path="/company")
        )
        item_id = str(uuid.uuid4())
        batch = [
            (
    ...
    async def test_batch_delete_async(self):
        container = await self.test_database.create_container(
            id="batch_delete_async" + str(uuid.uuid4()), partition_key=PartitionKey(path="/company")
        )
        create_batch = []
        delete_batch = []
        for i in range(10):
    ...
    async def test_batch_lsn_async(self):
        container = await self.test_database.create_container(
            id="batch_lsn_async" + str(uuid.uuid4()), partition_key=PartitionKey(path="/company")
        )
        # Create test items
    ...
    async def test_batch_subpartition(self):
        ...
        await container.upsert_item(
            {"id": item_ids[0], "key": "value", "state": "WA", "city": "Redmond", "zipcode": "98052"}
        )
        await container.upsert_item(
            {"id": item_ids[1], "key": "value", "state": "WA", "city": "Redmond", "zipcode": "98052"}
        )
        await container.upsert_item(
            {"id": item_ids[2], "key": "value", "state": "WA", "city": "Redmond", "zipcode": "98052"}
        )
        ...
        batch_response = await container.execute_item_batch(
            batch_operations=batch, partition_key=["WA", "Redmond", "98052"]
        )
        assert len(batch_response) == 6
