...
class TestQueryFeedRange:
    ...
    def test_query_with_both_feed_range_and_partition_key(self, container_id):
        ...
        expected_error_message = (
            "'feed_range' and 'partition_key' are exclusive parameters, please only set one of them."
        )
        query = "SELECT * from c"
        partition_key = PK_VALUES[0]
        feed_range = container.feed_range_from_partition_key(partition_key)
        with pytest.raises(ValueError) as e:
            list(container.query_items(query=query, feed_range=feed_range, partition_key=partition_key))
    ...
    def test_query_with_feed_range_during_partition_split_combined(self, container_id):
        ...
        for feed_range in feed_ranges_before_split:
            count_items = list(container.query_items(query="SELECT VALUE COUNT(1) FROM c", feed_range=feed_range))
            initial_count += count_items[0] if count_items else 0

            sum_items = list(
                container.query_items(
                    query='SELECT VALUE SUM(c["value"]) FROM c WHERE IS_DEFINED(c["value"])', feed_range=feed_range
                )
            )
        ...
        assert (
            expected_pk_values == actual_pk_values
        ), f"Expected {len(expected_pk_values)} PKs, got {len(actual_pk_values)}"
        ...
        assert (
            expected_pk_values == actual_pk_values_order_by
        ), f"Expected {len(expected_pk_values)} PKs, got {len(actual_pk_values_order_by)}"
    ...
    def test_query_with_static_continuation(self):
        ...
        for i in range(10):
            query_by_page = container.query_items(
                ...
                max_item_count=1,
                continuation="-RID:~a0NPAOszCpOChB4AAAAAAA==#RT:1#TRC:2#ISV:2#IEO:65567#QCF:8",
            ).by_page()
    ...
    def test_query_with_continuation(self):
        ...
        for feed in feed_ranges:
            query_kwargs = {"query": query, "feed_range": feed, "priority": "Low", "max_item_count": 1}
