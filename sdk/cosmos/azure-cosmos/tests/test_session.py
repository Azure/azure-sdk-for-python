...
class TestSession(unittest.TestCase):
    ...
    def test_manual_session_token_takes_precedence(self):
        ...
        self.created_collection.create_item(body={"id": "precedence_doc_1" + str(uuid.uuid4()), "pk": "mypk"})
        ...
        with cosmos_client.CosmosClient(self.host, self.masterKey) as other_client:
            ...
            item2 = other_collection.create_item(body={"id": "precedence_doc_2" + str(uuid.uuid4()), "pk": "mypk"})
    ...
    def test_session_token_sm_for_ops(self):
        ...
        up_item = self.created_collection.upsert_item(
            body={"id": "1" + str(uuid.uuid4()), "pk": "mypk"}, raw_response_hook=test_config.no_token_response_hook
        )
        replaced_item = self.created_collection.replace_item(
            item=up_item["id"],
        ...
        batch_result = self.created_collection.execute_item_batch(
        ...
        self.created_collection.delete_item(
    ...
    def test_session_token_with_space_in_container_name(self):
        ...
        try:
            # Session token should be sent for document read/batch requests only - verify it is not sent for write requests
            created_document = test_container.create_item(
                body={"id": "1" + str(uuid.uuid4()), "pk": "mypk"}, raw_response_hook=test_config.no_token_response_hook
            )
            response_session_token = created_document.get_response_headers().get(HttpHeaders.SessionToken)
            read_item = test_container.read_item(
                item=created_document["id"], partition_key="mypk", raw_response_hook=test_config.token_response_hook
            )
    ...
    def test_session_token_mwr_for_ops(self):
        ...
        # Session token should not be sent for control plane operations
        test_container = db.create_container(
            str(uuid.uuid4()), PartitionKey(path="/id"), raw_response_hook=test_config.no_token_response_hook
        )
        ...
        created_document = container.create_item(
            body={"id": "1" + str(uuid.uuid4()), "pk": "mypk"}, raw_response_hook=test_config.token_response_hook
        )
        response_session_token = created_document.get_response_headers().get(HttpHeaders.SessionToken)
        ...
        read_item2 = container.read_item(
            item=created_document["id"], partition_key="mypk", raw_response_hook=test_config.token_response_hook
        )
    ...
    def test_clear_session_token(self):
        ...
        try:
            ...
            self.assertEqual(
                self.client.client_connection.session.get_session_token(
                    "dbs/" + self.created_db.id + "/colls/" + self.created_collection.id, None, {}, None, None, None
                ),
                "",
            )
