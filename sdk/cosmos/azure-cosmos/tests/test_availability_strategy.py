...
NON_TRANSIENT_STATUS_CODES = [(400, None), (409, None), (405, None), (412, None), (413, None), (401, None), (404, 0)]


def _create_doc():
    return {"id": str(uuid.uuid4()), "pk": "test_pk", "name": "sample document", "key": "value"}
...
def _perform_read_operation(
    ...
    **kwargs,
    ...
    if operation == READ:
        container.read_item(
            item=created_doc["id"], partition_key=created_doc["pk"], excluded_locations=excluded_locations, **kwargs
        )
        ...
        response = list(container.read_all_items(excluded_locations=excluded_locations, **kwargs))
        assert any(item["id"] == created_doc["id"] for item in response)

    elif operation == CHANGE_FEED:
        response = list(
            container.query_items_change_feed(
                partition_key=created_doc["pk"], excluded_locations=excluded_locations, **kwargs
            )
        )
        any(item["id"] == created_doc["id"] for item in response)

    _validate_response_uris(
        expected_uris, excluded_uris, operation_type=_get_operation_type(operation), resource_type=ResourceType.Document
    )
...
def _perform_write_operation(
    ...
    **kwargs,
    ...
    if operation == CREATE:
        doc = _create_doc()
        container.create_item(body=doc, retry_write=retry_write, excluded_locations=excluded_locations, **kwargs)

    elif operation == UPSERT:
        doc = _create_doc()
        container.upsert_item(body=doc, retry_write=retry_write, excluded_locations=excluded_locations, **kwargs)
        ...
        doc = _create_doc()
        batch_ops = [("upsert", (doc,))]
        container.execute_item_batch(
            batch_ops, partition_key=doc["pk"], retry_write=retry_write, excluded_locations=excluded_locations, **kwargs
        )
...
def _validate_response_uris(expected_location_uris, excluded_location_uris, operation_type=None, resource_type=None):
    ...
    assert all(
        location not in excluded_location_uris for location in req_urls
    ), "Found request being routed to excluded regions unexpected"
...
class TestAvailabilityStrategy:
    ...
    def _get_custom_transport_with_fault_injection(self, predicate, error_lambda):
        """Setup test with fault injection"""
        custom_transport = FaultInjectionTransport()
        custom_transport.add_fault(predicate, error_lambda)
        return custom_transport

    def _clean_up_container(self, database_id: str, container_id: str):
        container = self.client_without_fault.get_database_client(database_id).get_container_client(container_id)
        all_items = list(container.read_all_items())
        for item in all_items:
            try:
                container.delete_item(item["id"], item["pk"])
            except Exception:
                pass
    ...
    @pytest.mark.parametrize(
        "operation", [READ, QUERY, QUERY_PK, READ_ALL, CHANGE_FEED, CREATE, UPSERT, REPLACE, DELETE, PATCH, BATCH]
    )
    @pytest.mark.parametrize(
        "client_availability_strategy, request_availability_strategy",
        [
    ...
    def test_availability_strategy_in_steady_state(
        self, operation, client_availability_strategy, request_availability_strategy
    ):
        """Test for steady state, operations go to first preferred location even with availability strategy enabled"""
        setup = self._setup_method_with_custom_transport(None, availability_strategy=client_availability_strategy)
    ...
    def test_client_availability_strategy_failover(
        ...
        predicate = lambda r: (
            ...
            and FaultInjectionTransport.predicate_is_operation_type(r, _get_operation_type(operation))
            and FaultInjectionTransport.predicate_targets_region(r, uri_down)
        )

        error_lambda = lambda r: FaultInjectionTransport.error_after_delay(
            1000, CosmosHttpResponseError(status_code=400, message="Injected Error")  # Add delay to trigger hedging
        )
        ...
        setup = self._setup_method_with_custom_transport(
            custom_transport, multiple_write_locations=True, availability_strategy=client_availability_strategy
        )
        ...
        if operation in [READ, QUERY, QUERY_PK, READ_ALL, CHANGE_FEED]:
            _perform_read_operation(
                operation, setup["col"], doc, expected_uris, [], availability_strategy=request_availability_strategy
            )
        else:
            _perform_write_operation(
                operation,
    ...
    def test_non_transient_errors_from_failed_over_region(
        self, operation, status_code: int, sub_status_code: Optional[int]
    ):
        ...
        predicate = lambda r: (
            FaultInjectionTransport.predicate_is_document_operation(r)
            and FaultInjectionTransport.predicate_is_operation_type(r, _get_operation_type(operation))
            and FaultInjectionTransport.predicate_targets_region(r, failed_over_uri)
        )
        error_lambda = lambda r: FaultInjectionTransport.error_after_delay(
            0,
            CosmosHttpResponseError(
                status_code=status_code, message=f"Injected {status_code} Error", sub_status=sub_status_code
            ),
        )
        ...
        predicate_first_region = lambda r: (
            FaultInjectionTransport.predicate_is_document_operation(r)
            and FaultInjectionTransport.predicate_is_operation_type(r, _get_operation_type(operation))
            and FaultInjectionTransport.predicate_targets_region(r, uri_down)
        )
        error_lambda_first_region = lambda r: FaultInjectionTransport.error_after_delay(
            500, CosmosHttpResponseError(status_code=503, message="Injected Error")
        )
        ...
        with pytest.raises(CosmosHttpResponseError) as exc_info:
            if operation in [READ, QUERY, QUERY_PK, READ_ALL, CHANGE_FEED]:
                _perform_read_operation(operation, setup["col"], doc, expected_uris, [], availability_strategy=strategy)
            else:
                _perform_write_operation(
                    operation, setup["col"], doc, expected_uris, [], retry_write=True, availability_strategy=strategy
                )
    ...
    def test_transient_error_from_failed_over_region(self, operation):
        ...
        error_lambda = lambda r: FaultInjectionTransport.error_after_delay(
            0, ServiceResponseError(message="Generic Service Error")
        )
    ...
    def test_request_level_disable_override_client_strategy(self, operation):
        ...
        error_lambda = lambda r: FaultInjectionTransport.error_after_delay(
            ...
            CosmosHttpResponseError(
                status_code=400, message="Injected Error"
            ),  # using non retryable errors to verify the request will only go to the first region
        )
        custom_transport = self._get_custom_transport_with_fault_injection(predicate, error_lambda)
        setup = self._setup_method_with_custom_transport(custom_transport, availability_strategy=client_strategy)
        setup_without_fault = self._setup_method_with_custom_transport(None)
        doc = _create_doc()
        setup_without_fault["col"].create_item(body=doc)
        ...
        with pytest.raises(CosmosHttpResponseError) as exc_info:
            if operation in [READ, QUERY, QUERY_PK, READ_ALL, CHANGE_FEED]:
                _perform_read_operation(
                    operation, setup["col"], doc, expected_uris, excluded_uris, availability_strategy=False
                )
            else:
                _perform_write_operation(
                    operation,
    ...
    def test_request_level_enable_override_client_disable(self, operation):
        ...
        if operation in [READ, QUERY, QUERY_PK, READ_ALL, CHANGE_FEED]:
            _perform_read_operation(
                operation, setup["col"], doc, expected_uris, [], availability_strategy=request_strategy
            )
        else:
            _perform_write_operation(
                operation,
                ...
                [],
                retry_write=True,
                availability_strategy=request_strategy,
            )
    ...
    def test_no_cross_region_request_with_retry_write_disabled(self, operation):
        ...
        with pytest.raises(CosmosHttpResponseError) as exc_info:
            _perform_write_operation(
                operation, setup["col"], doc, expected_uris, excluded_uris, availability_strategy=strategy
            )
    ...
    def test_per_partition_circular_breaker_with_cancelled_first_future(self, operation):
        ...
        try:
            ...
            error_lambda = lambda r: FaultInjectionTransport.error_after_delay(
                1000, CosmosHttpResponseError(status_code=503, message="Injected Error")  # Add delay to trigger hedging
            )
            ...
            setup_with_fault_injection = self._setup_method_with_custom_transport(
                custom_transport, multiple_write_locations=True, container_id=self.TEST_SINGLE_PARTITION_CONTAINER_ID
            )
            setup_without_fault = self._setup_method_with_custom_transport(
                None, container_id=self.TEST_SINGLE_PARTITION_CONTAINER_ID
            )
    ...
    def test_custom_thread_pool_executor(self):
        ...
        predicate = lambda r: (
            FaultInjectionTransport.predicate_is_document_operation(r)
            and FaultInjectionTransport.predicate_is_operation_type(r, OperationType.Read)
            and FaultInjectionTransport.predicate_targets_region(r, uri_down)
        )
        error_lambda = lambda r: FaultInjectionTransport.error_after_delay(
            500, CosmosHttpResponseError(status_code=400, message="Injected Error")
        )
        ...
        setup_with_transport = self._setup_method_with_custom_transport(
            custom_transport, availability_strategy_executor=ThreadPoolExecutor(max_workers=1)
        )
        ...
        with pytest.raises(CosmosHttpResponseError) as exc_info:
            _perform_read_operation(
                READ, setup_with_transport["col"], doc, [uri_down], [failed_over_uri], availability_strategy=strategy
            )
    ...
    def test_default_availability_strategy_with_ppaf_enabled(self, operation):
        ...
        custom_transport.add_response_transformation(is_get_account_predicate, ppaf_enabled_database_account)
        ...
        if operation in [READ, QUERY, QUERY_PK, READ_ALL, CHANGE_FEED]:
            _perform_read_operation(operation, setup["col"], doc, expected_uris, [])
        else:
            _perform_write_operation(operation, setup["col"], doc, expected_uris, [], retry_write=True)
        self._clean_up_container(setup["db"].id, setup["col"].id)
