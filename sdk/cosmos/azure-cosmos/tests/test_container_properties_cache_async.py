...
class TestContainerPropertiesCache(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_container_properties_cache_async(self):
        ...
        try:
            await client.get_database_client(database_name).create_container(
                id=container_name, partition_key=PartitionKey(path="/" + container_pk)
            )
    ...
    async def test_container_recreate_create_upsert_replace_item_async(self):
        ...
        # Create The Container
        try:
            await created_db.create_container(id=container_name, partition_key=PartitionKey(path="/" + container_pk))
        except exceptions.CosmosResourceExistsError:
            self.fail("Container Should not Already Exist.")
        ...
        await created_db.delete_container(container_name)
        try:
            await created_db.create_container(id=container_name, partition_key=PartitionKey(path="/" + container2_pk))
        except exceptions.CosmosResourceExistsError:
            self.fail("Container Should not Already Exist.")
        ...
        client.client_connection._CosmosClientConnection__container_properties_cache = copy.deepcopy(old_cache)
        try:
            await created_db.get_container_client(container_name).replace_item(
                item=item, body={"id": "item1", container2_pk: "val"}
            )
        except exceptions.CosmosHttpResponseError as e:
            self.fail("{}".format(e.http_error_message))
    ...
    async def test_container_recreate_create_upsert_replace_item_sub_partitioning_async(self):
        ...
        try:
            ...
            assert False, "{}".format(e.http_error_message)
        # Finally test replace item
        item = await created_db.get_container_client(container_name).read_item(
            "item1", partition_key=["ventura", "oxnard"]
        )
        client.client_connection._CosmosClientConnection__container_properties_cache = copy.deepcopy(old_cache)
        try:
            await created_db.get_container_client(container_name).replace_item(
                item=item, body={"id": "item1", "county": "ventura", "city": "oxnard"}
            )
        except exceptions.CosmosHttpResponseError as e:
            assert False, "{}".format(e.http_error_message)
    ...
    async def test_offer_throughput_container_recreate_async(self):
        ...
        try:
            created_container = await created_db.create_container(
                id=container_name, partition_key=PartitionKey(path="/" + container_pk), offer_throughput=600
            )
        except exceptions.CosmosResourceExistsError:
            assert False, "Container Should not Already Exist."
        ...
        try:
            created_container = await created_db.create_container(
                id=container_name, partition_key=PartitionKey(path="/" + container2_pk), offer_throughput=800
            )
        except exceptions.CosmosResourceExistsError:
            assert False, "Container Should not Already Exist."
    ...
    async def test_container_recreate_read_item_async(self):
        ...
        try:
            created_container = await created_db.create_container(
                id=container_name, partition_key=PartitionKey(path="/" + container2_pk)
            )
        except exceptions.CosmosResourceExistsError:
            assert False, "Container Should not Already Exist."
        try:
    ...
    async def test_container_recreate_read_item_sub_partition_async(self):
        ...
        try:
            ...
            item_to_read2 = await created_container.create_item(
                body={"id": "item2", "country": "MEX", "state": "Michoacán"}
            )  # cspell:disable-line
        except exceptions.CosmosHttpResponseError as e:
            assert False, "{}".format(e.http_error_message)
    ...
    async def test_container_recreate_query_async(self):
        ...
        try:
            ...
            query_result = [item async for item in created_container.query_items(query=query, parameters=parameters)]
    ...
    async def test_container_recreate_transactional_batch(self):
        ...
        try:
            ...
            batch_response = await created_container.execute_item_batch(
                batch_operations=batch_operations, partition_key="Microsoft"
            )
