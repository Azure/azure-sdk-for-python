...
class TestReadItems(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.container = self.database.create_container(
            id="read_items_container_" + str(uuid.uuid4()), partition_key=PartitionKey(path="/id")
        )
    ...
    def _setup_fault_injection(self, error_to_inject, inject_once=False):
        ...
        def predicate(request):
            ...
            is_query = request.method == "POST" and FaultInjectionTransport.predicate_is_operation_type(
    ...
    def test_read_items_different_partition_key(self):
        ...
        container_pk = self.database.create_container(
            id="read_items_pk_container_" + str(uuid.uuid4()), partition_key=PartitionKey(path="/pk")
        )
        try:
            items_to_read = []
            item_ids = []
    ...
    def test_read_items_order_using_zip_comparison(self):
        ...
        container_pk = self.database.create_container(
            id="read_order_zip_container_" + str(uuid.uuid4()), partition_key=PartitionKey(path="/pk")
        )
        ...
        try:
            ...
            self.assertTrue(all(matching), "Order was not preserved. Input order doesn't match output order.")
    ...
    def test_read_items_multiple_physical_partitions_and_hook(self):
        ...
        try:
            ...
            for pk_range in pk_ranges:
                ...
                items_in_partition = list(
                    multi_partition_container.query_items("SELECT c.id FROM c", partition_key_range_id=pk_range_id)
                )
            ...
            read_items_result = multi_partition_container.read_items(items=items_to_read, response_hook=response_hook)
