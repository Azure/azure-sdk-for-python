...
from test_per_partition_automatic_failover import (
    create_failover_errors,
    create_threshold_errors,
    session_retry_hook,
    ppaf_user_agent_hook,
)
from test_per_partition_circuit_breaker_mm import (
    ...
    PK_VALUE,
    BATCH,
    write_operations_errors_and_boolean,
)
...
class TestPerPartitionAutomaticFailoverAsync:
    ...
    async def setup_method_with_custom_transport(
        self, custom_transport: Optional[AioHttpTransport], default_endpoint=host, read_first=False, **kwargs
    ):
    ...
    async def setup_info(
        self, error=None, max_count=None, is_batch=False, exclude_client_regions=False, session_error=False, **kwargs
    ):
        ...
        predicate = lambda r: (
            FaultInjectionTransportAsync.predicate_req_for_document_with_id(r, doc_fail_id)
            and FaultInjectionTransportAsync.predicate_is_write_operation(r, "com")
        )
        ...
        if is_batch:
            success_response = FaultInjectionTransportAsync.MockHttpResponse(
                mock_request,
                200,
                [{"statusCode": 200}],
            )
        ...
        if error:
            custom_transport.add_fault(
                predicate=predicate, fault_factory=error, max_inner_count=max_count, after_max_count=success_response
            )
        if session_error:
            read_predicate = lambda r: (
                FaultInjectionTransportAsync.predicate_is_operation_type(r, "Read")
                and FaultInjectionTransportAsync.predicate_req_for_document_with_id(r, doc_fail_id)
            )
            read_error = CosmosHttpResponseError(status_code=404, message="Some injected error.", sub_status=1002)
            ...
            custom_transport.add_fault(
                ...
                fault_factory=error_lambda,
                max_inner_count=max_count,
        ...
        setup = await self.setup_method_with_custom_transport(
            None, default_endpoint=self.host, exclude_client_regions=exclude_client_regions, **kwargs
        )
        custom_setup = await self.setup_method_with_custom_transport(
            custom_transport, default_endpoint=self.host, exclude_client_regions=exclude_client_regions, **kwargs
        )
        return setup, doc_fail_id, doc_success_id, custom_setup, custom_transport, predicate

    @pytest.mark.parametrize(
        "write_operation, error, exclude_regions", write_operations_errors_and_boolean(create_failover_errors())
    )
    async def test_ppaf_partition_info_cache_and_routing_async(self, write_operation, error, exclude_regions):
        ...
        setup, doc_fail_id, doc_success_id, custom_setup, custom_transport, predicate = await self.setup_info(
            error_lambda, 1, write_operation == BATCH, exclude_client_regions=exclude_regions
        )
        container = setup["col"]
        ...
        await fault_injection_container.create_item(
            body={"id": doc_success_id, "pk": PK_VALUE, "name": "sample document", "key": "value"}
        )
        ...
        await perform_write_operation(write_operation, container, fault_injection_container, doc_fail_id, PK_VALUE)
    ...
    @pytest.mark.parametrize(
        "write_operation, error, exclude_regions", write_operations_errors_and_boolean(create_threshold_errors())
    )
    async def test_ppaf_partition_thresholds_and_routing_async(self, write_operation, error, exclude_regions):
        ...
        for i in range(consecutive_failures):
            ...
            with pytest.raises((CosmosHttpResponseError, ServiceResponseError)) as exc_info:
                await perform_write_operation(
                    write_operation, container, fault_injection_container, doc_fail_id, PK_VALUE
                )
            assert exc_info.value == error

        # Verify that the threshold for consecutive failures is updated
        pk_range_wrappers = list(
            global_endpoint_manager.ppaf_thresholds_tracker.pk_range_wrapper_to_failure_count.keys()
        )
        assert len(pk_range_wrappers) == 1
        failure_count = global_endpoint_manager.ppaf_thresholds_tracker.pk_range_wrapper_to_failure_count[
            pk_range_wrappers[0]
        ]
        ...
        await perform_write_operation(
            write_operation, container, fault_injection_container, str(uuid.uuid4()), PK_VALUE
        )

        failure_count = global_endpoint_manager.ppaf_thresholds_tracker.pk_range_wrapper_to_failure_count.get(
            pk_range_wrappers[0], 0
        )
    ...
    async def test_ppaf_session_unavailable_retry_async(self, write_operation, error, exclude_regions):
        ...
        region = fault_injection_container.client_connection._global_endpoint_manager.location_cache.get_location_from_endpoint(
            uri
        )
    ...
    async def test_ppaf_user_agent_feature_flag_async(self):
        ...
        await fault_injection_container.upsert_item(
            body={"id": doc_success_id, "pk": PK_VALUE, "name": "sample document", "key": "value"},
