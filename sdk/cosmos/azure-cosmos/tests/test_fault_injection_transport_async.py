...
class TestFaultInjectionTransportAsync(IsolatedAsyncioTestCase):
    ...
    async def asyncTearDown(cls):
        ...
        created_database = cls.mgmt_client.get_database_client(cls.database_id)
        try:
            await asyncio.wait_for(created_database.delete_container(cls.single_partition_container_name), MGMT_TIMEOUT)
        except Exception as containerDeleteError:
            logger.warning(
                "Exception trying to delete database {}. {}".format(created_database.id, containerDeleteError)
            )
        finally:
            try:
    ...
    async def test_throws_injected_error_async(self: "TestFaultInjectionTransportAsync"):
        ...
        predicate: Callable[[HttpRequest], bool] = (
            lambda r: FaultInjectionTransportAsync.predicate_req_for_document_with_id(r, id_value)
        )
        custom_transport.add_fault(
            predicate,
            lambda r: asyncio.create_task(
                FaultInjectionTransportAsync.error_after_delay(
                    10000, CosmosHttpResponseError(status_code=502, message="Some random reverse proxy error.")
                )
            ),
        )
    ...
    async def test_swr_mrr_succeeds_async(self: "TestFaultInjectionTransportAsync"):
        ...
        is_write_operation_in_read_region_predicate: Callable[[HttpRequest], bool] = (
            lambda r: FaultInjectionTransportAsync.predicate_is_write_operation(r, expected_read_region_uri)
        )
        ...
        emulator_as_multi_region_sm_account_transformation: Callable[
            [HttpRequest, Callable[[], AsyncHttpResponse]], AioHttpTransportResponse
        ...
        initialized_objects = await TestFaultInjectionTransportAsync.setup_method_with_custom_transport(
            custom_transport, preferred_locations=["Read Region", "Write Region"]
        )
        try:
            container: ContainerProxy = initialized_objects["col"]
    ...
    async def test_swr_mrr_region_down_read_succeeds_async(self: "TestFaultInjectionTransportAsync"):
        ...
        is_request_to_read_region: Callable[[HttpRequest], bool] = (
            lambda r: FaultInjectionTransportAsync.predicate_targets_region(r, expected_read_region_uri)
        )

        custom_transport.add_fault(
            is_request_to_read_region, lambda r: asyncio.create_task(FaultInjectionTransportAsync.error_region_down())
        )
        ...
        emulator_as_multi_region_sm_account_transformation: Callable[
            [HttpRequest, Callable[[HttpRequest], AsyncHttpResponse]], AioHttpTransportResponse
        ] = lambda r, inner: FaultInjectionTransportAsync.transform_topology_swr_mrr(
            write_region_name="Write Region", read_region_name="Read Region", inner=inner
        )
        custom_transport.add_response_transformation(
    ...
    async def test_swr_mrr_region_down_envoy_read_succeeds_async(self: "TestFaultInjectionTransportAsync"):
        ...
        custom_transport.add_fault(
            ...
            lambda r: asyncio.create_task(
                FaultInjectionTransportAsync.error_after_delay(
                    500, CosmosHttpResponseError(status_code=502, message="Some random reverse proxy error.")
                )
            ),
        )
    ...
    async def test_swr_mrr_all_regions_down_for_read_async(self: "TestFaultInjectionTransportAsync"):
        ...
        # Inject rule to simulate regional outage in "First Region"
        is_request_to_first_region: Callable[[HttpRequest], bool] = lambda r: (
            FaultInjectionTransportAsync.predicate_targets_region(r, expected_write_region_uri)
            and FaultInjectionTransportAsync.predicate_is_document_operation(r)
            and not FaultInjectionTransportAsync.predicate_is_write_operation(r, expected_write_region_uri)
        )

        # Inject rule to simulate regional outage in "Second Region"
        is_request_to_second_region: Callable[[HttpRequest], bool] = lambda r: (
            FaultInjectionTransportAsync.predicate_targets_region(r, expected_read_region_uri)
            and FaultInjectionTransportAsync.predicate_is_document_operation(r)
            and not FaultInjectionTransportAsync.predicate_is_write_operation(r, expected_write_region_uri)
        )
        ...
        custom_transport.add_fault(
            is_request_to_second_region, lambda r: asyncio.create_task(FaultInjectionTransportAsync.error_region_down())
        )
    ...
    async def test_mwr_all_regions_down_async(self: "TestFaultInjectionTransportAsync"):
        ...
        # Inject rule to simulate regional outage in "Second Region"
        is_request_to_second_region: Callable[[HttpRequest], bool] = (
            lambda r: FaultInjectionTransportAsync.predicate_targets_region(r, second_region_uri)
            and FaultInjectionTransportAsync.predicate_is_document_operation(r)
        )
