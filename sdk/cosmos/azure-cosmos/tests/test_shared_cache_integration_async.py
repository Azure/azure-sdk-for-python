...
class TestSharedCacheIntegrationAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncTearDown(self):
        ...
        # Clear ALL five shared-cache globals (not just the routing-map dict)
        # to keep refcount/lock/in-flight state consistent. The in-flight dict
        # was added alongside the asyncio.shield-based fetch coalescing; if
        # we leave it dangling a later test could join a task bound to this
        # test's torn-down event loop.
        from azure.cosmos._routing.aio.routing_map_provider import (
        ...
        with _shared_cache_lock:
            ...
            _shared_inflight_fetches.pop(self.host, None)
