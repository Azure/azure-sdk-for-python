...
class TestBackwardsCompatibilityAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_session_token_compatibility_async(self):
        ...
        database_list2 = [
            db async for db in self.client.query_databases(query="select * from c", session_token=str(uuid.uuid4()))
        ]
        ...
        container_list2 = [
            cont
            async for cont in self.created_database.query_containers(
                query="select * from c", session_token=str(uuid.uuid4())
            )
        ]
        ...
        replace_container = await self.created_database.replace_container(
    ...
    async def test_etag_match_condition_compatibility_async(self):
        ...
        assert database is not None
        database2 = await self.client.create_database_if_not_exists(
            str(uuid.uuid4()), etag=str(uuid.uuid4()), match_condition=MatchConditions.IfNotModified
        )
        assert database2 is not None
        await self.client.delete_database(
            database2.id, etag=str(uuid.uuid4()), match_condition=MatchConditions.IfModified
        )
        try:
            await database2.read()
            pytest.fail("Database read should have failed")
        ...
        assert replace_container_read != container2_read
        assert "defaultTtl" in replace_container_read  # Check for default_ttl as a new additional property
        await self.created_database.delete_container(
        ...
        item2 = await container.upsert_item(
        ...
        assert item is not None
        item2 = await container.upsert_item({"id": str(uuid.uuid4()), "pk": 0}, etag=None, match_condition=None)
        assert item2 is not None
        batch_operations = [
