...
def get_test_data_with_expected_output(_client_only_output_data, _client_and_request_output_data):
    ...
    all_test_data = [
        input_data + [output_data] for input_data, output_data in zip(all_input_test_data, all_output_data)
    ]
...
def verify_endpoint(
    messages, client, expected_locations, multiple_write_locations=True, operation_type=None, resource_type=None
):
    ...
    location_mapping = (
        client.client_connection._global_endpoint_manager.location_cache.account_locations_by_write_endpoints
    )
    default_endpoint = (
        client.client_connection._global_endpoint_manager.location_cache.default_regional_routing_context.get_primary()
    )
...
class TestExcludedLocations:
    ...
    def test_read_item(self, test_data):
        ...
        if request_excluded_locations is None:
            ...
            container.read_item(
                item=ITEM_ID, partition_key=PARTITION_KEY_VALUES, excluded_locations=request_excluded_locations
            )
        ...
        if request_excluded_locations is None:
            ...
            list(
                container.query_items(
                    query, partition_key=PREFIX_PARTITION_KEY, excluded_locations=request_excluded_locations
                )
            )
    ...
    def test_query_items_with_query_plan(self, test_data):
        ...
        if request_excluded_locations is None:
            ...
            list(
                container.query_items(
                    query, enable_cross_partition_query=True, excluded_locations=request_excluded_locations
                )
            )
        ...
        if request_excluded_locations is None:
            ...
            items = list(
                container.query_items_change_feed(
                    ...
                    partition_key=PREFIX_PARTITION_KEY,
    ...
    def test_replace_item(self, test_data):
        ...
        for multiple_write_locations in [True, False]:
            # Client setup and create an item
            client, db, container = init_container(
    ...
    def test_patch_item(self, test_data):
        ...
        for multiple_write_locations in [True, False]:
            ...
            if request_excluded_locations is None:
                container.patch_item(item=ITEM_ID, partition_key=PARTITION_KEY_VALUES, patch_operations=operations)
            else:
    ...
    def test_execute_item_batch(self, test_data):
        ...
        for multiple_write_locations in [True, False]:
            ...
            for i in range(3):
                body = {"id": f"doc2-{str(uuid.uuid4())}"}
                body.update(PARTITION_KEY_ITEMS)
                batch_operations.append(("create", (body,)))
