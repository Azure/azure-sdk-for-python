...
class TestCRUDOperationsAsyncResponsePayloadOnWriteDisabled(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_collection_crud_async(self):
        ...
        collections = [
            collection
            async for collection in created_db.query_containers(
                query="SELECT * FROM root r WHERE r.id=@id", parameters=[{"name": "@id", "value": collection_id}]
            )
        ]
        ...
        await self.__assert_http_failure_with_status(StatusCodes.NOT_FOUND, created_container.read)
    ...
    async def test_partitioned_collection_async(self):
        ...
        assert (
            collection_definition.get("partitionKey").get("paths")[0]
            == created_collection_properties["partitionKey"]["paths"][0]
        )
        assert (
            collection_definition.get("partitionKey").get("kind")
    ...
    async def test_partitioned_collection_quota_async(self):
        ...
        retrieved_collection_properties = await created_collection.read(
            populate_partition_key_range_statistics=True, populate_quota_info=True
        )
    ...
    async def test_partitioned_collection_partition_key_extraction_async(self):
        ...
        created_collection = await created_db.create_container(
            id=collection_id, partition_key=PartitionKey(path="/address/state", kind=documents.PartitionKind.Hash)
        )
        ...
        created_collection1 = await created_db.create_container(
            id=collection_id, partition_key=PartitionKey(path="/address", kind=documents.PartitionKind.Hash)
        )
        ...
        created_collection2 = await created_db.create_container(
            id=collection_id, partition_key=PartitionKey(path="/address/state/city", kind=documents.PartitionKind.Hash)
        )
    ...
    async def test_partitioned_collection_partition_key_extraction_special_chars_async(self):
        ...
        document_definition = {"id": "document1", "level' 1*()": {"le/vel2": "val1"}}
        ...
        document_definition = {"id": "document2", 'level" 1*()': {"le/vel2": "val2"}}
    ...
    async def test_partitioned_collection_document_crud_and_query_async(self):
        ...
        document_definition = {"id": "document", "key": "value"}
        ...
        read_document = await created_collection.read_item(
            item=created_document.get("id"), partition_key=created_document.get("id")
        )
        ...
        replaced_document = await created_collection.replace_item(item=read_document, body=document_definition)
        self.assertDictEqual(replaced_document, {})
        replaced_document = await created_collection.replace_item(
            item=read_document, body=document_definition, no_response=True
        )
        self.assertDictEqual(replaced_document, {})
        replaced_document = await created_collection.replace_item(
            item=read_document, body=document_definition, no_response=False
        )
        ...
        document_list = [
            document
            async for document in created_collection.query_items(
    ...
    async def test_partitioned_collection_permissions_async(self):
        ...
        all_collection = await created_db.create_container(
            id=collection_id, partition_key=PartitionKey(path="/key", kind=documents.PartitionKind.Hash)
        )
        ...
        async with CosmosClient(
            TestCRUDOperationsAsyncResponsePayloadOnWriteDisabled.host, resource_tokens
        ) as restricted_client:
            print("Async Initialization")

            document_definition = {"id": "document1", "key": 1}
            ...
            await self.__assert_http_failure_with_status(
                StatusCodes.FORBIDDEN, read_collection.create_item, document_definition
            )
            ...
            await self.__assert_http_failure_with_status(
                StatusCodes.FORBIDDEN, all_collection.create_item, document_definition
            )
            ...
            await all_collection.delete_item(item=created_document["id"], partition_key=document_definition["key"])

            # Delete document in read_collection should fail since it has only read permissions for this collection
            await self.__assert_http_failure_with_status(
                StatusCodes.FORBIDDEN, read_collection.delete_item, document_definition["id"], document_definition["id"]
            )
    ...
    async def test_partitioned_collection_execute_stored_procedure_async(self):
        ...
        sproc = {
            ...
            "body": (
                ...
                + '   client.createDocument(client.getSelfLink(), { id: "'
                + document_id
                + '", pk : 2}, '
        ...
        await self.__assert_http_failure_with_status(
            StatusCodes.BAD_REQUEST, created_collection.scripts.execute_stored_procedure, created_sproc["id"]
        )
    ...
    async def test_partitioned_collection_partition_key_value_types_async(self):
        ...
        document_definition = {"id": "document1" + str(uuid.uuid4()), "pk": None, "spam": "eggs"}

        # create document with partitionKey set as None here
        await created_collection.create_item(body=document_definition)

        document_definition = {"id": "document1" + str(uuid.uuid4()), "spam": "eggs"}

        # create document with partitionKey set as Undefined here
        await created_collection.create_item(body=document_definition)

        document_definition = {"id": "document1" + str(uuid.uuid4()), "pk": True, "spam": "eggs"}

        # create document with bool partitionKey
        await created_collection.create_item(body=document_definition)

        document_definition = {"id": "document1" + str(uuid.uuid4()), "pk": "value", "spam": "eggs"}

        # create document with string partitionKey
        await created_collection.create_item(body=document_definition)

        document_definition = {"id": "document1" + str(uuid.uuid4()), "pk": 100, "spam": "eggs"}

        # create document with int partitionKey
        await created_collection.create_item(body=document_definition)

        document_definition = {"id": "document1" + str(uuid.uuid4()), "pk": 10.50, "spam": "eggs"}

        # create document with float partitionKey
        await created_collection.create_item(body=document_definition)

        document_definition = {"name": "sample document", "spam": "eggs", "pk": "value"}

        # Should throw an error because automatic id generation is disabled always.
        await self.__assert_http_failure_with_status(
            StatusCodes.BAD_REQUEST, created_collection.create_item, document_definition
        )
    ...
    async def test_partitioned_collection_conflict_crud_and_query_async(self):
        ...
        await self.__assert_http_failure_with_status(
            StatusCodes.NOT_FOUND, created_collection.get_conflict, conflict_definition["id"], conflict_definition["id"]
        )
        ...
        conflict_list = [
            conflict
            async for conflict in created_collection.query_conflicts(
    ...
    async def test_document_crud_async(self):
        ...
        document_definition = {"name": "sample document", "spam": "eggs", "key": "value", "pk": "pk"}

        created_document = await created_collection.create_item(
            body=document_definition, enable_automatic_id_generation=True, no_response=False
        )
        ...
        await self.__assert_http_failure_with_status(
            StatusCodes.CONFLICT, created_collection.create_item, duplicated_definition_with_id
        )
        ...
        replaced_document = await created_collection.replace_item(item=created_document["id"], body=created_document)
        self.assertDictEqual(replaced_document, {})

        replaced_document = await created_collection.replace_item(
            item=created_document["id"], body=created_document, no_response=True
        )
        self.assertDictEqual(replaced_document, {})

        replaced_document = await created_collection.replace_item(
            item=created_document["id"], body=created_document, no_response=False
        )
        assert replaced_document is not None
        ...
        try:
            await created_collection.replace_item(
                etag=replaced_document["_etag"], item=replaced_document["id"], body=replaced_document
            )
        ...
        try:
            await created_collection.replace_item(
                match_condition=MatchConditions.IfNotModified, item=replaced_document["id"], body=replaced_document
            )
        ...
        try:
            await created_collection.replace_item(
                match_condition=MatchConditions.IfModified, item=replaced_document["id"], body=replaced_document
            )
            self.fail("should fail if only match condition specified")
        except ValueError:
            pass
        ...
        try:
            await created_collection.replace_item(
                match_condition=replaced_document["_etag"], item=replaced_document["id"], body=replaced_document
            )
        ...
        replaced_document_conditional = await created_collection.read_item(
            item=replaced_document["id"], partition_key=replaced_document["pk"]
        )
        ...
        # delete document
        await created_collection.delete_item(item=replaced_document, partition_key=replaced_document["pk"])
        # read documents after deletion
        await self.__assert_http_failure_with_status(
            StatusCodes.NOT_FOUND, created_collection.read_item, replaced_document["id"], replaced_document["id"]
        )
    ...
    async def test_document_upsert_async(self):
        ...
        document_definition = {"id": "doc", "name": "sample document", "spam": "eggs", "key": "value", "pk": "pk"}
        ...
        with self.assertRaises(exceptions.CosmosHttpResponseError):
            await created_collection.upsert_item(
                body=created_document, match_condition=MatchConditions.IfNotModified, etag=new_document["_etag"]
            )
    ...
    async def test_geospatial_index_async(self):
        ...
        await collection.create_item(body={"id": "loc1", "Location": {"type": "Point", "coordinates": [20.0, 20.0]}})
        await collection.create_item(body={"id": "loc2", "Location": {"type": "Point", "coordinates": [100.0, 100.0]}})
        results = [
            result
            async for result in collection.query_items(
    ...
    async def test_user_crud_async(self):
        ...
        results = [
            ...
            async for user in db.query_users(
                query="SELECT * FROM root r WHERE r.id=@id", parameters=[{"name": "@id", "value": user_id}]
            )
        ]
        ...
        await self.__assert_http_failure_with_status(StatusCodes.NOT_FOUND, deleted_user.read)
    ...
    async def test_permission_crud_async(self):
        ...
        results = [
            permission
            async for permission in user.query_permissions(
                query="SELECT * FROM root r WHERE r.id=@id", parameters=[{"name": "@id", "value": permission.id}]
            )
        ]
        assert results is not None
        ...
        await self.__assert_http_failure_with_status(StatusCodes.NOT_FOUND, user.get_permission, permission.id)
    ...
    async def test_authorization_async(self):
        ...
        async def __setup_entities():
            ...
            # create collection
            collection = await db.create_container(
                id="test_authorization" + str(uuid.uuid4()), partition_key=PartitionKey(path="/id", kind="Hash")
            )
            # create document1
            document = await collection.create_item(
                body={"id": "doc1", "spam": "eggs", "key": "value"}, no_response=False
            )
        ...
        async with CosmosClient(
            TestCRUDOperationsAsyncResponsePayloadOnWriteDisabled.host,
            ...
            resource_tokens = {
                ...
                + entities["db"].id
                + "/colls/"
                + entities["coll"].id: entities["permissionOnColl"].properties["_token"]
            }
        ...
        async with CosmosClient(
            ...
            await self.__assert_http_failure_with_status(StatusCodes.FORBIDDEN, db.delete_container, success_coll)
            ...
            success_doc = await success_coll.read_item(item=doc_id, partition_key=doc_id)
            assert success_doc is not None
            assert success_doc["id"] == entities["doc"]["id"]

            # 5. Failure-- Use Col Permission to Delete Doc
            await self.__assert_http_failure_with_status(
                StatusCodes.FORBIDDEN, success_coll.delete_item, doc_id, doc_id
            )
            ...
            resource_tokens = {
                ...
                + entities["coll"].id
                + "/docs/"
                + doc_id: entities["permissionOnDoc"].properties["_token"]
            }
        ...
        async with CosmosClient(
            ...
            read_doc = (
                await doc_client.get_database_client(db.id)
                .get_container_client(success_coll.id)
                .read_item(doc_id, doc_id)
            )
            assert read_doc["id"] == doc_id

            # 6. Success-- Use Doc permission to delete doc
            await doc_client.get_database_client(db.id).get_container_client(success_coll.id).delete_item(
                doc_id, doc_id
            )
            assert read_doc["id"] == doc_id
    ...
    async def test_trigger_crud_async(self):
        ...
        triggers = [
            trigger
            async for trigger in collection.scripts.query_triggers(
                query="SELECT * FROM root r WHERE r.id=@id",
                parameters=[{"name": "@id", "value": trigger_definition["id"]}],
            )
        ]
        ...
        await self.__assert_http_failure_with_status(
            StatusCodes.NOT_FOUND, collection.scripts.delete_trigger, replaced_trigger["id"]
        )
    ...
    async def test_udf_crud_async(self):
        ...
        udf_definition = {"id": "sample udf", "body": "function() {var x = 10;}"}
        ...
        results = [
            udf
            async for udf in collection.scripts.query_user_defined_functions(
                query="SELECT * FROM root r WHERE r.id=@id", parameters=[{"name": "@id", "value": udf_definition["id"]}]
            )
        ]
        assert results is not None
        # replace udf
        ...
        await self.__assert_http_failure_with_status(
            StatusCodes.NOT_FOUND, collection.scripts.get_user_defined_function, replaced_udf["id"]
        )
    ...
    async def test_sproc_crud_async(self):
        ...
        sproc_definition = {"id": "sample sproc", "serverScript": "function() {var x = 10;}"}
        ...
        sprocs = [
            sproc
            async for sproc in collection.scripts.query_stored_procedures(
                query="SELECT * FROM root r WHERE r.id=@id",
                parameters=[{"name": "@id", "value": sproc_definition["id"]}],
            )
        ]
        ...
        await self.__assert_http_failure_with_status(
            StatusCodes.NOT_FOUND, collection.scripts.get_stored_procedure, replaced_sproc["id"]
        )
    ...
    async def test_script_logging_execute_stored_procedure_async(self):
        ...
        result = await created_collection.scripts.execute_stored_procedure(sproc=created_sproc["id"], partition_key=1)
        ...
        result = await created_collection.scripts.execute_stored_procedure(
            sproc=created_sproc["id"], enable_script_logging=True, partition_key=1
        )
        ...
        result = await created_collection.scripts.execute_stored_procedure(
            sproc=created_sproc["id"], enable_script_logging=False, partition_key=1
        )
                ...
                "includedPaths": [
                    {
                        "path": "/",
                        "indexes": [
                            {"kind": documents.IndexKind.Hash, "dataType": documents.DataType.Number, "precision": 2}
                        ],
                    }
                ],
    ...
    async def test_create_default_indexing_policy_async(self):
        ...
        collection = await db.create_container(
            ...
            indexing_policy={"indexingMode": documents.IndexingMode.Consistent, "automatic": True},
            partition_key=PartitionKey(path="/id", kind="Hash"),
        )
        collection_properties = await collection.read()
        await self._check_default_indexing_policy_paths(collection_properties["indexingPolicy"])
                ...
                "includedPaths": [
                    {
                        "path": "/*",
                        "indexes": [
                            {"kind": documents.IndexKind.Hash, "dataType": documents.DataType.String},
                            {"kind": documents.IndexKind.Range, "dataType": documents.DataType.Number},
                        ],
                    }
                ]
            },
    ...
    async def test_create_indexing_policy_with_composite_and_spatial_indexes_async(self):
        ...
        indexing_policy = {
            ...
            "compositeIndexes": [
                [
                    {"path": "/path1", "order": "ascending"},
                    {"path": "/path2", "order": "descending"},
                    {"path": "/path3", "order": "ascending"},
                ],
                [
                    {"path": "/path4", "order": "ascending"},
                    {"path": "/path5", "order": "descending"},
                    {"path": "/path6", "order": "ascending"},
                ],
            ],
        }
    ...
    async def _check_default_indexing_policy_paths(self, indexing_policy):
        ...
        root_included_path = __get_first(
            [included_path for included_path in indexing_policy["includedPaths"] if included_path["path"] == "/*"]
        )
    ...
    async def test_client_request_timeout_when_connection_retry_configuration_specified_async(self):
        ...
        async with CosmosClient(
            ...
            retry_on_status_codes=[500, 502, 504],
        ) as client:
            with self.assertRaises(ServiceResponseError):
                databaseForTest = client.get_database_client(self.configs.TEST_DATABASE_ID)
                container = databaseForTest.get_container_client(self.configs.TEST_SINGLE_PARTITION_CONTAINER_ID)
    ...
    async def test_query_iterable_functionality_async(self):
        ...
        collection = await self.database_for_test.create_container(
            "query-iterable-container-async", PartitionKey(path="/pk")
        )
        ...
        resources = {"coll": collection, "doc1": doc1, "doc2": doc2, "doc3": doc3}
    ...
    async def test_trigger_functionality_async(self):
        ...
        db = self.database_for_test
        # create collections
        collection1 = await db.create_container(
            id="test_trigger_functionality 1 " + str(uuid.uuid4()), partition_key=PartitionKey(path="/key", kind="Hash")
        )
        collection2 = await db.create_container(
            id="test_trigger_functionality 2 " + str(uuid.uuid4()), partition_key=PartitionKey(path="/key", kind="Hash")
        )
        collection3 = await db.create_container(
            id="test_trigger_functionality 3 " + str(uuid.uuid4()), partition_key=PartitionKey(path="/key", kind="Hash")
        )
        # create triggers
        ...
        document_1_1 = await collection1.create_item(
            body={"id": "doc1", "key": "value"}, pre_trigger_include="t1", no_response=False
        )
        ...
        document_1_3 = await collection1.create_item(
            body={"id": "responseheaders", "key": "value"}, pre_trigger_include="t1", no_response=False
        )
        ...
        document_2_1 = await collection2.create_item(
            body={"id": "doc2", "key": "value2"}, pre_trigger_include="t2", no_response=False
        )
        assert document_2_1["id"] == "doc2"
        document_2_2 = await collection2.create_item(
            body={"id": "Doc3", "prop": "empty", "key": "value2"}, pre_trigger_include="t3", no_response=False
        )
        ...
        with self.assertRaises(Exception):
            await collection3.create_item(
                body={"id": "Docoptype", "key": "value2"}, post_trigger_include="triggerOpType"
            )
    ...
    async def test_stored_procedure_functionality_async(self):
        ...
        result = await collection.scripts.execute_stored_procedure(sproc=retrieved_sproc["id"], partition_key=1)
        assert result == 999
        sproc2 = {
        ...
        result = await collection.scripts.execute_stored_procedure(sproc=retrieved_sproc2["id"], partition_key=1)
        assert int(result) == 123456789
        sproc3 = {
            "id": "storedProcedure3" + str(uuid.uuid4()),
            "body": ("function (input) {" + "  getContext().getResponse().setBody(" + "      'a' + input.temp);" + "}"),
        }
        ...
        result = await collection.scripts.execute_stored_procedure(
            sproc=retrieved_sproc3["id"], parameters={"temp": "so"}, partition_key=1
        )
        assert result == "aso"
    ...
    async def test_index_progress_headers_async(self):
        ...
        none_coll = await created_db.create_container(
            ...
            indexing_policy={"indexingMode": documents.IndexingMode.NoIndex, "automatic": False},
            partition_key=PartitionKey(path="/id", kind="Hash"),
        )
    ...
    async def test_get_resource_with_dictionary_and_object_async(self):
        ...
        created_item = await created_container.create_item(
            {"id": "1" + str(uuid.uuid4()), "pk": "pk"}, no_response=False
        )
        ...
        created_sproc = await created_container.scripts.create_stored_procedure(
            {"id": "storedProcedure" + str(uuid.uuid4()), "body": "function () { }"}
        )
        ...
        created_udf = await created_container.scripts.create_user_defined_function(
            {"id": "sample udf" + str(uuid.uuid4()), "body": "function() {var x = 10;}"}
        )
    ...
    async def test_delete_all_items_by_partition_key_async(self):
        ...
        for i in range(1, 3):
            newDoc = await created_collection.upsert_item(dict(id="item{}".format(i), pk=partition_key1))
            self.assertDictEqual(newDoc, {})

        # add items for partition key 2
        pk2_item = await created_collection.upsert_item(
            dict(id="item{}".format(3), pk=partition_key2), no_response=False
        )
    ...
    async def test_patch_operations_async(self):
        ...
        patched_item = await created_container.patch_item(
            item=item_id, partition_key="patch_item_pk", patch_operations=operations
        )
        ...
        try:
            await created_container.patch_item(item=item_id, partition_key="patch_item_pk", patch_operations=operations)
        except exceptions.CosmosHttpResponseError as e:
    ...
    async def test_conditional_patching_async(self):
        ...
        try:
            await created_container.patch_item(
                item=item_id,
        ...
        patched_item = await created_container.patch_item(
            item=item_id, partition_key="patch_item_pk", patch_operations=operations, filter_predicate=filter_predicate
        )
        # Verify results from patch operations
        self.assertDictEqual(patched_item, {})
        patched_item = await created_container.read_item(item=item_id, partition_key="patch_item_pk")
    ...
    async def test_priority_level_async(self):
        ...
        async def priority_mock_execute_function(function, *args, **kwargs):
            if args:
                priority_headers.append(
                    args[4].headers[HttpHeaders.PriorityLevel] if HttpHeaders.PriorityLevel in args[4].headers else ""
                )
        ...
        # repeat for query
        query = [
            doc async for doc in created_container.query_items("Select * from c", partition_key="pk1", priority="High")
        ]
