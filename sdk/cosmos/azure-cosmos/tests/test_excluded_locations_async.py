...
from test_excluded_locations import (
    ...
    set_test_data_type,
    read_item_test_data,
    write_item_test_data,
    read_and_write_item_test_data,
    verify_endpoint,
)
    ...
    async def test_read_all_items_async(self, test_data):
        ...
        async with CosmosClient(
            ...
            if request_excluded_locations is None:
                ...
                all_items = [
                    item async for item in container.read_all_items(excluded_locations=request_excluded_locations)
                ]
    ...
    async def test_query_items_with_query_plan_async(self, test_data):
        ...
        async with CosmosClient(
            ...
            if request_excluded_locations is None:
                ...
                all_items = [
                    item async for item in container.query_items(query, excluded_locations=request_excluded_locations)
                ]
            ...
            if request_excluded_locations is None:
                all_items = [
                    ...
                    async for item in container.query_items_change_feed(
                        start_time="Beginning", partition_key=PREFIX_PARTITION_KEY
                    )
                ]
            else:
                all_items = [
            ...
            async with CosmosClient(
                ...
                if request_excluded_locations is None:
                    await container.patch_item(
                        item=ITEM_ID, partition_key=PARTITION_KEY_VALUES, patch_operations=operations
                    )
                else:
                    await container.patch_item(
                        item=ITEM_ID,
            ...
            async with CosmosClient(
                ...
                if request_excluded_locations is None:
                    ...
                    await container.delete_item(
                        item_id, PARTITION_KEY_VALUES, excluded_locations=request_excluded_locations
                    )
