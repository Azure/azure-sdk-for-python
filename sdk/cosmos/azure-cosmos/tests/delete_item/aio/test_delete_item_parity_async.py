...
# license information.
# -------------------------------------------------------------------------
"""Cross-backend parity tests for delete_item (async): run both backends in one
...
@pytest.mark.asyncio
async def test_L0_async_create_then_delete(container_for):
    """async create then delete — parity on both backends."""
    item_id = uuid.uuid4().hex

    async def _do(client):
        c = client.get_database_client("parity_db").get_container_client(container_for.id)
        await c.upsert_item({"id": item_id, "pk": "a"})
        return await c.delete_item(item_id, partition_key="a")

    cmp = await run_on_both_backends_async(_do, description="[L0] async delete")
    cmp.print_report()
    cmp.assert_functional_parity()


@pytest.mark.asyncio
async def test_L5_async_delete_missing_raises(container_for):
    """async delete of a missing id raises CosmosResourceNotFoundError on both."""
    async def _do(client):
        c = client.get_database_client("parity_db").get_container_client(container_for.id)
        return await c.delete_item("missing-" + uuid.uuid4().hex, partition_key="a")

    cmp = await run_on_both_backends_async(_do, description="[L5] async delete missing -> 404")
    cmp.print_report()
