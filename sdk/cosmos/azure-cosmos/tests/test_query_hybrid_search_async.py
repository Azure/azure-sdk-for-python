...
class TestFullTextHybridSearchQueryAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_hybrid_search_queries_async(self):
        ...
        query = (
            "SELECT TOP 20 c.index, c.title FROM c WHERE FullTextContains(c.title, 'John') OR "
        ...
        query = (
            "SELECT TOP 10 c.index, c.title FROM c WHERE "
    ...
    async def test_weighted_vs_non_weighted_reciprocal_rank_fusion_async(self):
        ...
        ), "Non-weighted and differently direction weighted RRF results should not match."
    ...
    async def test_weighted_rrf_hybrid_search_with_params_and_response_hook_async(self):
        ...
        param_results = self.test_container.query_items(param_query, parameters=params, response_hook=response_hook)
        param_results = [res async for res in param_results]
        param_indices = [res["index"] for res in param_results]
    ...
    async def test_hybrid_and_non_hybrid_param_queries_equivalence_async(self):
        ...
        param_hybrid_results = self.test_container.query_items(param_hybrid, parameters=params_hybrid)
