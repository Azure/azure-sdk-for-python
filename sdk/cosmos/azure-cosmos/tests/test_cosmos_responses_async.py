...
class TestCosmosResponsesAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_database_headers_async(self):
        first_response = await self.client.create_database(
    ...
    async def test_create_database_if_not_exists_headers_async(self):
        first_response = await self.client.create_database_if_not_exists(
            id="responses_test" + str(uuid.uuid4()), return_properties=True
        )
        assert len(first_response[1].get_response_headers()) > 0
    ...
    async def test_create_container_headers_async(self):
        first_response = await self.test_database.create_container(
    ...
    async def test_create_container_if_not_exists_headers_negative_async(self):
        first_response = await self.test_database.create_container_if_not_exists(
            id="responses_test1", partition_key=PartitionKey(path="/company"), return_properties=True
        )
    ...
    async def test_replace_container_headers_async(self):
        ...
        second_response = await self.test_database.replace_container(
