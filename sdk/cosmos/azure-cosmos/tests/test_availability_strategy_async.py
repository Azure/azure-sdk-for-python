...
async def _perform_read_operation(
    ...
    **kwargs,
    ...
    if operation == READ:
        ...
        response = [item async for item in container.read_all_items(excluded_locations=excluded_locations, **kwargs)]
        assert any(item["id"] == created_doc["id"] for item in response)
    elif operation == CHANGE_FEED:
        response = [
            item
            async for item in container.query_items_change_feed(
...
async def _perform_write_operation(
    ...
    **kwargs,
    ...
    if operation == CREATE:
        doc = _create_doc()
        await container.create_item(body=doc, retry_write=retry_write, excluded_locations=excluded_locations, **kwargs)
    elif operation == UPSERT:
        doc = _create_doc()
        await container.upsert_item(body=doc, retry_write=retry_write, excluded_locations=excluded_locations, **kwargs)
...
class TestAsyncAvailabilityStrategy:
    ...
    async def _setup_method_with_custom_transport(
        self, write_locations, read_locations, custom_transport, default_endpoint=None, retry_write=False, **kwargs
    ):
    ...
    async def test_availability_strategy_in_steady_state_async(
        self, operation, client_availability_strategy, request_availability_strategy, setup
    ):
        ...
        setup_with_transport = await self._setup_method_with_custom_transport(
            setup["write_locations"], setup["read_locations"], None, availability_strategy=client_availability_strategy
        )
        doc = _create_doc()
        ...
        await self._clean_up_container(
            setup["client_without_fault"], setup_with_transport["db"].id, setup_with_transport["col"].id
        )
    ...
    async def test_client_availability_strategy_failover_async(
        ...
        predicate = lambda r: (
            ...
            and FaultInjectionTransportAsync.predicate_is_operation_type(r, _get_operation_type(operation))
            and FaultInjectionTransportAsync.predicate_targets_region(r, uri_down)
        )
        ...
        setup_without_fault = await self._setup_method_with_custom_transport(
            setup["write_locations"], setup["read_locations"], None
        )
    ...
    async def test_non_transient_errors_from_failed_over_region_async(
        self, operation, status_code: int, sub_status_code: Optional[int], setup
    ):
        ...
        predicate = lambda r: (
            FaultInjectionTransportAsync.predicate_is_document_operation(r)
            and FaultInjectionTransportAsync.predicate_is_operation_type(r, _get_operation_type(operation))
            and FaultInjectionTransportAsync.predicate_targets_region(r, failed_over_uri)
        )
        error_lambda = lambda r: FaultInjectionTransportAsync.error_after_delay(
            0,
        ...
        setup_with_fault = await self._setup_method_with_custom_transport(
            setup["write_locations"], setup["read_locations"], custom_transport, multiple_write_locations=True
        )
        ...
        with pytest.raises(CosmosHttpResponseError) as exc_info:
            if operation in [READ, QUERY, QUERY_PK, READ_ALL, CHANGE_FEED]:
                await _perform_read_operation(
                    operation, setup_with_fault["col"], doc, expected_uris, [], availability_strategy=strategy
                )
            else:
                await _perform_write_operation(
                    operation,
        ...
        await setup_without_fault["client"].close()
        await self._clean_up_container(
            setup["client_without_fault"], setup_with_fault["db"].id, setup_with_fault["col"].id
        )
    ...
    async def test_transient_error_from_failed_over_region_async(self, operation, setup):
        ...
        with pytest.raises(CosmosHttpResponseError) as exc_info:
            if operation in [READ, QUERY, QUERY_PK, READ_ALL, CHANGE_FEED]:
                await _perform_read_operation(
                    operation, setup_with_transport["col"], doc, expected_uris, [], availability_strategy=strategy
                )
            else:
                await _perform_write_operation(
                    operation,
    ...
    async def test_request_level_disabled_override_client_strategy_async(self, operation, setup):
        ...
        custom_transport = self._get_custom_transport_with_fault_injection(predicate, error_lambda)
        setup_with_transport = await self._setup_method_with_custom_transport(
            setup["write_locations"], setup["read_locations"], custom_transport, availability_strategy=client_strategy
        )
    ...
    async def test_request_level_enabled_override_client_disabled_async(self, operation, setup):
        ...
        if operation in [READ, QUERY, QUERY_PK, READ_ALL, CHANGE_FEED]:
            await _perform_read_operation(
                operation, setup_with_transport["col"], doc, expected_uris, [], availability_strategy=request_strategy
            )
        else:
            await _perform_write_operation(
                operation,
    ...
    async def test_no_cross_region_request_with_retry_write_disabled_async(self, operation, setup):
        ...
        error_lambda = lambda r: FaultInjectionTransportAsync.error_after_delay(
            500, CosmosHttpResponseError(status_code=400, message="Injected Error")  # Add delay to trigger hedging
        )
        custom_transport = self._get_custom_transport_with_fault_injection(predicate, error_lambda)
        setup_with_transport = await self._setup_method_with_custom_transport(
            setup["write_locations"], setup["read_locations"], custom_transport, multiple_write_locations=True
    ...
    async def test_per_partition_circular_breaker_with_cancelled_first_future_async(self, operation, setup):
        ...
        await self._clean_up_container(
            setup["client_without_fault"], setup_with_fault_injection["db"].id, setup_with_fault_injection["col"].id
        )
    ...
    async def test_max_concurrency_async(self, setup):
        ...
        predicate = lambda r: (
            FaultInjectionTransportAsync.predicate_is_document_operation(r)
            and FaultInjectionTransportAsync.predicate_is_operation_type(r, OperationType.Read)
            and FaultInjectionTransportAsync.predicate_targets_region(r, uri_down)
        )
        error_lambda = lambda r: FaultInjectionTransportAsync.error_after_delay(
    ...
    async def test_default_availability_strategy_with_ppaf_enabled_async(self, operation, setup):
        ...
        if operation in [READ, QUERY, QUERY_PK, READ_ALL, CHANGE_FEED]:
            ...
            await _perform_write_operation(
                operation, setup_with_transport["col"], doc, [uri_down, failed_over_uri], [], retry_write=True
            )
        await setup_with_transport["client"].close()
        await setup_without_fault["client"].close()
        await self._clean_up_container(
