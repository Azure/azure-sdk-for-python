...
class TestSessionAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_manual_session_token_takes_precedence_async(self):
        ...
        async with CosmosClient(self.host, self.masterKey) as other_client:
            other_collection = other_client.get_database_client(self.TEST_DATABASE_ID).get_container_client(
    ...
    async def test_session_token_swr_for_ops_async(self):
        ...
        test_container = await self.created_db.create_container(
            str(uuid.uuid4()), PartitionKey(path="/id"), raw_response_hook=test_config.no_token_response_hook
        )
        await self.created_db.get_container_client(container=self.created_container).read(
        ...
        up_item = await self.created_container.upsert_item(
            body={"id": "1" + str(uuid.uuid4()), "pk": "mypk"}, raw_response_hook=test_config.no_token_response_hook
        )
        replaced_item = await self.created_container.replace_item(
            item=up_item["id"],
            body={"id": up_item["id"], "song": "song", "pk": "mypk"},
            raw_response_hook=test_config.no_token_response_hook,
        )
        created_document = await self.created_container.create_item(
            body={"id": "1" + str(uuid.uuid4()), "pk": "mypk"}, raw_response_hook=test_config.no_token_response_hook
        ...
        read_item = await self.created_container.read_item(
            item=created_document["id"], partition_key="mypk", raw_response_hook=test_config.token_response_hook
        )
        read_item2 = await self.created_container.read_item(
            item=created_document["id"], partition_key="mypk", raw_response_hook=test_config.token_response_hook
        )

        # Since the session hasn't been updated (no write requests have happened) verify session is still the same
        assert (
            read_item.get_response_headers().get(HttpHeaders.SessionToken)
        ...
        batch_result = await self.created_container.execute_item_batch(
            batch_operations, "mypk", raw_response_hook=test_config.token_response_hook
        )
        ...
        await self.created_container.delete_item(
            replaced_item["id"], replaced_item["pk"], raw_response_hook=test_config.no_token_response_hook
        )
        assert self.created_db.client_connection.last_response_headers.get(HttpHeaders.SessionToken) is not None
        assert (
            self.created_db.client_connection.last_response_headers.get(HttpHeaders.SessionToken)
            != batch_response_token
        )
    ...
    async def test_session_token_with_space_in_container_name_async(self):
        ...
        try:
            # Session token should be sent for document read/batch requests only - verify it is not sent for write requests
            created_document = await test_container.create_item(
                body={"id": "1" + str(uuid.uuid4()), "pk": "mypk"}, raw_response_hook=test_config.no_token_response_hook
            )
            response_session_token = created_document.get_response_headers().get(HttpHeaders.SessionToken)
            read_item = await test_container.read_item(
                item=created_document["id"], partition_key="mypk", raw_response_hook=test_config.token_response_hook
            )
            ...
            assert read_item.get_response_headers().get(HttpHeaders.SessionToken) == response_session_token
        finally:
    ...
    async def test_session_token_mwr_for_ops_async(self):
        ...
        is_get_account_predicate: Callable[[HttpRequest], bool] = (
            lambda r: FaultInjectionTransportAsync.predicate_is_database_account_call(r)
        )
        emulator_as_multi_write_region_account_transformation: Callable[
            [HttpRequest, Callable[[HttpRequest], Awaitable[AsyncHttpResponse]]], AioHttpTransportResponse
        ] = lambda r, inner: FaultInjectionTransportAsync.transform_topology_mwr(
            first_region_name="First Region", second_region_name="Second Region", inner=inner
        )
        ...
        # Session token should not be sent for control plane operations
        test_container = await db.create_container(
            str(uuid.uuid4()), PartitionKey(path="/id"), raw_response_hook=test_config.no_token_response_hook
        )
        ...
        await container.upsert_item(
            body={"id": "0" + str(uuid.uuid4()), "pk": "mypk"}, raw_response_hook=test_config.no_token_response_hook
        )
        up_item = await container.upsert_item(
            body={"id": "1" + str(uuid.uuid4()), "pk": "mypk"}, raw_response_hook=test_config.token_response_hook
        )
        replaced_item = await container.replace_item(
            item=up_item["id"],
            body={"id": up_item["id"], "song": "song", "pk": "mypk"},
            raw_response_hook=test_config.token_response_hook,
        )
        created_document = await container.create_item(
            body={"id": "1" + str(uuid.uuid4()), "pk": "mypk"}, raw_response_hook=test_config.token_response_hook
        )
        response_session_token = created_document.get_response_headers().get(HttpHeaders.SessionToken)
        ...
        read_item2 = await container.read_item(
            item=created_document["id"], partition_key="mypk", raw_response_hook=test_config.token_response_hook
        )
        ...
        await container.delete_item(
            replaced_item["id"], replaced_item["pk"], raw_response_hook=test_config.token_response_hook
        )
    ...
    def _MockExecuteFunctionSessionReadFailureOnce(self, function, *args, **kwargs):
        ...
        raise exceptions.CosmosHttpResponseError(
            status_code=StatusCodes.NOT_FOUND, message="Read Session not available", response=response
        )
    ...
    async def test_clear_session_token_async(self):
        ...
        try:
            ...
            session_token = await self.client.client_connection.session.get_session_token_async(
                "dbs/" + self.created_db.id + "/colls/" + self.created_container.id, None, {}, None, None, None
            )
