...
must use the selected backend and retain their requested data.
"""
...
from common._parity_helpers import run_target_operation_async, run_on_both_backends_async, skip_unless_emulator, skip_unless_rust_binding
...
def database_id():
    ...
    name = "parity_read_container_a_" + uuid.uuid4().hex
    try:
...
async def test_read_container_properties_async(database_id, container_id):
    ...
    async def _do(client):
        ...
        return _normalize_container(await run_target_operation_async(client, container.read))
...
async def test_read_container_quota_and_statistics_use_selected_backend_async(
    database_id, container_id, quota, statistics
):
    ...
    comparison = await run_on_both_backends_async(read_metadata, description="async container metadata extras")
    comparison.print_report()
    comparison.assert_functional_parity()


@pytest.mark.parametrize("condition", [MatchConditions.IfPresent, MatchConditions.IfNotModified])
async def test_read_container_conditions_async(database_id, container_id, condition):
    async def read_conditionally(client):
        container = client.get_database_client(database_id).get_container_client(container_id)
        etag = (await container.read())["_etag"] if condition == MatchConditions.IfNotModified else '"unused"'
        return _normalize_container(await run_target_operation_async(
            client, lambda: container.read(etag=etag, match_condition=condition),
        ))

    comparison = await run_on_both_backends_async(read_conditionally, description="async conditional container read")
    comparison.assert_functional_parity()


async def test_read_container_response_hook_cannot_change_cached_partition_key_async(database_id, container_id):
    async def read_with_hook(client):
        container = client.get_database_client(database_id).get_container_client(container_id)
        calls = []
        ...
        properties = await run_target_operation_async(client, lambda: container.read(response_hook=response_hook))
    ...
    comparison = await run_on_both_backends_async(read_with_hook, description="async container response-hook isolation")
    comparison.assert_functional_parity()
...
async def test_read_missing_container_raises_404_async(database_id):
    ...
    async def _do(client):
        ...
        return await run_target_operation_async(client, container.read)
