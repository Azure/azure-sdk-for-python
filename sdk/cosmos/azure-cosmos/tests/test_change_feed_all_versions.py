...
def assert_change_feed(expected, actual):
    ...
    actual = sorted(
        actual,
        key=lambda k: (
            (k[METADATA][OPERATION_TYPE], k[CURRENT][ID])
            if k[METADATA][OPERATION_TYPE] == CREATE
            else (k[METADATA][OPERATION_TYPE], k[PREVIOUS][ID])
        ),
    )
...
class TestChangeAllVersionsFeed:
    ...
    def test_query_change_feed_all_versions_and_deletes(self, setup):
        ...
        created_collection = setup["created_db"].create_container(
        ...
        query_iterable = created_collection.query_items_change_feed(continuation=cont_token1, mode=mode)
        ...
        query_iterable = created_collection.query_items_change_feed(mode=mode, partition_key="pk1")
        list(query_iterable)
        ...
        for document in new_documents:
            created_item = created_collection.create_item(body=document)
            created_items.append(created_item)
        query_iterable = created_collection.query_items_change_feed(continuation=pk_cont_token, mode=mode)
    ...
    def test_query_change_feed_all_versions_and_deletes_errors(self, setup):
        created_collection = setup["created_db"].create_container(
            "change_feed_test_" + str(uuid.uuid4()), PartitionKey(path="/pk")
        )
        mode = "AllVersionsAndDeletes"
        ...
        assert (
            str(e.value)
            == "Invalid mode was used: 'test_invalid_mode'. Supported modes are ['LatestVersion', 'AllVersionsAndDeletes']."
        )
        ...
        assert (
            str(e.value)
            == "'AllVersionsAndDeletes' mode is not supported if 'partition_key_range_id' was used. Please use 'feed_range' instead."
        )
        ...
        assert (
            str(e.value)
            == "'AllVersionsAndDeletes' mode is only supported if 'is_start_from_beginning' is 'False'. Please use 'is_start_from_beginning=False' or 'continuation' instead."
        )
        ...
        assert (
            str(e.value)
            == "'AllVersionsAndDeletes' mode is only supported if 'start_time' is 'Now'. Please use 'start_time=\"Now\"' or 'continuation' instead."
        )
        ...
        with pytest.raises(ValueError) as e:
            continuation_json = {
                ...
                "startFrom": {"Type": "Now"},
                "continuation": {
                    "Range": {"isMaxInclusive": False, "isMinInclusive": True, "max": "FF", "min": ""},
                    "continuation": [
                        {
                            "range": {
