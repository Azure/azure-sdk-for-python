...
import test_config
from azure.cosmos._change_feed.change_feed_iterable import ChangeFeedIterable
...
class TestAllVersionsChangeFeedResponse:
    def test_previous_images_are_preserved(self):
        changes = [
            {
        ...
        continuation, unpacked = iterable._unpack(changes)
...
class TestChangeAllVersionsFeed:
    ...
    def test_query_change_feed_all_versions_and_deletes(self, setup):
        ...
        for item in created_items:
            ...
            replaced_items.append(created_collection.replace_item(item=item[ID], body=replacement))
        query_iterable = created_collection.query_items_change_feed(
    ...
    def test_query_change_feed_ttl_delete(self, setup):
        ...
        try:
            ...
            setup["key_db"].delete_container(cid)
