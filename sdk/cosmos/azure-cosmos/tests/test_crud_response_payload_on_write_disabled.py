...
class TestCRUDOperationsResponsePayloadOnWriteDisabled(unittest.TestCase):
    ...
    def test_partitioned_collection_partition_key_extraction_special_chars(self):
        ...
        collection_definition2 = {
            ...
            "partitionKey": {"paths": ["/'level\" 1*()'/'le/vel2'"], "kind": documents.PartitionKind.Hash},
        }
    ...
    def test_partitioned_collection_document_crud_and_query(self):
        ...
        document_definition = {"id": "document", "key": "value", "pk": "pk"}

        headerEnvelope = CosmosResponseHeaderEnvelope()
        no_response = created_collection.create_item(
            body=document_definition, response_hook=headerEnvelope.capture_response_headers
        )
        ...
        read_document = created_collection.read_item(
            item=document_definition.get("id"), partition_key=document_definition.get("pk")
        )
        ...
        no_Response = created_collection.replace_item(
            item=read_document, body=document_definition, response_hook=headerEnvelope.capture_response_headers
        )
        self.assertDictEqual(no_response, {})
        ...
        no_Response = created_collection.upsert_item(
            body=document_definition, response_hook=headerEnvelope.capture_response_headers
        )
        self.assertDictEqual(no_response, {})
    ...
    def test_partitioned_collection_permissions(self):
        ...
        self.__AssertHTTPFailureWithStatus(StatusCodes.FORBIDDEN, read_collection.create_item, document_definition)
        ...
        self.__AssertHTTPFailureWithStatus(StatusCodes.FORBIDDEN, all_collection.create_item, document_definition)
        ...
        created_document = all_collection.delete_item(
            item=created_document["id"], partition_key=document_definition["key"]
        )
    ...
    def test_partitioned_collection_execute_stored_procedure(self):
        ...
        self.__AssertHTTPFailureWithStatus(
            StatusCodes.BAD_REQUEST, created_collection.scripts.execute_stored_procedure, created_sproc["id"], 3
        )
    ...
    def test_partitioned_collection_conflict_crud_and_query(self):
        ...
        try:
            list(
                created_collection.query_conflicts(
                    query="SELECT * FROM root r WHERE r.resourceType='"
                    + conflict_definition.get("resourceType")  # nosec
                    + "'"
                )
            )
        except Exception:
    ...
    def test_document_crud_response_payload_enabled_via_override(self):
        ...
        self.assertEqual(created_document.get("name"), document_definition["name"])
        ...
        created_document = created_collection.create_item(body=document_definition, no_response=False)
        self.assertEqual(created_document.get("name"), document_definition["name"])
        self.assertEqual(created_document.get("id"), document_definition["id"])
        ...
        documents = list(created_collection.read_all_items())
        self.assertEqual(
            len(documents), before_create_documents_count + 2, "create should increase the number of documents"
        )
        # query documents
        documents = list(
            created_collection.query_items(
        ...
        self.assertEqual(replaced_document["name"], "replaced document", "document id property should change")
        self.assertEqual(replaced_document["spam"], "not eggs", "property should have changed")
        self.assertEqual(created_document["id"], replaced_document["id"], "document id should stay the same")
        ...
        with self.assertRaises(ValueError):
            created_collection.replace_item(
                etag=replaced_document["_etag"], item=replaced_document["id"], body=replaced_document, no_response=False
            )
        ...
        self.assertEqual(
            replaced_document_conditional["name"],
            "replaced document based on condition",
            "document id property should change",
        )
        self.assertEqual(replaced_document_conditional["spam"], "new spam field", "property should have changed")
        self.assertEqual(
            replaced_document_conditional["id"], replaced_document["id"], "document id should stay the same"
        )
        # read document
        one_document_from_read = created_collection.read_item(
            item=replaced_document["id"], partition_key=replaced_document["pk"]
        )
        self.assertEqual(replaced_document["id"], one_document_from_read["id"])
        # delete document
        created_collection.delete_item(item=replaced_document, partition_key=replaced_document["pk"])
    ...
    def test_document_crud(self):
        ...
        # create a document with auto ID generation
        document_definition = {"name": "sample document", "spam": "eggs", "key": "value", "pk": "pk", "id": id}
        ...
        documents = list(created_collection.read_all_items())
        self.assertEqual(
            len(documents), before_create_documents_count + 1, "create should increase the number of documents"
        )
        # query documents
        documents = list(
            created_collection.query_items(
        ...
        with self.assertRaises(ValueError):
            created_collection.replace_item(
                etag=to_be_replaced_document["_etag"], item=to_be_replaced_document["id"], body=to_be_replaced_document
            )
        ...
        self.assertEqual(replaced_document_conditional["spam"], "new spam field", "property should have changed")
        self.assertEqual(
            replaced_document_conditional["id"], to_be_replaced_document["id"], "document id should stay the same"
        )
        # read document
        one_document_from_read = created_collection.read_item(
            item=to_be_replaced_document["id"], partition_key=to_be_replaced_document["pk"]
        )
        self.assertEqual(to_be_replaced_document["id"], one_document_from_read["id"])
        # delete document
        created_collection.delete_item(item=to_be_replaced_document, partition_key=to_be_replaced_document["pk"])
        # read documents after deletion
        self.__AssertHTTPFailureWithStatus(
            StatusCodes.NOT_FOUND,
    ...
    def test_document_upsert(self):
        ...
        id = "doc"
        document_definition = {"id": id, "name": "sample document", "spam": "eggs", "pk": "pk", "key": "value"}

        # create document using Upsert API
        headerEnvelope = CosmosResponseHeaderEnvelope()
        none_response = created_collection.upsert_item(
            body=document_definition, response_hook=headerEnvelope.capture_response_headers
        )
        ...
        none_response = created_collection.upsert_item(
            body=created_document, response_hook=headerEnvelope.capture_response_headers
        )
        self.assertDictEqual(none_response, {})
        ...
        self.assertEqual(upserted_document["name"], created_document["name"], "document name property should change")
        self.assertEqual(upserted_document["spam"], created_document["spam"], "property should have changed")

        # verify id property
        self.assertEqual(upserted_document["id"], created_document["id"], "document id should stay the same")

        # read documents after upsert and verify count doesn't increases again
        documents = list(created_collection.read_all_items())
        self.assertEqual(len(documents), before_create_documents_count + 1, "number of documents should remain same")
        ...
        # verify id property
        self.assertEqual(created_document["id"], new_document["id"], "document id should be same")

        # read documents after upsert and verify count increases
        documents = list(created_collection.read_all_items())
        self.assertEqual(
            len(documents), before_create_documents_count + 2, "upsert should increase the number of documents"
        )
        ...
        documents = list(created_collection.read_all_items())
        self.assertEqual(len(documents), before_create_documents_count, "number of documents should remain same")
    ...
    def test_geospatial_index(self):
        ...
        collection.create_item(body={"id": "loc1", "Location": {"type": "Point", "coordinates": [20.0, 20.0]}})
        collection.create_item(body={"id": "loc2", "Location": {"type": "Point", "coordinates": [100.0, 100.0]}})
        results = list(
    ...
    def test_user_crud(self):
        ...
        # query users
        results = list(
            db.query_users(query="SELECT * FROM root r WHERE r.id=@id", parameters=[{"name": "@id", "value": user_id}])
        )
        ...
        self.assertEqual(replaced_user.id, replaced_user_id, "user id should change")
        self.assertEqual(user_properties["id"], replaced_user.id, "user id should stay the same")
        # read user
        ...
        self.__AssertHTTPFailureWithStatus(StatusCodes.NOT_FOUND, deleted_user.read)
    ...
    def test_user_upsert(self):
        ...
        # verify id property
        self.assertEqual(upserted_user.id, user.id, "user id should remain same")
    ...
    def test_permission_crud(self):
        ...
        self.assertEqual(permission.id, "new permission", "permission id error")
        # list permissions after creation
        permissions = list(user.list_permissions())
        self.assertEqual(len(permissions), before_create_count + 1)
        # query permissions
        results = list(
            user.query_permissions(
        ...
        self.assertEqual(replaced_permission.id, "replaced permission", "permission id should change")
        self.assertEqual(permission.id, replaced_permission.id, "permission id should stay the same")
        # read permission
        ...
        self.__AssertHTTPFailureWithStatus(StatusCodes.NOT_FOUND, user.get_permission, permission.id)
    ...
    def test_permission_upsert(self):
        ...
        # verify id property
        self.assertEqual(created_permission.id, permission_definition["id"], "permission id error")
        ...
        # verify id property
        self.assertEqual(upserted_permission.id, created_permission.id, "permission id should remain same")

        # verify changed property
        self.assertEqual(
            upserted_permission.permission_mode, permission_definition["permissionMode"], "permissionMode should change"
        )
        ...
        self.assertEqual(new_permission.id, created_permission.id, "permission id should be same")

        self.assertEqual(
            new_permission.resource_link, created_permission.resource_link, "permission resource should be same"
        )
    ...
    def test_authorization(self):
        def __SetupEntities(client):
            ...
            document = collection.create_item(
                body={"id": id, "spam": "eggs", "key": "value"},
            )
            ...
            self.assertIsNotNone(permission_on_coll.properties["_token"], "permission token is invalid")
            ...
            self.assertIsNotNone(permission_on_doc.properties["_token"], "permission token is invalid")
        ...
        try:
            cosmos_client.CosmosClient(self.host, {}, "Session", connection_policy=self.connectionPolicy)
        ...
        client = cosmos_client.CosmosClient(
            self.host, self.masterKey, "Session", connection_policy=self.connectionPolicy
        )
        # setup entities
        ...
        self.__AssertHTTPFailureWithStatus(StatusCodes.FORBIDDEN, db.delete_container, success_coll)
        # 3. Success-- Use Col Permission to Read All Docs
        success_documents = list(success_coll.read_all_items())
        self.assertTrue(success_documents != None, "error reading documents")
        self.assertEqual(len(success_documents), 1, "Expected 1 Document to be successfully read")
        # 4. Success-- Use Col Permission to Read Doc

        docId = entities["doc"]["id"]
        success_doc = success_coll.read_item(item=docId, partition_key=docId)
        self.assertTrue(success_doc != None, "error reading document")
        self.assertEqual(success_doc["id"], entities["doc"]["id"], "Expected to read children using parent permissions")

        # 5. Failure-- Use Col Permission to Delete Doc
        self.__AssertHTTPFailureWithStatus(StatusCodes.FORBIDDEN, success_coll.delete_item, docId, docId)
        ...
        resource_tokens = {
            ...
            + entities["coll"].id
            + "/docs/"
            + docId: entities["permissionOnDoc"].properties["_token"]
        }
    ...
    def test_trigger_crud(self):
        ...
        for property in trigger_definition:
            if property != "serverScript":
                ...
                self.assertEqual(trigger["body"], "function() {var x = 10;}")

        # read triggers after creation
        triggers = list(collection.scripts.list_triggers())
        self.assertEqual(
            len(triggers), before_create_triggers_count + 1, "create should increase the number of triggers"
        )
        # query triggers
        triggers = list(
            collection.scripts.query_triggers(
                query="SELECT * FROM root r WHERE r.id=@id",
        ...
        for property in trigger_definition:
            if property != "serverScript":
                ...
                self.assertEqual(replaced_trigger["body"], "function() {var x = 20;}")
    ...
    def test_udf_crud(self):
        ...
        for property in udf_definition:
            self.assertEqual(
                udf[property], udf_definition[property], "property {property} should match".format(property=property)
            )

        # read udfs after creation
        udfs = list(collection.scripts.list_user_defined_functions())
        self.assertEqual(len(udfs), before_create_udfs_count + 1, "create should increase the number of udfs")
        # query udfs
        results = list(
            collection.scripts.query_user_defined_functions(
        ...
        for property in udf_definition:
            self.assertEqual(
                replaced_udf[property], udf[property], "property {property} should match".format(property=property)
            )
        # read udf
    ...
    def test_sproc_crud(self):
        ...
        sproc_definition = {"id": sproc_id, "serverScript": "function() {var x = 10;}"}
        ...
        sprocs = list(collection.scripts.list_stored_procedures())
        self.assertEqual(len(sprocs), before_create_sprocs_count + 1, "create should increase the number of sprocs")
        # query sprocs
        sprocs = list(
            collection.scripts.query_stored_procedures(
                query="SELECT * FROM root r WHERE r.id=@id",
        ...
        for property in sproc_definition:
            if property != "serverScript":
                ...
                self.assertEqual(replaced_sproc["body"], "function() {var x = 20;}")
        # read sproc
    ...
    def test_script_logging_execute_stored_procedure(self):
        ...
        result = created_collection.scripts.execute_stored_procedure(sproc=created_sproc["id"], partition_key=1)
        ...
        self.assertEqual(result, "Success!")
        self.assertEqual(
            urllib.quote("The value of x is 1."),
    ...
    def test_collection_indexing_policy(self):
        ...
        self.assertEqual(
            collection_properties["indexingPolicy"]["indexingMode"],
    ...
    def test_client_request_timeout(self):
        ...
        if not ("localhost" in self.host or "127.0.0.1" in self.host):
            ...
            client = cosmos_client.CosmosClient(
                self.host, self.masterKey, "Session", connection_policy=connection_policy
            )
    ...
    def test_query_iterable_functionality(self):
        collection = self.databaseForTest.create_container(
            "query-iterable-container", partition_key=PartitionKey("/pk")
        )
        ...
        self.assertEqual(3, len(docs), "QueryIterable should return all documents" + " using continuation")
        ...
        for doc in iter(results):
            counter += 1
            if counter == 1:
                self.assertEqual(resources["doc1"]["id"], doc["id"], "first document should be doc1")
            elif counter == 2:
                self.assertEqual(resources["doc2"]["id"], doc["id"], "second document should be doc2")
            elif counter == 3:
                self.assertEqual(resources["doc3"]["id"], doc["id"], "third document should be doc3")
    ...
    def test_trigger_functionality(self):
        ...
        collection3 = db.create_container(
        ...
        self.assertEqual(document_1_1["id"], "DOC1t1", "id should be capitalized")
        ...
        self.assertEqual(document_2_1["id"], "doc2", "id shouldn't change")
        ...
        with self.assertRaises(Exception):
            collection3.create_item(
                body={"id": "Docoptype", "key": "value2"}, post_trigger_include="triggerOpType", no_response=False
            )
    ...
    def test_stored_procedure_functionality(self):
        ...
        result = collection.scripts.execute_stored_procedure(sproc=retrieved_sproc["id"], partition_key=1)
        ...
        result = collection.scripts.execute_stored_procedure(sproc=retrieved_sproc2["id"], partition_key=1)
        ...
        result = collection.scripts.execute_stored_procedure(
            sproc=retrieved_sproc3["id"], params={"temp": "so"}, partition_key=1
        )
    ...
    def __ValidateOfferResponseBody(self, offer, expected_coll_link, expected_offer_type):
        ...
        self.assertTrue(
            offer.properties["_self"].find(offer.properties["id"]) != -1, "Offer id not contained in offer self link."
        )
    ...
    def test_offer_replace(self):
        ...
        self.assertEqual(
            expected_offer.properties.get("content").get("offerThroughput") + 100,
            replaced_offer.properties.get("content").get("offerThroughput"),
        )
        self.assertEqual(expected_offer.offer_throughput + 100, replaced_offer.offer_throughput)
    ...
    def test_patch_operations(self):
        ...
        none_response = created_container.patch_item(
            item="patch_item", partition_key=pkValue, patch_operations=operations
        )
        self.assertDictEqual(none_response, {})
    ...
    def test_conditional_patching(self):
        ...
        try:
            created_container.patch_item(
                item="conditional_patch_item",
