...
class TestNoneOptionsAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_container_read_none_options_async(self):
        result = await self.container.read(
            populate_partition_key_range_statistics=None, populate_quota_info=None, priority=None, initial_headers=None
        )
        assert result
    ...
    async def test_container_read_items_none_options_async(self):
        item = await self._create_sample_item()
        results = await self.container.read_items(
            [(item["id"], item["pk"])],
    ...
    async def test_patch_item_none_options_async(self):
        ...
        patched = await self.container.patch_item(
            item["id"],
            partition_key=item["pk"],
    ...
    async def test_query_conflicts_none_options_async(self):
        pager = self.container.query_conflicts(
            "SELECT * FROM c", parameters=None, partition_key=None, max_item_count=None, response_hook=None
        )
        conflicts = [c async for c in pager]
    ...
    async def test_delete_all_items_by_partition_key_none_options_async(self):
        ...
        await self.container.delete_all_items_by_partition_key(
            pk_value, pre_trigger_include=None, post_trigger_include=None, session_token=None, throughput_bucket=None
        )
    ...
    async def test_execute_item_batch_none_options_async(self):
        ...
        batch_result = await self.container.execute_item_batch(
            ops,
        ...
        assert any(r.get("resourceBody").get("id") == id1 for r in batch_result) or any(
            r.get("resourceBody").get("id") == id2 for r in batch_result
        )

    async def test_query_items_change_feed_none_options_async(self):
        for _ in range(15):
            await self.container.create_item(
                {"id": str(uuid.uuid4()), "pk": "cf-pk", "value": 100},
                pre_trigger_include=None,
                post_trigger_include=None,
                indexing_directive=None,
                enable_automatic_id_generation=False,
        ...
        pager = self.container.query_items_change_feed(
            max_item_count=None,
