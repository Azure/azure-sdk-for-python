...
# license information.
# -------------------------------------------------------------------------
"""End-to-end parity tests for ``aio.Container.patch_item`` across backends.

Async twin of ``tests/patch_item/sync/test_patch_item_parity.py``, following the
same graduated structure used by the ``create_item``, ``read_item`` and
``delete_item`` parity suites: each group of tests adds one more variable to the
one before it, so a failure names the narrowest thing that broke.

Why this file exists at full strength: sync and async are *separate* container
implementations sharing only the option-merge helpers, so ``patch_item`` reaches
the wire through ``ItemHelper`` on sync and ``AsyncItemHelper`` on async, and
only the async path awaits ``_set_partition_key``. A divergence between those
two helpers is invisible to the sync suite however thorough it is, and this
package has already shipped one such bug (the ``_closing`` flag present on the
async backend but missing on the sync one).
...
What this file pins for async ``patch_item``:
...
**Four confirmed rust-path gaps are marked as skips in this file**, each found
by running these tests live and then reproducing the failure in isolation with
a fresh client (2/2 deterministic repeats each, so none is a cross-test
artifact):
...
def container_for(request):
    ...
    lifetime concerns. The patches under test all run on the async client
    ...
    cname = "pta_" + request.node.name + "_" + uuid.uuid4().hex[:6]
...
def _async_patch_call(container_id: str, patch_operations: list, **kwargs):
    """Build an async seed-then-patch closure the harness runs once per backend."""
    async def _do(client):
        cont = client.get_database_client("parity_db").get_container_client(container_id)
        item_id = uuid.uuid4().hex
        await cont.create_item(_seed_document(item_id))
        return await cont.patch_item(
            item=item_id,
...
async def _run_patch(container, patch_operations: list, summary: str,
                     **kwargs) -> BackendComparison:
    """Run one async patch on both backends and print the side-by-side report."""
    description = "async {} -- ops={}, kwargs={}".format(
        summary,
        [op.get("op") for op in patch_operations],
        sorted(kwargs.keys()) or "(none)",
    )
    cmp = await run_on_both_backends_async(
        _async_patch_call(container.id, patch_operations, **kwargs),
...
@pytest.mark.asyncio
async def test_async_baseline_set(container_for):
    """a single ``set`` on an existing field, no optional kwargs.

    If the baseline fails, async ``patch_item`` is broken on one backend and no other
    result in this file is meaningful.
    """
    cmp = await _run_patch(container_for, [{"op": "set", "path": "/n", "value": 99}],
...
@pytest.mark.asyncio
async def test_async_op_add_new_field(container_for):
    """``add`` introduces a field the seed document does not have."""
    cmp = await _run_patch(container_for, [{"op": "add", "path": "/added", "value": "x"}],
                           summary="add new field")
    cmp.assert_functional_parity()


@pytest.mark.asyncio
async def test_async_op_replace_existing_field(container_for):
    ...
    """
    cmp = await _run_patch(container_for, [{"op": "replace", "path": "/label", "value": "after"}],
                           summary="replace existing field")
    cmp.assert_functional_parity()


@pytest.mark.asyncio
async def test_async_op_remove_field(container_for):
    ...
    """
    cmp = await _run_patch(container_for, [{"op": "remove", "path": "/doomed"}],
                           summary="remove field")
    cmp.assert_functional_parity()


@pytest.mark.asyncio
async def test_async_op_incr(container_for):
    ...
    """
    cmp = await _run_patch(container_for, [{"op": "incr", "path": "/n", "value": 5}],
                           summary="incr /n by 5")
    cmp.assert_functional_parity()


@pytest.mark.asyncio
async def test_async_op_nested_path(container_for):
    """a nested path (``/nested/inner``) must survive the trip intact."""
    cmp = await _run_patch(container_for, [{"op": "set", "path": "/nested/inner", "value": 42}],
                           summary="set nested path")
    cmp.assert_functional_parity()


@pytest.mark.asyncio
async def test_async_multiple_operations_applied_in_order(container_for):
    ...
    cmp = await _run_patch(container_for, operations,
...
@pytest.mark.skip(reason="Confirmed rust gap: pre_trigger_include does not work on patch_item. "
                         ...
                         "fresh client each time), and it reproduces whether or not the named "
                         "trigger exists, so it is not trigger-validation. A plain patch with no "
                         "kwargs succeeds on both backends, so the failure is specific to carrying "
                         "this option on patch. Suspected to originate in the driver's patch "
                         "implementation, which commits a patch as an internal read-merge-replace "
                         "(azure_data_cosmos_driver/src/driver/pipeline/patch_handler.rs) rather "
                         "than as a single request. Un-skip once the driver is fixed and the "
                         "extension is rebuilt.")
@pytest.mark.asyncio
async def test_async_pre_trigger_include(container_with_trigger):
    ...
    """
    cmp = await _run_patch(container_with_trigger, [{"op": "set", "path": "/n", "value": 2}],
...
@pytest.mark.skip(reason="Confirmed rust gap: same failure as the pre_trigger_include test above "
                         ...
                         "core-python succeeds). Kept as a separate test so the post-trigger "
                         "surface is covered independently once the driver gap is fixed.")
@pytest.mark.asyncio
async def test_async_post_trigger_include(container_for):
    ...
    cmp = await _run_patch(container_for, [{"op": "set", "path": "/n", "value": 2}],
...
@pytest.mark.skip(reason="Confirmed rust divergence: a caller-supplied session_token on patch_item "
                         ...
                         "CosmosResourceNotFoundError 404/sub-status 1002 'The read session is not "
                         "available for the input session token'. Deterministic (2/2 repeats, a "
                         "fresh client each time). Neither behaviour is obviously wrong in "
                         "isolation -- core-python's session retry policy clears an unusable token "
                         "and retries, which rust does not do -- but the two backends must agree "
                         "before this ships. The same token is accepted by both backends on "
                         "create_item, so the divergence is specific to patch.")
@pytest.mark.asyncio
...
@pytest.mark.skip(reason="Confirmed rust gap: no_response is ignored on patch_item. Verified live "
                         ...
                         "(Content-Length 0) while rust returns the full post-image. The same "
                         "flag IS honoured by rust on create_item in the very same probe, so this "
                         "is specific to patch. Root cause is visible in the driver: it commits a "
                         "patch as an internal read-merge-replace and then synthesizes the "
                         "response from its own locally-merged bytes "
                         ...
                         "bandwidth on an opt-out that silently does nothing.")
@pytest.mark.asyncio
...
@pytest.mark.asyncio
async def test_async_filter_predicate_matching(container_for):
...
async def test_async_response_hook_fires_once(container_for):
    ...
    cmp = await run_on_both_backends_async(
        _do,
        description="async patch response_hook fires exactly once per backend",
        request_kwargs={"response_hook": "<callable>"},
    )
    cmp.print_report()
    print("async patch response_hook fired: core-python={} rust={}".format(
...
@pytest.mark.asyncio
async def test_async_patch_missing_id_raises_not_found(container_for):
    ...
    cmp = await run_on_both_backends_async(
        _do, description="async patch missing id -> 404")
    cmp.print_report()
...
@pytest.mark.asyncio
async def test_async_stale_etag_raises_precondition_failed(container_for):
    ...
    cmp = await run_on_both_backends_async(
        _do, description="async patch with stale etag -> 412")
    cmp.print_report()
    assert not cmp.core_python.succeeded
    assert not cmp.rust.succeeded
    cmp.assert_functional_exception_parity()
...
@pytest.mark.asyncio
async def test_async_current_etag_succeeds(container_for):
    ...
    cmp = await run_on_both_backends_async(
        _do, description="async patch with current etag -> success")
    cmp.print_report()
    cmp.assert_functional_parity()


@pytest.mark.asyncio
async def test_async_etag_without_match_condition_raises_value_error(container_for):
    ...
    Pinning it on the async path proves async reaches that same shared gate.
    """
    async def _do(client):
        cont = client.get_database_client("parity_db").get_container_client(container_for.id)
        item_id = uuid.uuid4().hex
    ...
    cmp = await run_on_both_backends_async(
        _do, description="async patch etag without match_condition -> ValueError")
    cmp.print_report()
...
@pytest.mark.skip(reason="Confirmed rust gap: a patch whose path cannot be resolved is not mapped "
                         ...
                         "Deterministic (2/2 repeats). This matters to customers: code that "
                         "catches CosmosHttpResponseError or inspects status_code -- the "
                         "documented way to handle a bad patch -- does not work on the rust path. "
                         "Note the 404 and 412 patch tests above DO produce correctly typed "
                         "exceptions, so the gap is confined to failures the driver detects "
                         "locally while merging rather than reporting from an HTTP response.")
@pytest.mark.asyncio
async def test_async_invalid_patch_path_raises(container_for):
