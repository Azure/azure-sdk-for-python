...
class TestSubpartitionCrudAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_collection_crud_subpartition_async(self):
        ...
        created_collection = await created_db.create_container(
            ...
            partition_key=PartitionKey(path=["/pk1", "/pk2"], kind="MultiHash"),
        ...
        container_proxy = await created_db.create_container(
        ...
        assert (
            PartitionKey(path=["/id1", "/id2", "/id3"], kind="MultiHash") == container_proxy_properties["partitionKey"]
        )
    ...
    async def test_partitioned_collection_prefix_partition_query_subpartition_async(self):
        ...
        document_list = [
            document async for document in created_collection.query_items(query="Select * from c", partition_key=["CA"])
        ]
        ...
        document_list = [
            document
            async for document in created_collection.query_items(
                query="Select * from c", partition_key=["CA", "Oxnard"]
            )
        ]  # pylint: disable=line-too-long
        ...
        document_list = [
            document
            async for document in created_collection.query_items(
                query='Select * from c where c.zipcode = "93033"'
                # pylint: disable=line-too-long
                ,
                partition_key=["CA"],
            )
        ...
        document_list = [
            document
            async for document in created_collection.query_items(query="Select * from c", partition_key=[None, "93033"])
        ]
        ...
        document_list = [
            document
            async for document in created_collection.query_items(query="Select * from c", partition_key=[0xFF, 0xFF])
        ]
        assert 0 == len(document_list)
        ...
        try:
            document_list = [
                document async for document in created_collection.query_items(query="Select * from c", partition_key=[])
            ]
            self.fail("Test did not fail as expected")
        except exceptions.CosmosHttpResponseError as error:
            assert error.status_code == StatusCodes.BAD_REQUEST
