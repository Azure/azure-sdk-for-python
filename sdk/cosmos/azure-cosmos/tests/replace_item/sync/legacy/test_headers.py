...
"""Sync ``test_container_replace_item_throughput_bucket`` on the
``_backend="rust"`` path.
...
Run: ``pytest --noconftest tests/replace_item/sync/legacy/test_headers.py -v``
"""
import os
import unittest
import uuid
...
class TestHeaders(unittest.TestCase):
    ...
    def test_container_replace_item_throughput_bucket(self):
        """Verify replace_item forwards the per-request throughput_bucket kwarg as the x-ms-cosmos-throughput-bucket header (item id differs from the body id, exercising the URL-id-from-item path)."""
        # Source: tests/test_headers.py::TestHeaders.test_container_replace_item_throughput_bucket
