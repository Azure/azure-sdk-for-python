...
class TestRetryableWrites:
    ...
    def setup_method_with_custom_transport(
        ...
        **kwargs
        ...
        client = CosmosClient(default_endpoint, key, consistency_level="Session", transport=custom_transport, **kwargs)
    ...
    def test_retryable_writes_request_level(self, setup):
        ...
        read_item_create = container.read_item(item=test_item_create["id"], partition_key=test_item_create["pk"])
        ...
        try:
            container.replace_item(
                item=test_item_replace["id"], body={"id": "3", "pk": "test", "data": "updated data"}, retry_write=2
            )
        except exceptions.CosmosHttpResponseError as e:
            assert me.counter > 1, "Expected multiple retries due to simulated errors"
        finally:
        ...
        replaced_item = container.read_item(item=test_item_replace["id"], partition_key=test_item_replace["pk"])
    ...
    def test_retryable_writes_client_level(self, setup):
        ...
        try:
            container.replace_item(item=test_item_replace["id"], body={"id": "8", "pk": "test", "data": "updated data"})
        except exceptions.CosmosHttpResponseError as e:
            assert me.counter > 1, "Expected multiple retries due to simulated errors"
        finally:
            # Restore the original retry_utility.execute function
    ...
    def test_retryable_writes_hpk_request_level(self, setup):
        ...
        # Verify the item was created
        read_item_create = container.read_item(
        ...
        # Verify the item was patched
        patched_item_hpk = container.read_item(
            item=test_item["id"], partition_key=[test_item["state"], test_item["city"]]
        )
        assert (
            patched_item_hpk["data"] == "patched retryable write test"
        ), "Item was not patched successfully after retries"
        ...
        try:
            container.replace_item(
                ...
                body={"id": "6", "state": "CA", "city": "San Francisco", "data": "updated data"},
            )
            pytest.fail("Expected an exception without retry_write")
        except exceptions.CosmosHttpResponseError:
            assert me.counter == 1, "Expected no retries without retry_write"
        ...
        try:
            ...
            _retry_utility.ExecuteFunction = original_execute
        # Verify the item was replaced
        replaced_item_hpk = container.read_item(
    ...
    def test_retryable_writes_hpk_client_level(self, setup):
        ...
        try:
            container.replace_item(
                item=test_item_replace["id"],
                body={"id": "8", "state": "CA", "city": "San Francisco", "data": "updated data"},
            )
        except exceptions.CosmosHttpResponseError as e:
            assert me.counter > 1, "Expected multiple retries due to simulated errors"
        finally:
    ...
    def test_retryable_writes_mrw_request_level(self, injected_error):
        ...
        is_request_to_first_region: Callable[[HttpRequest], bool] = (
            lambda r: FaultInjectionTransport.predicate_targets_region(r, first_region_uri)
            and FaultInjectionTransport.predicate_is_document_operation(r)
        )

        custom_transport.add_fault(is_request_to_first_region, lambda r: injected_error)
        ...
        replace_document = container.replace_item(
        ...
        patch_document = container.patch_item(
