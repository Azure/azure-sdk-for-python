...
def create_doc():
    return {"id": str(uuid.uuid4()), "pk": PK_VALUE, "name": "sample document", "key": "value"}
...
def create_errors(errors=None):
    ...
    for error_code in error_codes:
        errors.append(CosmosHttpResponseError(status_code=error_code, message="Some injected error."))
...
def validate_unhealthy_partitions(global_endpoint_manager, expected_unhealthy_partitions):
    health_info_map = (
        global_endpoint_manager.global_partition_endpoint_manager_core.partition_health_tracker.pk_range_wrapper_to_health_info
    )
...
def perform_write_operation(operation, container, fault_injection_container, doc_id, pk, expected_uri=None):
    doc = {"id": doc_id, "pk": pk, "name": "sample document", "key": "value"}
    if operation == CREATE:
        ...
        sleep(1)
        new_doc = {"id": doc_id, "pk": pk, "name": "sample document" + str(uuid), "key": "value"}
        ...
        resp = fault_injection_container.patch_item(
            item=doc["id"], partition_key=doc["pk"], patch_operations=operations
        )
...
class TestPerPartitionCircuitBreakerMM:
    ...
    def test_write_consecutive_failure_threshold(self, write_operation, error):
        ...
        container, doc, expected_uri, uri_down, fault_injection_container, custom_transport, predicate = (
            self.setup_info(error_lambda)
        )
        global_endpoint_manager = fault_injection_container.client_connection._global_endpoint_manager

        with pytest.raises((CosmosHttpResponseError, ServiceResponseError)) as exc_info:
            perform_write_operation(
                write_operation, container, fault_injection_container, str(uuid.uuid4()), PK_VALUE, expected_uri
            )
        ...
        try:
            perform_write_operation(
                write_operation, container, fault_injection_container, str(uuid.uuid4()), PK_VALUE, uri_down
            )
        finally:
    ...
    def test_read_consecutive_failure_threshold(self, read_operation, error):
        ...
        perform_read_operation(read_operation, fault_injection_container, doc["id"], doc["pk"], expected_uri)
        ...
        try:
            ...
            for i in range(10):
                ...
                if i == 4 or i == 8:
                    ...
                    custom_transport.add_fault(predicate, lambda r: FaultInjectionTransport.error_after_delay(0, error))
                else:
                    with pytest.raises((CosmosHttpResponseError, ServiceResponseError)) as exc_info:
                        perform_write_operation(
                            write_operation,
    ...
    def test_read_failure_rate_threshold(self, read_operation, error):
        ...
        try:
            ...
            for i in range(num_operations):
                ...
                perform_read_operation(read_operation, fault_injection_container, doc["id"], PK_VALUE, expected_uri)
            if read_operation in (CHANGE_FEED, QUERY, READ_ALL_ITEMS):
                # these operations are cross partition so they would mark both partitions as unavailable
                expected_unhealthy_partitions = 5
            else:
    ...
    def test_stat_reset(self):
        ...
        error_lambda = lambda r: FaultInjectionTransport.error_after_delay(
            0, CosmosHttpResponseError(status_code=status_code, message="Some injected error.")
        )
        container, doc, expected_uri, uri_down, fault_injection_container, custom_transport, predicate = (
        ...
        try:
            for i in range(2):
                validate_unhealthy_partitions(global_endpoint_manager, 0)
                # read will fail and retry in other region
                perform_read_operation(READ, fault_injection_container, doc["id"], PK_VALUE, expected_uri)
                try:
                    perform_write_operation(
                        CREATE, container, fault_injection_container, str(uuid.uuid4()), PK_VALUE, expected_uri
                    )
    ...
    def test_service_request_error(self, read_operation, write_operation):
        ...
        custom_transport.add_fault(predicate, lambda r: FaultInjectionTransport.error_region_down())
    ...
    def test_circuit_breaker_user_agent_feature_flag_mm(self):
        ...
        container.upsert_item(
            body={"id": str(uuid.uuid4()), "pk": PK_VALUE, "name": "sample document", "key": "value"},
