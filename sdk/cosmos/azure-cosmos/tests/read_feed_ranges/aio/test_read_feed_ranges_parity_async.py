...
# license information.
# -------------------------------------------------------------------------
"""Async parity tests for ``Container.read_feed_ranges``: run the same call once on the existing
...
def container_for(request):
    ...
    container_id = "parity_feed_ranges_async_" + request.node.name + "_" + uuid.uuid4().hex[:6]
...
async def test_read_feed_ranges_baseline_async(container_for):
    ...
    async def _do(client):
        ...
        return _normalize_feed_ranges([feed_range async for feed_range in container.read_feed_ranges()])
...
async def test_read_feed_ranges_force_refresh_async(container_for):
    ...
    async def _do(client):
        container = client.get_database_client("parity_db").get_container_client(container_for.id)
        async for _ in container.read_feed_ranges():
            pass
        return _normalize_feed_ranges(
            [feed_range async for feed_range in container.read_feed_ranges(force_refresh=True)]
        )
