...
def create_failover_errors():
    ...
    for error_code in error_codes:
        errors.append(CosmosHttpResponseError(status_code=error_code, message="Some injected error.", sub_status=3))
    return errors
...
class TestPerPartitionAutomaticFailover:
    ...
    def setup_info(
        ...
        predicate = lambda r: (
            FaultInjectionTransport.predicate_req_for_document_with_id(r, doc_fail_id)
            and FaultInjectionTransport.predicate_is_write_operation(r, "com")
        )
        ...
        if is_batch:
            success_response = FaultInjectionTransport.MockHttpResponse(
                mock_request,
                200,
                [{"statusCode": 200}],
            )
        ...
        if session_error:
            read_predicate = lambda r: (
                FaultInjectionTransport.predicate_is_operation_type(r, "Read")
    ...
    def test_ppaf_partition_info_cache_and_routing(self, write_operation, error, exclude_regions):
        ...
        fault_injection_container.create_item(
        ...
        perform_write_operation(write_operation, container, fault_injection_container, doc_fail_id, PK_VALUE)
        ...
        perform_write_operation(write_operation, container, fault_injection_container, str(uuid.uuid4()), PK_VALUE)
        partition_info = global_endpoint_manager.partition_range_to_failover_info[pk_range_wrapper]
    ...
    def test_ppaf_partition_thresholds_and_routing(self, write_operation, error, exclude_regions):
        ...
        error_lambda = lambda r: FaultInjectionTransport.error_after_delay(0, error)
        setup, doc_fail_id, doc_success_id, custom_setup, custom_transport, predicate = self.setup_info(
            error=error_lambda, exclude_client_regions=exclude_regions
        )
        container = setup["col"]
        fault_injection_container = custom_setup["col"]
        global_endpoint_manager = fault_injection_container.client_connection._global_endpoint_manager
    ...
    def test_ppaf_user_agent_feature_flag(self):
        ...
        fault_injection_container.upsert_item(
            body={"id": doc_success_id, "pk": PK_VALUE, "name": "sample document", "key": "value"},
