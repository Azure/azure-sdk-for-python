...
class TestHealthCheckAsync:
    ...
    async def test_health_check_success_async(self, setup):
        ...
        async def mock_execute_function(function, *args, **kwargs):
            if args:
                ...
                args[1].location_endpoint_to_route = (
                    args[1].location_endpoint_to_route.replace("-eastus", "").replace("-westus", "")
                )
        ...
        locational_endpoint = _location_cache.LocationCache.GetLocationalEndpoint(self.host, REGION_2)
        expected_regional_routing_contexts.append(RegionalRoutingContext(locational_endpoint))
        read_regional_routing_context = setup[
            COLLECTION
    ...
    async def test_health_check_failure_async(self, setup):
        ...
        setup[
            COLLECTION
        ].client_connection._global_endpoint_manager.location_cache.location_unavailability_info_by_endpoint.clear()
        self.original_getDatabaseAccountStub = (
            _global_endpoint_manager_async._GlobalEndpointManager._GetDatabaseAccountStub
        )
        _global_endpoint_manager_async._GlobalEndpointManager._GetDatabaseAccountStub = self.MockGetDatabaseAccount(
        ...
        try:
            ...
            while (
                len(
                    setup[
                        COLLECTION
                    ].client_connection._global_endpoint_manager.location_cache.location_unavailability_info_by_endpoint
                )
                < len(REGIONS)
                and time.time() - start_time < 10
            ):
        ...
        unavailable_endpoint_info = setup[
            COLLECTION
        ].client_connection._global_endpoint_manager.location_cache.location_unavailability_info_by_endpoint
        assert len(unavailable_endpoint_info) == num_unavailable_endpoints
        write_endpoints = {_location_cache.LocationCache.GetLocationalEndpoint(self.host, REGION_1)}
