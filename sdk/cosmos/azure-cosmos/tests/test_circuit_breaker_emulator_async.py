...
from test_per_partition_circuit_breaker_mm import (
    create_doc,
...
async def create_custom_transport_mm():
    ...
    emulator_as_multi_write_region_account_transformation = (
        lambda r, inner: FaultInjectionTransportAsync.transform_topology_mwr(
            first_region_name="Write Region", second_region_name="Read Region", inner=inner
        )
    )
...
class TestCircuitBreakerEmulatorAsync:
    ...
    async def create_custom_transport_sm_mrr(self):
        ...
        is_write_operation_in_read_region_predicate = (
            lambda r: FaultInjectionTransportAsync.predicate_is_write_operation(r, self.host)
        )

        custom_transport.add_fault(
            is_write_operation_in_read_region_predicate, lambda r: FaultInjectionTransportAsync.error_write_forbidden()
        )
        ...
        emulator_as_multi_region_sm_account_transformation = (
            lambda r, inner: FaultInjectionTransportAsync.transform_topology_swr_mrr(
                write_region_name="Write Region", read_region_name="Read Region", inner=inner
            )
        )
        custom_transport.add_response_transformation(
            is_get_account_predicate, emulator_as_multi_region_sm_account_transformation
        )
        return custom_transport
    ...
    async def setup_info(self, error=None, mm=False):
        ...
        doc = create_doc()
        predicate = lambda r: (
            FaultInjectionTransportAsync.predicate_is_resource_type(r, ResourceType.Collection)
            and FaultInjectionTransportAsync.predicate_is_operation_type(r, documents._OperationType.Delete)
            and FaultInjectionTransportAsync.predicate_targets_region(r, uri_down)
        )
        if error is not None:
            custom_transport.add_fault(predicate, error)
        custom_setup = await self.setup_method_with_custom_transport(
            custom_transport, default_endpoint=self.host, multiple_write_locations=mm
        )
        setup = await self.setup_method_with_custom_transport(
            None, default_endpoint=self.host, multiple_write_locations=mm
        )
    ...
    async def test_write_consecutive_failure_threshold_delete_all_items_by_pk_sm_async(self, setup_teardown, error):
        ...
        setup, doc, expected_uri, uri_down, custom_setup, custom_transport, predicate = await self.setup_info(
            error_lambda
        )
