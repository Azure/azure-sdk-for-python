...
class TestVectorSimilarityQuery(unittest.TestCase):
    ...
    def test_ordering_distances(self):
        ...
        for i in range(1, 11):
            ...
            flat_list = list(
                self.created_flat_euclidean_container.query_items(
                    query=vanilla_query, enable_cross_partition_query=True
                )
            )
            verify_ordering(flat_list, "euclidean")

            quantized_list = list(
                self.created_quantized_cosine_container.query_items(
            ...
            disk_ann_list = list(
                self.created_diskANN_dotproduct_container.query_items(
                    query=specs_query, enable_cross_partition_query=True
                )
            )
    ...
    def test_vector_query_pagination(self):
        ...
        query_iterable = self.created_quantized_cosine_container.query_items(
            query=query, enable_cross_partition_query=True, max_item_count=3
        )
        all_fetched_res = []
        count = 0
    ...
    def test_vector_query_large_data(self):
        ...
        query_iterable = self.created_large_container.query_items(query=query, enable_cross_partition_query=True)
    ...
    def test_vector_query_cross_partition_response_hook(self):
        ...
        query_iterable = self.created_quantized_cosine_container.query_items(
            query=query, enable_cross_partition_query=True, response_hook=response_hook
        )
        result_list = list(query_iterable)
