...
def setup():
    ...
    test_client = (
        cosmos_client.CosmosClient(
            config.host, config.masterKey, multiple_write_locations=use_multiple_write_locations
        ),
    )
    return {
...
class TestChangeFeed:
    ...
    def test_get_feed_ranges(self, setup):
        created_collection = setup["created_db"].create_container(
            "get_feed_ranges_" + str(uuid.uuid4()), PartitionKey(path="/pk")
        )
    ...
    def test_query_change_feed_with_different_filter(self, change_feed_filter_param, setup):
        created_collection = setup["created_db"].create_container(
            f"change_feed_test_{change_feed_filter_param}_{str(uuid.uuid4())}", PartitionKey(path="/pk")
        )
        ...
        for pageSize in [1, 100]:
            # verify iterator
            query_iterable = created_collection.query_items_change_feed(
                continuation=continuation2, max_item_count=pageSize, **filter_param
            )
    ...
    def test_query_change_feed_with_start_time(self, setup):
        created_collection = setup["created_db"].create_container_if_not_exists(
            "query_change_feed_start_time_test", PartitionKey(path="/pk")
        )
        batchSize = 50
    ...
    def test_query_change_feed_with_multi_partition(self, setup):
        created_collection = setup["created_db"].create_container(
            "change_feed_test_" + str(uuid.uuid4()), PartitionKey(path="/pk"), offer_throughput=11000
        )
