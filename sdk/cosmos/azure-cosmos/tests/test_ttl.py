...
class TestTimeToLive(unittest.TestCase):
    ...
    def test_collection_and_document_ttl_values(self):
        ...
        created_collection = self.created_db.create_container(
            id="test_ttl_values1" + str(uuid.uuid4()), partition_key=PartitionKey(path="/id"), default_ttl=ttl
        )
        ...
        document_definition = {"id": "doc1" + str(uuid.uuid4()), "name": "sample document", "key": "value", "ttl": 0}

        # 0 is an unsupported value for ttl. Valid values are -1 or a non-zero positive 32-bit integer value
        self.__AssertHTTPFailureWithStatus(StatusCodes.BAD_REQUEST, created_collection.create_item, document_definition)
