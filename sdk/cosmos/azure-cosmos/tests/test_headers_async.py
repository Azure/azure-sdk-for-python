...
class TestHeadersAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_request_precedence_throughput_bucket_async(self):
        client = CosmosClient(self.host, self.masterKey, throughput_bucket=client_throughput_bucket_number)
        database = client.get_database_client(self.configs.TEST_DATABASE_ID)
        created_container = await database.create_container(str(uuid.uuid4()), PartitionKey(path="/pk"))
    ...
    async def test_container_delete_all_items_by_partition_key_throughput_bucket_async(self):
        ...
        for i in range(1, 3):
            await created_collection.upsert_item(dict(id="item{}".format(i), pk=partition_key1))
    ...
    async def test_request_precedence_priority_async(self):
        ...
        async with CosmosClient(self.host, self.masterKey, priority=client_priority) as client:
            database = client.get_database_client(self.configs.TEST_DATABASE_ID)
