...
- `Client Timeout`: can be changed by passing the `timeout` option. Changes the value of the per-operation(can have multiple requests e.g for cross partition queries) client timeout. If not present,
