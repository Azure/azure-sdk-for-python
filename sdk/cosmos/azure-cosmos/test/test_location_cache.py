...
class TestLocationCache(unittest.TestCase):
    ...
    def create_spy_client(self, use_multiple_write_locations, enable_endpoint_discovery,
        ...
        client = cosmos_client_connection.CosmosClientConnection(self.DEFAULT_ENDPOINT, {'masterKey': "SomeKeyValue"},
                                                                 consistency_level="Session",
                                                                 connection_policy=connectionPolicy)
        return client

    def test_validate_retry_on_session_not_availabe_with_disable_multiple_write_locations_and_endpoint_discovery_disabled(
            self):
    ...
    def validate_retry_on_session_not_availabe_with_endpoint_discovery_disabled(self, is_preferred_locations_list_empty,
        ...
        client = self.create_spy_client(use_multiple_write_locations, enable_endpoint_discovery,
                                        is_preferred_locations_list_empty)
    ...
    def test_validate_location_cache(self):
        ...
        for i in range(0, 8):
            ...
            self.validate_location_cache(use_multiple_write_locations, endpoint_discovery_enabled,
                                         is_preferred_list_empty)
        cosmos_client_connection.CosmosClientConnection.GetDatabaseAccount = self.original_get_database_account
    ...
    def initialize(self, use_multiple_write_locations, enable_endpoint_discovery, is_preferred_locations_list_empty):
        ...
        connectionPolicy.ConnectionRetryConfiguration = 5
        client = cosmos_client_connection.CosmosClientConnection("", {}, consistency_level="Session",
                                                                 connection_policy=connectionPolicy)
        self.global_endpoint_manager = client._global_endpoint_manager

    def validate_location_cache(self, use_multiple_write_locations, endpoint_discovery_enabled,
                                is_preferred_list_empty):
        for write_location_index in range(3):
            for read_location_index in range(2):
                ...
                for i in range(0, read_location_index):
                    self.location_cache.mark_endpoint_unavailable_for_read(
                        self.database_account.ReadableLocations[i]['databaseAccountEndpoint'])
                    self.global_endpoint_manager.mark_endpoint_unavailable_for_read(
                        self.database_account.ReadableLocations[i]['databaseAccountEndpoint'])

                for i in range(0, write_location_index):
                    self.location_cache.mark_endpoint_unavailable_for_write(
                        self.database_account.WritableLocations[i]['databaseAccountEndpoint'])
                ...
                self.validate_endpoint_refresh(use_multiple_write_locations, endpoint_discovery_enabled,
                ...
                self.validate_request_endpoint_resolution(use_multiple_write_locations, endpoint_discovery_enabled,
                                                          available_write_endpoints, available_read_endpoints)
    ...
    def validate_endpoint_refresh(self, use_multiple_write_locations, endpoint_discovery_enabled,
                                  preferred_available_write_endpoints,
        ...
        if len(self.preferred_locations) > 0:
            ...
            is_most_preferred_location_unavailable_for_read = True if len(preferred_available_read_endpoints) == 0 else \
                preferred_available_read_endpoints[0] != most_preferred_read_endpoint
            ...
            if use_multiple_write_locations:
                is_most_preferred_location_unavailable_for_write = True if len(
                    preferred_available_write_endpoints) == 0 else preferred_available_write_endpoints[
                                                                       0] != most_preferred_write_endpoint

        if not endpoint_discovery_enabled:
            self.assertFalse(should_refresh_endpoints)
        else:
            self.assertEqual(
                is_most_preferred_location_unavailable_for_read or is_most_preferred_location_unavailable_for_write,
    ...
    def validate_request_endpoint_resolution(self, use_multiple_write_locations, endpoint_discovery_enabled,
        ...
        if not endpoint_discovery_enabled:
            ...
            second_available_write_endpoint = write_endpoint if write_endpoint != first_available_write_endpoint else \
                available_write_endpoints[1]
        else:
            first_available_write_endpoint = self.DEFAULT_ENDPOINT
            second_available_write_endpoint = self.DEFAULT_ENDPOINT
        ...
        first_write_endpoint = self.DEFAULT_ENDPOINT if not endpoint_discovery_enabled else \
            self.database_account.WritableLocations[0]['databaseAccountEndpoint']

        second_write_endpoint = self.DEFAULT_ENDPOINT if not endpoint_discovery_enabled else \
            self.database_account.WritableLocations[1]['databaseAccountEndpoint']
        ...
        self.assertTrue(
            second_available_write_endpoint == self.resolve_endpoint_for_write_request(ResourceType.Document, True))
        self.assertTrue(
            first_available_write_endpoint == self.resolve_endpoint_for_write_request(ResourceType.Document, False))
    ...
    def resolve_endpoint_for_write_request(self, resource_type, use_alternate_write_endpoint):
        ...
        request = RequestObject(resource_type, operation_type)
        request.route_to_location_with_preferred_location_flag(1 if use_alternate_write_endpoint else 0,
                                                               ResourceType.IsCollectionChild(resource_type))
        return self.location_cache.resolve_service_endpoint(request)
