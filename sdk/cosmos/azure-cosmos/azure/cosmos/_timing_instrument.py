...
Call azure.cosmos._timing.print_timing_summary() when done.
"""
import time
from azure.cosmos import _timing
...
# 1. Instrument ExecuteAsync - retry policy setup + overall retry wrapper
...
async def _timed_execute_async(client, global_endpoint_manager, function, *args, **kwargs):
    ...
    _timing.record("ExecuteAsync (retry wrapper)", time.perf_counter() - t0)
    return result

_retry_utility_async.ExecuteAsync = _timed_execute_async


# 2. Instrument AsynchronousRequest - everything from body serialization to response parsing
...
async def _timed_async_request(client, request_params, global_endpoint_manager,
    ...
    result = await _orig_async_request(client, request_params, global_endpoint_manager,
                                        connection_policy, pipeline_client, request, request_data, **kwargs)
    _timing.record("AsynchronousRequest (total)", time.perf_counter() - t0)
    return result

_asynchronous_request.AsynchronousRequest = _timed_async_request


# 3. Instrument _PipelineRunFunction - just the HTTP pipeline
...
async def _timed_pipeline_run(pipeline_client, request, **kwargs):
    ...
    _timing.record("PipelineRunFunction (HTTP+policies)", time.perf_counter() - t0)
    return result
...
async def _timed_fetch_no_retries(self, fetch_function):
    ...
    _timing.record("fetch_items_no_retries", time.perf_counter() - t0)
    return result
...
def _timed_get_headers(*args, **kwargs):
    ...
    _timing.record("GetHeaders", time.perf_counter() - t0)
    return result

_base.GetHeaders = _timed_get_headers

print("[TIMING] Instrumentation installed. Call azure.cosmos._timing.print_timing_summary() when done.")
