...
# license information.
# -------------------------------------------------------------------------
"""The throughput family coordinator: reads and replaces a container's throughput.

This is the throughput counterpart to
:class:`~azure.cosmos._helpers.item_helper.ItemHelper`. The public proxy methods
``ContainerProxy.get_throughput`` and ``ContainerProxy.replace_throughput`` (sync
and async) each gather their arguments and call one function here; the function
does the engine work and hands back a finished ``ThroughputProperties``.

What "throughput" is, in customer terms: the provisioned request-unit (RU/s)
budget for a container. Cosmos does not store that number on the container itself
but as a separate account-level "offer" record, so both operations first read
that offer (a query against the account's offers), and ``replace_throughput``
...
from .._base import _deserialize_throughput, _replace_throughput, build_options
...
def _offer_query(container_self_link: str) -> dict[str, Any]:
    # The filter both engines send to find this container's single offer record:
    # the account's offers feed keyed by the container's self-link.
    return {
...
def _rust_call_state(
    client_connection: Any,
    container_rid: str,
    kwargs: Mapping[str, Any],
) -> tuple[Any, Dict[str, Any], Dict[str, Any]]:
    # Build the three things the rust path needs from one public call: the client's
    # selected backend, the rust request options, and the leftover kwargs used only
    # to decide rust-eligibility. Returned together so each public function reads
    # ``client_connection._backend`` exactly once, in one place.
...
def get_throughput(
    *,
    ...
    query_spec = _offer_query(properties["_self"])
    container_rid = properties["_rid"]
    legacy_options: Dict[str, Any] = {Constants.ContainerRID: container_rid}
    selected_backend, rust_options, rust_kwargs = _rust_call_state(client_connection, container_rid, kwargs)
...
async def get_throughput_async(
    *,
    client_connection: Any,
    container_link: str,
...
async def replace_throughput_async(
    *,
    client_connection: Any,
    container_link: str,
    get_properties: Callable[[], Awaitable[Mapping[str, Any]]],
