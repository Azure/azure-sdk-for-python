...
# Database throughput is shared by the containers in that database.
...
def get_database_throughput(
    *,
    ...
    properties = get_properties()
    query_spec = _offer_query(properties["_self"])
    selected_backend, rust_options, rust_kwargs = _rust_call_state(client_connection, None, kwargs)
    backend = coerce_backend(selected_backend)
    offers = backend.run_operation(
        build_prepared=lambda: prepare_read_offer_request(
            client_connection=client_connection,
