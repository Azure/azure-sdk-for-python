...
class _GlobalPartitionEndpointManagerForPerPartitionAutomaticFailoverAsync(
    ...
    def is_per_partition_automatic_failover_applicable(self, request: RequestObject) -> bool:
        ...
        if self.location_cache.can_use_multiple_write_locations_for_request(
            request
        ...
        if (
            ...
            and request.operation_type != _OperationType.ExecuteJavaScript
        ):
            return False

        return True

    def try_ppaf_failover_threshold(self, pk_range_wrapper: "PartitionKeyRangeWrapper", request: "RequestObject"):
        ...
        if request and self.is_per_partition_automatic_failover_applicable(request):
            if self.ppaf_thresholds_tracker.get_pk_failures(pk_range_wrapper) >= int(
                os.environ.get(Constants.TIMEOUT_ERROR_THRESHOLD_PPAF, Constants.TIMEOUT_ERROR_THRESHOLD_PPAF_DEFAULT)
            ):
                ...
                with self._threshold_lock:
                    ...
                    if self.ppaf_thresholds_tracker.get_pk_failures(pk_range_wrapper) >= int(
                        os.environ.get(
                            Constants.TIMEOUT_ERROR_THRESHOLD_PPAF, Constants.TIMEOUT_ERROR_THRESHOLD_PPAF_DEFAULT
                        )
                    ):
                        ...
                        logger.warning(
                            "PPAF - Failover threshold reached for partition key range: %s for region: %s",  # pylint: disable=line-too-long
                            pk_range_wrapper,
                            location,
                        )
                        regional_context = self.location_cache.account_read_regional_routing_contexts_by_location.get(
                            location
                        ).primary_endpoint
                        partition_level_info.unavailable_regional_endpoints[location] = regional_context

    def resolve_service_endpoint_for_partition(
        self, request: RequestObject, pk_range_wrapper: Optional[PartitionKeyRangeWrapper]
    ) -> str:
            ...
            if pk_range_wrapper in self.partition_range_to_failover_info:
                ...
                if request.location_endpoint_to_route is not None:
                    ...
                    if endpoint_region in partition_failover_info.unavailable_regional_endpoints:
                        ...
                        if (
                            partition_failover_info.current_region is not None
                            and endpoint_region != partition_failover_info.current_region
                        ):
                            ...
                            if len(self.location_cache.account_read_regional_routing_contexts_by_location) == len(
                                partition_failover_info.unavailable_regional_endpoints
                            ):
                                ...
                                logger.warning(
                                    "All available regions for partition %s are unavailable." " Refreshing cache.",
                                    pk_range_wrapper,
                                )
    ...
    async def record_failure(
        self, request: RequestObject, pk_range_wrapper: Optional[PartitionKeyRangeWrapper] = None
    ) -> None:
