...
contract has the same shape as the sync one (``create_item`` plus a
``name`` identifier) with one difference: ``create_item`` here is a
coroutine, so the async container can ``await`` it without bridging
threads.
...
class AsyncCosmosBackend(abc.ABC):
    ...
    inherits from this class. The async container's ``create_item``
    method holds one of these by interface and awaits ``create_item`` on
    it without knowing which concrete backend it has — that is the
    ...
    that returns ``None`` from ``create_item`` is interpreted as
    "I have nothing to return; the caller should run its existing
    in-place implementation." That contract goes away once the helper
    layer takes over request prep and response parsing — at that point
    every backend will return a real ``BackendResponse`` and the
    ...
    async def create_item(self, prepared: Optional[PreparedRequest]) -> Optional[BackendResponse]:
        """Issue a single ``create_item`` call and return its result.

        Returning ``None`` (the temporary contract until the helper
        layer takes over request prep) tells the caller to use its
        existing in-place implementation. Returning a
