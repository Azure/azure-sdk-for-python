...
### Breaking Changes
  ...
  - Model `ExtensionValue` moved instance variable `version`, `extension_type` and `publisher` under property `properties` whose type is `ExtensionValueProperties`
  - Model `ExtensionValueV2` moved instance variable `version`, `extension_type`, `publisher`, `extension_uris`, `extension_signature_uri`, `operating_system` and `architecture` under property `properties` whose type is `ExtensionValueV2Properties`
  - Model `LicenseProfileMachineInstanceView` moved instance variable `software_assurance_customer` under property `software_assurance` whose type is `LicenseProfileMachineInstanceViewSoftwareAssurance`
  - Model `LicenseProfileMachineInstanceView` moved instance variable `subscription_status`, `product_type`, `enrollment_date`, `billing_start_date`, `disenrollment_date`, `billing_end_date`, `error` and `product_features` under property `product_profile` whose type is `LicenseProfileArmProductProfileProperties`
...
### Other Changes
  ...
  - Deleted model `AgentVersion`/`HybridIdentityMetadata`/`KeyDetails`/`KeyProperties`/`MachineRunCommandUpdate`/`NetworkConfiguration`/`PrivateLinkScopesResource`/`ProxyResourceAutoGenerated`/`ResourceAutoGenerated` which actually were not used by SDK users
  - Deleted operation mixin `HybridComputeManagementClientOperationsMixin` which actually was not used by SDK users
