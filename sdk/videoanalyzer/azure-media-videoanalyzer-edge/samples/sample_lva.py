...
def build_pipeline_topology():
    ...
    sink = VideoSink(
        ...
        inputs=[node],
        video_name="video",
        local_media_cache_path="/var/lib/videoanalyzer/tmp/",
        local_media_cache_maximum_size_mi_b="1024",
    )
...
def build_live_pipeline():
    ...
    live_pipeline_properties = LivePipelineProperties(
        description="Sample description", topology_name=pipeline_topology_name, parameters=[url_param]
    )
...
def invoke_method_helper(method):
    ...
    payload = registry_manager.invoke_device_module_method(
        device_id=device_id, module_id=module_d, direct_method_request=direct_method
    ).payload
...
def create_remote_device_adapter(device_name, iot_device_name):
    ...
    try:
        ...
        iot_device = registry_manager.create_device_with_certificate_authority(
            device_id=iot_device_name, status="enabled"
        )

    remote_device_properties = RemoteDeviceAdapterProperties(
        target=RemoteDeviceAdapterTarget(host="camerasimulator"),
        iot_hub_device_connection=IotHubDeviceConnection(
            device_id=iot_device_name, credentials=SymmetricKeyCredentials(key=iot_device.authentication.symmetric_key)
        ),
    )
...
def main():
    ...
    remote_device_adapter = create_remote_device_adapter(
        device_name="RemoteDeviceAdapter1", iot_device_name="iotdevicesample1"
    )
    try:
