...
|[`InteractiveBrowserCredential`][interactive_cred_ref]| Interactively authenticates a user with the default system browser. | [OAuth2 authentication code](https://learn.microsoft.com/azure/active-directory/develop/v2-oauth2-auth-code-flow)| It is a known limitation that `InteractiveBrowserCredential` does not work for CodeSpaces. Please use `DeviceCodeCredential` as a workaround.
