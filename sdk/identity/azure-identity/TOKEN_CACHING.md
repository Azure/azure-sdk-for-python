...
Users can enable the cache to fall back to storing data in plaintext by setting the allow_unencrypted_storage argument to True in TokenCachePersistenceOptions. Enabling this does not disable encryption, but does allow plaintext storage as a fallback if encryption attempts fail. This is substantially less secure since tokens aren't encypted to the current user, and anyone with access to the system could potentially access tokens from the cache. As such, enabling this setting is not recommended.
