...
class Request:
    ...
    def assert_matches(self, request):
        ...
        if self.base_url and self.base_url != request.url.split("?")[0]:
            discrepancies.append("url:\n\texpected base url '{}', actual was '{}'".format(self.base_url, request.url))
        ...
        if self.url_substring and self.url_substring not in request.url:
            discrepancies.append("url: '{}' not in '{}'".format(self.url_substring, request.url))

        if self.authority and url.netloc != self.authority:
            add_discrepancy("authority", self.authority, url.netloc)
