...
from azure.identity import CredentialUnavailableError, UserAssertion, UsernamePasswordCredential
    ...
    async def test_obo(self):
        ...
        class AzureClient(SubscriptionClient):
            async def get_resource(self):
                async for _ in self.subscriptions.list():
                    pass
    ...
    async def send(request, **_):
        ...
        return mock_response(
            json_payload=build_aad_response(
                ...
                refresh_token=request.body["assertion"],
...
@pytest.mark.asyncio
async def test_requires_assertion():
    """The credential should raise CredentialUnavailableError when no user assertion is set"""
    with pytest.raises(CredentialUnavailableError):
        await OnBehalfOfCredential("tenant-id", "client-id", "secret").get_token("scope")
