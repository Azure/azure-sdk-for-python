...
async def test_no_cached_token(get_token_method):
    ...
    # acquire_token_silently is called twice: once in the fast-path (outside lock) and
    # once inside the lock for the double-check before making a network request.
    assert credential.acquire_token_silently.call_count == 2
...
@pytest.mark.parametrize("get_token_method", GET_TOKEN_METHODS)
async def test_refresh_on_in_past(get_token_method):
...
@pytest.mark.parametrize("get_token_method", GET_TOKEN_METHODS)
async def test_refresh_on_in_future(get_token_method):
...
@pytest.mark.parametrize("get_token_method", GET_TOKEN_METHODS)
async def test_expired_token_ignores_retry_delay(get_token_method):
...
@pytest.mark.parametrize("get_token_method", GET_TOKEN_METHODS)
async def test_refresh_on_respects_retry_delay(get_token_method):
...
async def test_event_loop_cleanup_removes_locks():
    ...
    # Close and drop all strong references to the spare loop
    spare_loop.close()
    del spare_loop
