...
async def test_refresh_on_in_past(get_token_method):
    ...
    With background refresh, the cached token is returned immediately while a refresh runs
    in the background.
    """
    ...
    # The cached token is returned immediately; refresh occurs in a background task.
    assert token.token == CACHED_TOKEN
