...
class Backend:
    ...
    def _register_default_ciphers(self) -> None:
        ...
        self.register_cipher_adapter(
            ChaCha20,
            type(None),
            GetCipherByName("chacha" if self._lib.CRYPTOGRAPHY_IS_LIBRESSL else "chacha20"),
        )
        ...
        if rust_openssl._legacy_provider_loaded or not self._lib.CRYPTOGRAPHY_OPENSSL_300_OR_GREATER:
    ...
    def _handle_key_loading_error(self, errors: list[rust_openssl.OpenSSLError]) -> typing.NoReturn:
        if not errors:
            raise ValueError(
            ...
            errors[0]._lib_reason_match(self._lib.ERR_LIB_EVP, self._lib.EVP_R_BAD_DECRYPT)
    ...
    def load_key_and_certificates_from_pkcs12(self, data: bytes, password: bytes | None) -> tuple[
        ...
        with self._zeroed_null_terminated_buf(password) as password_buf:
            with self._zeroed_null_terminated_buf(name) as name_buf:
                ...
                if p12 == self._ffi.NULL:
                    ...
                    raise ValueError(
                        ("Failed to create PKCS12 (does the key match the " "certificate?)"),
                        errors,
                    )
