...
class _CipherContext:
    ...
    def __init__(self, backend: Backend, cipher, mode, operation: int) -> None:
        ...
        if evp_cipher == self._backend._ffi.NULL:
            ...
            msg += (
                "is not supported by this backend (Your version of OpenSSL " "may be too old. Current version: {}.)"
        ...
        if res == 0 and (
            (
                ...
                and errors[0]._lib_reason_match(lib.ERR_LIB_EVP, lib.EVP_R_XTS_DUPLICATED_KEYS)
            )
            or (
                lib.Cryptography_HAS_PROVIDERS
                and errors[0]._lib_reason_match(lib.ERR_LIB_PROV, lib.PROV_R_XTS_DUPLICATED_KEYS)
            )
        ):
    ...
    def update_into(self, data: bytes, buf: bytes) -> int:
        ...
        if len(buf) < (total_data_len + self._block_size_bytes - 1):
            raise ValueError(
                "buffer must be at least {} bytes for this payload".format(len(data) + self._block_size_bytes - 1)
            )
    ...
    def finalize(self) -> bytes:
        ...
        if res == 0:
            ...
            self._backend.openssl_assert(
                ...
                or (
                    lib.CRYPTOGRAPHY_IS_BORINGSSL and errors[0].reason == lib.CIPHER_R_DATA_NOT_MULTIPLE_OF_BLOCK_LENGTH
                ),
                errors=errors,
            )
