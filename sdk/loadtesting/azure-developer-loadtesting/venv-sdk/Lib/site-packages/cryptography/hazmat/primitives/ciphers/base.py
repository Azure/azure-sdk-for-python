...
class _AEADCipherContext(AEADCipherContext):
    ...
    def authenticate_additional_data(self, data: bytes) -> None:
        ...
        if self._aad_bytes_processed > self._ctx._mode._MAX_AAD_BYTES:
            raise ValueError(
                "{} has a maximum AAD byte limit of {}".format(self._ctx._mode.name, self._ctx._mode._MAX_AAD_BYTES)
            )
...
class _AEADDecryptionContext(_AEADCipherContext, AEADDecryptionContext):
    def finalize_with_tag(self, tag: bytes) -> bytes:
        ...
        if self._ctx._tag is not None:
            raise ValueError(
                "tag provided both in mode and in call with finalize_with_tag:" " tag should only be provided once"
            )
