...
def _byte_padding_update(buffer_: bytes | None, data: bytes, block_size: int) -> tuple[bytes, bytes]:
    if buffer_ is None:
...
def _byte_unpadding_update(buffer_: bytes | None, data: bytes, block_size: int) -> tuple[bytes, bytes]:
    if buffer_ is None:
        raise AlreadyFinalized("Context was already finalized.")
