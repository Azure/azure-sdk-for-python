...
class PersistenceNotFound(PersistenceError):
    ...
    def __init__(self, err_no=None, message=None, location=None):
        super(PersistenceNotFound, self).__init__(
            err_no=errno.ENOENT, message=message or "Persistence not found", location=location
        )
            ...
            try:
                ...
                if ex.exit_status == self._KeychainError.ITEM_NOT_FOUND:
                    ...
                    raise PersistenceNotFound(
                        ...
                        message=("Keychain persistence not initialized. " "You can recover by call a save() first."),
                    )
...
class LibsecretPersistence(BasePersistence):
    ...
    def __init__(self, signal_location, schema_name=None, attributes=None, **kwargs):
        ...
        """
        # pylint: disable=import-outside-toplevel
        from .libsecret import LibSecretAgent, trial_run  # This uncertain import is deferred till runtime

        trial_run()
        self._agent = LibSecretAgent(schema_name or _auto_hash(signal_location), attributes or {}, **kwargs)
        self._file_persistence = FilePersistence(signal_location)  # Favor composition
    ...
    def load(self):
        ...
        if data is None:
            ...
            raise PersistenceNotFound(
                message=("Keyring persistence not initialized. " "You can recover by call a save() first.")
            )
        return data
