...
class CommonDBTests(unittest.TestCase):
    ...
    def helpTestDataType(
        ...
        for inParam in inputs:
            ...
            if allowedReturnValues:
                ...
                assert isinstance(rs[0], type(pyData)), 'result type "%s" must be instance of %s' % (
                    type(rs[0]),
                    type(pyData),
                )
            ...
            if compareAlmostEqual and DBAPIDataTypeString == "DATETIME":
                ...
                s = float(pyData)
                v = float(rs[0])
                assert abs(v - s) / s < 0.00001, "Values not almost equal recvd=%s, expected=%f" % (rs[0], s)
            else:
                if allowedReturnValues:
                    ok = False
                    self.assertTrue(
    ...
    def testRowIterator(self):
        ...
        for inParam in inputs:
            ...
            try:
                crsr.execute(
                    "INSERT INTO xx_%s (fldId,fldTwo,fldThree,fldFour) VALUES (?,?,?,?)" % config.tmp,
    ...
    def testFormatParamstyle(self):
        ...
        for inParam in inputs:
            fldId += 1
            sql = (
                "INSERT INTO xx_" + config.tmp + " (fldId,fldConst,fldData) VALUES (%s,'thi%s :may cause? trouble', %s)"
            )
            try:
