...
SimpleFixtureFunction = TypeVar("SimpleFixtureFunction", bound=Callable[..., Awaitable[object]])
FactoryFixtureFunction = TypeVar("FactoryFixtureFunction", bound=Callable[..., AsyncIterator[object]])
...
def _get_asyncio_mode(config: Config) -> Mode:
    ...
    try:
        ...
        raise pytest.UsageError(f"{val!r} is not a valid asyncio_mode. Valid modes: {modes}.")
...
def _preprocess_async_fixtures(
    ...
    for fixtures in fixturemanager._arg2fixturedefs.values():
        for fixturedef in fixtures:
            ...
            if fixturedef in processed_fixturedefs or not _is_coroutine_or_asyncgen(func):
                continue
...
def _inject_fixture_argnames(fixturedef: FixtureDef, event_loop_fixture_id: str) -> None:
    """
...
def _perhaps_rebind_fixture_func(func: _T, instance: Optional[Any], unittest: bool) -> _T:
...
def _wrap_asyncgen_fixture(fixturedef: FixtureDef, event_loop_fixture_id: str) -> None:
    ...
    def _asyncgen_fixture_wrapper(request: FixtureRequest, **kwargs: Any):
        ...
        gen_obj = func(**_add_kwargs(func, kwargs, event_loop_fixture_id, event_loop, request))
...
class PytestAsyncioFunction(Function):
    ...
    @classmethod
    def item_subclass_for(cls, item: Function, /) -> Union[Type["PytestAsyncioFunction"], None]:
        """
...
) -> Union[pytest.Item, pytest.Collector, List[Union[pytest.Item, pytest.Collector]], None]:
...
def pytest_pycollect_makeitem_convert_async_functions_to_subclass(
    ...
    """
    hook_result = yield
    node_or_list_of_nodes: Union[pytest.Item, pytest.Collector, List[Union[pytest.Item, pytest.Collector]], None] = (
...
def pytest_collectstart(collector: pytest.Collector) -> None:
    try:
        collector_scope = next(
            scope for cls, scope in _fixture_scope_by_collector_type.items() if isinstance(collector, cls)
        )
...
def pytest_generate_tests(metafunc: Metafunc) -> None:
    ...
    if event_loop_fixture_id:
        ...
        metafunc._arg2fixturedefs[event_loop_fixture_id] = fixturemanager._arg2fixturedefs[event_loop_fixture_id]
...
def pytest_runtest_setup(item: pytest.Item) -> None:
    ...
    if not getattr(obj, "hypothesis", False) and getattr(obj, "is_hypothesis_test", False):
        pytest.fail(
...
def _retrieve_scope_root(item: Union[Collector, Item], scope: str) -> Collector:
    ...
    for node in reversed(item.listchain()):
        if isinstance(node, scope_root_type):
            ...
            return node
    error_message = (
        f"{item.name} is marked to be run in an event loop with scope {scope}, " f"but is not part of any {scope}."
    )
