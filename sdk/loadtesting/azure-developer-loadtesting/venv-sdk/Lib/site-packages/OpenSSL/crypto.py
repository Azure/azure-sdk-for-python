    ...
    def from_cryptography_key(cls, crypto_key: _Key) -> "PKey":
        ...
        if isinstance(crypto_key, (rsa.RSAPublicKey, dsa.DSAPublicKey)):
            return load_publickey(
                FILETYPE_ASN1,
                crypto_key.public_bytes(Encoding.DER, PublicFormat.SubjectPublicKeyInfo),
            )
        else:
...
class X509Name:
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        ...
        if type(name) is not str:  # noqa: E721
            raise TypeError(f"attribute name must be string, not " f"'{type(value).__name__:.200}'")
...
class X509Store:
    ...
    def add_crl(self, crl: Union["_CRLInternal", x509.CertificateRevocationList]) -> None:
        """
        ...
        if isinstance(crl, x509.CertificateRevocationList):
            ...
            raise TypeError("CRL must be of type OpenSSL.crypto.CRL or " "cryptography.x509.CertificateRevocationList")
...
class X509StoreContext:
    ...
    def _exception_from_context(store_ctx: Any) -> X509StoreContextError:
        ...
        """
        message = _ffi.string(_lib.X509_verify_cert_error_string(_lib.X509_STORE_CTX_get_error(store_ctx))).decode(
            "utf-8"
        )
        errors = [
...
utils.deprecated(
    Revoked,
    __name__,
    ("CRL support in pyOpenSSL is deprecated. You should use the APIs " "in cryptography."),
    DeprecationWarning,
    name="Revoked",
)
...
class CRL:
    ...
    @classmethod
    def from_cryptography(cls, crypto_crl: x509.CertificateRevocationList) -> "_CRLInternal":
        """
