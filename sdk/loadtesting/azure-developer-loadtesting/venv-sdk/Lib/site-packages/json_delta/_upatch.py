    ...
    def gen_stanzas(point, max_point, adding=True):
        ...
        if not is_none_key(key):
            for p, next_key in keys:
                next_point, next_key = skip_key(origin + p, next_key, origin, keys, lambda k: k == key)
                ...
                point, key = skip_key(next_point, next_key, origin, keys, is_none_key)
