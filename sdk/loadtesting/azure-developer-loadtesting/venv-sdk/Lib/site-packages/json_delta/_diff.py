...
def diff(
    left_struc, right_struc, array_align=True, compare_lengths=True, common_key_threshold=0.0, verbose=True, key=None
):
    ...
    if array_align or compare_lengths:
        ...
        key_lengths = keypath_lengths([tuple(p) for p in all_paths(right_struc)])
        val_lengths["i"] = 3
        ...
        if array_align == "udiff":
            ...
            pad_val_lengths = {k: v + whitespace_count(v) for k, v in val_lengths.items()}
            pad_obj_lengths = {k: v + whitespace_count(v) for k, v in obj_lengths.items()}
            pad_key_lengths = {k: v + whitespace_count(v) for k, v in key_lengths.items()}
    else:
    ...
    def pad_key_length(keypath):
        ...
        if keypath not in pad_key_lengths:
            pad_key_lengths[keypath] = key_length(keypath) + whitespace_count(keypath)
    ...
    def sub_diff(left_struc, right_struc, key):
        common = 0.0 if common_key_threshold == 0.0 else commonality(left_struc, right_struc)
        if common < common_key_threshold or not structure_comparable(left_struc, right_struc):
        ...
        if compare_lengths and (diff_length([[key[:], right_struc]]) < diff_length(my_diff)):
    ...
    def needle_diff(left_struc, right_struc, key):
        ...
        down_col = 0
        lastrow = [[[key + [sub_i]] for sub_i in range(i)] for i in range(len(left_struc), -1, -1)]
        ...
        def modify_cand():
            ...
            if col_i + 1 < len(lastrow):
                ...
                return basis + sub_diff(left_elem, right_elem, key=mutand_key)
        ...
        for right_i, right_elem in enumerate(right_struc):
            ...
            for left_i, left_elem in enumerate(left_struc):
                ...
                cands = (c for c in (modify_cand(), delete_cand(), append_cand(), insert_cand()) if c is not None)
    ...
    if verbose:
        msg = "Size of delta {:.3f}% size of original " "(original: {} chars, delta: {} chars)"
        print(msg.format(*compute_diff_stats(right_struc, my_diff, True), file=sys.stderr))
    return my_diff
...
def split_diff(stanzas):
    ...
    return (objs, seqs[mod_point:ins_point], seqs[:mod_point], seqs[ins_point:])
