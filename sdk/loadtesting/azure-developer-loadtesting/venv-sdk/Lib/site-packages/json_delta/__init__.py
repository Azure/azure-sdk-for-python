...
def diff(
    ...
    """
    return _diff_func(left_struc, right_struc, verbose=verbose, key=key, **_check_diff_parms(vars()))
...
def _check_diff_parms(parms):
    ...
    """
    options = {k: parms[k] for k in ("array_align", "compare_lengths", "common_key_threshold") if parms[k] is not None}
    if parms["minimal"] is None:
        return options

    if parms["minimal"]:
        defaults = {"array_align": True, "compare_lengths": True, "common_key_threshold": 0.0}
    else:
        defaults = {"array_align": False, "compare_lengths": False, "common_key_threshold": 0.5}

    if (not set(defaults.items()).issuperset(options.items())) and parms["verbose"]:
...
\tcommon_key_threshold: {common_key_threshold}""".format(
                **defaults
            ),
            file=sys.stderr,
        )
...
def load_and_diff(
    ...
    both=None,
    array_align=None,
    compare_lengths=None,
    common_key_threshold=None,
    minimal=None,
    verbose=True,
):
...
def load_and_udiff(left=None, right=None, both=None, stanzas=None, indent=0):
    ...
    '''
    return _load_and_func(udiff, left, right, both, patch=stanzas, indent=indent)


def load_and_upatch(struc=None, json_udiff=None, both=None, reverse=False):
