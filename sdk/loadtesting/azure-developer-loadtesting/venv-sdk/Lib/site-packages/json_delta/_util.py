...
def sniff_encoding(bytestring, starts=JSON_STARTS, complete=True):
    ...
    test = lambda k, p: k == p if complete else lambda k, p: k.startswith(p)
    candidates = set()
    ...
    if not candidates:
        raise ValueError("String does not begin with " "any of the specified start chars.")
    else:
...
def decode_json(file_or_str):
    ...
    if isinstance(file_or_str, bytes):
        file_or_str = file_or_str.decode(sniff_encoding(file_or_str, JSON_STARTS))
...
def decode_udiff(file_or_str):
    ...
    if isinstance(file_or_str, bytes):
        if not file_or_str:
            ...
            file_or_str = file_or_str.decode(sniff_encoding(file_or_str, UDIFF_STARTS))
    if isinstance(file_or_str, Basestring):
        return file_or_str
...
def uniquify(obj, key=lambda x: x):
    ...
    seen = set()
    seen_add = seen.add
    return [x for x in obj if (key(x) not in seen and not seen_add(key(x)))]
...
def stanzas_addressing(stanzas, keypath):
    ...
    for stanza in stanzas:
        ...
        elif in_array(stanza_keypath) and len(stanza) in (1, 3) and len(stanza_keypath) <= len(keypath):
            kp_mutand_pos = len(stanza_keypath) - 1
            assert keypath[kp_mutand_pos] != stanza_keypath[-1], (stanza, keypath)
...
def in_x_error(key, offender):
    ...
    msg = (
        "keypath elements must be instances of "
        "str, unicode, int or long,\n"
        "    not {} (key[{}] == {})".format(type(offender).__name__, key.index(offender), repr(offender))
    )
...
def in_object(key, accept_None=False):
    ...
    try:
        offender = next((k for k in key[:-1] if (not isinstance(k, Basestring) and type(k) not in IntegerTypes)))
    except StopIteration:
        pass
    else:
        raise in_x_error(key, offender)
    out = bool(key and (isinstance(key[-1], Basestring) or (accept_None and key[-1] is None)))
    ...
    while point < len(jstring):
        if jstring[point] == "{":
            ...
            if key and key[-1] is None:
                ...
                while string_end < len(jstring) and jstring[string_end] in " \r\n\t:":
                    string_end += 1
