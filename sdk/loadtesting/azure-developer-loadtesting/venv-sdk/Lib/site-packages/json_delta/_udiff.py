...
def udiff(left, right, patch=None, indent=0, use_ellipses=True, entry=True):
    ...
    '''
    if patch is None:
        patch = diff(left, right, array_align="udiff", compare_lengths=False, common_key_threshold=0.0, verbose=False)
    elif entry:
    ...
    if not patch and type(left) in TERMINALS:
        ...
        left_lines, right_lines = udiff_dict(left, right, patch, indent, use_ellipses)
    else:
        assert isinstance(left, list), (left, right)
        assert isinstance(right, list), (left, right)
        left_lines, right_lines = udiff_list(left, right, patch, indent, use_ellipses)
...
def _add_differing_matter(left, right, left_lines, right_lines, indent):
    ...
    for matter, seq, sigil in ((left, left_lines, "-"), (right, right_lines, "+")):
...
def curry_functions(local_ns):
    ...
    return (
        partial(_add_common_matter, **acm_subset),
...
def udiff_dict(left, right, stanzas, indent=0, use_ellipses=True):
    ...
    (add_common_matter, add_differing_matter, commafy_last) = curry_functions(vars())
    ...
    if use_ellipses:
        keys_in_diff = uniquify([stanza[0][0] for stanza in stanzas if stanza[0]])
    else:
    ...
    if common_keys:
        ...
        value = next(udiff(left[key], right[key], [], indent + 1, entry=False)[0][0])
    ...
    for i, key in enumerate(keys_in_diff):
        ...
        if sub_diff == [[[]]]:  # Handle deletions from left
            ...
            [left_side, right_side] = udiff(left[key], right[key], sub_diff, indent + 1, use_ellipses, entry=False)
...
def udiff_list(left, right, stanzas, indent=0, use_ellipses=True):
    ...
    def compute_commas(idx):
        ...
        for seq in left_a, right_a:
            out.append(any(((not isinstance(elem, Gap)) for elem in seq[idx:])))
        return tuple(out)
    ...
    while pos < len(left_a):
        try:
            key = next(keys)
            sub_diff = in_one_level(stanzas, predicate_count(left_a[:key], lambda x: not isinstance(x, Gap)))
        ...
        if use_ellipses:
            if pos_diff >= 1:
                ...
                left_ext, right_ext = udiff(left_a[pos], right_a[pos], [], indent + 1, use_ellipses, entry=False)
            ...
            if pos_diff >= 2:
                assert left_a[key - 1] == right_a[key - 1], (left_a, right_a, key - 1)
                commafy_last()
                left_ext, right_ext = udiff(
                    left_a[key - 1], right_a[key - 1], [], indent + 1, use_ellipses, entry=False
                )
                add_differing_matter(left=left_ext, right=right_ext)
        else:
        ...
        if isinstance(right_a[key], Gap):
            ...
            left_ext, right_ext = udiff(left_a[key], right_a[key], sub_diff, indent + 1, use_ellipses, entry=False)
            add_differing_matter(left=left_ext, right=right_ext)
            if left_comma and right_comma:
                commafy_last()
            elif left_comma:
