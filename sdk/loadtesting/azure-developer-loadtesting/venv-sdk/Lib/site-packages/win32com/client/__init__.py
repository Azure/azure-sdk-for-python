...
def CastTo(ob, target, typelib=None):
    ...
    if typelib is not None:  # caller specified target typelib (TypelibSpec). See e.g. selecttlb.EnumTlbs().
        ...
        if not hasattr(mod, target):
            raise ValueError(
                "The interface name '%s' does not appear in the " "specified library %r" % (target, typelib.ver_desc)
            )
        ...
        if target_clsid is None:
            raise ValueError(
                "The interface name '%s' does not appear in the " "same library as object '%r'" % (target, ob)
            )
