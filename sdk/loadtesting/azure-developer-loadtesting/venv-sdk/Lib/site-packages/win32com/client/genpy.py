...
class AliasItem(build.OleItem, WritableItem):
    ...
    def __init__(self, typeinfo, attr, doc=None, bForUser=1):
        ...
        ai = attr[14]
        self.attr = attr
        if type(ai) == type(()) and type(ai[1]) == type(0):  # XXX - This is a hack - why tuples?  Need to resolve?
            href = ai[1]
    ...
    def WriteEnumerationItems(self, stream):
        ...
        for name in names:
            ...
            if vdesc[4] == pythoncom.VAR_CONST:
                ...
                print(
                    "\t%-30s=%-10s # from enum %s" % (build.MakePublicAttributeName(name, True), use, enumName),
                    file=stream,
                )
                num += 1
        return num
...
class DispatchItem(build.DispatchItem, WritableItem):
    ...
    def WriteClassBody(self, generator):
        ...
        for name in names:
            ...
            if entry.desc.wFuncFlags & pythoncom.FUNCFLAG_FRESTRICTED and dispid != pythoncom.DISPID_NEWENUM:
                continue
        ...
        for key in names:
            ...
            if generator.bBuildHidden or not entry.hidden:
                ...
                if resultName:
                    print(
                        "\t\t# Property '%s' is an object of type '%s'" % (key, resultName),
                        file=stream,
                    )
                lkey = key.lower()
                details = entry.desc
        ...
        for key in names:
            ...
            if generator.bBuildHidden or not entry.hidden:
                if entry.GetResultName():
                    print(
                        "\t\t# Method '%s' returns object of type '%s'" % (key, entry.GetResultName()),
                        file=stream,
                    )
                details = entry.desc
        ...
        print("\t\ttry:", file=stream)
        print(
            "\t\t\tob = self._oleobj_.InvokeTypes(%d,LCID,%d,(13, 10),())" % (pythoncom.DISPID_NEWENUM, invkind),
            file=stream,
        )
        ...
        if specialItems["count"]:
            ...
            print(
                "\t#This class has Count() %s - allow len(ob) to provide this" % (typename),
                file=stream,
            )
            for line in ret:
                print(line, file=stream)
...
class Generator:
    ...
    def do_gen_file_header(self):
        ...
        print("# By python version %s" % (sys.version.replace("\n", "-"),), file=self.file)
    ...
    def do_gen_child_item(self, oleitem):
        ...
        if oleitem.bWritten:
            print(
                'win32com.client.CLSIDToClass.RegisterCLSID( "%s", %s )' % (oleitem.clsid, oleitem.python_name),
                file=self.file,
            )
