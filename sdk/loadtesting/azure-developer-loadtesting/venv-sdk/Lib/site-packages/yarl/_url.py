...
class URL:
    ...
    def __new__(cls, val="", *, encoded=False, strict=None):
        ...
        if not encoded:
            if not val[1]:  # netloc
                ...
                netloc = cls._make_netloc(val.username, val.password, host, port, encode=True, requote=True)
        ...
        *,
        ...
        if authority:
            if encoded:
                ...
                netloc = cls._make_netloc(tmp.username, tmp.password, tmp.hostname, tmp.port, encode=True)
        elif not user and not password and not host and not port:
            netloc = ""
        else:
            netloc = cls._make_netloc(user, password, host, port, encode=not encoded, encode_host=not encoded)
        if not encoded:
        ...
        url = cls(SplitResult(scheme, netloc, path, query_string, fragment), encoded=True)
    ...
    def authority(self):
        ...
        """
        return self._make_netloc(self.user, self.password, self.host, self.port, encode_host=False)
    ...
    def _make_child(self, segments, encoded=False):
        ...
        for seg in reversed(segments):
            if not seg:
                continue
            if seg[0] == "/":
                raise ValueError(f"Appending path {seg!r} starting from slash is forbidden")
            seg = seg if encoded else self._PATH_QUOTER(seg)
            if "/" in seg:
                parsed += (sub for sub in reversed(seg.split("/")) if sub and sub != ".")
            elif seg != ".":
                parsed.append(seg)
        parsed.reverse()
    ...
    @classmethod
    def _make_netloc(cls, user, password, host, port, encode=False, encode_host=True, requote=False):
    ...
    def with_user(self, user):
        ...
        return URL(
            self._val._replace(netloc=self._make_netloc(user, password, val.hostname, val.port)),
            encoded=True,
        )
    ...
    def with_password(self, password):
        ...
        val = self._val
        return URL(
            self._val._replace(netloc=self._make_netloc(val.username, password, val.hostname, val.port)),
            encoded=True,
        )
    ...
    def with_host(self, host):
        ...
        val = self._val
        return URL(
            self._val._replace(netloc=self._make_netloc(val.username, val.password, host, val.port)),
            encoded=True,
        )
    ...
    def with_port(self, port):
        ...
        val = self._val
        return URL(
            self._val._replace(netloc=self._make_netloc(val.username, val.password, val.hostname, port)),
            encoded=True,
        )
