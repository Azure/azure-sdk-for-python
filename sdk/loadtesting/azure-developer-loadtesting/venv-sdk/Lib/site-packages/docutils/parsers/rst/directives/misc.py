...
class Include(Directive):
    ...
    def run(self):
        ...
        try:
            include_file = io.FileInput(source_path=path, encoding=encoding, error_handler=error_handler)
            ...
            raise self.severe(f'Problems with "{self.name}" directive ' f"path:\n{io.error_string(error)}.")
        else:
...
class Raw(Directive):
    ...
    def run(self):
        settings = self.state.document.settings
        if not settings.raw_enabled or (
            not settings.file_insertion_enabled and ("file" in self.options or "url" in self.options)
        ):
            raise self.warning('"%s" directive disabled.' % self.name)
        ...
        if self.content:
            ...
            path = adapt_path(self.options["file"], self.state.document.current_source, settings.root_prefix)
            try:
                raw_file = io.FileInput(source_path=path, encoding=encoding, error_handler=error_handler)
            except OSError as error:
                raise self.severe(f'Problems with "{self.name}" directive ' f"path:\n{io.error_string(error)}.")
            else:
            ...
            try:
                ...
                raise self.severe(
                    f'Problems with "{self.name}" directive URL '
                    f'"{self.options["url"]}":\n'
                    f"{io.error_string(error)}."
                )
            raw_file = io.StringInput(
                source=raw_text, source_path=source, encoding=encoding, error_handler=error_handler
            )
            try:
                text = raw_file.read()
            except UnicodeError as error:
    ...
    def run(self):
        ...
        for elem in element:
            if not node and isinstance(elem, nodes.paragraph):
                ...
                return [
                    self.reporter.error(
                        f'Error in "{self.name}" directive: may contain ' "a single paragraph only.", line=self.lineno
                    )
                ]
        if node:
...
class MetaBody(states.SpecializedBody):
    ...
    def parsemeta(self, match):
        ...
        node = nodes.meta()
        node["content"] = nodes.unescape(utils.escape2null(" ".join(indented)))
        if not indented:
