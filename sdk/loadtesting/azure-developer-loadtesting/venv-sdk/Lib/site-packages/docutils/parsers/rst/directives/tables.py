    ...
    def check_table_dimensions(self, rows, header_rows, stub_columns):
        if len(rows) < header_rows:
            error = self.reporter.error(
                "%s header row(s) specified but "
                'only %s row(s) of data supplied ("%s" directive).' % (header_rows, len(rows), self.name),
    ...
    def get_column_widths(self, n_cols):
        if isinstance(self.widths, list):
            if len(self.widths) != n_cols:
                ...
                error = self.reporter.error(
                    '"%s" widths do not match the ' "number of columns in table (%s)." % (self.name, n_cols),
                    nodes.literal_block(self.block_text, self.block_text),
                    line=self.lineno,
                )
                raise SystemMessagePropagation(error)
    ...
    def run(self):
        if not self.content:
            warning = self.reporter.warning(
                "Content block expected " 'for the "%s" directive; none found.' % self.name,
                nodes.literal_block(self.block_text, self.block_text),
                line=self.lineno,
            )
            return [warning]
        ...
        if len(node) != 1 or not isinstance(node[0], nodes.table):
            error = self.reporter.error(
                "Error parsing content block for the " '"%s" directive: exactly one table expected.' % self.name,
                nodes.literal_block(self.block_text, self.block_text),
                line=self.lineno,
            )
            return [error]
...
class CSVTable(Table):
    ...
    class HeaderDialect(csv.Dialect):
        ...
        def __init__(self):
            warnings.warn(
                "CSVTable.HeaderDialect will be removed " "in Docutils 0.22.", PendingDeprecationWarning, stacklevel=2
            )
            super().__init__()
    ...
    def check_requirements():
        warnings.warn(
            "CSVTable.check_requirements()" " is not required with Python 3" " and will be removed in Docutils 0.22.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def process_header_option(self):
        ...
        if "header" in self.options:  # separate table header in option
            rows, max_header_cols = self.parse_csv_data_into_rows(
                self.options["header"].split("\n"), self.DocutilsDialect(self.options), source
            )
    ...
    def run(self):
        try:
            if not self.state.document.settings.file_insertion_enabled and (
                "file" in self.options or "url" in self.options
            ):
                warning = self.reporter.warning(
                    "File and URL access " 'deactivated; ignoring "%s" directive.' % self.name,
                    nodes.literal_block(self.block_text, self.block_text),
                    line=self.lineno,
                )
                return [warning]
    ...
    def get_csv_data(self):
        ...
        if self.content:
            ...
            if "file" in self.options or "url" in self.options:
                error = self.reporter.error(
                    '"%s" directive may not both ' "specify an external file and have content." % self.name,
                    nodes.literal_block(self.block_text, self.block_text),
                    line=self.lineno,
                )
                raise SystemMessagePropagation(error)
            ...
            try:
                csv_file = FileInput(source_path=source, encoding=encoding, error_handler=error_handler)
            ...
            try:
                ...
                severe = self.reporter.severe(
                    'Problems with "%s" directive URL "%s":\n%s.' % (self.name, self.options["url"], error),
                    nodes.literal_block(self.block_text, self.block_text),
                    line=self.lineno,
                )
                raise SystemMessagePropagation(severe)
            csv_file = StringInput(source=csv_text, source_path=source, encoding=encoding, error_handler=error_handler)
            csv_data = csv_file.read().splitlines()
        else:
            error = self.reporter.warning(
                'The "%s" directive requires content; none supplied.' % self.name,
                nodes.literal_block(self.block_text, self.block_text),
                line=self.lineno,
            )
            raise SystemMessagePropagation(error)
    ...
    def parse_csv_data_into_rows(self, csv_data, dialect, source):
        ...
        for row in csv_reader:
            row_data = []
            for cell in row:
                cell_data = (0, 0, 0, statemachine.StringList(cell.splitlines(), source=source))
...
class ListTable(Table):
    ...
    def check_list_content(self, node):
        if len(node) != 1 or not isinstance(node[0], nodes.bullet_list):
            error = self.reporter.error(
                'Error parsing content block for the "%s" directive: ' "exactly one bullet list expected." % self.name,
                nodes.literal_block(self.block_text, self.block_text),
                line=self.lineno,
            )
            raise SystemMessagePropagation(error)
            ...
            if len(item) != 1 or not isinstance(item[0], nodes.bullet_list):
                ...
                if len(item[0]) != num_cols:
                    error = self.reporter.error(
                        ...
                        "(%s vs %s)." % (self.name, item_index + 1, len(item[0]), num_cols),
                        nodes.literal_block(self.block_text, self.block_text),
                        line=self.lineno,
                    )
                    raise SystemMessagePropagation(error)
