...
class Reporter:
    ...
    def system_message(self, level, message, *children, **kwargs):
        ...
        if self.stream and (
            level >= self.report_level or self.debug_flag and level == self.DEBUG_LEVEL or level >= self.halt_level
        ):
...
def decode_path(path):
    ...
    try:
        ...
        if not path:
            return ""
        raise ValueError("`path` value must be a String or ``None``, " f"not {path!r}")
...
def get_stylesheet_list(settings):
    ...
    """
    assert not (
        settings.stylesheet and settings.stylesheet_path
    ...
    if settings.stylesheet_path:
        ...
        stylesheets = [find_file_in_dirs(path, settings.stylesheet_dirs) for path in stylesheets]
    return stylesheets
...
release_level_abbreviations = {"alpha": "a", "beta": "b", "candidate": "rc", "final": ""}
