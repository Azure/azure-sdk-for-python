...
under = {
    "underbrace": ("\u23DF", 0.1),  # DejaVu Math -0.7
    "underleftarrow": ("\u2190", -0.2),
...
fractions = {
    # name:   attributes
    "frac": {},
    "cfrac": {"displaystyle": True, "scriptlevel": 0, "class": "cfrac"},  # in LaTeX with padding
...
def tex_token(string):
    ...
    """
    m = re.match(
        r"""((?P<cmd>\\[a-zA-Z]+)\s* # TeX command, skip whitespace
    ...
    while len(string) > 0:
        ...
        if c == "\\":  # start of a LaTeX macro
            ...
            if c == "-" and len(node):
                ...
                if previous_node.text and previous_node.text in "([=" or previous_node.get("class") == "mathopen":
        ...
        if node is None:
            ...
            raise MathError(f'No insertion point for "{string}". ' f'Unbalanced braces in "{source[:-len(string)]}"?')
...
def handle_cmd(name, node, string):  # noqa: C901 TODO make this less complex
    ...
    if name in bigdelimiters:
        ...
        if delimiter not in "()[]/|.":
            try:
                ...
                raise MathError(f'Unsupported "\\{name}" delimiter ' f'"{delimiter}"!')
        if size:
    ...
    if name in over:
        ...
        new_node = mover(mo(over[name][0], accent=True), switch=True, accent=False)
        node.append(new_node)
        return new_node, string
    ...
    if name in layout_styles:  # 'displaystyle', 'textstyle', ...
        if len(node) > 0:
            raise MathError(rf'Declaration "\{name}" must be first command ' "in a group!")
...
def handle_script_or_limit(node, c, limits=""):
    ...
    elif limits == "movablelimits" or getattr(child, "text", "") in movablelimits:
    ...
    if c == "_":
        ...
        elif limits in ("limits", "movablelimits") or limits == "" and child.get("movablelimits", None):
...
def align_attributes(rows):
    atts = {"class": "ams-align", "displaystyle": True}
...
def tex2mathml(tex_math, as_block=False):
    ...
    if as_block:
        ...
        if len(rows) > 1:
            ...
            node = mtd()
            math_tree.append(mtable(mtr(node), CLASS="ams-align", displaystyle=True))
