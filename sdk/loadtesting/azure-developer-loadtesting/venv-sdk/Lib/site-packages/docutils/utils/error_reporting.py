...
warnings.warn(
    "The `docutils.utils.error_reporting` module is deprecated "
    "and will be removed in Docutils 0.21 or later.\n"
    ...
    def __unicode__(self):
        ...
        * else decode with `self.encoding` and `self.decoding_errors`.
        ...
        try:
            ...
            if isinstance(self.data, Exception):
                args = [
                    unicode(SafeString(arg, self.encoding, decoding_errors=self.decoding_errors))
...
class ErrorOutput:
    ...
    def write(self, data):
        ...
        if isinstance(data, Exception):
            data = unicode(SafeString(data, self.encoding, self.encoding_errors, self.decoding_errors))
        try:
