...
from docutils import __version__, __version_details__, SettingsSpec, io, utils, readers, writers
...
class Publisher:
    ...
    def setup_option_parser(self, usage=None, description=None, settings_spec=None, config_section=None, **defaults):
        warnings.warn(
            "Publisher.setup_option_parser is deprecated, " "and will be removed in Docutils 0.21.",
            DeprecationWarning,
            stacklevel=2,
        )
        if config_section:
    ...
    def get_settings(self, usage=None, description=None, settings_spec=None, config_section=None, **defaults):
        ...
        """
        option_parser = self._setup_settings_parser(usage, description, settings_spec, config_section, **defaults)
    ...
    def set_destination(self, destination=None, destination_path=None):
        if destination_path is None:
            if (
                ...
                raise SystemExit(
                    "The positional argument <destination> is "
                    "obsoleted by the --output option.  "
                    "You cannot use them together."
                )
            if self.settings.output == "-":  # means stdout
                self.settings.output = None
            destination_path = self.settings.output or self.settings._destination
...
def publish_cmdline_to_binary(
    ...
    - `description`: Program description, output for the "--help" option
      (along with command-line option descriptions).
    """
    publisher = Publisher(reader, parser, writer, settings=settings, destination_class=destination_class)
    publisher.set_components(reader_name, parser_name, writer_name)
    output = publisher.publish(
        argv,
        usage,
