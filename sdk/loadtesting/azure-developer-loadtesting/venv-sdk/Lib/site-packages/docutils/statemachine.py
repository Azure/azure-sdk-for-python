...
class StateMachine:
            ...
            while True:
                try:
                    try:
                        ...
                        if self.debug:
                            print("\nStateMachine.run: %s.eof transition" % state.__class__.__name__, file=sys.stderr)
        ...
        if next_state:
            if self.debug and next_state != self.current_state:
                print(
                    ...
                    '"%s" to "%s" (input line %s).' % (self.current_state, next_state, self.abs_line_number()),
                    file=sys.stderr,
                )
    ...
    def insert_input(self, input_lines, source):
        self.input_lines.insert(
            self.line_offset + 1, "", source="internal padding after " + source, offset=len(input_lines)
        )
    ...
    def check_line(self, context, state, transitions=None):
        ...
        for name in transitions:
            ...
            if self.debug:
                print('\nStateMachine.check_line: No match in state "%s".' % state.__class__.__name__, file=sys.stderr)
    ...
    def error(self):
        ...
        print("module %s, line %s, function %s" % (module, line, function), file=sys.stderr)
...
class ViewList:
    ...
    def __getitem__(self, i):
        if isinstance(i, slice):
            ...
            return self.__class__(
                self.data[i.start : i.stop], items=self.items[i.start : i.stop], parent=self, parent_offset=i.start or 0
            )
        else:
            return self.data[i]
    ...
    def trim_start(self, n=1):
        ...
        if n > len(self.data):
            raise IndexError(
                "Size of trim too large; can't trim %s items " "from a list of size %s." % (n, len(self.data))
            )
        elif n < 0:
...
def string2lines(astring, tab_width=8, convert_whitespace=False, whitespace=re.compile("[\v\f]")):
    """
