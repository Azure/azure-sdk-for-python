...
class Input(TransformSpec):
    ...
    def decode(self, data):
        ...
        if self.encoding and self.encoding.lower() == "unicode":
            assert isinstance(data, str), 'input encoding is "unicode" ' "but `data` is no `str` instance"
...
class Output(TransformSpec):
    ...
    def encode(self, data):
        ...
        """
        if self.encoding and self.encoding.lower() == "unicode":
            assert isinstance(data, str), 'output encoding is "unicode" ' "but `data` is no `str` instance"
            return data
...
class ErrorOutput:
    ...
    def __init__(self, destination=None, encoding=None, encoding_errors="backslashreplace", decoding_errors="replace"):
        """
        :Parameters:
        ...
        self.encoding = encoding or getattr(destination, "encoding", None) or _locale_encoding or "ascii"
...
class FileInput(Input):
    ...
    def __init__(self, source=None, source_path=None, encoding=None, error_handler="strict", autoclose=True, mode="r"):
        """
        :Parameters:
        ...
        if source is None:
            if source_path:
                try:
                    self.source = open(source_path, mode, encoding=self.encoding, errors=self.error_handler)
...
class FileOutput(Output):
    ...
    def __init__(
        ...
        if handle_io_errors is not None:
            warnings.warn(
                'io.FileOutput: init argument "handle_io_errors" ' "is ignored and will be removed in " "Docutils 2.0.",
                DeprecationWarning,
                stacklevel=2,
            )
        if mode is not None:
    ...
    def write(self, data):
        ...
        try:
            ...
            raise UnicodeError(
                "Unable to encode output data. output-encoding is: " f"{self.encoding}.\n({error_string(err)})"
            )
        finally:
            if self.autoclose:
                self.close()
