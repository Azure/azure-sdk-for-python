...
class DocInfo(Transform):
    ...
    def check_compound_biblio_field(self, field, name):
        ...
        if isinstance(f_body[0], nodes.enumerated_list) and "\n" not in f_body.rawsource.strip():
            # parse into a dummy document and use created nodes
            _document = utils.new_document("*DocInfo transform*", field.document.settings)
            parser = parsers.rst.Parser()
            parser.parse("\\" + f_body.rawsource, _document)
            if len(_document.children) == 1 and isinstance(_document.children[0], nodes.paragraph):
        ...
        f_body += self.document.reporter.warning(
            f'Bibliographic field "{name}"\nmust contain ' f"a single <paragraph>, not {content}.", base_node=field
        )
        return False
