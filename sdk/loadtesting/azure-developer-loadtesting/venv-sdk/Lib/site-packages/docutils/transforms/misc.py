...
class ClassAttribute(Transform):
    ...
    def apply(self):
        ...
        error = self.document.reporter.error(
            'No suitable element following "%s" directive' % pending.details["directive"],
...
class Transitions(Transform):
    ...
    def visit_transition(self, node):
        ...
        if (
            ...
            and (index == 1 or isinstance(node.parent[1], nodes.subtitle) and index == 2)
        ):
            assert isinstance(node.parent, nodes.document) or isinstance(node.parent, nodes.section)
            error = self.document.reporter.error(
                "Document or section may not begin with a transition.", source=node.source, line=node.line
            )
