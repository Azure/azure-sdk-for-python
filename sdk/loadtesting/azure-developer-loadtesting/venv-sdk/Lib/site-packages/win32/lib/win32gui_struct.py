...
def UnpackMENUITEMINFO(s):
    ...
    if fMask & win32con.MIIM_STRING:
        ...
        text = None
    return _MakeResult(
        "MENUITEMINFO fType fState wID hSubMenu hbmpChecked " "hbmpUnchecked dwItemData text hbmpItem",
        (
            fType,
            fState,
            wID,
...
def EmptyTVITEM(hitem, mask=None, text_buf_size=512):
    ...
    if mask & commctrl.TVIF_TEXT:
        ...
        text_addr = text_buf_size = 0
    buf = struct.pack(_tvitem_fmt, mask, hitem, 0, 0, text_addr, text_buf_size, 0, 0, 0, 0)  # text
    return array.array("b", buf), extra
...
def UnpackTVITEM(buffer):
    ...
    if item_textptr:
        ...
        text = None
    return _MakeResult(
        "TVITEM item_hItem item_state item_stateMask " "text item_image item_selimage item_cChildren item_param",
        (
            item_hItem,
            item_state,
            item_stateMask,
...
def UnpackLVITEM(buffer):
    ...
    if item_textptr:
        ...
        text = None
    return _MakeResult(
        "LVITEM item_item item_subItem item_state " "item_stateMask text item_image item_param item_indent",
        (
            item_item,
            item_subItem,
            item_state,
...
def EmptyLVITEM(item, subitem, mask=None, text_buf_size=512):
    ...
    if mask is None:
        mask = (
            commctrl.LVIF_IMAGE | commctrl.LVIF_INDENT | commctrl.LVIF_TEXT | commctrl.LVIF_PARAM | commctrl.LVIF_STATE
        )
...
def PackLVCOLUMN(fmt=None, cx=None, text=None, subItem=None, image=None, order=None):
    ...
    if text is None:
        ...
        text_addr, _ = text_buffer.buffer_info()
        text_len = len(text)
    buf = struct.pack(_lvcolumn_fmt, mask, fmt, cx, text_addr, text_len, subItem, image, order)  # text
    return array.array("b", buf), extra
...
def EmptyLVCOLUMN(mask=None, text_buf_size=512):
    ...
    if mask & commctrl.LVCF_TEXT:
        ...
        text_addr = text_buf_size = 0
    buf = struct.pack(_lvcolumn_fmt, mask, 0, 0, text_addr, text_buf_size, 0, 0, 0)  # text
    return array.array("b", buf), extra
