...
def _read_account_by_id(account_id, correlation_id):
    ...
    pymsalruntime.read_account_by_id(
        account_id, correlation_id, lambda result, callback_data=callback_data: callback_data.complete(result)
    )
...
def _convert_result(result, client_id, expected_token_type=None):  # Mimic an on-the-wire response from AAD
    ...
    return_value = {
        ...
        for k, v in {
            "access_token": (
                result.get_authorization_header().split()[1]  # It returns "pop SignedHttpRequest"
            ...
            "token_type": (
                "pop" if result.is_pop_authorization() else (expected_token_type or "bearer")
    ...
    if return_value["token_type"].lower() == "ssh-cert" and not likely_a_cert:
        raise TokenTypeError("Broker could not get an SSH Cert: {}...".format(return_value["access_token"][:8]))
...
def _signin_silently(
    ...
    if auth_scheme:
        params.set_pop_params(
            auth_scheme._http_method, auth_scheme._url.netloc, auth_scheme._url.path, auth_scheme._nonce
        )
    callback_data = _CallbackData()
    ...
    callback_data.signal.wait()
    return _convert_result(callback_data.result, client_id, expected_token_type=kwargs.get("token_type"))
...
def _acquire_token_silently(
    authority, client_id, account_id, scopes, claims=None, correlation_id=None, auth_scheme=None, **kwargs
):
    ...
    pymsalruntime.acquire_token_silently(
        params, correlation_id, account, lambda result, callback_data=callback_data: callback_data.complete(result)
    )
    callback_data.signal.wait()
    return _convert_result(callback_data.result, client_id, expected_token_type=kwargs.get("token_type"))
...
def _signout_silently(client_id, account_id, correlation_id=None):
    ...
    pymsalruntime.signout_silently(  # New in PyMsalRuntime 0.7
        client_id, correlation_id, account, lambda result, callback_data=callback_data: callback_data.complete(result)
    )
    callback_data.signal.wait()
    error = callback_data.result.get_error()
    if error:
