...
def _get_username(id_token_claims):
    return id_token_claims.get("preferred_username", id_token_claims.get("upn"))  # AAD  # ADFS 2019
...
class TokenCache(object):
    ...
    def __init__(self):
        ...
        self.key_makers = {
            ...
            self.CredentialType.ACCESS_TOKEN: lambda home_account_id=None, environment=None, client_id=None, realm=None, target=None, **ignored_payload_from_a_real_token: "-".join(
                [
                    home_account_id or "",
                    environment or "",
    ...
    def _find(self, credential_type, target=None, query=None):  # O(n) generator
        ...
        if (
            ...
            preferred_result = self._get_access_token(
                query["home_account_id"], query["environment"], query["client_id"], query["realm"], target
            )
            if preferred_result:
        ...
        with self._lock:
            ...
            for entry in self._cache.get(credential_type, {}).values():
                if is_subdict_of(query or {}, entry) and (
                    target_set <= set(entry.get("target", "").split()) if target else True
                ):
    ...
    def __add(self, event, now=None):
        ...
        with self._lock:
            ...
            if access_token:
                ...
                expires_in = int(response.get("expires_in", default_expires_in))  # AADv1-like endpoint returns a string
                ext_expires_in = int(response.get("ext_expires_in", expires_in))  # AADv1-like endpoint returns a string
                at = {
                    "credential_type": self.CredentialType.ACCESS_TOKEN,
                    "secret": access_token,
                    "home_account_id": home_account_id,
            ...
            if client_info and not event.get("skip_account_creation"):
                account = {
                    ...
                    "authority_type": event.get(
                        ...
                        self.AuthorityType.ADFS if realm == "adfs" else self.AuthorityType.MSSTS,
                    ),
    ...
    def modify(self, credential_type, old_entry, new_key_value_pairs=None):
        ...
        with self._lock:
            if new_key_value_pairs:  # Update with them
                ...
                entries[key] = dict(old_entry, **new_key_value_pairs)  # Do not use entries[key] b/c it might not exist
...
class SerializableTokenCache(TokenCache):
    ...
    def modify(self, credential_type, old_entry, new_key_value_pairs=None):
        super(SerializableTokenCache, self).modify(credential_type, old_entry, new_key_value_pairs)
        self.has_state_changed = True
