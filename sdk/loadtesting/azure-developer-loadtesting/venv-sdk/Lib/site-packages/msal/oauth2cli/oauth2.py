    ...
    def __init__(
        ...
        if http_client:
            if verify is not None or proxies is not None or timeout is not None:
                raise ValueError("verify, proxies, or timeout is not allowed " "when http_client is in use")
    ...
    def _obtain_token(  # The verb "obtain" is influenced by OAUTH2 RFC 6749
        ...
        **kwargs  # Relay all extra parameters to underlying requests
        ...
        if self.default_body.get("client_assertion_type") and self.client_assertion:
            ...
            encoder = self.client_assertion_encoders.get(self.default_body["client_assertion_type"], lambda a: a)
        ...
        resp = (post or self._http_client.post)(
            self.configuration["token_endpoint"], headers=_headers, params=params, data=_data, **kwargs
        )
...
def _generate_pkce_code_verifier(length=43):
    ...
    code_challenge = (
        ...
        base64.urlsafe_b64encode(hashlib.sha256(verifier.encode("ascii")).digest()).rstrip(b"=")
...
class Client(BaseClient):  # We choose to implement all 4 grants in 1 class
    ...
    def _build_auth_request_uri(self, response_type, redirect_uri=None, scope=None, state=None, **kwargs):
    ...
    def build_auth_request_uri(self, response_type, redirect_uri=None, scope=None, state=None, **kwargs):
    ...
    def obtain_token_by_auth_code_flow(self, auth_code_flow, auth_response, scope=None, **kwargs):
        ...
        if auth_code_flow.get("state") != auth_response.get("state"):
            raise ValueError("state mismatch: {} vs {}".format(auth_code_flow.get("state"), auth_response.get("state")))
        if scope and set(scope) - set(auth_code_flow.get("scope", [])):
            raise ValueError("scope must be None or a subset of %s" % auth_code_flow.get("scope"))
        ...
        **kwargs
        ...
        try:
            with _AuthCodeReceiver(port=listen_port) as receiver:
                ...
                return self._obtain_token_by_browser(receiver, redirect_uri=uri, **kwargs)
        except PermissionError:
            raise ValueError("Can't listen on port %s. You may try port 0." % listen_port)
    ...
    def _obtain_token_by_browser(
        ...
        **kwargs
        ...
        return self.obtain_token_by_auth_code_flow(flow, auth_response, scope=scope, **kwargs)
    ...
    def obtain_token_by_authorization_code(self, code, redirect_uri=None, scope=None, **kwargs):
        ...
        """
        warnings.warn("Use obtain_token_by_auth_code_flow() instead", DeprecationWarning)
        return self._obtain_token_by_authorization_code(code, redirect_uri=redirect_uri, scope=scope, **kwargs)

    def _obtain_token_by_authorization_code(self, code, redirect_uri=None, scope=None, **kwargs):
        data = kwargs.pop("data", {})
    ...
    def obtain_token_by_username_password(self, username, password, scope=None, **kwargs):
    ...
    def _obtain_token(
        self, grant_type, params=None, data=None, also_save_rt=False, on_obtaining_tokens=None, *args, **kwargs
    ):
    ...
    def obtain_token_by_refresh_token(
        ...
        **kwargs
        ...
        resp = super(Client, self).obtain_token_by_refresh_token(
            rt_getter(token_item) if not isinstance(token_item, string_types) else token_item,
            scope=scope,
