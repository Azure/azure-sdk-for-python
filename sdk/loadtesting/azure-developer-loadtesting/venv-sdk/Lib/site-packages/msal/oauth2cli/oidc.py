...
def decode_id_token(id_token, client_id=None, issuer=None, nonce=None, now=None):
    ...
    if client_id:
        valid_aud = client_id in decoded["aud"] if isinstance(decoded["aud"], list) else client_id == decoded["aud"]
        if not valid_aud:
    ...
    if nonce and nonce != decoded.get("nonce"):
        raise IdTokenNonceError(
            "11. Nonce must be the same value " "as the one that was sent in the Authentication Request.", _now, decoded
        )
...
class Client(oauth2.Client):
    ...
    def decode_id_token(self, id_token, nonce=None):
        ...
        return decode_id_token(id_token, nonce=nonce, client_id=self.client_id, issuer=self.configuration.get("issuer"))
    ...
    def build_auth_request_uri(self, response_type, nonce=None, **kwargs):
        ...
        return super(Client, self).build_auth_request_uri(response_type, nonce=nonce, **kwargs)
    ...
    def obtain_token_by_authorization_code(self, code, nonce=None, **kwargs):
        ...
        """
        warnings.warn("Use obtain_token_by_auth_code_flow() instead", DeprecationWarning)
        ...
        if "id_token_claims" in result and nonce and nonce != nonce_in_id_token:
            raise ValueError('The nonce in id token ("%s") should match your nonce ("%s")' % (nonce_in_id_token, nonce))
        return result
    ...
    def initiate_auth_code_flow(self, scope=None, **kwargs):
        ...
        flow = super(Client, self).initiate_auth_code_flow(scope=_scope, nonce=_nonce_hash(nonce), **kwargs)
        flow["nonce"] = nonce
    ...
    def obtain_token_by_auth_code_flow(self, auth_code_flow, auth_response, **kwargs):
        ...
        for descriptions on other parameters and return value.
        """
        result = super(Client, self).obtain_token_by_auth_code_flow(auth_code_flow, auth_response, **kwargs)
        if "id_token_claims" in result:
            nonce_in_id_token = result.get("id_token_claims", {}).get("nonce")
            expected_hash = _nonce_hash(auth_code_flow["nonce"])
            if nonce_in_id_token != expected_hash:
                raise RuntimeError(
                    'The nonce in id token ("%s") should match our nonce ("%s")' % (nonce_in_id_token, expected_hash)
                )

            if auth_code_flow.get("max_age") is not None:
                auth_time = result.get("id_token_claims", {}).get("auth_time")
                if not auth_time:
                    raise RuntimeError("13. max_age was requested, ID token should contain auth_time")
                now = int(time.time())
