...
def _input_boolean(message):
    return input("{} (N/n/F/f or empty means False, otherwise it is True): ".format(message)) not in (
        "N",
        "n",
        "F",
        "f",
...
try:
    ...
    logging.warning("python-dotenv is not installed. " "You may need to set environment variables manually.")
...
def _acquire_token_interactive(app, scopes=None, data=None):
    ...
    result = app.acquire_token_interactive(
        ...
        data=data or {},
        auth_scheme=(
            placeholder_auth_scheme if app.is_pop_supported() and _input_boolean("Acquire AT POP via Broker?") else None
        ),
    )
...
def _main():
    ...
    cache_choice = _select_options(
        [
            ...
            },
            {
                ...
                "Useful for testing acquire_token_silent() repeatedly".format(_token_cache_filename),
            },
        ],
    ...
    while True:
        func = _select_options(
            methods_to_be_tested + [_exit], option_renderer=lambda f: f.__doc__, header="MSAL Python APIs:"
        )
        try:
            func(app)
