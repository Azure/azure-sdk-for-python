...
def extract_certs(public_cert_content):
    ...
    public_certificates = re.findall(
        r"-----BEGIN CERTIFICATE-----(?P<cert_value>[^-]+)-----END CERTIFICATE-----", public_cert_content, re.I
    )
...
def _preferred_browser():
    ...
    if (user_has_no_preference or user_wont_mind_edge) and os.path.exists(browser_path):
        try:
            ...
            try:
                registration_available = isinstance(webbrowser.get(browser_name), webbrowser.BackgroundBrowser)
...
class _ClientWithCcsRoutingInfo(Client):
    ...
    def obtain_token_by_auth_code_flow(self, auth_code_flow, auth_response, **kwargs):
        ...
        client_info = json.loads(decode_part(auth_response["client_info"])) if auth_response.get("client_info") else {}
...
class ClientApplication(object):
    ...
    def __init__(
        ...
        if exclude_scopes and not isinstance(exclude_scopes, list):
            raise ValueError("Invalid exclude_scopes={}. It need to be a list of strings.".format(repr(exclude_scopes)))
        self._exclude_scopes = frozenset(exclude_scopes or [])
        if "openid" in self._exclude_scopes:
            raise ValueError(
                'Invalid exclude_scopes={}. You can not opt out "openid" scope'.format(repr(exclude_scopes))
            )
        ...
        if http_client:
            ...
            self.http_client.request = functools.partial(self.http_client.request, timeout=timeout)
        ...
        self.client, self._regional_client = self._build_client(client_credential, self.authority)
        self.authority_groups = None
        self._telemetry_buffer = {}
        self._telemetry_lock = Lock()

    def _decide_broker(self, allow_broker, enable_pii_log):
        is_confidential_app = self.client_credential or isinstance(self, ConfidentialClientApplication)
        ...
        if allow_broker:
            warnings.warn(
                "allow_broker is deprecated. " "Please use PublicClientApplication(..., enable_broker_on_windows=True)",
                DeprecationWarning,
            )
        ...
        if (
            ...
            try:
                ...
                logger.exception("Broker is unavailable on this platform. " "We will fallback to non-broker.")
    ...
    def _decorate_scope(self, scopes, reserved_scope=frozenset(["openid", "profile", "offline_access"])):
        ...
        if scope_set & reserved_scope:
            ...
            raise ValueError("API does not accept {} value as user-provided scopes".format(reserved_scope))
    ...
    def _build_telemetry_context(self, api_id, correlation_id=None, refresh_reason=None):
    ...
    def _build_client(self, client_credential, authority, skip_regional_client=False):
        ...
        if isinstance(client_credential, dict):
            ...
            if client_credential.get("client_assertion"):
                ...
                passphrase_bytes = (
                    _str2bytes(client_credential["passphrase"]) if client_credential.get("passphrase") else None
                )
                ...
                elif client_credential.get("private_key") and client_credential.get("thumbprint"):  # PEM blob
                ...
                assertion = JwtAssertionCreator(
                    private_key, algorithm="RS256", sha1_thumbprint=sha1_thumbprint, headers=headers
                )
                client_assertion = assertion.create_regenerative_assertion(
                    audience=authority.token_endpoint, issuer=self.client_id, additional_claims=self.client_claims or {}
                )
        else:
        ...
        central_configuration = {
            ...
            or urljoin(authority.token_endpoint, "devicecode"),
        }
        central_client = _ClientWithCcsRoutingInfo(
            ...
            on_obtaining_tokens=lambda event: self.token_cache.add(dict(event, environment=authority.instance)),
            on_removing_rt=self.token_cache.remove_rt,
            on_updating_rt=self.token_cache.update_rt,
        )

        regional_client = None
        if client_credential and not skip_regional_client:  # Currently regional endpoint only serves some CCA flows
            ...
            if regional_authority:
                regional_configuration = {
                    ...
                    or urljoin(regional_authority.token_endpoint, "devicecode"),
                }
    ...
    def initiate_auth_code_flow(
        ...
        flow = client.initiate_auth_code_flow(
            ...
            claims=_merge_claims_challenge_and_capabilities(self._client_capabilities, claims_challenge),
            max_age=max_age,
    ...
    def get_authorization_request_url(
        ...
        **kwargs
        ...
        if authority:
            warnings.warn("We haven't decided if this method will accept authority parameter")
        ...
        warnings.warn("Change your get_authorization_request_url() " "to initiate_auth_code_flow()", DeprecationWarning)
            ...
            self.client.obtain_token_by_auth_code_flow(
                ...
                data=dict(
                    ...
                    claims=_merge_claims_challenge_and_capabilities(
                        self._client_capabilities, auth_code_flow.pop("claims_challenge", None)
                    ),
                ),
                **kwargs
            )
    ...
    def acquire_token_by_authorization_code(
        ...
        **kwargs
        ...
        self._validate_ssh_cert_input_data(kwargs.get("data", {}))
        warnings.warn(
            "Change your acquire_token_by_authorization_code() " "to acquire_token_by_auth_code_flow()",
            DeprecationWarning,
        )
        with warnings.catch_warnings(record=True):
            telemetry_context = self._build_telemetry_context(self.ACQUIRE_TOKEN_BY_AUTHORIZATION_CODE_ID)
    ...
    def get_accounts(self, username=None):
        ...
        if username:
            ...
            accounts = [a for a in accounts if a["username"].lower() == lowercase_username]
            if not accounts:
                logger.debug(
    ...
    def _find_msal_accounts(self, environment):
        interested_authority_types = [TokenCache.AuthorityType.ADFS, TokenCache.AuthorityType.MSSTS]
        ...
        grouped_accounts = {
            ...
            for a in self.token_cache.find(TokenCache.CredentialType.ACCOUNT, query={"environment": environment})
    ...
    def _get_authority_aliases(self, instance):
        ...
        if not self.authority_groups:
            self.authority_groups = [set(group["aliases"]) for group in self._get_instance_metadata()]
    ...
    def _sign_out(self, home_account):
        ...
        for rt in [
            rt
            for rt in self.token_cache.find(TokenCache.CredentialType.REFRESH_TOKEN, query=owned_by_home_account)
    ...
    def _acquire_token_by_cloud_shell(self, scopes, data=None):
        ...
        response = _obtain_token(self.http_client, scopes, client_id=self.client_id, data=data)
        ...
        try:
            ...
            if (
                ...
                if response:  # Broker provides a decisive outcome
                    account_was_established_by_broker = account.get("account_source") == _GRANT_TYPE_BROKER
    ...
    def _acquire_token_silent_by_finding_rt_belongs_to_me_or_my_family(self, authority, scopes, account, **kwargs):
        query = {
            "environment": authority.instance,
            "home_account_id": (account or {}).get("home_account_id"),
        ...
        if app_metadata.get("family_id"):  # Meaning this app belongs to this family
            last_resp = at = self._acquire_token_silent_by_finding_specific_refresh_token(
                authority, scopes, dict(query, family_id=app_metadata["family_id"]), **kwargs
            )
            if at and "error" not in at:
                return at
    ...
    def _get_app_metadata(self, environment):
        return self.token_cache._get_app_metadata(environment=environment, client_id=self.client_id, default={})
    ...
    def _acquire_token_silent_by_finding_specific_refresh_token(
        ...
        **kwargs
        ...
        logger.debug(
            ...
            len(matches),
            {k: _pii_less_home_account_id(v) if k == "home_account_id" and v else v for k, v in query.items()},
        )
        ...
        telemetry_context = self._build_telemetry_context(
            self.ACQUIRE_TOKEN_SILENT_ID, correlation_id=correlation_id, refresh_reason=refresh_reason
        )
    ...
    def acquire_token_by_refresh_token(self, refresh_token, scopes, **kwargs):
        ...
        self._validate_ssh_cert_input_data(kwargs.get("data", {}))
        telemetry_context = self._build_telemetry_context(
            self.ACQUIRE_TOKEN_BY_REFRESH_TOKEN, refresh_reason=msal.telemetry.FORCE_REFRESH
        )
        response = _clean_up(
        ...
        **kwargs
        ...
        if False:  # Disabled, for now. It was if self._enable_broker:
            ...
            response = _signin_silently(
                ...
                validateAuthority=(
                    "no" if (self.authority._is_known_to_developer or self._instance_discovery is False) else None
                ),
                claims=claims,
                auth_scheme=auth_scheme,
            )
        ...
        telemetry_context = self._build_telemetry_context(self.ACQUIRE_TOKEN_BY_USERNAME_PASSWORD_ID)
        headers = telemetry_context.generate_headers()
            ...
            if user_realm_result.get("account_type") == "Federated":
                response = _clean_up(
                    self._acquire_token_by_username_password_federated(
                        user_realm_result, username, password, scopes=scopes, data=data, headers=headers, **kwargs
                    )
                )
        if response is None:  # Either ADFS or not federated
            response = _clean_up(
                self.client.obtain_token_by_username_password(
                    username, password, scope=scopes, headers=headers, data=data, **kwargs
                )
            )
        if "access_token" in response:
            response[self._TOKEN_SOURCE] = self._TOKEN_SOURCE_IDP
    ...
    def _acquire_token_by_username_password_federated(
        ...
        if user_realm_result.get("federation_metadata_url"):
            wstrust_endpoint = mex_send_request(user_realm_result["federation_metadata_url"], self.http_client)
        ...
        if not grant_type:
            raise RuntimeError("RSTR returned unknown token type: %s", wstrust_result.get("type"))
...
class PublicClientApplication(ClientApplication):  # browser app or mobile app
    ...
    def __init__(self, client_id, client_credential=None, **kwargs):
        ...
        super(PublicClientApplication, self).__init__(client_id, client_credential=None, **kwargs)
        ...
        **kwargs
        ...
        if self._enable_broker:
            ...
            if extra_scopes_to_consent:
                logger.warning("Ignoring parameter extra_scopes_to_consent, " "which is not supported by broker")
        ...
        telemetry_context = self._build_telemetry_context(self.ACQUIRE_TOKEN_INTERACTIVE)
        response = _clean_up(
    ...
    def _acquire_token_interactive_via_broker(
        ...
        **kwargs
        ...
        authority = "https://{}/{}".format(self.authority.instance, self.authority.tenant)
        validate_authority = (
            "no" if (self.authority._is_known_to_developer or self._instance_discovery is False) else None
        )
        ...
        if prompt == "none" or not prompt:  # Must/Can attempt _signin_silently()
            ...
            wrong_account_error_message = 'prompt="none" will not work for login_hint="non-default-user"'
            if is_wrong_account:
            ...
            if prompt == "none":
                ...
                if is_wrong_account or "error" in response and response.get("_broker_status") in recoverable_errors:
        ...
        return _signin_interactively(
            ...
            self.client_id,
            scopes,
            None if parent_window_handle is self.CONSOLE_WINDOW_HANDLE else parent_window_handle,
            validateAuthority=validate_authority,
            login_hint=login_hint,
            prompt=prompt,
            claims=claims,
    ...
    def acquire_token_by_device_flow(self, flow, claims_challenge=None, **kwargs):
        ...
        """
        telemetry_context = self._build_telemetry_context(
            self.ACQUIRE_TOKEN_BY_DEVICE_FLOW_ID, correlation_id=flow.get(self.DEVICE_FLOW_CORRELATION_ID)
        )
        response = _clean_up(
...
class ConfidentialClientApplication(ClientApplication):  # server-side web app
    ...
    def acquire_token_for_client(self, scopes, claims_challenge=None, **kwargs):
        ...
        return _clean_up(
            self._acquire_token_silent_with_error(scopes, None, claims_challenge=claims_challenge, **kwargs)
        )

    def _acquire_token_for_client(self, scopes, refresh_reason, claims_challenge=None, **kwargs):
    ...
    def remove_tokens_for_client(self):
        ...
        for env in [self.authority.instance] + self._get_authority_aliases(self.authority.instance):
            for at in self.token_cache.find(
                TokenCache.CredentialType.ACCESS_TOKEN,
                query={
                    "client_id": self.client_id,
                    "environment": env,
    ...
    def acquire_token_on_behalf_of(self, user_assertion, scopes, claims_challenge=None, **kwargs):
        ...
        :return: A dict representing the json response from Microsoft Entra:
            ...
            - an error response would contain "error" and usually "error_description".
        """
        telemetry_context = self._build_telemetry_context(self.ACQUIRE_TOKEN_ON_BEHALF_OF_ID)
