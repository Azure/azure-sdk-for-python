...
class Authority(object):
    ...
    def __init__(
        ...
        if oidc_authority_url:
            ...
            tenant_discovery_endpoint = self._initialize_oidc_authority(oidc_authority_url)
        else:
        ...
        try:
            openid_config = tenant_discovery(tenant_discovery_endpoint, self._http_client)
        except ValueError:
            error_message = (
                ...
                "or https://tenant_name.b2clogin.com/tenant.onmicrosoft.com/policy. ".format(authority_url)
            ) + " Also please double check your tenant name or GUID is correct."
            raise ValueError(error_message)
        logger.debug('openid_config("%s") = %s', tenant_discovery_endpoint, openid_config)
    ...
    def _initialize_oidc_authority(self, oidc_authority_url):
        ...
        self._is_known_to_developer = (
            True  # Not really relevant, but application.py uses this to bypass authority validation
        )
        return oidc_authority_url + "/.well-known/openid-configuration"

    def _initialize_entra_authority(self, authority_url, validate_authority, instance_discovery):
        ...
        if instance_discovery_endpoint and not (is_known_to_microsoft or self._is_known_to_developer):
            payload = _instance_discovery(
                "https://{}{}/oauth2/v2.0/authorize".format(self.instance, authority.path),
                self._http_client,
                instance_discovery_endpoint,
            )
            ...
            tenant_discovery_endpoint = authority._replace(
                path="{prefix}{version}/.well-known/openid-configuration".format(
                    prefix=(
                        tenant if is_ciam and len(authority.path) <= 1 else authority.path  # Path-less CIAM
                    ),  # In B2C, it is "/tenant/policy"
...
def canonicalize(authority_or_auth_endpoint):
    ...
    raise ValueError(
        ...
        "or https://{tenant_name}.b2clogin.com/{tenant_name}.onmicrosoft.com/policy" % authority_or_auth_endpoint
    )


def _instance_discovery(url, http_client, instance_discovery_endpoint, **kwargs):
    resp = http_client.get(
        instance_discovery_endpoint, params={"authorization_endpoint": url, "api-version": "1.0"}, **kwargs
    )
    return json.loads(resp.text)
