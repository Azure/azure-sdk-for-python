...
def init_argument_parser(name=None, **kwargs):
    ...
    if name in _parsers:
        raise ValueError(
            ("kwargs besides 'name' can only be passed in the" " first time. '%s' ArgumentParser already exists: %s")
            % (name, _parsers[name])
        )
...
class YAMLConfigFileParser(ConfigFileParser):
    ...
    def _load_yaml(self):
        ...
        try:
            ...
            raise ConfigFileParserException(
                "Could not import yaml. " "It can be installed by running 'pip install PyYAML'"
            )
...
class CompositeConfigParser(ConfigFileParser):
    ...
    def get_syntax_description(self):
        def guess_format_name(classname):
            strip = classname.lower().strip("_").replace("parser", "").replace("config", "").replace("file", "")
...
class ArgumentParser(argparse.ArgumentParser):
    ...
    def __init__(self, *args, **kwargs):
        ...
        ignore_unknown_config_file_keys = kwargs.pop("ignore_unknown_config_file_keys", False)
        config_file_parser_class = kwargs.pop("config_file_parser_class", DefaultConfigFileParser)
        ...
        config_arg_help_message = kwargs.pop("config_arg_help_message", "config file path")
        args_for_writing_out_config_file = kwargs.pop("args_for_writing_out_config_file", [])
        ...
        for stream in reversed(config_streams):
            ...
            for key, value in config_items.items():
                if key in known_config_keys:
                    ...
                    discard_this_key = already_on_command_line(args, action.option_strings, self.prefix_chars)
                else:
                    action = None
    ...
    def convert_item_to_command_line_arg(self, action, key, value):
        ...
        if action is not None and isinstance(action, ACTION_TYPES_THAT_DONT_NEED_A_VALUE):
            ...
            accepts_list_and_has_nargs = (
                ...
                and (isinstance(action, argparse._StoreAction) or isinstance(action, argparse._AppendAction))
                and (action.nargs in ("+", "*") or (isinstance(action.nargs, int) and action.nargs > 1))
            )
            ...
            raise ValueError("Unexpected value type {} for value: {}".format(type(value), value))
            ...
            try:
                ...
                for config_file in config_files:
                    try:
                        ...
                        pass
                self.error("Unable to open config file: %s. Error: %s" % (user_config_file, msg))
    ...
    def format_help(self):
        ...
        if self._add_config_file_help:
            ...
            config_settable_args = [
                ...
                and not (a.dest == "help" or a.is_config_file_arg or a.is_write_out_config_file_arg)
            ]
            ...
            if config_settable_args and (default_config_files or config_path_actions):
                ...
                msg += ("Args that start with '%s' can also be set in " "a config file") % cc
                ...
                if default_config_files or config_arg_string:
                    msg += " (%s)." % " or ".join(
                        tuple(default_config_files) + tuple(filter(None, [config_arg_string]))
                    )
        ...
        if added_env_var_help or added_config_file_help:
            ...
            msg += " In general, command-line values override %s." % (" which override ".join(value_sources))
...
def already_on_command_line(existing_args_list, potential_command_line_args, prefix_chars):
    ...
    return any(potential_arg in arg_names for potential_arg in potential_command_line_args)
