...
USE_LIMITED_API = sys.platform != "win32" or sys.version_info < (3, 0) or sys.version_info >= (3, 5)
...
class Recompiler:
        ...
        if self.ffi._included_ffis:
            ...
            for ffi_to_include in self.ffi._included_ffis:
                ...
                if included_source is None:
                    raise VerificationError(
                        "not implemented yet: ffi.include() of a Python-based " "ffi inside a C-based ffi"
                    )
    ...
    def write_py_source_to_f(self, f):
        ...
        for i in range(num_includes):
            ...
            if included_source is not None:
                raise VerificationError(
                    "not implemented yet: ffi.include() of a C-based " "ffi inside a Python-based ffi"
                )
    ...
    def _convert_funcarg_to_c(self, tp, fromvar, tovar, errcode):
        ...
        #
            ...
            self._prnt(
                "  if (_cffi_to_c((char *)&%s, _cffi_type(%d), %s) < 0)" % (tovar, self._gettypenum(tp), fromvar)
            )
    ...
    def _convert_funcarg_to_c_ptr_or_array(self, tp, fromvar, tovar, errcode):
        ...
        self._prnt(
            "    %s = ((size_t)datasize) <= 640 ? " "(%s)alloca((size_t)datasize) : NULL;" % (tovar, tp.get_c_name(""))
        )
        self._prnt(
    ...
    def _generate_cpy_function_ctx(self, tp, name):
        ...
        self._lsts["global"].append(
            GlobalExpr(name, "_cffi_f_%s" % name, CffiOp(meth_kind, type_index), size="_cffi_d_%s" % name)
        )
        ...
        for fname, ftype, fbitsize, fqual in self._enum_fields(tp):
            try:
                if ftype.is_integer_type() or fbitsize >= 0:
                    ...
                    if fname != "":
                        prnt(
                            "  (void)((p->%s) | 0);  /* check that '%s.%s' is " "an integer */" % (fname, cname, fname)
                        )
                    continue
                ...
                while isinstance(ftype, model.ArrayType) and (ftype.length is None or ftype.length == "..."):
                    ...
                    fname = fname + "[0]"
                prnt(
                    "  { %s = &p->%s; (void)tmp; }" % (ftype.get_c_name("*tmp", "field %r" % fname, quals=fqual), fname)
                )
    ...
    def _struct_ctx(self, tp, cname, approxname, named_ptr=None):
        ...
        if tp not in self.ffi._parser._included_declarations and (
            named_ptr is None or named_ptr not in self.ffi._parser._included_declarations
        ):
            if tp.fldtypes is None:
                pass  # opaque
        ...
        self._lsts["struct_union"].append(
            StructUnionExpr(tp.name, type_index, flags, size, align, comment, first_field_index, c_fields)
        )
    ...
    def _add_missing_struct_unions(self):
        ...
        for tp, order in lst:
            if tp not in self._seen_struct_unions:
                if tp.partial:
                    raise NotImplementedError(
                        "internal inconsistency: %r is " "partial but was not seen at " "this point" % (tp,)
                    )
    ...
    def _enum_ctx(self, tp, cname):
        ...
        for enumerator, enumvalue in zip(tp.enumerators, tp.enumvalues):
            self._lsts["global"].append(
                GlobalExpr(enumerator, "_cffi_const_%s" % enumerator, type_op, check_value=enumvalue)
            )
        #
    ...
    def _generate_cpy_macro_ctx(self, tp, name):
        if tp == "...":
            if self.target_is_python:
                raise VerificationError(
                    "cannot use the syntax '...' in '#define %s ...' when " "using the ABI mode" % (name,)
                )
            check_value = None
        else:
            check_value = tp  # an integer
    ...
    _generate_cpy_dllexport_python_collecttype = _generate_cpy_extern_python_plus_c_collecttype = (
    ...
    def _emit_bytecode_ArrayType(self, tp, index):
        ...
        if tp.length is None:
            ...
            raise VerificationError(
                ...
                "used on global arrays or on fields of structures" % (str(tp).replace("/*...*/", "..."),)
            )
        else:
