...
class TestMgmtHybridConnection(AzureMgmtRecordedTestCase):
    ...
    def test_hybridconnetion_curd(self, resource_group, location):
        ...
        createdhybridconnectionresponse = self.relay_client.hybrid_connections.create_or_update(
            resource_group_name,
            namespace_name,
            hybridconnection_name,
            {"requires_client_authorization": True, "user_metadata": "User data for HybridConnection"},
        )
        ...
        getlistbynamespacehybridconnectionresponse = list(
            self.relay_client.hybrid_connections.list_by_namespace(resource_group_name, namespace_name)
        )
        ...
        assert createhybridconnectionauthorule.name, (
        ...
        gethybridconnectionauthorule = self.relay_client.hybrid_connections.get_authorization_rule(
            resource_group_name, namespace_name, hybridconnection_name, authoRule_name
        )
        assert gethybridconnectionauthorule.name, (
            authoRule_name == "Authorization rule name not as passed as parameter - get_authorization_rule "
        )
        assert len(gethybridconnectionauthorule.rights), (
        ...
        updatehybridconnectionauthorule = self.relay_client.hybrid_connections.create_or_update_authorization_rule(
            resource_group_name, namespace_name, hybridconnection_name, authoRule_name, gethybridconnectionauthorule
        )
        assert updatehybridconnectionauthorule.name, (
            authoRule_name
            == "Authorization rule name not as passed as parameter for update call - create_or_update_authorization_rule "
        )
        assert len(updatehybridconnectionauthorule.rights), (
        ...
        assert len(hybridconnectionauthorulelist), (
        ...
        regenratePrimarykeyauthorizationrule = self.relay_client.hybrid_connections.regenerate_keys(
            resource_group_name, namespace_name, hybridconnection_name, authoRule_name, {"key_type": "PrimaryKey"}
        )
        ...
        regenrateSecondarykeyauthorizationrule = self.relay_client.hybrid_connections.regenerate_keys(
            resource_group_name, namespace_name, hybridconnection_name, authoRule_name, {"key_type": "SecondaryKey"}
        )
