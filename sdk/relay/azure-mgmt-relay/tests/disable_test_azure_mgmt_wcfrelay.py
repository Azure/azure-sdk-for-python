...
from azure.mgmt.relay.models import (
    RelayNamespace,
    Sku,
    Relaytype,
    AuthorizationRule,
...
class TestMgmtWcfRelay(AzureMgmtRecordedTestCase):
    ...
    def test_wcfrelay_curd(self, resource_group, location):
        ...
        createdwcfrelayresponse = self.relay_client.wcf_relays.create_or_update(
            resource_group_name, namespace_name, wcfrelay_name, wcfrelayparameter
        )
        ...
        getlistbynamespacewcfrelayresponse = list(
            self.relay_client.wcf_relays.list_by_namespace(resource_group_name, namespace_name)
        )
        assert len(getlistbynamespacewcfrelayresponse) > 0

        # update the Created eventhub
        wcfrelayupdateparameter = WcfRelay(relay_type=Relaytype.net_tcp, user_metadata="User data for WcfRelay updated")
        updatewcfrelayresponse = self.relay_client.wcf_relays.create_or_update(
            resource_group_name, namespace_name, wcfrelay_name, wcfrelayupdateparameter
        )
        ...
        assert createwcfrelayauthorule.name, (
        ...
        assert getwcfrelayauthorule.name, (
        ...
        updatewcfrelayauthorule = self.relay_client.wcf_relays.create_or_update_authorization_rule(
            resource_group_name, namespace_name, wcfrelay_name, authoRule_name, getwcfrelayauthorule
        )
        assert updatewcfrelayauthorule.name, (
            authoRule_name
            == "Authorization rule name not as passed as parameter for update call - create_or_update_authorization_rule "
        )
        assert len(updatewcfrelayauthorule.rights), (
        ...
        wcfrelayauthorulelist = list(
            self.relay_client.wcf_relays.list_authorization_rules(resource_group_name, namespace_name, wcfrelay_name)
        )
        assert len(wcfrelayauthorulelist), (
        ...
        regenratePrimarykeyauthorizationrule = self.relay_client.wcf_relays.regenerate_keys(
            resource_group_name, namespace_name, wcfrelay_name, authoRule_name, {"key_type": "PrimaryKey"}
        )
        ...
        regenrateSecondarykeyauthorizationrule = self.relay_client.wcf_relays.regenerate_keys(
            resource_group_name, namespace_name, wcfrelay_name, authoRule_name, {"key_type": "SecondaryKey"}
        )
