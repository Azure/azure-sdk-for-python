...
class TestBreakSentence(TextTranslationTest):
    ...
    def test_with_language(self, **kwargs):      
        ...
        input_text_elements = [ InputTextItem(text = "รวบรวมแผ่นคำตอบ ระยะเวลาของโครงการ วิธีเลือกชายในฝัน หมายเลขซีเรียลของระเบียน วันที่สิ้นสุดของโครงการเมื่อเสร็จสมบูรณ์ ปีที่มีการรวบรวม ทุกคนมีวัฒนธรรมและวิธีคิดเหมือนกัน ได้รับโทษจำคุกตลอดชีวิตใน ฉันลดได้ถึง 55 ปอนด์ได้อย่างไร  ฉันคิดว่าใครๆ ก็ต้องการกำหนดเมนูอาหารส่วนบุคคล") ]

        response = client.find_sentence_boundaries(content = input_text_elements, language="th")
        assert response is not None
        expectedLengths = [78, 41, 110, 46]
        for i in range(len(expectedLengths)):
            assert expectedLengths[i] == response[0].sent_len[i]
    ...
    def test_with_language_script(self, **kwargs):      
        ...
        response = client.find_sentence_boundaries(content = input_text_elements, language="zh-Hans", script= "Latn")
        assert response is not None
    ...
    def test_with_multiple_languages(self, **kwargs):      
        ...
        input_text_elements = [ InputTextItem(text = "hello world"), InputTextItem(text = "العالم هو مكان مثير جدا للاهتمام") ]
