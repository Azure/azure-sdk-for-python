...
DocumentTranslationClientPreparer = functools.partial(_DocumentTranslationClientPreparer, SingleDocumentTranslationClient)
...
class TestSingleDocumentTranslation(AsyncDocumentTranslationTest):
    ...
    async def test_translate_text_document(self, **kwargs):        
        if self.is_live: # running these tests in Live mode only
            ...
            document_translate_content = DocumentTranslateContent(document=_TestSingleDocumentTranslation()._get_document_content())

            # Invoke document translation
            response_stream =await client.document_translate(body=document_translate_content, target_language=target_languages)
    ...
    async def test_translate_single_csv_glossary(self, **kwargs):        
        if self.is_live: # running these tests in Live mode only
            ...
            # prepare translation content
            document_translate_content = DocumentTranslateContent(document=_TestSingleDocumentTranslation()._get_document_content(), glossary=_TestSingleDocumentTranslation()._get_single_glossary_content())
    ...
    async def test_translate_multiple_csv_glossary(self, **kwargs):        
        if self.is_live: # running these tests in Live mode only
            ...
            # prepare translation content
            document_translate_content = DocumentTranslateContent(document=_TestSingleDocumentTranslation()._get_document_content(), glossary=_TestSingleDocumentTranslation()._get_multiple_glossary_contents())
