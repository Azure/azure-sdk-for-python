...
class MgmtACRTest20170301(AzureMgmtTestCase):
    def setUp(self):
        ...
        self.client = self.create_mgmt_client(
            azure.mgmt.containerregistry.ContainerRegistryManagementClient, api_version="2017-03-01"
        )
    ...
    def _disabled_test_basic_registry(self, resource_group, location, storage_account, storage_account_key):
        ...
        registry = self.client.registries.create(
            ...
            registry_create_parameters=RegistryCreateParameters(
                location=location,
                sku=Sku(name=DEFAULT_SKU_NAME),
                storage_account=StorageAccountParameters(name=storage_account.name, access_key=storage_account_key),
            ),
        ).result()
