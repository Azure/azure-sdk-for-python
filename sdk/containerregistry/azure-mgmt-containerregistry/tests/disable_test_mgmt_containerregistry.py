...
class MgmtACRTest(AzureMgmtTestCase):
    def setUp(self):
        ...
        self.client = self.create_mgmt_client(azure.mgmt.containerregistry.ContainerRegistryManagementClient)
    ...
    def _disabled_test_classic_registry(self, resource_group, location, storage_account):
        ...
        registry = self.client.registries.create(
            ...
            registry=Registry(
                location=location,
                sku=Sku(name=SkuName.classic),
                storage_account=StorageAccountProperties(id=storage_account.id),
            ),
        ).result()
