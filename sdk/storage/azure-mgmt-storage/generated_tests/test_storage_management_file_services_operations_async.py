...
class TestStorageManagementFileServicesOperationsAsync(AzureMgmtRecordedTestCase):
    def setup_method(self, method):
        self.client = self.create_mgmt_client(StorageManagementClient, is_async=True)

    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_file_services_list(self, resource_group):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_file_services_set_service_properties(self, resource_group):
        response = await self.client.file_services.set_service_properties(
            resource_group_name=resource_group.name,
            account_name="str",
            parameters={
                "cors": {
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_file_services_get_service_properties(self, resource_group):
