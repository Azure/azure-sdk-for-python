...
### Features Added
  ...
  - Model `BlobServiceItems` added property `next_link`
  ...
  - Model `ListBlobInventoryPolicy` added property `next_link`
  ...
  - Model `ObjectReplicationPolicies` added property `next_link`
  ...
  - Model `StorageSkuListResult` added property `next_link`
...
### Breaking Changes
  ...
  - Method `LocalUsers.__init__` removed default value `None` from its parameter `value`
  ...
  - Method `DeletedAccountsOperations.get` re-ordered its parameters from `['self', 'deleted_account_name', 'location', 'kwargs']` to `['self', 'location', 'deleted_account_name', 'kwargs']`
