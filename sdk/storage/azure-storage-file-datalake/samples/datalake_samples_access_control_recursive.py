...
def recursive_access_control_sample(filesystem_client):
    ...
    def progress_callback(acl_changes):
        print(
            (
                "In this batch: {} directories and {} files were processed successfully, {} failures were counted. "
                + "In total, {} directories and {} files were processed successfully, {} failures were counted."
            ).format(
                acl_changes.batch_counters.directories_successful,
                acl_changes.batch_counters.files_successful,
                acl_changes.batch_counters.failure_count,
                acl_changes.aggregate_counters.directories_successful,
                acl_changes.aggregate_counters.files_successful,
                acl_changes.aggregate_counters.failure_count,
            )
        )
    ...
    try:
        acl_change_result = directory_client.set_access_control_recursive(
            acl=acl, progress_hook=progress_callback, batch_size=5
        )
        ...
        if error.continuation_token:
            acl_change_result = directory_client.set_access_control_recursive(
                acl=acl, continuation_token=error.continuation_token, progress_hook=progress_callback, batch_size=5
            )

    print(
        "Summary: {} directories and {} files were updated successfully, {} failures were counted.".format(
            acl_change_result.counters.directories_successful,
