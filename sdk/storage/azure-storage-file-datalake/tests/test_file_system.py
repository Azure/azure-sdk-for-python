...
from azure.storage.filedatalake import ContainerSessionProvider
...
class TestFileSystem(StorageRecordedTestCase):
    ...
    def test_bad_audience_service_client(self, **kwargs):
        ...
        token_credential = self.get_credential(DataLakeServiceClient)
        fsc = FileSystemClient(
            url, file_system_name, credential=token_credential, audience="https://badaudience.blob.core.windows.net/"
        )
    ...
    def test_create_session(self, **kwargs):
        ...
        credential = self.get_credential(DataLakeServiceClient)
        ...
        with DataLakeServiceClient(
            ...
            try:
                fs1.create_file_system()
            except ResourceExistsError:
                pass

            data1 = b"abc123"
            file1 = fs1.get_file_client("file1")
            file1.upload_data(data1, overwrite=True, raw_response_hook=capture_auth_header.hook("f1_upload"))
            assert capture_auth_header["f1_upload"].startswith("Bearer ")

            actual1 = file1.download_file(raw_response_hook=capture_auth_header.hook("f1_download")).readall()
            ...
            try:
                fs2.create_file_system()
            except ResourceExistsError:
                pass

            data2 = b"def456"
            file2 = fs2.get_file_client("file2")
            file2.upload_data(data2, overwrite=True)

            actual2 = file2.download_file(raw_response_hook=capture_auth_header.hook("f2_download")).readall()
            ...
            file1 = fs1.get_file_client("file1")
            file1.download_file(raw_response_hook=capture_auth_header.hook("f1_download2")).readall()
            ...
            file1.download_file(raw_response_hook=capture_auth_header.hook("f1_download3")).readall()
    ...
    def test_sessions_disabled(self, **kwargs):
        ...
        with DataLakeServiceClient(
            ...
            actual = file_client.download_file(raw_response_hook=capture_auth_header.hook("download")).readall()
            assert data == actual
    ...
    def test_create_session_with_session_provider(self, **kwargs):
        ...
        session_provider = ContainerSessionProvider(blob_url, credential, api_version="2026-10-06")
