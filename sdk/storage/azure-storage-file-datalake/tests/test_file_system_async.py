...
class TestFileSystemAsync(AsyncStorageRecordedTestCase):
    ...
    async def test_list_file_systems_with_include_metadata_async(self, **kwargs):
        ...
        async for fs in self.dsc.list_file_systems(
            name_starts_with=file_system.file_system_name, include_metadata=True
        ):
    ...
    async def test_list_file_systems_by_page(self, **kwargs):
        ...
        file_systems = []
        async for fs in (
            await self.dsc.list_file_systems(results_per_page=3, name_starts_with="file", include_metadata=True)
            .by_page()
            .__anext__()
        ):
            file_systems.append(fs)
    ...
    async def test_service_client_session_closes_after_filesystem_creation(self, **kwargs):
        ...
        async with DataLakeServiceClient(self.dsc.url, credential=datalake_storage_account_key.secret) as ds_client:
    ...
    async def test_get_access_control_using_delegation_sas_async(self, **kwargs):
        ...
        delegation_key = await dsc.get_user_delegation_key(datetime.utcnow(), datetime.utcnow() + timedelta(hours=1))
        ...
        sas_directory_client = DataLakeDirectoryClient(
            self.dsc.url, file_system_name, directory_client_name, credential=token, logging_enable=True
        )
    ...
    async def test_list_paths_using_file_sys_delegation_sas_async(self, **kwargs):
        ...
        sas_directory_client = FileSystemClient(self.dsc.url, file_system_name, credential=token)
        paths = []
