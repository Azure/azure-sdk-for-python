...
class TestDirectoryAsync(AsyncStorageRecordedTestCase):
    ...
    async def test_create_sub_directory_proposed_lease_id_async(self, **kwargs):
        ...
        directory_name = self._get_directory_reference()
        directory_client = self.dsc.get_directory_client(self.file_system_name, directory_name)
        directory_client = await directory_client.create_sub_directory(
            sub_directory="sub1", lease_id=test_string, lease_duration=test_duration
        )
    ...
    async def test_using_oauth_token_credential_to_create_directory(self, **kwargs):
        ...
        directory_client = DataLakeDirectoryClient(
            self.dsc.url, self.file_system_name, directory_name, credential=token_credential
        )
    ...
    async def test_set_access_control_if_none_modified(self, **kwargs):
        ...
        response = await directory_client.set_access_control(
            permissions="0777", etag=resp["etag"], match_condition=MatchConditions.IfNotModified
        )
        # Assert
        assert response is not None
    ...
    async def test_set_access_control_recursive_throws_exception_containing_continuation_token(self, **kwargs):
        ...
        with pytest.raises(AzureError) as acl_error:
            await directory_client.set_access_control_recursive(
                acl=acl, batch_size=2, max_batches=2, raw_response_hook=callback, retry_total=0
            )
    ...
    async def test_set_access_control_recursive_in_batches_with_progress_callback(self, **kwargs):
        ...
        summary = await directory_client.set_access_control_recursive(
            acl=acl, progress_hook=progress_callback, batch_size=2
        )
    ...
    async def test_set_access_control_recursive_with_failures(self, **kwargs):
        ...
        summary = await owner_dir_client.set_access_control_recursive(
            acl=acl, progress_hook=progress_callback, batch_size=2
        )
    ...
    async def test_set_access_control_recursive_in_batches_with_explicit_iteration(self, **kwargs):
        ...
        while result.continuation is not None:
            result = await directory_client.set_access_control_recursive(
    ...
    async def test_update_access_control_recursive_in_batches_with_progress_callback(self, **kwargs):
        ...
        summary = await directory_client.update_access_control_recursive(
            acl=acl, progress_hook=progress_callback, batch_size=2
        )
    ...
    async def test_update_access_control_recursive_with_failures(self, **kwargs):
        ...
        summary = await owner_dir_client.update_access_control_recursive(
            acl=acl, progress_hook=progress_callback, batch_size=2
        )
    ...
    async def test_update_access_control_recursive_continue_on_failures(self, **kwargs):
        ...
        summary = await owner_dir_client.update_access_control_recursive(
            acl=acl, progress_hook=progress_callback, batch_size=2, continue_on_failure=True
        )
    ...
    async def test_remove_access_control_recursive_in_batches_with_progress_callback(self, **kwargs):
        ...
        summary = await directory_client.remove_access_control_recursive(
    ...
    async def test_rename_from(self, **kwargs):
        ...
        await new_directory_client._rename_path(
            "/" + self.file_system_name + "/" + directory_name, content_settings=content_settings
        )
    ...
    async def test_rename_dir_with_file_sas(self, **kwargs):
        ...
        token = generate_directory_sas(
            ...
            permission=DirectorySasPermissions(read=True, create=True, write=True, delete=True, move=True),
        ...
        new_token = generate_directory_sas(
            ...
            "newdir",
            datalake_storage_account_key.secret,
            permission=DirectorySasPermissions(read=True, create=True, write=True, delete=True),
            expiry=datetime.utcnow() + timedelta(hours=1),
        )
    ...
    async def test_using_directory_sas_to_create_file(self, **kwargs):
        ...
        service_client2 = DataLakeServiceClient(
            "https://abc.dfs.core.windows.net", credential="fake", api_version="2019-02-02"
        )
        filesys_client2 = service_client2.get_file_system_client("filesys")
        dir_client2 = DataLakeDirectoryClient(
            "https://abc.dfs.core.windows.net", "filesys", "dir", credential="fake", api_version="2019-02-02"
        )
