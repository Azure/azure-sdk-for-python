...
class TestStorageFileNFSAsync(AsyncStorageRecordedTestCase):
    ...
    @FileSharePreparer()
    @recorded_by_proxy_async
    async def test_symlink(self, **kwargs):
        premium_storage_file_account_name = kwargs.pop("premium_storage_file_account_name")
        ...
        resp = await symbolic_link_file_client.symlink_to(
            target=target,
            metadata=metadata,
            owner=owner,
            group=group
    ...
    @FileSharePreparer()
    @recorded_by_proxy_async
    async def test_symlink_error(self, **kwargs):
        premium_storage_file_account_name = kwargs.pop("premium_storage_file_account_name")
        ...
        with pytest.raises(ResourceNotFoundError) as e:
            await symbolic_link_file_client.symlink_to(target=target)
        assert 'ParentNotFound' in e.value.args[0]
