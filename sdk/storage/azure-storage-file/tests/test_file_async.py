...
class StorageFileTestAsync(AsyncFileTestCase):
    ...
    async def _setup_share(self, fsc, remote=False):
    ...
    async def _create_file(self, fsc, file_name=None):
    ...
    async def _create_empty_file(self, fsc, file_name=None, file_size=2048):
        await self._setup_share(fsc)
        file_name = self.get_resource_name(TEST_FILE_PREFIX) if file_name is None else file_name
        share_client = fsc.get_share_client(self.share_name)
        file_client = share_client.get_file_client(file_name)
    ...
    async def _get_file_client(self, fsc):
        await self._setup_share(fsc)
    ...
    async def _create_remote_share(self, fsc2):
    ...
    async def _create_remote_file(self, fsc2, file_data=None):
        if not file_data:
    ...
    async def _wait_for_async_copy(self, share_name, file_path, fsc):
        count = 0
    ...
    async def assert_file_equal(self, file_client, expected_data):
    ...
    async def test_create_file_when_file_permission_is_too_long(self, resource_group, location, storage_account, storage_account_key):
        fsc = FileServiceClient(self._account_url(storage_account.name), credential=storage_account_key, transport=AiohttpTestTransport())
        file_client = await self._get_file_client(fsc)
    ...
    async def test_create_file_with_invalid_file_permission(self, resource_group, location, storage_account, storage_account_key):
        # Arrange
        fsc = FileServiceClient(self._account_url(storage_account.name), credential=storage_account_key, transport=AiohttpTestTransport())
        file_name = await self._get_file_client(fsc)
    ...
    @StorageAccountPreparer(name_prefix='pyacrstorage', playback_fake_resource=FAKE_STORAGE)
    @AsyncFileTestCase.await_prepared_test
    async def test_update_range_without_enough_bytes(self, resource_group, location, storage_account, storage_account_key):
    ...
    async def test_update_range_from_file_url(self, resource_group, location, storage_account, storage_account_key):
        ...
        source_file_client = await self._create_file(fsc, file_name=source_file_name)
        data = b'abcdefghijklmnop' * 32
        await source_file_client.upload_range(data, 0, 511)

        destination_file_name = 'filetoupdate'
        destination_file_client = await self._create_empty_file(fsc, file_name=destination_file_name)
    ...
    async def test_update_big_range_from_file_url(self, resource_group, location, storage_account, storage_account_key):
        ...
        source_file_client = await self._create_empty_file(fsc, file_name=source_file_name, file_size=1024 * 1024)
        data = b'abcdefghijklmnop' * 65536
        await source_file_client.upload_range(data, 0, end)

        destination_file_name = 'filetoupdate1'
        destination_file_client = await self._create_empty_file(fsc, file_name=destination_file_name, file_size=1024 * 1024)
    ...
    async def test_copy_file_with_existing_file_async(self, resource_group, location, storage_account, storage_account_key):
        # Arrange
        fsc = FileServiceClient(self._account_url(storage_account.name), credential=storage_account_key, transport=AiohttpTestTransport())
        source_client = await self._create_file(fsc)
        file_client = FileClient(
            self._account_url(storage_account.name),
            share=self.share_name,
            file_path='file1copy',
    ...
    async def test_abort_copy_file_with_synchronous_copy_fails_async(self, resource_group, location, storage_account, storage_account_key):
        # Arrange
        fsc = FileServiceClient(self._account_url(storage_account.name), credential=storage_account_key, transport=AiohttpTestTransport())
        source_file = await self._create_file(fsc)
    ...
    async def test_shared_write_access_file_async(self, resource_group, location, storage_account, storage_account_key):
        ...
        file_client_admin = await self._create_file(fsc)
