...
class StoragePageBlobTest(StorageTestCase):
    ...
    def test_create_blob(self, resource_group, location, storage_account, storage_account_key):
        bsc = BlobServiceClient(self.account_url(storage_account, "blob"), credential=storage_account_key, connection_data_block_size=4 * 1024, max_page_size=4 * 1024, **self.get_client_kwargs())
        self._setup(bsc)
        blob = self._get_blob_reference(bsc)
    ...
    def test_blob_tier_on_create(self, resource_group, location, storage_account, storage_account_key):
        ...
        pbs = BlobServiceClient(url, credential=credential, **self.get_client_kwargs())
