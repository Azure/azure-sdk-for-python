...
from azure.storage.blob._shared.request_handlers import (
...
class _StorageAccountHosts(StorageAccountHostsMixin):
...
@patch(
    "azure.storage.blob._shared.base_client._construct_endpoints",
    return_value=("account", "", "account-secondary.blob.core.windows.net"),
)
def test_primary_hostname_falls_back_to_parsed_url(_):
    parsed_url = urlparse("https://account.blob.core.windows.net")

    client = _StorageAccountHosts(parsed_url, "blob")

    assert client.primary_hostname == "account.blob.core.windows.net"
