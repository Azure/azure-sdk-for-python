...
class TestStorageBlobAccessConditionsAsync(AsyncStorageRecordedTestCase):
    ...
    async def _create_container_and_block_blob(self, container_name, blob_name, blob_data, bsc):
        await self._create_container(container_name, bsc)
    ...
    async def _create_container_and_page_blob(self, container_name, blob_name, content_length, bsc):
        await self._create_container(container_name, bsc)
        blob = bsc.get_blob_client(container_name, blob_name)
        await blob.create_page_blob(str(content_length))
        return blob
    ...
    async def test_multi_put_block_contains_headers(self, **kwargs):
        ...
        await blob.upload_blob(data, headers={"x-custom-header": "test_value"}, raw_request_hook=_validate_headers)
    ...
    async def test_put_blob_with_if_modified(self, **kwargs):
        ...
        self._setup()
        data = b"hello world"
        blob = await self._create_container_and_block_blob(self.container_name, "blob1", data, bsc)
        test_datetime = self.get_datetime_variable(variables, "if_modified", datetime.utcnow() - timedelta(minutes=15))
    ...
    async def test_get_blob_with_if_modified(self, **kwargs):
        ...
        self._setup()
        blob = await self._create_container_and_block_blob(self.container_name, "blob1", b"hello world", bsc)
        test_datetime = self.get_datetime_variable(variables, "if_modified", datetime.utcnow() - timedelta(minutes=15))
    ...
    async def test_set_blob_props_with_if_modified(self, **kwargs):
        ...
        self._setup()
        await self._create_container_and_block_blob(self.container_name, "blob1", b"hello world", bsc)
        test_datetime = self.get_datetime_variable(variables, "if_modified", datetime.utcnow() - timedelta(minutes=15))
        # Act
    ...
    async def test_set_blob_props_with_if_none_match(self, **kwargs):
        ...
        blob = bsc.get_blob_client(self.container_name, "blob1")
        await blob.set_http_headers(
            content_settings, etag="0x111111111111111", match_condition=MatchConditions.IfModified
        )
    ...
    async def test_get_blob_metadata_with_if_none_match(self, **kwargs):
        ...
        blob = bsc.get_blob_client(self.container_name, "blob1")
        md = (
            await blob.get_blob_properties(etag="0x111111111111111", match_condition=MatchConditions.IfModified)
        ).metadata
    ...
    async def test_set_blob_metadata_with_if_match_fail(self, **kwargs):
        ...
        with pytest.raises(ResourceModifiedError) as e:
            ...
            await blob.set_blob_metadata(
                metadata, etag="0x111111111111111", match_condition=MatchConditions.IfNotModified
            )
    ...
    async def test_lease_blob_with_if_modified(self, **kwargs):
        ...
        # Act
        blob = bsc.get_blob_client(self.container_name, "blob1")
        lease = await blob.acquire_lease(if_modified_since=test_datetime, lease_id=test_lease_id)
    ...
    async def test_lease_blob_with_if_unmodified(self, **kwargs):
        ...
        # Act
        blob = bsc.get_blob_client(self.container_name, "blob1")
        lease = await blob.acquire_lease(if_unmodified_since=test_datetime, lease_id=test_lease_id)
    ...
    async def test_lease_blob_with_if_match(self, **kwargs):
        ...
        lease = await blob.acquire_lease(
            lease_id=test_lease_id, etag=etag, match_condition=MatchConditions.IfNotModified
        )
    ...
    async def test_lease_blob_with_if_none_match_fail(self, **kwargs):
        ...
        # Act
        with pytest.raises(ResourceModifiedError) as e:
            await blob.acquire_lease(lease_id=test_lease_id, etag=etag, match_condition=MatchConditions.IfModified)
    ...
    async def test_put_block_list_with_if_modified(self, **kwargs):
        ...
        self._setup()
        blob = await self._create_container_and_block_blob(self.container_name, "blob1", b"", bsc)
        await asyncio.gather(
            *[blob.stage_block("1", b"AAA"), blob.stage_block("2", b"BBB"), blob.stage_block("3", b"CCC")]
        )
        test_datetime = self.get_datetime_variable(variables, "if_modified", datetime.utcnow() - timedelta(minutes=15))
    ...
    async def test_update_page_with_if_modified(self, **kwargs):
        ...
        self._setup()
        await self._create_container_and_page_blob(self.container_name, "blob1", 1024, bsc)
        test_datetime = self.get_datetime_variable(variables, "if_modified", datetime.utcnow() - timedelta(minutes=15))
    ...
    async def test_get_page_ranges_iter_with_if_modified(self, **kwargs):
        ...
        self._setup()
        blob = await self._create_container_and_page_blob(self.container_name, "blob1", 2048, bsc)
        data = b"abcdefghijklmnop" * 32
        test_datetime = self.get_datetime_variable(variables, "if_modified", datetime.utcnow() - timedelta(minutes=15))
        await asyncio.gather(
            blob.upload_page(data, offset=0, length=512), blob.upload_page(data, offset=1024, length=512)
        )
    ...
    async def test_append_block_with_if_modified(self, **kwargs):
        ...
        self._setup()
        blob = await self._create_container_and_append_blob(self.container_name, "blob1", bsc)
        test_datetime = self.get_datetime_variable(variables, "if_modified", datetime.utcnow() - timedelta(minutes=15))
        # Act
        for i in range(5):
    ...
    async def test_append_block_with_if_modified_fail(self, **kwargs):
        ...
        with pytest.raises(ResourceModifiedError) as e:
            for i in range(5):
                await blob.append_block(f"block {i}", if_modified_since=test_datetime)
    ...
    async def test_append_block_with_if_match(self, **kwargs):
        ...
        for i in range(5):
            etag = (await blob.get_blob_properties()).etag
            resp = await blob.append_block(f"block {i}", etag=etag, match_condition=MatchConditions.IfNotModified)
            assert resp is not None
        ...
        with pytest.raises(HttpResponseError):
            for i in range(5):
                await blob.append_block(
                    f"block {i}", etag="0x111111111111111", match_condition=MatchConditions.IfNotModified
                )
    ...
    async def test_append_block_with_if_none_match(self, **kwargs):
        ...
        for i in range(5):
            resp = await blob.append_block(
                f"block {i}", etag="0x8D2C9167D53FC2C", match_condition=MatchConditions.IfModified
            )
            assert resp is not None
    ...
    async def test_append_block_with_if_none_match_fail(self, **kwargs):
        ...
        with pytest.raises(ResourceModifiedError) as e:
            for i in range(5):
                etag = (await blob.get_blob_properties()).etag
                await blob.append_block(f"block {i}", etag=etag, match_condition=MatchConditions.IfModified)
    ...
    async def test_append_blob_from_bytes_with_if_modified(self, **kwargs):
        ...
        blob = await self._create_container_and_append_blob(self.container_name, blob_name, bsc)
        test_datetime = self.get_datetime_variable(variables, "if_modified", datetime.utcnow() - timedelta(minutes=15))
