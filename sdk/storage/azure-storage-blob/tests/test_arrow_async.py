...
class TestStorageApacheArrowAsync(AsyncStorageRecordedTestCase):
    ...
    async def test_arrow_list_no_blobs(self, **kwargs):
        ...
        blobs_list = [blob async for blob in container.list_blobs(use_arrow=True)]
    ...
    @pytest.mark.asyncio
    async def test_arrow_mock_xml_response(self):
        blob_names = [
            "a/b/blob1",
            "a/b/blob2",
            "a/c/blob3",
        ...
        with patch.object(
            ...
            blobs_list = [blob async for page in page_iterator async for blob in page]
    ...
    async def test_arrow_walk_no_blobs(self, **kwargs):
        ...
        await self._setup(storage_account_name, storage_account_key)
        container = self.bsc.get_container_client(self.container_name)
        blobs_list = [blob async for blob in container.walk_blobs(use_arrow=True)]
    ...
    @pytest.mark.asyncio
    async def test_arrow_mock_walk_xml_response(self):
        blob_names = [
            "a/b/blob1",
            "a/b/blob2",
            "a/c/blob3",
