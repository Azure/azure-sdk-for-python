...
class StorageBlobTagsTest(StorageTestCase):
    ...
    def _create_block_blob(self, blob_tags=None, container_name=None):
        blob_name = self._get_blob_reference()
        blob_client = self.bsc.get_blob_client(container_name or self.container_name, blob_name)
        resp = blob_client.upload_blob(self.byte_data, length=len(self.byte_data), overwrite=True, blob_tags=blob_tags)
    ...
    def _create_append_blob(self, blob_tags=None):
        blob_name = self._get_blob_reference()
        blob_client = self.bsc.get_blob_client(self.container_name, blob_name)
        resp = blob_client.create_append_blob(blob_tags=blob_tags)
        return blob_client, resp

    def _create_page_blob(self, blob_tags=None):
        blob_name = self._get_blob_reference()
        blob_client = self.bsc.get_blob_client(self.container_name, blob_name)
    ...
    def test_set_blob_tags_for_a_version(self, resource_group, location, storage_account, storage_account_key):
        ...
        # Act
        blob_tags = {"tag1": "firsttag", "tag2": "secondtag", "tag3": "thirdtag"}
        resp = blob_client.set_blob_tags(blob_tags, version_id=resp['version_id'])
    ...
    def test_get_blob_tags(self, resource_group, location, storage_account, storage_account_key):
        ...
        for key, value in resp.items():
            self.assertEqual(blob_tags[key], value)
    ...
    def test_get_blob_tags_for_a_snapshot(self, resource_group, location, storage_account, storage_account_key):
        ...
        blob_client, resp = self._create_block_blob(blob_tags=blob_tags)
    ...
    def test_get_blob_properties_returns_tags_num(self, resource_group, location, storage_account, storage_account_key):
        ...
        # Assert
        self.assertIsNotNone(resp)
        self.assertEqual(resp.tag_count, len(blob_tags))
        self.assertEqual(downloaded.properties.tag_count, len(blob_tags))

    @GlobalStorageAccountPreparer()
    def test_create_append_blob_with_tags(self, resource_group, location, storage_account, storage_account_key):
        self._setup(storage_account, storage_account_key)
        blob_tags = {"+-./:=_ ": "firsttag", "tag2": "+-./:=_", "+-./:=_1": "+-./:=_"}
        blob_client, resp = self._create_append_blob(blob_tags=blob_tags)
    ...
    def test_create_page_blob_with_tags(self, resource_group, location, storage_account, storage_account_key):
        self._setup(storage_account, storage_account_key)
        blob_tags = {"tag1": "firsttag", "tag2": "secondtag", "tag3": "thirdtag"}
        blob_client, resp = self._create_page_blob(blob_tags=blob_tags)
    ...
    def test_commit_block_list_with_tags(self, resource_group, location, storage_account, storage_account_key):
        ...
        blob_client.commit_block_list(block_list, blob_tags=blob_tags)

        resp = blob_client.get_blob_tags()

        # Assert
        self.assertIsNotNone(resp)
        self.assertEqual(len(resp), len(blob_tags))
    ...
    def test_start_copy_from_url_with_tags(self, resource_group, location, storage_account, storage_account_key):
        ...
        copy = copyblob.start_copy_from_url(sourceblob, blob_tags=blob_tags)
    ...
    def test_list_blobs_returns_tags(self, resource_group, location, storage_account, storage_account_key):
        ...
        for blob in blob_list:
            self.assertEqual(blob.tag_count, len(blob_tags))
            for key, value in blob.blob_tags.items():
                self.assertEqual(blob_tags[key], value)
    ...
    def test_filter_blobs(self, resource_group, location, storage_account, storage_account_key):
        ...
        blob_tags = {"tag1": "firsttag", "tag2": "secondtag", "tag3": "thirdtag"}
        self._create_block_blob(blob_tags=blob_tags)
        self._create_block_blob(blob_tags=blob_tags, container_name=container_name1)
        self._create_block_blob(blob_tags=blob_tags, container_name=container_name2)
        self._create_block_blob(blob_tags=blob_tags, container_name=container_name3)
