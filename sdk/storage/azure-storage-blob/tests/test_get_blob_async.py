...
class TestStorageGetBlobTest(AsyncStorageRecordedTestCase):
    ...
    async def test_get_blob_to_bytes_snapshot(self, **kwargs):
        ...
        snapshot = self.bsc.get_blob_client(
            self.container_name, self.byte_blob, snapshot=snapshot_ref
        )

        await blob.upload_blob(
            self.byte_data, overwrite=True
        )  # Modify the blob so the Etag no longer matches
    ...
    async def test_get_blob_to_bytes_with_progress(self, **kwargs):
        ...
        # Act
        content = await (
            await blob.download_blob(raw_response_hook=callback, max_concurrency=2)
        ).readall()
    ...
    async def test_get_blob_to_bytes_non_parallel(self, **kwargs):
        ...
        # Act
        content = await (
            await blob.download_blob(raw_response_hook=callback, max_concurrency=1)
        ).readall()
    ...
    async def test_readinto_raises_exceptions(self, **kwargs):
        ...
        with tempfile.TemporaryFile() as temp_file:
            downloader = await blob.download_blob(
                max_concurrency=2, raw_response_hook=callback
            )
            with pytest.raises(ValueError):
    ...
    async def test_get_blob_to_stream_with_progress(self, **kwargs):
        ...
        with tempfile.TemporaryFile() as temp_file:
            downloader = await blob.download_blob(
                raw_response_hook=callback, max_concurrency=2
            )
            read_bytes = await downloader.readinto(temp_file)
            # Assert
            assert read_bytes == len(self.byte_data)
    ...
    async def test_get_blob_to_stream_non_parallel(self, **kwargs):
        ...
        with tempfile.TemporaryFile() as temp_file:
            downloader = await blob.download_blob(
                raw_response_hook=callback, max_concurrency=1
            )
            read_bytes = await downloader.readinto(temp_file)
    ...
    async def test_ranged_get_blob_to_path(self, **kwargs):
        ...
        FILE_PATH = "ranged_get_blob_to_path_async.temp.{}.dat".format(
            str(uuid.uuid4())
        )
        with tempfile.TemporaryFile() as temp_file:
            downloader = await blob.download_blob(
                offset=1, length=end_range - 1, max_concurrency=2
            )
            read_bytes = await downloader.readinto(temp_file)
    ...
    async def test_ranged_get_blob_to_path_invalid_range_parallel(self, **kwargs):
        ...
        # Act
        FILE_PATH = "path_invalid_range_parallel_async.temp.{}.dat".format(
            str(uuid.uuid4())
        )
        end_range = 2 * self.config.max_single_get_size
        with tempfile.TemporaryFile() as temp_file:
            downloader = await blob.download_blob(
                offset=1, length=end_range, max_concurrency=2
            )
            read_bytes = await downloader.readinto(temp_file)
    ...
    async def test_get_blob_exact_chunk_size(self, **kwargs):
        ...
        byte_data = self.get_random_bytes(
            self.config.max_single_get_size + self.config.max_chunk_get_size
        )
        blob = self.bsc.get_blob_client(self.container_name, blob_name)
        await blob.upload_blob(byte_data)
    ...
    async def test_get_blob_to_stream_with_md5(self, **kwargs):
        ...
        with tempfile.TemporaryFile() as temp_file:
            downloader = await blob.download_blob(
                validate_content=True, max_concurrency=2
            )
            read_bytes = await downloader.readinto(temp_file)
    ...
    async def test_get_blob_with_md5(self, **kwargs):
        ...
        # Act
        content = await (
            await blob.download_blob(validate_content=True, max_concurrency=2)
        ).readall()
    ...
    async def test_get_blob_range_to_stream_with_overall_md5(self, **kwargs):
        ...
        with tempfile.TemporaryFile() as temp_file:
            downloader = await blob.download_blob(
                offset=0, length=1024, validate_content=True, max_concurrency=2
            )
            read_bytes = await downloader.readinto(temp_file)
    ...
    async def test_get_blob_progress_chunked(self, **kwargs):
        ...
        # Act
        stream = await blob.download_blob(
            max_concurrency=1, progress_hook=progress.assert_progress
        )
        await stream.readall()
    ...
    async def test_get_blob_progress_chunked_parallel(self, **kwargs):
        ...
        # Act
        stream = await blob.download_blob(
            max_concurrency=3, progress_hook=progress.assert_progress
        )
        await stream.readall()
    ...
    async def test_get_blob_into_upload(self, **kwargs):
        ...
        blob2 = self.bsc.get_blob_client(
            self.container_name, self._get_blob_reference() + "-copy"
        )
    ...
    async def test_get_blob_read_progress_chars(self, **kwargs):
        ...
        progress = CustomProgressTracker()
        stream = await blob.download_blob(
            encoding="utf-8", progress_hook=progress.assert_progress
        )
    ...
    async def test_get_blob_read_chars_ranged(self, **kwargs):
        ...
        stream = await blob.download_blob(
            offset=offset, length=length, encoding="utf-8"
        )
