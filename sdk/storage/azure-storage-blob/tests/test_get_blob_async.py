...
class TestStorageGetBlobTest(AsyncStorageRecordedTestCase):
    ...
    async def test_get_blob_to_text_with_progress(self, **kwargs):
        ...
        # Act
        stream = await blob.download_blob(raw_response_hook=callback, max_concurrency=2, encoding="UTF-8")
        content = await stream.readall()
    ...
    async def test_get_blob_to_text_non_parallel(self, **kwargs):
        ...
        # Act
        stream = await blob.download_blob(raw_response_hook=callback, max_concurrency=1, encoding="UTF-8")
        content = await stream.readall()
