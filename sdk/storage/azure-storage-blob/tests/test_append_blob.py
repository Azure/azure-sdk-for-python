...
class TestStorageAppendBlob(StorageRecordedTestCase):
    ...
    def test_create_blob(self, **kwargs):
        ...
        bsc = BlobServiceClient(
            self.account_url(storage_account_name, "blob"), storage_account_key.secret, max_block_size=4 * 1024
        )
        self._setup(bsc)
        blob_name = self._get_blob_reference()
    ...
    def test_append_block_high_throughput(self, **kwargs):
        ...
        bsc = BlobServiceClient(
            self.account_url(storage_account_name, "blob"), storage_account_key.secret, max_block_size=100 * 1024 * 1024
        )
        self._setup(bsc)
        blob = self._create_blob(bsc)
    ...
    def test_create_append_blob_with_no_overwrite(self, **kwargs):
        ...
        # Act
        blob.upload_blob(data1, overwrite=True, blob_type=BlobType.AppendBlob, metadata={"blobdata": "Data1"})
