...
class TestStorageAppendBlob(StorageRecordedTestCase):
    ...
    def _get_bearer_token_string(
        ...
        return (
            "Bearer "
            + f"{self.get_credential(BlobServiceClient).get_token(resource).token}"
        )
    ...
    @BlobPreparer()
    @recorded_by_proxy
    def test_append_block_high_throughput(self, **kwargs):
        storage_account_name = kwargs.pop("storage_account_name")
        storage_account_key = kwargs.pop("storage_account_key")
        ...
        # Assert
        self.assertBlobEqual(blob, data * 2)
    ...
    def test_append_block_from_url_with_oauth(self, **kwargs):
        ...
        destination_blob_client.append_block_from_url(
            source_blob_client.url, source_authorization=token
        )
    ...
    def test_append_block_from_url(self, **kwargs):
        ...
        assert destination_blob_properties.get("last_modified") == resp.get(
            "last_modified"
        )
    ...
    def test_append_block_from_url_with_source_if_unmodified(self, **kwargs):
        ...
        with pytest.raises(ResourceModifiedError):
            destination_blob_client.append_block_from_url(
                ...
                source_offset=0,
                source_length=LARGE_BLOB_SIZE,
                if_unmodified_since=source_blob_properties.get("last_modified")
                - timedelta(hours=15),
            )
    ...
    def test_append_block_from_url_with_if_match(self, **kwargs):
        ...
        destination_blob_properties_on_creation = (
    ...
    def test_app_blob_from_stream_nonseekable_chnked_upload_known_size(self, **kwargs):
        ...
        with tempfile.TemporaryFile() as temp_file:
            ...
            blob.upload_blob(
                non_seekable_file, length=blob_size, blob_type=BlobType.AppendBlob
            )
    ...
    def test_append_blob_from_stream_chunked_upload_with_count_parallel(self, **kwargs):
        ...
        with tempfile.TemporaryFile() as temp_file:
            temp_file.write(data)
            temp_file.seek(0)
            append_resp = blob.upload_blob(
                temp_file, length=blob_size, blob_type=BlobType.AppendBlob
            )
        blob_properties = blob.get_blob_properties()
    ...
    def test_create_append_blob_with_immutability_policy(self, **kwargs):
        ...
        expiry_time = self.get_datetime_variable(
            variables, "expiry_time", datetime.utcnow() + timedelta(seconds=10)
        )
        immutability_policy = ImmutabilityPolicy(
            expiry_time=expiry_time, policy_mode=BlobImmutabilityPolicyMode.Unlocked
        )
        blob.create_append_blob(
            immutability_policy=immutability_policy, legal_hold=True
        )
    ...
    def test_append_block_from_file_to_blob_with_oauth(self, **kwargs):
        ...
        bsc = BlobServiceClient(account_url, storage_account_key.secret)
        self._setup(bsc)
