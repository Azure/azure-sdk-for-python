...
import sys
from typing import (
    Any, AnyStr, Callable, Dict, List, IO, Iterable, Iterator, Optional, overload, Union,
    TYPE_CHECKING
)
...
def _get_blob_name(blob: Union[BlobProperties, str]) -> str:
...
class ContainerClient(StorageAccountHostsMixin, StorageEncryptionMixin):    # pylint: disable=too-many-public-methods
    ...
    def delete_blobs(
        ...
        **kwargs: Any
        ...
        :keyword delete_snapshots:
            Required if a blob has associated snapshots. Values include:
             - "only": Deletes only the blobs snapshots.
             - "include": Deletes the blob along with all snapshots.
        :paramtype delete_snapshots: Literal['only', 'include']
        ...
        reqs, options = self._generate_delete_blobs_options(
            *blobs,
            delete_snapshots=delete_snapshots,
    ...
    def set_standard_blob_tier_blobs(
        ...
        **kwargs: Any
        ...
        :rtype: Iterator[~azure.core.pipeline.transport.HttpResponse]
        """
        reqs, options = self._generate_set_tiers_options(
            standard_blob_tier,
            *blobs,
