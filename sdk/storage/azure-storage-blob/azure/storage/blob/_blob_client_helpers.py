...
def _upload_blob_options(  # pylint:disable=too-many-statements
    ...
    **kwargs: Any,
    ...
    if cpk:
        kwargs["encryption_key"] = cpk.key_value
        kwargs["encryption_key_sha256"] = cpk.key_hash
        kwargs["encryption_algorithm"] = cpk.algorithm
...
def _upload_blob_from_url_options(source_url: str, **kwargs: Any) -> Dict[str, Any]:
    ...
    if source_cpk:
        kwargs["source_encryption_key"] = source_cpk.key_value
        kwargs["source_encryption_key_sha256"] = source_cpk.key_hash
        kwargs["source_encryption_algorithm"] = source_cpk.algorithm
...
def _download_blob_options(
    ...
    **kwargs,
    ...
    if cpk:
        ...
        options["encryption_key_sha256"] = cpk.key_hash
        options["encryption_algorithm"] = cpk.algorithm
    options.update(kwargs)
    return options
...
def _set_http_headers_options(content_settings: Optional["ContentSettings"] = None, **kwargs: Any) -> Dict[str, Any]:
    ...
    if content_settings:
        options["blob_cache_control"] = content_settings.cache_control
        options["blob_content_type"] = content_settings.content_type
        options["blob_content_md5"] = content_settings.content_md5
...
def _create_append_blob_options(
    content_settings: Optional["ContentSettings"] = None, metadata: Optional[Dict[str, str]] = None, **kwargs: Any
) -> Dict[str, Any]:
    headers = kwargs.pop("headers", {})
    headers.update(add_metadata_headers(metadata))
    access_conditions = get_access_conditions(kwargs.pop("lease", None))
...
def _start_copy_from_url_options(  # pylint:disable=too-many-statements
    source_url: str, metadata: Optional[Dict[str, str]] = None, incremental_copy: bool = False, **kwargs: Any
) -> Dict[str, Any]:
    source_url = _encode_source_url(source_url=source_url)
    headers = kwargs.pop("headers", {})
    headers.update(add_metadata_headers(metadata))
    ...
    if requires_sync is True:
        ...
        if source_token_intent:
            raise ValueError("Source token intent is only supported for sync copy, please specify requires_sync=True")
        if copy_source_tags:
            raise ValueError("Copying source tags is only supported for sync copy, please specify requires_sync=True")
...
def _stage_block_options(
    block_id: str, data: Union[bytes, str, Iterable[AnyStr], IO[AnyStr]], length: Optional[int] = None, **kwargs: Any
) -> Dict[str, Any]:
...
def _stage_block_from_url_options(
    ...
    **kwargs: Any,
    ...
    if cpk:
        ...
        options["encryption_algorithm"] = cpk.algorithm
    if source_cpk:
        options["source_encryption_key"] = source_cpk.key_value
        options["source_encryption_key_sha256"] = source_cpk.key_hash
        options["source_encryption_algorithm"] = source_cpk.algorithm
    options.update(kwargs)
    return options
...
def _upload_page_options(page: bytes, offset: int, length: int, **kwargs: Any) -> Dict[str, Any]:
...
def _upload_pages_from_url_options(
    ...
    cpk_info_kwargs = {}
    if cpk:
        cpk_info_kwargs["encryption_key"] = cpk.key_value
        cpk_info_kwargs["encryption_key_sha256"] = cpk.key_hash
        cpk_info_kwargs["encryption_algorithm"] = cpk.algorithm
    source_cpk = kwargs.pop("source_cpk", None)
    source_cpk_info_kwargs = {}
    if source_cpk:
        source_cpk_info_kwargs["source_encryption_key"] = source_cpk.key_value
        source_cpk_info_kwargs["source_encryption_key_sha256"] = source_cpk.key_hash
        source_cpk_info_kwargs["source_encryption_algorithm"] = source_cpk.algorithm
...
def _append_block_options(
    data: Union[bytes, str, Iterable[AnyStr], IO[AnyStr]], length: Optional[int] = None, **kwargs: Any
) -> Dict[str, Any]:
    if isinstance(data, str):
    ...
    if maxsize_condition or appendpos_condition is not None:
        ...
        append_conditions_kwargs["append_position"] = appendpos_condition
    cpk = kwargs.pop("cpk", None)
    cpk_info_kwargs = {}
    if cpk:
        cpk_info_kwargs["encryption_key"] = cpk.key_value
...
def _append_block_from_url_options(
    copy_source_url: str, source_offset: Optional[int] = None, source_length: Optional[int] = None, **kwargs: Any
) -> Dict[str, Any]:
