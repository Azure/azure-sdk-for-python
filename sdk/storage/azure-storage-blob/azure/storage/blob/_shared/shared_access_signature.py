...
from .utils import _sign_string, url_quote
...
class SharedAccessSignature(object):
    ...
    def __init__(self, account_name, account_key, user_delegation_key=None, x_ms_version=X_MS_VERSION):
        '''
...
class ResourceSharedAccessSignature(SharedAccessSignature):
    '''
    ...
    def __init__(self, account_name, account_key=None, user_delegation_key=None):
        ...
        '''
        super(ResourceSharedAccessSignature, self).__init__(account_name, account_key, x_ms_version=X_MS_VERSION)
        self.user_delegation_key = user_delegation_key
    ...
    def generate_blob(self, container_name, blob_name, snapshot=None, permission=None,
        ...
        sas = _ResourceSharedAccessHelper()
        sas.add_base(permission, expiry, start, ip, protocol, self.x_ms_version)
...
class _SharedAccessHelper(object):
    ...
    def add_id(self, policy_id):
        self._add_query(_QueryStringConstants.SIGNED_IDENTIFIER, policy_id)
...
class _ResourceSharedAccessHelper(_SharedAccessHelper):
    def __init__(self):
        super(_ResourceSharedAccessHelper, self).__init__()

    def add_timestamp(self, timestamp):
        self._add_query(_QueryStringConstants.SIGNED_TIMESTAMP, timestamp)
    ...
    def add_resource_signature(self, account_name, account_key, path, user_delegation_key=None):
        ...
        string_to_sign = \
            (self.get_value_to_append(_QueryStringConstants.SIGNED_PERMISSION) +
             self.get_value_to_append(_QueryStringConstants.SIGNED_START) +
             self.get_value_to_append(_QueryStringConstants.SIGNED_EXPIRY) +
             canonicalized_resource)

        if user_delegation_key is not None:
            self._add_query(_QueryStringConstants.SIGNED_OID, user_delegation_key.signed_oid)
            self._add_query(_QueryStringConstants.SIGNED_TID, user_delegation_key.signed_tid)
            self._add_query(_QueryStringConstants.SIGNED_KEY_START, user_delegation_key.signed_start)
            self._add_query(_QueryStringConstants.SIGNED_KEY_EXPIRY, user_delegation_key.signed_expiry)
            self._add_query(_QueryStringConstants.SIGNED_KEY_SERVICE, user_delegation_key.signed_service)
            self._add_query(_QueryStringConstants.SIGNED_KEY_VERSION, user_delegation_key.signed_version)

            string_to_sign += \
                (self.get_value_to_append(_QueryStringConstants.SIGNED_OID) +
                 self.get_value_to_append(_QueryStringConstants.SIGNED_TID) +
                 self.get_value_to_append(_QueryStringConstants.SIGNED_KEY_START) +
                 self.get_value_to_append(_QueryStringConstants.SIGNED_KEY_EXPIRY) +
                 self.get_value_to_append(_QueryStringConstants.SIGNED_KEY_SERVICE) +
                 self.get_value_to_append(_QueryStringConstants.SIGNED_KEY_VERSION))
        else:
            string_to_sign += self.get_value_to_append(_QueryStringConstants.SIGNED_IDENTIFIER)

        string_to_sign += \
            (self.get_value_to_append(_QueryStringConstants.SIGNED_IP) +
             self.get_value_to_append(_QueryStringConstants.SIGNED_PROTOCOL) +
             self.get_value_to_append(_QueryStringConstants.SIGNED_VERSION) +
             self.get_value_to_append(_QueryStringConstants.SIGNED_RESOURCE) +
             self.get_value_to_append(_QueryStringConstants.SIGNED_TIMESTAMP) +
             self.get_value_to_append(_QueryStringConstants.SIGNED_CACHE_CONTROL) +
             self.get_value_to_append(_QueryStringConstants.SIGNED_CONTENT_DISPOSITION) +
             self.get_value_to_append(_QueryStringConstants.SIGNED_CONTENT_ENCODING) +
             self.get_value_to_append(_QueryStringConstants.SIGNED_CONTENT_LANGUAGE) +
             self.get_value_to_append(_QueryStringConstants.SIGNED_CONTENT_TYPE))
    ...
    def get_token(self):
        ...
        exclude = [_QueryStringConstants.SIGNED_TIMESTAMP]
