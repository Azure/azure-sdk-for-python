...
import functools
import warnings
from typing import Any, cast, Dict, Iterable, List, Optional, Union, TYPE_CHECKING
...
class BlobServiceClient(  # type: ignore [misc]
    ...
    :keyword bool use_session: Whether to authenticate eligible requests with a short-lived,
        per-container session credential instead of the bearer token. Requires a TokenCredential.
    ...
    :paramtype session_provider: ~azure.storage.blob.aio.AsyncSessionProvider
    ...
    async def undelete_container(
        self, deleted_container_name: str, deleted_container_version: str, **kwargs: Any
    ) -> ContainerClient:
