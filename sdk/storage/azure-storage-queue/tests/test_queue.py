...
class TestStorageQueue(StorageRecordedTestCase):
    ...
    def test_create_queue_fail_on_exist_different_metadata(self, **kwargs):
        ...
        qsc = QueueServiceClient(url, storage_account_key.secret)
        queue_client = self._get_queue_reference(qsc)
        created = queue_client.create_queue()
        with pytest.raises(ResourceExistsError):
    ...
    def test_get_user_delegation_sas(self, **kwargs):
        ...
        user_delegation_key_1 = service.get_user_delegation_key(
            start=start, expiry=expiry
        )
        user_delegation_key_2 = service.get_user_delegation_key(
            start=start, expiry=expiry
        )
