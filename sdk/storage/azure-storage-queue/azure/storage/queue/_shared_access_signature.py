...
def generate_account_sas(
    ...
    **kwargs: Any
    ...
    :type resource_types: Union[~azure.storage.queue.ResourceTypes, str]
    :param permission:
        The permissions associated with the shared access signature. The
        user is restricted to operations allowed by the permissions.
    :type permission: Union[~azure.storage.queue.AccountSasPermissions, str]
    :param expiry:
        The time at which the shared access signature becomes invalid.
        Azure will always convert values to UTC. If a date is passed in
        without timezone info, it is assumed to be UTC.
    :type expiry: Union[~datetime.datetime, str]
    :param start:
        The time at which the shared access signature becomes valid. If
        omitted, start time for this call is assumed to be the time when the
        storage service receives the request. Azure will always convert values
