# coding=utf-8
# --------------------------------------------------------------------------
# Copyright (c) Microsoft Corporation. All rights reserved.
# Licensed under the MIT License. See License.txt in the project root for license information.
# Code generated by Microsoft (R) Python Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is regenerated.
# --------------------------------------------------------------------------

from azure.identity import DefaultAzureCredential

from azure.mgmt.marketplace import MarketplaceClient

"""
# PREREQUISITES
    pip install azure-identity
    pip install azure-mgmt-marketplace
# USAGE
    python create_approval_request.py

    Before run the sample, please set the values of the client ID, tenant ID and client secret
    of the AAD application as environment variables: AZURE_CLIENT_ID, AZURE_TENANT_ID,
    AZURE_CLIENT_SECRET. For more info about how to get the value, please see:
    https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal
"""


def main():
    client = MarketplaceClient(
        credential=DefaultAzureCredential(),
    )

    response = client.private_store.create_approval_request(
        private_store_id="a0e28e55-90c4-41d8-8e34-bb7ef7775406",
        request_approval_id="marketplacetestthirdparty.md-test-third-party-2",
        payload={
            "properties": {
                "plansDetails": [
                    {
                        "justification": "Because I want to....",
                        "planId": "testPlanA",
                        "subscriptionId": "4ca4753c-5a1e-4913-b849-2c68880e03c2",
                        "subscriptionName": "Test subscription 2",
                    },
                    {
                        "justification": "try me :)",
                        "planId": "*",
                        "subscriptionId": "4ca4753c-5a1e-4913-b849-2c68880e03c2",
                        "subscriptionName": "Test subscription 2",
                    },
                ],
                "publisherId": "marketplacetestthirdparty",
            }
        },
    )
    print(response)


# x-ms-original-file: 2025-01-01/CreateApprovalRequest.json
if __name__ == "__main__":
    main()
