...
class TestDevCenterClientAsync(AzureRecordedTestCase):
    ...
    async def test_list_projects_async(self, **kwargs):
        ...
        project_list = []
        list_projects_response = client.list_projects()

        async for project in list_projects_response:
    ...
    async def test_create_dev_box_async(self, **kwargs):
        ...
        # Create DevBox
        async with client:
            create_devbox_poller = await client.begin_create_dev_box(project_name, user, devbox_name, {"poolName": pool_name})
            devbox_result = await create_devbox_poller.result()
