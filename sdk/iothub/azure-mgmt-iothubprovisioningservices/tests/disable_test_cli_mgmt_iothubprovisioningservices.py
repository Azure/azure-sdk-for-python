...
class MgmtIotDpsClientTest(AzureMgmtTestCase):
    ...
    def setUp(self):
        ...
        self.mgmt_client = self.create_mgmt_client(azure.mgmt.iothubprovisioningservices.IotDpsClient)
    ...
    def test_iothubprovisioningservices(self, resource_group):
        ...
        result = self.mgmt_client.iot_dps_resource.create_or_update(
        ...
        result = self.mgmt_client.dps_certificate.list(
            resource_group_name=RESOURCE_GROUP, provisioning_service_name=PROVISIONING_SERVICE_NAME
        )

        # /IotDpsResource/get/DPSGetValidSku[get]
        result = self.mgmt_client.iot_dps_resource.list_valid_skus(
            resource_group_name=RESOURCE_GROUP, provisioning_service_name=PROVISIONING_SERVICE_NAME
        )

        # /IotDpsResource/get/DPSGet[get]
        result = self.mgmt_client.iot_dps_resource.get(
            resource_group_name=RESOURCE_GROUP, provisioning_service_name=PROVISIONING_SERVICE_NAME
        )
        ...
        result = self.mgmt_client.iot_dps_resource.list_keys(
            resource_group_name=RESOURCE_GROUP, provisioning_service_name=PROVISIONING_SERVICE_NAME
        )

        # /IotDpsResource/patch/DPSPatch[patch]
        BODY = {"tags": {"foo": "bar"}}
        result = self.mgmt_client.iot_dps_resource.update(
            resource_group_name=RESOURCE_GROUP,
            provisioning_service_name=PROVISIONING_SERVICE_NAME,
            provisioning_service_tags=BODY,
        )
        result = result.result()

        # /IotDpsResource/post/DPSCheckName[post]
        BODY = {"name": "test213123"}
        ...
        result = self.mgmt_client.iot_dps_resource.delete(
            resource_group_name=RESOURCE_GROUP, provisioning_service_name=PROVISIONING_SERVICE_NAME
        )
