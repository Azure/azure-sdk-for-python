...
class TestIndividualEnrollments(AzureRecordedTestCase):
    ...
    def test_default_credentials(self, iothub_dps_endpoint):
        ...
        enrollments = client.enrollment.query(query_specification={"query": "SELECT *"})
        with pytest.raises(StopIteration):
            enrollments.next()

    @ProvisioningServicePreparer()
    @recorded_by_proxy
    def test_named_key(self, iothub_dps_endpoint, iothub_dps_conn_str):
    ...
    @ProvisioningServicePreparer()
    @recorded_by_proxy
    def test_sas_creds(self, iothub_dps_endpoint, iothub_dps_conn_str):
        cs_parts = parse_connection_string(iothub_dps_conn_str)
