...
class UtilsTest(unittest.TestCase):
    ...
    def get_token_with_custom_expiry(expires_on):
        ...
        token_template = (
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9." + base64expiry + ".adM-ddBZZlQ1WlN3pdPBOF5G4Wh9iZpxNP_fSvpF4cWs"
        )
    ...
    def test_convert_datetime_to_utc_int(self):
        ...
        pst_time_in_sec = _convert_datetime_to_utc_int(
            datetime(1970, 1, 1, 0, 0, 0, 0, tzinfo=ZoneInfo("America/Vancouver"))
        )
        assert pst_time_in_sec == 8 * 3600
        # EST is UTC-5
        est_time_in_sec = _convert_datetime_to_utc_int(
            datetime(1970, 1, 1, 0, 0, 0, 0, tzinfo=ZoneInfo("America/New_York"))
        )
        assert est_time_in_sec == 5 * 3600
        # CST is UTC+8
        cst_time_in_sec = _convert_datetime_to_utc_int(
            datetime(1970, 1, 1, 0, 0, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))
        )
    ...
    def test_access_token_expiry_deserialized_correctly_from_payload(self):
        ...
        token = create_access_token(self.get_token_with_custom_expiry(token_expiry))
