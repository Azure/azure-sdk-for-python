...
class MmsClient(object):  # pylint: disable=client-accepts-api-version-keyword
    """A client to interact with the AzureCommunicationService Mms gateway asynchronously.

    This client provides operations to send an MMS via a phone number.
   ...
   :param str endpoint:
    ...
    async def send(self, from_,  # type: str
        ...
        return await self._sms_service_client.mms.send(
            request,
            cls=lambda pr, r, e: [
                MmsSendResult(
                    to=item.to,
    ...
    async def __aenter__(self) -> "MMSClient":
    ...
    async def close(self) -> None:
        """Close the :class:
        `~azure.communication.sms.aio.MmsClient` session.
        """
