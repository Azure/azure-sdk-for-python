...
import pytest
from _router_test_case_async import RouterTestAsyncCase
...
class DistributionPolicyAsyncTest(RouterTestAsyncCase):
    def __init__(self, method_name):
        super(DistributionPolicyAsyncTest, self).__init__(method_name)

    def setUp(self):
        super(DistributionPolicyAsyncTest, self).setUp()
    ...
    async def tearDown(self):
        super(DistributionPolicyAsyncTest, self).tearDown()

        # delete in live mode
        if not self.is_playback():
            if any(self.policy_ids):
                async with self.router_client:
                    for policy_id in self.policy_ids:
                        await self.router_client.delete_distribution_policy(identifier = policy_id)
    ...
    async def test_create_distribution_policy(self):
        ...
        async with self.router_client:
            for mode in distribution_modes:
                distribution_policy_response = await self.router_client.upsert_distribution_policy(
    ...
    async def test_update_distribution_policy(self):
        ...
        async with self.router_client:
            for mode in distribution_modes:
                ...
                updated_distribution_policy = await self.router_client.upsert_distribution_policy(
                    identifier = dp_identifier,
                    distribution_policy = distribution_policy_response
                )
    ...
    async def test_get_distribution_policy(self):
        ...
        async with self.router_client:
            for mode in distribution_modes:
                ...
                queried_distribution_policy = await self.router_client.get_distribution_policy(identifier = dp_identifier)
                validate_distribution_policy(
    ...
    async def test_delete_distribution_policy(self):
        ...
        async with self.router_client:
            for mode in distribution_modes:
                ...
                await self.router_client.delete_distribution_policy(identifier = dp_identifier)
                with pytest.raises(ResourceNotFoundError) as nfe:
                    await self.router_client.get_distribution_policy(identifier = dp_identifier)
