...
async def test_create_connection():
    calling_server_client = _test_utils.create_mock_calling_server_client(status_code=201, payload={
...
async def test_get_call_connection():
    calling_server_client = _test_utils.create_calling_server_client_async()
    call_connection = calling_server_client.get_call_connection(_test_constants.CALL_ID)
...
async def test_join_call():
    ...
    join_call_options = JoinCallOptions(
        ...
        requested_call_events=EventSubscriptionType.PARTICIPANTS_UPDATED)
    join_call_result = await calling_server_client.join_call(
