...
class TestCallMediaClientAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_play_file_play_interrupt_hold_audio(self):
        mock_play = AsyncMock()
        self.call_media_operations.play = mock_play
        play_source = FileSource(url=self.url)

        await self.call_connection_client.play_media(play_source=play_source, play_to=[self.target_user], interrupt_hold_audio=True)
    ...
    async def test_interrupt_audio_and_announce(self):
        mock_hold = AsyncMock()
        mock_interrupt_audio_and_announce = AsyncMock()
        mock_unhold = AsyncMock()
        self.call_media_operations.hold = mock_hold
        ...
        play_sources = [FileSource(url=self.url),  TextSource(text='test test test')]
        await self.call_connection_client.interrupt_audio_and_announce(target_participant=self.target_user, play_sources=play_sources)
