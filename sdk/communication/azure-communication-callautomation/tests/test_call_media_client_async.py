...
from azure.communication.callautomation.aio import CallConnectionClient
...
class TestCallMediaClientAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_play_file_to_all_back_compat_with_barge_in(self):
        ...
        await self.call_connection_client.play_media_to_all(
            play_source=play_source, interrupt_call_media_operation=True
        )
    ...
    async def test_recognize_dtmf_with_multiple_play_prompts(self):
        ...
        with pytest.raises(ValueError) as e:
            await self.call_connection_client.start_recognizing_media(
                target_participant=self.target_user, input_type="foo"
            )
    ...
    async def test_start_transcription(self):
        ...
        await self.call_connection_client.start_transcription(
    ...
    async def test_unhold(self):
        ...
        await self.call_connection_client.unhold(
            target_participant=self.target_user, operation_context=operation_context
        )
    ...
    async def test_stop_media_streaming(self):
        ...
        await self.call_connection_client.stop_media_streaming(operation_callback_url=self.operation_callback_url)
