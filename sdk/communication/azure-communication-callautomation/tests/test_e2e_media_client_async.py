...
from azure.communication.callautomation._models import PiiRedactionOptions, SummarizationOptions
from azure.communication.callautomation._generated.models._enums import MediaStreamingTransportType, RedactionType
...
class TestMediaAutomatedLiveTestAsync(CallAutomationRecordedTestCaseAsync):
    ...
    @recorded_by_proxy_async
    async def test_summarize_call(self):
        # try to establish the call
        caller = await self.identity_client.create_user()
        target = await self.identity_client.create_user()
        ...
        await call_connection.summarize_call(operation_context="summarize_call_context", summarization=summarization_options)
        
        # check for TranscriptionCallSummaryUpdated event
        transcription_call_summary_updated = self.check_for_event('TranscriptionCallSummaryUpdated', call_connection._call_connection_id, timedelta(seconds=30))
        if transcription_call_summary_updated is None:
            raise ValueError("TranscriptionCallSummaryUpdated event is None")
