...
class TestCallRecordingClient(unittest.TestCase):
    ...
    def test_start_recording_byos(self):
        def mock_send(_, **kwargs):
            kwargs.pop("stream", None)
            if kwargs:
                raise ValueError(f"Received unexpected kwargs in transport: {kwargs}")
            return mock_response(status_code=200, json_payload={
                "recording_id": "withblobstorage",
                "recording_state": "2"
            })
        ...
        target_participant = CommunicationUserIdentifier("testId")
        channel_affinity=ChannelAffinity(target_participant=target_participant, channel=0)
        recording_storage = AzureBlobContainerRecordingStorage(container_url="https://dummyurl")
        callautomation_client.start_recording(call_locator=call_locator, channel_affinity=[channel_affinity], recording_storage=recording_storage)
        callautomation_client.start_recording(call_locator=call_locator, channel_affinity=[channel_affinity])
        with pytest.raises(ValueError):
            recording_storage = AzureBlobContainerRecordingStorage("")
            callautomation_client.start_recording(recording_storage=recording_storage)