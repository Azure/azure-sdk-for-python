...
class TestCallRecordingClientAsync(unittest.IsolatedAsyncioTestCase):
    recording_id = "123"
    ...
    async def test_start_recording_with_call_connection_id(self):
        async def mock_send(_, **kwargs):
            kwargs.pop("stream", None)
            if kwargs:
                raise ValueError(f"Received unexpected kwargs in transport: {kwargs}")
        ...
        target_participant = CommunicationUserIdentifier("testId")
        channel_affinity = ChannelAffinity(target_participant=target_participant, channel=0)
        await callautomation_client.start_recording(call_connection_id=self.call_connection_id, channel_affinity=[channel_affinity])