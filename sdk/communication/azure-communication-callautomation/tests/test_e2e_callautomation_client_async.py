...
class TestCallAutomationClientAutomatedLiveTestAsync(CallAutomationRecordedTestCaseAsync):
    ...
    async def test_create_VOIP_call_and_answer_custom_context_async(self):
        ...
        caller = self.identity_client.create_user()
        target = self.identity_client.create_user()
        unique_id, call_connection, _ = await self.establish_callconnection_voip_answercall_withcustomcontext(caller, target)
    ...
    async def test_start_recording_with_call_connection_id_async(self):
        ...
        start_recording = await call_automation_client.start_recording(call_connection_id=call_connection_id, recording_state_callback_url=callback_url, channel_affinity=[channel_affinity]
            )
    ...
    async def test_start_recording_with_server_call_id_async(self):
        ...
        # start recording request with call connection id.
        start_recording = await call_automation_client.start_recording(server_call_id = server_call_id, recording_state_callback_url=callback_url, channel_affinity=[channel_affinity]
            )
        await asyncio.sleep(5)
