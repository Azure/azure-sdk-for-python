...
@pytest.mark.skip(reason="Skipping automated live async tests for now")
class TestCallAutomationClientAutomatedLiveTestAsync(CallAutomationRecordedTestCaseAsync):
    ...
    async def test_add_participant_then_cancel_request_async(self):
        ...
        connected_event = self.check_for_event_async(
            "CallConnected", call_connection_id, timedelta(seconds=15)
        )
        participant_updated_event = self.check_for_event_async(
        ...
        cancel_add_participant_succeeded_event = self.check_for_event_async(
    ...
    async def test_create_VOIP_call_and_connect_call_then_hangup_async(self):
        ...
        # check returned events
        connected_event = self.check_for_event_async('CallConnected', call_connection._call_connection_id, timedelta(seconds=15))
        participant_updated_event = self.check_for_event_async('ParticipantsUpdated', call_connection._call_connection_id, timedelta(seconds=15))
        ...
        connect_call_connected_event = self.check_for_event_async('CallConnected', connect_call_connection.call_connection_id, timedelta(seconds=20))
    ...
    async def test_start_recording_with_call_connection_id_async(self):
        ...
        recording_state_changed_event = self.check_for_event_async('RecordingStateChanged', call_connection_id, timedelta(seconds=30))
