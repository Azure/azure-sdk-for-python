...
request = {"description": "Discovery group made using the Defender EASM SDK sample", "seeds": assets}
...
for group in client.discovery_groups.list():
    ...
    for run in itertools.islice(runs, 5):
        print(
            f" - started: {run['startedDate']}, finished: {run['completedDate']}, assets found: {run['totalAssetsFoundCount']}"
        )
