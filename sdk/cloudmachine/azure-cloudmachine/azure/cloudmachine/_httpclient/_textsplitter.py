...
class TextSplitter(ABC):
    ...
    def __call__(self, pages: List[Tuple[int, int, str]]) -> Generator[Tuple[int, str], None, None]:
        if False:
...
class SentenceTextSplitter(TextSplitter):
    ...
    def split_page_by_max_tokens(self, page_num: int, text: str) -> Generator[Tuple[int, str], None, None]:
        """
    ...
    def __call__(self, pages: List[Tuple[int, int, str]]) -> Generator[Tuple[int, str], None, None]:
        ...
        def find_page(offset):
            for i in range(num_pages - 1):
                if offset >= pages[i][1] and offset < pages[i + 1][1]:
                    return pages[i][0]
            return pages[num_pages - 1][0]

        all_text = "".join(page[2] for page in pages)
...
class SimpleTextSplitter(TextSplitter):
    ...
    def __call__(self, pages: List[Tuple[int, int, str]]) -> Generator[SplitPage, None, None]:
        all_text = "".join(page[2] for page in pages)
        if len(all_text.strip()) == 0:
            return
