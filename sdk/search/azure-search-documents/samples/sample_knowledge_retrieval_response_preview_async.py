...
"""
DESCRIPTION:
    Demonstrates preview retrieval request and response options using async clients.
...
async def main():
    ...
    # [START sample_knowledge_retrieval_response_preview_async]
    ...
    async with index_client:
        try:
            ...
            try:
                ...
                semantic_result = await retrieval_client.retrieve(semantic_request)
                print_retrieval_summary(semantic_result)

                stream_event_types = []
                stream_request_id = None
                stream = await retrieval_client.retrieve_stream(semantic_request)
                async with stream:
                ...
                message_result = await retrieval_client.retrieve(message_request)
                ...
                no_rerank_result = await retrieval_client.retrieve(no_rerank_request)
                assert all(reference.reranker_score is None for reference in no_rerank_result.references or [])
            finally:
                await retrieval_client.close()
            # [END sample_knowledge_retrieval_response_preview_async]
        finally:
