...
# ------------------------------------
"""Async unit tests for ``SearchClient`` patched public behavior."""
...
class TestSearchClientConstructorAsync:
...
class TestSearchRequestBuildingAsync:
    ...
    @pytest.mark.asyncio
    @mock.patch("azure.search.documents.aio._operations._operations._SearchClientOperationsMixin._search_post")
    async def test_search_passes_query_language_speller_rewrites_semantic_fields_and_hybrid_search(
    ...
    @pytest.mark.asyncio
    @mock.patch("azure.search.documents.aio._operations._operations._SearchClientOperationsMixin._search_post")
    async def test_search_converts_atsearch_metadata_keys_on_each_result(self, mock_search_post):
    ...
    async def test_search_metadata_accessors_fetch_first_page_once_and_clear_continuation(self, mock_search_post):
        ...
        async with create_search_client() as client:
            ...
            assert await result.get_count() == 12
            result._first_page_iterator_instance.continuation_token = CONTINUATION_TOKEN
            assert await result.get_coverage() == 98.5
            assert await result.get_answers() == search_result.answers
            assert await result.get_facets() == {"Category": [{"count": 4}]}
    ...
    async def test_get_debug_info_returns_response_debug_info(self, mock_search_post):
        ...
        async with create_search_client() as client:
            ...
            assert await result.get_debug_info() is debug_info
...
class TestSearchSuggestionRequestsAsync:
    @pytest.mark.asyncio
    @mock.patch("azure.search.documents.aio._operations._operations._SearchClientOperationsMixin._suggest_post")
    async def test_suggest_forwards_public_arguments_to_generated_operation(self, mock_suggest_post):
        ...
        mock_suggest_post.assert_awaited_once_with(
            search_text=SEARCH_TEXT,
            suggester_name=SUGGESTER_NAME,
            filter="Rating ge 4",
            use_fuzzy_matching=True,
    ...
    @pytest.mark.asyncio
    async def test_suggest_requires_suggester_name(self):
        async with create_search_client() as client:
            with pytest.raises(TypeError):
                await client.suggest(QUERY_WITHOUT_SUGGESTER)

    @pytest.mark.asyncio
    @mock.patch("azure.search.documents.aio._operations._operations._SearchClientOperationsMixin._autocomplete_post")
    async def test_autocomplete_forwards_public_arguments_to_generated_operation(self, mock_autocomplete_post):
        ...
        assert results == expected
        mock_autocomplete_post.assert_awaited_once_with(
            search_text=SEARCH_TEXT,
            suggester_name=SUGGESTER_NAME,
    ...
    @pytest.mark.asyncio
    async def test_autocomplete_requires_suggester_name(self):
        async with create_search_client() as client:
            with pytest.raises(TypeError):
                await client.autocomplete(QUERY_WITHOUT_SUGGESTER)


class TestSearchDocumentIndexingAsync:
    @pytest.mark.asyncio
    @pytest.mark.parametrize("method_name", DOCUMENT_METHODS)
    async def test_document_action_helpers_create_batch_and_forward_options(self, method_name):
        documents = [
            {"HotelId": DOCUMENT_KEY, "HotelName": "Northwind Lodge"},
            {"HotelId": REPLACEMENT_DOCUMENT_KEY, "HotelName": "Contoso Suites"},
        ]
        with mock.patch.object(
            SearchClient, "index_documents", new_callable=mock.AsyncMock, return_value=[]
        ) as mock_index_documents:
            async with create_search_client() as client:
                method = getattr(client, method_name)
                result = await method(documents, request_id="request-1")

        assert result == []
        mock_index_documents.assert_awaited_once()
    ...
    @pytest.mark.asyncio
    @mock.patch("azure.search.documents.aio._operations._operations._SearchClientOperationsMixin._index")
    async def test_index_documents_maps_413_and_returns_indexing_results(self, mock_index):
        ...
        async with create_search_client() as client:
            ...
            results = await client.index_documents(batch, request_id="request-1")
    ...
    @pytest.mark.asyncio
    @mock.patch("azure.search.documents.aio._operations._operations._SearchClientOperationsMixin._index")
    async def test_index_documents_splits_batch_when_request_is_too_large(self, mock_index):
        ...
        async with create_search_client() as client:
            ...
            results = await client.index_documents(batch)
    ...
    @pytest.mark.asyncio
    @mock.patch("azure.search.documents.aio._operations._operations._SearchClientOperationsMixin._index")
    async def test_index_documents_raises_request_too_large_for_single_action(self, mock_index):
        ...
        async with create_search_client() as client:
            ...
            with pytest.raises(RequestEntityTooLargeError):
                await client.index_documents(batch)
