...
# ------------------------------------
"""Live tests for asynchronous File knowledge source operations."""
...
_FILE_CAPABILITIES = (
    "azure.search.documents.indexes.aio.SearchIndexClient.create_or_update_knowledge_source",
    "azure.search.documents.indexes.aio.SearchIndexClient.delete_knowledge_source",
    "azure.search.documents.indexes.aio.SearchIndexClient.upload_knowledge_source_file_multipart",
    "azure.search.documents.indexes.aio.SearchIndexClient.update_knowledge_source_file",
    "azure.search.documents.indexes.aio.SearchIndexClient.list_knowledge_source_files",
    "azure.search.documents.indexes.aio.SearchIndexClient.delete_knowledge_source_file",
...
class TestSearchIndexClientKnowledgeSourceFilesAsync(AzureRecordedTestCase):
    @live_test()
    async def test_multipart_upload_update_list_and_delete(
        self,
        endpoint: str,
        ...
        async with make_index_client(endpoint) as client:
            try:
                await client.create_or_update_knowledge_source(
                ...
                primary = await client.upload_knowledge_source_file_multipart(
                    source_name,
                    UploadKnowledgeSourceFileMultipartRequest(
                ...
                annex = await client.upload_knowledge_source_file_multipart(
                    source_name,
                    UploadKnowledgeSourceFileMultipartRequest(
                ...
                updated = await client.update_knowledge_source_file(
                    source_name,
                    primary_id,
                    UpdateKnowledgeSourceFileRequest(
                        metadata=primary_metadata,
                ...
                files = [
                    file
                    async for file in client.list_knowledge_source_files(
                        source_name,
                        prefix="hotels/",
                        search="hotels",
                        page_size=1,
                ...
                await client.delete_knowledge_source_file(source_name, primary_id)
                primary_id = None
                await client.delete_knowledge_source_file(source_name, annex_id)
                annex_id = None
                assert [file async for file in client.list_knowledge_source_files(source_name, prefix="hotels/")] == []
            finally:
                if primary_id is not None:
                    await safe_delete(client.delete_knowledge_source_file, source_name, primary_id)
                if annex_id is not None:
                    await safe_delete(client.delete_knowledge_source_file, source_name, annex_id)
                await safe_delete(client.delete_knowledge_source, source_name)
