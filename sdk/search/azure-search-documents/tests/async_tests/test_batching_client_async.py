...
class TestSearchBatchingClientAsync(object):
    ...
    async def test_callback_new(self):
        on_new = mock.Mock()
        async with SearchIndexDocumentBatchingClient("endpoint", "index name", CREDENTIAL, auto_flush=False, on_new=on_new) as client:
            await client.add_upload_actions(["upload1"])
    ...
    async def test_callback_error(self):
        ...
        on_error = mock.Mock()
        async with SearchIndexDocumentBatchingClient("endpoint",
                                               "index name",
                                               CREDENTIAL,
                                               auto_flush=False,
            ...
            await client.add_upload_actions({"id": 0})
            await client.flush()
