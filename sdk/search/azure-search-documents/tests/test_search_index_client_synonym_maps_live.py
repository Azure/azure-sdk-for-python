...
class TestSearchIndexClientSynonymMap(AzureRecordedTestCase):
    @live_test()
    def test_create_get_list_and_delete_synonym_map(self, endpoint):
        client = make_index_client(endpoint)
        synonym_map_name = self.get_resource_name("synonym-map-create")
        try:
            created_synonym_map = client.create_synonym_map(_build_synonym_map(synonym_map_name))

            assert created_synonym_map.name == synonym_map_name
            assert created_synonym_map.synonyms == SYNONYMS

            retrieved_synonym_map = client.get_synonym_map(synonym_map_name)
            assert retrieved_synonym_map.name == synonym_map_name
            assert retrieved_synonym_map.synonyms == SYNONYMS

            synonym_map_names = client.get_synonym_map_names()
            assert synonym_map_name in synonym_map_names
            assert synonym_map_name in {synonym_map.name for synonym_map in client.get_synonym_maps()}

            selected_synonym_maps = client.get_synonym_maps(select=["name"])
            ...
            client.delete_synonym_map(synonym_map_name)
            with pytest.raises(HttpResponseError):
                client.get_synonym_map(synonym_map_name)
        finally:
            safe_delete(client.delete_synonym_map, synonym_map_name)
            client.close()

    @live_test()
    def test_create_or_update_synonym_map_accepts_model_and_mapping(self, endpoint):
        client = make_index_client(endpoint)
        ...
        try:
            model_result = client.create_or_update_synonym_map(_build_synonym_map(model_synonym_map_name))
            ...
            model_result = client.create_or_update_synonym_map(model_result)
            assert model_result.synonyms == REPLACEMENT_SYNONYMS

            mapping = _build_synonym_map(mapping_synonym_map_name).as_dict()
            mapping_result = client.create_or_update_synonym_map(mapping)
            assert mapping_result.name == mapping_synonym_map_name
            assert mapping_result.synonyms == SYNONYMS
        finally:
            safe_delete(client.delete_synonym_map, model_synonym_map_name)
            safe_delete(client.delete_synonym_map, mapping_synonym_map_name)
            client.close()

    @live_test()
    def test_create_or_update_synonym_map_uses_model_etag_and_match_condition(self, endpoint):
        client = make_index_client(endpoint)
        synonym_map_name = self.get_resource_name("synonym-map-create-or-update-match-condition")
        try:
            synonym_map = client.create_synonym_map(_build_synonym_map(synonym_map_name))
            original_e_tag = synonym_map.e_tag

            client.create_or_update_synonym_map(_build_synonym_map(synonym_map_name, synonyms=REPLACEMENT_SYNONYMS))

            synonym_map.e_tag = original_e_tag
            synonym_map.synonyms = SYNONYMS
            with pytest.raises(HttpResponseError):
                client.create_or_update_synonym_map(synonym_map, match_condition=MatchConditions.IfNotModified)
        finally:
            safe_delete(client.delete_synonym_map, synonym_map_name)
            client.close()

    @live_test()
    def test_delete_synonym_map_accepts_model_etag_and_match_condition(self, endpoint):
        client = make_index_client(endpoint)
        ...
        try:
            ...
            synonym_map.e_tag = original_e_tag
            with pytest.raises(HttpResponseError):
                client.delete_synonym_map(synonym_map, match_condition=MatchConditions.IfNotModified)
        finally:
            safe_delete(client.delete_synonym_map, synonym_map_name)
            client.close()
