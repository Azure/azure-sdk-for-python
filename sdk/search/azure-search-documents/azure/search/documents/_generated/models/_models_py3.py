...
class SearchDocumentsResult(_serialization.Model):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.semantic_query_rewrites_result_type = None
...
class SearchOptions(_serialization.Model):
    ...
    :ivar captions: This parameter is only valid if the query type is ``semantic``. If set, the
     ...
     captions can be configured by appending the pipe character '|' followed by the
     'count-:code:`<number of maximum character length>`', such as 'extractive|maxcharlength-600'.
