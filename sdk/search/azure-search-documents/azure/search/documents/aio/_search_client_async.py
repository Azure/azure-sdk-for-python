...
class SearchClient(HeadersMixin):
    ...
    async def get_document_count(self, **kwargs: Any) -> int:
        ...
        return int(await self._client.documents_operations.count(index_name=self._index_name, **kwargs))
    ...
    async def get_document(self, key: str, selected_fields: Optional[List[str]] = None, **kwargs: Any) -> Dict:
        ...
        """
        kwargs["headers"] = self._merge_client_headers(kwargs.get("headers"))
        result = await self._client.documents_operations.get(index_name=self._index_name, key=key, selected_fields=selected_fields, **kwargs)
    ...
    async def suggest(
        ...
        **kwargs
        ...
        response = await self._client.documents_operations.suggest_post(index_name=self._index_name, suggest_request=request, **kwargs)
    ...
    async def autocomplete(
        ...
        **kwargs
        ...
        response = await self._client.documents_operations.autocomplete_post(index_name=self._index_name, autocomplete_request=request, **kwargs)
        assert response.results is not None  # Hint for mypy
        results = [r.as_dict() for r in response.results]
        return results
