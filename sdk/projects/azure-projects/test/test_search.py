...
def _get_outputs(suffix="", rg=None):
    outputs = {
        ...
        'resource_group': Output(f"AZURE_SEARCH_RESOURCE_GROUP{suffix.upper()}", rg if rg else DefaultResourceGroup().name),
    }
    outputs.update({
        'endpoint': Output(
...
def test_search_properties():
    ...
    assert fields['searchservice_foo'].properties == {'name': 'foo', 'tags': {'test': 'value'}, 'properties': {'publicNetworkAccess': 'Disabled'}}
    ...
    assert fields['searchservice_testa'].properties == {'name': param1, 'sku': {'name': param2}, 'properties': {'publicNetworkAccess': param3}}
...
def test_search_reference():
    ...
    assert r._settings['resource_id']() == f"/subscriptions/{TEST_SUB}/resourceGroups/bar/providers/Microsoft.Search/searchServices/foo"
...
def test_search_export_existing(export_dir):
    ...
    export(TestInfra(resource_group=ResourceGroup.reference(name='test'), identity=None), output_dir=export_dir[0], infra_dir=export_dir[2], name="test")
...
def test_search_app():
    ...
    override_client = SearchClient("https://foobar.search.windows.net/", credential=DefaultAzureCredential(), index_name="index")
    ...
    override_client = SearchIndexClient("https://foobar.search.windows.net/", credential=DefaultAzureCredential(), api_version="v2.0")
    app = TestApp(client=override_client)
    assert app.client._client._config.endpoint == "https://foobar.search.windows.net/"
