...
async def main():
    ...
    with MyFirstApp.provision() as first_app:
        container = first_app.data.create_container("samplecontainer") #get_container_client("samplecontainer")  #
        ...
        # You can also see the default configuration that the resources were deployed with.
        # Note that default resources on the auto-infrastructure share the same attribute names
        # as the app class.
        print(f"Deployment configuration: {first_app.infra.data.parent.DEFAULTS}")
    ...
    class MyStorage(AzureInfrastructure):
        blob_storage: BlobStorage = BlobStorage(sku="Standard_LRS", allow_blob_public_access=False)
