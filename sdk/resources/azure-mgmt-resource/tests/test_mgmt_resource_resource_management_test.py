...
template = {
    ...
    "parameters": {
        "location": {
            ...
            "metadata": {"description": "Location to deploy to"},
        }
    },
    "resources": [
        {
    ...
    "outputs": {
        "myparameter": {"type": "object", "value": "[reference('Microsoft.Compute/availabilitySets/availabilitySet1')]"}
    },
}
import pytest
...
class TestMgmtResource(AzureMgmtRecordedTestCase):
    ...
    def test_tag_operations(self):
        ...
        # Create or update
        tag = self.resource_client.tags.create_or_update(tag_name)
        self.assertEqual(tag.tag_name, tag_name)

        # Create or update value
        tag = self.resource_client.tags.create_or_update_value(tag_name, tag_value)
        ...
        # Delete value
        self.resource_client.tags.delete_value(tag_name, tag_value)

        # Delete tag
        self.resource_client.tags.delete(tag_name)

        # Create or update at scope
        SUBSCRIPTION_ID = self.settings.SUBSCRIPTION_ID
        SCOPE = "subscriptions/" + SUBSCRIPTION_ID
        BODY = {"properties": {"tags": {"tagKey1": "tagValue1", "tagKey2": "tagValue2"}}}
        tag = self.resource_client.tags.create_or_update_at_scope(SCOPE, BODY)

        # Get at scope
        tag = self.resource_client.tags.get_at_scope(SCOPE)

        # TODO: need example file
        # Update at scope
        BODY = {"operation": "Delete", "properties": {"tags": {"tagKey1": "tagValue1"}}}
        tag = self.resource_client.tags.update_at_scope(SCOPE, BODY)

        # TODO: need example file
        # Delete at scope
        self.resource_client.tags.delete_at_scope(SCOPE)
    ...
    def test_resource_groups(self):
        ...
        # List resources
        resources = list(self.resource_client.resources.list_by_resource_group(group_name))

        # Export template
        BODY = {"resources": ["*"]}
        template = self.resource_client.resource_groups.begin_export_template(group_name, BODY)
        template.result()
    ...
    def test_resources(self, resource_group, location):
        ...
        resource_exist = self.resource_client.resources.check_existence_by_id(resource_id, api_version="2019-10-01")

        create_result = self.resource_client.resources.begin_create_or_update_by_id(
            resource_id, parameters={"location": self.region}, api_version="2019-07-01"
        )
        ...
        get_result = self.resource_client.resources.get_by_id(resource_id, api_version="2019-07-01")

        resources = list(self.resource_client.resources.list(filter="name eq '{}'".format(resource_name)))
        self.assertEqual(len(resources), 1)

        # List resources by group
        resources = self.resource_client.resources.list_by_resource_group(resource_group.name)
        ...
        async_move = self.resource_client.resources.begin_validate_move_resources(
            resource_group.name, {"resources": [get_result.id], "target_resource_group": new_group.id}
        )
        async_move.result()
        ...
        result = self.resource_client.resources.begin_delete_by_id(new_resource_id, api_version="2019-07-01")
        result = result.result()

        async_delete = self.resource_client.resource_groups.begin_delete(new_group_name)
        async_delete.wait()
    ...
    def test_deployments_basic(self, resource_group, location):
        ...
        deployment_exists = self.resource_client.deployments.check_existence(resource_group.name, deployment_name)
        ...
        deployment_params = {"mode": "Incremental", "template": template, "parameters": parameters}
        ...
        deployment_operations = list(
            self.resource_client.deployment_operations.list(resource_group.name, deployment_name)
        )
        ...
        with self.assertRaises(azure.core.exceptions.ResourceExistsError) as cm:
            self.resource_client.deployments.cancel(resource_group.name, deployment_name)
        self.assertIn("cannot be cancelled", cm.exception.message)

        # Validate
        validation = self.resource_client.deployments.begin_validate(
            resource_group.name, deployment_name, {"properties": deployment_params}
        )
        validation = validation.result()
        self.assertTrue(hasattr(validation, "properties"))

        # Export template
        export = self.resource_client.deployments.export_template(resource_group.name, deployment_name)
        self.assertTrue(hasattr(export, "template"))

        # Delete the template
        async_delete = self.resource_client.deployments.begin_delete(resource_group.name, deployment_name)
        async_delete.wait()
    ...
    def test_deployments_at_scope(self, resource_group, location):
        ...
        SCOPE = "subscriptions/{subscriptionId}/resourcegroups/{resourceGroupName}".format(
            subscriptionId=SUBSCRIPTION_ID, resourceGroupName=resource_group.name
        )
        ...
        # Check deployment existence
        deployment_exists = self.resource_client.deployments.check_existence_at_scope(SCOPE, deployment_name)
        # # self.assertFalse(deployment_exists)
        ...
        # List deployment operations
        deployment_operations = list(self.resource_client.deployment_operations.list_at_scope(SCOPE, deployment_name))
        self.assertGreater(len(deployment_operations), 1)
        ...
        # Should throw, since the deployment is done => cannot be cancelled
        with self.assertRaises(azure.core.exceptions.ResourceExistsError) as cm:
            self.resource_client.deployments.cancel_at_scope(SCOPE, deployment_name)
        self.assertIn("cannot be cancelled", cm.exception.message)

        # Validate
        result = self.resource_client.deployments.begin_validate_at_scope(
            SCOPE, deployment_name, {"properties": deployment_params}
        )
        validation = result.result()
        self.assertTrue(hasattr(validation, "properties"))

        # Export template
        export = self.resource_client.deployments.export_template_at_scope(SCOPE, deployment_name)
        self.assertTrue(hasattr(export, "template"))

        # Delete the template
        async_delete = self.resource_client.deployments.begin_delete_at_scope(SCOPE, deployment_name)
        async_delete.wait()
    ...
    def test_deployments_at_management_group(self):
        ...
        deployment_params = {"mode": "Incremental", "template_link": template, "parameters_link": parameters}
        ...
        # List deployment operations
        deployment_operations = list(
            self.resource_client.deployment_operations.list_at_management_group_scope(group_id, deployment_name)
        )
        ...
        # Should throw, since the deployment is done => cannot be cancelled
        with self.assertRaises(azure.core.exceptions.ResourceExistsError) as cm:
            self.resource_client.deployments.cancel_at_management_group_scope(group_id, deployment_name)
        self.assertIn("cannot be cancelled", cm.exception.message)

        # Validate
        result = self.resource_client.deployments.begin_validate_at_management_group_scope(
            group_id, deployment_name, {"location": "West US", "properties": deployment_params}
        )
        validation = result.result()
        self.assertTrue(hasattr(validation, "properties"))

        # Export template
        export = self.resource_client.deployments.export_template_at_management_group_scope(group_id, deployment_name)
        self.assertTrue(hasattr(export, "template"))
    ...
    def test_deployments_at_subscription(self):
        ...
        # Check deployment existence
        deployment_exists = self.resource_client.deployments.check_existence_at_subscription_scope(deployment_name)
        # self.assertFalse(deployment_exists)
        from azure.mgmt.resource.resources.v2019_10_01 import models as models1001
        ...
        result = self.resource_client.deployments.begin_what_if_at_subscription_scope(
            deployment_name, {"location": "West US", "properties": deployment_params}
        )
        result = result.result()

        # List deployment operations
        deployment_operations = list(
            self.resource_client.deployment_operations.list_at_subscription_scope(deployment_name)
        )
        self.assertGreater(len(deployment_operations), 0)
        ...
        # Should throw, since the deployment is done => cannot be cancelled
        with self.assertRaises(azure.core.exceptions.ResourceExistsError) as cm:
            self.resource_client.deployments.cancel_at_subscription_scope(deployment_name)
        self.assertIn("cannot be cancelled", cm.exception.message)
        ...
        # Export template
        export = self.resource_client.deployments.export_template_at_subscription_scope(deployment_name)
        self.assertTrue(hasattr(export, "template"))

        # Delete the template
        async_delete = self.resource_client.deployments.begin_delete_at_subscription_scope(deployment_name)
        async_delete.wait()
    ...
    def test_deployments_at_tenant(self):
        ...
        # Check deployment existence
        deployment_exists = self.resource_client.deployments.check_existence_at_tenant_scope(deployment_name)
        ...
        # List deployment operations
        deployment_operations = list(self.resource_client.deployment_operations.list_at_tenant_scope(deployment_name))
        self.assertGreater(len(deployment_operations), 1)
        ...
        # Should throw, since the deployment is done => cannot be cancelled
        with self.assertRaises(azure.core.exceptions.ResourceExistsError) as cm:
            self.resource_client.deployments.cancel_at_tenant_scope(deployment_name)
        self.assertIn("cannot be cancelled", cm.exception.message)

        # Validate
        validation = self.resource_client.deployments.validate_at_tenant_scope(
            deployment_name, {"properties": deployment_params}
        )
        self.assertTrue(hasattr(validation, "properties"))

        # Export template
        export = self.resource_client.deployments.export_template_at_tenant_scope(deployment_name)
        self.assertTrue(hasattr(export, "template"))

        # Delete the template
        async_delete = self.resource_client.deployments.begin_delete_at_tenant_scope(deployment_name)
        async_delete.wait()
