...
class TestMicrosoftPlanetaryComputerProTilerInfoOperationsOperationsAsync(
    MicrosoftPlanetaryComputerProClientTestBaseAsync
):
    @MicrosoftPlanetaryComputerProPreparer()
    @recorded_by_proxy_async
    async def test_tiler_info_operations_get(self, microsoftplanetarycomputerpro_endpoint):
        client = self.create_async_client(endpoint=microsoftplanetarycomputerpro_endpoint)
        response = await client.tiler_info_operations.get(
            collection_id="str",
            item_id="str",
        )
