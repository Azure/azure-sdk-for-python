...
class TestMicrosoftPlanetaryComputerProMapsIntervalLegendsOperationsAsync(
    MicrosoftPlanetaryComputerProClientTestBaseAsync
):
    @MicrosoftPlanetaryComputerProPreparer()
    @recorded_by_proxy_async
    async def test_maps_interval_legends_get_by_classmap_name(self, microsoftplanetarycomputerpro_endpoint):
        client = self.create_async_client(endpoint=microsoftplanetarycomputerpro_endpoint)
        response = await client.maps_interval_legends.get_by_classmap_name(
            classmap_name="str",
        )
