...
class TestMicrosoftPlanetaryComputerProStacCollectionThumbnailsOperationsAsync(
    MicrosoftPlanetaryComputerProClientTestBaseAsync
):
    @MicrosoftPlanetaryComputerProPreparer()
    @recorded_by_proxy_async
    async def test_stac_collection_thumbnails_get(self, microsoftplanetarycomputerpro_endpoint):
        client = self.create_async_client(endpoint=microsoftplanetarycomputerpro_endpoint)
        response = await client.stac_collection_thumbnails.get(
            collection_id="str",
        )
