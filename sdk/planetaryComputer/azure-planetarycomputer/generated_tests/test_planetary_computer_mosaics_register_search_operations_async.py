...
class TestPlanetaryComputerMosaicsRegisterSearchOperationsAsync(PlanetaryComputerClientTestBaseAsync):
    @PlanetaryComputerPreparer()
    @recorded_by_proxy_async
    async def test_mosaics_register_search_register(self, planetarycomputer_endpoint):
        client = self.create_async_client(endpoint=planetarycomputer_endpoint)
        response = await client.mosaics_register_search.register(
            body={
                "bbox": 0.0,
                "collections": ["str"],
                "datetime": "str",
