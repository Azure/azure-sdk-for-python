...
class TestMicrosoftPlanetaryComputerProTilerAssetStatisticsOperationsAsync(
    MicrosoftPlanetaryComputerProClientTestBaseAsync
):
    @MicrosoftPlanetaryComputerProPreparer()
    @recorded_by_proxy_async
    async def test_tiler_asset_statistics_get_all(self, microsoftplanetarycomputerpro_endpoint):
        client = self.create_async_client(endpoint=microsoftplanetarycomputerpro_endpoint)
        response = await client.tiler_asset_statistics.get_all(
            collection_id="str",
            item_id="str",
        )
