...
"""
FILE: sample_custom_encoder_marshmallow_async.py
...
USAGE:
    python sample_custom_encoder_marshmallow_async.py
...
class Product(Schema):
    ...
    row_key = fields.UUID(required=True)
    price = fields.Float()
    last_updated = fields.DateTime()
    product_id = fields.UUID()
    inventory_count = fields.Int()
    barcode = fields.Raw()
    item_details = fields.Nested(Car)


class MyEncoder(TableEntityEncoderABC[dict]):
    def prepare_key(self, key: UUID) -> str:  # type: ignore[override]
        return super().prepare_key(str(key))

    def encode_entity(self, entity: dict) -> Dict[str, Union[str, int, float, bool]]:
        encoded = {}
...
class InsertUpdateDeleteEntity(object):
    def __init__(self):
        ...
        self.table_name = "CustomEncoderMarshmallowAsync"

        self.entity = Product().dump(
            {
                "partition_key": "PK",
                "row_key": uuid4(),
                "price": 4.99,
    ...
    async def create_delete_entity(self):
        ...
        async with table_client:
            ...
            result = await table_client.get_entity(
                self.entity["partition_key"], self.entity["row_key"], encoder=MyEncoder()
            )
    ...
    async def upsert_update_entities(self):
        ...
        async with table_client:
            ...
            inserted_entity = await table_client.get_entity(
                entity2["partition_key"], entity2["row_key"], encoder=MyEncoder()
            )
            ...
            merged_entity = await table_client.get_entity(
                entity1["partition_key"], entity1["row_key"], encoder=MyEncoder()
            )
            print(f"Merged entity: {merged_entity}")

            entity3 = Product().dump(
                {
                    "partition_key": entity1["partition_key"],
                    "row_key": entity1["row_key"],
                    "item_details": {"color": RGBColor(50.1, 60.1, 70.1)},
                }
            )
            ...
            replaced_entity = await table_client.get_entity(
                entity3["partition_key"], entity3["row_key"], encoder=MyEncoder()
            )
