...
class _TableTest(PerfStressTest):
    ...
    def __init__(self, arguments):
        ...
        endpoint = (
            self.get_from_env("AZURE_TABLES_ENDPOINT")
            if self.args.online
            else "https://127.0.0.1:10002/devstoreaccount1"
        )
        credential = (
            DefaultAzureCredential() if self.args.online else AzureNamedKeyCredential("devstoreaccount1", "fake")
        )
        async_credential = (
            AsyncAzureCredential() if self.args.online else AzureNamedKeyCredential("devstoreaccount1", "fake")
        )
        ...
        if self.args.no_client_share:
            self.service_client = SyncTableServiceClient(endpoint=endpoint, credential=credential, transport=transport)
            self.async_service_client = AsyncTableServiceClient(
                endpoint=endpoint, credential=async_credential, transport=async_transport
            )
        else:
...
class TestTransport(RequestsTransport):
    ...
    def send(self, request, **kwargs):
        ...
        if request.url.startswith(self.entity_url) and request.method == "GET":
            ...
            response.body = (
                lambda: b'{"odata.metadata":"https://antischpytables.table.core.windows.net/$metadata#cTpCdzGhGwlYNaBVnNtxJHBHGTkTCl","value":['
                + content
                + b"]}"
            )
            return response
