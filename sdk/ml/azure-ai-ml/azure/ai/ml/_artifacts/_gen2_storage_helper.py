...
class Gen2StorageClient:
    ...
    def download(
        self, starts_with: str, destination: Union[str, os.PathLike] = Path.home()
    ) -> None:
        ...
        try:
            ...
            for item in mylist:
                file_name = (
                    item.name[len(starts_with) :].lstrip("/") or Path(starts_with).name
                )
    ...
    def list(self, starts_with: str) -> List[str]:
        ...
        """
        return [
            f.get("name") for f in self.file_system_client.get_paths(path=starts_with)
        ]
