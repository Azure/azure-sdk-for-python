...
class DockerfileResolver(object):
    ...
    def write_file(
        ...
        """
        file_name = (
            f"{file_prefix}.Dockerfile" if file_prefix else "Dockerfile"
        )
        self._local_dockerfile_path = str(
            Path(directory_path, file_name).resolve()
        )
        with open(
            self._local_dockerfile_path,
            "w",
