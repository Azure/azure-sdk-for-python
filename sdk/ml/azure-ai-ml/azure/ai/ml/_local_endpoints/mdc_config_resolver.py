...
class MdcConfigResolver(object):
    ...
    def _construct(self, data_collector: DataCollector) -> None:
        ...
        if (
            ...
            self.mdc_config["captureHeaders"] = (
    ...
    def write_file(self, directory_path: str) -> None:
        ...
        mdc_setting_path = str(
            Path(directory_path, self.local_config_name).resolve()
        )
        with open(
            mdc_setting_path, "w", encoding=DefaultOpenEncoding.WRITE
        ) as f:
            d = json.dumps(self.mdc_config)
            f.write(f"{d}")

        self.environment_variables = {
            "AZUREML_MDC_CONFIG_PATH": self.config_path
        }
