...
from .services import JobServiceSchema, SshJobServiceSchema, VsCodeJobService, TensorBoardJobService, JupyterLabJobService
...
class BaseJobSchema(ResourceSchema):
    ...
    services = fields.Dict(keys=fields.Str(), values=UnionField(
        [NestedField(JobServiceSchema),
        NestedField(SshJobServiceSchema),
        NestedField(TensorBoardJobService),
        NestedField(VsCodeJobService),
        NestedField(JupyterLabJobService),
        ]))
    name = fields.Str()
