...
def validate_arm_str(arm_str: str) -> bool:
    reg_str = r"/subscriptions/[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}?/resourcegroups/.*/providers/[a-z.a-z]*/[a-z]*/.*"
...
def get_subnet_str(
    vnet_name: str, subnet: str, sub_id: Optional[str] = None, rg: Optional[str] = None
) -> str:
...
def replace_key_in_odict(odict: OrderedDict, old_key: Any, new_key: Any):
    if not odict or old_key not in odict:
        return odict
    return OrderedDict(
        [(new_key, v) if k == old_key else (k, v) for k, v in odict.items()]
    )
...
def exit_if_registry_assets(data: Dict, caller: str) -> None:
    ...
    if (
        ...
        raise ValidationError(
            f"Registry reference for environments is not supported for {caller}"
        )
    if (
        "model" in data
        and data["model"]
        and isinstance(data["model"], str)
        and data["model"].startswith(startswith)
    ):
        raise ValidationError(
            f"Registry reference for models is not supported for {caller}"
        )
    if (
        ...
        raise ValidationError(
            f"Registry reference for code_configuration.code is not supported for {caller}"
        )


def _resolve_group_inputs_for_component(
    component, **kwargs
