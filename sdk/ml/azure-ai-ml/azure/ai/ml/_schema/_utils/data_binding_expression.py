...
def _add_data_binding_to_field(field, attrs_to_skip, schema_stack):
    if hasattr(field, DATA_BINDING_SUPPORTED_KEY) and getattr(
        field, DATA_BINDING_SUPPORTED_KEY
    ):
        return field
    ...
    if isinstance(field, UnionField):
        for field_obj in field.union_fields:
            if not _is_literal(field_obj):
                _add_data_binding_to_field(
                    field_obj, attrs_to_skip, schema_stack=schema_stack
                )
        ...
        if field.value_field is not None:
            field.value_field = _add_data_binding_to_field(
                field.value_field, attrs_to_skip, schema_stack=schema_stack
            )
    elif isinstance(field, fields.List):
        # handle list
        field.inner = _add_data_binding_to_field(
            field.inner, attrs_to_skip, schema_stack=schema_stack
        )
    elif isinstance(field, ExperimentalField):
        field = ExperimentalField(
            _add_data_binding_to_field(
                field.experimental_field, attrs_to_skip, schema_stack=schema_stack
            ),
        ...
        support_data_binding_expression_for_fields(
            field.schema, attrs_to_skip, schema_stack=schema_stack
        )
    else:
...
def support_data_binding_expression_for_fields(  # pylint: disable=name-too-long
    ...
    """
    if hasattr(schema, DATA_BINDING_SUPPORTED_KEY) and getattr(
        schema, DATA_BINDING_SUPPORTED_KEY
    ):
        return
    ...
    for attr, field_obj in schema.load_fields.items():
        if attr not in attrs_to_skip:
            schema.load_fields[attr] = _add_data_binding_to_field(
                field_obj, attrs_to_skip, schema_stack=schema_stack
            )
    for attr, field_obj in schema.dump_fields.items():
        if attr not in attrs_to_skip:
            schema.dump_fields[attr] = _add_data_binding_to_field(
                field_obj, attrs_to_skip, schema_stack=schema_stack
            )
    schema_stack.pop()
