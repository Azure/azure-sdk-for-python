...
class MLTableMetadataSchema(YamlFileSchema):
    ...
    def make(self, data: Dict, **kwargs):
        ...
        paths = [
            MLTableMetadataPath(pathDict=pathDict) for pathDict in data.pop("paths")
        ]
        return MLTableMetadata(
            base_path=self.context[BASE_PATH_CONTEXT_KEY], **data, paths=paths
        )
