...
from azure.ai.ml._schema.automl.table_vertical.table_vertical_limit_settings import (
...
class AutoMLTableVerticalSchema(AutoMLVerticalSchema):
    limits = NestedField(
        AutoMLTableLimitsSchema(), data_key=AutoMLConstants.LIMITS_YAML
    )
    featurization = NestedField(
        TableFeaturizationSettingsSchema(), data_key=AutoMLConstants.FEATURIZATION_YAML
    )
