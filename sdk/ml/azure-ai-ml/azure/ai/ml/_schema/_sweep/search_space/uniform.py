...
class UniformSchema(metaclass=PatchedSchemaMeta):
    type = StringTransformedEnum(
        required=True, allowed_values=SearchSpace.UNIFORM_LOGUNIFORM
    )
    min_value = UnionField(
        [DumpableIntegerField(strict=True), fields.Float()], required=True
    )
    max_value = UnionField(
        [DumpableIntegerField(strict=True), fields.Float()], required=True
    )
    ...
    def predump(self, data, **kwargs):
        ...
        if not isinstance(data, (Uniform, LogUniform)):
            raise ValidationError(
                "Cannot dump non-Uniform or non-LogUniform object into UniformSchema"
            )
        if data.type.lower() not in SearchSpace.UNIFORM_LOGUNIFORM:
            raise ValidationError(
                BASE_ERROR_MESSAGE + str(SearchSpace.UNIFORM_LOGUNIFORM)
            )
        return data
    ...
    def make(self, data, **kwargs):
        ...
        return (
            Uniform(**data) if data[TYPE] == SearchSpace.UNIFORM else LogUniform(**data)
        )


class QUniformSchema(metaclass=PatchedSchemaMeta):
    type = StringTransformedEnum(
        required=True, allowed_values=SearchSpace.QUNIFORM_QLOGUNIFORM
    )
    min_value = UnionField(
        [DumpableIntegerField(strict=True), fields.Float()], required=True
    )
    ...
    def make(self, data, **kwargs):
        ...
        return (
            QUniform(**data)
            if data[TYPE] == SearchSpace.QUNIFORM
            else QLogUniform(**data)
        )
    ...
    def predump(self, data, **kwargs):
        ...
        if not isinstance(data, (QUniform, QLogUniform)):
            raise ValidationError(
                "Cannot dump non-QUniform or non-QLogUniform object into UniformSchema"
            )
        return data
