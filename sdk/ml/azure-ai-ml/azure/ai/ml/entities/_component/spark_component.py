    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(environment, Environment):
            if environment.image is not None and environment.image != DUMMY_IMAGE:
                module_logger.warning(
                    "Spark job will only install the packages defined in the Conda configuration. It will not create a "
                    "docker container using the image defined in the environment."
                )
            else:
                environment = Environment(conda_file=environment.conda_file, image=DUMMY_IMAGE)
