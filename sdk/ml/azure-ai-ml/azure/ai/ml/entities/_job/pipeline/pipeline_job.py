...
from azure.ai.ml.entities._job.identity import AmlToken, Identity, UserIdentity, ManagedIdentity
...
from azure.ai.ml.entities._job.pipeline._io import InputsAttrDict, OutputsAttrDict, PipelineIOMixin, PipelineInput
        ...
        for node_name, node in self.jobs.items():
            if hasattr(node, "compute") and node.compute is None:
                if (
                    ...
                    continue
                if isinstance(node, Spark) and hasattr(node, "resources") and node.resources:
                    continue
