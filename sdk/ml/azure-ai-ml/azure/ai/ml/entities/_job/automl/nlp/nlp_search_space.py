...
class NlpSearchSpace(RestTranslatableMixin):
        ...
        *,
        ...
        if isinstance(learning_rate_scheduler, NlpLearningRateScheduler):
            ...
            learning_rate_scheduler.values = [
                camel_to_snake(item.value)
                if isinstance(item, NlpLearningRateScheduler)
                else item
        ...
        if isinstance(model_name, NlpModels):
            ...
            model_name.values = [
                item.value if isinstance(item, NlpModels) else item
                for item in model_name.values
            ]
    ...
    def _to_rest_object(self) -> NlpParameterSubspace:
        return NlpParameterSubspace(
            ...
            warmup_ratio=_convert_to_rest_object(self.warmup_ratio)
    ...
    def _from_rest_object(cls, obj: NlpParameterSubspace) -> "NlpSearchSpace":
        return cls(
            ...
            warmup_ratio=_convert_from_rest_object(obj.warmup_ratio)
            if obj.warmup_ratio is not None
            else None,
    ...
    def _from_search_space_object(cls, obj: SearchSpace) -> "NlpSearchSpace":
        return cls(
            gradient_accumulation_steps=(
                ...
                if hasattr(obj, "gradient_accumulation_steps")
                else None
            ),
