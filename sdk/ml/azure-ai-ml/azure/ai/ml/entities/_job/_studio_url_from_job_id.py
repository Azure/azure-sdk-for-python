...
from azure.ai.ml._azure_environments import _get_default_cloud_name, _get_aml_resource_id_from_metadata
...
def studio_url_from_job_id(job_id: str) -> Optional[str]:
    ...
    if m:
        return f"{resource_id}/runs/{m.group('run_id')}?wsid=/subscriptions/{m.group('subscription')}/resourcegroups/{m.group('resource_group')}/workspaces/{m.group('workspace')}"  # fmt: skip
    return None
