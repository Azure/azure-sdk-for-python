...
class DoWhile(LoopNode):
    ...
    def _load_from_dict(
        ...
        loaded_data = load_from_dict(
            DoWhileSchema, data, context, additional_message, **kwargs
        )
    ...
    def _create_instance_from_schema_dict(
        cls,
        pipeline_jobs: Dict[str, BaseNode],
        loaded_data: Dict,
        ...
        for k, v in loaded_data.pop("mapping", {}).items():
            output_name = cls._get_data_binding_expression_value(
                k, regex=r"\{\{.*\.%s\.outputs\.(.*)\}\}" % body_name
            )
            input_names = v if isinstance(v, list) else [v]
            input_names = [
                cls._get_data_binding_expression_value(
                    item, regex=r"\{\{.*\.%s\.inputs\.(.*)\}\}" % body_name
                )
                for item in input_names
            ]
            mapping[output_name] = [
                get_port_obj(body, item, validate_port=validate_port)
                for item in input_names
            ]
        ...
        if "condition" in loaded_data:
            ...
            condition_name = cls._get_data_binding_expression_value(
                loaded_data.pop("condition"),
                regex=r"\{\{.*\.%s\.outputs\.(.*)\}\}" % body_name,
            )
            condition_value = get_port_obj(
                body, condition_name, is_input=False, validate_port=validate_port
            )
        else:
    ...
    def _from_rest_object(cls, obj: dict, pipeline_jobs: dict) -> "DoWhile":
        ...
        return cls._create_instance_from_schema_dict(
            pipeline_jobs, obj, validate_port=False
        )
    ...
    def set_limits(
        ...
        **kwargs,  # pylint: disable=unused-argument
        ...
        if isinstance(self.limits, DoWhileJobLimits):
            self.limits._max_iteration_count = (
                max_iteration_count  # pylint: disable=protected-access
            )
        else:
    ...
    def _customized_validate(self):
        ...
        validation_result.merge_with(
            self._validate_body_output_mapping(raise_error=False)
        )
    ...
    def _validate_port(
        ...
        if (
            ...
            and port_obj._owner._instance_id
            != self.body._instance_id  # pylint: disable=protected-access
        ):
            ...
            port_obj is None or port_obj._port_name not in node_ports
        ):  # pylint: disable=protected-access
        ...
        if self.condition is not None:
            ...
            if validation_result.passed:
                ...
                condition_name = (
                    ...
                    else self.condition._port_name
                )
    ...
    def _validate_do_while_limit(self, raise_error=True):
        ...
        if isinstance(self.limits.max_iteration_count, InputOutputBase):
            ...
            self.limits.max_iteration_count > DO_WHILE_MAX_ITERATION
            or self.limits.max_iteration_count < 0
        ):
            validation_result.append_error(
                yaml_path="limit.max_iteration_count",
    ...
    def _validate_body_output_mapping(self, raise_error=True):
        ...
        if not isinstance(self.mapping, dict):
            validation_result.append_error(
                yaml_path="mapping",
                message=f"Mapping expects a dict type but passes in a {type(self.mapping)} type.",
            )
        else:
            ...
            for output, inputs in self.mapping.items():
                ...
                if validate_results.passed:
                    is_primitive_output = self.body._outputs[
                        output_name
                    ]._is_primitive_type
                    ...
                    for item in inputs:
                        ...
                        input_output_mapping[input_name] = input_output_mapping.get(
                            input_name, []
                        ) + [output_name]
                        is_primitive_type = self.body._inputs[
                            input_name
                        ]._meta._is_primitive_type
            ...
            for _input, outputs in input_output_mapping.items():
                if len(outputs) > 1:
                    validation_result.append_error(
                        yaml_path="mapping",
                        message=f"Input {_input} has been linked to multiple outputs {outputs}.",
                    )
        return validation_result.try_raise(
            self._get_validation_error_target(), raise_error=raise_error
        )
