...
from azure.ai.ml._schema._feature_store_entity.feature_store_entity_schema import (
...
class FeatureStoreEntity(Asset):
    ...
    def _to_rest_object(self) -> FeaturestoreEntityVersion:
        ...
        return FeaturestoreEntityVersion(
            properties=feature_store_entity_version_properties
        )

    @classmethod
    def _from_rest_object(
        cls, rest_obj: FeaturestoreEntityVersion
    ) -> "FeatureStoreEntity":
        ...
        featurestoreEntity = FeatureStoreEntity(
            ...
            index_columns=[
                DataColumn._from_rest_object(column)
                for column in rest_object_details.index_columns
            ],
    ...
    def _from_container_rest_object(
        cls, rest_obj: FeaturestoreEntityContainer
    ) -> "FeatureStoreEntity":
    ...
    def _load(
        ...
        **kwargs,
        ...
        loaded_schema = load_from_dict(
            FeatureStoreEntitySchema, data, context, **kwargs
        )
        return FeatureStoreEntity(**loaded_schema)

    def _to_dict(self) -> Dict:
        # pylint: disable=no-member
        return FeatureStoreEntitySchema(context={BASE_PATH_CONTEXT_KEY: "./"}).dump(
            self
        )
