...
from azure.ai.ml.entities._indexes.documents import Document, DocumentChunksIterator
...
class MLIndex:
    ...
    def as_langchain_vectorstore(self, credential: Optional[TokenCredential] = None):
        ...
        with track_activity(logger, "MLIndex.as_langchain_vectorstore") as activity_logger:
            ...
            if index_kind == "acs":
                from azure.ai.ml.entities._indexes.indexes.azure_search import import_azure_search_or_so_help_me
                ...
                if langchain_pkg_version >= pkg_version.parse("0.1.00"):
                    from azure.ai.ml.entities._indexes.utils.acs import _get_azuresearch_module_instance
                    ...
                    from azure.ai.ml.entities._indexes.langchain.acs import AzureCognitiveSearchVectorStore
                ...
                if engine == "langchain.vectorstores.FAISS":
                    ...
                    with tempfile.TemporaryDirectory() as tmpdir:
                        ...
                        try:
                            ...
                            from azure.ai.ml.entities._indexes.langchain.vendor.vectorstores.faiss import FAISS
                    ...
                    error_fmt_str = """Failed to import langchain faiss bridge module with: {e}\n"
                        ...
                        """
                    try:
                        from azure.ai.ml.entities._indexes.langchain.faiss import azureml_faiss_as_langchain_faiss
                    except Exception as e:
                ...
                try:
                    import pinecone
                    from azure.core.credentials import AzureKeyCredential
                    from azure.ai.ml.entities._indexes.tasks.update_pinecone import get_pinecone_environment
                    try:
                        from langchain.vectorstores import Pinecone
                    except ImportError:
                        # TODO: this is deprecated. Use langchain_pinecone instead.
                ...
                try:
                    from azure.core.credentials import AzureKeyCredential
                    from azure.ai.ml.entities._indexes.tasks.update_azure_cosmos_mongo_vcore import get_mongo_client
                    try:
    ...
    def as_native_index_client(self, credential: Optional[TokenCredential] = None):
        ...
        An azure.search.documents.SearchClient for acs indexes or an azure.ai.ml.entities._indexes.indexes.indexFaissAndDocStore for faiss indexes.
        """
        index_kind = self.index_config.get("kind", None)
        if index_kind == "acs":
            connection_credential = get_connection_credential(self.index_config, credential=credential)
            ...
            from azure.core.credentials import AzureKeyCredential
            from azure.ai.ml.entities._indexes.tasks.update_milvus import MILVUS_URI_KEY
    ...
    def set_embeddings_connection(
        self,
        connection: Optional[Union[str, WorkspaceConnection]],
        credential: Optional[TokenCredential] = None,
    ) -> "MLIndex":
    ...
    def from_files(
        ...
        """
        from azure.ai.ml.entities._indexes.documents import DocumentChunksIterator, split_documents
