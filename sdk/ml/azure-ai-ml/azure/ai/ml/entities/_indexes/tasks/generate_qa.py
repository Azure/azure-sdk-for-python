...
from azure.ai.ml.entities._indexes.data_generation.qa import GenerationResult, QADataGenerator, QAType
from azure.ai.ml.entities._indexes.utils.connections import connection_to_credential, get_connection_by_id_v2, get_connection_credential
...
def get_model_config(
    llm_config: Dict[str, str], openai_api_type: str, openai_api_version: str, activity_logger: Logger
):
    """
    Get model_config from llm_config. llm_config format is used in Baker pipelines.

    model_config format is accepted by `azure.ai.ml.entities._indexes.models.init_llm()`.
    """
    ...
    connection_id = (
        ...
        if llm_config["llm_connection"] is not None
        else os.environ.get("AZUREML_WORKSPACE_CONNECTION_ID_AOAI")
    )
    if connection_id:
        connection_config = {
        ...
        credential = get_connection_credential(connection_config, data_plane=True)
...
def main(parser_args, run, logger: Logger, activity_logger: Logger):
    ...
    model_config = get_model_config(
        llm_config, parser_args.openai_api_type, parser_args.openai_api_version, activity_logger
    )
    ...
    try:
        result = qa_generator.generate(
            input_dir=parser_args.input_data,
    ...
    with track_activity(logger, "generate_qa") as activity_logger:
        try:
            ...
            activity_logger.error(
                f"generate_qa failed with exception: {traceback.format_exc()}"
