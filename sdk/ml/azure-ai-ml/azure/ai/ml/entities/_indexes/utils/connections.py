...
from ._requests import create_session_with_retry, send_post_request
...
def connection_to_credential(connection: Union[dict, Connection], data_plane: bool = True):
    ...
    if isinstance(connection, dict):
        ...
        if auth_type == "ApiKey":
            ...
            if connection["properties"]["category"] == "CognitiveSearch":
                ...
                logger.info(f"The '{connection.get('name', 'no-name connection')}' is an AAD auth type CognitiveSearch connection.")
                return DefaultAzureCredential(process_timeout=60)
            elif connection["properties"]["category"] == "AzureOpenAI":
                logger.info(f"The '{connection.get('name', 'no-name connection')}' is an AAD auth type AzureOpenAI connection.")
                return _token_provider_for_aoai_connection(data_plane)
            else:
                raise ValueError(f"Unknown category '{connection['properties']['category']}' for AAD auth type connection")
...
def _token_provider_for_aoai_connection(data_plane: bool = True) -> Callable[[], str]:
    """Token provider for AOAI connection.data plane operations by default."""
    try:
        from azure.identity import DefaultAzureCredential, get_bearer_token_provider
    except ImportError as e:
        raise ValueError("Please install azure-identity with `pip install azure-identity --upgrade`.") from e
