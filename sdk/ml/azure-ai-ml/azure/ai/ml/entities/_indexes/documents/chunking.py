...
from azure.ai.ml.entities._indexes.documents.document import Document, DocumentSource, StaticDocument
from azure.ai.ml.entities._indexes.langchain.vendor.text_splitter import TextSplitter
from azure.ai.ml.entities._indexes.utils import merge_dicts
from azure.ai.ml.entities._indexes.utils.logging import get_logger, safe_mlflow_log_metric
from azure.ai.ml.entities._indexes.utils.tokens import tiktoken_cache_dir, token_length_function
...
def get_langchain_splitter(file_extension: str, arguments: dict) -> TextSplitter:
    ...
    if file_extension == ".py":
        from azure.ai.ml.entities._indexes.langchain.vendor.text_splitter import Language, RecursiveCharacterTextSplitter
    ...
    if use_nltk:
        _init_nltk()
        from azure.ai.ml.entities._indexes.langchain.vendor.text_splitter import NLTKTextSplitter
    ...
    if file_extension == ".txt" or file_extension in formats_to_treat_as_txt_once_loaded:
        from azure.ai.ml.entities._indexes.langchain.vendor.text_splitter import TokenTextSplitter
        ...
        if use_rcts:
            from azure.ai.ml.entities._indexes.langchain.vendor.text_splitter import MarkdownTextSplitter
