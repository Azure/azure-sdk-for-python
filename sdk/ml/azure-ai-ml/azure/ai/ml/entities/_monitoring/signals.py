...
@experimental
class LlmDataSchema(RestTranslatableMixin):
    ...
    @classmethod
    def _from_rest_object(cls, obj: RestMonitoringInputData) -> "LlmDataSchema":
        return cls(
            input_data=Input(
                path=obj.uri,
                type=obj.job_input_type,
...
class GenerationSafetyQualitySignal(RestTranslatableMixin):
    ...
    :paramtype input_datasets: Optional[dict[str, ~azure.ai.ml.entities.LlmDataSchema]]
    ...
    def __init__(
        self,
        *,
        production_data: List[LlmDataSchema] = None,
    ...
    def _from_rest_object(cls, obj: RestGenerationSafetyQualityMonitoringSignal) -> "GenerationSafetyQualitySignal":
        return cls(
            production_data=[LlmDataSchema._from_rest_object(data) for data in obj.production_data],
