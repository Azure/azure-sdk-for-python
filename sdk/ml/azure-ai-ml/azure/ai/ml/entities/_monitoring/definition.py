...
class MonitorDefinition(RestTranslatableMixin):
    ...
    def __init__(
        ...
        *,
        ...
        alert_notification: Optional[
            Union[Literal[AZMONITORING], AlertNotification]
        ] = None,
    ) -> None:
    ...
    def _to_rest_object(self, **kwargs) -> RestMonitorDefinition:
        ...
        if self.alert_notification:
            if (
                isinstance(self.alert_notification, str)
                and self.alert_notification.lower() == AZMONITORING
            ):
        ...
        return RestMonitorDefinition(
            compute_id="spark",
            monitoring_target=(
                self.monitoring_target.endpoint_deployment_id
                or self.monitoring_target.model_id
            )
    ...
    def _from_rest_object(
        cls, obj: RestMonitorDefinition, **kwargs
    ) -> "MonitorDefinition":
        tags = kwargs.get("tags")
        from_rest_alert_notification = None
        if obj.alert_notification_setting:
            if isinstance(
                obj.alert_notification_setting, AzMonMonitoringAlertNotificationSettings
            ):
                from_rest_alert_notification = AZMONITORING
            else:
                from_rest_alert_notification = AlertNotification._from_rest_object(
                    obj.alert_notification_setting
                )
        return cls(
            compute=SparkResourceConfiguration(
            ...
            monitoring_target=MonitoringTarget(
                endpoint_deployment_id=obj.monitoring_target
            ),
            monitoring_signals={
                signal_name: MonitoringSignal._from_rest_object(signal)
