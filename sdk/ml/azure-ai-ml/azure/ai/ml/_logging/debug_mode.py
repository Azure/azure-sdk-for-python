...
class diagnostic_log(object):
    ...
    def __init__(
        ...
        self._namespaces = (
            INTERESTING_NAMESPACES if namespaces is None else namespaces
        )
        ...
        if context_name is None:
            ...
            context_name = inspect.getouterframes(inspect.currentframe(), 2)[
                1
            ].function
        ...
        file_handler = logging.FileHandler(
            filename=self._filename, encoding="utf-8"
        )
    ...
    def start_capture(self) -> None:
        ...
        for namespace in self._namespaces:
            module_logger.debug(
                "Adding [%s] debug logs to this file", namespace
            )
    ...
    def stop_capture(self) -> None:
        ...
        print(
            "Disabling log capture. Resulting file is at {}".format(
                self._filename
            )
        )
        ...
        for namespace in self._namespaces:
            module_logger.debug(
                "Removing [%s] debug logs to this file", namespace
            )
            n_logger = logging.getLogger(namespace)
