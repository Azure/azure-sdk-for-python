...
def load_file(file_path: str) -> str:
    ...
    :rtype: str
    """
    from azure.ai.ml.exceptions import ErrorCategory, ErrorTarget, ValidationErrorType, ValidationException  # These imports can't be placed in at top file level because it will cause a circular import in exceptions.py via _get_mfe_url_override
...
def resolve_short_datastore_url(value: Union[PathLike, str], workspace: OperationScope) -> str:
    from azure.ai.ml.exceptions import ValidationException # These imports can't be placed in at top file level because it will cause a circular import in exceptions.py via _get_mfe_url_override
...
def validate_ml_flow_folder(path: str, model_type: string) -> None:
    from azure.ai.ml.exceptions import ErrorTarget, ValidationErrorType, ValidationException  # These imports can't be placed in at top file level because it will cause a circular import in exceptions.py via _get_mfe_url_override
...
def _validate_missing_sub_or_rg_and_raise(subscription_id: str, resource_group: str):
    ...
    as appropriate."""
    from azure.ai.ml.exceptions import ErrorCategory, ErrorTarget, ValidationException  # These imports can't be placed in at top file level because it will cause a circular import in exceptions.py via _get_mfe_url_override
