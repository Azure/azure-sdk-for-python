...
from typing import Any, AnyStr, Callable, Dict, List, Optional, Tuple, Union, IO
...
def download_text_from_url(
    source_uri: str, requests_pipeline: HttpPipeline, timeout: Union[float, Tuple[float, float]] = None
) -> str:
...
def initialize_logger_info(logger: logging.Logger, terminator="\n") -> None:
...
def _get_mfe_base_url_from_discovery_service(
    ...
    all_urls = json.loads(
        download_text_from_url(discovery_url, create_requests_pipeline_with_retry(requests_pipeline=requests_pipeline))
    )
