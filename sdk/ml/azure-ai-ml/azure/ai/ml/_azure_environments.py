...
def _get_default_cloud_name() -> str:
    ...
    if arm_metadata_url is not None:
        clouds = _get_clouds_by_metadata_url(
            arm_metadata_url
        )  # prefer ARM metadata url when set
        for cloud_name in clouds:  # pylint: disable=consider-using-dict-items
            if (
                clouds[cloud_name][EndpointURLS.RESOURCE_MANAGER_ENDPOINT]
                in arm_metadata_url
            ):
...
def _set_cloud(cloud_name: Optional[str] = None):
    ...
    if cloud_name is not None:
        try:
            ...
            msg = 'Unknown cloud environment supplied: "{0}".'.format(
                cloud_name
            )
            raise MlException(message=msg, no_personal_data_message=msg) from e
    else:
...
def _get_base_url_from_metadata(
    ...
    if base_url is None:
        ...
        base_url = str(
            cloud_details.get(EndpointURLS.RESOURCE_MANAGER_ENDPOINT)
        ).strip("/")
    return base_url


def _get_aml_resource_id_from_metadata(
    ...
    cloud_details = _get_cloud_details(cloud_name)
    aml_resource_id = str(
        cloud_details.get(EndpointURLS.AML_RESOURCE_ID)
    ).strip("/")
    return aml_resource_id


def _get_active_directory_url_from_metadata(
    cloud_name: Optional[str] = None,
) -> str:
    ...
    """
    cloud_details = _get_cloud_details(cloud_name)
    active_directory_url = str(
        cloud_details.get(EndpointURLS.ACTIVE_DIRECTORY_ENDPOINT)
    ).strip("/")
...
def _get_azure_portal_id_from_metadata(
    cloud_name: Optional[str] = None,
) -> str:
...
def _get_registry_discovery_endpoint_from_metadata(
    cloud_name: Optional[str] = None,
) -> str:
...
def _get_registry_discovery_url(cloud: dict, cloud_suffix: str = "") -> str:
    ...
    if cloud_name in _environments:
        return _environments[cloud_name][
            EndpointURLS.REGISTRY_DISCOVERY_ENDPOINT
        ]
...
def _get_clouds_by_metadata_url(
    metadata_url: str,
...
def _convert_arm_to_cli(arm_cloud_metadata) -> Dict[str, Dict[str, str]]:
    ...
    for cloud in arm_cloud_metadata:
        try:
            ...
            registry_discovery_url = _get_registry_discovery_url(
                cloud, cloud_suffix
            )
            cli_cloud_metadata_dict[cloud_name] = {
                EndpointURLS.AZURE_PORTAL_ENDPOINT: cloud["portal"],
                EndpointURLS.RESOURCE_MANAGER_ENDPOINT: cloud[
                    "resourceManager"
                ],
                EndpointURLS.ACTIVE_DIRECTORY_ENDPOINT: cloud[
                    "authentication"
                ]["loginEndpoint"],
            ...
            module_logger.warning(
                "Property on cloud not found in arm cloud metadata"
            )
...
def _add_cloud_to_environments(kwargs):
    ...
    _environments[kwargs["cloud"]] = {
        ...
        EndpointURLS.AML_RESOURCE_ID: cloud_metadata[
            EndpointURLS.AML_RESOURCE_ID
        ],
        EndpointURLS.STORAGE_ENDPOINT: cloud_metadata[
