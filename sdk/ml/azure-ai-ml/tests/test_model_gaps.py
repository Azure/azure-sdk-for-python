...
class TestModelPackageGaps(AzureRecordedTestCase):
    def test_share_with_nonexistent_registry_raises(
        self, client: MLClient, randstr: Callable[[], str], tmp_path: Path
    ) -> None:
        # Attempting to share a model to an inaccessible registry should raise
        name = f"model_{randstr('name')}"
        version = "1"

        with pytest.raises((ResourceNotFoundError, HttpResponseError, Exception)):
            client.models.share(
                name=name,
                version=version,
