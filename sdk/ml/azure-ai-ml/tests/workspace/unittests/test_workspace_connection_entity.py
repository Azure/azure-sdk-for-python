...
from azure.ai.ml.constants._common import WorkspaceConnectionTypes, CognitiveServiceKinds
...
class TestWorkspaceConnectionEntity:
    ...
    def test_azure_open_ai(self):
        ...
        assert type(ws_connection) == AzureOpenAIWorkspaceConnection
    ...
    def test_ai_services(self):
        ...
        assert type(ws_connection) == AzureAIServiceWorkspaceConnection
        assert ws_connection.type == WorkspaceConnectionTypes.AZURE_AI_SERVICES
    ...
    def test_content_safety(self):
        ...
        assert type(ws_connection) == AzureContentSafetyWorkspaceConnection
        assert ws_connection.type == WorkspaceConnectionTypes.AZURE_CONTENT_SAFETY
    ...
    def test_speech(self):
        ...
        assert type(ws_connection) == AzureSpeechServicesWorkspaceConnection
        assert ws_connection.type == WorkspaceConnectionTypes.AZURE_SPEECH_SERVICES
    ...
    def test_search(self):
        ...
        assert type(ws_connection) == AzureAISearchWorkspaceConnection
        assert ws_connection.type == WorkspaceConnectionTypes.AZURE_SEARCH
    ...
    def test_api_key(self):
        ws_connection = load_workspace_connection(source="./tests/test_configs/workspace_connection/api_key_conn.yaml")
        assert type(ws_connection) == APIKeyWorkspaceConnection
    ...
    def test_open_ai(self):
        ...
        assert type(ws_connection) == OpenAIWorkspaceConnection
    ...
    def test_serp(self):
        ws_connection = load_workspace_connection(source="./tests/test_configs/workspace_connection/serp.yaml")
        assert type(ws_connection) == SerpWorkspaceConnection
    ...
    def test_git(self):
        ws_connection = load_workspace_connection(source="./tests/test_configs/workspace_connection/git_pat.yaml")
        ...
        ws_connection = load_workspace_connection(source="./tests/test_configs/workspace_connection/git_no_cred.yaml")
        assert type(ws_connection) == WorkspaceConnection
        assert ws_connection.type == camel_to_snake(ConnectionCategory.GIT)
    ...
    def test_python_feed(self):
        ...
        ws_connection = load_workspace_connection(
            source="./tests/test_configs/workspace_connection/python_feed_user_pass.yaml"
        )
        assert type(ws_connection) == WorkspaceConnection
        assert ws_connection.type == camel_to_snake(ConnectionCategory.PYTHON_FEED)
        ...
        ws_connection = load_workspace_connection(
            source="./tests/test_configs/workspace_connection/python_feed_no_cred.yaml"
        )
        assert type(ws_connection) == WorkspaceConnection
        assert ws_connection.type == camel_to_snake(ConnectionCategory.PYTHON_FEED)
        assert ws_connection.credentials.type == ConnectionAuthType.NONE
    ...
    def test_container_registry(self):
        ...
        ws_connection = load_workspace_connection(
            source="./tests/test_configs/workspace_connection/container_registry_user_pass.yaml"
        )
        assert ws_connection.type == camel_to_snake(ConnectionCategory.CONTAINER_REGISTRY)
        assert ws_connection.credentials.type == camel_to_snake(ConnectionAuthType.USERNAME_PASSWORD)
    ...
    def test_serverless(self):
        ...
        assert type(ws_connection) == ServerlessWorkspaceConnection
    ...
    def test_bad_credential_load_attempts(self):
        with pytest.raises(ValidationError):
            _ = load_workspace_connection(source="./tests/test_configs/workspace_connection/blob_store_bad_cred.yaml")

        with pytest.raises(ValueError):
            _ = WorkspaceConnection(
                name="bad",
                target="bad",
                type=WorkspaceConnectionTypes.AZURE_DATA_LAKE_GEN_2,
