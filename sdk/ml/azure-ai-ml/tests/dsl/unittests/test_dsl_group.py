...
class TestDSLGroup:
    ...
    def test_group_items(self):
        ...
        assert len(values) == 5
        values = sorted(values, key=lambda item: item["arg_name"])
    ...
    def test_group_inherit_parameter_order(self):
        ...
        for _id, (expected_key, value) in enumerate(zip(expected_keys, list(params.values()))):
            assert value.arg_name == expected_key
            if _id > 4:
                assert value.default is not None
                assert value.arg_name == f"int_param{value.default}"
    ...
    def test_group_defaults_with_outputs(self):
        ...
        assert (
                ...
                "str_param:str=None,"
                "input_folder:{'type':'uri_folder','arg_name':'input_folder'}=None,"
                "optional_int_param:int=5,"
                "output_folder:{'type':'uri_folder','arg_name':'output_folder'}=None)"
