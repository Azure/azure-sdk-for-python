...
from azure.ai.ml import dsl, command, MLClient, Input, Output, load_component, load_job, spark
...
class TestDSLPipeline:
    ...
    def test_dsl_pipeline_with_spark(self) -> None:
        yaml_file = "./tests/test_configs/dsl_pipeline/spark_job_in_pipeline/component.yml"

        @dsl.pipeline(description="submit a spark job using component sdk")
        def spark_pipeline_from_yaml(iris_data):
            spark_component: SparkComponent = load_component(path=yaml_file)
            node1: Spark = spark_component(
                file_input1=iris_data,
                file_input2=iris_data,
            )
            node1.compute = "spark31"
            node1.driver_cores = 2
            node1.driver_memory = "1g"
            node1.executor_cores = 1
            node1.executor_memory = "1g"
            node1.executor_instances = 1

            return {"output": node1.outputs.output}
        ...
        spark_node = dsl_pipeline.jobs["node1"]
        job_data_path_input = spark_node.inputs["file_input1"]._meta
        assert job_data_path_input
    ...
    def test_pipeline_with_spark_function(self):
        ...
        spark_job = SparkJob(
            code="./tests/test_configs/dsl_pipeline/spark_job_in_pipeline/src",
            entry={"file": "entry.py"},
            py_files=["utils.zip"],
            jars=["scalaproj.jar"],
            driver_cores=2,
        ...
        def pipeline(iris_data):
            node1 = component_func(file_input1=iris_data, file_input2=iris_data)
            node1.compute = synapse_compute_name
            node2 = spark_job_func(file_input1=iris_data, file_input2=node1.outputs.output)
            node2.compute = synapse_compute_name
            node3 = spark_function(file_input1=iris_data, file_input2=node2.outputs.output)
            return {
                "pipeline_output1": node1.outputs.output,
                "pipeline_output2": node2.outputs.output,
                "pipeline_output3": node3.outputs.output,
            }
            ...
            "properties": {
                ...
                "jobs": {
                    "node1": {
                        ...
                        "outputs": {"output": {"value": "${{parent.outputs.pipeline_output1}}", "type": "literal"}},
                        "_source": "YAML.COMPONENT",
                    },
                    "node2": {
                        "type": "spark",
                        ...
                        "inputs": {
                            ...
                            "file_input2": {
                                "job_input_type": "literal",
                                "value": "${{parent.jobs.node1.outputs.output}}",
                            },
                        },
                        "outputs": {"output": {"value": "${{parent.outputs.pipeline_output2}}", "type": "literal"}},
                        "_source": "CLASS",
                    },
                    "node3": {
                        "type": "spark",
                        ...
                        "outputs": {"output": {"value": "${{parent.outputs.pipeline_output3}}", "type": "literal"}},
                        "_source": "BUILDER",
                    },
                },
                "outputs": {
    ...
    def test_pipeline_with_spark_job(self):
        ...
        def pipeline(iris_data):
            spark_node = spark_job_func(file_input1=iris_data, file_input2=iris_data)
            spark_node.compute = synapse_compute_name
            return {
                "pipeline_output1": spark_node.outputs.output,
            }
        ...
        omit_fields = [
            ...
            "properties.jobs.spark_node.code",
        ]
        pipeline_job1 = pydash.omit(pipeline_job1, *omit_fields)
        assert pipeline_job1 == {
            "properties": {
    ...
    def test_nested_dsl_pipeline(self):
        ...
        actual_dict = pydash.omit(actual_dict, *["jobs.node1.component", "jobs.node2.component"])
