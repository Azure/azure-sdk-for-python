...
class TestWorkspaceOperationsGaps(AzureRecordedTestCase):
    ...
    def test_workspace_create_with_managed_network_provision_network(
        ...
        if "euap" in (location or ""):
            pytest.skip(
                f"Location '{location}' may not support required resource types for provisioning; skipping live test."
            )
    ...
    @pytest.mark.e2etest
    def test_begin_diagnose_returns_poller_and_result_raises(
        self, client: MLClient, randstr: Callable[[], str]
    ) -> None:
