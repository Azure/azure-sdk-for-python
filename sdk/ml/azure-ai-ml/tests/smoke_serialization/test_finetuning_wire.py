...
@pytest.mark.parametrize("golden_name", sorted(FINETUNING_BUILDERS))
def test_custom_finetuning_serializes(golden_name):
    """The CustomModelFineTuningJob rest object should serialize to wire (xfail: known break)."""
    entity = FINETUNING_BUILDERS[golden_name]()
    assert_serializes(entity._to_rest_object())


@pytest.mark.xfail(reason=_XFAIL_REASON, strict=False)
@pytest.mark.parametrize("golden_name", sorted(FINETUNING_BUILDERS))
def test_custom_finetuning_wire_matches_golden(golden_name):
    """The CustomModelFineTuningJob wire should match the golden from main (xfail: known break)."""
    entity = FINETUNING_BUILDERS[golden_name]()
