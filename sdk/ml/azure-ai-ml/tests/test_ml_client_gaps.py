...
class TestMLClientGaps:
    def test_create_or_update_with_unsupported_entity_raises_type_error(
        ...
        with pytest.raises(TypeError):
            client.create_or_update(
                unsupported_entity
            )  # should raise before any network call

    def test_from_config_raises_when_config_not_found(
        self, client: MLClient, tmp_path: Path
    ) -> None:
    ...
    def test__get_workspace_info_parses_scope_and_returns_parts(
        self, client: MLClient, tmp_path: Path
    ) -> None:
    ...
    def test__ml_client_cli_creates_client_and_repr_contains_subscription(
        self, client: MLClient
    ) -> None:
        # Use existing client's credential and subscription to create a cli client
        cli_client = MLClient._ml_client_cli(
            credentials=client._credential, subscription_id=client.subscription_id
        )
    ...
    def test_create_or_update_with_unsupported_type_raises_type_error(
        self, client: MLClient
    ) -> None:
        ...
        assert (
            "Please refer to create_or_update docstring for valid input types."
    ...
    def test_begin_create_or_update_with_unsupported_type_raises_type_error(
        ...
        assert (
            "Please refer to begin_create_or_update docstring for valid input types."
            in str(excinfo.value)
        )

    def test_ml_client_cli_returns_client_and_repr_includes_subscription(
        self, client: MLClient
    ) -> None:
...
class TestMLClientFromConfig:
    ...
    def test_from_config_with_scope_parses_scope_and_returns_client(
        self, client: MLClient, tmp_path: Path
    ) -> None:
        ...
        new_client = MLClient.from_config(
            credential=client._credential, path=str(cfg_file)
        )
