...
class TestFeatureStore(AzureRecordedTestCase):
    ...
    def test_feature_store_create_update_and_delete(
        ...
        fs = _FeatureStore(name=fs_name, description=fs_description, display_name=fs_display_name)
        fs_poller = client._feature_stores.begin_create(feature_store=fs)
        assert isinstance(fs_poller, LROPoller)

        create_fs = fs_poller.result()
        assert isinstance(create_fs, _FeatureStore)

        fs_result = client._feature_stores.get(fs_name)
        assert isinstance(fs_result, _FeatureStore)
        assert fs_result.name == fs_name

        fs_list = client._feature_stores.list()
        assert any(fs_name == fs.name for fs in fs_list)

        fs = _FeatureStore(name=fs_name, description=fs_updated_description, display_name=fs_display_name)
        fs_poller = client._feature_stores.begin_update(feature_store=fs)
        assert isinstance(fs_poller, LROPoller)

        updated_fs = fs_poller.result()
        assert isinstance(updated_fs, _FeatureStore)
        assert updated_fs.description == fs_updated_description

        fs_poller = client._feature_stores.begin_delete(name=fs_name, delete_dependent_resources=True)
        assert isinstance(fs_poller, LROPoller)
        fs_poller.result()

        with pytest.raises(Exception):
            client._feature_stores.get(fs_name)
