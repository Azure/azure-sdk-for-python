...
class TestAutoMLImageInstanceSegmentation:
    ...
    def test_image_instance_segmentation_task(self, run_type):
        ...
        if (run_type == "single") or (run_type == "sweep"):
            ...
            # image_instance_segmentation_job.limits = {"timeout": 60, "max_trials": 2, "max_concurrent_trials": 1}
    ...
    def test_set_image_model_with_valid_values(self, settings, expected):
        ...
        assert image_instance_segmentation_job.image_model.optimizer == expected[0]
        assert image_instance_segmentation_job.image_model.learning_rate_scheduler == expected[1]
        assert image_instance_segmentation_job.image_model.validation_metric_type == expected[2]
        assert image_instance_segmentation_job.image_model.model_size == expected[3]
