...
from azure.ai.ml._restclient.arm_ml_service.models import MLTableJobInput, TextNer
...
class TestAutoMLTextNerJob:
    ...
    def test_automl_nlp_text_ner_init(self, run_type):
        training_data_uri = "azureml://datastores/workspaceblobstore/paths/NLPTextNer/Train"
        validation_data_uri = "azureml://datastores/workspaceblobstore/paths/NLPTextNer/Valid"
        log_verbosity = "Debug"
    ...
    @pytest.mark.parametrize("run_type", ["single", "sweep"])
    def test_automl_nlp_text_ner_to_rest_object(self, run_type):
        ...
        # ``maxNodes`` was dropped from the arm_ml_service model; it is carried as a wire-key.
        ...
        # ``fixedParameters``/``sweepSettings``/``searchSpace`` were dropped from the arm model and
        # are carried as wire-keys built from the entity-level JSON-direct models.
    ...
    @pytest.mark.parametrize("run_type", ["single", "sweep"])
    def test_automl_nlp_text_ner_from_rest_object(self, run_type):
        log_verbosity = "Debug"
        training_data_uri = "azureml://datastores/workspaceblobstore/paths/NLPTextNer/Train"
        validation_data_uri = "azureml://datastores/workspaceblobstore/paths/NLPTextNer/Valid"
        dataset_language = "eng"
