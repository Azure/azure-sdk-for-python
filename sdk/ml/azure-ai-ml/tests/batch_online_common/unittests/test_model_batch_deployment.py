...
class TestModelBatchDeployment:
    ...
    def test_to_rest_object(self)->None:
        deployment = load_model_batch_deployment(TestModelBatchDeployment.MODEL_BATCH_DEPLOYMNET, params_override=[{"endpoint_name": "some-en-name"}])
        ...
        assert rest_deployment.properties.code_configuration.scoring_script == deployment.code_configuration.scoring_script
        ...
        assert rest_deployment.properties.max_concurrency_per_instance == deployment.settings.max_concurrency_per_instance
    ...
    def test_output_action_yaml_to_rest(self):
        assert ModelBatchDeployment._yaml_output_action_to_rest_output_action(BatchDeploymentOutputAction.APPEND_ROW) == BatchOutputAction.APPEND_ROW
        assert ModelBatchDeployment._yaml_output_action_to_rest_output_action(BatchDeploymentOutputAction.SUMMARY_ONLY) == BatchOutputAction.SUMMARY_ONLY
    ...
    def test_validation_fails_if_output_file_name_and_summary_output_action(self)->None:
        ...
        assert "When output_action is set to summary_only, the output_file_name need not to be specified." == str(ex.value)
