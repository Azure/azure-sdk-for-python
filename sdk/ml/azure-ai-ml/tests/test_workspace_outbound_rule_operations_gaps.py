...
class TestWorkspaceOutboundRuleOperationsGaps(AzureRecordedTestCase):
    @pytest.mark.e2etest
    def test_check_workspace_name_raises_validation_when_missing(self, client: MLClient) -> None:
        ...
        try:
            client.workspace_outbound_rules.get(workspace_name="", outbound_rule_name="some-rule")
            ...
            pytest.fail("Expected ValidationException or ResourceNotFoundError when workspace name missing")

    @pytest.mark.e2etest
    def test_list_outbound_rules_returns_iterable(self, client: MLClient, randstr: Callable[[], str]) -> None:
        ...
        try:
            ...
            pytest.xfail("Workspace not present in test subscription or service unavailable for listing outbound rules.")
    ...
    @pytest.mark.e2etest
    def test_check_workspace_name_raises_validation_exception(self, client: MLClient) -> None:
        ...
        try:
            client.workspace_outbound_rules.get(workspace_name="", outbound_rule_name="any-name")
        except ValidationException:
            return
        except ResourceNotFoundError:
    ...
    @pytest.mark.e2etest
    def test_list_outbound_rules_iterable_conversion(self, client: MLClient, randstr: Callable[[], str]) -> None:
        ...
        wname = getattr(client, "workspace_name", None) or f"e2etest_{randstr('wps')}_nop"
        try:
