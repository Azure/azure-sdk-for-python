...
class TestOnlineDeploymentGaps(AzureRecordedTestCase):
    ...
    def test_local_enable_gpu_raises_when_nvidia_missing(
        self,
        client: MLClient,
        rand_online_name: Callable[[], str],
        rand_online_deployment_name: Callable[[], str],
        ...
        try:
            ...
            with pytest.raises(LocalDeploymentGPUNotAvailable):
                client.online_deployments.begin_create_or_update(
                    blue_deployment, local=True, local_enable_gpu=True
                ).result()
        finally:
            client.online_endpoints.begin_delete(name=online_endpoint_name).result()

    def test_get_logs_invalid_container_type_raises_validation(
        self,
        client: MLClient,
        rand_online_name: Callable[[], str],
        rand_online_deployment_name: Callable[[], str],
...
class TestOnlineDeploymentOperationsGaps(AzureRecordedTestCase):
    def test_get_logs_invalid_container_type_raises_validation_without_endpoint(
...
class TestOnlineDeploymentLogsValidation(AzureRecordedTestCase):
    ...
    def test_get_logs_with_known_container_enum_does_not_raise_validation(
        ...
        try:
            ...
            ), "ValidationException was raised for a known EndpointDeploymentLogContainerType enum value"
