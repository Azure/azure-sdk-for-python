...
- Fixed `load_component` failing with `UserErrorException: Default value of Input cannot be set: Non-primitive type Input has no default value.` when loading a pipeline component YAML that specifies a `default` (e.g. a registered data asset reference like `azureml:my_data:1`) for a `uri_file`, `uri_folder`, `mltable`, `mlflow_model`, or `custom_model` input, matching the `pipelineComponent.schema.json` schema and documentation.
