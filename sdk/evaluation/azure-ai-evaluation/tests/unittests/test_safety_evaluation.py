...
class TestSafetyEvaluation:
    def test_validate_model_config_missing_keys(
        self, mock_credential, mock_model_config_dict_invalid
    ):
    ...
    def test_check_target_returns_context_true(
        self, safety_eval, mock_target_with_context
    ):
        assert safety_eval._check_target_returns_context(mock_target_with_context)

    def test_check_target_returns_context_async(
        self, safety_eval, mock_async_target, mock_async_target_with_context
    ):
    ...
    def test_validate_inputs_multi_turn_scenario(self, safety_eval, mock_target):
        ...
        assert (
            "not supported for content safety evaluation with more than 1 turn"
            in str(exc_info.value)
        )

    def test_validate_inputs_no_model_config(
        self, safety_eval_no_model_config, mock_target
    ):
        with pytest.raises(EvaluationException) as exc_info:
    ...
    def test_validate_inputs_scenario_not_content_safety(
        self, safety_eval, mock_target
    ):
        with pytest.raises(EvaluationException) as exc_info:
            safety_eval._validate_inputs(
                target=mock_target,
    ...
    def test_calculate_defect_rate(self, safety_eval, mock_eval_result_dict):
        ...
        assert isinstance(
            eval_result["metrics"]["content_safety.violence_defect_rate"], float
        )
    ...
    async def test_simulate_no_scenario(
        self, mock_open, mock__call__, safety_eval, mock_target
    ):
        mock_file = MagicMock()
    ...
    @pytest.mark.asyncio
    @patch(
        "azure.ai.evaluation.simulator.DirectAttackSimulator.__init__",
        return_value=None,
    )
    @patch(
        "azure.ai.evaluation.simulator.DirectAttackSimulator.__call__",
        new_callable=AsyncMock,
    )
    @patch("pathlib.Path.open", new_callable=MagicMock)
    async def test_simulate_direct_attack(
    ...
    @pytest.mark.asyncio
    @patch(
        "azure.ai.evaluation.simulator.IndirectAttackSimulator.__init__",
        return_value=None,
    )
    @patch(
        "azure.ai.evaluation.simulator.IndirectAttackSimulator.__call__",
        new_callable=AsyncMock,
    )
    @patch("pathlib.Path.open", new_callable=MagicMock)
    async def test_simulate_indirect_jailbreak(
        self, mock_open, mock_call, mock_init, safety_eval, mock_target
    ):
        mock_file = MagicMock()
        mock_open.return_value.__enter__.return_value = mock_file
    ...
    @pytest.mark.asyncio
    @patch(
        "azure.ai.evaluation.simulator.AdversarialSimulator.__init__", return_value=None
    )
    @patch(
        "azure.ai.evaluation.simulator.AdversarialSimulator.__call__",
        new_callable=AsyncMock,
    )
    @patch("pathlib.Path.open", new_callable=MagicMock)
    async def test_simulate_adversarial(
        self, mock_open, mock_call, mock_init, safety_eval, mock_target
    ):
        mock_file = MagicMock()
        mock_open.return_value.__enter__.return_value = mock_file
    ...
    async def test_simulate_no_results(
        self, mock_call, mock_init, safety_eval, mock_target
    ):
    ...
    @patch("pathlib.Path.open", new_callable=MagicMock)
    async def test_simulate_passes_randomization_seed(
        self, mock_open, mock_call, mock_init, safety_eval, mock_target
    ):
    ...
    @pytest.mark.asyncio
    @patch(
        "azure.ai.evaluation._safety_evaluation._safety_evaluation._SafetyEvaluation._simulate"
    )
    @patch("azure.ai.evaluation._evaluate._evaluate.evaluate")
    async def test_call_with_async_target(
        self, mock_evaluate, mock_simulate, safety_eval, mock_async_target
    ):
        # Setup mocks
    ...
    def test_get_evaluators_code_vulnerability(self, safety_eval):
        ...
        assert (
            evaluators["code_vulnerability"].__class__.__name__
            == "CodeVulnerabilityEvaluator"
        )

    def test_get_evaluators_ungrounded_attributes(self, safety_eval):
        evaluators = safety_eval._get_evaluators(
            [_SafetyEvaluator.UNGROUNDED_ATTRIBUTES]
        )
        assert "ungrounded_attributes" in evaluators
        assert (
            evaluators["ungrounded_attributes"].__class__.__name__
            == "UngroundedAttributesEvaluator"
        )

    def test_validate_inputs_code_vulnerability_multi_turn(
        self, safety_eval, mock_target
    ):
        with pytest.raises(EvaluationException) as exc_info:
            safety_eval._validate_inputs(
    ...
    def test_validate_inputs_ungrounded_attributes_multi_turn(
        self, safety_eval, mock_target
    ):
        with pytest.raises(EvaluationException) as exc_info:
            safety_eval._validate_inputs(
    ...
    def test_randomization_seed_consistency(self):
        ...
        assert (
            data1 == data2
        ), "Same randomization_seed should produce identical results"
    ...
    def test_local_random_no_global_state_pollution(self):
        ...
        ), "Local Random usage should not affect global state"
