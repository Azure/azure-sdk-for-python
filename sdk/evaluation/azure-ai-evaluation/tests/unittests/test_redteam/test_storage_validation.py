...
class TestStorageValidation(unittest.TestCase):
    ...
    def test_test_storage_upload_success(self, mock_rest_client_class):
        ...
        mock_pending_upload_response.blob_reference_for_consumption.credential.sas_uri = "https://test.blob.core.windows.net/container?sas_token"
        
        mock_rest_client.evaluation_results.start_pending_upload.return_value = mock_pending_upload_response
        
        # Mock ContainerClient
        with patch('azure.ai.evaluation._common.evaluation_onedp_client.ContainerClient') as mock_container_client_class:
    ...
    def test_test_storage_upload_failure(self, mock_rest_client_class):
        ...
        mock_rest_client.evaluation_results.start_pending_upload.side_effect = Exception("Storage account not accessible")
