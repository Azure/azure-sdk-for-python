...
class TestObjectiveGeneratorFeatures:
    ...
    def test_objective_generator_with_custom_attack_seed_prompts(self):
        ...
        custom_prompts_path = os.path.join(
            os.path.dirname(__file__), "data", "custom_prompts.json"
        )
        generator = _AttackObjectiveGenerator(
        ...
        assert (
            len(
                generator.valid_prompts_by_category.get(RiskCategory.Violence.value, [])
            )
            == 1
        )
        assert (
            len(
                generator.valid_prompts_by_category.get(
                    RiskCategory.HateUnfairness.value, []
                )
            )
            == 1
        )
    ...
    def test_objective_generator_with_invalid_custom_attack_seed_prompts(self):
        ...
        with pytest.raises(
            ValueError, match="Custom attack seed prompts file not found"
        ):
            _AttackObjectiveGenerator(
    ...
    def test_objective_generator_with_absolute_path(self):
        ...
        ), patch("builtins.open", mock_open(read_data=mock_json_data)), patch(
            "logging.getLogger"
        ) as mock_logger:
