...
class TestRiskCategoryEnum:
    ...
    def test_risk_category_enum_values(self):
        ...
        assert RiskCategory.ISA.value == "isa"
...
class TestObjectiveGeneratorInitialization:
    ...
    def test_objective_generator_init_custom(self):
        ...
        generator_custom = _AttackObjectiveGenerator(risk_categories=[RiskCategory.Violence], num_objectives=5)
...
class TestObjectiveGeneratorFeatures:
    ...
    def test_objective_generator_with_num_objectives_zero(self):
        ...
        generator = _AttackObjectiveGenerator(risk_categories=[RiskCategory.Violence], num_objectives=0)
    ...
    def test_objective_generator_with_custom_attack_seed_prompts(self):
        ...
        generator = _AttackObjectiveGenerator(custom_attack_seed_prompts=custom_prompts_path)
    ...
    def test_objective_generator_with_invalid_custom_attack_seed_prompts(self):
        ...
        with pytest.raises(ValueError, match="Custom attack seed prompts file not found"):
            _AttackObjectiveGenerator(custom_attack_seed_prompts="nonexistent_file.json")
    ...
    def test_objective_generator_with_relative_path(self):
        ...
        ), patch("pathlib.Path.cwd", return_value=Path("/current/dir")), patch(
            "pathlib.Path.resolve", return_value=expected_abs_path
        ), patch(
            "builtins.open", mock_open(read_data=mock_json_data)
        ):

            rel_path = "relative/path/custom_prompts.json"
            generator = _AttackObjectiveGenerator(custom_attack_seed_prompts=rel_path)
    ...
    def test_objective_generator_with_absolute_path(self):
        ...
        ), patch("builtins.open", mock_open(read_data=mock_json_data)), patch("logging.getLogger") as mock_logger:

            generator = _AttackObjectiveGenerator(custom_attack_seed_prompts="/absolute/path/custom_prompts.json")
