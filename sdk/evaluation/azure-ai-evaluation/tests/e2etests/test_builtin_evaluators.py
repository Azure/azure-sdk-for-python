...
class TestBuiltInEvaluators:
    ...
    def test_math_evaluator_rouge_score(self, rouge_type):
        ...
        assert "rouge_score" in score and "rouge_properties" in score
        ...
        assert 0 <= score["rouge_properties"]["rouge_precision"] <= 1
        assert 0 <= score["rouge_properties"]["rouge_recall"] <= 1
        assert 0 <= score["rouge_properties"]["rouge_f1_score"] <= 1
    ...
    def test_quality_evaluator_fluency(self, sanitized_model_config, simple_conversation):
        ...
        assert score is not None
        assert score["fluency"] == score["fluency_score"] > 1.0
        assert score["fluency_reason"]

        # Test conversation input
        score2 = eval_fn(conversation=simple_conversation)
        assert score2["fluency"] == score2["fluency_score"] > 0
        assert score2["evaluation_per_turn"]["fluency"][0] == score2["evaluation_per_turn"]["fluency_score"][0] > 0
        assert score2["evaluation_per_turn"]["fluency"][1] == score2["evaluation_per_turn"]["fluency_score"][1] > 0
    ...
    def test_quality_evaluator_coherence(self, sanitized_model_config, simple_conversation):
        ...
        assert score is not None
        assert score["coherence"] == score["coherence_score"] > 1.0
        assert score["coherence_reason"]

        # Test conversation input
        score2 = eval_fn(conversation=simple_conversation)
        assert score2["coherence"] == score2["coherence_score"] > 0
        assert score2["evaluation_per_turn"]["coherence"][0] == score2["evaluation_per_turn"]["coherence_score"][0] > 0
        assert score2["evaluation_per_turn"]["coherence"][1] == score2["evaluation_per_turn"]["coherence_score"][1] > 0
    ...
    def test_quality_evaluator_similarity(self, sanitized_model_config):
        ...
        assert score is not None
        assert score["similarity"] == score["similarity_score"] > 1.0
    ...
    def test_quality_evaluator_groundedness(self, sanitized_model_config, simple_conversation):
        ...
        assert score is not None
        assert score["groundedness"] == score["groundedness_score"] > 1.0
        assert score["groundedness_reason"]

        # Test conversation input
        score2 = eval_fn(conversation=simple_conversation)
        assert score2["groundedness"] == score2["groundedness_score"] > 0
        assert (
            score2["evaluation_per_turn"]["groundedness"][0]
            == score2["evaluation_per_turn"]["groundedness_score"][0]
            > 0
        )
        assert (
            score2["evaluation_per_turn"]["groundedness"][1]
            == score2["evaluation_per_turn"]["groundedness_score"][1]
            > 0
        )
    ...
    def test_quality_evaluator_relevance(self, sanitized_model_config, simple_conversation):
        ...
        assert score is not None
        assert score["relevance"] == score["relevance_score"] > 1.0

        # Test conversation input
        score2 = eval_fn(conversation=simple_conversation)
        assert score2["relevance"] == score2["relevance_score"] > 0
        assert score2["evaluation_per_turn"]["relevance"][0] == score2["evaluation_per_turn"]["relevance_score"][0] > 0
        assert score2["evaluation_per_turn"]["relevance"][1] == score2["evaluation_per_turn"]["relevance_score"][1] > 0
    ...
    def test_quality_evaluator_retrieval(self, sanitized_model_config, simple_conversation):
        ...
        assert score is not None
        assert score["retrieval"] == score["retrieval_score"] > 0.0

        # Test conversation input
        score2 = eval_fn(conversation=simple_conversation)
        assert score2["retrieval"] == score2["retrieval_score"] > 0
        assert score2["evaluation_per_turn"]["retrieval"][0] == score2["evaluation_per_turn"]["retrieval_score"][0] > 0
    ...
    def test_composite_evaluator_qa(self, sanitized_model_config, parallel):
        ...
        assert score is not None
        assert score["groundedness"] == score["groundedness_score"] > 0.0
        assert score["relevance"] == score["relevance_score"] > 0.0
    ...
    def test_composite_evaluator_qa_with_openai_config(self, non_azure_openai_model_config, parallel):
        ...
        assert score is not None
        assert score["groundedness"] == score["groundedness_score"] == score["gpt_groundedness"] > 0.0
        assert score["relevance"] == score["relevance_score"] == score["gpt_relevance"] > 0.0
