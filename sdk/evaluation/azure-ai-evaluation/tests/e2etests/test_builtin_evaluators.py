...
class TestBuiltInEvaluators:
    ...
    def test_multimodal_evaluator_content_safety_json_image_urls_text_image_input_only_with_text_content(self, project_scope, azure_cred):
        evaluator = ContentSafetyMultimodalEvaluator(
            credential=azure_cred, azure_ai_project=project_scope
        )
        conversation = {
            ...
            "messages" : [
                {
                    "role": "system",
                    "content": [
                        { "type": "text", "text": "You are AI assistant who can describe images." }
                    ]
                },
                {
                    "role": "user",
    ...
    def test_multimodal_evaluator_content_safety_inference(self, project_scope, azure_cred):
        ...
        conversation = Conversation(
            messages = [
                SystemMessage( 
                    content=[ 
                        TextContentItem(text="You are an AI assitance who can describe images.") 
                    ], 
                ), 
                UserMessage( 
                    content=[ 
                ...
                AssistantMessage( 
                    content=[ 
                        TextContentItem(text="The image shows a man with short brown hair smiling, wearing a dark-colored shirt.") 
                    ], 
                ),
            ]
        )
    ...
    def test_multimodal_evaluator_sexual_json(self, project_scope, azure_cred):
        evaluator = SexualMultimodalEvaluator(
            credential=azure_cred, azure_ai_project=project_scope
        )
        conversation = {
            "messages" : [
