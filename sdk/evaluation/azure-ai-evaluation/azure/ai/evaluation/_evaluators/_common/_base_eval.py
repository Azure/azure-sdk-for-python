...
class DerivedEvalInput(TypedDict, total=False):
    """The eval input generated by EvaluatorBase._derive_messages_starter."""
...
class EvaluatorBase(ABC, Generic[T_EvalValue]):
    ...
    or messages as input. All such evaluators need to implement two functions of their own:
        - _convert_messages_to_eval_input
        - _do_eval
    ...
    :param eval_last_turn: If True, only the last turn of the messages will be evaluated. Default is False.
    ...
    def __init__(
        self,
        *,
        not_singleton_inputs: List[str] = ["messages", "kwargs"],
    ...
    def __call__(self, **kwargs) -> Union[DoEvalResult[T_EvalValue], AggregateResult[T_EvalValue]]:
        ...
        :keyword kwargs: A dictionary that contains inputs needed to evaluate a messages.
        :type kwargs: Dict
    ...
    def _derive_singleton_inputs(self) -> List[str]:
        ...
        By default, it's assumed that any input that is NOT kwargs or a messages are singleton inputs.
    ...
    def _derive_messages_converter(self) -> Callable[[Dict], List[DerivedEvalInput]]:
        """Produce the function that will be used to convert messages to a list of evaluable inputs.
        This uses the inputs derived from the _derive_singleton_inputs function to determine which
        aspects of a messages ought to be extracted.

        :return: The function that will be used to convert messages to evaluable inputs.
        :rtype: Callable
        """
        ...
        def converter(messages: Dict) -> List[DerivedEvalInput]:
            messages = cast(List[Dict[str, Any]], messages["messages"])
            global_context = messages.get("context", None)
            # Extract queries, responses from messages
            queries: List[Dict[str, Any]] = []
            responses: List[Dict[str, Any]] = []

            # Convert messages slice into queries and responses.
    ...
    def _convert_kwargs_to_eval_input(self, **kwargs) -> Union[List[Dict], List[DerivedEvalInput]]:
        ...
        (like a query and response), or they receive messages that iss a list of dictionary
        values.

        The self._singleton_inputs list assigned during initialization is used to find and extract
        singleton keywords, and self._allow_converssation_input is used to determine if a messages
        is a valid input.

        If both messages and singletons are allowed, the function will raise an exception if both
        are inputted.

        This function must be overridden by child classes IF they need to both a messages and
        ...
        # Check that both messages and other inputs aren't set
        if messages is not None and any(singletons.values()):
            raise EvaluationException(
                message="Invalid input",
                internal_message=f"Both messages and individual inputs were provided to {type(self).__name__}",
                blame=ErrorBlame.USER_ERROR,
                category=ErrorCategory.INVALID_VALUE,
                target=ErrorTarget.MESSAGES,
            )
        # Handle messages
        if messages is not None:
            return self._derive_messages_converter()(messages)
        # Handle Singletons
        ...
        raise EvaluationException(
            ...
            internal_message=f"Neither messages nor individual inputs provided to {type(self).__name__}.",
            blame=ErrorBlame.USER_ERROR,
            category=ErrorCategory.INVALID_VALUE,
            target=ErrorTarget.MESSAGES,
        )
    ...
    def _aggregate_results(self, per_turn_results: List[DoEvalResult[T_EvalValue]]) -> AggregateResult[T_EvalValue]:
        """Aggregate the evaluation results of each messages turn into a single result.
        ...
        :param per_turn_results: List of evaluation results for each turn in the messages.
...
class AsyncEvaluatorBase:
    ...
    async def __call__(self, *, query=None, response=None, context=None, messages=None, **kwargs):
        if messages is not None:
