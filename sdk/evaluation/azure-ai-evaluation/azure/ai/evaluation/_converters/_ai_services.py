...
class AIAgentConverter:
    ...
    def __init__(self, project_client: AIProjectClient):
        ...
        self._data_retriever = AIAgentConverter._get_data_retriever(
    ...
    def _list_tool_calls_chronological(
        ...
        run_steps_chronological = self._data_retriever._list_run_steps_chronological(
    ...
    def _extract_function_tool_definitions(
        thread_run: object,
    ) -> List[Union[ToolDefinition, OpenAPIToolDefinition]]:
        """
        ...
        for tool in thread_run.tools:
            ...
            if tool.type == _FUNCTION:
                ...
                tool_definition = OpenAPIToolDefinition(
                    ...
                    default_params=openapi_tool.default_params.as_dict()
                    if openapi_tool.default_params
                    else None,
                    functions=[
                        ToolDefinition(
                            name=func.get("name"),
                ...
                if (
                    tool.type in _BUILT_IN_DESCRIPTIONS
                    and tool.type in _BUILT_IN_PARAMS
                ):
                    final_tools.append(
                        ToolDefinition(
                            type=tool.type,
    ...
    @staticmethod
    def _break_into_query_responses(
        messages: List[Message], run_id: str
    ...
    @staticmethod
    def _filter_run_ids_up_to_run_id(
        run_ids: List[str], run_id: str, include_run_id: bool = True
    ) -> List[str]:
        """
        ...
        for single_turn in ai_services_messages:
            ...
            if single_turn.role == _USER:
                final_messages.append(
                    UserMessage(content=content_list, createdAt=single_turn.created_at)
                )
                continue
    ...
    def _retrieve_tool_calls_up_to_including_run_id(
        ...
        if not exclude_tool_calls_previous_runs:
            ...
            run_ids_up_to_run_id = AIAgentConverter._filter_run_ids_up_to_run_id(
                self._data_retriever._list_run_ids_chronological(thread_id),
                run_id,
    ...
    def _retrieve_all_tool_calls(
        self, thread_id: str, run_ids: List[str]
    ) -> List[Message]:
        """
    ...
    def _is_agent_tool_call(message: Message) -> bool:
        ...
        return (
            ...
            and message.content[0]["type"]
            == _TOOL_CALL  # Not interested in assistant's toolcalls.
        )
    ...
    def convert(
        ...
        thread_run: object = self._data_retriever._get_run(
            thread_id=thread_id, run_id=run_id
        )

        # Walk through the "user-facing" conversation history and start adding messages.
        chronological_conversation = self._data_retriever._list_messages_chronological(
            thread_id
        )

        # Since this is Xth run of out possibly N runs, we are only interested is messages that are before the run X.
        chrono_until_run_id = AIAgentConverter._filter_messages_up_to_run_id(
            chronological_conversation, run_id
        )
        ...
        final_messages.extend(
            self._retrieve_tool_calls_up_to_including_run_id(
                thread_id, run_id, exclude_tool_calls_previous_runs
            )
        )
        ...
        query, responses = AIAgentConverter._break_into_query_responses(
    ...
    def _prepare_single_thread_evaluation_data(
        self, thread_id: str, filename: str = None
    ) -> List[dict]:
        """
        ...
        all_sorted_tool_calls = AIAgentConverter._sort_messages(
            self._retrieve_all_tool_calls(thread_id, run_ids)
        )

        # The last run should have all the tool definitions.
        thread_run = self._data_retriever._get_run(
            thread_id=thread_id, run_id=run_ids[-1]
        )
        instructions = thread_run.instructions
        ...
        for run_id in run_ids:
            ...
            simple_messages = AIAgentConverter._filter_messages_up_to_run_id(
                chronological_conversation, run_id
            )

            # Now we need to convert from OpenAI's general ThreadMessage model into our Azure Agents models.
            typed_simple_messages = AIAgentConverter._extract_typed_messages(
                simple_messages
            )

            # We also need to filter out the tool calls that are not from the current run.
            sorted_tool_calls = AIAgentConverter._filter_messages_up_to_run_id(
                all_sorted_tool_calls, run_id
            )
    ...
    def prepare_evaluation_data(
        self, thread_ids=Union[str, List[str]], filename: str = None
    ) -> List[dict]:
        """
        ...
        if isinstance(thread_ids, str):
            return self._prepare_single_thread_evaluation_data(
                thread_id=thread_ids, filename=filename
            )
        ...
        with ThreadPoolExecutor(max_workers=self._MAX_WORKERS) as executor:
            ...
            futures = {
                executor.submit(
                    self._prepare_single_thread_evaluation_data, str(thread_id), None
                ): thread_id
    ...
    def _convert_from_conversation(
        ...
        final_messages: List[Message] = []
        for converted_message in AIAgentConverter._filter_messages_up_to_run_id(
            converted_messages, run_id
        ):
        ...
        final_result = EvaluatorData(
            query=query, response=responses, tool_definitions=tool_definitions
        )
