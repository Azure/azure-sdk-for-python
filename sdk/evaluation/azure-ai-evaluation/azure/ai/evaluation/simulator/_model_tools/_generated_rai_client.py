...
class GeneratedRAIClient:
    ...
    async def get_attack_objectives(
        ...
        *,
        ...
        :param client_id: Optional client ID to be used when obtaining the authentication token
        :type client_id: Optional[str]
        :return: The attack objectives
        :rtype: Dict
        """
        try:
            # Store client_id temporarily for use in token retrieval
            # If client_id is provided, update the token manager to use it
            original_token = None
            if client_id:
                # Get a new token with client_id
                original_token = self.token_manager.token
                self.token_manager.token = None  # Force token refresh
                # The token manager will use client_id in the next get_token call
                self.token_manager.client_id = client_id
            ...
            # Restore original token state if client_id was used
            if client_id:
                if hasattr(self.token_manager, 'client_id'):
                    delattr(self.token_manager, 'client_id')
                self.token_manager.token = original_token
            
            return response

        except Exception as e:
            # Clean up client_id if it was set
            if client_id and hasattr(self.token_manager, 'client_id'):
                delattr(self.token_manager, 'client_id')
                if original_token:
                    self.token_manager.token = original_token
    ...
    def _fetch_or_reuse_token(self, credential: TokenCredential, token: Optional[str] = None, client_id: Optional[str] = None) -> str:
        ...
        :type token: str
        :param client_id: Optional client ID to be used when obtaining the token
        :type client_id: Optional[str]
        ...
        # Get token with optional client_id
        if client_id:
            return credential.get_token(TokenScope.DEFAULT_AZURE_MANAGEMENT, client_id=client_id).token
        else:
