...
class RedTeam:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                with open(os.path.join(tmpdir, artifact_name), "w", encoding=DefaultOpenEncoding.WRITE) as f:
                    if data_only:
                        ...
                        redteam_output.scan_result["redteaming_scorecard"] = redteam_output.scan_result.get(
                            "scorecard", None
                        )
                        redteam_output.scan_result["redteaming_parameters"] = redteam_output.scan_result.get(
                            "parameters", None
                        )
                        redteam_output.scan_result["redteaming_data"] = redteam_output.scan_result.get(
                            "attack_details", None
                        )
            ...
            try:
                ...
                if "tense" in strategy:
                    objectives_response = await self.generated_rai_client.get_attack_objectives(
                        risk_category=risk_cat_value, application_scenario=application_scenario or "", strategy="tense"
                    )
                else:
                    objectives_response = await self.generated_rai_client.get_attack_objectives(
                        risk_category=risk_cat_value, application_scenario=application_scenario or "", strategy=None
                    )
    ...
    async def _prompt_sending_orchestrator(
        ...
        try:
            ...
            if len(all_prompts) > batch_size:
                self.logger.debug(
                    f"Processing {len(all_prompts)} prompts in batches of {batch_size} for {strategy_name}/{risk_category}"
                )
                batches = [all_prompts[i : i + batch_size] for i in range(0, len(all_prompts), batch_size)]

                for batch_idx, batch in enumerate(batches):
                    self.logger.debug(
                        f"Processing batch {batch_idx+1}/{len(batches)} with {len(batch)} prompts for {strategy_name}/{risk_category}"
                    )
                    ...
                    try:
                        ...
                        self.logger.debug(
                            f"Successfully processed batch {batch_idx+1} for {strategy_name}/{risk_category} in {batch_duration:.2f} seconds"
                        )

                        # Print progress to console
                        if batch_idx < len(batches) - 1:  # Don't print for the last batch
                            print(
                                f"Strategy {strategy_name}, Risk {risk_category}: Processed batch {batch_idx+1}/{len(batches)}"
                            )

                    except asyncio.TimeoutError:
                        self.logger.warning(
                            f"Batch {batch_idx+1} for {strategy_name}/{risk_category} timed out after {timeout} seconds, continuing with partial results"
                        )
                        self.logger.debug(
                            f"Timeout: Strategy {strategy_name}, Risk {risk_category}, Batch {batch_idx+1} after {timeout} seconds.",
                            exc_info=True,
                        )
                        ...
                        log_error(
                            self.logger, f"Error processing batch {batch_idx+1}", e, f"{strategy_name}/{risk_category}"
                        )
                        self.logger.debug(
                            f"ERROR: Strategy {strategy_name}, Risk {risk_category}, Batch {batch_idx+1}: {str(e)}"
                        )
                        self.red_team_info[strategy_name][risk_category]["status"] = TASK_STATUS["INCOMPLETE"]
                        self._write_pyrit_outputs_to_file(
                            orchestrator=orchestrator,
                ...
                self.logger.debug(
                    f"Processing {len(all_prompts)} prompts in a single batch for {strategy_name}/{risk_category}"
                )
                batch_start_time = datetime.now()
                try:
                    await asyncio.wait_for(
                        orchestrator.send_prompts_async(
                            prompt_list=all_prompts, memory_labels={"risk_strategy_path": output_path, "batch": 1}
                        ),
                        timeout=timeout,  # Use provided timeout
                    )
                    batch_duration = (datetime.now() - batch_start_time).total_seconds()
                    self.logger.debug(
                        f"Successfully processed single batch for {strategy_name}/{risk_category} in {batch_duration:.2f} seconds"
                    )
                except asyncio.TimeoutError:
                    self.logger.warning(
                        f"Prompt processing for {strategy_name}/{risk_category} timed out after {timeout} seconds, continuing with partial results"
                    )
    ...
    # Replace with utility function
    def _get_orchestrators_for_attack_strategies(
        self, attack_strategy: List[Union[AttackStrategy, List[AttackStrategy]]]
    ) -> List[Callable]:
                                                ...
                                                if r.get("inputs.conversation", {}).get("messages") == messages:
                                                    ...
                                                    for risk in self.risk_categories:
                                                        ...
                                                        if (
                                                            ...
                                                            and f"outputs.{risk_value}.{risk_value}_reason" in r
                                                        ):
    ...
    async def _evaluate(
        ...
        self.logger.debug(
            f"Evaluate called with data_path={data_path}, risk_category={risk_category.value}, strategy={strategy_name}, output_path={output_path}, data_only={data_only}, scan_name={scan_name}"
        )
        if data_only:
            return None
        ...
        evaluators_dict = {
            risk_category.value: RISK_CATEGORY_EVALUATOR_MAP[risk_category](
        ...
        try:
            ...
            try:
                ...
                orchestrator = await call_orchestrator(
                    self.chat_target, all_prompts, converter, strategy_name, risk_category.value, timeout
                )
    ...
    async def scan(
        ...
        *,
        ...
        with self._start_redteam_mlflow_run(self.azure_ai_project, scan_name) as eval_run:
            ...
            self.logger.info(
                f"Total tasks: {self.total_tasks} ({len(self.risk_categories)} risk categories * {len(flattened_attack_strategies)} strategies * {len(orchestrators)} orchestrators)"
            )
            ...
            for combo_idx, (call_orchestrator, strategy, risk_category) in enumerate(combinations):
                ...
                self.logger.debug(
                    f"[{combo_idx+1}/{len(combinations)}] Creating task: {call_orchestrator.__name__} + {strategy_name} + {risk_category.value}"
                )
            ...
            if parallel_execution and orchestrator_tasks:
                ...
                for i in range(0, len(orchestrator_tasks), max_parallel_tasks):
                    ...
                    try:
                        ...
                        await asyncio.wait_for(
                            asyncio.gather(*batch), timeout=timeout * 2  # Double timeout for batches
                        )
                    except asyncio.TimeoutError:
            ...
            await self._log_redteam_results_to_mlflow(redteam_output=output, eval_run=eval_run, data_only=data_only)
