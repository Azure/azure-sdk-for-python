...
class FoundryExecutionManager:
    ...
    def _group_results_by_strategy(
        ...
        Each strategy is assigned its own JSONL data file (produced by
        ``_split_jsonl_by_strategy``) so that ``ResultProcessor.to_red_team_result``
        reads only the conversations that belong to that strategy — matching the
        legacy orchestrator path where each (strategy, risk) pair had its own file.
