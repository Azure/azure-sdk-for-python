...
def _split_evaluators_and_grader_configs(
    ...
    """
    LOGGER.info(
        f"AOAI: Splitting {len(evaluators)} evaluators into AOAI graders and standard evaluators..."
    )
    true_evaluators = {}
    aoai_graders = {}
    ...
    LOGGER.info(
        f"AOAI: Found {len(aoai_graders)} AOAI graders and {len(true_evaluators)} standard evaluators."
    )
...
def _begin_aoai_evaluation(
    ...
    **kwargs: Any,
    ...
    grader_mapping_list = list(
        _get_graders_and_column_mappings(graders, column_mappings)
    )
    LOGGER.info(
        f"AOAI: Will create {len(grader_mapping_list)} separate evaluation run(s) based on column mappings."
    )

    for idx, (selected_graders, selected_column_mapping) in enumerate(
        grader_mapping_list
    ):
        LOGGER.info(
        ...
        all_eval_run_info.append(
            _begin_single_aoai_evaluation(
                selected_graders, data, selected_column_mapping, run_name, **kwargs
            )
        )

    LOGGER.info(
        f"AOAI: Successfully created {len(all_eval_run_info)} evaluation run(s)."
    )
...
def _begin_single_aoai_evaluation(
    ...
    **kwargs: Any,
    ...
    LOGGER.info(
        f"AOAI: Preparing evaluation for {len(graders)} grader(s): {list(graders.keys())}"
    )
    grader_name_list = []
    grader_list = []
    ...
    if azure_ai_project is not None:
        try:
            ...
            credential = (
                first_grader._credential
                if first_grader._credential
            ...
            LOGGER.info(
                f"AOAI: Using Foundry client for evaluation (endpoint: {endpoint})"
            )
    ...
    LOGGER.info(
        f"AOAI: Generating data source config with {len(effective_column_mapping)} column mapping(s)..."
    )
    if data_source_config == {}:
        data_source_config = _generate_data_source_config(
            data, effective_column_mapping
        )
    LOGGER.info(
        f"AOAI: Data source config generated with schema type: {data_source_config.get('type')}"
    )

    # Create eval group
    LOGGER.info(
        f"AOAI: Creating eval group with {len(grader_list)} testing criteria..."
    )
...
def _combine_item_schemas(
    data_source_config: Dict[str, Any], kwargs: Dict[str, Any]
) -> None:
    if (
        not kwargs
    ...
    if "item_schema" in data_source_config:
        item_schema = (
            kwargs["item_schema"]["required"]
            if "required" in kwargs["item_schema"]
            else []
        )
        for key in kwargs["item_schema"]["properties"]:
            if key not in data_source_config["item_schema"]["properties"]:
                data_source_config["item_schema"]["properties"][key] = kwargs[
                    "item_schema"
                ]["properties"][key]
...
def _get_evaluation_run_results(
    ...
    LOGGER.info(
        f"AOAI: Retrieving results from {len(all_run_info)} evaluation run(s)..."
    )
    run_metrics = {}
    output_df = pd.DataFrame()
    for idx, run_info in enumerate(all_run_info):
        LOGGER.info(
            f"AOAI: Fetching results for run {idx + 1}/{len(all_run_info)} (ID: {run_info['eval_run_id']})..."
        )
    ...
    LOGGER.info(
        f"AOAI: Successfully retrieved all results. Combined dataframe shape: {output_df.shape}"
    )
...
def _get_single_run_results(
    ...
    LOGGER.info(
        f"AOAI: Eval run {run_info['eval_run_id']} completed with status: {run_results.status}"
    )
    ...
    LOGGER.info(
        f"AOAI: Processing results and calculating metrics for run {run_info['eval_run_id']}..."
    )
    ...
    for criteria_result in run_results.per_testing_criteria_results:
        ...
        LOGGER.info(
            f"AOAI: Grader '{grader_name}': {passed} passed, {failed} failed, pass_rate={ratio:.4f}"
        )

    # Collect all results with pagination
    LOGGER.info(
        f"AOAI: Collecting output items for run {run_info['eval_run_id']} with pagination..."
    )
    ...
    LOGGER.info(
        f"AOAI: Collected {len(all_results)} total output items across all pages."
    )
    ...
    if expected is not None:
        ...
        LOGGER.info(
            f"AOAI: Validating result count: expected {expected} rows, received {pre_len} rows."
        )
    ...
    if "__azure_ai_evaluation_index" in output_df.columns:
        output_df.drop(
            columns=["__azure_ai_evaluation_index"], inplace=True, errors="ignore"
        )
...
def _get_graders_and_column_mappings(
    ...
    if column_mappings is None:
        LOGGER.info(
            "AOAI: No column mappings provided, each grader will have its own eval run."
        )
    ...
    return [
        (
            ...
            (
                None
                if column_mappings is None
                else column_mappings.get(name, default_mapping)
            ),
        )
        for name, grader in graders.items()
    ]
...
def _generate_data_source_config(
    ...
    LOGGER.info(
        f"AOAI: Found {len(referenced_paths)} referenced paths in column mappings: {referenced_paths}"
    )
    ...
    if not referenced_paths or not has_nested:
        ...
        LOGGER.info(
            "AOAI: Using flat schema generation (no nested structures detected)."
        )
        data_source_config = {
            "type": "custom",
            "item_schema": {
        ...
        for key in column_mapping.keys():
            if (
                key in input_data_df
                and len(input_data_df[key]) > 0
                and isinstance(input_data_df[key].iloc[0], list)
            ):
            ...
            req.append(key)
        LOGGER.info(
            f"AOAI: Flat schema generated with {len(props)} properties: {list(props.keys())}"
        )
    ...
    if len(first_segments) == 1:
        ...
        if only_seg == WRAPPER_KEY:
            ...
            LOGGER.info(
                f"AOAI: All paths start with wrapper '{WRAPPER_KEY}', will strip from schema."
            )
    ...
    if strip_wrapper:
        ...
        if stripped:
            ...
            LOGGER.info(
                f"AOAI: Effective paths after stripping wrapper: {effective_paths}"
            )

    LOGGER.info(
        f"AOAI: Building nested schema from {len(effective_paths)} effective paths..."
    )
    nested_schema = _build_schema_tree_from_paths(
        effective_paths, force_leaf_type="string"
    )

    LOGGER.info(
        f"AOAI: Nested schema generated successfully with type '{nested_schema.get('type')}'"
    )
    return {
        "type": "custom",
...
def _get_data_source(
    ...
    for name, formatted_entry in column_mapping.items():
        if not (
            isinstance(formatted_entry, str)
            and formatted_entry.startswith("${")
            and formatted_entry.endswith("}")
        ):
            continue
    ...
    LOGGER.info(
        f"AOAI: Processed {len(path_specs)} path specifications from column mappings."
    )
...
def _begin_eval_run(
    ...
    LOGGER.info(
        f"AOAI: Creating eval run '{run_name}' for eval group {eval_group_id}..."
    )
    ...
    eval_run = client.evals.runs.create(
        ...
        data_source=cast(
            Any, data_source
        ),  # Cast for type checker: dynamic schema dict accepted by SDK at runtime
        name=run_name,
        metadata={
...
def _wait_for_run_conclusion(
    ...
    while True:
        ...
        LOGGER.info(
            f"AOAI: Polling iteration {iters}, status: {response.status}, total wait: {total_wait:.1f}s"
        )
        if response.status not in ["queued", "in_progress"]:
            LOGGER.info(
                f"AOAI: Eval run {eval_run_id} reached terminal status: {response.status}"
            )
            return response
        if total_wait > max_wait_seconds:
            raise EvaluationException(
