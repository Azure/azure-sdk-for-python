...
- Management and send operations now bound AMQP link acquisition by the caller's timeout and deduct the time spent from the operation itself. Previously only the operation was timed, so a link that never became ready could block indefinitely even when a timeout was supplied.
