...
- Fixed a bug where `PEEK_LOCK` settlements on the pure-Python AMQP transport were sent pre-settled, so the service's terminal outcome (including `com.microsoft:message-lock-lost` rejections) was discarded and a settlement the service never applied was indistinguishable from success — surfacing only as the message being redelivered at lock expiry. Receivers now accept `await_settlement_outcome=True`, which sends the disposition unsettled and waits for the service to confirm it, matching the .NET, Java, JavaScript, and Go SDKs. Opt-in; the default is unchanged and it is not supported with `uamqp_transport=True`.
