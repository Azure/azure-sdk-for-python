...
class TestServiceBusQueue(AzureMgmtRecordedTestCase):
        ...
        def message_content():
            for i in range(5):
                ...
                assert (
                    ...
                    == message.raw_amqp_message.properties.creation_time + message.raw_amqp_message.header.time_to_live
                )
    ...
    def test_queue_message_amqp_over_websocket(
        ...
        **kwargs
        ...
        with ServiceBusClient(servicebus_fully_qualified_namespace, DefaultAzureCredential(), uamqp_transport=uamqp_transport, transport_type=TransportType.AmqpOverWebsocket) as sb_client:
            with sb_client.get_queue_sender(servicebus_queue.name) as sender:
    ...
    def test_queue_message_http_proxy_setting(self, uamqp_transport):
        ...
        sb_client = ServiceBusClient.from_connection_string(
            mock_conn_str, http_proxy=http_proxy, uamqp_transport=uamqp_transport
        )
    ...
    def test_queue_send_timeout(
        ...
        **kwargs
        ...
        if not uamqp_transport:
            # Amqp
            with ServiceBusClient(
                servicebus_fully_qualified_namespace, DefaultAzureCredential(), uamqp_transport=uamqp_transport
            ) as sb_client:
    ...
    def test_queue_receive_iterator_resume_after_link_detach(
        ...
        **kwargs
        ...
        with sb_client.get_queue_sender(servicebus_queue.name) as sender:
            sender.send_messages([ServiceBusMessage("test1"), ServiceBusMessage("test2"), ServiceBusMessage("test3")])
        with sb_client.get_queue_receiver(servicebus_queue.name, max_wait_time=10) as receiver:
