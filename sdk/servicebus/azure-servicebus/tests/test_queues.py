...
class ServiceBusQueueTests(AzureMgmtTestCase):
    ...
    def test_queue_by_servicebus_conn_str_client_iter_messages_with_abandon(self, servicebus_namespace_connection_string, servicebus_queue, **kwargs):
        ...
        with ServiceBusClient.from_connection_string(
            ...
            with sb_client.get_queue_receiver(servicebus_queue.name, max_wait_time=5, mode=ReceiveSettleMode.PeekLock) as receiver:
            ...
            with sb_client.get_queue_receiver(servicebus_queue.name, max_wait_time=20, mode=ReceiveSettleMode.PeekLock) as receiver:
                count = 0
                for message in receiver:
                    print_message(_logger, message)
                    message.complete()
        ...
        with ServiceBusClient.from_connection_string(
            ...
            with sb_client.get_queue_sender(servicebus_queue.name) as sender:
                ...
                with sb_client.get_queue_receiver(servicebus_queue.name, max_wait_time=5) as receiver:
                    ...
                    time_1 = utc_now()
                    for message in receiver.receive_forever(max_wait_time=10):
                        messages.append(message)
                        time_2 = utc_now()
                        for message in receiver.receive_forever(max_wait_time=1):
                            messages.append(message)
                        time_3 = utc_now()
                    ...
                    for message in receiver.receive_forever(max_wait_time=3):
                        messages.append(message)
                    time_5 = utc_now()
