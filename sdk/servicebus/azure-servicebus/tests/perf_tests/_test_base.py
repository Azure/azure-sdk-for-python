...
class _ReceiveTest:
    ...
    def add_arguments(parser) -> None:
        parser.add_argument(
            "--message-size", nargs="?", type=int, help="Size of a single message. Defaults to 100 bytes", default=100
        )
        parser.add_argument(
            "--num-messages",
            nargs="?",
            type=int,
            help="Maximum number of messages to receive. Defaults to 100",
            default=100,
        )
        parser.add_argument(
            "--peeklock", action="store_true", help="Receive using PeekLock mode and message settlement.", default=False
        )
        parser.add_argument(
            "--uamqp-transport",
            action="store_true",
        ...
        parser.add_argument(
            ...
            nargs="?",
            type=int,
            help="Max time to wait for messages before closing. Defaults to 0.",
            default=0,
        )
        parser.add_argument(
            "--preload", nargs="?", type=int, help="Number of messages to preload. Default is 10000.", default=10000
        )
...
class _SubscriptionReceiveTest(_ReceiveTest, PerfStressTest):
    def __init__(self, arguments) -> None:
        ...
        self.async_receiver = self.async_servicebus_client.get_subscription_receiver(
            topic_name=self.topic_name, subscription_name=self.subscription_name
        )
...
class _SendTest(BatchPerfTest):
    ...
    def add_arguments(parser) -> None:
        ...
        parser.add_argument(
            ...
            nargs="?",
            type=int,
            help="Size of a single batch message. Defaults to 100 messages",
            default=100,
        )
        parser.add_argument(
            "--uamqp-transport",
