...
class BaseHandler:
    ...
    async def _get_management_handler(self):
        if self._connection:
            return await self._servicebus_client._get_management_handler(self._entity_uri)  # pylint: disable=protected-access
        return self._handler

    async def _close_management_handler(self):
        if self._connection:
            await self._servicebus_client._close_management_handler(self._entity_uri)  # pylint: disable=protected-access
        else:
            await self._close_handler()
