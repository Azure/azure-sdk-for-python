...
class Connection(object):  # pylint:disable=too-many-instance-attributes
    ...
    def _outgoing_empty(self):
        ...
        if self._network_trace:
            _LOGGER.debug("[Connection:%s] -> EmptyFrame()", self._network_trace_params["amqpConnection"])
        try:
            raise self._error
        except TypeError:
            pass
    ...
    def _outgoing_header(self):
        ...
        self._last_frame_sent_time = time.time()
        if self._network_trace:
            _LOGGER.debug("[Connection:%s] -> Header(%r)", self._network_trace_params["amqpConnection"], HEADER_FRAME)
    ...
    def _incoming_header(self, _, frame):
        ...
        """
        if self._network_trace:
            _LOGGER.debug("[Connection:%s] <- Header(%r)", self._network_trace_params["amqpConnection"], frame)
    ...
    def _outgoing_open(self):
        ...
        if self._network_trace:
            _LOGGER.debug("[Connection:%s] -> %r", self._network_trace_params["amqpConnection"], open_frame)
    ...
    def _incoming_open(self, channel, frame):
        ...
        if self._network_trace:
            _LOGGER.debug("[Connection:%s] <- %r", self._network_trace_params["amqpConnection"], OpenFrame(*frame))
        if channel != 0:
            _LOGGER.error("[Connection:%s] OPEN frame received on a channel that is not 0.", self._network_trace_params["amqpConnection"])
            self.close(
                error=AMQPError(
        ...
        if self.state == ConnectionState.OPENED:
            _LOGGER.error("[Connection:%s] OPEN frame received in the OPENED state.", self._network_trace_params["amqpConnection"])
            self.close()
        if frame[4]:
    ...
    def _outgoing_close(self, error=None):
        ...
        if self._network_trace:
            _LOGGER.debug("[Connection:%s] -> %r", self._network_trace_params["amqpConnection"], close_frame)
    ...
    def _incoming_close(self, channel, frame):
        ...
        """
        if self._network_trace:
            _LOGGER.debug("[Connection:%s] <- %r", self._network_trace_params["amqpConnection"], CloseFrame(*frame))
        disconnect_states = [
    ...
    def _process_incoming_frame(self, channel, frame):  # pylint:disable=too-many-return-statements
        ...
        try:
            ...
            if performative == 1:
                return False
            _LOGGER.error("[Connection:%s] Unrecognized incoming frame: %r", self._network_trace_params["amqpConnection"], frame)
            return True
        except KeyError:
