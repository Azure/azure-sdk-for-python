...
class TestServiceBusNamespacesOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_namespaces_begin_failover(self, resource_group):
        response = await (
            await self.client.namespaces.begin_failover(
                resource_group_name=resource_group.name,
                namespace_name="str",
