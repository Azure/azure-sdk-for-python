...
class TestMgmtWorkloadsSapVirtualInstance(AzureMgmtRecordedTestCase):
    ...
    def test_sap_virtual_instance(self, resource_group):
        ...
        identity_resource_id, identity_principal_id = self.create_user_assigned_identity(resource_group.name, AZURE_LOCATION, IDENTITY_NAME)
        ...
        ssh_public_key, ssh_private_key = self.create_ssh_key_pair(resource_group.name, AZURE_LOCATION, SSH_PUBLIC_KEY_NAME)
        ...
        poller = self.mgmt_client.sap_virtual_instances.begin_create(resource_group.name, SAP_VIRTUAL_INSTANCE_NAME, BODY)
        c = 0
        while poller.done() != True:
            time.sleep(60)
            c += 1
            if c == 120:
                break
