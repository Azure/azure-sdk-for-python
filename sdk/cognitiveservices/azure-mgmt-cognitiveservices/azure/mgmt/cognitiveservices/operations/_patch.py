...
from azure.mgmt.core.polling.arm_polling import ARMPolling, BodyContentPolling, StatusCheckPolling
...
def _compute_provisioning_error(response: Any) -> HttpResponseError:
    """Build a descriptive error from a compute resource whose provisioning failed.

    Because the poller watches the compute resource (not the operation-status endpoint), the failure
    detail lives in the compute's ``properties.errors``. Surface that instead of azure-core's generic
    ``Operation returned an invalid status 'OK'`` (which happens because the terminal poll is a plain
    HTTP 200 with no OData error body).
    """
    state = None
    detail = ""
    try:
        ...
        for err in props.get("errors") or []:
            if isinstance(err, dict):
                ...
                parts.append(f"{code}: {message}" if code and message else str(message or code))
        detail = "; ".join(p for p in parts if p)
    except Exception:  # pylint: disable=broad-except
        pass
    text = f"Compute provisioning {state or 'failed'}."
    if detail:
        text = f"{text} {detail}"
    return HttpResponseError(response=response, message=text)


class _ComputeResourcePolling(ARMPolling):
    """``ARMPolling`` that tolerates the compute-create read-after-write window.
    ...
    * its poller follows the ``Azure-AsyncOperation`` header to
    ...
    def begin_create_or_update(
        ...
        **kwargs: Any,
        ...
        response.read()  # load the body so the poller/deserializer can read it
        ...
        # ``_ComputeResourcePolling`` additionally tolerates the ~20s read-after-write window where the
        # resource GET returns 404 "Cluster not found" right after the create is accepted.
        if polling is True:
            polling_method: PollingMethod = cast(
                PollingMethod,
                _ComputeResourcePolling(
                    lro_delay,
