...
### Features Added
  ...
  - Operation group `AccountsOperations` added parameter `content_type` in method `regenerate_key`
  ...
  - Operation group `RaiBlocklistItemsOperations` added parameter `content_type` in method `batch_delete`
...
### Breaking Changes
  ...
  - Deleted operation groups `ManagedComputeDeploymentsOperations`/`ComputeOperationsOperations`/`ManagedComputeUsagesOperationGroupOperations`/`ComputesOperations`/`WorkbenchesOperations`/`ManagedComputeCapacitiesOperations`
  ...
  - Method `ProjectConnectionsOperations.list` changed its parameter `category`/`include_all`/`target` from `positional_or_keyword` to `keyword_only`
...
### Other Changes
  ...
  - Deleted model `AccountListResult`/`AccountModelListResult`/`CommitmentPlanAccountAssociationListResult`/`CommitmentPlanListResult`/`CommitmentTierListResult`/`ConnectionPropertiesV2BasicResourceArmPaginatedResult`/`DefenderForAISettingResult`/`DeploymentListResult`/`DeploymentSkuListResult`/`EncryptionScopeListResult`/`ErrorDetailAutoGenerated`/`ErrorResponseAutoGenerated`/`ModelCapacityListResult`/`ModelListResult`/`NetworkSecurityPerimeterConfigurationList`/`OperationListResult`/`ProjectListResult`/`QuotaTierListResult`/`RaiBlockListItemsResult`/`RaiBlockListResult`/`RaiContentFilterListResult`/`RaiPolicyListResult`/`RaiTopicResult`/`ResourceSkuListResult` which actually were not used by SDK users
