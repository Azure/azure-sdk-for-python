...
# --------------------------------------------------------------------------
"""Async unit tests for Cost Control request serialization and response handling."""
...
class _FakeAsyncHttpResponse:
    ...
    async def read(self, *args, **kwargs):  # pylint: disable=unused-argument
        return self.content
...
def _make_client(status_code=200, body=None, headers=None):
    ...
    def send(request, **kwargs):  # pylint: disable=unused-argument
        response = _FakeAsyncHttpResponse(request, status_code, body, headers)
        return SimpleNamespace(http_request=request, http_response=response, context={})

    client._client._pipeline.run = AsyncMock(side_effect=send)  # pylint: disable=protected-access
    return client
...
@pytest.mark.asyncio
async def test_create_serializes_thresholds_and_if_none_match():
    client = _make_client(201, COST_CONTROL_RESPONSE, {"ETag": '"response-etag"'})

    result = await client.cost_controls.create_or_update(
        "rg",
        "acct",
        "monthly-budget",
        COST_CONTROL,
...
@pytest.mark.asyncio
async def test_get_deserializes_cost_control_and_etag_header():
    client = _make_client(200, COST_CONTROL_RESPONSE, {"ETag": '"response-etag"'})

    result, response_headers = await client.cost_controls.get(
        "rg",
        "acct",
        "monthly-budget",
...
@pytest.mark.asyncio
async def test_update_sends_if_match_and_replacement_rules():
    ...
    await client.cost_controls.update(
        "rg",
        "acct",
        "monthly-budget",
        patch,
...
@pytest.mark.asyncio
async def test_list_deserializes_cost_controls():
    client = _make_client(200, {"value": [COST_CONTROL_RESPONSE], "nextLink": None})

    result = [cost_control async for cost_control in client.cost_controls.list("rg", "acct")]
...
@pytest.mark.asyncio
async def test_delete_sends_if_match():
    client = _make_client(204)

    result = await client.cost_controls.delete(
        "rg",
        "acct",
        "monthly-budget",
        etag='"body-etag"',
...
@pytest.mark.asyncio
async def test_account_update_serializes_explicit_null_cost_control_connections():
    client = _make_client(
        200,
        {
    ...
    await client.accounts.begin_update(
        "rg",
        "acct",
        {"properties": {"costControlConnections": None}},
        polling=False,
