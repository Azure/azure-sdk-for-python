...
class TestNetAppSubvolume(AzureMgmtRecordedTestCase):
    ...
    def test_crud_subvolumes(self):
        ...
        try:
            create_volume(self.client, setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1, setup.LOCATION, vnet=setup.PERMA_VNET, volume_only=True, enable_subvolumes="Enabled")

            # create
            subvolume = create_subvolume(self.client, setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1, subvolumeName, "/sub_vol_1.txt", 1000000)
            
            assert subvolume.name == setup.PERMA_ACCOUNT + "/" + setup.PERMA_POOL + "/" + volumeName1 + "/" + subvolumeName
            ...
            subvolume = self.client.subvolumes.begin_update(setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1, subvolumeName, subvolume_patch).result()
            print("\tDone")
            wait_for_subvolume(self.client, setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1, subvolumeName)
            ...
            # get
            subvolume_info = self.client.subvolumes.get(setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1, subvolumeName)
            assert subvolume_info.name == setup.PERMA_ACCOUNT + "/" + setup.PERMA_POOL + "/" + volumeName1 + "/" + subvolumeName
            assert subvolume_info.path == "/sub_vol_update.txt"
        finally:
            # delete
            delete_subvolume(self.client, setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1, subvolumeName)
            subvolume_list = self.client.subvolumes.list_by_volume(setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1)
    ...
    def test_list_by_volume(self):
        ...
        try:
            create_volume(self.client, setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1, setup.LOCATION, vnet=setup.PERMA_VNET, enable_subvolumes="Enabled")

            # create
            subvolume = create_subvolume(self.client, setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1, subvolumeName, "/sub_vol_2.txt", 1000000)
            subvolume2 = create_subvolume(self.client, setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1, subvolumeName2, "/sub_vol_3.txt", 2000000)
            ...
            delete_subvolume(self.client, setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1, subvolumeName2)
            subvolume_list = self.client.subvolumes.list_by_volume(setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1)
            assert len(list(subvolume_list)) == 0
        finally:
            # clean up
    ...
    def test_get_metadata(self):
        ...
        try:
            ...
            # create
            subvolume = create_subvolume(self.client, setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1, subvolumeName, "/sub_vol_4.txt", 123)
            assert subvolume.name == setup.PERMA_ACCOUNT + "/" + setup.PERMA_POOL + "/" + volumeName1 + "/" + subvolumeName
            assert subvolume.path == "/sub_vol_4.txt"

            # get metadata
            metadata = self.client.subvolumes.begin_get_metadata(
                setup.TEST_RG, setup.PERMA_ACCOUNT, setup.PERMA_POOL, volumeName1, subvolumeName).result()
