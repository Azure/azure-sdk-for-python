...
class VolumesOperations(object):
    ...
    def _replication_status_method_initial(
            self, resource_group_name, account_name, pool_name, volume_name, custom_headers=None, raw=False, **operation_config):
        # Construct URL
    ...
    def replication_status_method(
        ...
        :param polling: True for ARMPolling, False for no polling, or a
         polling object for personal polling strategy
        :return: An instance of LROPoller that returns ReplicationStatus or
         ClientRawResponse<ReplicationStatus> if raw==True
        :rtype:
         ~msrestazure.azure_operation.AzureOperationPoller[~azure.mgmt.netapp.models.ReplicationStatus]
         or
         ~msrestazure.azure_operation.AzureOperationPoller[~msrest.pipeline.ClientRawResponse[~azure.mgmt.netapp.models.ReplicationStatus]]
        :raises: :class:`CloudError<msrestazure.azure_exceptions.CloudError>`
        """
        raw_result = self._replication_status_method_initial(
            resource_group_name=resource_group_name,
            account_name=account_name,
            pool_name=pool_name,
            volume_name=volume_name,
