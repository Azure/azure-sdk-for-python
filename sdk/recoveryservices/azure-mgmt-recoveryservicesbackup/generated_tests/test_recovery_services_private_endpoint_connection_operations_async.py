...
class TestRecoveryServicesPrivateEndpointConnectionOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_private_endpoint_connection_begin_put(self, resource_group):
        response = await (
            await self.client.private_endpoint_connection.begin_put(
                vault_name="str",
                resource_group_name=resource_group.name,
                private_endpoint_connection_name="str",
                parameters={
