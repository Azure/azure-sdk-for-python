...
class TestMgmtNetwork(AzureMgmtRecordedTestCase):
    ...
    def create_virtual_hub(self, location, group_name, virtual_wan_name, virtual_hub_name):
        ...
        BODY = {"location": location, "tags": {"key1": "value1"}, "disable_vpn_encryption": False, "type": "Basic"}
    ...
    def create_vm(self, group_name, location, vm_name, network_name, subnet_name, interface_name):
        ...
        BODY = {
            ...
            "network_profile": {"network_interfaces": [{"id": nic_id, "properties": {"primary": True}}]},
        }
    ...
    def create_vm_extension(self, group_name, location, vm_name, vm_extension_name):
        ...
        result = self.compute_client.virtual_machine_extensions.begin_create_or_update(
            group_name, vm_name, vm_extension_name, BODY
        )
        result = result.result()
    ...
    def create_virtual_network_gateway(
        self, group_name, location, vn_gateway, network_name, subnet_name, public_ip_address_name, ipconfig_name
    ):
        # create network
        subnet = self.create_virtual_network(group_name, location, network_name, subnet_name)
    ...
    def test_network_watcher_troubleshoot(self, resource_group):
        ...
        result = self.mgmt_client.network_watchers.begin_get_troubleshooting(
            resource_group.name, NETWORK_WATCHER_NAME, BODY
        )
        result = result.result()
    ...
    def test_network_watcher_ip_flow(self, resource_group):
        ...
        if self.is_live:
            self.create_vm(
                RESOURCE_GROUP, AZURE_LOCATION, VIRTUAL_MACHINE_NAME, VIRTUAL_NETWORK_NAME, SUBNET_NAME, INTERFACE_NAME
            )
            self.create_vm_extension(
                RESOURCE_GROUP, AZURE_LOCATION, VIRTUAL_MACHINE_NAME, VIRTUAL_MACHINE_EXTENSION_NAME
            )
    ...
    def test_network_watcher_flow_log(self, resource_group):
        ...
        result = self.mgmt_client.network_security_groups.begin_create_or_update(
            resource_group.name, NETWORK_SECURITY_GROUP_NAME, BODY
        )
        result = result.result()
        ...
        result = self.mgmt_client.flow_logs.begin_create_or_update(
            resource_group.name, NETWORK_WATCHER_NAME, FLOW_LOG_NAME, BODY
        )
        result = result.result()
    ...
    def test_network_watcher_monitor(self, resource_group):
        ...
        result = self.mgmt_client.connection_monitors.begin_create_or_update(
            resource_group.name, NETWORK_WATCHER_NAME, CONNECTION_MONITOR_NAME_V2, BODY
        )
        result = result.result()
        ...
        result = self.mgmt_client_v190601.connection_monitors.begin_create_or_update(
            resource_group.name, NETWORK_WATCHER_NAME, CONNECTION_MONITOR_NAME, BODY
        )
        result = result.result()

        # Get connection monitor[get]
        result = self.mgmt_client.connection_monitors.get(
            resource_group.name, NETWORK_WATCHER_NAME, CONNECTION_MONITOR_NAME
        )
        ...
        # Delete connection monitor[delete]
        result = self.mgmt_client.connection_monitors.begin_delete(
            resource_group.name, NETWORK_WATCHER_NAME, CONNECTION_MONITOR_NAME_V2
        )
        result = result.result()
    ...
    def test_network_watcher_packet_capture(self, resource_group):
        ...
        result = self.mgmt_client.packet_captures.begin_create(
            resource_group.name, NETWORK_WATCHER_NAME, PACKET_CAPTURE_NAME, BODY
        )
        result = result.result()
        ...
        result = self.mgmt_client.packet_captures.begin_get_status(
            resource_group.name, NETWORK_WATCHER_NAME, PACKET_CAPTURE_NAME
        )
        result = result.result()
    ...
    def test_network_watcher(self, resource_group):
        ...
        result = self.mgmt_client.network_watchers.begin_get_vm_security_rules(
            resource_group.name, NETWORK_WATCHER_NAME, BODY
        )
        result = result.result()
        ...
        # Get Topology[post]
        BODY = {"target_resource_group_name": resource_group.name}
    ...
    def test_network(self, resource_group):
        ...
        result = self.mgmt_client.security_rules.begin_create_or_update(
            resource_group.name, NETWORK_SECURITY_GROUP_NAME, SECURITY_RULE_NAME, BODY
        )
        result = result.result()
        ...
        result = self.mgmt_client.default_security_rules.get(
            resource_group.name, NETWORK_SECURITY_GROUP_NAME, DEFAULT_SECURITY_RULE_NAME
        )

        # Get network security rule in network security group[get]
        result = self.mgmt_client.security_rules.get(
            resource_group.name, NETWORK_SECURITY_GROUP_NAME, SECURITY_RULE_NAME
        )
