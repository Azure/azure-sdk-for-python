...
class TestMgmtNetwork(AzureMgmtRecordedTestCase):
    ...
    def setup_method(self, method):
        self.mgmt_client = self.create_mgmt_client(az_network.NetworkManagementClient)

        if self.is_live:
            import azure.mgmt.privatedns as az_privatedns

            self.dns_client = self.create_mgmt_client(az_privatedns.PrivateDnsManagementClient)
    ...
    def create_virtual_network(self, group_name, location, network_name, subnet_name1, subnet_name2):
        ...
        async_subnet_creation = self.mgmt_client.subnets.begin_create_or_update(
            ...
            network_name,
            subnet_name2,
            {"address_prefix": "10.0.1.0/24", "private_endpoint_network_policies": "disabled"},
        )
    ...
    def create_private_dns_zone(self, group_name, zone_name):
        if self.is_live:
            ...
            zone = self.dns_client.private_zones.begin_create_or_update(group_name, zone_name, {"location": "global"})
    ...
    def test_network(self, resource_group):
        ...
        subnet, _ = self.create_virtual_network(
            RESOURCE_GROUP, AZURE_LOCATION, VIRTUAL_NETWORK_NAME, SUBNET_NAME_1, SUBNET_NAME_2
        )
        ...
        result = self.mgmt_client.private_link_services.list_private_endpoint_connections(
        ...
        result = self.mgmt_client.private_link_services.list_auto_approved_private_link_services_by_resource_group(
            resource_group_name=RESOURCE_GROUP, location=LOCATION
        )
