...
class TestNetworkManagementVirtualNetworkGatewaysOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_virtual_network_gateways_begin_get_effective_routes(self, resource_group):
        response = await (
            await self.client.virtual_network_gateways.begin_get_effective_routes(
                resource_group_name=resource_group.name,
                virtual_network_gateway_name="str",
                api_version="2025-09-01",
            )
