...
class TestNetworkManagementPublicIPPrefixesOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_public_ip_prefixes_get(self, resource_group):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_public_ip_prefixes_begin_create_or_update(self, resource_group):
        response = await (
            await self.client.public_ip_prefixes.begin_create_or_update(
                resource_group_name=resource_group.name,
                public_ip_prefix_name="str",
                parameters={
                    "customIPPrefix": {"id": "str"},
                    "etag": "str",
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_public_ip_prefixes_update_tags(self, resource_group):
        response = await self.client.public_ip_prefixes.update_tags(
            resource_group_name=resource_group.name,
            public_ip_prefix_name="str",
