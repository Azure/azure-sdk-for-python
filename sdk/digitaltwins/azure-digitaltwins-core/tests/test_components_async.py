...
from azure.core.exceptions import ResourceNotFoundError, HttpResponseError, ResourceModifiedError
...
class TestDigitalTwinsComponentAsync(AzureRecordedTestCase):
    ...
    async def test_update_component_add(self, recorded_test, digitaltwin):
        ...
        patch = [{"op": "add", "path": "/ComponentProp1", "value": "5"}]
    ...
    async def test_update_component_multiple(self, recorded_test, digitaltwin):
        ...
        await self._set_up_models(client)
        patch = [
            {"op": "replace", "path": "/ComponentProp1", "value": "value2"},
            {
                "op": "remove",
                "path": "/ComponentProp1",
            },
    ...
    async def test_update_component_invalid_patch(self, recorded_test, digitaltwin):
        ...
        patch = [{"op": "add", "path": "/ComponentProp2", "value": "5"}]
        with pytest.raises(HttpResponseError):
            await client.update_component(DIGITAL_TWIN_ID, "Component1", patch)

        patch = [{"op": "replace", "path": "/ComponentProp1", "value": 42}]
        with pytest.raises(HttpResponseError):
            await client.update_component(DIGITAL_TWIN_ID, "Component1", patch)
    ...
    async def test_update_component_conditionally_if_not_modified(self, recorded_test, digitaltwin):
        ...
        await client.update_component(
            DIGITAL_TWIN_ID, "Component1", patch, match_condition=MatchConditions.IfNotModified, etag=twin["$etag"]
        )
        component = await client.get_component(DIGITAL_TWIN_ID, "Component1")
        assert "ComponentProp1" in component
        assert component["ComponentProp1"] == "value2"
    ...
    async def test_update_component_conditionally_if_present(self, recorded_test, digitaltwin):
        ...
        await self._set_up_models(client)
        patch = [{"op": "replace", "path": "/ComponentProp1", "value": "value2"}]
        await client.update_component(DIGITAL_TWIN_ID, "Component1", patch, match_condition=MatchConditions.IfPresent)
        component = await client.get_component(DIGITAL_TWIN_ID, "Component1")
        assert "ComponentProp1" in component
        assert component["ComponentProp1"] == "value2"
    ...
    async def test_update_component_invalid_conditions(self, digitaltwin):
        ...
        with pytest.raises(ValueError):
            await client.update_component(
                DIGITAL_TWIN_ID, "Component1", patch, match_condition=MatchConditions.IfPresent, etag="etag-value"
            )

        with pytest.raises(ValueError):
            await client.update_component(
                DIGITAL_TWIN_ID, "Component1", patch, match_condition=MatchConditions.IfNotModified
            )

        with pytest.raises(ValueError):
            await client.update_component(
                DIGITAL_TWIN_ID, "Component1", patch, match_condition=MatchConditions.IfModified, etag="etag-value"
            )

        with pytest.raises(ValueError):
            await client.update_component(
                DIGITAL_TWIN_ID, "Component1", patch, match_condition=MatchConditions.IfMissing
            )
    ...
    async def test_publish_component_telemetry(self, recorded_test, digitaltwin):
        ...
        client = self._get_client(digitaltwin["endpoint"])
        await self._set_up_models(client)
        published = await client.publish_component_telemetry(DIGITAL_TWIN_ID, "Component1", telemetry)
    ...
    async def test_publish_component_telemetry_with_message_id(self, recorded_test, digitaltwin):
        ...
        published = await client.publish_component_telemetry(
            DIGITAL_TWIN_ID, "Component1", telemetry, message_id=self.create_random_name("message-")
        )
        assert published is None
    ...
    async def test_publish_component_telemetry_not_existing(self, recorded_test, digitaltwin):
        ...
        client = self._get_client(digitaltwin["endpoint"])
        with pytest.raises(ResourceNotFoundError):
            await client.publish_component_telemetry("foo", "Component1", telemetry)
        with pytest.raises(HttpResponseError):
            await client.publish_component_telemetry("foo", "Component2", telemetry)
