...
### Other Changes
  ...
  - Deleted model `ChatTranscriptsListResult`/`CommunicationsListResult`/`FilesListResult`/`OperationsListResult`/`ProblemClassificationsListResult`/`ServicesListResult`/`SupportTicketsListResult` which actually were not used by SDK users
