...
class TestLogAnalyticsManagementWorkspacePurgeOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_workspace_purge_begin_purge_lake_data(self, resource_group):
        response = await (
            await self.client.workspace_purge.begin_purge_lake_data(
                resource_group_name=resource_group.name,
                workspace_name="str",
                body={
                    "table": "str",
                    "timeRange": {"endTime": "2020-02-20 00:00:00", "startTime": "2020-02-20 00:00:00"},
                },
            )
