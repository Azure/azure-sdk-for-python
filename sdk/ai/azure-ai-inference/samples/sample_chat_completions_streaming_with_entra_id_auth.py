...
DESCRIPTION:
    ...
    It also shows how to set the model deployment name, which is supported
    by Selfhosted Endpoints (aka "model as a platform" (MaaP) or "real-time endpoints").
    Model deployment name gets sent to the service as HTTP request
    header `azureml-model-deployment`, and is optional.
...
def sample_chat_completions_streaming_with_entra_id_auth():
    ...
    try:
        ...
        print("`model_deployment` will not be set.")
