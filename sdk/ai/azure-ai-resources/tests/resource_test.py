...
from azure.ai.resources.entities import Resource
...
class TestResources:
    def test_get_and_list(self, ai_client: AIClient):
        ...
        resources = ai_client.resources.list()
        ...
        for listed_resource in resources:
            ...
            if name in expected_resources:
                expected_count += 1
                gotten_resource = ai_client.resources.get(name=name)
    ...
    def test_create_update_and_delete(self, ai_client: AIClient):
        ...
        created_poller = ai_client.resources.begin_create(resource=new_local_resource)
        ...
        updated_poller = ai_client.resources.begin_update(resource=new_local_resource)
        ...
        delete_poller = ai_client.resources.begin_delete(name=new_local_resource.name, delete_dependent_resources=True)
        delete_poller.wait()
        with pytest.raises(ResourceNotFoundError):
            ai_client.resources.get(name=new_local_resource.name)
