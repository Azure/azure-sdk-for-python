...
from azure.ai.resources.index._dataindex.entities import CitationRegex, Data, DataIndex, Embedding, IndexSource, IndexStore

logger = logging.getLogger(__name__)


def test_dataindex_config_load(test_dir):
    data_index1 = load_data(source=test_dir / "index" / "dataindex" / "local_docs_to_acs_mlindex.yaml")
    data_index2 = DataIndex(
        ...
        source=IndexSource(
            input_data=Data(
                type="uri_folder",
                path=str(test_dir / "index" / "data" / "documents")
            ),
            chunk_size=200,
