...
class MLIndexOperations():
    ...
    @distributed_trace
    @monitor_with_activity(logger, "MLIndex.List", ActivityType.PUBLICAPI)
    def list(
        self,
        **kwargs,
    ) -> Iterable[MLIndex]:
    ...
    @distributed_trace
    @monitor_with_activity(logger, "MLIndex.Get", ActivityType.PUBLICAPI)
    def get(self, name: str, version: Optional[str] = None, label: Optional[str] = None) -> MLIndex:
        ...
        if "azureml.mlIndexAsset" not in data.properties:
            raise Exception(f"No MLIndex with name {name} and version {version} found.")

        return MLIndex._from_data_asset(data)

    @distributed_trace
    @monitor_with_activity(logger, "MLIndex.CreateOrUpdate", ActivityType.PUBLICAPI)
    ...
    @distributed_trace
    @monitor_with_activity(logger, "MLIndex.Download", ActivityType.PUBLICAPI)
