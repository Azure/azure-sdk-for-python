...
def _filter_parameters(model_class: Type, parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    **Note:** Classes inherited from the model check that the parameters are present
    ...
    be relevant for classes, not inherited from azure.ai.project._model_base.Model.
...
class ConnectionProperties:
    ...
    :vartype authentication_type: ~azure.ai.project.models._models.AuthenticationType
    :ivar connection_type: The connection type .
    :vartype connection_type: ~azure.ai.project.models._models.ConnectionType
...
class ToolSet:
    ...
    def validate_tool_type(self, tool_type: Type[Tool]) -> None:
        ...
        if isinstance(tool_type, AsyncFunctionTool):
            raise ValueError(
                "AsyncFunctionTool is not supported in ToolSet.  To use async functions, use AsyncToolSet and agents operations in azure.ai.project.aio."
            )
