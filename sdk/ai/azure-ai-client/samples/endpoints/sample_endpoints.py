...
if endpoint.properties.category == ConnectionType.AZURE_OPEN_AI:

    if endpoint.properties.auth_type == AuthenticationType.API_KEY:
        print("====> Creating AzureOpenAI client using API key authentication")
        client = AzureOpenAI(
            api_key=endpoint.properties.credentials.key,
            azure_endpoint=endpoint.properties.target,
            api_version="2024-08-01-preview", # TODO: Is this needed?
        )
    elif endpoint.properties.auth_type == AuthenticationType.AAD:
        print("====> Creating AzureOpenAI client using Entra ID authentication")
        client = AzureOpenAI(
            # See https://learn.microsoft.com/en-us/python/api/azure-identity/azure.identity?view=azure-python#azure-identity-get-bearer-token-provider
            azure_ad_token_provider=get_bearer_token_provider(endpoint.properties.token_credential, "https://cognitiveservices.azure.com/.default"),
            azure_endpoint=endpoint.properties.target,
            api_version="2024-08-01-preview",
        )
    elif endpoint.properties.auth_type == AuthenticationType.SAS:
...
elif endpoint.properties.category == ConnectionType.SERVERLESS:

    if endpoint.properties.auth_type == AuthenticationType.API_KEY:
        print("====> Creating ChatCompletionsClient using API key authentication")
        client = ChatCompletionsClient(
           endpoint=endpoint.properties.target,
