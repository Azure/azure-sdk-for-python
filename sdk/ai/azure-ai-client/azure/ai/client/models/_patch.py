...
class EndpointProperties:
    ...
    def __init__(
            self,
            *,
            connection: ConnectionsListSecretsResponse,
            token_credential: TokenCredential = None
        ) -> None:
        self.name=connection.name
        self.authentication_type=connection.properties.auth_type
        self.endpoint_type=connection.properties.category
        self.endpoint_url=(connection.properties.target[:-1] if connection.properties.target.endswith("/") else connection.properties.target)
        self.key: str = None
