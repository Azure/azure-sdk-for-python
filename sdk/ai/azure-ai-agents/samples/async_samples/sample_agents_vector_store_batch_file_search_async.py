...
async def main() -> None:
    async with DefaultAzureCredential() as creds:
        async with AgentsClient(
            ...
            file = await agents_client.upload_file_and_poll(
                file_path="../product_info_1.md", purpose=FilePurpose.AGENTS
            )
            print(f"Uploaded file, file ID: {file.id}")

            # Create a vector store with no file and wait for it to be processed
            vector_store = await agents_client.create_vector_store_and_poll(file_ids=[], name="sample_vector_store")
            print(f"Created vector store, vector store ID: {vector_store.id}")

            # Add the file to the vector store or you can supply file ids in the vector store creation
            vector_store_file_batch = await agents_client.create_vector_store_file_batch_and_poll(
            ...
            run = await agents_client.create_and_process_run(thread_id=thread.id, agent_id=agent.id)
            ...
            await agents_client.delete_vector_store(vector_store.id)
