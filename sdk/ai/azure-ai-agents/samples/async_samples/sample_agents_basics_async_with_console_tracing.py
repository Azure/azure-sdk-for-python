...
async def main() -> None:
    ...
    async with DefaultAzureCredential() as creds:
        async with AgentsClient(endpoint=os.environ["PROJECT_ENDPOINT"], credential=creds) as agent_client:
            ...
            thread = await agent_client.create_thread()
            print(f"Created thread, thread ID: {thread.id}")

            message = await agent_client.create_message(
                thread_id=thread.id, role="user", content="Hello, tell me a joke"
            )
            print(f"Created message, message ID: {message.id}")

            run = await agent_client.create_run(thread_id=thread.id, agent_id=agent.id)
            ...
            messages = await agent_client.list_messages(thread_id=thread.id, order=ListSortOrder.ASCENDING)
