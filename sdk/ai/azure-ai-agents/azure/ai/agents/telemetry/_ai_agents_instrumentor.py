...
class _AIAgentsInstrumentorPreview:
    ...
    def _trace_sync_function(
        ...
        *,
        ...
        def inner(*args, **kwargs):  # pylint: disable=R0911
            ...
            if class_function_name.startswith("AgentsClientOperationsMixin.create_thread"):
                kwargs.setdefault("merge_span", True)
                return self.trace_create_thread(function, *args, **kwargs)
            if class_function_name.startswith("AgentsClientOperationsMixin.create_message"):
                kwargs.setdefault("merge_span", True)
                return self.trace_create_message(function, *args, **kwargs)
            if class_function_name.startswith("AgentsClient.create_run"):
                kwargs.setdefault("merge_span", True)
                return self.trace_create_run(OperationName.START_THREAD_RUN, function, *args, **kwargs)
            if class_function_name.startswith("AgentsClient.create_and_process_run"):
                kwargs.setdefault("merge_span", True)
                return self.trace_create_run(OperationName.PROCESS_THREAD_RUN, function, *args, **kwargs)
            if class_function_name.startswith("AgentsClient.submit_tool_outputs_to_run"):
                kwargs.setdefault("merge_span", True)
                return self.trace_submit_tool_outputs(False, function, *args, **kwargs)
            if class_function_name.startswith("AgentsClient.submit_tool_outputs_to_stream"):
                kwargs.setdefault("merge_span", True)
                return self.trace_submit_tool_outputs(True, function, *args, **kwargs)
            if class_function_name.startswith("AgentsClient._handle_submit_tool_outputs"):
                return self.trace_handle_submit_tool_outputs(function, *args, **kwargs)
            if class_function_name.startswith("AgentsClient.create_stream"):
                kwargs.setdefault("merge_span", True)
                return self.trace_create_stream(function, *args, **kwargs)
            if class_function_name.startswith("AgentsClientOperationsMixin.list_messages"):
                kwargs.setdefault("merge_span", True)
                return self.trace_list_messages(function, *args, **kwargs)
            if class_function_name.startswith("AgentsClientOperationsMixin.list_run_steps"):
    ...
    def _agents_apis(self):
        sync_apis = (
            ...
            ("azure.ai.agents", "AgentsClient", "create_thread", TraceType.AGENTS, "thread_create"),
            ("azure.ai.agents", "AgentsClient", "create_message", TraceType.AGENTS, "message_create"),
            ("azure.ai.agents", "AgentsClient", "create_run", TraceType.AGENTS, "create_run"),
            (
                "azure.ai.agents",
                "AgentsClient",
            ...
            ("azure.ai.agents", "AgentsClient", "create_stream", TraceType.AGENTS, "create_stream"),
            ("azure.ai.agents", "AgentsClient", "list_messages", TraceType.AGENTS, "list_messages"),
            ("azure.ai.agents", "AgentsClient", "list_run_steps", TraceType.AGENTS, "list_run_steps"),
