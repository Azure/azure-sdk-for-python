...
class TestAgentClientAsync(AzureRecordedTestCase):
    ...
    async def test_create_run(self, **kwargs):
        ...
        async with self.create_client(**kwargs) as client:
            ...
            # create run
            run = await client.create_run(thread_id=thread.id, agent_id=agent.id)
            assert run.id
    ...
    async def test_list_runs(self, **kwargs):
        ...
        # create second run
        run2 = await client.create_run(thread_id=thread.id, agent_id=agent.id)
        assert run2.id
    ...
    async def _test_file_search(
        ...
        if streaming:
            ...
            run = await ai_client.create_and_process_run(thread_id=thread.id, agent_id=agent.id)
    ...
    async def test_azure_function_call(self, **kwargs):
        ...
        async with self.create_client(**kwargs) as client:
            ...
            run = await client.create_and_process_run(thread_id=thread.id, agent_id=agent.id)
