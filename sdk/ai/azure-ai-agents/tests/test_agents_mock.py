...
class TestAgentsMock:
    ...
    def test_multiple_agents_create(
        ...
        with agents_client:
            ...
            agents_client.create_and_process_run(thread_id="some_thread_id", agent_id=agent1.id)
            self._assert_tool_call(agents_client.submit_tool_outputs_to_run, "run123", toolset1)

            agents_client.create_and_process_run(thread_id="some_thread_id", agent_id=agent2.id)
    ...
    def test_handle_submit_tool_outputs(
        ...
        with agents_client:
            ...
            # Create run with new tool set, which also can be none.
            run = agents_client.create_and_process_run(thread_id="some_thread_id", agent_id=agent1.id)
            self._assert_tool_call(agents_client.submit_tool_outputs_to_run, "run123", toolset)
