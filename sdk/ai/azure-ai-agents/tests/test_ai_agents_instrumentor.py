...
class TestAiAgentsInstrumentor(AzureRecordedTestCase):
    ...
    def test_agent_streaming_with_toolset_with_tracing_content_recording_enabled(self, **kwargs):
        ...
        thread = client.create_thread()
        message = client.create_message(thread_id=thread.id, role="user", content="What is the weather in New York?")

        with client.create_stream(thread_id=thread.id, agent_id=agent.id, event_handler=MyEventHandler()) as stream:
            stream.until_done()
    ...
    def test_agent_streaming_with_toolset_with_tracing_content_recording_enabled_unicode(self, **kwargs):
        ...
        thread = client.create_thread()
        message = client.create_message(thread_id=thread.id, role="user", content="Времето в София?")
    ...
    def test_agent_streaming_run_steps_with_toolset_with_tracing_content_recording_enabled(self, **kwargs):
        ...
        with client.create_stream(thread_id=thread.id, agent_id=agent.id, event_handler=event_handler) as stream:
            stream.until_done()
        ...
        print("Deleted agent")
        messages = client.list_messages(thread_id=thread.id)
        client.list_run_steps(thread_id=thread.id, run_id=event_handler.run_id)
        client.close()
