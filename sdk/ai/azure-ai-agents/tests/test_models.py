...
from azure.ai.agents.models._models import VectorStoreDataSource
...
class TestModels:
    ...
    _DATA_SOURCES = [
        _models.VectorStoreDataSource(
            asset_identifier="azureai:/123",
            asset_type=_models.VectorStoreDataSourceAssetType.URI_ASSET),
        _models.VectorStoreDataSource(
            asset_identifier="azureai:/456",
            asset_type=_models.VectorStoreDataSourceAssetType.URI_ASSET),
    ]
    ...
    @pytest.mark.parametrize("files,ds", [
        (_FILES, None),
        (_FILES, []),
        (None, _DATA_SOURCES),
        ([], _DATA_SOURCES),
    ...
    def test_create_code_interpreter_tool(self, files, ds):
        ...
        tool_respurces = code_interpreter.resources
        if not files and not ds:
            assert tool_respurces.code_interpreter is None
        if files:
            assert sorted(tool_respurces.code_interpreter.file_ids) == sorted(files)
            assert tool_respurces.code_interpreter.data_sources is None
        if ds:
            assert tool_respurces.code_interpreter.file_ids is None
            self._assert_data_sources_are_the_same(tool_respurces.code_interpreter.data_sources, ds)

    def test_assert_code_interpreter_tool_raises(self):
        """Test that ifboth file_ids and data sources are ptovided the error is raised."""
    ...
    def test_add_files(self, file_id, expected):
        ...
        tool_respurces = code_interpreter.resources
        assert tool_respurces.code_interpreter is not None
        assert sorted(tool_respurces.code_interpreter.file_ids) == sorted(expected)

    @pytest.mark.parametrize('ds,expected', [
        (_DATA_SOURCES[0], _DATA_SOURCES),
        (_models.VectorStoreDataSource(asset_identifier="azueai://789", asset_type=_models.VectorStoreDataSourceAssetType.URI_ASSET),
         _DATA_SOURCES + [_models.VectorStoreDataSource(asset_identifier="azueai://789", asset_type=_models.VectorStoreDataSourceAssetType.URI_ASSET)]),
    ])
    def test_add_data_sources(self, ds, expected):
        ...
        tool_respurces = code_interpreter.resources
        assert tool_respurces.code_interpreter is not None
        self._assert_data_sources_are_the_same(tool_respurces.code_interpreter.data_sources, expected)

    def test_add_files_raises(self):
        """Test that addition of file to the CodeInterpreter with existin data sources raises the exception."""
        code_interpreter = _models.CodeInterpreterTool(data_sources=TestModels._DATA_SOURCES)
        with pytest.raises(ValueError) as e:
    ...
    def test_add_data_source_raises(self):
        """Test that addition of a data source to CodeIterpreter with file IDs raises the exception."""
        code_interpreter = _models.CodeInterpreterTool(file_ids=TestModels._FILES)
        with pytest.raises(ValueError) as e:
            code_interpreter.add_data_source(_models.VectorStoreDataSource(asset_identifier="azueai://789",
                                                                           asset_type=_models.VectorStoreDataSourceAssetType.URI_ASSET))
        assert _models.CodeInterpreterTool._INVALID_CONFIGURATION == e.value.args[0]
