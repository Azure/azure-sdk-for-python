...
class TestAgentsMock:
    ...
    def _set_toolcalls(
        self, agents_client: AgentsClient, toolset1: Optional[AsyncToolSet], toolset2: Optional[AsyncToolSet]
    ) -> None:
        ...
        if toolset1 and toolset2:
            ...
            agents_client.enable_auto_function_calls(function_tool=function_tool)
        elif toolset1:
            agents_client.enable_auto_function_calls(toolset=toolset1)
        elif toolset2:
            agents_client.enable_auto_function_calls(toolset=toolset2)
    ...
    async def test_multiple_agents_create(
        ...
        async with agents_client:
            ...
            agent2 = await agents_client.create_agent(
                model="gpt-4-1106-preview",
                name="second",
                instructions="You are a helpful agent",
                toolset=toolset2,
            ...
            await agents_client.create_and_process_run(thread_id="some_thread_id", agent_id=agent1.id)
            self._assert_tool_call(agents_client.submit_tool_outputs_to_run, "run123", toolset1)

            await agents_client.create_and_process_run(thread_id="some_thread_id", agent_id=agent2.id)
    ...
    async def test_update_agent_tools(
        ...
        async with agents_client:
            ...
            agent1 = await agents_client.update_agent(agent1.id, toolset=toolset2)
            if toolset2 is None:
    ...
    async def test_handle_submit_tool_outputs(
        ...
        async with agents_client:
            ...
            # Create run with new tool set, which also can be none.
            run = await agents_client.create_and_process_run(thread_id="some_thread_id", agent_id=agent1.id)
            self._assert_tool_call(agents_client.submit_tool_outputs_to_run, "run123", toolset)
