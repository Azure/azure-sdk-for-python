...
from azure.ai.assistants.models._patch import AssistantEventHandler, AsyncAssistantEventHandler, ToolSet
from azure.ai.assistants.telemetry._utils import (
    AZ_AI_ASSISTANT_SYSTEM,
    ERROR_TYPE,
    GEN_AI_ASSISTANT_DESCRIPTION,
    GEN_AI_ASSISTANT_ID,
    GEN_AI_ASSISTANT_NAME,
...
__all__ = [
    "AIAssistantsInstrumentor",
]


_assistants_traces_enabled: bool = False
_trace_assistants_content: bool = False
...
class AIAssistantsInstrumentor:
    """
    A class for managing the trace instrumentation of AI Assistants.
    ...
    def __init__(self):
        ...
        self._impl = _AIAssistantsInstrumentorPreview()

    def instrument(self, enable_content_recording: Optional[bool] = None) -> None:
        """
        Enable trace instrumentation for AI Assistants.
    ...
    def uninstrument(self) -> None:
        """
        Remove trace instrumentation for AI Assistants.
    ...
    def is_instrumented(self) -> bool:
        """
        Check if trace instrumentation for AI Assistants is currently enabled.
...
class _AIAssistantsInstrumentorPreview:
    ...
    def instrument(self, enable_content_recording: Optional[bool] = None):
        ...
        if not self.is_instrumented():
            self._instrument_assistants(enable_content_recording)
        else:
            self._set_enable_content_recording(enable_content_recording=enable_content_recording)

    def uninstrument(self):
        """
        Disable trace instrumentation for AI Assistants.

        This method removes any active instrumentation, stopping the tracing
        of AI Assistants.
        """
        if self.is_instrumented():
            self._uninstrument_assistants()
    ...
    def _create_event_attributes(
        ...
        attrs: Dict[str, Any] = {GEN_AI_SYSTEM: AZ_AI_ASSISTANT_SYSTEM}
        if thread_id:
            attrs[GEN_AI_THREAD_ID] = thread_id

        if assistant_id:
            attrs[GEN_AI_ASSISTANT_ID] = assistant_id
    ...
    def _add_instructions_event(
        ...
        if _trace_assistants_content and (instructions or additional_instructions):
        ...
        attributes = self._create_event_attributes(assistant_id=assistant_id, thread_id=thread_id)
        attributes[GEN_AI_EVENT_CONTENT] = json.dumps(event_body, ensure_ascii=False)
    ...
    @staticmethod
    def assistant_api_response_to_str(response_format: Any) -> Optional[str]:
        """
        Convert response_format to string.

        :param response_format: The response format.
        :type response_format: ~azure.ai.assistants._types.AssistantsApiResponseFormatOption
        :returns: string for the response_format.
        :rtype: Optional[str]
        :raises: Value error if response_format is not of type AssistantsApiResponseFormatOption.
        """
        if isinstance(response_format, str) or response_format is None:
            return response_format
        if isinstance(response_format, AssistantsApiResponseFormatMode):
            return response_format.value
        if isinstance(response_format, _models.AssistantsApiResponseFormat):
    ...
    def start_thread_run_span(
        ...
        span = start_span(
            ...
            response_format=_AIAssistantsInstrumentorPreview.assistant_api_response_to_str(response_format),
        )
        if span and span.span_instance.is_recording and instructions and additional_instructions:
            self._add_instructions_event(
                span, instructions, additional_instructions, thread_id=thread_id, assistant_id=assistant_id
            )
    ...
    def start_submit_tool_outputs_span(
        ...
        event_handler: Optional[Union[AssistantEventHandler, AsyncAssistantEventHandler]] = None,
    ) -> "Optional[AbstractSpan]":
    ...
    def start_create_assistant_span(
        self,
        project_name: str,
        model: Optional[str] = None,
        ...
        span = start_span(
            OperationName.CREATE_ASSISTANT,
            project_name,
            span_name=f"{OperationName.CREATE_ASSISTANT.value} {name}",
            model=model,
            temperature=temperature,
            top_p=top_p,
            response_format=_AIAssistantsInstrumentorPreview.assistant_api_response_to_str(response_format),
        ...
        if span and span.span_instance.is_recording:
            if name:
                span.add_attribute(GEN_AI_ASSISTANT_NAME, name)
            if description:
                span.add_attribute(GEN_AI_ASSISTANT_DESCRIPTION, description)
    ...
    def trace_create_assistant(self, function, *args, **kwargs):
        ...
        span = self.start_create_assistant_span(
        ...
        with span:
            try:
                ...
                span.add_attribute(GEN_AI_ASSISTANT_ID, result.id)
    ...
    async def trace_create_assistant_async(self, function, *args, **kwargs):
        project_name = args[  # pylint: disable=protected-access # pyright: ignore [reportFunctionMemberAccess]
            0
        ]._config.project_name
        name = kwargs.get("name")
    ...
    def trace_submit_tool_outputs(self, stream, function, *args, **kwargs):
        ...
        with span:
            try:
                ...
                result = function(*args, **kwargs)
                if not isinstance(result, AssistantRunStream):
                    self.set_end_run(span, result)
            except Exception as exc:
                # Set the span status to error
                if isinstance(span.span_instance, Span):  # pyright: ignore [reportPossiblyUnboundVariable]
    ...
    async def trace_submit_tool_outputs_async(self, stream, function, *args, **kwargs):
        ...
        with span:
            try:
                ...
                result = await function(*args, **kwargs)
                if not isinstance(result, AsyncAssistantRunStream):
                    self.set_end_run(span, result)
            except Exception as exc:
                # Set the span status to error
                if isinstance(span.span_instance, Span):  # pyright: ignore [reportPossiblyUnboundVariable]
    ...
    def handle_run_stream_exit(self, _function, *args, **kwargs):
        ...
        if (
            assistant_run_stream.event_handler
            and assistant_run_stream.event_handler.__class__.__name__ == "_AssistantEventHandlerTraceWrapper"
        ):
            assistant_run_stream.event_handler.__exit__(exc_type, exc_val, exc_tb)
        elif (
            assistant_run_stream.event_handler
            and assistant_run_stream.event_handler.__class__.__name__ == "_AsyncAssistantEventHandlerTraceWrapper"
        ):
            assistant_run_stream.event_handler.__aexit__(exc_type, exc_val, exc_tb)

    def wrap_handler(
        self, handler: "Optional[AssistantEventHandler]" = None, span: "Optional[AbstractSpan]" = None
    ) -> "Optional[AssistantEventHandler]":
        # Do not create a handler wrapper if we do not have handler in the first place.
        if not handler:
            return None

        if isinstance(handler, _AssistantEventHandlerTraceWrapper):
            return handler

        if span and span.span_instance.is_recording:
            return _AssistantEventHandlerTraceWrapper(self, span, handler)

        return handler

    def wrap_async_handler(
        self, handler: "Optional[AsyncAssistantEventHandler]" = None, span: "Optional[AbstractSpan]" = None
    ) -> "Optional[AsyncAssistantEventHandler]":
        # Do not create a handler wrapper if we do not have handler in the first place.
        if not handler:
            return None

        if isinstance(handler, _AsyncAssistantEventHandlerTraceWrapper):
            return handler

        if span and span.span_instance.is_recording:
            return _AsyncAssistantEventHandlerTraceWrapper(self, span, handler)
    ...
    def _trace_sync_function(
        ...
        *,
        ...
        def inner(*args, **kwargs):  # pylint: disable=R0911
            ...
            if class_function_name.startswith("AssistantsClient.create_assistant"):
                kwargs.setdefault("merge_span", True)
                return self.trace_create_assistant(function, *args, **kwargs)
            if class_function_name.startswith("AssistantsClientOperationsMixin.create_thread"):
                kwargs.setdefault("merge_span", True)
                return self.trace_create_thread(function, *args, **kwargs)
            if class_function_name.startswith("AssistantsClientOperationsMixin.create_message"):
                kwargs.setdefault("merge_span", True)
                return self.trace_create_message(function, *args, **kwargs)
            if class_function_name.startswith("AssistantsClient.create_run"):
                kwargs.setdefault("merge_span", True)
                return self.trace_create_run(OperationName.START_THREAD_RUN, function, *args, **kwargs)
            if class_function_name.startswith("AssistantsClient.create_and_process_run"):
                kwargs.setdefault("merge_span", True)
                return self.trace_create_run(OperationName.PROCESS_THREAD_RUN, function, *args, **kwargs)
            if class_function_name.startswith("AssistantsClient.submit_tool_outputs_to_run"):
                kwargs.setdefault("merge_span", True)
                return self.trace_submit_tool_outputs(False, function, *args, **kwargs)
            if class_function_name.startswith("AssistantsClient.submit_tool_outputs_to_stream"):
                kwargs.setdefault("merge_span", True)
                return self.trace_submit_tool_outputs(True, function, *args, **kwargs)
            if class_function_name.startswith("AssistantsClient._handle_submit_tool_outputs"):
                return self.trace_handle_submit_tool_outputs(function, *args, **kwargs)
            if class_function_name.startswith("AssistantsClient.create_stream"):
                kwargs.setdefault("merge_span", True)
                return self.trace_create_stream(function, *args, **kwargs)
            if class_function_name.startswith("AssistantsClientOperationsMixin.list_messages"):
                kwargs.setdefault("merge_span", True)
                return self.trace_list_messages(function, *args, **kwargs)
            if class_function_name.startswith("AssistantRunStream.__exit__"):
    ...
    def _trace_async_function(
        ...
        *,
        ...
        async def inner(*args, **kwargs):  # pylint: disable=R0911
            ...
            if class_function_name.startswith("AssistantsClient.create_assistant"):
                kwargs.setdefault("merge_span", True)
                return await self.trace_create_assistant_async(function, *args, **kwargs)
            if class_function_name.startswith("AssistantsClientOperationsMixin.create_thread"):
                kwargs.setdefault("merge_span", True)
            ...
            if class_function_name.startswith("AsyncAssistantRunStream.__aexit__"):
                return self.handle_run_stream_exit(function, *args, **kwargs)
            # Handle the default case (if the function name does not match)
            return None  # Ensure all paths return
    ...
    def _assistants_api_list(self):
        sync_apis, async_apis = self._assistants_apis()
    ...
    def _available_assistants_apis_and_injectors(self):
        """
        Generates a sequence of tuples containing Assistants API classes, method names, and
        corresponding injector functions.

        :return: A generator yielding tuples.
        :rtype: tuple
        """
        yield from self._generate_api_and_injector(self._assistants_api_list())

    def _instrument_assistants(self, enable_content_tracing: bool = False):
        """This function modifies the methods of the Assistants API classes to
        ...
        global _assistants_traces_enabled
        global _trace_assistants_content
        if _assistants_traces_enabled:
            raise RuntimeError("Traces already started for AI Assistants")
        _assistants_traces_enabled = True
        _trace_assistants_content = enable_content_tracing
        for (
            api,
            method,
            trace_type,
    ...
    def _uninstrument_assistants(self):
        """This function restores the original methods of the Assistants API classes
        by assigning them back from the _original attributes of the modified methods.
        """
        # pylint: disable=W0603
        global _assistants_traces_enabled
        global _trace_assistants_content
        _trace_assistants_content = False
        for api, method, _, _, _ in self._available_assistants_apis_and_injectors():
            if hasattr(getattr(api, method), "_original"):
                setattr(api, method, getattr(getattr(api, method), "_original"))
        _assistants_traces_enabled = False

    def _is_instrumented(self):
        """This function returns True if Assistants API has already been instrumented
        for tracing and False if it has not been instrumented.

        :return: A value indicating whether the Assistants API is currently instrumented or not.
        :rtype: bool
        """
        return _assistants_traces_enabled
    ...
    def _set_enable_content_recording(self, enable_content_recording: bool = False) -> None:
        ...
        :type enable_content_recording: bool
        """
        global _trace_assistants_content  # pylint: disable=W0603
        _trace_assistants_content = enable_content_recording
    ...
    def _is_content_recording_enabled(self) -> bool:
        ...
        :rtype bool
        """
        return _trace_assistants_content


class _AssistantEventHandlerTraceWrapper(AssistantEventHandler):
    def __init__(
        self,
        instrumentor: _AIAssistantsInstrumentorPreview,
        span: "AbstractSpan",
        inner_handler: Optional[AssistantEventHandler] = None,
    ):
        super().__init__()
        self.span = span
...
class _AsyncAssistantEventHandlerTraceWrapper(AsyncAssistantEventHandler):
    def __init__(
        self,
        instrumentor: _AIAssistantsInstrumentorPreview,
        span: "AbstractSpan",
        inner_handler: Optional[AsyncAssistantEventHandler] = None,
    ):
        super().__init__()
        self.span = span
        self.inner_handler = inner_handler
