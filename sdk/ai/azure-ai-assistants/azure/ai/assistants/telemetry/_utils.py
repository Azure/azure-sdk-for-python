...
GEN_AI_ASSISTANT_ID = "gen_ai.assistant.id"
GEN_AI_ASSISTANT_NAME = "gen_ai.assistant.name"
GEN_AI_ASSISTANT_DESCRIPTION = "gen_ai.assistant.description"
...
AZ_AI_ASSISTANT_SYSTEM = "az.ai.assistants"
...
def trace_tool_execution(
    ...
    assistant_id: Optional[str] = None,  # TODO: would be nice to have this, but need to propagate somehow
...
def start_span(
    ...
    gen_ai_system: Optional[str] = AZ_AI_ASSISTANT_SYSTEM,
    ...
    if span and span.span_instance.is_recording:
        if gen_ai_system:
            span.add_attribute(GEN_AI_SYSTEM, AZ_AI_ASSISTANT_SYSTEM)
        ...
        if assistant_id:
            span.add_attribute(GEN_AI_ASSISTANT_ID, assistant_id)
...
def enable_telemetry(destination: Union[TextIO, str, None] = None, **kwargs) -> None:  # pylint: disable=unused-argument
    ...
    try:
        ...
        assistants_instrumentor = AIAssistantsInstrumentor()
        if not assistants_instrumentor.is_instrumented():
            assistants_instrumentor.instrument()
    except Exception as exc:  # pylint: disable=broad-exception-caught
        logger.warning("Could not call `AIAssistantsInstrumentor().instrument()`", exc_info=exc)
