...
def _parse_event(event_data_str: str) -> Tuple[str, StreamEventData]:
    ...
    if event_type in {
        AssistantStreamEvent.THREAD_RUN_CREATED.value,
        AssistantStreamEvent.THREAD_RUN_QUEUED.value,
        AssistantStreamEvent.THREAD_RUN_INCOMPLETE.value,
        AssistantStreamEvent.THREAD_RUN_IN_PROGRESS.value,
        AssistantStreamEvent.THREAD_RUN_REQUIRES_ACTION.value,
        AssistantStreamEvent.THREAD_RUN_COMPLETED.value,
        AssistantStreamEvent.THREAD_RUN_FAILED.value,
        AssistantStreamEvent.THREAD_RUN_CANCELLING.value,
        AssistantStreamEvent.THREAD_RUN_CANCELLED.value,
        AssistantStreamEvent.THREAD_RUN_EXPIRED.value,
    }:
        event_obj = _safe_instantiate(ThreadRun, parsed_data)
    elif event_type in {
        AssistantStreamEvent.THREAD_RUN_STEP_CREATED.value,
        AssistantStreamEvent.THREAD_RUN_STEP_IN_PROGRESS.value,
        AssistantStreamEvent.THREAD_RUN_STEP_COMPLETED.value,
        AssistantStreamEvent.THREAD_RUN_STEP_FAILED.value,
        AssistantStreamEvent.THREAD_RUN_STEP_CANCELLED.value,
        AssistantStreamEvent.THREAD_RUN_STEP_EXPIRED.value,
    }:
        event_obj = _safe_instantiate(RunStep, parsed_data)
    elif event_type in {
        AssistantStreamEvent.THREAD_MESSAGE_CREATED.value,
        AssistantStreamEvent.THREAD_MESSAGE_IN_PROGRESS.value,
        AssistantStreamEvent.THREAD_MESSAGE_COMPLETED.value,
        AssistantStreamEvent.THREAD_MESSAGE_INCOMPLETE.value,
    }:
        event_obj = _safe_instantiate(ThreadMessage, parsed_data, generated_class=ThreadMessageGenerated)
    elif event_type == AssistantStreamEvent.THREAD_MESSAGE_DELTA.value:
        event_obj = _safe_instantiate(MessageDeltaChunk, parsed_data, generated_class=MessageDeltaChunkGenerated)

    elif event_type == AssistantStreamEvent.THREAD_RUN_STEP_DELTA.value:
...
class FunctionTool(BaseFunctionTool):
    ...
    def execute(self, tool_call: RequiredFunctionToolCall) -> Any:
        ...
        try:
            ...
            # Return error message as JSON string back to assistant in order to make possible self
...
class AsyncFunctionTool(BaseFunctionTool):
    ...
    async def execute(self, tool_call: RequiredFunctionToolCall) -> Any:  # pylint: disable=invalid-overridden-method
        ...
        try:
            ...
            error_message = f"Error executing function '{tool_call.function.name}': {e}"
            logging.error(error_message)
            # Return error message as JSON string back to assistant in order to make possible self correction
...
class AzureFunctionTool(Tool[AzureFunctionToolDefinition]):
    """
    A tool that is used to inform assistant about available the Azure function.
...
class BaseToolSet:
    """
    Abstract class for a collection of tools that can be used by an assistant.
    """
...
class ToolSet(BaseToolSet):
    """
    A collection of tools that can be used by an synchronize assistant.
    """
...
class AsyncToolSet(BaseToolSet):
    """
    A collection of tools that can be used by an asynchronous assistant.
    """
...
T = TypeVar("T")
BaseAsyncAssistantEventHandlerT = TypeVar("BaseAsyncAssistantEventHandlerT", bound="BaseAsyncAssistantEventHandler")
...
class BaseAsyncAssistantEventHandler(AsyncIterator[T]):
    ...
    def initialize(
        ...
        submit_tool_outputs: Callable[[ThreadRun, "BaseAsyncAssistantEventHandler[T]"], Awaitable[None]],
    ):
...
class BaseAssistantEventHandler(Iterator[T]):

    def __init__(self) -> None:
        self.response_iterator: Optional[Iterator[bytes]] = None
        self.submit_tool_outputs: Optional[Callable[[ThreadRun, "BaseAssistantEventHandler[T]"], None]] = None
        self.buffer: Optional[bytes] = None

    def initialize(
        self,
        response_iterator: Iterator[bytes],
        submit_tool_outputs: Callable[[ThreadRun, "BaseAssistantEventHandler[T]"], None],
    ) -> None:
        self.response_iterator = (
...
class AssistantEventHandler(BaseAssistantEventHandler[Tuple[str, StreamEventData, Optional[EventFunctionReturnT]]]):
    ...
    def _process_event(self, event_data_str: str) -> Tuple[str, StreamEventData, Optional[EventFunctionReturnT]]:
        ...
        if (
            ...
            cast(Callable[[ThreadRun, "BaseAssistantEventHandler"], Awaitable[None]], self.submit_tool_outputs)(
                event_data_obj, self
            )
...
class AsyncAssistantRunStream(Generic[BaseAsyncAssistantEventHandlerT]):
    def __init__(
        self,
        response_iterator: AsyncIterator[bytes],
        submit_tool_outputs: Callable[[ThreadRun, BaseAsyncAssistantEventHandlerT], Awaitable[None]],
        event_handler: BaseAsyncAssistantEventHandlerT,
    ):
        ...
        self.event_handler.initialize(
            self.response_iterator,
            cast(Callable[[ThreadRun, BaseAsyncAssistantEventHandler], Awaitable[None]], submit_tool_outputs),
        )
...
class AssistantRunStream(Generic[BaseAssistantEventHandlerT]):
    def __init__(
        self,
        response_iterator: Iterator[bytes],
        submit_tool_outputs: Callable[[ThreadRun, BaseAssistantEventHandlerT], None],
        event_handler: BaseAssistantEventHandlerT,
    ):
        self.response_iterator = response_iterator
        self.event_handler = event_handler
        self.submit_tool_outputs = submit_tool_outputs
        self.event_handler.initialize(
            self.response_iterator,
            cast(Callable[[ThreadRun, BaseAssistantEventHandler], None], submit_tool_outputs),
        )
...
__all__: List[str] = [
    ...
    "AzureAISearchTool",
    "AzureFunctionTool",
    "BaseAsyncAssistantEventHandler",
    "BaseAssistantEventHandler",
    "CodeInterpreterTool",
