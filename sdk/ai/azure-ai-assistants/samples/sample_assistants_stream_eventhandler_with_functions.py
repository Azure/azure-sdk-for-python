...
USAGE:
    python sample_assistants_stream_eventhandler_with_functions.py
    ...
    def on_thread_run(self, run: "ThreadRun") -> None:
        ...
        if run.status == "requires_action" and isinstance(run.required_action, SubmitToolOutputsAction):
            ...
            if tool_outputs:
                ...
                assistants_client.submit_tool_outputs_to_stream(
...
with assistants_client:
    ...
    with assistants_client.create_stream(
        thread_id=thread.id, assistant_id=assistant.id, event_handler=MyEventHandler(functions)
    ) as stream:
        stream.until_done()
