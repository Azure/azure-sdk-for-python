...
class TestBaseAsyncAssistantEventHandler:
    class MyAssistantEventhHandler(BaseAsyncAssistantEventHandler[str]):
    ...
    async def mock_callable(self, _: ThreadRun, __: BaseAsyncAssistantEventHandler[str]) -> None:
        pass
    ...
    async def test_event_handler_process_response_when_break_around_event_separators(self):
        ...
        handler = self.MyAssistantEventhHandler()
...
class TestAsyncAssistantEventHandler:

    deserializable_events = [
        AssistantStreamEvent.THREAD_CREATED.value,
        AssistantStreamEvent.ERROR.value,
        AssistantStreamEvent.DONE.value,
    ]

    class MyAssistantEventHandler(AsyncAssistantEventHandler[None]):
        pass

    @pytest.mark.asyncio
    @patch("azure.ai.assistants.models._patch._parse_event")
    async def test_tool_calls(self, mock_parse_event: AsyncMock):
        ...
        handler = self.MyAssistantEventHandler()
    ...
    @pytest.mark.asyncio
    @patch("azure.ai.assistants.models._patch.AsyncAssistantEventHandler.on_unhandled_event")
    @pytest.mark.parametrize("event_type", [e.value for e in AssistantStreamEvent])
    async def test_parse_event(self, mock_on_unhandled_event: AsyncMock, event_type: str):
        # Make sure all the event types defined in AssistantStreamEvent are deserializable except Created, Done, and Error
        ...
        if event_type == AssistantStreamEvent.THREAD_CREATED.value:
