...
class TestAssistantClient(AzureRecordedTestCase):
    ...
    def test_create_delete_assistant(self, **kwargs):
        ...
        with self.create_client(**kwargs) as client:
            ...
            self._do_test_create_assistant(client=client, body=None, functions=None)

    @assistantClientPreparer()
    @recorded_by_proxy
    def test_create_assistant_with_body(self, **kwargs):
        """test assistant creation with body: JSON"""
        # create client
        with self.create_client(**kwargs) as client:
            assert isinstance(client, AssistantsClient)
            print("Created client")

            # create body for assistant and call helper function
            body = {"name": "my-assistant", "model": "gpt-4o", "instructions": "You are helpful assistant"}
            self._do_test_create_assistant(client=client, body=body, functions=None)

    @assistantClientPreparer()
    @recorded_by_proxy
    def test_create_assistant_with_iobytes(self, **kwargs):
        """test assistant creation with body: IO[bytes]"""
        ...
        with self.create_client(**kwargs) as client:
            ...
            self._do_test_create_assistant(client=client, body=io.BytesIO(binary_body), functions=None)
    ...
    def test_create_assistant_with_tools(self, **kwargs):
        ...
        with self.create_client(**kwargs) as client:
            ...
            self._do_test_create_assistant(client=client, body=None, functions=functions)

    @assistantClientPreparer()
    @recorded_by_proxy
    def test_create_assistant_with_tools_and_resources(self, **kwargs):
        """test assistant creation with tools and resources"""
    ...
    def _do_test_create_assistant(self, client, body, functions):
        """helper function for creating assistant with different body inputs"""
        ...
        assert assistant.name == "my-assistant"
    ...
    def test_update_assistant(self, **kwargs):
        """test assistant update without body"""

        # create client
        with self.create_client(**kwargs) as client:
            assert isinstance(client, AssistantsClient)
            self._do_test_update_assistant(client=client, use_body=False, use_io=False)

    @assistantClientPreparer()
    @pytest.mark.skip("Update assistant with body is failing")
    @recorded_by_proxy
    def test_update_assistant_with_body(self, **kwargs):
        """test assistant update with body: JSON"""

        # create client
        with self.create_client(**kwargs) as client:
            assert isinstance(client, AssistantsClient)
            self._do_test_update_assistant(client=client, use_body=True, use_io=False)

    @assistantClientPreparer()
    @pytest.mark.skip("Update assistant with body is failing")
    @recorded_by_proxy
    def test_update_assistant_with_iobytes(self, **kwargs):
        """test assistant update with body: IO[bytes]"""

        # create client
        with self.create_client(**kwargs) as client:
            assert isinstance(client, AssistantsClient)
            self._do_test_update_assistant(client=client, use_body=True, use_io=True)

    def _do_test_update_assistant(self, client, use_body, use_io):
        ...
        # update assistant
        if use_body:
            body = {"assistant_id": assistant.id, "name": "my-assistant2"}
            if use_io:
                binary_body = json.dumps(body).encode("utf-8")
    ...
    def test_assistant_list(self, **kwargs):
        ...
        with self.create_client(**kwargs) as client:
            ...
            assistant2 = client.assistants.create_assistant(model="gpt-4o", name="my-assistant2", instructions="You are helpful assistant")
            assert client.assistants.list_assistants().data.__len__() == list_length + 2
            assert (
                client.assistants.list_assistants().data[0].id == assistant.id or client.assistants.list_assistants().data[1].id == assistant.id
            )
    ...
    def test_list_messages(self, **kwargs):
        ...
        with self.create_client(**kwargs) as client:
            ...
            message1 = client.assistants.create_message(thread_id=thread.id, role="user", content="Hello, tell me a joke")
            assert message1.id
            ...
            with client.assistants.create_stream(thread_id=thread.id, assistant_id=assistant.id) as stream:
                for event_type, event_data, _ in stream:
                    assert (
                        ...
                        or event_type == AssistantStreamEvent.DONE
                    )
    ...
    def test_create_stream_with_iobytes(self, **kwargs):
        ...
        with self.create_client(**kwargs) as client:
            ...
            # create stream
            with client.assistants.create_stream(thread_id=thread.id, body=io.BytesIO(binary_body), stream=True) as stream:
                for event_type, event_data, _ in stream:
                    assert (
                        isinstance(event_data, (MessageDeltaChunk, ThreadMessage, ThreadRun, RunStep))
                        or event_type == AssistantStreamEvent.DONE
    ...
    def _do_test_submit_tool_outputs_to_stream(self, client, use_body, use_io):
        ...
        with client.assistants.create_stream(thread_id=thread.id, assistant_id=assistant.id) as stream:
            for event_type, event_data, _ in stream:
                ...
                # Check if tools are needed
                if (
                    event_type == AssistantStreamEvent.THREAD_RUN_REQUIRES_ACTION
                    ...
                    tool_event_handler = AssistantEventHandler()
                    if tool_outputs:
                        if use_body:
                        ...
                        for tool_event_type, tool_event_data, _ in tool_event_handler:
                            assert (
                                ...
                                or tool_event_type == AssistantStreamEvent.DONE
                            )
    ...
    @assistantClientPreparer()
    @recorded_by_proxy
    def test_create_assistant_with_invalid_code_interpreter_tool_resource(self, **kwargs):
        """test assistant creation with invalid code interpreter tool resource."""
        # create client
        with self.create_client(**kwargs) as client:
    ...
    @assistantClientPreparer()
    @recorded_by_proxy
    def test_create_assistant_with_invalid_file_search_tool_resource(self, **kwargs):
        """test assistant creation with invalid file search tool resource."""
        # create client
        with self.create_client(**kwargs) as client:
    ...
    def test_file_search_add_vector_store(self, **kwargs):
        """Test the assistant with file search and vector store creation."""
    ...
    def _test_file_search(
        self, ai_client: AssistantsClient, vector_store: VectorStore, file_id: Optional[str], streaming: bool
    ) -> None:
    ...
    def _do_test_create_assistant_with_interpreter(self, **kwargs):
        ...
        if "file_path" in kwargs:
            file = ai_client.assistants.upload_file_and_poll(file_path=kwargs["file_path"], purpose=FilePurpose.ASSISTANTS)
    ...
    def test_azure_ai_search_tool(self, **kwargs):
        """Test using the AzureAISearchTool with an assistant."""
        # create client
        with self.create_client(**kwargs) as client:
            assert isinstance(client, AssistantsClient)

            # Create AzureAISearchTool 
            connection_name = kwargs.pop("azure_ai.assistants_assistants_tests_search_connection_name", "my-search-connection-name")
    ...
    def _do_test_include_file_search_results(self, use_stream, include_content, **kwargs):
        ...
        with self.create_client(**kwargs) as ai_client:
            ...
            if use_stream:
                run = None
                with ai_client.assistants.create_stream(thread_id=thread.id, assistant_id=assistant.id, include=include) as stream:
                    for event_type, event_data, _ in stream:
                        if isinstance(event_data, ThreadRun):
                            run = event_data
                ...
                run = ai_client.assistants.create_and_process_run(thread_id=thread.id, assistant_id=assistant.id, include=include)
