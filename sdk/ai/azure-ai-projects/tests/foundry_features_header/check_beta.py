...
for sc_name in sorted(dir(client.beta)):
    ...
    if not callable(sc):
        ...
        for m_name in sorted(dir(sc)):
            ...
            print(
                f"    {m_name}: ismethod={inspect.ismethod(method)}, isfunction={inspect.isfunction(method)}, iscoroutinefunction={inspect.iscoroutinefunction(method)}, callable={callable(method)}, type={type(method)}"
            )
