...
class TestToolboxes(TestBase):

    @servicePreparer()
    @recorded_by_proxy
    def test_shell_and_web_iq_tools(self, **kwargs) -> None:
        ...
        with self.create_client(allow_preview=True, **kwargs) as project_client:
            try:
                try:
                    project_client.toolboxes.delete(name=toolbox_name)
                ...
                fetched = project_client.toolboxes.get_version(name=toolbox_name, version=created.version)
