...
class TestVoiceAgentTelephonyCampaign(TestBase):
    """
    ...
    NOTE: All tests in this file are currently marked `skip`:
      - Probing this environment's live Voice Agents test resource with
        ...
        route *is* registered, but the call-job/campaign feature isn't yet enabled for the API
    ...
    def test_telephony_call_job_not_found(self, **kwargs):
        ...
        with pytest.raises(ResourceNotFoundError):
            project_client.beta.agent_telephony.get_call_job(agent_name=agent_name, call_job_id=fake_call_job_id)
        with pytest.raises(ResourceNotFoundError):
            project_client.beta.agent_telephony.cancel_call_job(
                agent_name=agent_name,
    ...
    # To run only this test:
    # pytest tests\agents\test_voice_agent_telephony_campaign.py::TestVoiceAgentTelephonyCampaign::test_telephony_campaign_not_found -s
    @pytest.mark.skip(
        reason="agent_telephony routes are registered but return 400 UnsupportedApiVersion for "
        "api-version 'v1' on the live test service. Un-skip once the service supports 'v1' for "
        "this operation group."
    ...
    @servicePreparer()
    @recorded_by_proxy()
    def test_telephony_campaign_not_found(self, **kwargs):
        """
        ...
        with pytest.raises(ResourceNotFoundError):
            project_client.beta.agent_telephony.get_campaign(agent_name=agent_name, campaign_id=fake_campaign_id)
        with pytest.raises(ResourceNotFoundError):
            project_client.beta.agent_telephony.cancel_campaign(agent_name=agent_name, campaign_id=fake_campaign_id)
        with pytest.raises(ResourceNotFoundError):
            project_client.beta.agent_telephony.pause_campaign(agent_name=agent_name, campaign_id=fake_campaign_id)
        with pytest.raises(ResourceNotFoundError):
            project_client.beta.agent_telephony.resume_campaign(agent_name=agent_name, campaign_id=fake_campaign_id)
    ...
    # To run only this test:
    # pytest tests\agents\test_voice_agent_telephony_campaign.py::TestVoiceAgentTelephonyCampaign::test_telephony_operation_not_found -s
    @pytest.mark.skip(
        reason="agent_telephony routes are registered but return 400 UnsupportedApiVersion for "
        "api-version 'v1' on the live test service. Un-skip once the service supports 'v1' for "
        "this operation group."
    ...
    @servicePreparer()
    @recorded_by_proxy()
    def test_telephony_operation_not_found(self, **kwargs):
        """
        Test the generic long-running-operation status endpoint used to poll
        `begin_import_campaign_recipients`/`begin_publish_campaign`/
        `begin_validate_campaign`, against a nonexistent operation id, which returns
        404.
        ...
        with pytest.raises(ResourceNotFoundError):
            project_client.beta.agent_telephony.get_operation(agent_name=agent_name, operation_id=fake_operation_id)
