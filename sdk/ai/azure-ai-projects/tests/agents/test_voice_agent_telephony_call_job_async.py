...
class TestVoiceAgentTelephonyCallJobAsync(TestBase):
    """
    ...
    NOTE: All tests in this file are currently marked `skip`:
      - Probing this environment's live Voice Agents test resource with
        ...
        this route *is* registered, but the call-job feature isn't yet enabled for the API
    ...
    real-provider limitation documented for `create_binding` in `test_voice_agent_telephony_async.py`).
    ...
    # pytest tests\agents\test_voice_agent_telephony_call_job_async.py::TestVoiceAgentTelephonyCallJobAsync::test_telephony_call_job_not_found -s
    @pytest.mark.skip(
