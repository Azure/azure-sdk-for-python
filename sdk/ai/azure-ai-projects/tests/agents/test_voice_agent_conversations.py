...
class TestVoiceAgentConversations(TestBase):
    ...
    def test_read_conversation(self, **kwargs):  # pylint: disable=too-many-locals
        ...
        try:
            ...
            found = any(c.id == conversation_id for c in conversations.list(_AGENT_NAME))
            assert found, "Expected the new conversation to appear in list"

            # The conversation envelope.
            conversation = conversations.get(_AGENT_NAME, conversation_id)
            ...
            responses = list(conversations.list_responses(_AGENT_NAME, conversation_id))
            assert len(responses) >= 1
            first_response = responses[0]
            response_detail = conversations.get_response(_AGENT_NAME, conversation_id, first_response.id)
            assert response_detail.id == first_response.id

            # The items produced by that response (does not raise; count may be 0 or more).
            list(conversations.list_response_items(_AGENT_NAME, conversation_id, first_response.id))

            # The ordered conversation items -- the full transcript (user + assistant + tool events).
            items = list(conversations.list_items(_AGENT_NAME, conversation_id))
            assert len(items) >= 1
            first_item_id = items[0].get("id")
            assert first_item_id
            fetched_item = conversations.get_item(_AGENT_NAME, conversation_id, first_item_id)
            ...
            recording = conversations.get_audio(_AGENT_NAME, conversation_id)
