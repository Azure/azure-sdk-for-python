...
servicePreparer = functools.partial(
    EnvironmentVariableLoader,
        "",  # optional env-var prefix (use non-empty if your test vars share one)
...
```
- If all your test environment variables share a prefix (for example `TEST_`), pass that prefix as the second positional
    argument to `EnvironmentVariableLoader` and include the prefix on the keys you provide (for example,
    `TEST_AZURE_AI_PROJECT_ENDPOINT`).
