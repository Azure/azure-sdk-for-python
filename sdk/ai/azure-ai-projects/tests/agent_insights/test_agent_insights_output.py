...
ON_DEMAND_OUTPUT = [
    ...
    "Listed runs: 1",
    "Listed insights: 1",
    "Insight `insight-test`: title=`Require `approval``, severity=high, status=active, traces=8, fix kind=prose.",
    "Recommended action: Check approval first.",
    "Retrieved insight `insight-test` with status active.",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("traces=8", "traces=0"),
        ("fix kind=prose", "fix kind=not returned"),
        ("Check approval first.", "None"),
        ("Retrieved insight `insight-test`", "Retrieved insight `another-insight`"),
