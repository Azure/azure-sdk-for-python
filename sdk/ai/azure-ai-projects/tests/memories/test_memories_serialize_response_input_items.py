...
from azure.ai.projects.operations._patch_memories import _serialize_response_input_items
...
def test_serialize_response_input_items_with_none() -> None:
    assert _serialize_response_input_items(None) is None


def test_serialize_response_input_items_with_str() -> None:
    result = _serialize_response_input_items("hello")
    assert result == [{"role": "user", "type": "message", "content": "hello"}]


def test_serialize_response_input_items_with_single_easy_input_message_param() -> None:
    items = [_easy_input_message_param("single message")]
    result = _serialize_response_input_items(items)
    assert result == [{"role": "user", "type": "message", "content": "single message"}]


def test_serialize_response_input_items_with_three_easy_input_message_params() -> None:
    items = [
...
def test_serialize_response_input_items_with_list_of_dict_messages() -> None:
