    ...
    for i, line in enumerate(sig_lines):
        ...
        if 'foundry_features:' in line and 'Literal[FoundryFeaturesOptInKeys.' in line:
            ...
            if ', *,' in line and '**kwargs' in line:
                ...
                new_line = re.sub(
                    r',\s*\*\s*,\s*foundry_features:\s*Literal\[FoundryFeaturesOptInKeys\.\w+\]\s*,\s*',
                    ', ',
                    line
                )
                ...
                new_line = re.sub(
                    r',?\s*foundry_features:\s*Literal\[FoundryFeaturesOptInKeys\.\w+\]\s*,?',
                    '',
                    line
                )
    ...
    while i < len(lines):
        ...
        if stripped.startswith(('@distributed_trace', '@overload')):
            ...
            if decorator != 'overload':
                ...
                # Track this method
                if not any(m['method_name'] == method_name and m['foundry_value'] == foundry_value 
