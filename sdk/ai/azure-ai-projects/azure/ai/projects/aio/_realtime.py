...
class ResponseResource(_BaseResource):
    ...
    async def create(
        ...
        *,
        response: Optional[
            Union["_models.VoiceAgentResponseCreateParams", Mapping[str, Any]]
        ] = None,
        event_id: Optional[str] = None,
    ) -> None:
    ...
    async def cancel(
        self, *, response_id: Optional[str] = None, event_id: Optional[str] = None
    ) -> None:
...
class AsyncRealtimeConnectionManager:  # pylint: disable=too-many-instance-attributes
    ...
    async def enter(self) -> AsyncRealtimeConnection:  # pylint: disable=too-many-locals
        ...
        if not url.startswith("wss://"):
            raise ValueError(
                "The realtime WebSocket URL must use wss:// to protect credentials in transit."
            )
