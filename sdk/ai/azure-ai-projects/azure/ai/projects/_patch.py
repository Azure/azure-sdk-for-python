...
class AIProjectClient(AIProjectClientGenerated):  # pylint: disable=too-many-instance-attributes
    ...
    @distributed_trace
    def get_openai_client(self, **kwargs) -> "OpenAI":  # type: ignore[name-defined]  # pylint: disable=too-many-statements
    ...
    def get_openai_client_legacy(
        ...
        if connection_name:
            ...
            if isinstance(connection.credentials, ApiKeyCredentials):
                ...
                logger.debug(
                    "[get_openai_client_legacy] Creating OpenAI client using API key authentication, on connection `%s`, endpoint `%s`, api_version `%s`",  # pylint: disable=line-too-long
                    connection_name,
                    azure_endpoint,
                    api_version,
                )
                ...
                logger.debug(
                    "[get_openai_client_legacy] Creating OpenAI using Entra ID authentication, on connection `%s`, endpoint `%s`, api_version `%s`",  # pylint: disable=line-too-long
                    connection_name,
                    azure_endpoint,
                    api_version,
                )
        ...
        logger.debug(  # pylint: disable=specify-parameter-names-in-call
            "[get_openai_client_legacy] Creating OpenAI client using Entra ID authentication, on parent AI Services resource, endpoint `%s`, api_version `%s`",  # pylint: disable=line-too-long
            azure_endpoint,
            api_version,
        )
