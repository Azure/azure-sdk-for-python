...
class EvaluatorsOperations(EvaluatorsOperationsGenerated):
    ...
    def _start_pending_upload_and_get_container_client(
        ...
        from ._patch import _FOUNDRY_FEATURES_HEADER_NAME
    ...
    def _get_next_version(self, name: str) -> str:
        ...
        try:
            versions = list(self.list_versions(name=name, headers={_FOUNDRY_FEATURES_HEADER_NAME: _EVALUATORS_FOUNDRY_FEATURES_VALUE}))
            if versions:
                numeric_versions = []
                for v in versions:
