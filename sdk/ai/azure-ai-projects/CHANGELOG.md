...
* Added Reinforcement Learning Environments (RLE) support:
  ...
  * New OpenEnv client interface `project_client.rle.get_openenv_client(name, version=None, min_concurrency=1, ...)` returning an `OpenEnvClient` / `AsyncOpenEnvClient` context manager. The environment is resolved by `name` (and `version` when supplied). Entering the client reserves the requested concurrency quota (`min_concurrency`) in advance and fails fast if it cannot be satisfied (v1 does not queue). `get_instance()` hands out a reserved `RLEInstance` / `AsyncRLEInstance` — a context manager exposing `instance_id`, `environment_id`, a data-plane URI (`dataplane_uri`), and the `reset`/`step`/`state`/`health`/`metadata`/`schema` runtime operations. Instances may be shared across episodes (returned to the pool for reuse) or scoped to a single episode; closing the client releases every reserved instance.
