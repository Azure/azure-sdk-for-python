...
* Added Reinforcement Learning Environments (RLE) support:
  * New generated models: `RLEnvironment`, `RLEnvironmentVersion`, `RLEInstanceGroup`, `RLEInstance`, `CreateRLEnvironmentRequest`, `CreateRLEInstanceGroupRequest`, `UpdateRLEInstanceGroupRequest`, `RLEInstanceGroupResourceProfile`, `RLEResetRequest`, `RLEStepRequest`, `RLEStepResult`, `RLEnvironmentState`, `ListRLEnvironmentsResponse`, `ListRLEInstanceGroupsResponse`, `ListRLEInstancesResponse`, and the `RLEInstanceGroupStatus`, `RLEInstanceStatus`, `RLEInstanceLeaseState`, and `RLEnvironmentDiskImageConversionStatus` enums.
  ...
  * New OpenEnv client interface `project_client.rle.get_openenv_client(name, version=None, num_instances=1, ...)` returning an `OpenEnvClient` / `AsyncOpenEnvClient` context manager. On both the sync and async surfaces `get_openenv_client(...)` is a plain (non-awaited) factory that performs no I/O. The environment is resolved by `name` (and `version` when supplied) on context entry, so a missing or invalid environment fails when the client is entered. Entering the client creates a single `RLEInstanceGroup` that reserves `num_instances` of concurrency on the service and fails fast if the quota cannot be granted (v1 does not queue). The service owns the pool and the reservation — the client keeps no local pool. `get_instance()` leases a running instance on demand (it is a plain accessor on the sync surface and awaited on the async surface), returning an `OpenEnvInstance` / `AsyncOpenEnvInstance` exposing `id`, `instance_group_id`, the underlying `instance`, and the `reset`/`step`/`state`/`health`/`metadata`/`schema` runtime operations. Leasing more than `num_instances` concurrently fails with `RLEAtCapacityError` until an outstanding instance is released. Closing the client deletes the instance group, which releases any instances still leased on the service; the client keeps no local list of leased instances.
  * Runtime operations (`reset`, `step`, `state`, `health`, `metadata`, `schema`) are issued against the instance runtime routes and flow through the `AIProjectClient` pipeline (auth, retries, tracing). The `reset`/`step` operations return the generated `RLEStepResult` model and `state` returns `RLEnvironmentState`; `health`, `metadata`, and `schema` return the raw JSON payload as a dictionary.
