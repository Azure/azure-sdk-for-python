...
* Extended voice agents with telephony, WebRTC, and sub-agent consultation:
  ...
  * Added the `agent_endpoint_conversations.get_agent_conversation_item_generated_audio`/`get_agent_conversation_item_generated_audio_content` methods for reading back a conversation item's *generated* audio, a subordinate artifact that can differ from what the listener heard when playback was interrupted, returning `VoiceGeneratedItemAudioResponse`. This is a new top-level operation group, distinct from `beta.agent_endpoint_conversations`.
