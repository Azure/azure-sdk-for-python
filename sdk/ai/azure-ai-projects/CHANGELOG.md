...
* Added Reinforcement Learning Environments (RLE) support:
  ...
  * New OpenEnv client interface `project_client.rle.get_openenv_client(name, version=None, ...)` returning an `OpenEnvClient` / `AsyncOpenEnvClient` context manager. On both the sync and async surfaces `get_openenv_client(...)` is a plain (non-awaited) factory that performs no I/O, so usage is symmetric (`with ... as client:` and `async with ... as client:`). The environment is resolved by `name` (and `version` when supplied) on context entry, so a missing or invalid environment fails when the client is entered. Entering the client also reserves its instances in advance and fails fast if the quota cannot be satisfied (v1 does not queue). `get_instance()` is a plain (non-awaited) accessor on both surfaces that hands out a reserved `OpenEnvInstance` / `AsyncOpenEnvInstance` — a context manager exposing `id`, `environment_id`, a data-plane URI (`dataplane_uri`), and the `reset`/`step`/`state`/`health`/`metadata`/`schema` runtime operations. Instances may be shared across episodes (returned to the pool for reuse) or scoped to a single episode; closing the client releases every reserved instance.
  * Runtime operations (`reset`, `step`, `state`, `health`, `metadata`, `schema`) are issued directly against the instance's data-plane URI (`dataplane_uri`) using the unprefixed OpenEnv routes (`/reset`, `/step`, `/state`, `/health`, `/metadata`, `/schema`), flowing through the `AIProjectClient` pipeline (auth, retries, tracing) rather than the control-plane sandbox routes. The `reset`/`step`/`state` operations return the generated `RLEStepResult` and `RLEnvironmentState` models; `health`, `metadata`, and `schema` return the raw JSON payload as a dictionary.
