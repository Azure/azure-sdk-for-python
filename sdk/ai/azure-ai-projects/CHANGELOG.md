...
* Updated Skills upload/download samples (`sample_skills_upload_and_download.py`, `sample_skills_upload_and_download_async.py`) to build the `canvas-design.zip` package from `samples/skills/assets/canvas-design/` at runtime instead of relying on a checked-in zip archive.
