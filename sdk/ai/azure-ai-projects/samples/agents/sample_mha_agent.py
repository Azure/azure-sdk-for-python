...
DESCRIPTION:
    ...
    additive `harness="ghcp"` field on `PromptAgentDefinition`. Everything else about authoring
...
def _get_final_response(stream) -> "Response":
    ...
    Managed (`harness="ghcp"`) agents reply with `Content-Type: text/event-stream`, so the
...
def _get_started_response(stream) -> "Response":
    ...
    Managed (`harness="ghcp"`) agents reply with `Content-Type: text/event-stream`, so
...
def sample_managed_agent_basic() -> None:
    ...
    field this adds to the request body is `harness="ghcp"` on `PromptAgentDefinition`, which
...
def sample_managed_agent_with_skill() -> None:
    ...
    `harness="ghcp"` on the definition - the skill/toolbox usage is identical to what a Prompt
    Agent uses today.
    """
    ...
    with (
        ...
        # Create a MANAGED agent that uses the skill. The only managed delta is harness="ghcp".
        agent = project_client.agents.create_version(
            agent_name=agent_name,
            definition=PromptAgentDefinition(
                model=model_deployment_name,
