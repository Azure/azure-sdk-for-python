...
DESCRIPTION:
    ...
    `.fine_tuning_jobs` methods to create, pause, resume, list events, and list checkpoints for fine-tuning jobs.
...
with DefaultAzureCredential(exclude_interactive_browser_credential=False) as credential:
    ...
    with AIProjectClient(endpoint=endpoint, credential=credential) as project_client:
        ...
        with project_client.get_openai_client(api_version="2025-04-01-preview") as open_ai_client:
            ...
            # [START finetuning_reinforcement_job_sample]
            # Upload training and validation files using OpenAI client
            ...
            # Create a fine-tuning job using OpenAI client with reinforcement method
            grader = {
                "type": "score_model",
                "model": "o3-mini",
                "input": [
            ...
            paused_job = open_ai_client.fine_tuning.jobs.pause(fine_tuning_job.id)
            print(paused_job)

            # Resume the fine-tuning job
            print(f"\nResuming fine-tuning job with ID: {fine_tuning_job.id}")
            resumed_job = open_ai_client.fine_tuning.jobs.resume(fine_tuning_job.id)
            print(resumed_job)

            # List events for the fine-tuning job
            print(f"\nListing events for fine-tuning job: {fine_tuning_job.id}")
            for event in open_ai_client.fine_tuning.jobs.list_events(fine_tuning_job.id, limit=10):
                print(event)
            ...
            for checkpoint in open_ai_client.fine_tuning.jobs.checkpoints.list(fine_tuning_job.id, limit=10):
                print(checkpoint)
            # [END finetuning_reinforcement_job_sample]
