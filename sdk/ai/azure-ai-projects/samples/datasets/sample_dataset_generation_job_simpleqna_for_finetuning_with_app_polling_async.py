...
USAGE:
    python sample_dataset_generation_job_simpleqna_for_finetuning_with_app_polling_async.py
...
async def main() -> None:
    async with (
        ...
        # ------------------------------------------------------------------
        # 3. Poll from application code until the job reaches a terminal state.
        # ------------------------------------------------------------------
        ...
        if job.status == JobStatus.FAILED:
            ...
            raise RuntimeError(f"Data generation job `{job.id}` failed: {message}")
        ...
        # ------------------------------------------------------------------
        # 4. Inspect the generated fine-tuning file outputs.
        # ------------------------------------------------------------------
