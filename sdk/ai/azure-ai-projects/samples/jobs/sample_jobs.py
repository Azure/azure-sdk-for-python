...
USAGE:
    ...
    4) JOB_NAME - Optional. The name of the job to create. Defaults to "sample-command-job".
"""
...
job_name = os.environ.get("JOB_NAME", "sample-command-job")
...
with (
    ...
    yaml_created_job = project_client.beta.jobs.create_or_update(name="sample-command-job-from-yaml", job=yaml_job)
    print(yaml_created_job)

    # --- Get a job ---
    print(f"\nGet job `{job_name}`:")
    retrieved_job = project_client.beta.jobs.get(name=job_name)
    print(retrieved_job)
    ...
    services = project_client.beta.jobs.show_services(job_name)
    if services:
    ...
    project_client.beta.jobs.stream(name=job_name)

    # --- Download job outputs ---
    print(f"\nDownload outputs for job `{job_name}`:")
    project_client.beta.jobs.download(name=job_name, all=True)
    print("Downloaded.")

    # --- Cancel a job ---
    print(f"\nCancel job `{job_name}`:")
    cancel_poller = project_client.beta.jobs.begin_cancel(job_name)
    cancel_poller.result()
    print("Cancelled.")

    # --- Delete a job ---
    print(f"\nDelete job `{job_name}`:")
    delete_poller = project_client.beta.jobs.begin_delete(job_name)
