...
USAGE:
    ...
    3) FOUNDRY_AGENT_CONTAINER_IMAGE - The Hosted Agent container image in the format '<registry>/<repository>[:<tag>|@<digest>]'
"""
...
http_logging_policy.allowed_query_params.update({"limit", "after", "api-version"})
...
with (
    ...
    for i in range(3):
        ...
        logger.debug(f"Creating session {i+1}/3 with id: {session_id}")
    ...
    logger.info("Listing sessions for the agent (limit: 2)...")
    sessions = project_client.beta.agents.list_sessions(agent_name=agent_name, limit=2)
