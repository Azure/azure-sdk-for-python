...
from azure.ai.generative._telemetry import GEN_AI_INTERNAL_LOGGER_NAMESPACE, get_appinsights_log_handler, OpsLogger, initialize_logger_info
from azure.ai.generative._telemetry.logging_handler import AzureGenAILogHandler
...
class TestLoggingHandler:
    def test_logging_enabled(self) -> None:
        with patch("azure.ai.generative._telemetry.logging_handler.in_jupyter_notebook", return_value=False):
            handler = get_appinsights_log_handler(user_agent=USER_AGENT)
            assert isinstance(handler, logging.NullHandler)

        with patch("azure.ai.generative._telemetry.logging_handler.in_jupyter_notebook", return_value=True):
            handler = get_appinsights_log_handler(user_agent=USER_AGENT)
            assert isinstance(handler, AzureLogHandler)
            assert isinstance(handler, AzureGenAILogHandler)
...
class TestOpsLogger:
    def test_init(self) -> None:
        ...
        assert test_logger.package_logger.name == GEN_AI_INTERNAL_LOGGER_NAMESPACE + test_name
