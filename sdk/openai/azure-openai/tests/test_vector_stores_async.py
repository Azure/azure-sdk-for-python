...
class TestVectorStoresAsync(AzureRecordedTestCase):
    ...
    async def test_vector_stores_crud(
        ...
        try:
            ...
            retrieved_vector = await client_async.vector_stores.retrieve(vector_store_id=vector_store.id)
            ...
            vector_store_files = client_async.vector_stores.files.list(vector_store_id=vector_store.id)
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("api_type, api_version", [(ASST_AZURE, PREVIEW), (GPT_4_OPENAI, "v1")])
    async def test_vector_stores_batch_crud(
        self, client_async: openai.AsyncAzureOpenAI | openai.AsyncOpenAI, api_type, api_version, **kwargs
    ):
        file_name = f"test{uuid.uuid4()}.txt"
        ...
        file_2 = await client_async.files.create(file=open(path_2, "rb"), purpose="assistants")
        try:
            vector_store = await client_async.vector_stores.create(name="Support FAQ")
