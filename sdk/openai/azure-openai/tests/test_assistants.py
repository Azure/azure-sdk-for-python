...
class TestAssistants(AzureRecordedTestCase):
    ...
    def test_assistants_bing(self, client, api_type, api_version, **kwargs):
        try:
            client = client.with_options(default_headers={"X-Ms-Enable-Preview":"true"})
            assistant = client.beta.assistants.create(
                name="python test",
                instructions="When asked for up-to-date information, use the browser tool.",
                tools=[{"type": "browser", "browser": { "bing_resource_id": os.environ["BING_RESOURCE_ID"]}}],
                model="gpt-4"
            )
            ...
            message = client.beta.threads.messages.create(
                thread_id=thread.id,
                role="user",
                content="What's the date and what's headline news today?",
            )
