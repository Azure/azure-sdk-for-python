...
class TestMgmtMonitorClient(AzureMgmtRecordedTestCase):
    ...
    def setup_method(self, method):
        self.mgmt_client = self.create_mgmt_client(azure.mgmt.monitor.MonitorManagementClient)
        ...
        if self.is_live:
            ...
            self.web_client = self.create_mgmt_client(WebSiteManagementClient)
            ...
            self.insight_client = self.create_mgmt_client(ApplicationInsightsManagementClient)
    ...
    def create_workspace(self, group_name, location, workspace_name):
        ...
        result = self.loganalytics_client.workspaces.begin_create_or_update(group_name, workspace_name, BODY)
    ...
    # use track 1 version
    def create_vmss(self, group_name, location, vmss_name, network_name, subnet_name, interface_name):
        subnet = self.create_virtual_network(group_name, location, network_name, subnet_name)
        NIC_ID = self.create_network_interface(group_name, location, interface_name, subnet)
    ...
    def test_alert_rule(self, resource_group):
        ...
        data_source = azure.mgmt.monitor.models.RuleMetricDataSource(
            resource_uri=resource_id, metric_name="CPU Credits Consumed"
        )
        ...
        rule_action = azure.mgmt.monitor.models.RuleEmailAction(
            send_to_service_owners=True, custom_emails=["monitoringemail@microsoft.com"]
        )
                ...
                "all_of": [
                    {
                        ...
                        "failing_periods": {"number_of_evaluation_periods": "4", "min_failing_periods_to_alert": "4"},
    ...
    def test_action_groups(self, resource_group):
        ...
        BODY = {
            ...
            "enabled": True,
            "email_receivers": [
                {"name": "John Doe's email", "email_address": "johndoe@email.com", "use_common_alert_schema": False}
            ],
            "sms_receivers": [{"name": "John Doe's mobile", "country_code": "1", "phone_number": "1234567890"}],
        }
        ...
        BODY = {"receiver_name": "John Doe's mobile"}
        ...
        BODY = {"tags": {"key1": "value1", "key2": "value2"}, "properties": {"enabled": False}}
    ...
    def test_activity_log_alerts(self, resource_group):
        ...
        BODY = {
            ...
            "enabled": True,
            "condition": {
                "all_of": [{"field": "category", "equals": "Administrative"}, {"field": "level", "equals": "Error"}]
            },
            "actions": {
                "action_groups": [
                    # {
        ...
        result = self.mgmt_client.activity_log_alerts.create_or_update(
            resource_group.name, ACTIVITY_LOG_ALERT_NAME, BODY
        )
    ...
    def test_autoscale_settings(self, resource_group):
        ...
        if self.is_live:
            vmss = self.create_vmss(
                RESOURCE_GROUP, AZURE_LOCATION, VMSS_NAME, NETWORK_NAME, SUBNET_NAME, INTERFACE_NAME
            )
            vmss_id = vmss.id
        else:
            vmss_id = (
        ...
        BODY = {
            "location": "West US",
            "profiles": [{"name": "adios", "capacity": {"minimum": "1", "maximum": "10", "default": "1"}, "rules": []}],
            "enabled": True,
    ...
    def test_scheduled_query_rules(self, resource_group):
        ...
        BODY = {
            ...
            "schedule": {"frequency_in_minutes": "15", "time_window_in_minutes": "15"},
            "action": {
                "odata.type": "Microsoft.WindowsAzure.Management.Monitoring.Alerts.Models.Microsoft.AppInsights.Nexus.DataContracts.Resources.ScheduledQueryRules.AlertingAction",
                "severity": "1",
                "azns_action": {"action_group": [], "email_subject": "Email Header", "custom_webhook_payload": "{}"},
                "trigger": {
        ...
        result = self.mgmt_client.scheduled_query_rules.create_or_update(
            resource_group.name, SCHEDULED_QUERY_RULE_NAME, BODY
        )
    ...
    def test_guest_diagnostics_settings(self, resource_group):
        ...
        self.mgmt_client.guest_diagnostics_settings.create_or_update(
            resource_group.name, DIAGNOSTIC_SETTINGS_NAME, BODY
        )
