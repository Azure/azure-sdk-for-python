...
from azure.monitor.querylogs._helpers import native_col_type
...
class TestLogsClient(AzureMonitorQueryLogsTestCase):
    ...
    def test_logs_resource_query(self, recorded_test, monitor_info):
        ...
        # Note: This test would need a resource ID for logs resource query
        # For now, we'll skip this test or modify it to test workspace query instead
        response = client.query_workspace(
            monitor_info["workspace_id"], "AppRequests | summarize count()", timespan=None
        )
