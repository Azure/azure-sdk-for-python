    ...
    def _transmit(self, envelopes: List[TelemetryItem]) -> ExportResult:
        ...
        if len(envelopes) > 0:
            ...
            try:
                ...
                if _is_retryable_code(response_error.status_code):
                    ...
                    if self._consecutive_redirects < self.client._config.redirect_policy.max_redirects:  # type: ignore
                        ...
                        if redirect_has_headers and url.scheme and url.netloc:  # pylint: disable=E0606
                            ...
                            if not self._is_stats_exporter():
                                if self._customer_sdkstats_metrics and self._should_collect_customer_sdkstats():
                                    _track_dropped_items(self._customer_sdkstats_metrics, envelopes, DropCode.CLIENT_EXCEPTION, exception_categories._CLIENT_EXCEPTION.value)
                                logger.error(
                ...
                if self._customer_sdkstats_metrics and self._should_collect_customer_sdkstats():
                    _track_dropped_items(self._customer_sdkstats_metrics, envelopes, DropCode.CLIENT_EXCEPTION, exception_categories.CLIENT_EXCEPTION.value)
