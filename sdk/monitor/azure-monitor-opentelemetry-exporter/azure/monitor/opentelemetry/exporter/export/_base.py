    ...
    def _transmit(self, envelopes: List[TelemetryItem]) -> ExportResult:
        ...
        if len(envelopes) > 0:
            ...
            if (
                ...
                if granted == 0:
                    logger.warning(
                        "Rate limiter rejected entire batch of %d envelopes. "
                        "Routing to local storage for retry.",
                        len(envelopes),
                    )
                    return ExportResult.FAILED_RETRYABLE
                if granted < len(envelopes):
                    # Send what we can, route the rest to local storage via FAILED_RETRYABLE
                    overflow = envelopes[granted:]
                    envelopes = envelopes[:granted]
                    logger.info(
                        "Rate limiter admitted %d of %d envelopes; "
                        "%d envelopes deferred to local storage.",
                        granted,
