...
from azure.monitor.opentelemetry.exporter._generated._configuration import (
        ...
        for blob in self.storage.gets():
            ...
            if blob.lease(self._timeout + 5):
                ...
                if blob_data is not None:
                    envelopes = [
                        _format_storage_telemetry_item(TelemetryItem.from_dict(x))
                        for x in blob_data
                    ]
...
def _format_storage_telemetry_item(item: TelemetryItem) -> TelemetryItem:
    ...
    if hasattr(item, "data") and item.data is not None:
        if hasattr(item.data, "base_data") and isinstance(
            item.data.base_data, MonitorDomain
        ):
