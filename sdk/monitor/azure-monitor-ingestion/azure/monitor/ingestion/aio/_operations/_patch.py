...
import logging
import sys
from typing import Callable, List, Any, Awaitable, Union, IO, Optional
...
class LogsIngestionClientOperationsMixin(GeneratedOps):
    async def upload(  # pylint: disable=arguments-renamed, arguments-differ
        ...
        **kwargs: Any
        ...
        :param on_error: The asynchronous callback function that is called when a set of logs fail to upload.
        ...
        :type on_error: Optional[Callable[..., Awaitable[None]]]
        :return: None
        :rtype: None
        ...
        for gzip_data, log_chunk in _create_gzip_requests(logs):
            try:
                ...
                if on_error:
                    await on_error(error=err, logs=log_chunk)
                else:
