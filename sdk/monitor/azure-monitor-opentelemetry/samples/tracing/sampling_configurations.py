...
# Set the OTEL_TRACES_SAMPLER_ARG environment variable to 0.1, it has to be a number between 0 and 1,
# else it will throw an error and default to 1.0
...
# Set the OTEL_TRACES_SAMPLER_ARG environment variable to 0.45, it has to be a number between 0 and 1,
# else it will throw an error and default to 1.0
...
# Set the OTEL_TRACES_SAMPLER_ARG environment variable to the desired rate limit (e.g., 0.5 means one
# trace every two seconds, while 5.0 means five traces per second)

# Using fixed percentage sampler
# Set the OTEL_TRACES_SAMPLER environment variable to "microsoft.fixed_percentage"
# Set the OTEL_TRACES_SAMPLER_ARG environment variable to 0.2, it has to be a number between 0 and 1,
# else it will throw an error and default to 1.0
...
# Passing the enable_trace_based_sampling_for_logs=True argument to configure_azure_monitor ensure that
# log records associated with unsampled traces are dropped by the `Logger`.
# A log record is considered associated with an unsampled trace if it has a valid `SpanId` and its `TraceFlags`
# indicate that the trace is unsampled.
