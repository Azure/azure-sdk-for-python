...
class MgmtAttestationTest(AzureMgmtTestCase):
    ...
    def test_attestation(self, resource_group):
        ...
        result = self.mgmt_client.attestation_providers.create(
            resource_group_name=RESOURCE_GROUP, provider_name=PROVIDER_NAME, creation_params=CREATION_PARAMS
        )

        # /AttestationProviders/get/AttestationProviders_Get[get]
        result = self.mgmt_client.attestation_providers.get(
            resource_group_name=RESOURCE_GROUP, provider_name=PROVIDER_NAME
        )
        ...
        result = self.mgmt_client.attestation_providers.update(
            resource_group_name=RESOURCE_GROUP, provider_name=PROVIDER_NAME, tags=TAGS
        )

        # /AttestationProviders/delete/AttestationProviders_Delete[delete]
        result = self.mgmt_client.attestation_providers.delete(
            resource_group_name=RESOURCE_GROUP, provider_name=PROVIDER_NAME
        )
