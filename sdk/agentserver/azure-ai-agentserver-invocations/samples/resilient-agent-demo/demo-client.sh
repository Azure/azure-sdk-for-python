...
# (AGENT_RESILIENT_RESEARCH_AGENT_INVOCATIONS_ENDPOINT) when run from the demo
...
ENDPOINT="${ENDPOINT:-https://<account>.services.ai.azure.com/api/projects/<project>/agents/resilient-research-agent/endpoint/protocols}"
API_VERSION="v1"
...
# AGENT_RESILIENT_RESEARCH_AGENT_INVOCATIONS_ENDPOINT looks like
...
ensure_endpoint() {
    if [[ "$ENDPOINT" == *"<account>"* || "$ENDPOINT" == *"<project>"* ]]; then
        local azd_inv
        azd_inv="$(azd env get-value AGENT_RESILIENT_RESEARCH_AGENT_INVOCATIONS_ENDPOINT 2>/dev/null || true)"
    ...
    if [[ "$ENDPOINT" == *"<account>"* || "$ENDPOINT" == *"<project>"* ]]; then
        ...
        echo -e "  export ENDPOINT=\"https://<account>.services.ai.azure.com/api/projects/<project>/agents/resilient-research-agent/endpoint/protocols\"" >&2
        exit 1
    fi
}
