#!/usr/bin/env python3
"""Local resilient crash-recovery demo for the invocations resilient-agent-demo.

Runs the `resilient-research-agent` **entirely on your machine** — the resilient
...
``~/.agentserver-tasks`` dir, so you do **not** need the hosted Foundry task API
...
What it demonstrates, automatically, in one run:
  ...
  2. ``POST /invocations {"message": "<topic>"}`` starts a long-running research
     task (one resilient phase checkpoint per research phase). It returns an
  ...
  4. Restarts the server against the **same** resilient root + session id. On
...
APP_PY = HERE.parent / "src" / "resilient-research-agent" / "app.py"
...
# The agent hard-codes its streams + checkpoints under ~/.agentserver-tasks.
AGENT_STATE_DIR = Path.home() / ".agentserver-tasks"
...
# Child-process env: real LLM via the project endpoint, resilience local.
...
def main() -> int:
    ...
    banner(f"[1/4] Starting local resilient agent (task store {RESILIENT_ROOT}, session {SESSION_ID})")
    p1 = start_server("1")
