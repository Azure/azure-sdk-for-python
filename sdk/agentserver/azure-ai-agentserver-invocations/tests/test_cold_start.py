...
# ---------------------------------------------------------
"""Cold-start regression coverage for the SDK-owned startup path."""
...
_PROCESS_TO_READINESS_BUDGET_SECONDS = 4.0
...
def test_sdk_cold_start_budgets() -> None:
    """A minimal SDK host becomes ready within broad cold-start regression budgets."""
    runs = [_measure_cold_start() for _ in range(_RUN_COUNT)]
    best = {metric: min(run[metric] for run in runs) for metric in runs[0]}

    print(f"best-of-{_RUN_COUNT} SDK cold-start metrics: {best}")
