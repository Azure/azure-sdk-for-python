...
(WebSocket invocations_ws protocol).
"""
import os
...
if _HAS_OTEL:
    ...
    if isinstance(existing, SdkTracerProvider) and existing is not _PROVIDER:
        existing.add_span_processor(SimpleSpanProcessor(_EXPORTER))
    else:
...
def test_ws_framework_span_is_child_of_invoke_span():
...
def test_ws_framework_span_is_child_streaming():
