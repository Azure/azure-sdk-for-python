...
# ---------------------------------------------------------
"""Tests for graceful shutdown with InvocationWSAgentServerHost."""
import asyncio
import logging
...
def _make_server_with_shutdown(**kwargs) -> tuple[InvocationWSAgentServerHost, list]:
    """Create InvocationWSAgentServerHost with a tracked shutdown handler."""
...
@pytest.mark.asyncio
async def test_ws_shutdown_handler_called_on_lifespan_exit():
...
@pytest.mark.asyncio
async def test_ws_shutdown_handler_timeout(caplog):
    """Shutdown handler that exceeds timeout is warned about."""
    server = InvocationWSAgentServerHost(graceful_shutdown_timeout=1)
    calls: list[str] = []
...
@pytest.mark.asyncio
async def test_ws_shutdown_handler_exception(caplog):
...
def test_ws_default_graceful_shutdown_timeout():
    """Default graceful shutdown timeout is 30 seconds."""
    app = InvocationWSAgentServerHost()
    assert app._graceful_shutdown_timeout == 30


def test_ws_custom_graceful_shutdown_timeout():
    """Custom graceful_shutdown_timeout is stored."""
    server = InvocationWSAgentServerHost(graceful_shutdown_timeout=60)
    assert server._graceful_shutdown_timeout == 60


def test_ws_zero_graceful_shutdown_timeout():
...
def test_ws_health_endpoint_during_operation():
...
def test_ws_multiple_requests_before_shutdown():
