...
from azure.ai.agentserver.agentframework.persistence._foundry_conversation_message_store import (
...
@pytest.mark.unit
def test_foundry_message_store_is_base_history_provider() -> None:
    provider = FoundryConversationMessageStore(project_client=Mock())
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_foundry_message_store_reads_remote_and_cached_messages(monkeypatch) -> None:
    provider = FoundryConversationMessageStore(project_client=Mock())
