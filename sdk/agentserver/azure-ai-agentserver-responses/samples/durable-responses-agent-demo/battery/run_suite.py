#!/usr/bin/env python3
"""Hosted battery driver for durable-responses-agent-demo.
...
AGENT = "durable-responses-agent-demo"
ENDPOINT_BASE = (
...
def fire_crash() -> dict:
    ...
    running the in-flight durable task. ``os._exit(137)`` drops the streaming
    connection mid-flight, which we detect as the positive "sandbox went down"
    signal. The platform restores the sandbox within ~2 min and the durable
    task recovers.
    """
...
def steer_crash(rid: str) -> dict:
    """Deliver a 'crash' input onto the TARGET's own durable task via steering.

    A fresh ``POST /responses`` "crash" may be routed to a different replica
    than the one holding the target durable task's lease, so it would not kill
    the right container. Steering with ``previous_response_id`` queues the input
    on the SAME durable task, so the lease-holding container processes it and
...
def case_crash_recovery(d: Path, combo: dict, stream: bool) -> dict:
    ...
    # terminal is itself proof the durable run recovered (it could not finish on
...
# Comfortably over the core inline threshold (~200 KB) so the durable-task input
...
def case_oversized_input(d: Path, combo: dict) -> dict:
    """>300 KB normal input → durable-input attachment spill round-trips losslessly."""
