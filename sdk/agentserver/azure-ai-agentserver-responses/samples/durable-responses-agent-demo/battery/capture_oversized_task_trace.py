#!/usr/bin/env python3
"""Capture a full, untruncated HTTP trace of the failing oversized durable-task
...
byte-faithful to what the durable-responses path sends, wrapping the transport to
...
class AzCliCredential:
    ...
    async def get_token(self, *scopes, **kwargs) -> AccessToken:  # noqa: ARG002
        tok = subprocess.run(
            ["az", "account", "get-access-token", "--resource", AUTH_RESOURCE,
...
def _fmt(rec: dict, attach_bytes: int, attach_key: str) -> str:
    ...
    out = []
    out.append("=" * 100)
    out.append("RAW HTTP TRACE — durable-task create (POST /tasks) with oversized attachment")
    out.append(f"captured: {datetime.now(timezone.utc).isoformat()}")
    out.append(f"attachment: key={attach_key!r} value_size={attach_bytes} bytes "
...
async def main() -> None:
    ...
    # >threshold size (the responses durable path spills the serialized input here).
    big = "A long research input. " * ((attach_bytes // 23) + 1)
    big = big[:attach_bytes]
    attach_key = "input"
    session_id = f"task-trace-{uuid.uuid4().hex}"
    task_id = f"durable-resp-{uuid.uuid4().hex}"
