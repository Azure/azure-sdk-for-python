...
| [Resilient LangGraph](https://github.com/Azure/azure-sdk-for-python/tree/main/sdk/agentserver/azure-ai-agentserver-responses/samples/sample_21_resilient_langgraph.py) | LangGraph `SqliteSaver` composed with framework `stream.checkpoint()` / `context.persisted_response`; `context.conversation_chain_id` as the thread id |
