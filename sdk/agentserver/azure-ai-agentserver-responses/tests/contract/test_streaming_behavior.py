...
async def _throwing_before_yield_handler(request: Any, context: Any):
...
async def _throwing_after_created_handler(request: Any, context: Any):
...
def test_streaming__forwards_emitted_event_before_late_handler_failure() -> None:
    async def _fail_after_first_event_handler(request: Any, context: Any):
        async def _events():
            yield {
