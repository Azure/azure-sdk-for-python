...
def test_sync_handler_exception_returns_500() -> None:
    ...
    async def _raising_handler(request: Any, context: Any):
        async def _events():
...
def test_sync_no_terminal_event_still_completes() -> None:
    ...
    async def _no_terminal_handler(request: Any, context: Any):
        async def _events():
            stream = ResponseEventStream(response_id=context.response_id, model=getattr(request, "model", None))
            yield stream.emit_created()
            yield stream.emit_in_progress()
...
def test_s007_wrong_first_event_sync() -> None:
    ...
    async def _wrong_first_event_handler(request: Any, context: Any):
        async def _events():
...
def test_s008_mismatched_id_stream() -> None:
    ...
    async def _mismatched_id_handler(request: Any, context: Any):
        async def _events():
...
def test_s009_terminal_status_on_created_stream() -> None:
    ...
    async def _terminal_on_created_handler(request: Any, context: Any):
        async def _events():
            yield {
                "type": "response.created",
                "response": {
...
def test_s007_valid_handler_not_affected() -> None:
    ...
    async def _compliant_handler(request: Any, context: Any):
        async def _events():
            stream = ResponseEventStream(response_id=context.response_id, model=getattr(request, "model", None))
            yield stream.emit_created()
            yield stream.emit_completed()
