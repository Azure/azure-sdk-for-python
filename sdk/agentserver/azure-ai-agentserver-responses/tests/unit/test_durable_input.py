...
"""Unit tests for the typed durable-recovery boundary (Spec 033 §3.1).
...
def _make_input(**overrides) -> DurableResponseInput:
    kwargs = dict(
    ...
    return DurableResponseInput(**kwargs)
...
def test_round_trip_preserves_all_fields() -> None:
    ...
    restored = DurableResponseInput.from_task_input(original.to_task_input())
...
def test_input_embedded_once_no_input_items_key() -> None:
    ...
    assert DurableResponseInput.from_task_input(params).request.input == "crash during task"
...
def test_agent_reference_model_is_normalized_not_leaked() -> None:
    ...
    non-serializable value into the durable input."""
    durable = _make_input(agent_reference=AgentReference(name="agent-x", version="2"))
    params = durable.to_task_input()  # would raise TypeError if the model leaked
    json.dumps(params)
...
def test_from_task_input_missing_request_raises() -> None:
    with pytest.raises(ValueError):
        DurableResponseInput.from_task_input({"response_id": "resp_abc"})


def test_from_task_input_missing_response_id_raises() -> None:
    with pytest.raises(ValueError):
        DurableResponseInput.from_task_input({"request": {"input": "hi"}})


def test_from_task_input_non_dict_raises() -> None:
    with pytest.raises(ValueError):
        DurableResponseInput.from_task_input(None)  # type: ignore[arg-type]
...
def test_isolation_method_and_params_helper_agree() -> None:
    ...
    durable = _make_input()
    params = durable.to_task_input()

    iso_typed = durable.isolation()
...
def test_isolation_absent_keys_default_to_none() -> None:
    durable = _make_input(user_isolation_key=None, chat_isolation_key=None)
    iso = durable.isolation()
