...
from azure.ai.agentserver.responses.streaming._sse import (
...
def test_sse_writer__strips_storage_only_saved_at_from_mapping_events() -> None:
    event = {"type": "response.created", "sequence_number": 0, "_saved_at": "internal"}

    encoded = _sse.encode_sse_event(event)  # type: ignore[arg-type]

    assert "_saved_at" not in encoded
...
def test_sse_writer__injects_monotonic_sequence_numbers() -> None:
    ...
    def _extract_sequence_number(encoded: str) -> int:
        data_line = next(
            line for line in encoded.splitlines() if line.startswith("data:")
        )
    ...
    assert (
        seq_first == 0
    ), f"first sequence_number must be 0 for a new stream, got {seq_first}"
    assert (
        seq_second == 1
    ), f"second sequence_number must be 1 for a new stream, got {seq_second}"
