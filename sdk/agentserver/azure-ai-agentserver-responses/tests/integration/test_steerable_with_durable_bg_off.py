...
``steerable_conversations=True + durable_background=False``. This e2e
...
durable_background=False)`` raised ValueError at construction time.
Post-spec-024: this combination is valid; the lock/queue semantics of
steering are independent of the durability/recovery disposition.
...
def test_options_construction_with_steerable_and_durable_bg_off() -> None:
...
@pytest.mark.asyncio
async def test_steerable_chain_extends_across_turns_with_durable_bg_off() -> None:
    """Three sequential turns on the same conversation_id all complete.

    Verifies the chain extends regardless of the durability disposition.
    ...
    def _handler(request, context, cancellation_signal):  # pylint: disable=unused-argument
        async def _events():
    ...
    with TestClient(host) as client:
        conversation_id = "conv_steerable_durable_off_test"
