...
"""E2E tests for durable background orchestration (Phase 1).
...
def _make_durable_app(handler, *, steerable: bool = False, **kwargs) -> TestClient:
    """Create a TestClient with a durable ResponsesAgentServerHost."""
...
# ---------------------------------------------------------------------------
# Baseline: Normal completion (background + store=true + durable)
# ---------------------------------------------------------------------------


class TestDurableOrchestrationBaseline:
    """Verify background durable responses complete normally (no crash)."""
    ...
    def test_durability_context_accessible_in_handler(self) -> None:
        ...
        # DurabilityContext should be populated (or None if not yet wired)
        # Phase 1 wiring makes it available
        dc = captured.get("durability")
        # Initially None until T011 wires the durable path into run_background
        # After T011: assert dc is not None; assert dc.entry_mode == "fresh"


class TestDurableOrchestrationFailure:
    """Tests for handler failures in durable mode."""
...
class TestDurableOrchestrationParallelForks:
