...
"""E2E tests for durable conversation locking (Phase 2).
...
- durable_background=False opt-out: no task wrapping, plain asyncio
"""
...
def _make_app(handler, *, durable: bool = True, steerable: bool = False) -> TestClient:
    """Create a TestClient with configurable durability options."""
...
# ---------------------------------------------------------------------------
# durable_background=False opt-out
# ---------------------------------------------------------------------------


class TestDurableOptOut:
    """durable_background=False: plain asyncio, no task wrapping."""
    ...
    def test_non_durable_store_false_still_works(self) -> None:
        ...
        # store=false, background=false → foreground non-durable
