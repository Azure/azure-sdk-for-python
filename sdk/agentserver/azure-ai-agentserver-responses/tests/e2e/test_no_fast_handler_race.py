...
runs inside the durable task body, so the race is architecturally
impossible.

This test fires many fast Row 2 (``durable_background=False``,
...
``_start_durable_background`` runs before the handler can call
...
async def test_no_fast_handler_race_row_3(
    ...
    harness = make_harness(
        durable_background=True,  # row 3 is durable_background-agnostic
        handler_sleep_ms=HANDLER_SLEEP_MS,
        shutdown_grace_seconds=LONG_GRACE_S,
    )
    await harness.start()
