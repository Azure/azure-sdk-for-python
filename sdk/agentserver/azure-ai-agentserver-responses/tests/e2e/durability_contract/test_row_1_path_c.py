...
"""Row 1 × Path C — ``(store=true, bg=true, durable_bg=True)`` × ``stream=F/T``.

Path C: SIGKILL mid-handler — no in-process action runs. On the next
process lifetime, the durable-task primitive's recovery re-invokes the
...
- ``stream=True``: **RED — divergence 1.** Same root cause as Path B:
  no durable record exists for the streamed POST. Phase 3 closes this.
