...
The streaming sub-contract (``durability-contract.md`` clause 3) says: on
...
Contract source: ``docs/durability-contract.md`` § Streaming sub-contract,
...
async def _full_stream(client, response_id: str) -> list[dict]:
    """GET the full durable stream from the start and collect parsed events."""
    events: list[dict] = []
...
async def test_reset_event_carries_corrected_output_items(
    ...
    try:
        ...
        assert reset_markers == ["L0_phase0", "L0_phase1"], (
            ...
            "(durability-contract.md streaming clause 3). Expected "
