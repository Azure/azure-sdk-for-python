...
durable boundary embedded the input twice. This test fails if a recovered
...
from tests.e2e.durability_contract.conftest import poll_until_terminal

_PARITY_HANDLER = "tests.e2e.durability_contract._input_parity_handler"


@pytest.mark.asyncio
async def test_recovered_input_parity(tmp_path: Path) -> None:
    """A recovered durable-background handler sees the same inputs as fresh entry."""
...
async def test_recovered_input_parity_oversized(tmp_path: Path) -> None:
    ...
    The durable-task input exceeds the inline threshold and spills to
