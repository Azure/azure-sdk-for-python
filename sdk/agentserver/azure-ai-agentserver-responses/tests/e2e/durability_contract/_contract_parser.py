...
"""Parse ``durability-contract.md`` § The matrix into typed records.
...
class ContractRow:
    """One row of ``durability-contract.md`` § The matrix.
    ...
    durable_background: str  # "True" | "False" | "any"
    path_a_text: str
    path_b_text: str
    path_c_text: str
...
def _contract_path() -> Path:
    """Locate ``durability-contract.md`` relative to this test file.
...
def _extract_matrix_section(text: str) -> str:
    ...
    if match is None:
        raise ValueError(
            "Could not find '## The matrix' section in durability-contract.md. "
...
def _parse_matrix_table(section: str) -> list[ContractRow]:
    ...
    Expected column layout (per contract doc):
        ...
        | Row | store | background | durable_background | Path A | Path B | Path C |
    """
    ...
    for raw_line in section.splitlines():
        ...
        rows.append(
            ContractRow(
                ...
                durable_background=cells[3].strip("` "),
                path_a_text=cells[4],
                path_b_text=cells[5],
                path_c_text=cells[6],
            )
