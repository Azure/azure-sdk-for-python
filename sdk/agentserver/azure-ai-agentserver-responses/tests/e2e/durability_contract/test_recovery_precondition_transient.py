...
The recovery gate (``_durable_orchestrator.py:629-653``) drops a recovered
...
case the contract also requires (``durability-contract.md`` recovery gate;
``responses-durability-spec.md`` §7.1).

Real signal only: a real SIGKILL after the response is durably persisted, then a
...
_HANDLER = "tests.e2e.durability_contract._transient_recovery_handler"
...
async def test_recovery_proceeds_on_transient_store_error(tmp_path: Path) -> None:
    ...
    try:
        ...
        # Real crash AFTER persistence → the response IS durably created
