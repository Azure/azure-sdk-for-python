...
"""Row 2 × Path C — ``(store=true, bg=true, durable_bg=False)`` × ``stream=F/T``.
...
``failed`` (with ``code=server_error``) via the durable-task primitive's
...
``_start_durable_background`` on ``durable_background AND store``. With
``durable_background=False`` no durable record is created; next-lifetime
recovery finds nothing for the response; nothing marks it failed.
The response stays ``in_progress`` indefinitely.

Phase 4 closes this by creating a bookkeeping durable record for every
