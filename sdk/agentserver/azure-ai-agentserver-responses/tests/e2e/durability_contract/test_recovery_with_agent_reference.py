...
``json.dumps``-serializable). That model flows into the durable-task input
(``_start_durable_background`` -> ``start_durable`` -> ``_split_runtime_refs``).
If it is persisted un-normalized, the core durable ``create_and_start`` ->
``_resolve_input_storage`` size check raises
``TypeError: Object of type AgentReference is not JSON serializable`` and the
whole durable start **silently falls back to a non-durable ``asyncio.create_task``**
— so no durable task exists and crash recovery never happens.

Every other durability test sends NO ``agent_reference`` (so
...
``agent_reference`` on the request and then crashes (Path C). Because durable
start is **provider-agnostic**, the bug reproduces locally: if the model leaks
into the durable input, the durable task is never created, the SIGKILL'd
non-durable task is lost, and recovery never reaches ``completed`` — failing
this test. With the fix (normalize model -> dict before persisting) the durable
...
_AGENT_REFERENCE = {
    ...
    "name": "durability-conformance-agent",
    "version": "1",
}
...
async def test_row_1_path_c_recovers_with_agent_reference(
    ...
    """A durable bg request carrying an ``agent_reference`` MUST still start a
    durable task and recover after SIGKILL.

    Regression guard for the hosted ``AgentReference is not JSON serializable``
    durable-start failure that silently degraded durable background responses to
    non-durable ``asyncio.create_task`` (no crash recovery).
    """
    ...
    try:
        ...
        # If agent_reference broke durable start, the SIGKILL'd asyncio fallback
        # left no durable record -> this never reaches "completed".
