...
Real-signal conformance (Constitution Principle X): a durable background
...
_DROP_HANDLER = "tests.e2e.durability_contract._drop_handler"
...
async def test_recovery_drop_when_unpersisted(tmp_path: Path) -> None:
    """A non-streaming durable background response crashed before
    ...
    *before* the stream-vs-non-stream dispatch (verified by code position);
    ...
    try:
        ...
        # sleep. The durable task record exists; the response is NOT persisted.
        ...
        # The response was never durably created — GET MUST be not-found.
