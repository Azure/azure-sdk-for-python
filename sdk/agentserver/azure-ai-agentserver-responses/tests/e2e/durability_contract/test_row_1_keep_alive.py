...
"""Row 1 × Path C with SSE keep-alive ENABLED — durability must not depend on
...
(:meth:`_ResponseOrchestrator._live_stream`) used to create the durable task
ONLY on its non-keep-alive code path; with keep-alive enabled it ran the
handler inline and never created a durable task. Stored background responses
...
durable_bg=True``) MUST create a durable task and recover after a crash
(Path C) **regardless of keep-alive**. It mirrors ``test_row_1_path_c`` but
runs with keep-alive on.

Expected on the BUGGED orchestrator: RED — no durable task is created under
keep-alive, so recovery never happens and ``poll_until_terminal`` times out.
Expected on the FIXED orchestrator: GREEN — the durable task is created, the
...
Constitution: Principle X (Durability Contract Conformance), Principle XI
...
async def test_row_1_keep_alive_path_c(make_harness: Callable[..., CrashHarness], stream: bool) -> None:
    ...
    asserting ``L1_done`` proves the durable task was created and recovered
    ...
    try:
        ...
        # Path C for Row 1 is recovery (NOT marked-failed): a durable task was
