...
2. Emit ``response.created`` so the response is durably persisted (recovery
...
async def _observed(request: CreateResponse, context: ResponseContext) -> dict:
    ...
    return {
        ...
        "isolation_user_key": context.isolation.user_key,
        "isolation_chat_key": context.isolation.chat_key,
