...
"""E2E tests for durable samples (17-22).
...
# ---------------------------------------------------------------------------
# Sample 17: Durable Claude (tests the handler pattern, no real Anthropic SDK)
# ---------------------------------------------------------------------------
...
class TestSample17DurableClaude:
...
# ---------------------------------------------------------------------------
# Sample 18: Durable Copilot (tests the handler pattern, no real OpenAI SDK)
# ---------------------------------------------------------------------------
...
class TestSample18DurableCopilot:
    def test_streaming_emits_created_first(self) -> None:
...
# ---------------------------------------------------------------------------
# Sample 19: Durable Streaming (simulated LLM)
# ---------------------------------------------------------------------------
...
class TestSample19DurableStreaming:
    def test_streaming_emits_created_first(self) -> None:
...
# ---------------------------------------------------------------------------
# Sample 20: Durable Steering (with CancellationReason)
# ---------------------------------------------------------------------------
...
class TestSample20DurableSteering:
...
# ---------------------------------------------------------------------------
# Sample 22: Durable Multi-turn
# ---------------------------------------------------------------------------


def _make_sample22_app() -> TestClient:
    options = ResponsesServerOptions(durable_background=True, steerable_conversations=False)
    app = ResponsesAgentServerHost(options=options)
...
class TestSample22DurableMultiturn:
    ...
    def test_done_terminates_session(self) -> None:
        """When durability context is available, 'done' produces session-complete message."""
        client = _make_sample22_app()
