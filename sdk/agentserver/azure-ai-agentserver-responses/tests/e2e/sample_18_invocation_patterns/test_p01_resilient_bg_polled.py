...
"""Sample 18 invocation pattern p01 — resilient_bg + bg + polled.

Pattern: ``(store=true, background=true, resilient_background=True, stream=False)``.
...
in a resilient task, so server crashes mid-handler trigger re-invoke.
...
- **Path B** — SIGTERM with short grace while the handler is awaiting
  ...
  grace). The framework leaves the resilient task ``in_progress`` so
