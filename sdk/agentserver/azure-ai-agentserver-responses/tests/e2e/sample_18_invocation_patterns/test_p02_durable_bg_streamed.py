...
"""Sample 18 invocation pattern p02 — durable_bg + bg + streamed.

Pattern: ``(store=true, background=true, durable_background=True, stream=True)``.

The closure of spec 014 divergence 1. The user POSTs a streaming
background request; the framework runs the handler inside the durable
