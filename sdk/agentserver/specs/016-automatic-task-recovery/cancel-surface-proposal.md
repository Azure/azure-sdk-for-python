...
- **Rename + fix `ctx.was_steered` → `ctx.is_steered_turn`** (user direction 2026-06-01): the existing `was_steered` field name suggests "the task was steered at some point in its history" but the developer's actual need is "is this turn's invocation the result of a steering drain". The current implementation at `_manager.py:876-880` makes the flag sticky-True after the first drain forever (clauses `pending_inputs non-empty` and `generation > 0` both make it True for every subsequent entry, even normal resumes), which is the bug behind the confusion. The fix has two parts: (a) rename to `is_steered_turn` so the field name accurately conveys present-tense "this current invocation is a steered turn"; (b) set `is_steered_turn = True` ONLY when constructed by `_try_drain_steering` directly (at `_manager.py:1499`); for every other entry path (`create_and_start`, `_start_existing_task` for both fresh-resume and recovery), `is_steered_turn = False`. The flag is **orthogonal** to `entry_mode` — a `(entry_mode="recovered", is_steered_turn=True)` combination is legal and meaningful (previous process crashed mid-drain; new process picks up the queued steering input). Docstring + dev guide MUST explicitly call out this orthogonality so developers don't treat the two axes as a single state machine.
- **Drop `ctx.steering_generation`** from `TaskContext.__slots__` / `__init__` / `_try_drain_steering`'s new-context construction (user direction 2026-06-01). No legitimate developer use case the handler can't cover with their own `ctx.metadata`-stored counter: it counts only drains (not normal turns), so it's not a "session length" metric; the legitimate "did the record change since I read it" need is covered by etag CAS; idempotency is a handler concern, served by metadata. The internal payload field `_steering["generation"]` is RETAINED for drain bookkeeping (drain order, persistence invariants) — deletion of the internal field is deferred to the FR-021..FR-024a implementation plan phase where we trace every read site against the post-FR-023 invariants. If no load-bearing internal use remains after that trace, the internal field is dropped too; otherwise it stays internal.
