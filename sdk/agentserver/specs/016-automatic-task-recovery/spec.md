...
7. **Lease owner identity is incomplete.** The stable lease owner is currently derived from the session ID alone (`session:{session_id}`). If two different agents ever share a session ID — a configuration error or a future multi-agent platform topology — they would collide on lease ownership and step on each other's tasks, and the platform's `binding_mismatch` protection (which guards against split-brain on the *same* agent+session) wouldn't catch it. The owner string must include both the agent name (from `FOUNDRY_AGENT_NAME`, already read elsewhere in the framework) and the session ID.
...
- **FR-004a (lease owner identity scoped to agent + session)**: The stable lease owner string MUST be derived from BOTH the agent name (from `FOUNDRY_AGENT_NAME`) AND the session identifier — not from the session ID alone. Today the derivation is `session:{session_id}`, which means two different agents that happen to share a session ID (a misconfiguration or a future multi-agent platform topology) could collide on lease ownership and step on each other's tasks; the platform's `binding_mismatch` protection (FR-006) protects against split-brain on the same agent+session but is silent on this orthogonal case. Including the agent name closes that hole at the framework layer.
  - The framework MUST resolve the agent name from `FOUNDRY_AGENT_NAME` (the platform env var already read by `_config.py` and used by the tracing path). If the env var is unset or empty, the framework MUST use a documented fallback (e.g., `"unknown-agent"`) so the lease owner is always well-defined; the fallback choice MUST be the same one the rest of the framework already uses for agent name (so traces, logs, and lease ownership agree on the same string).
  - The on-the-wire format of the owner string is plan-phase detail (e.g., `"{agent_name}:{session_id}"`, `"agent={agent_name};session={session_id}"`, etc.) — the spec requires only that BOTH components are present and that the format is stable across process restarts within the same agent+session.
  - This is a pre-release contract change; existing lease records (if any) generated with the old `session:{session_id}` format MUST be considered orphan ownership and either reclaimed via the standard recovery path or ignored — no migration code is required.
...
- **SC-005a (lease owner identity)**: The lease owner string derived by the framework contains BOTH the agent name and the session ID. A test asserts: (a) given `FOUNDRY_AGENT_NAME=agentA` and `session_id=S1`, the owner differs from the owner under `FOUNDRY_AGENT_NAME=agentB` with the same `session_id=S1`; (b) the owner is stable across process restarts within the same `(agent_name, session_id)` pair; (c) when `FOUNDRY_AGENT_NAME` is unset, the framework falls back to the same agent-name string used by the rest of the framework (tracing, logs); (d) the owner string is symbolically traceable to both inputs (the test asserts both `agent_name` and `session_id` substrings appear in the owner, OR uses whatever stable serialization the plan phase picks — the contract is "both components present", not the specific format).
...
### Conformance test map (non-duplication rule — extend existing tests; do NOT create parallel suites)

The audit of `azure-ai-agentserver-core/tests/durable/` identified the existing home for every conformance assertion this spec requires. New test modules are justified ONLY when no existing home exists for the surface area — currently only the split-brain / eviction and azure.core-transport concerns qualify. The conformance gap-list document MUST cite this table and record any deviation with explicit justification.

| Surface area (with FR references) | Existing test file → extension required | Or: new file + justification |
|---|---|---|
| Public surface enumeration: presence of new symbols (`timeout_exceeded`, `cancel_requested`, `pending_input_count`, `is_steered_turn`, `exit_for_recovery`); absence of removed symbols (`stale_timeout`, `was_steered`, `pending_inputs`, `steering_generation`, `TaskTerminated`, `is_superseded`); `TaskResult.status` Literal narrowing (FR-001, FR-010, FR-016..FR-022, FR-027) | **Extend `test_public_api_surface.py`** with: new presence assertions (`hasattr` for the added fields/methods on `TaskContext` and `TaskRun`); new absence assertions (`hasattr == False` for removed fields; `ImportError` for `TaskTerminated`); `TaskResult.status.__args__ == ("completed", "suspended")`; signature inspection for `exit_for_recovery()` (only `self`). | — |
| Developer-guide doc-review meta-test invariants (presence of new field names + new prose phrases; absence of removed symbols; absence of "lease will eventually expire" false claim) (FR-001, FR-010, FR-016..FR-022, FR-026..FR-027, SC-018) | **Extend `test_dev_guide_review.py`** with the new presence/absence regex invariants enumerated in the Authoring Sequence. | — |
| `@task` decorator surface (rejected kwargs raise `TypeError`; signature inspection; `TaskOptions` slot enumeration) (FR-001) | **Extend `test_decorator.py`** with negative assertions for the removed `stale_timeout` kwarg. | — |
| `Task.run` / `Task.start` / `Task.options` lifecycle, including resume-after-suspend, terminal-task `TaskConflictError`, scheduling-primitive observability (FR-005, FR-008, FR-012) | **Extend `test_lifecycle.py`** with the Invariant-1 sweep cells for `live-mine` and `dead-reclaimable` (the third column — `dead-evicted` — extends here too, but the underlying split-brain provider fixture lives in the new module below). | — |
| `TaskContext.entry_mode` observability across fresh / resumed / recovered + composition with `is_steered_turn` (FR-020 orthogonality) (SC-011) | **Extend `test_entry_mode.py`** with the `(entry_mode="recovered", is_steered_turn=True)` composite case. | — |
| Steering behavior: drain order, suspend-output preservation, terminal-with-queued-input `TaskConflictError`, FIFO multi-steerer ordering, `pending_input_count` live semantics, `is_steered_turn` correctness (FR-010..FR-014, FR-019..FR-020) | **Extend `test_steering.py`** with the parametrized sweeps from SC-008 / SC-010 / SC-011 (handler-end × steerer-timing combinations; live-count vs snapshot; sticky-True regression guard). | — |
| Metadata auto-flush invariant across every terminal-of-turn boundary (FR-015, SC-009) | **Extend `test_metadata.py`** with the per-boundary marker-survives-crash sweep. | — |
| Cancellation + timeout behavior: cooperative semantics, watchdog respawn, per-turn budget, durable wall-clock anchor, clock-skew clamping, cancel-cause booleans (FR-016..FR-018, FR-023..FR-026, SC-010, SC-012, SC-013) | **Extend `test_cancellation_timeout.py`** with the new boolean-cause assertions, per-turn-budget regression sweep, crash-recovery-preserves-budget sweep, and clock-skew clamping test. The existing file already owns this surface area for spec 005; this is its natural extension. | — |
| `ctx.exit_for_recovery()` semantics: success path leaves `in_progress`, recovery re-enters, misuse raises `RuntimeError`, signature has no parameters (FR-027..FR-028, SC-015) | **Extend `test_cancellation_timeout.py`** (shutdown is logically part of the cancellation-and-lifecycle test surface) OR `test_lifecycle.py` (since the success path is observable as a recovered re-entry). The gap-list MUST pick one home and justify; do NOT create a separate `test_exit_for_recovery.py` unless both candidates are deemed wrong fits during the plan phase. | — |
| `TaskRun.terminate()` and `TaskTerminated` removal: import raises `ImportError`; `hasattr(TaskRun, 'terminate') == False`; ported tests no longer reference removed symbols (FR-022, SC-014) | **Extend `test_public_api_surface.py`** (surface assertions) AND port existing tests in `test_cancellation_timeout.py` that exercised `.terminate(...)` (rewrite them to call `.cancel()` and assert handler-chosen terminal, OR to call `.cancel()` against a `raise`-on-cancel handler and assert `TaskFailed`). | — |
| Existing recovery infrastructure (`get_active_run` reclaim semantics, lease-liveness signal) (FR-002 layer (c), FR-004, FR-005, US3, US4) | **Extend the existing test file covering `get_active_run`** (the gap-list MUST cite the exact existing module — candidates: `test_lifecycle.py` if it already covers `get_active_run`, else `test_get.py`). | — |
| Lease owner identity scoped to agent + session (FR-004a, SC-005a) | **Extend `test_local_provider.py`** (which currently constructs `lease_owner` strings directly for its fixtures) OR `test_lifecycle.py` (which exercises the manager's owner derivation end-to-end). Gap-list picks one. The test sets `FOUNDRY_AGENT_NAME` via env-var override and asserts the derived owner string differs across two agents sharing a session ID, and asserts stability across simulated process restart. There is also a unit test of `derive_lease_owner` itself — extend whichever existing test file covers `_lease.py` (gap-list cites). | — |
| Completeness meta-test sees every new public symbol (FR-001, FR-010, FR-016..FR-022, FR-027, SC-016, SC-018) | **Extend `test_contract_completeness.py`** automatically picks these up via its existing `__all__` traversal; verify it stays green after the surface changes land. | — |
| **Split-brain eviction & store-error classifier** (FR-006..FR-008, US2, SC-002, SC-005, SC-006) | — | **NEW: `test_split_brain_eviction.py`.** Justification: requires a custom provider stub that returns `409 + binding_mismatch` and exercises every store-write site (reclaim, lease renewal, terminal write, input enqueue) against the classifier seam. This is a genuinely new fixture concern that doesn't fit any existing thematic module. The new module must be referenced from `test_contract_completeness.py`'s coverage matrix (or its equivalent). |
| **Azure.core transport migration** (FR-029..FR-034, SC-016, SC-017) | — | **NEW: `test_hosted_provider_transport.py`** (or similar — gap-list picks the exact name). Justification: requires a fake `AsyncHttpTransport` injected into the pipeline to assert policy-chain composition, retry behavior, gzip round-trip, and non-JSON error classification. This is a transport-layer concern with no existing thematic home in the durable test suite. |
| **Periodic recovery scan** (FR-002 layer (b), FR-009, SC-004) | **Extend `test_lifecycle.py`** OR a new module if the periodic-scan fixture grows beyond ~50 lines. The gap-list picks the home. The test uses the FR-009 test-only hook (interval override or trigger function) and asserts on the public-surface outcome (handler re-entered with `entry_mode == "recovered"`), NOT on internal scan-task state. | If a new file: `test_periodic_recovery.py`, justified only if the fixture is unwieldy in `test_lifecycle.py`. |

**Hardening pre-existing tests that this spec's changes break:**

- Tests asserting `TaskResult.is_superseded` or `TaskResult.status == "superseded"` MUST be rewritten to assert the natural multi-turn outcome (suspended / completed / terminal).
- Tests asserting `ctx.pending_inputs` data access MUST be rewritten to `ctx.pending_input_count` integer assertions.
- Tests asserting `ctx.was_steered` MUST be renamed to `ctx.is_steered_turn` AND have their assertions reviewed for the sticky-True bug (a test that asserted `True` on a post-drain resume was relying on the buggy behavior and MUST flip to `False`).
- Tests asserting `ctx.steering_generation` MUST be removed or rewritten to assert observable outcomes (e.g., `ctx.is_steered_turn`).
- Tests calling `run.terminate(...)` and asserting `TaskTerminated` MUST be ported to `.cancel()` per the table above.
- Tests that used `@task(stale_timeout=...)` MUST be rewritten to use the FR-009 test-only hook.

### Exit checklist

- [ ] **Conformance gap-list document produced.** Required deliverable in this spec's directory: `conformance-gap-list.md`. Cites the Conformance Test Map above and, for each affected symbol/behavior, records: (a) the existing test file that owns the surface area (per the table); (b) the specific test names being added or rewritten; (c) the pre-existing tests being ported/migrated; (d) the task ID in `tasks.md` that lands each test RED. Any deviation from the table (e.g., a new file added where the table prescribed an extension) MUST be justified in the gap list with reviewer sign-off.
- [ ] **Non-duplication rule satisfied.** No parallel test suite is created for surface areas the table maps to existing files. The only new modules are the two flagged in the table (`test_split_brain_eviction.py`, `test_hosted_provider_transport.py`) — both justified by genuinely new fixture concerns.
- [ ] **Every removed / changed / added symbol has a paired conformance test** per the Conformance Test Map. The test lands RED in a commit that precedes the implementation commit (verifiable from git history).
- [ ] **Pre-existing tests broken by surface changes are ported, not deleted.** Tests that exercised removed surface (`is_superseded`, `pending_inputs`, `was_steered`, `steering_generation`, `terminate()`, `stale_timeout`) MUST be rewritten per the "Hardening pre-existing tests" subsection above. Deletion is allowed only when the underlying behavior the test was asserting is no longer a contract (record the deletion + justification in the gap list).
...
- [ ] **Completeness meta-test** (`test_contract_completeness.py`) stays green — picks up new public symbols automatically; gap-list verifies no symbol slips through unnoticed.
- [ ] **Dev guide updated** per Docs ↔ Samples Loop above; doc-review meta-test (`test_dev_guide_review.py`) passes with the new presence/absence invariants from this spec.
