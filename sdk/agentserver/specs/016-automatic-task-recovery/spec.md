...
This spec therefore also: removes `"superseded"` from `TaskResult.status`; removes `_pending_steering_futures`; restructures `_try_drain_steering` to run strictly *after* the suspend resolution; and codifies what happens to a queued steering input when the handler chose to terminate (return or raise) rather than suspend. Detailed requirements in FR-020 through FR-024.
...
- **FR-021 (suspend path delivers handler output)**: When a handler returns `Suspended(output=X, reason=R)`, the current turn's caller's `TaskResult` MUST be exactly `TaskResult(task_id=..., status="suspended", output=X, suspension_reason=R)`. This MUST hold whether or not a steering input is queued at the moment the suspend resolves. The framework MUST NOT race a drain ahead of the suspend resolution; the suspend MUST be persisted and the current turn's future MUST be set FIRST, then the drain may re-enter the handler for the queued input.
- **FR-022 (return / raise paths are terminal, even with queued inputs)**: When a handler `return`s a value `V`, the current turn's caller's `TaskResult` MUST be `TaskResult(status="completed", output=V)` and the task MUST transition to terminal state `completed`. When a handler raises, the current turn's caller's `TaskResult` future MUST raise that exception and the task MUST transition to terminal state `failed`. In either case, if `_steering.pending_inputs` is non-empty, the SAME store write that persists the terminal transition MUST also clear `pending_inputs` (single write, single consistent state). Every queued-input's `TaskRun.result()` future MUST then be resolved with `TaskConflictError(current_status="completed" | "failed")` — the exact same exception type and shape a fresh `.start()` against an already-terminal task would raise. No new error type for "queue orphaned by terminal".
...
| Steering = plain multi-turn + a queue (no `superseded` status, no parallel future array). Handler decides what to do with `ctx.cancel`; framework never claims to know if the handler honored it. | `durable-task-guide.md` (Steering section, rewritten per FR-020..FR-024) |
...
- [ ] **Public-surface change?** YES. This spec removes `stale_timeout` from `@task`, `Task.options()`, `TaskOptions.__slots__`, the `@task` overload signatures, and any docstring references. Also changes the behavior of `TaskManager.get_active_run` from "in-memory lookup only" to "store query + inline reclaim on dead lease". Both are tracked in Principle XII scope. **Additionally (FR-020..FR-024)**: `TaskResult.status` Literal narrows from `"completed" | "suspended" | "superseded"` to `"completed" | "suspended"`, the `TaskResult.is_superseded` property is removed, the `_pending_steering_futures` internal attribute is removed, and `_try_drain_steering` loses its `partial_output` parameter. The observable semantics of `.run()` / `.start()` on a steerable task with a queued input shift: the original caller now sees the natural suspend / completed / failed outcome (with the handler's emitted output preserved on suspend) rather than a synthetic `superseded` status; queued steerers whose turn never runs (because the handler terminated) now receive `TaskConflictError` instead of `superseded`.
- [ ] **Affected symbols enumerated?**
  ...
  - `__all__` in `durable/__init__.py`: no additions; verify no `stale_timeout` mentions; verify no `superseded` mentions.
