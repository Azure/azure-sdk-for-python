...
# ---------------------------------------------------------------------------
# Helpers — build dicts that the model class constructors accept.
#
# The agentserver-core model classes serialize correctly when constructed
# with a single dict argument containing pre-serialized nested objects
# (i.e. nested dicts, not Python model instances).  This matches the
# pattern used by Jeff Hollan's hosted-agent-cli skills template which
# is the proven reference implementation for streaming SSE on the Foundry
# platform.
# ---------------------------------------------------------------------------

def _make_message_item(item_id: str, text: str, *, status: str = "completed") -> dict:
    """Build a serialized assistant message item dict."""
    return {
        "type": "message",
        "id": item_id,
        "role": "assistant",
...
def _make_response(
    ...
    """Build a serialized response dict."""
    resp: Dict[str, Any] = {
        "object": "response",
        "id": response_id,
        "status": status,
...
class CopilotStreamingResponseConverter:
    ...
    def _resp(self, status: str, output=None, usage=None, error=None) -> dict:
            ...
            case SessionEvent(type=SessionEventType.ASSISTANT_TURN_START):
                ...
                if is_first_turn:
                    yield ResponseCreatedEvent({
                        ...
                        "response": self._resp("in_progress"),
                    })
