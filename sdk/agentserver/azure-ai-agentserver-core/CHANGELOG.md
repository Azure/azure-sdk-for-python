...
- **New consolidated developer guide** — `docs/durable-task-guide.md` is the canonical end-to-end learning arc for the durable-task primitive (8 sections + a Rename Map appendix that documents every retired symbol from spec 015). The previous `docs/durable-task-overview.md` and `docs/durable-task-developer-guide.md` redirect files have been removed; update any in-tree or external links to point at `durable-task-guide.md`. The new guide is the source of truth for handler authors and is enforced by an automated CI doc-review meta-test (`tests/durable/test_dev_guide_review.py`) that checks public-symbol coverage, absence of retired names, presence of the 5 canonical statements (retry_attempt semantics, crash-recovery budget rule, callable namespace facade, reserved `_` prefix, explicit `flush()`), required-section presence, cross-guide consistency, and ambiguity heuristics.
...
- **`Task.run()` and `Task.start()` no longer accept per-call `session_id`, `retry`, or `stream_handler` kwargs.** All three are configured *outside* the call site:
  - `session_id` is platform-derived from the `FOUNDRY_AGENT_SESSION_ID` environment variable, not chosen by handler code.
  - `retry` (a `RetryPolicy`) is supplied to `@task(retry=...)` or `Task.options(retry=...)`. Per-call overrides would silently disappear when crash recovery re-invokes the task on a fresh process, so they were never safe to expose.
  - `stream_handler` (the per-invocation handler instance) is replaced by `@task(stream_handler_factory=lambda task_id: ...)` — recovery needs a way to reconstruct the handler from the persisted task identity, so a factory on the decorator is the only durable shape.
- **`Task.run()` and `Task.start()` no longer accept per-call `title`, `tags`, or `stale_timeout` kwargs.** `title` and `tags` are characteristics of the task identity and are configured on `@task(title=..., tags=...)` (both accept static values or a `(input, task_id) -> ...` callable factory). `stale_timeout` is a recovery-safety knob that must survive crash/recovery, so it now lives on `@task(stale_timeout=...)` and `Task.options(stale_timeout=...)`; default unchanged at 300 seconds. See `docs/durable-task-guide.md` §7 "Stale-task recovery and `stale_timeout`" for the precise semantics.
- **`Task.run()` now accepts `input_id` and `if_last_input_id`** (parity with `Task.start()`). Allows fire-and-await callers to apply the same optimistic-concurrency precondition as fire-and-handle callers without a separate `.start()` + `.result()` round-trip.
- **`TaskContext.session_id` is now internal (`_session_id`).** It was framework-internal plumbing for the manager and not intended for handler consumption; no external consumers existed in the tree.
