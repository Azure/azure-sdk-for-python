...
Request and response bodies are typed models generated from
``type_spec/main.tsp`` (see ``azure.ai.agentserver.core.storage._generated``)
...
hand-written convenience types (``JSONObject``, ``Order``, ``KeyPage``) that
...
class KeyPage:
    ...
    over the generated ``ListResponseStateStoreKey`` envelope, exposing its
    ``data`` array as ``keys`` to match this client's naming.
    """

    keys: list[StateStoreKey]
...
def deserialize_state_item_metadata(body: str) -> StateStoreItemMetadata:
    return StateStoreItemMetadata(load_json(body))
...
def deserialize_list_keys_response(body: str) -> KeyPage:
    page = ListResponseStateStoreKey(load_json(body))
    return KeyPage(
