...
def _durable_module() -> Any:
    return importlib.import_module("azure.ai.agentserver.core.durable")


def _exceptions_module() -> Any:
    return importlib.import_module("azure.ai.agentserver.core.durable._exceptions")


def _public_symbol(name: str) -> Any:
    return getattr(_durable_module(), name)


def _assert_public_import_raises_import_error(name: str) -> None:
    with pytest.raises(ImportError):
        exec(f"from azure.ai.agentserver.core.durable import {name}", {})
...
class TestPublicExceptionExports:
    ...
    def test_7_public_exceptions_exported(self):
        ...
        for name in PUBLIC_EXCEPTION_NAMES:
            ...
            assert hasattr(_durable_module(), name)

    def test_TaskNotFound_not_in_public_all(self):
        assert "TaskNotFound" not in getattr(_durable_module(), "__all__", ())
    ...
    def test_TaskPreconditionFailed_not_in_public_all(self):
        assert "TaskPreconditionFailed" not in getattr(_durable_module(), "__all__", ())
    ...
    def test_OutputTooLarge_not_in_public_all(self):
        assert "OutputTooLarge" not in getattr(_durable_module(), "__all__", ())
...
class TestTaskDeferredSemantics:
    ...
    def test_TaskDeferred_in_public_surface(self):
        from azure.ai.agentserver.core.durable import TaskDeferred
