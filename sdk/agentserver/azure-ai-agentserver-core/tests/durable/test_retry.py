...
from azure.ai.agentserver.core.durable import RetryPolicy, TaskContext, TaskFailed, task, multi_turn_task
...
class TestRetryAttemptDurability:
    ...
    async def _seed_stale_task(
        ...
        ``payload["_retry_attempt"]`` is the durable counter that
    ...
    async def test_retry_attempt_cross_lifetime_durability(self, tmp_path) -> None:
        ...
        manager, mgr_mod = await self._setup_manager(tmp_path)
        try:
            await self._seed_stale_task(manager, tmp_path, task_id="durable-1", retry_attempt=2)
            result = await handler.run(task_id="durable-1", input="ignored-by-recovery")
            assert result == "ok"
            assert observed == [2], (
