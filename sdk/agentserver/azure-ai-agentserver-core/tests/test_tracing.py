...
class TestFlushSpansAsync:
    ...
    async def test_coalesces_concurrent_awaited_flushes(self) -> None:
        ...
        with mock.patch.object(_tracing.trace, "get_tracer_provider", return_value=provider):
            ...
            await _wait_for(provider.started)
            followers = [asyncio.create_task(_tracing.flush_spans_async(5000)) for _ in range(50)]
            await asyncio.sleep(0)
...
@pytest.mark.asyncio
async def test_trace_stream_flushes_after_final_chunk_without_blocking_event_loop() -> None:
    provider = _BlockingFlushProvider(block_first=True)
    span = mock.Mock()
