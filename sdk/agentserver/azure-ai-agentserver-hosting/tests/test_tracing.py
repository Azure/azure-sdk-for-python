...
from azure.ai.agentserver.hosting._config import (
    resolve_agent_name,
    resolve_agent_version,
    resolve_appinsights_connection_string,
)
from azure.ai.agentserver.hosting._constants import Constants
from azure.ai.agentserver.hosting._tracing import _parse_baggage_key
...
class TestTracingToggle:
    ...
    def test_tracing_enabled_via_appinsights_env_var(self) -> None:
        with mock.patch.dict(os.environ, {Constants.APPLICATIONINSIGHTS_CONNECTION_STRING: "InstrumentationKey=test"}):
            with mock.patch(
                "azure.ai.agentserver.hosting._tracing.TracingHelper.__init__",
                return_value=None,
            ):
                agent = AgentServer()
...
class TestSetupAzureMonitor:
    ...
    def _tracing_mocks() -> contextlib.ExitStack:
        ...
        stack.enter_context(mock.patch("azure.ai.agentserver.hosting._tracing._HAS_OTEL", True))
        stack.enter_context(mock.patch("azure.ai.agentserver.hosting._tracing.trace", create=True))
        stack.enter_context(
            mock.patch("azure.ai.agentserver.hosting._tracing.TraceContextTextMapPropagator", create=True)
        )
        stack.enter_context(
            mock.patch("azure.ai.agentserver.hosting._tracing._ensure_trace_provider", return_value=mock.MagicMock())
        )
        return stack
    ...
    def test_setup_azure_monitor_called_when_conn_str_provided(self) -> None:
        with self._tracing_mocks():
            with mock.patch(
                "azure.ai.agentserver.hosting._tracing.TracingHelper._setup_azure_monitor"
            ) as mock_setup:
                with mock.patch("azure.ai.agentserver.hosting._tracing.TracingHelper._setup_otlp_export"):
                    from azure.ai.agentserver.hosting._tracing import TracingHelper
