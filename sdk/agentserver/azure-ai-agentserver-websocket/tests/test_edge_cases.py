...
# ---------------------------------------------------------
"""Edge-case tests for InvocationAgentServerHost over WebSocket."""
import uuid
...
def test_invocation_id_auto_generated(echo_client):
...
def test_invocation_id_accepted_from_message(echo_client):
...
def test_invocation_id_generated_when_empty(echo_client):
...
def test_streaming_has_invocation_id():
...
# ---------------------------------------------------------------------------
# Multiple sequential invocations on same connection
# ---------------------------------------------------------------------------

def test_multiple_sequential_invocations(echo_client):
    """Multiple sequential invocations on the same WebSocket connection."""
    with echo_client.websocket_connect("/invocations/ws") as ws:
        ids = set()
        for i in range(10):
...
def test_get_without_invocation_id(echo_client):
...
def test_cancel_without_invocation_id(echo_client):
