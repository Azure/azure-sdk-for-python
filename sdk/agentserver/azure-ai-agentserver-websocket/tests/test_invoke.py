...
from azure.ai.agentserver.invocations import InvocationContext
...
def test_invoke_returns_invocation_id(echo_client):
...
def test_invoke_unique_invocation_ids(echo_client):
...
def test_invoke_accepts_custom_invocation_id(echo_client):
...
def test_streaming_returns_chunks(streaming_client):
    ...
    with streaming_client.websocket_connect("/invocations/ws") as ws:
        ws.send_json({"action": "invoke", "payload": {}})
        chunks = []
        while True:
            resp = ws.receive_json()
...
def test_streaming_has_invocation_id(streaming_client):
...
def test_invoke_error_returns_error(failing_client):
    ...
    with failing_client.websocket_connect("/invocations/ws") as ws:
        ws.send_json({"action": "invoke", "payload": {}})
        resp = ws.receive_json()
...
def test_invoke_error_has_invocation_id(failing_client):
