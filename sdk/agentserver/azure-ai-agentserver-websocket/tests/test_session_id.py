...
# ---------------------------------------------------------------------------
# get_conversation does NOT include session_id (not part of get protocol)
# ---------------------------------------------------------------------------

def test_get_conversation_no_session_id(async_storage_client):
...
def test_cancel_conversation_no_session_id(async_storage_client):
    """cancel_conversation response does not include session_id."""
    with async_storage_client.websocket_connect("/conversations/ws") as ws:
        ws.send_json({"action": "invoke", "payload": {"key": "data"}})
        invoke_resp = ws.receive_json()
        inv_id = invoke_resp["conversation_id"]
