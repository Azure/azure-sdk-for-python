...
# ---------------------------------------------------------------------------
# get_websocket does NOT include session_id (not part of get protocol)
# ---------------------------------------------------------------------------

def test_get_websocket_no_session_id(async_storage_client):
    """get_websocket response does not include session_id."""
...
def test_cancel_websocket_no_session_id(async_storage_client):
    """cancel_websocket response does not include session_id."""
    with async_storage_client.websocket_connect("/websocket/ws") as ws:
        ws.send_json({"action": "invoke", "payload": {"key": "data"}})
        invoke_resp = ws.receive_json()
        inv_id = invoke_resp["websocket_id"]
