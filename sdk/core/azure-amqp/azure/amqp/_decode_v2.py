...
def _decode_null(buffer: Buffer) -> Tuple[Buffer, None]:
    return buffer, None


def _decode_true(buffer: Buffer) -> Tuple[Buffer, Literal[True]]:
    return buffer, True


def _decode_false(buffer: Buffer) -> Tuple[Buffer, Literal[False]]:
    return buffer, False


def _decode_zero(buffer: Buffer) -> Tuple[Buffer, Literal[0]]:
    return buffer, 0


def _decode_empty(buffer: Buffer) -> Tuple[Buffer, Literal['[]']]:
    return buffer, []


def _decode_boolean(buffer: Buffer) -> Tuple[Buffer, bool]:
    return buffer[1:], buffer[:1] == b'\x01'


def _decode_ubyte(buffer: Buffer) -> Tuple[Buffer, int]:
    return buffer[1:], buffer[0]


def _decode_ushort(buffer: Buffer) -> Tuple[Buffer, int]:
    return buffer[2:], c_unsigned_short.unpack(buffer[:2])[0]


def _decode_uint_small(buffer: Buffer) -> Tuple[Buffer, int]:
    return buffer[1:], buffer[0]


def _decode_uint_large(buffer: Buffer) -> Tuple[Buffer, int]:
    return buffer[4:], c_unsigned_int.unpack(buffer[:4])[0]


def _decode_ulong_small(buffer: Buffer) -> Tuple[Buffer, int]:
    return buffer[1:], buffer[0]


def _decode_ulong_large(buffer: Buffer) -> Tuple[Buffer, int]:
    return buffer[8:], c_unsigned_long_long.unpack(buffer[:8])[0]


def _decode_byte(buffer: Buffer) -> Tuple[Buffer, int]:
    return buffer[1:], c_signed_char.unpack(buffer[:1])[0]


def _decode_short(buffer: Buffer) -> Tuple[Buffer, int]:
    return buffer[2:], c_signed_short.unpack(buffer[:2])[0]


def _decode_int_small(buffer: Buffer) -> Tuple[Buffer, int]:
    return buffer[1:], c_signed_char.unpack(buffer[:1])[0]


def _decode_int_large(buffer: Buffer) -> Tuple[Buffer, int]:
    return buffer[4:], c_signed_int.unpack(buffer[:4])[0]


def _decode_long_small(buffer: Buffer) -> Tuple[Buffer, int]:
    return buffer[1:], c_signed_char.unpack(buffer[:1])[0]


def _decode_long_large(buffer: Buffer) -> Tuple[Buffer, int]:
    return buffer[8:], c_signed_long_long.unpack(buffer[:8])[0]


def _decode_float(buffer: Buffer) -> Tuple[Buffer, float]:
    return buffer[4:], c_float.unpack(buffer[:4])[0]


def _decode_double(buffer: Buffer) -> Tuple[Buffer, float]:
    return buffer[8:], c_double.unpack(buffer[:8])[0]


def _decode_timestamp(buffer: Buffer) -> Tuple[Buffer, int]:
    return buffer[8:], c_signed_long_long.unpack(buffer[:8])[0]


def _decode_uuid(buffer: Buffer) -> Tuple[Buffer, uuid.UUID]:
    return buffer[16:], uuid.UUID(bytes=buffer[:16].tobytes())


def _decode_binary_small(buffer: Buffer) -> Tuple[Buffer, bytes]:
    length_index = buffer[0] + 1
    return buffer[length_index:], buffer[1:length_index].tobytes()


def _decode_binary_large(buffer: Buffer) -> Tuple[Buffer, bytes]:
...
def decode_payload(buffer: Buffer) -> Message:
    message = Message()
    while buffer:
...
def decode_empty_frame(header: Buffer) -> Tuple[int, bytes]:
