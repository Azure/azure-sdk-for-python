...
class AMQPError:
    ...
    info: Optional[Mapping[AnyStr, AMQP_DEFINED_TYPES]] = None
    """Details related to the error."""
    _code: Literal[0x0000001d] = field(default=0x0000001d, init=False, repr=False, hash=False)
    ...
    def _describe(self) -> AMQPDefinition[AMQPTypes.described, Tuple[AMQPDefinition[AMQPTypes.ulong, int], AMQPDefinition[AMQPTypes.list, List[AMQPDefinition]]]]:
        body: List[AMQPDefinition] = [
            {TYPE_KEY: AMQPTypes.symbol, VALUE_KEY: self.condition},
            AMQP_NONE if self.description is None else {TYPE_KEY: AMQPTypes.string, VALUE_KEY: self.description},
            AMQP_NONE if self.info is None else {TYPE_KEY: AMQPTypes.fields, VALUE_KEY: self.info},
        ]
        return {
...
class Received:
    ...
    _code: Literal[0x00000023] = field(default=0x00000023, init=False, repr=False, hash=False)

    def _describe(self) -> AMQPDefinition[AMQPTypes.described, Tuple[AMQPDefinition[AMQPTypes.ulong, int], AMQPDefinition[AMQPTypes.list, List[AMQPDefinition]]]]:
        body: List[AMQPDefinition] = [
            {TYPE_KEY: AMQPTypes.uint, VALUE_KEY: self.section_number},
            {TYPE_KEY: AMQPTypes.ulong, VALUE_KEY: self.section_offset},
        ]
        return {
            TYPE_KEY: AMQPTypes.described,
            VALUE_KEY: (
...
class Accepted:
    ...
    _code: Literal[0x00000024] = field(default=0x00000024, init=False, repr=False, hash=False)
    ...
    def _describe(self) -> AMQPDefinition[AMQPTypes.described, Tuple[AMQPDefinition[AMQPTypes.ulong, int], AMQPDefinition[AMQPTypes.list, List[AMQPDefinition]]]]:
        return {
            ...
            VALUE_KEY: (
                {TYPE_KEY: AMQPTypes.ulong, VALUE_KEY: self._code},
                {TYPE_KEY: AMQPTypes.list, VALUE_KEY: []},
            ),
        }
...
class Rejected:
    ...
    """The error that caused the message to be rejected.
    ...
    _code: Literal[0x00000025] = field(default=0x00000025, init=False, repr=False, hash=False)
...
class Released:
    ...
    """
    _code: Literal[0x00000026] = field(default=0x00000026, init=False, repr=False, hash=False)
...
class Modified:
    ...
    """Prevent redelivery.
        ...
        Link Endpoint."""
    message_annotations: Optional[Mapping[AnyStr, AMQP_DEFINED_TYPES]] = None
    """Message attributes.
    ...
    _code: Literal[0x00000027] = field(default=0x00000027, init=False, repr=False, hash=False)

    def _describe(self) -> AMQPDefinition[AMQPTypes.described, Tuple[AMQPDefinition[AMQPTypes.ulong, int], AMQPDefinition[AMQPTypes.list, List[AMQPDefinition]]]]:
        body: List[AMQPDefinition] = [
            AMQP_NONE if self.delivery_failed is None else {TYPE_KEY: AMQPTypes.boolean, VALUE_KEY: self.delivery_failed},
            AMQP_NONE if self.undeliverable_here is None else {TYPE_KEY: AMQPTypes.boolean, VALUE_KEY: self.undeliverable_here},
            AMQP_NONE if self.message_annotations is None else {TYPE_KEY: AMQPTypes.fields, VALUE_KEY: self.message_annotations},
        ]
        return {
            TYPE_KEY: AMQPTypes.described,
            VALUE_KEY: (
