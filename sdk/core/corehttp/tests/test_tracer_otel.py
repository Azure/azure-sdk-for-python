...
from corehttp.instrumentation.tracing._tracer import TracerManager, default_tracer_manager
...
from corehttp.instrumentation.tracing.opentelemetry_tracer import OpenTelemetryTracer
from corehttp.exceptions import ClientAuthenticationError
...
def test_tracer_manager(tracing_helper):
    """Test basic usage of a TracerManager."""
    ...
    assert isinstance(tracer_manager.tracer, OpenTelemetryTracer)
    assert isinstance(tracer_manager.tracer._tracer, OtelTracer)

    with tracer_manager.tracer.start_span("test_span") as span:
...
def test_default_tracer_manager():
...
def test_error_type_attribute_corehttp_error():
    tracer = default_tracer_manager.tracer
    assert tracer
    ...
    assert span.span_instance.attributes.get("error.type") == "corehttp.exceptions.ClientAuthenticationError"


def test_tracer_get_current_span():
    """Test that the tracer can get the current OpenTelemetry span instance."""
    assert default_tracer_manager.tracer

    with default_tracer_manager.tracer.start_span("test_span") as span:
        with default_tracer_manager.tracer.start_span("test_span2", kind=SpanKind.CLIENT) as span2:
            current_span = default_tracer_manager.tracer.get_current_span()
...
def test_nested_span_suppression(tracing_helper):
    ...
    with default_tracer_manager.tracer.start_span(name="outer-span", kind=SpanKind.INTERNAL) as outer_span:
        assert isinstance(outer_span.span_instance, OtelSpan)
        assert outer_span.span_instance.kind == OtelSpanKind.INTERNAL

        with default_tracer_manager.tracer.start_span(name="inner-span", kind=SpanKind.INTERNAL) as inner_span:
            assert isinstance(inner_span.span_instance, NonRecordingSpan)
...
def test_nested_span_suppression_with_multiple_outer_spans(tracing_helper):
    ...
    with default_tracer_manager.tracer.start_span(name="outer-span-1", kind=SpanKind.INTERNAL) as outer_span_1:
        assert isinstance(outer_span_1.span_instance, OtelSpan)
        assert outer_span_1.span_instance.kind == OtelSpanKind.INTERNAL

        with default_tracer_manager.tracer.start_span(name="inner-span-1", kind=SpanKind.INTERNAL) as inner_span_1:
            assert isinstance(inner_span_1.span_instance, NonRecordingSpan)

    assert len(tracing_helper.exporter.get_finished_spans()) == 1

    with default_tracer_manager.tracer.start_span(name="outer-span-2", kind=SpanKind.INTERNAL) as outer_span_2:
        assert isinstance(outer_span_2.span_instance, OtelSpan)
        assert outer_span_2.span_instance.kind == OtelSpanKind.INTERNAL

        with default_tracer_manager.tracer.start_span(name="inner-span-2", kind=SpanKind.INTERNAL) as inner_span_2:
            assert isinstance(inner_span_2.span_instance, NonRecordingSpan)
...
def test_nested_span_suppression_with_nested_client(tracing_helper):
    ...
    with default_tracer_manager.tracer.start_span(name="outer-span", kind=SpanKind.INTERNAL) as outer_span:
        ...
        with default_tracer_manager.tracer.start_span(name="inner-span", kind=SpanKind.INTERNAL) as inner_span:
            ...
            with default_tracer_manager.tracer.start_span(name="client-span", kind=SpanKind.CLIENT) as client_span:
...
def test_get_trace_context(tracing_helper):
