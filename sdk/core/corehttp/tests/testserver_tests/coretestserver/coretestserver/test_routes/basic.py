...
def bytes():
    return Response("Hello, world!".encode(), status=200, mimetype="text/plain")


@basic_api.route("/html", methods=["GET"])
def html():
    return Response("<html><body>Hello, world!</html></body>", status=200, mimetype="text/html")


@basic_api.route("/json", methods=["GET"])
def json():
    return Response('{"greeting": "hello", "recipient": "world"}', status=200, mimetype="application/json")
