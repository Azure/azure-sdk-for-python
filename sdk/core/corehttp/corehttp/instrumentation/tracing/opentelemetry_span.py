...
class OpenTelemetrySpan:
    ...
    def kind(self) -> Optional[SpanKind]:
        ...
        :rtype: ~corehttp.instrumentation.tracing.SpanKind
        """
        try:
    ...
    def __enter__(self) -> Self:
        ...
        if not isinstance(self.span_instance, NonRecordingSpan):
            if self.kind == SpanKind.CLIENT:
                # Since corehttp already instruments HTTP calls, we need to suppress any automatic HTTP instrumentation
    ...
    def set_status(self, status_code: StatusCode, description: Optional[str] = None) -> None:
        ...
        :type status_code: ~corehttp.instrumentation.tracing.StatusCode
