...
def send_request(httpx_client):
    def _send_request(request):
        response = httpx_client.send_request(request, stream=False)
...
def test_raise_for_status(httpx_client):
    response = httpx_client.send_request(
...
def test_send_request_return_pipeline_response(httpx_client):
    # we use return_pipeline_response for some cases in autorest
    request = HttpRequest("GET", "/basic/string")
    response = httpx_client.send_request(request, _return_pipeline_response=True)
