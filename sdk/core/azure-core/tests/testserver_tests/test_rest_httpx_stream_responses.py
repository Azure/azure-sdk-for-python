...
def test_iter_raw(httpx_client):
    request = HttpRequest("GET", "/streams/basic")
    with httpx_client.send_request(request, stream=True) as response:
        raw = b""
...
def test_iter_raw_on_iterable(httpx_client):
...
def test_iter_with_error(httpx_client):
...
def test_iter_bytes(httpx_client):
    request = HttpRequest("GET", "/streams/basic")
...
def test_iter_text(httpx_client):
...
def test_iter_lines(httpx_client):
...
def test_sync_streaming_response(httpx_client):
    request = HttpRequest("GET", "/streams/basic")
...
def test_cannot_read_after_stream_consumed(httpx_client, port):
    request = HttpRequest("GET", "/streams/basic")
...
def test_cannot_read_after_response_closed(port, httpx_client):
    request = HttpRequest("GET", "/streams/basic")
...
def test_decompress_plain_no_header(httpx_client):
    # thanks to Xiang Yan for this test!
    account_name = "coretests"
    url = "https://{}.blob.core.windows.net/tests/test.txt".format(account_name)
    request = HttpRequest("GET", url)
    response = httpx_client.send_request(request, stream=True)
    with pytest.raises(ResponseNotReadError):
        response.content
    response.read()
    assert response.content == b"test"

def test_compress_plain_no_header(httpx_client):
    # thanks to Xiang Yan for this test!
    account_name = "coretests"
    url = "https://{}.blob.core.windows.net/tests/test.txt".format(account_name)
    request = HttpRequest("GET", url)
...
def test_decompress_compressed_no_header(httpx_client):
    # thanks to Xiang Yan for this test!
    account_name = "coretests"
...
def test_decompress_compressed_header(httpx_client):
    # thanks to Xiang Yan for this test!
    account_name = "coretests"
...
def test_iter_read(httpx_client):
...
def test_iter_read_back_and_forth(httpx_client):
    # thanks to McCoy Patiño for this test!
...
def test_stream_with_return_pipeline_response(httpx_client):
    request = HttpRequest("GET", "/basic/lines")
    pipeline_response = httpx_client.send_request(request, stream=True, _return_pipeline_response=True)
...
def test_error_reading(httpx_client):
    request = HttpRequest("GET", "/errors/403")
    with httpx_client.send_request(request, stream=True) as response:
        response.read()
