...
def get_non_odata_v4_response_body():
    return Response('{"code": 400, "error": {"global": ["MY-ERROR-MESSAGE-THAT-IS-COMING-FROM-THE-API"]}}', status=400)
...
def get_text_body():
    return Response("I am throwing an error", status=400)


@errors_api.route("/odatav4", methods=["GET"])
def get_odatav4():
    return Response(
        '{"error": {"code": "501", "message": "Unsupported functionality", '
        '"target": "query", "details": [{"code": "301", "target": "$search", '
