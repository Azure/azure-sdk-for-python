...
full_combination = list(product(sas_url_expected_url, http_request))
@pytest.mark.parametrize("sas_url_expected_url,http_request", full_combination)
def test_azure_sas_credential_policy(sas_url_expected_url, http_request):
    """Tests to see if we can create an AzureSasCredentialPolicy"""
    sas, url, expected_url = sas_url_expected_url
