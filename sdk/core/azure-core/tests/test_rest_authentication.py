...
def test_bearer_policy_send():
    ...
    def verify_request(pipeline_request):
...
def test_bearer_policy_cannot_complete_challenge():
    ...
    # assert response.http_response is expected_response # can't assert this, bc we change pipeline transport to rest. will assert headers instead
    # assert response.http_response.headers == {"WWW-Authenticate": 'Basic realm="localhost"'}
...
def test_azure_sas_credential_policy(sas, url, expected_url):
    ...
    def verify_authorization(request):
        ...
        # kind of hacky, but we expect internal_response in pipeline.run (which is fair), so going to hack something here
        return FakeResponse()
