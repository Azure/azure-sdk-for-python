...
def send_request(client):
    async def _send_request(request):
        return await client.send_request(request, stream_response=True)
...
@pytest.mark.asyncio
async def test_iter_raw(send_request):
...
async def _iter_raw_with_chunk_size_helper(chunk_size, send_request):
    response = await send_request(HttpRequest("GET", "http://localhost:3000/streams/jpeg"))
    raw = b""
