...
def test_iter_raw(port, transport, requesttype):
...
@pytest.mark.parametrize("transport,requesttype", product(SYNC_TRANSPORTS, HTTP_REQUESTS))
def test_iter_raw_on_iterable(port, transport, requesttype):
    request = create_http_request(requesttype, "GET", "/streams/iterable")
    client = MockRestClient(port, transport=transport())
    with client.send_request(request, stream=True) as response:
        raw = b""
        for part in response.iter_raw():
...
@pytest.mark.parametrize("transport,requesttype", product(SYNC_TRANSPORTS, HTTP_REQUESTS))
def test_iter_with_error(port, transport, requesttype):
    ...
    request = create_http_request(requesttype, "GET", "http://doesNotExist")
...
@pytest.mark.parametrize("transport,requesttype", product(SYNC_TRANSPORTS, HTTP_REQUESTS))
def test_iter_bytes(port, transport, requesttype):
    request = create_http_request(requesttype, "GET", "/streams/basic")
    client = MockRestClient(port, transport=transport())
    with client.send_request(request, stream=True) as response:
        raw = b""
...
@pytest.mark.parametrize("transport,requesttype", product(SYNC_TRANSPORTS, HTTP_REQUESTS))
def test_sync_streaming_response(port, transport, requesttype):
    request = create_http_request(requesttype, "GET", "/streams/basic")
    client = MockRestClient(port, transport=transport())
    with client.send_request(request, stream=True) as response:
...
@pytest.mark.parametrize("transport,requesttype", product(SYNC_TRANSPORTS, HTTP_REQUESTS))
def test_cannot_read_after_stream_consumed(port, transport, requesttype):
    request = create_http_request(requesttype, "GET", "/streams/basic")
    client = MockRestClient(port, transport=transport())
    with client.send_request(request, stream=True) as response:
        content = b""
...
@pytest.mark.parametrize("transport,requesttype", product(SYNC_TRANSPORTS, HTTP_REQUESTS))
def test_cannot_read_after_response_closed(port, transport, requesttype):
    request = create_http_request(requesttype, "GET", "/streams/basic")
    client = MockRestClient(port, transport=transport())
    with client.send_request(request, stream=True) as response:
        response.close()
...
@pytest.mark.parametrize("transport,requesttype", product(SYNC_TRANSPORTS, HTTP_REQUESTS))
def test_error_reading(port, transport, requesttype):
    request = create_http_request(requesttype, "GET", "/errors/403")
    client = MockRestClient(port, transport=transport())
    with client.send_request(request, stream=True) as response:
        response.read()
...
@pytest.mark.parametrize("transport,requesttype", product(SYNC_TRANSPORTS, HTTP_REQUESTS))
def test_pass_kwarg_to_iter_bytes(port, transport, requesttype):
    request = create_http_request(requesttype, "GET", "/basic/string")
    client = MockRestClient(port, transport=transport())
    response = client.send_request(request, stream=True)
...
@pytest.mark.parametrize("transport,requesttype", product(SYNC_TRANSPORTS, HTTP_REQUESTS))
def test_pass_kwarg_to_iter_raw(port, transport, requesttype):
    request = create_http_request(requesttype, "GET", "/basic/string")
    client = MockRestClient(port, transport=transport())
    response = client.send_request(request, stream=True)
...
@pytest.mark.parametrize("transport,requesttype", product(SYNC_TRANSPORTS, HTTP_REQUESTS))
def test_decompress_compressed_header(port, transport, requesttype):
    # expect plain text
    request = create_http_request(requesttype, "GET", "/encoding/gzip")
    client = MockRestClient(port, transport=transport())
...
@pytest.mark.parametrize("transport,requesttype", product(SYNC_TRANSPORTS, HTTP_REQUESTS))
def test_deflate_decompress_compressed_header(port, transport, requesttype):
    # expect plain text
    request = create_http_request(requesttype, "GET", "/encoding/deflate")
    client = MockRestClient(port, transport=transport())
    response = client.send_request(request)
    content = response.read()
...
@pytest.mark.parametrize("transport,requesttype", product(SYNC_TRANSPORTS, HTTP_REQUESTS))
def test_decompress_compressed_header_stream(port, transport, requesttype):
    # expect plain text
    request = create_http_request(requesttype, "GET", "/encoding/gzip")
    client = MockRestClient(port, transport=transport())
    response = client.send_request(request, stream=True)
