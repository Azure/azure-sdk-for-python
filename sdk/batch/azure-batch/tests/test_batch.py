...
class BatchTest(AzureMgmtTestCase):
    ...
    def test_batch_tasks(self, batch_job, **kwargs):
        ...
        self.assertEqual(task.container_settings.registry.username, 'username')
        ...
        task = client.task.get(batch_job.id, task_param.id)
        self.assertIsInstance(task, models.BatchTask)
        self.assertEqual(task.user_identity.username, 'task-user')
    ...
    def test_batch_jobs(self, **kwargs):
        ...
        self.assertEqual(stats.num_succeeded_tasks, '0') # thos doesn't seem right
