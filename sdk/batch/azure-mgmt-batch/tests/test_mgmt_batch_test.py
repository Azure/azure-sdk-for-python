...
class TestMgmtBatch(AzureMgmtRecordedTestCase):
    ...
    def setup_method(self, method):
        self.mgmt_batch_client = self.create_mgmt_client(
            azure.mgmt.batch.BatchManagementClient, base_url=AZURE_ARM_ENDPOINT
        )
        if self.is_live:
            self.mgmt_network = self.create_mgmt_client(
                azure.mgmt.network.NetworkManagementClient, base_url=AZURE_ARM_ENDPOINT
            )
    ...
    def test_mgmt_batch_account_name(self):
        ...
        availability = self.mgmt_batch_client.location.check_name_availability(
            AZURE_LOCATION, {"name": "randombatchaccount@5^$g9873495873"}
        )
        ...
        availability = self.mgmt_batch_client.location.check_name_availability(
            AZURE_LOCATION, {"name": self._get_account_name()}
        )
        assert isinstance(availability, models.CheckNameAvailabilityResult)
    ...
    def test_mgmt_batch_byos_account(self, resource_group, location):
        ...
        batch_account = models.BatchAccountCreateParameters(
            location=location, pool_allocation_mode=models.PoolAllocationMode.user_subscription
        )
    ...
    def test_mgmt_batch_account(self, resource_group, location):
        ...
        updated = self.mgmt_batch_client.batch_account.update(
            resource_group.name, account_name, models.BatchAccountUpdateParameters(tags=update_tags)
        )
    ...
    def test_mgmt_batch_applications(self, resource_group, location, storage_account, storage_account_key):
        ...
        storage_resource = "/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Storage/storageAccounts/{}".format(
            self.get_settings_value("SUBSCRIPTION_ID"), resource_group.name, storage_account.name
        )
        batch_account = models.BatchAccountCreateParameters(
            location=location, auto_storage=models.AutoStorageBaseProperties(storage_account_id=storage_resource)
        )
        # account_name = self._get_account_name()
        ...
        response = self.mgmt_batch_client.application_package.activate(
            resource_group.name, account_name, application_id, application_ver, {"format": "zip"}
        )
        assert response.state == models.PackageState.active

        # Test Update Application
        params = models.Application(
            allow_updates=False, display_name="my_updated_name", default_version=application_ver
        )
    ...
    def test_mgmt_batch_certificates(self, resource_group, location, batch_account):
        ...
        response = self.mgmt_batch_client.certificate.create(
            resource_group.name, batch_account.name, certificate, parameters
        )
    ...
    def test_mgmt_batch_pools(self, resource_group, location, batch_account):
        ...
        parameters = models.Pool(
            ...
            scale_settings=models.ScaleSettings(
                fixed_scale=models.FixedScaleSettings(target_dedicated_nodes=0, target_low_priority_nodes=0)
            ),
        )
        ...
        assert (
            pool.deployment_configuration.virtual_machine_configuration.node_agent_sku_id == "batch.node.windows amd64"
        )
    ...
    def test_mgmt_batch_account_advanced(self, resource_group, location):
        ...
        subnet_id = (
            "/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/virtualNetworks/{}/subnets/{}".format(
                self.get_settings_value("SUBSCRIPTION_ID"), resource_group.name, vnet_name, subnet_name
            )
        )
        private_endpoint_name = self.get_resource_name("pe")
        private_connection_name = self.get_resource_name("pec")
        private_link_service_id = (
            "/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Batch/batchAccounts/{}".format(
                self.get_settings_value("SUBSCRIPTION_ID"), resource_group.name, batch_account_name
            )
        )
        batch_account = models.BatchAccountCreateParameters(
            location=location,
        ...
        self.mgmt_batch_client.batch_account.begin_create(
            resource_group_name=resource_group.name, account_name=batch_account_name, parameters=batch_account
        ).result()
        if self.is_live:
            self.mgmt_network.virtual_networks.begin_create_or_update(
                ...
                parameters=self.mgmt_network.models().VirtualNetwork(
                    address_space=self.mgmt_network.models().AddressSpace(address_prefixes=["10.0.0.0/16"]),
                    location=location,
                    subnets=[
                        self.mgmt_network.models().Subnet(
                            address_prefix="10.0.0.0/24", name=subnet_name, private_endpoint_network_policies="Disabled"
                        )
                    ],
                ),
            ).result()
        ...
        private_links = self.mgmt_batch_client.private_link_resource.list_by_batch_account(
            resource_group_name=resource_group.name, account_name=batch_account_name
        )
        ...
        self.mgmt_batch_client.private_endpoint_connection.begin_update(
            ...
            private_endpoint_connection_name=private_endpoint.name,
            parameters={
                "private_link_service_connection_state": {"status": "Approved", "description": "Approved for test"}
            },
        ).result()
