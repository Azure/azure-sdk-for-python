...
class MgmtDataLakeStoreTest(AzureMgmtTestCase):
    ...
    def test_adls_accounts(self, resource_group, location):
        ...
        self.assertEqual(
            azure.mgmt.datalake.store.models.DataLakeStoreAccountStatus.succeeded, adls_account.provisioning_state
        )
        ...
        adls_account = self.adls_account_client.accounts.get(resource_group.name, account_name)
        ...
        adls_account_no_encryption = self.adls_account_client.accounts.get(
            resource_group.name, account_name_no_encryption
        )

        # full validation of the create
        self.assertEqual(adls_account_no_encryption.name, account_name_no_encryption)
        self.assertEqual(
            azure.mgmt.datalake.store.models.DataLakeStoreAccountStatus.succeeded,
        ...
        self.assertEqual(
            azure.mgmt.datalake.store.models.EncryptionState.enabled, adls_account_no_encryption.encryption_state
        )
        ...
        adls_account = self.adls_account_client.accounts.begin_update(
            resource_group.name,
            account_name,
            azure.mgmt.datalake.store.models.UpdateDataLakeStoreAccountParameters(tags={"tag2": "value2"}),
        ).result()
        ...
        self.adls_account_client.accounts.begin_delete(resource_group.name, account_name).wait()
