...
class PipelineOperations:
    ...
    def get_pipelines_by_workspace(self, **kwargs: Any) -> AsyncIterable["_models.PipelineListResponse"]:
        """Lists pipelines.
    ...
    async def _create_or_update_pipeline_initial(
        self, pipeline_name: str, pipeline: "_models.PipelineResource", *, if_match: Optional[str] = None, **kwargs: Any
    ...
    async def begin_create_or_update_pipeline(
        ...
        if cont_token is None:
            raw_result = await self._create_or_update_pipeline_initial(
                pipeline_name=pipeline_name, pipeline=pipeline, if_match=if_match, cls=lambda x, y, z: x, **kwargs
            )
    ...
    async def _delete_pipeline_initial(self, pipeline_name: str, **kwargs: Any) -> None:
    ...
    async def begin_delete_pipeline(self, pipeline_name: str, **kwargs: Any) -> AsyncLROPoller[None]:
        ...
        if cont_token is None:
            raw_result = await self._delete_pipeline_initial(
                pipeline_name=pipeline_name, cls=lambda x, y, z: x, **kwargs
            )
    ...
    async def begin_rename_pipeline(
        ...
        if cont_token is None:
            raw_result = await self._rename_pipeline_initial(
                pipeline_name=pipeline_name, new_name=new_name, cls=lambda x, y, z: x, **kwargs
            )
