...
async def send_event_data_batch(producer, serializer):
    ...
    # Use the serialize method to convert dict object to bytes with the given avro schema and set body of EventData.
    # The serialize method will automatically register the schema into the Schema Registry Service and
    # schema will be cached locally for future usage.
    event_data = await serializer.serialize(data=dict_data, schema=SCHEMA_STRING, message_type=EventData)
    print(f'The bytes of serialized dict data is {next(event_data.body)}.')
