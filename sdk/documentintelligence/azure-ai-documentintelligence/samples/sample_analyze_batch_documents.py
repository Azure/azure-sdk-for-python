...
def analyze_batch_docs():
    ...
    from azure.ai.documentintelligence.models import AnalyzeBatchDocumentsRequest, AzureBlobContentSource, AnalyzeBatchResultOperation, AnalyzeBatchResult
    ...
    if poller.done():
        ...
        result: AnalyzeBatchResultOperation = document_intelligence_client.get_analyze_batch_result(model_id="model_id", operation_id=operation_id)
        if result.status == "succeeded":
            final_result: AnalyzeBatchResult = result.result
            print(f"Succeeded count: {final_result.succeeded_count}")
            print(f"Failed count: {final_result.failed_count}")
            print(f"Skipped count: {final_result.skipped_count}")
        else:
    ...
    if poller.done():
        ...
        poller2 = document_intelligence_client.get_batch_analyze_result(contination_token)
        if poller2.done():
