...
def client_access_url_provider():
    ...
    return service_client.get_client_access_token(roles=["webpubsub.joinLeaveGroup", "webpubsub.sendToGroup"])["url"]
...
async def client_access_url_provider_async():
    ...
    return (
        await service_client_async.get_client_access_token(roles=["webpubsub.joinLeaveGroup", "webpubsub.sendToGroup"])
    )["url"]
...
async def send_async() -> None:
    ...
    client_async = AsyncClient(
        credential=WebPubSubClientCredentialAsync(client_access_url_provider=client_access_url_provider_async),
    )
    async with client_async:
        group_name = "send_compare_between_async_sync"
        await client_async.join_group(group_name)
        start = time.time()
        await asyncio.gather(*[send_item_async(client_async, i) for i in range(MESSAGE_COUNT)])
...
if __name__ == "__main__":
    ...
    print(f"it takes {TIME_COST} seconds to send {MESSAGE_COUNT} messages with Sync API")
    print(f"it takes {TIME_COST_ASYNC} seconds to send {MESSAGE_COUNT} messages with Async API")
