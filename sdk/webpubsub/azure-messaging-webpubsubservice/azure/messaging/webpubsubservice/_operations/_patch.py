...
class WebPubSubServiceClientOperationsMixin(WebPubSubServiceClientOperationsMixinGenerated):
    @distributed_trace
    def get_client_access_token(self, *, webpubsub_client_access: Optional[str] = "Default", **kwargs: Any) -> JSON:
        ...
        :keyword webpubsub_client_access: The type of client. Case-insensitive. If not set, it's "Default". For Web
        ...
        path = "/clients/mqtt/hubs/" if webpubsub_client_access.lower() == "mqtt" else "/client/hubs/"
        ...
        if isinstance(self._config.credential, AzureKeyCredential):
            ...
            token = super().get_client_access_token(client_type=webpubsub_client_access, **kwargs).get("token")
        return {
            "baseUrl": client_url,
            "token": token,
