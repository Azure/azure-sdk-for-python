...
def _decode_token(client, token, path="/client/hubs/hub"):
    return jwt.decode(
        token, client._config.credential.key, algorithms=["HS256"], audience=client._config.endpoint + path
    )
...
async def test_generate_uri_contains_expected_payloads_dto(user_id, roles, groups):
    ...
    token = await client.get_client_access_token(
        user_id=user_id, roles=roles, minutes_to_expire=minutes_to_expire, groups=groups
    )
    assert token
    assert len(token) == 3
...
test_cases = [
    (
        ...
        "hub",
        "ws://localhost:8080/client/hubs/hub",
    ),
...
test_cases = [
    (
        "Endpoint=http://localhost;Port=8080;AccessKey={};Version=1.0;".format(access_key),
        "hub",
        "ws://localhost:8080/clients/mqtt/hubs/hub",
    ),
...
test_cases = [
    (
        "Endpoint=http://localhost;Port=8080;AccessKey={};Version=1.0;".format(access_key),
        "hub",
        "ws://localhost:8080/clients/socketio/hubs/hub",
    ),
