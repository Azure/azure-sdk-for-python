...
class TestWebpubsubSmoke(WebpubsubTest):
    ...
    @WebpubsubPowerShellPreparer()
    @recorded_by_proxy
    def test_webpubsub_send_to_all_api_management_proxy(
        self, webpubsub_endpoint, webpubsub_reverse_proxy_endpoint=None
    ):
        client = self.create_client(
            endpoint=webpubsub_endpoint, hub="hub", reverse_proxy_endpoint=webpubsub_reverse_proxy_endpoint
        )
    ...
    def test_webpubsub_send_request(self, webpubsub_endpoint):
        ...
        request = build_web_pub_sub_service_send_to_all_request(
            "Hub", content="test_webpubsub_send_request", content_type="text/plain"
        )
    ...
    def test_webpubsub_send_to_all_api_management_proxy_counter_test(self, webpubsub_endpoint):
        client = self.create_client(
            endpoint=webpubsub_endpoint, hub="hub", reverse_proxy_endpoint="https://example.azure-api.net"
        )
        ...
        if exc_info.type is HttpResponseError:
            ...
            assert (
                "No such host is known" in response_content["Message"]
                or "Name or service not known" in response_content["Message"]
            )  # The former for Windows and the latter for Linux
