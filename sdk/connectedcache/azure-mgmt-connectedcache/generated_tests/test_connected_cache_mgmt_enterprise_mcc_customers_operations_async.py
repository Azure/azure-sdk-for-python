...
class TestConnectedCacheMgmtEnterpriseMccCustomersOperationsAsync(AzureMgmtRecordedTestCase):
    def setup_method(self, method):
        self.client = self.create_mgmt_client(ConnectedCacheMgmtClient, is_async=True)

    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_enterprise_mcc_customers_get(self, resource_group):
        response = await self.client.enterprise_mcc_customers.get(
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_enterprise_mcc_customers_begin_create_or_update(self, resource_group):
        response = await (
            await self.client.enterprise_mcc_customers.begin_create_or_update(
                resource_group_name=resource_group.name,
                customer_resource_name="str",
                resource={
                    "location": "str",
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_enterprise_mcc_customers_update(self, resource_group):
        response = await self.client.enterprise_mcc_customers.update(
            resource_group_name=resource_group.name,
            customer_resource_name="str",
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_enterprise_mcc_customers_begin_delete(self, resource_group):
        response = await (
            await self.client.enterprise_mcc_customers.begin_delete(
                resource_group_name=resource_group.name,
                customer_resource_name="str",
            )
        ).result()  # call '.result()' to poll until service return final result
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_enterprise_mcc_customers_list_by_resource_group(self, resource_group):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_enterprise_mcc_customers_list_by_subscription(self, resource_group):
