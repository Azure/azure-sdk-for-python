...
class MgmtKeyVaultTest(AzureMgmtTestCase):
    ...
    def test_keyvault(self, resource_group):
        ...
        VAULT_NAME = "myVault"
        OPERATION_KIND = "myOperationKind"
        LOCATION = "myLocation"
        ...
        result = self.mgmt_client.vaults.update_access_policy(resource_group_name=RESOURCE_GROUP, vault_name=VAULT_NAME, operation_kind=OPERATION_KIND, parameters=BODY)
        ...
        result = self.mgmt_client.vaults.update(resource_group_name=RESOURCE_GROUP, vault_name=VAULT_NAME, properties= + ToSnakeCase(k).toUpperCase() + )
