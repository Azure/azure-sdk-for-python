...
class TestMgmtKeyVault(AzureMgmtRecordedTestCase):
    ...
    def setup_method(self, method):
        self.mgmt_client = self.create_mgmt_aio_client(azure.mgmt.keyvault.aio.KeyVaultManagementClient)
    ...
    def test_keyvault(self, resource_group):
        ...
        result = self.event_loop.run_until_complete(
            self.mgmt_client.vaults.begin_create_or_update(
        ...
        BODY = {
            ...
            "properties": {
                ...
                "network_acls": {
                    ...
                    "ip_rules": [{"value": "124.56.78.91"}, {"value": "'10.91.4.0/24'"}],
                    "virtual_network_rules": [
                        {
                            ...
                            + "/resourceGroups/"
                            + RESOURCE_GROUP
                            + "/providers/Microsoft.Network/virtualNetworkstest-vnet/subnetssubnet1"
                        }
                    ],
                },
                "enabled_for_deployment": True,
        ...
        result = self.event_loop.run_until_complete(
            self.mgmt_client.vaults.update_access_policy(
        ...
        result = self.event_loop.run_until_complete(
            self.mgmt_client.vaults.get(resource_group_name=RESOURCE_GROUP, vault_name=VAULT_NAME)
        )

        # /Vaults/get/List vaults in the specified resource group[get]
        result = self.to_list(
            self.mgmt_client.vaults.list_by_resource_group(resource_group_name=RESOURCE_GROUP, top="1")
        )

        # /Vaults/get/List deleted vaults in the specified subscription[get]
        result = self.to_list(self.mgmt_client.vaults.list_deleted())

        # /Vaults/get/List vaults in the specified subscription[get]
        result = self.to_list(self.mgmt_client.vaults.list_by_subscription(top="1"))
        ...
        result = self.event_loop.run_until_complete(
            self.mgmt_client.vaults.update(
        ...
        result = self.event_loop.run_until_complete(
            self.mgmt_client.vaults.check_name_availability(
        ...
        result = self.event_loop.run_until_complete(
            self.mgmt_client.vaults.delete(resource_group_name=RESOURCE_GROUP, vault_name=VAULT_NAME)
        )

        # /Vaults/get/Retrieve a deleted vault[get]
        result = self.event_loop.run_until_complete(
            self.mgmt_client.vaults.get_deleted(location=LOCATION, vault_name=VAULT_NAME)
        )
