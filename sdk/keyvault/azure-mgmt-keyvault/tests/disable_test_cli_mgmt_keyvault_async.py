...
class TestMgmtKeyVault(AzureMgmtAsyncTestCase):
    ...
    def test_keyvault(self, **kwargs):
        ...
        result = self.event_loop.run_until_complete(
            self.mgmt_client.vaults.update_access_policy(resource_group_name=RESOURCE_GROUP, vault_name=VAULT_NAME, operation_kind=OPERATION_KIND, parameters=PARAMETERS)
        )
        ...
        result = self.to_list(
            self.mgmt_client.vaults.list_deleted()
        )

        # /Vaults/get/List vaults in the specified subscription[get]
        result = self.to_list(
            self.mgmt_client.vaults.list_by_subscription(top="1")
        )
        ...
        result = self.event_loop.run_until_complete(
            self.mgmt_client.vaults.update(resource_group_name=RESOURCE_GROUP, vault_name=VAULT_NAME, parameters=PROPERTIES)
        )

        # /Vaults/post/Validate a vault name[post]
        result = self.event_loop.run_until_complete(
            self.mgmt_client.vaults.check_name_availability({ 'name': 'sample-vault', 'type': 'Microsoft.KeyVault/vaults' })
        )
