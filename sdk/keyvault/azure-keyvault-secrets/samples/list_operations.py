...
def run_sample():
    ...
    try:
        ...
        secrets = client.list_secret_properties()
        for secret in secrets:
            retrieved_secret = client.get_secret(secret.properties.name)
            print(
                "Secret with name '{0}' and value {1} was found.".format(retrieved_secret.properties.name, retrieved_secret.properties.name)
            )
        ...
        # with the new value.
        updated_secret = client.set_secret(bank_secret.properties.name, "newSecretValue")
        print(
            "Secret with name '{0}' was updated with new value '{1}'".format(updated_secret.properties.name, updated_secret.value)
        )
        ...
        secret_versions = client.list_secret_versions(bank_secret.properties.name)
        ...
        client.delete_secret(bank_secret.properties.name)
        ...
        for deleted_secret in deleted_secrets:
            print(
                "Secret with name '{0}' has recovery id '{1}'".format(deleted_secret.properties.name, deleted_secret.recovery_id)
            )
