...
async def run_sample():
    ...
    try:
        ...
        async for secret in secrets:
            retrieved_secret = await client.get_secret(secret.properties.name)
            print(
                "Secret with name '{0}' with value '{1}' was found.".format(
                    retrieved_secret.properties.name, retrieved_secret.value
                )
            )
        ...
        # with the new value.
        updated_secret = await client.set_secret(bank_secret.properties.name, "newSecretValue")
        print(
        ...
        async for secret in secret_versions:
            print("Bank Secret with name '{0}' has version: '{1}'".format(secret.properties.name, secret.version))

        # The bank account and storage accounts got closed. Let's delete bank and storage accounts secrets.
        await client.delete_secret(bank_secret.properties.name)
