...
class CertificateClientTests(KeyVaultTestCase):
    ...
    async def _import_common_certificate(self, client, cert_name):
        ...
        cert_password = '123'
        cert_policy = CertificatePolicyGenerated(x509_certificate_properties=X509CertificateProperties(
    ...
    def _validate_certificate_bundle(self, cert, vault, cert_name, cert_policy):
        ...
        self._validate_issuer_parameters(
            issuer_parameters_bundle=cert_policy.issuer_parameters,
            issuer_parameters=cert.policy.issuer_parameters
        )
        if cert_policy.x509_certificate_properties:
            self._validate_x509_properties(x509_bundle=cert_policy.x509_certificate_properties, x509=cert.policy.x509_properties)
    ...
    def _validate_issuer_parameters(self, issuer_parameters_bundle, issuer_parameters):
        self.assertEqual(issuer_parameters_bundle.name, issuer_parameters.issuer_name)
        self.assertEqual(issuer_parameters_bundle.certificate_type, issuer_parameters.certificate_type)
        self.assertEqual(issuer_parameters_bundle.certificate_transparency, issuer_parameters.certificate_transparency)

    def _validate_x509_properties(self, x509_bundle, x509):
        self.assertIsNotNone(x509)
        self.assertEqual(x509_bundle.subject, x509.subject_name)
        if x509_bundle.ekus:
            self.assertEqual(x509_bundle.ekus, x509.ekus)
        if x509_bundle.key_usage:
            self.assertEqual(x509_bundle.key_usage, x509.key_usage)
        self.assertEqual(x509_bundle.validity_in_months, x509.validity_in_months)
        if not x509_bundle.subject_alternative_names:
            return
        if x509_bundle.subject_alternative_names.emails:
            policy_emails = x509.san_emails
            for san_email in x509_bundle.subject_alternative_names.emails:
        ...
        if x509_bundle.subject_alternative_names.upns:
            policy_upns_list = x509.san_upns
            for san_upns in x509_bundle.subject_alternative_names.upns:
        ...
        if x509_bundle.subject_alternative_names.dns_names:
            policy_dns_names = x509.san_dns_names
            for san_dns_name in x509_bundle.subject_alternative_names.dns_names:
    ...
    async def test_list_certificate_versions(self, vault_client, **kwargs):
        ...
        await self._validate_certificate_list(certificates=(client.list_certificate_versions(cert_name)), expected=expected)
    @ResourceGroupPreparer()
    @AsyncVaultClientPreparer()
    @AsyncKeyVaultTestCase.await_prepared_test
    ...
    async def test_policy(self, vault_client, **kwargs):
        ...
        retrieved_policy = await client.get_policy(name=cert_name)
        ...
        await client.update_policy(name=cert_name, policy=CertificatePolicy._from_certificate_policy_bundle(cert_policy))
        updated_cert_policy = await client.get_policy(name=cert_name)
