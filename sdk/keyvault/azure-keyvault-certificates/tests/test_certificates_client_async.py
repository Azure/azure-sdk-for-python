...
class CertificateClientTests(KeyVaultTestCase):
    ...
    async def _import_common_certificate(self, client, cert_name):
        ...
        return await client.import_certificate(cert_name, CertificateClientTests.CERT_CONTENT, policy=cert_policy, password=cert_password)
    ...
    @KeyVaultPreparer()
    @KeyVaultClientPreparer()
    async def test_import_certificate_no_policy(self, client):
        certificate = await client.import_certificate(
            certificate_name="importCertificate",
            certificate_bytes=CertificateClientTests.CERT_CONTENT,
            password="123"
        )
