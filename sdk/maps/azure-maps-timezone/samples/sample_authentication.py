# coding: utf-8
# -------------------------------------------------------------------------
# Copyright (c) Microsoft Corporation. All rights reserved.
# Licensed under the MIT License. See License.txt in the project root for
# license information.
# --------------------------------------------------------------------------

"""
FILE: sample_authentication.py
DESCRIPTION:
    This sample demonstrates how to authenticate with the Azure Maps Timezone
    service with an Subscription key. See more details about authentication here:
    https://docs.microsoft.com/azure/azure-maps/how-to-manage-account-keys
USAGE:
    python sample_authentication.py
    Set the environment variables with your own values before running the sample:
    - AZURE_SUBSCRIPTION_KEY - your Azure Maps subscription key
    - TENANT_ID - your tenant ID that wants to access Azure Maps account
    - CLIENT_ID - your client ID that wants to access Azure Maps account
    - CLIENT_SECRET - your client secret that wants to access Azure Maps account
    - MAPS_CLIENT_ID - your Azure Maps client ID
"""

import os

def authentication_maps_service_client_with_subscription_key_credential():
    # [START create_maps_timezone_service_client_with_key]
    from azure.core.credentials import AzureKeyCredential
    from azure.maps.timezone import MapsTimezoneClient

    subscription_key = os.getenv("AZURE_SUBSCRIPTION_KEY")

    maps_timezone_client = MapsTimezoneClient(credential=AzureKeyCredential(subscription_key))
    # [END create_maps_timezone_service_client_with_key]

    iana_id = "America/New_York"
    result = maps_timezone_client.get_timezone_by_id(iana_id)

    print('IANA "{}" timezone is {}'.format(iana_id, result.time_zones[0].names.generic))


def authentication_maps_service_client_with_aad_credential():
    # [START create_maps_timezone_service_client_with_aad]
    from azure.identity import DefaultAzureCredential
    from azure.maps.timezone import MapsTimezoneClient

    credential = DefaultAzureCredential()
    maps_client_id = os.getenv("MAPS_CLIENT_ID")

    maps_timezone_client = MapsTimezoneClient(client_id=maps_client_id, credential=credential)
    # [END create_maps_timezone_service_client_with_aad]

    iana_id = "America/New_York"
    result = maps_timezone_client.get_timezone_by_id(iana_id)

    print('IANA "{}" timezone is {}'.format(iana_id, result.time_zones[0].names.generic))


if __name__ == '__main__':
    authentication_maps_service_client_with_subscription_key_credential()
    authentication_maps_service_client_with_aad_credential()