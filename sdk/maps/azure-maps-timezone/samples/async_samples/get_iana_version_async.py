...
"""
FILE: get_iana_version_async.py
DESCRIPTION:
    This API returns the current IANA version number as Metadata.
USAGE:
    python get_iana_version_async.py
...
async def get_iana_version_async():
    ...
    try:
        async with timezone_client:
            result = await timezone_client.get_iana_version()
            print(result)
...
if __name__ == "__main__":
    asyncio.run(get_iana_version_async())
