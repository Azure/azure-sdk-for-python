...
### Breaking Changes
  ...
  - Method `AlertsOperations.get_all_tenant` changed its parameter `target_resource`/`target_resource_type`/`target_resource_group`/`monitor_service`/`monitor_condition`/`severity`/`alert_state`/`alert_rule`/`smart_group_id`/`include_context`/`include_egress_config`/`page_count`/`sort_by`/`sort_order`/`select`/`time_range`/`custom_time_range` from `positional_or_keyword` to `keyword_only`
...
### Other Changes
  ...
  - Deleted model `AlertEnrichmentsList`/`AlertsList`/`OperationsList` which actually were not used by SDK users
  - Deleted enum `ApiVersion` which actually were not used by SDK users
