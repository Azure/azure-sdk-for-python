...
class MgmtAlertsTest(AzureMgmtTestCase):
    ...
    def test_alertsmanagement(self, resource_group):
        ...
        BODY = {
            ...
            "properties": {
                "scope": {
                    ...
                    "values": ["/subscriptions/" + SUBSCRIPTION_ID + "/resourceGroups/" + RESOURCE_GROUP],
                },
                "conditions": {
                    "severity": {"operator": "Equals", "values": ["Sev0", "Sev2"]},
                    "monitor_service": {"operator": "Equals", "values": ["Platform", "Application Insights"]},
                    "monitor_condition": {"operator": "Equals", "values": ["Fired"]},
                    "target_resource_type": {"operator": "NotEquals", "values": ["Microsoft.Compute/VirtualMachines"]},
                },
                "type": "Suppression",
                "suppression_config": {
        ...
        result = self.mgmt_client.action_rules.create_update(
            resource_group_name=RESOURCE_GROUP, action_rule_name=ACTION_RULE_NAME, action_rule=BODY
        )
        ...
        result = self.mgmt_client.action_rules.get_by_name(
            resource_group_name=RESOURCE_GROUP, action_rule_name=ACTION_RULE_NAME
        )
        ...
        result = self.mgmt_client.action_rules.delete(
            resource_group_name=RESOURCE_GROUP, action_rule_name=ACTION_RULE_NAME
        )
