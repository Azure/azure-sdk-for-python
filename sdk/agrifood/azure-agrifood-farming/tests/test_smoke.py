...
class FarmBeatsSmokeTestCase(FarmBeatsTest):
    ...
    def test_farmer(self, agrifood_endpoint, agrifood_farmer_id):
        ...
        retrieved_farmer = client.farmers.get(farmer_id=agrifood_farmer_id)
    ...
    @FarmBeatsPowerShellPreparer()
    def test_boundary(self, agrifood_endpoint, agrifood_farmer_id, agrifood_boundary_id):
        client = self.create_client(agrifood_endpoint=agrifood_endpoint)
        boundary_id = agrifood_boundary_id + "boundary"
        boundary = self.create_boundary_if_not_exist(client, agrifood_farmer_id, boundary_id)
    ...
    @FarmBeatsPowerShellPreparer()
    def test_satellite_job(self, agrifood_endpoint, agrifood_farmer_id, agrifood_boundary_id, agrifood_job_id_prefix):
        client = self.create_client(agrifood_endpoint=agrifood_endpoint)
        ...
        end_date_time = datetime.datetime(2020, 1, 31, tzinfo=Utc())
    ...
    @FarmBeatsPowerShellPreparer()
    def test_scenes(self, agrifood_endpoint, agrifood_farmer_id, agrifood_boundary_id):
        client = self.create_client(agrifood_endpoint=agrifood_endpoint)
        boundary_id = "{}-scenes".format(agrifood_boundary_id)
        start_date_time = datetime.datetime(2020, 1, 1, tzinfo=Utc())
    ...
    @FarmBeatsPowerShellPreparer()
    def test_download_image(self, agrifood_endpoint, agrifood_farmer_id, agrifood_boundary_id):
        client = self.create_client(agrifood_endpoint=agrifood_endpoint)
        boundary_id = "{}-scenes".format(agrifood_boundary_id)
        scenes = [f for f in client.scenes.list(farmer_id=agrifood_farmer_id, boundary_id=boundary_id)]
