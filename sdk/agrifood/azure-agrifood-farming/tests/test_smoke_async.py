...
class FarmBeatsSmokeTestCaseAsync(FarmBeatsTestAsync):
    ...
    @FarmBeatsPowerShellPreparer()
    async def test_boundary(self, farmbeats_endpoint, farmbeats_farmer_id, farmbeats_boundary_id):
        client = self.create_client(farmbeats_endpoint=farmbeats_endpoint)
    ...
    @FarmBeatsPowerShellPreparer()
    async def test_satellite_job(self, farmbeats_endpoint, farmbeats_farmer_id, farmbeats_boundary_id, farmbeats_job_id_prefix):
        client = self.create_client(farmbeats_endpoint=farmbeats_endpoint)
    ...
    @FarmBeatsPowerShellPreparer()
    async def test_scenes(self, farmbeats_endpoint, farmbeats_farmer_id, farmbeats_boundary_id):
        client = self.create_client(farmbeats_endpoint=farmbeats_endpoint)
        boundary_id = "{}-scenes".format(farmbeats_boundary_id)
        start_date_time = datetime.datetime(2020, 1, 1)
        end_date_time = datetime.datetime(2020, 12, 31)
    ...
    @FarmBeatsPowerShellPreparer()
    async def test_download_image(self, farmbeats_endpoint, farmbeats_farmer_id, farmbeats_boundary_id):
        client = self.create_client(farmbeats_endpoint=farmbeats_endpoint)
        boundary_id = "{}-scenes".format(farmbeats_boundary_id)
        scenes = [f async for f in client.scenes.list(farmer_id=farmbeats_farmer_id, boundary_id=boundary_id)]
