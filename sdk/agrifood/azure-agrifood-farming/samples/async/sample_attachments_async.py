...
async def sample_attachments_async():
    ...
    try:
        ...
        await client.attachments.get(party_id=party_id, attachment_id=attachment_on_party_id)
    ...
    try:
        ...
        await client.attachments.get(party_id=party_id, attachment_id=attachment_on_farm_id)
        print("Attachment already exists. Not updating file.")
        ...
        attachment = {"resourceId": farm_id, "resourceType": "Farm", "name": "a"}
    ...
    async for attachment in party_attachments:
        downloaded_attachment = await client.attachments.download(party_id=party_id, attachment_id=attachment["id"])
        out_path = Path(
            "./data/attachments/"
