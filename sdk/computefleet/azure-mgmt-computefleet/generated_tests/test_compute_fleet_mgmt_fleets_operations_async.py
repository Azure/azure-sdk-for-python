...
class TestComputeFleetMgmtFleetsOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_fleets_list_virtual_machines(self, resource_group):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_fleets_begin_cancel(self, resource_group):
        response = await (
            await self.client.fleets.begin_cancel(
                resource_group_name=resource_group.name,
                fleet_name="str",
            )
        ).result()  # call '.result()' to poll until service return final result
