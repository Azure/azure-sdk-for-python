...
class TestNewRelicObservabilityMgmtMonitorsOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_monitors_latest_linked_saa_s(self, resource_group):
        response = await self.client.monitors.latest_linked_saa_s(
            resource_group_name=resource_group.name,
            monitor_name="str",
            api_version="2025-05-01-preview",
        )
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_monitors_begin_link_saa_s(self, resource_group):
        response = await (
            await self.client.monitors.begin_link_saa_s(
                resource_group_name=resource_group.name,
                monitor_name="str",
