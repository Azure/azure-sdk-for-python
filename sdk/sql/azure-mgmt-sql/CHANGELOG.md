...
### Features Added
  ...
  - Model `DatabaseProperties` added property `provisioning_state`
  - Model `DatabaseUpdateProperties` added property `provisioning_state`
  - Model `DeletedServerProperties` added property `original_resource_group`
  - Model `DeletedServerProperties` added property `scheduled_purge_time`
  - Model `DistributedAvailabilityGroupProperties` added property `link_mode`
  - Model `ElasticPoolProperties` added property `current_sku`
  - Model `ElasticPoolUpdateProperties` added property `current_sku`
  - Model `EndpointCertificateProperties` added property `trusted_root_certificates`
  - Model `ManagedInstanceAdministratorProperties` added property `principal_type`
  - Model `ManagedInstanceDtcProperties` added property `fqdn_enabled`
  - Enum `PrivateEndpointProvisioningState` added member `CANCELED`
  - Enum `PrivateEndpointProvisioningState` added member `CREATED`
  - Enum `PrivateEndpointProvisioningState` added member `IN_PROGRESS`
  - Enum `PrivateEndpointProvisioningState` added member `SUCCEEDED`
  - Enum `ServerNetworkAccessFlag` added member `SECURED_BY_PERIMETER`
  - Model `ServerUsageProperties` added property `next_reset_time`
  - Model `ServerUsageProperties` added property `resource_name`
...
### Breaking Changes
  ...
  - Deleted or renamed enum value `PrivateEndpointProvisioningState.APPROVING`
  - Deleted or renamed enum value `PrivateEndpointProvisioningState.DROPPING`
  - Deleted or renamed enum value `PrivateEndpointProvisioningState.READY`
  - Deleted or renamed enum value `PrivateEndpointProvisioningState.REJECTING`
