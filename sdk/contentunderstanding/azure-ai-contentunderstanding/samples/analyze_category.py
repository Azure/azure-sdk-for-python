...
# --------------------------------------------------------------------------
"""
Async sample: Create a classifier to categorize financial documents without automatic page segmentation.

This sample demonstrates how to:
1. Create a custom analyzer with content categories for document classification
2. Disable automatic page segmentation by category (enable_segment=False)
3. Classify documents into categories (Invoice, Bank Statement, Loan Application)
4. View classification results without automatic segmentation
5. Clean up resources

The key difference from analyze_category_enable_segments.py is that enable_segment=False,
which means the analyzer will classify the entire document as a single unit without
automatically segmenting pages by category.
...
async def main() -> None:
    ...
    async with ContentUnderstandingClient(endpoint=endpoint, credential=credential) as client:
        ...
        print("Note: enable_segment=False - document will be classified as a single unit\n")

        # Create an analyzer with content categories for document classification
        # enable_segment=False disables automatic segmentation - entire document is classified as one unit
        ...
        # Note: With enable_segment=False, each document will be classified as a single unit.
        ...
        for test_file in test_files:
            ...
            for content in analyze_result.contents:
                if content.kind == MediaContentKind.DOCUMENT:
                    ...
                    # When enable_segment=False, the document is classified as a single unit
            ...
            result_filename = f"analyze_category_{test_file.replace('.pdf', '')}_{timestamp}.json"
