...
# --------------------------------------------------------------------------
"""
FILE: sample_create_analyzer_workflow_async.py
...
USAGE:
    python sample_create_analyzer_workflow_async.py
...
async def main() -> None:
    ...
    async with ContentUnderstandingClient(endpoint=endpoint, credential=credential) as client:
        ...
        try:
            default_poller = await client.begin_create_analyzer(
                analyzer_id=default_analyzer_id,
                resource=default_analyzer,
                allow_replace=True,
            )
            await default_poller.result()

            agentic_poller = await client.begin_create_analyzer(
                analyzer_id=agentic_analyzer_id,
                resource=agentic_analyzer,
                allow_replace=True,
            )
            await agentic_poller.result()

            with open("sample_files/workflow_invoice_20_items.pdf", "rb") as f:
                sample_invoice = f.read()

            default_analyze = await client.begin_analyze(
                analyzer_id=default_analyzer_id,
                inputs=[AnalysisInput(data=sample_invoice)],
            )
            default_result: AnalysisResult = await default_analyze.result()

            agentic_analyze = await client.begin_analyze(
                analyzer_id=agentic_analyzer_id,
                inputs=[AnalysisInput(data=sample_invoice)],
            )
            agentic_result: AnalysisResult = await agentic_analyze.result()
            ...
            await client.delete_analyzer(default_analyzer_id)
            await client.delete_analyzer(agentic_analyzer_id)
