...
ABOUT ANALYSIS CONFIGS:
    ...
    - Signatures: Enabled by EnableLayout with detailed output - Signature regions and
      metadata (available only in ``2026-06-01-preview``; see also
    ``sample_detect_signatures_async.py``)

    For custom analyzers, you can configure these options in ContentAnalyzerConfig when creating
    the analyzer.

PREREQUISITES:
    To get started you'll need a Microsoft Foundry resource. See sample_update_defaults_async.py
    for setup guidance.
