...
# --------------------------------------------------------------------------
"""
FILE: sample_advanced_rehydrate_operation.py
...
USAGE:
    python sample_advanced_rehydrate_operation.py
...
def process_a(token_file: str) -> None:
    ...
    # [END rehydrate_process_a]


def process_b(token_file: str) -> None:
    ...
    result: AnalysisResult = resumed_poller.result()
    print(
...
def main() -> None:
    ...
    token_file = os.path.join(tempfile.gettempdir(), "cu-continuation-token.txt")

    if len(sys.argv) > 1:
        # Called as a subprocess
        role = sys.argv[1]
        if role == "--process-a":
            process_a(token_file)
        elif role == "--process-b":
            process_b(token_file)
        else:
            print(f"Unknown role: {role}")
            sys.exit(1)
    else:
        # Main orchestrator — spawn two separate subprocesses
        print("=== Two-Process Rehydration Demo ===\n")
