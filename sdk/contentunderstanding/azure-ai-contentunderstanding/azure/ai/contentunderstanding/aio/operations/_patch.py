...
class FacesOperations(FacesOperationsGenerated):
    ...
    @distributed_trace_async
    async def compare(self, *args, **kwargs) -> CompareFacesResult:
...
class PersonDirectoriesOperations(PersonDirectoriesOperationsGenerated):
    ...
    @distributed_trace_async
    async def find_similar_faces(self, person_directory_id: str, *args, **kwargs) -> FindSimilarFacesResult:
        ...
        if args and len(args) >= 1:
            ...
            if isinstance(input_data, str):
                ...
                return await super().find_similar_faces(
                    person_directory_id=person_directory_id,
                    face_source=face_source,
                    **kwargs
                )
        ...
        # Fall back to original method behavior
        return await super().find_similar_faces(person_directory_id=person_directory_id, **kwargs)

    @distributed_trace_async
    async def identify_person(self, person_directory_id: str, *args, **kwargs) -> IdentifyPersonResult:
        ...
        if args and len(args) >= 1:
            ...
            if isinstance(input_data, str):
                ...
                from ...models import FaceSource
                face_source = FaceSource(url=input_data)
                return await super().identify_person(
                    person_directory_id=person_directory_id,
                    face_source=face_source,
                    **kwargs
                )
        ...
        # Fall back to original method behavior
        return await super().identify_person(person_directory_id=person_directory_id, **kwargs)