...
# --------------------------------------------------------------------------
"""
TEST FILE: test_sample_content_source_async.py

DESCRIPTION:
    These live-only async tests validate the sample_content_source_async.py sample code.
    Keep this test in sync with samples/async_samples/sample_content_source_async.py.

USAGE:
    pytest test_sample_content_source_async.py
"""
...
class TestSampleContentSourceAsync(ContentUnderstandingClientTestBaseAsync):
    """Tests for sample_content_source_async.py"""

    @ContentUnderstandingPreparer()
    async def test_sample_content_source_async(self, **kwargs) -> None:
        """Validate field grounding sources as plain strings (sample_content_source_async.py)."""
        if not is_live():
        ...
        try:
            # Keep in sync with samples/async_samples/sample_content_source_async.py
