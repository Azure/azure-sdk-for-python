...
class TestSampleListAnalyzersAsync(ContentUnderstandingClientTestBaseAsync):
    ...
    async def test_sample_list_analyzers_async(self, azure_content_understanding_endpoint: str) -> None:
        ...
        # List all analyzers
        analyzers = []
        async for analyzer in client.list_analyzers():
        ...
        prebuilt_count = sum(1 for a in analyzers 
                            if hasattr(a, 'analyzer_id') and 
                            getattr(a, 'analyzer_id', '').startswith('prebuilt-'))
        custom_count = sum(1 for a in analyzers 
                          if hasattr(a, 'analyzer_id') and 
                          not getattr(a, 'analyzer_id', '').startswith('prebuilt-'))
