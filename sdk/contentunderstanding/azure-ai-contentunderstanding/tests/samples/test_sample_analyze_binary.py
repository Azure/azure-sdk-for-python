...
class TestSampleAnalyzeBinary(ContentUnderstandingClientTestBase):
    ...
    def _test_to_llm_input(self, result):
        ...
        Mirrors the [START convert_to_llm_input] block in sample_analyze_binary.py:
    ...
    def test_sample_analyze_binary_with_content_range(self, contentunderstanding_endpoint: str) -> None:
        ...
        full_poller = client.begin_analyze_binary(analyzer_id="prebuilt-documentSearch", binary_input=file_bytes)
        ...
        assert (
            page2_doc.pages[0].page_number == 2
        ...
        assert (
            combine2_doc.start_page_number == 1
        ), f"'1,3-4' should start at page 1, got {combine2_doc.start_page_number}"
        ...
        assert (
            range_page_count == expected_range_page_count
        ...
        assert (
            range_doc.end_page_number == full_doc.end_page_number
        ...
        assert (
            actual_range_pages == expected_range_pages
        ), f"'3-' page numbers should be {expected_range_pages}, got {actual_range_pages}"
        print(
            f"[PASS] '3-': {range_page_count} pages (pages {range_doc.start_page_number}-{range_doc.end_page_number})"
        )
        ...
        assert (
            combine_page_count == expected_combine_page_count
        ...
        assert (
            actual_combine_pages == expected_combine_pages
        ...
        assert (
            combine_doc.end_page_number == full_doc.end_page_number
