# coding=utf-8
# --------------------------------------------------------------------------
# Copyright (c) Microsoft Corporation. All rights reserved.
# Licensed under the MIT License. See License.txt in the project root for license information.
# Code generated by Microsoft (R) Python Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is regenerated.
# --------------------------------------------------------------------------

from azure.identity import DefaultAzureCredential

from azure.mgmt.azurestackhci import AzureStackHCIClient

"""
# PREREQUISITES
    pip install azure-identity
    pip install azure-mgmt-azurestackhci
# USAGE
    python edge_machine_network_adapter_jobs_create_or_update_apply_configuration.py

    Before run the sample, please set the values of the client ID, tenant ID and client secret
    of the AAD application as environment variables: AZURE_CLIENT_ID, AZURE_TENANT_ID,
    AZURE_CLIENT_SECRET. For more info about how to get the value, please see:
    https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal
"""


def main():
    client = AzureStackHCIClient(
        credential=DefaultAzureCredential(),
        subscription_id="SUBSCRIPTION_ID",
    )

    response = client.edge_machine_network_adapter_jobs.begin_create_or_update(
        resource_group_name="test-rg",
        edge_machine_name="EdgeMachine01",
        network_adapter_name="ethernet0",
        job_name="ApplyConfiguration",
        resource={
            "properties": {
                "deploymentMode": "Deploy",
                "jobType": "ApplyConfiguration",
                "targetConfiguration": {
                    "adapterName": "ethernet0",
                    "defaultGateway": "192.168.1.1",
                    "dnsServers": ["8.8.8.8", "8.8.4.4"],
                    "interfaceState": "up",
                    "ip4Address": "192.168.1.100",
                    "ipInterfaceType": "Static",
                    "subnetMask": "255.255.255.0",
                    "vlanId": 100,
                },
            }
        },
    ).result()
    print(response)


# x-ms-original-file: 2026-05-01-preview/EdgeMachineNetworkAdapterJobs_CreateOrUpdate_ApplyConfiguration.json
if __name__ == "__main__":
    main()
