...
class TestCloudHealthMgmtRelationshipsOperationsAsync(AzureMgmtRecordedTestCase):
    def setup_method(self, method):
        self.client = self.create_mgmt_client(CloudHealthMgmtClient, is_async=True)

    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_relationships_get(self, resource_group):
        response = await self.client.relationships.get(
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_relationships_list_by_health_model(self, resource_group):
        response = self.client.relationships.list_by_health_model(
            resource_group_name=resource_group.name,
            health_model_name="str",
        )
