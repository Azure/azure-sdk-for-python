...
async def generate_sas_token(client: PlanetaryComputerProClient, collection_id: str):
    ...
    get_token_response = await client.sas.get_token(collection_id=collection_id, duration_in_minutes=60)
    return get_token_response


async def sign_asset_href(client: PlanetaryComputerProClient, collection_id: str):
    ...
    get_sign_response = await client.sas.get_sign(href=href, duration_in_minutes=60)
    return get_sign_response.href, href  # Return both signed and unsigned hrefs


async def download_asset(signed_href: str):
...
async def main():
    ...
    signed_href, unsigned_href = await sign_asset_href(client, collection_id)

    # Using SAS token appended to unsigned URI
    sas_token_response = await generate_sas_token(client, collection_id)
    sas_token = sas_token_response.token
    href_with_sas = f"{unsigned_href}?{sas_token}"

    # Test both methods
    await download_asset(signed_href)
    await download_asset(href_with_sas)
