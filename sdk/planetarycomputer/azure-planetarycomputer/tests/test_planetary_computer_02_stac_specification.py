...
class TestPlanetaryComputerStacSpecification(PlanetaryComputerClientTestBase):
    ...
    def test_01_get_conformance_class(self, planetarycomputer_endpoint):
        ...
        assert len(conformance.conforms_to) >= 10, f"Expected at least 10 conformance classes, got {len(conformance.conforms_to)}"
        ...
        assert len(found_uris) == len(expected_uris), f"Expected all 4 core STAC URIs, found {len(found_uris)}: {found_uris}"
    ...
    def test_03_list_collections(self, planetarycomputer_endpoint):
        ...
        for i, collection in enumerate(collections.collections[:5]):
            ...
            if hasattr(collection, 'description') and collection.description:
                desc = collection.description[:150] + "..." if len(collection.description) > 150 else collection.description
    ...
    def test_05_search_items_with_spatial_filter(self, planetarycomputer_endpoint):
        ...
        assert len(search_response.features) >= 2, f"Expected at least 2 items in spatial search, got {len(search_response.features)}"
    ...
    def test_08_search_items_with_temporal_filter(self, planetarycomputer_endpoint):
        ...
        assert len(search_response.features) >= 5, f"Expected at least 5 items in temporal search, got {len(search_response.features)}"
    ...
    def test_09_search_items_with_sorting(self, planetarycomputer_endpoint):
        ...
        assert len(search_response_desc.features) >= 3, f"Expected at least 3 items in DESC sort, got {len(search_response_desc.features)}"
        ...
        assert len(search_response_asc.features) >= 3, f"Expected at least 3 items in ASC sort, got {len(search_response_asc.features)}"
    ...
    def test_11_update_stac_item(self, planetarycomputer_endpoint):
        ...
        try:
            ...
            stac_item = client.stac.get_item(collection_id=collection_id, item_id=item_id)
            ...
            # Verify the update
            updated_item = client.stac.get_item(collection_id=collection_id, item_id=item_id)
