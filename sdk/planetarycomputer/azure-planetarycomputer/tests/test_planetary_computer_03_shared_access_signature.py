...
class TestPlanetaryComputerSharedAccessSignature(PlanetaryComputerClientTestBase):
    ...
    def test_01_get_token_with_default_duration(self, planetarycomputer_endpoint, planetarycomputer_collection_id):
        ...
        assert isinstance(response, SharedAccessSignatureToken), f"Response should be SharedAccessSignatureToken, got {type(response)}"
        ...
        if is_live():
            ...
            assert re.search(sas_token_pattern, response.token), "Token should match SAS token format (st, se, sp, sv, sr, sig)"
        else:
    ...
    def test_03_get_sign_with_collection_thumbnail(self, planetarycomputer_endpoint, planetarycomputer_collection_id):
        ...
        # Assert response is correct type
        assert response is not None, "Response should not be None"
        assert isinstance(response, SharedAccessSignatureSignedLink), f"Response should be SharedAccessSignatureSignedLink, got {type(response)}"
        ...
        if is_live():
            ...
            assert re.search(sas_href_pattern, signed_href), "Signed HREF should contain SAS parameters (st, se, sp, sv, sr, sig)"
        else:
    ...
    def test_05_revoke_token(self, planetarycomputer_endpoint, planetarycomputer_collection_id):
        ...
        assert isinstance(token_response, SharedAccessSignatureToken), f"Response should be SharedAccessSignatureToken, got {type(token_response)}"
