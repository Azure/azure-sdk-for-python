...
DESCRIPTION:
    ...
    accounts the metadata carries a `blob_path` instead, and the bytes are read
...
def read_merged_recording(conversations, agent_name, conversation_id, preview) -> None:
    ...
    if recording.blob_path:
        # Bring-your-own-storage: download from your own storage using the returned path.
        print(f"Recording is stored in your own storage at: {recording.blob_path}")
        return
...
def read_first_item_audio(conversations, agent_name, conversation_id, preview) -> None:
    ...
    for item in conversations.list_agent_conversation_items(agent_name, conversation_id, foundry_features=preview):
        ...
        if metadata.blob_path:
            print(f"Item audio is stored in your own storage at: {metadata.blob_path}")
            return
