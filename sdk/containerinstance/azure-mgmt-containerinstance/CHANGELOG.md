...
### Breaking Changes
  ...
  - Model `ContainerGroup` moved instance variable `provisioning_state`, `secret_references`, `containers`, `image_registry_credentials`, `restart_policy`, `ip_address`, `os_type`, `volumes`, `instance_view`, `diagnostics`, `subnet_ids`, `dns_config`, `sku`, `encryption_properties`, `init_containers`, `extensions`, `confidential_compute_properties`, `priority`, `identity_acls`, `container_group_profile`, `standby_pool_profile` and `is_created_from_standby_pool` under property `properties` whose type is `ContainerGroupProperties`
  ...
  - Model `NGroupPatch` moved instance variable `elastic_profile`, `placement_profile`, `container_group_profiles`, `provisioning_state` and `update_profile` under property `properties` whose type is `NGroupProperties`
  - Deleted or renamed model `ContainerGroupProvisioningState`
  - Deleted or renamed model `ListResultContainerGroup`
  ...
  - Method `ContainerGroupsOperations.list` changed return type from `ItemPaged[_models.ListResultContainerGroup]` to `ItemPaged[_models.ContainerGroup]`
  - Method `ContainerGroupsOperations.list_by_resource_group` changed return type from `ItemPaged[_models.ListResultContainerGroup]` to `ItemPaged[_models.ContainerGroup]`

### Other Changes

  - Deleted model `NGroupSkus`/`NGroupsSkusList` which actually were not used by SDK users
