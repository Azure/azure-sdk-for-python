...
def _account_id(rg):
    """Build a storage-account resource id under this RG (the account does NOT need to exist for endpoint create)."""
    return (
...
class TestStorageMoverMgmtEndpointsOperations(AzureMgmtRecordedTestCase):
    ...
    def test_create_update_get_delete(self, resource_group):
        ...
        # --- Container endpoint ---
        c = self.client.endpoints.create_or_update(
        ...
        c_get = self.client.endpoints.get(
            resource_group_name=rg, storage_mover_name=sm_name, endpoint_name=c_name,
        )
        assert c_get.name == c_name
        assert c_get.properties.endpoint_type == "AzureStorageBlobContainer"

        # --- NFS mount endpoint ---
        nfs = self.client.endpoints.create_or_update(
        ...
        assert nfs.name == nfs_name
        assert nfs.properties.endpoint_type == "NfsMount"
        assert nfs.properties.export == "/"
        assert nfs.properties.host == "10.0.0.1"

        nfs_get = self.client.endpoints.get(
            resource_group_name=rg, storage_mover_name=sm_name, endpoint_name=nfs_name,
        )
        assert nfs_get.properties.host == "10.0.0.1"
        assert nfs_get.properties.export == "/"

        # --- SMB mount endpoint with credentials ---
        smb = self.client.endpoints.create_or_update(
        ...
        assert smb.name == smb_name
        assert smb.properties.endpoint_type == "SmbMount"
        assert smb.properties.host == "10.0.0.1"
        assert smb.properties.share_name == "testshare"

        # --- SMB endpoint update + delete ---
        ...
        smb_updated = self.client.endpoints.update(
            resource_group_name=rg, storage_mover_name=sm_name, endpoint_name=smb_name,
            endpoint={
                "identity": {"type": "None"},
                "properties": {
        ...
        assert smb_updated.name == smb_name
        assert smb_updated.properties.endpoint_type == "SmbMount"
        assert smb_updated.properties.host == "10.0.0.1"
        assert smb_updated.properties.share_name == "testshare"

        self.client.endpoints.begin_delete(
            resource_group_name=rg, storage_mover_name=sm_name, endpoint_name=smb_name,
        ).result()

        # --- Azure Storage SMB FileShare endpoint ---
        fs = self.client.endpoints.create_or_update(
        ...
        assert fs.properties.endpoint_type == "AzureStorageSmbFileShare"
        assert fs.properties.file_share_name == "testfileshare"

        fs_get = self.client.endpoints.get(
            resource_group_name=rg, storage_mover_name=sm_name, endpoint_name=fs_name,
        )
        assert fs_get.properties.file_share_name == "testfileshare"
        assert fs_get.properties.description == "new file share endpoint"

        # --- list + missing-endpoint check ---
        items = list(self.client.endpoints.list(
            resource_group_name=rg, storage_mover_name=sm_name,
        ))
        assert len(items) > 1
    ...
    def test_multi_cloud_connector_create_get_delete(self, resource_group):
        ...
        endpoint = self.client.endpoints.create_or_update(
    ...
    def test_nfs_file_share_create_get_delete(self, resource_group):
        ...
        assert endpoint.properties.storage_account_resource_id is not None
