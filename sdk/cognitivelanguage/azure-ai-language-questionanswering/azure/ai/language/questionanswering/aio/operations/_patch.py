...
# --------------------------------------------------------------------------
"""Async customizations for Question Answering operations.

Mirrors the synchronous rich customization layer to provide:
 - Overloads supporting both options-object and flattened parameter styles
 - Validation for mutual exclusivity (`qna_id` vs `question`)
 - Tuple/dict metadata normalization for filters
 - Flexible text document input (str list, dict list, TextDocument list)
 - Backwards compatible positional-body handling while forbidding `options` kw
"""

from typing import Any, List, overload, Optional, Union, Tuple, MutableMapping, IO, cast
import copy
...
class QuestionAnsweringOperations(QuestionAnsweringOperationsGenerated):  # type: ignore
    ...
    @overload
    async def get_answers(  # type: ignore[override]
        self,
    ...
    @overload
    async def get_answers_from_text(  # type: ignore[override]
        self,
        ...
        **kwargs: Any,
        ...
        if args and (len(args) == 1) and not kwargs.get("question"):
            ...
            try:
                ...
                pass
            return await super().get_answers_from_text(body, **kwargs)  # type: ignore
