...
class TestQueryText(QuestionAnsweringTestCase):
    def test_query_text_basic(self, recorded_test, qna_creds): # pylint: disable=unused-argument
    ...
    def test_query_text_with_str_records(self, recorded_test, qna_creds): # pylint: disable=unused-argument
        client = QuestionAnsweringClient(qna_creds["qna_endpoint"], AzureKeyCredential(qna_creds["qna_key"]))
        params = {
    ...
    def test_query_text_default_lang_override(self, recorded_test, qna_creds): # pylint: disable=unused-argument
