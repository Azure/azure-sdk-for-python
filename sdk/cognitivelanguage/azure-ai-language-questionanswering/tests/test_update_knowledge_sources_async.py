...
class TestSourcesQnasSynonymsAsync(QuestionAnsweringTestCase):
    ...
    async def test_add_source(self, recorded_test, qna_creds):
        async with AuthoringClient(qna_creds["qna_endpoint"], AzureKeyCredential(qna_creds["qna_key"])) as client:
            # create project
    ...
    async def test_add_synonym(self, recorded_test, qna_creds):
        async with AuthoringClient(qna_creds["qna_endpoint"], AzureKeyCredential(qna_creds["qna_key"])) as client:
            ...
            async for s in synonyms:
                if ("alterations" in s) and (
                    "qnamaker" in s["alterations"] and "qna maker" in s["alterations"]
                ):
                    synonym_added = True
