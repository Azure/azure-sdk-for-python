...
class PseudoParser(object):
    ...
    async def expand(self, models):
        ...
        for model in models:
            ...
            await self._expand(model, expanded_map)
        return expanded_map

    async def _expand(self, model, model_map):
        ...
        if dependencies_to_resolve:
            ...
            resolved_dependency_map = await self.resolver.resolve(dependencies_to_resolve)
            model_map.update(resolved_dependency_map)
            for dependency_model in resolved_dependency_map.values():
                await self._expand(dependency_model, model_map)
        else:
...
def _get_model_dependencies(model):
    ...
    if "contents" in model:
        ...
        dependencies.union(set(components))
    ...
    if "extends" in model:
        ...
        if isinstance(model["extends"], six.text_type):
            ...
            dependencies.add(model["extends"])
            ...
            for item in model["extends"]:
                if isinstance(item, six.text_type):
                    ...
                    dependencies.union(set(_get_model_dependencies(item)))
