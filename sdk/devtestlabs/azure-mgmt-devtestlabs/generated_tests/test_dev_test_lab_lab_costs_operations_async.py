...
class TestDevTestLabLabCostsOperationsAsync(AzureMgmtRecordedTestCase):
    def setup_method(self, method):
        self.client = self.create_mgmt_client(DevTestLabClient, is_async=True)

    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_lab_costs_get(self, resource_group):
        response = await self.client.lab_costs.get(
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_lab_costs_create_or_update(self, resource_group):
        response = await self.client.lab_costs.create_or_update(
            resource_group_name=resource_group.name,
            lab_name="str",
            name="str",
            lab_cost={
