...
class SchedulesOperations:
    ...
    @distributed_trace
    def create_or_update(
        self, resource_group_name: str, lab_name: str, name: str, schedule: Union[_models.Schedule, IO], **kwargs: Any
    ) -> _models.Schedule:
        """Create or replace an existing schedule.
    ...
    def list_applicable(
        ...
        def prepare_request(next_link=None):
            if not next_link:
                ...
                request = build_list_applicable_request(
                    resource_group_name=resource_group_name,
                    lab_name=lab_name,
                    name=name,
                    subscription_id=self._config.subscription_id,
                    api_version=api_version,
                    template_url=self.list_applicable.metadata["url"],
                    headers=_headers,
                    params=_params,
                )
                request = _convert_request(request)
