...
### Breaking Changes
  ...
  - Renamed enum `IsDeviceMatchConditionParametersMatchValuesItem` to `IsDeviceMatchValue`
  ...
  - Renamed enum `KeyVaultSigningKeyParametersTypeName` to `KeyVaultSigningKeyParametersType`
  - Renamed enum `RequestMethodMatchConditionParametersMatchValuesItem` to `RequestMethodMatchValue`
  - Renamed enum `RequestSchemeMatchConditionParametersMatchValuesItem` to `RequestSchemeMatchValue`
  ...
  - Deleted or renamed client mixin `CdnManagementClientOperationsMixin`
...
### Other Changes
  ...
  - Deleted model `AfdErrorResponse`/`ValidationToken` which actually were not used by SDK users
