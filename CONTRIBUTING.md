...
Live tests within the SDK typically require deployment of Azure resources in order to work. Additionally, your machine must be set up to use the test proxy service whether you run tests live or in playback mode. See [Azure SDK Tools Test Proxy](https://github.com/Azure/azure-sdk-tools/blob/main/tools/test-proxy/Azure.Sdk.Tools.TestProxy/README.md)

To prepare your resources to run tests live, you must first run (from the root of the repo):
```
\eng\common\TestResources\New-TestResources.cmd search
```

There is also a `.ps1` script for Powershell users. When this script completes, it will output environment variables you must set. Copy the output of the script and paste it into your terminal. Additionally, for running Python tests in VSCode, you will need to copy the key-value pairs into a `.env` file that lives at the location of your SDK's `setup.py` file. It is easier to run the `.cmd` script as the output is closer to the format needed by the `.env` file.

Finally, you need to set the following environment variables (in your terminal and/or `.env` file):
```
