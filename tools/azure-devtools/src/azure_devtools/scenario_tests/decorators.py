...
def live_only():
    return unittest.skipUnless(
        os.environ.get(ENV_LIVE_TEST, False), "This is a live only test. A live test will bypass all vcrpy components."
    )
...
class AllowLargeResponse(object):  # pylint: disable=too-few-public-methods
    ...
    def __call__(self, fn):
        def _preparer_wrapper(test_class_instance, **kwargs):
            ...
            large_resp_body = next(
                (r for r in test_class_instance.recording_processors if isinstance(r, LargeResponseBodyProcessor)), None
            )
            if large_resp_body:
